/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216643
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216643 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216643
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min((((((var_5 ? var_10 : var_2))) ? (((var_3 ? var_3 : var_1))) : (min(var_10, var_9)))), (((min(247, 4211678789))))));

    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] [4] &= ((~((((min(var_11, var_6))) ? var_5 : (arr_1 [8])))));
        var_14 -= var_11;
        var_15 ^= (~(max((arr_1 [2]), var_4)));
        var_16 = ((((((((13338650175343219083 ? 7 : -32))) ? var_0 : (arr_1 [i_0])))) ? (arr_1 [i_0]) : var_2));
    }
    #pragma endscop
}
