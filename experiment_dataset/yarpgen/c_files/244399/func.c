/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244399
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244399 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244399
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */long long int) (-(((/* implicit */int) var_3))));
    var_13 ^= ((/* implicit */unsigned short) var_11);
    var_14 = ((/* implicit */unsigned char) (~(((/* implicit */int) ((((/* implicit */int) var_11)) != (((/* implicit */int) ((var_8) != (var_8)))))))));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 2; i_0 < 16; i_0 += 3) 
    {
        arr_3 [i_0] = (!(((/* implicit */_Bool) ((unsigned long long int) arr_0 [i_0 + 1] [i_0 - 1]))));
        arr_4 [i_0 - 1] = ((/* implicit */long long int) ((_Bool) ((((/* implicit */_Bool) ((((/* implicit */_Bool) arr_2 [i_0])) ? (((/* implicit */int) arr_1 [i_0])) : (((/* implicit */int) arr_2 [(_Bool)1]))))) ? (((/* implicit */int) (!(((/* implicit */_Bool) arr_2 [4ULL]))))) : (((/* implicit */int) var_2)))));
    }
}
