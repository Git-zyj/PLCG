/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88746
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88746 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88746
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_20 = (max(var_20, (arr_4 [i_2] [i_1])));
                    var_21 = (max(var_21, (arr_2 [i_0] [i_1])));
                    var_22 = ((((var_12 ? (min(var_1, var_11)) : (arr_7 [i_0] [i_1] [i_1]))) / (((((var_15 ? (arr_2 [i_0] [i_0]) : var_18))) * (arr_6 [i_0 + 1] [i_0 - 1])))));
                    var_23 = (max(var_23, (arr_6 [i_1] [i_0])));
                }
            }
        }
    }
    var_24 = var_13;
    var_25 = ((var_12 >= (min(((max(var_16, var_4))), (var_19 & var_12)))));
    #pragma endscop
}
