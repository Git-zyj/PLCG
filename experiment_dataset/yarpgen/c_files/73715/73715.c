/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73715
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        arr_2 [i_0] = (max(var_2, ((-(!7863604225864915081)))));
        var_11 = (max(var_11, ((((arr_1 [i_0]) ? (((var_6 + 2147483647) << (((arr_1 [i_0]) - 185)))) : (((arr_1 [i_0]) ? (arr_1 [i_0]) : var_5)))))));
    }
    var_12 = (max((min(var_1, (var_7 * var_8))), ((var_0 ? (var_7 / var_3) : var_3))));
    #pragma endscop
}
