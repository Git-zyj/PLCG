/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237195
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237195 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237195
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_4;

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (((((((0 ? 12645677886582232876 : 606500984542141100))) && ((min((arr_0 [i_0] [i_0]), (arr_1 [i_0] [i_0])))))) ? (arr_1 [i_0] [i_0]) : (((-(((arr_1 [i_0] [i_0]) ? var_9 : (arr_2 [i_0]))))))));
        arr_4 [i_0] = (var_8 != (arr_1 [i_0] [i_0]));
        arr_5 [i_0] [i_0] = ((1 == ((1 ? 65525 : var_11))));
    }
    #pragma endscop
}
