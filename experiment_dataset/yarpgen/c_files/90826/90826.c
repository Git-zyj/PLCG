/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90826
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90826 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90826
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (var_14 && var_15);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_17 = ((((((((51452 ? 1048064 : 794211983))) ? 90 : (~-6699)))) ? 1181440598 : 4294967295));
                var_18 = (max(((((arr_2 [i_1] [i_1]) * ((794211983 & (arr_2 [i_0 - 1] [i_1])))))), (max((arr_3 [i_0] [i_1]), (arr_2 [i_0] [17])))));
            }
        }
    }
    var_19 = (max(var_19, (((-(((min(var_10, var_1)))))))));
    #pragma endscop
}
