/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103822
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned short) var_3);
    var_19 = ((/* implicit */long long int) (signed char)-7);
    var_20 |= ((/* implicit */long long int) var_17);
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 13; i_0 += 1) 
    {
        for (short i_1 = 0; i_1 < 15; i_1 += 1) 
        {
            {
                /* LoopSeq 1 */
                for (unsigned int i_2 = 4; i_2 < 13; i_2 += 2) 
                {
                    var_21 += ((/* implicit */signed char) ((((/* implicit */_Bool) ((((/* implicit */unsigned int) ((/* implicit */int) var_11))) & (arr_1 [i_0 + 2])))) ? (((/* implicit */unsigned int) ((/* implicit */int) ((((/* implicit */int) (short)20001)) == (((/* implicit */int) arr_2 [i_0 - 1] [i_2 - 2] [i_2 - 1])))))) : (min((arr_1 [i_0 + 2]), (((/* implicit */unsigned int) var_8))))));
                    arr_8 [i_2] = ((/* implicit */short) min((10626314526105002035ULL), (((/* implicit */unsigned long long int) var_10))));
                }
                var_22 = var_17;
            }
        } 
    } 
}
