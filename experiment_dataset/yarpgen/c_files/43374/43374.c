/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43374
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43374 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43374
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (var_7 * 60);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_12 = ((~(arr_4 [i_0])));
                arr_6 [i_1] [i_0] [i_0] = ((579052824 << (((((arr_1 [i_0] [i_0]) ^ (arr_1 [i_0] [i_1])))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            arr_12 [i_0] [i_0] = (arr_5 [i_0] [i_0] [i_0]);
                            arr_13 [i_0] [i_0] [1] = (arr_1 [i_0] [2]);
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 13;i_4 += 1)
                {
                    for (int i_5 = 2; i_5 < 11;i_5 += 1)
                    {
                        {
                            var_13 = (max(var_13, (arr_4 [i_1])));
                            var_14 *= (arr_7 [i_5] [i_5] [i_5 + 1]);
                            arr_19 [i_0] [i_0] [i_0] [i_0] = (arr_7 [i_0] [i_0] [i_5]);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
