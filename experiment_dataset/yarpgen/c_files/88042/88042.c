/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88042
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88042 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88042
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max(var_18, 51));

    /* vectorizable */
    for (int i_0 = 3; i_0 < 7;i_0 += 1)
    {
        arr_3 [i_0] = (arr_0 [i_0 + 3]);
        var_19 = (((var_1 ? var_12 : (arr_0 [i_0]))));
    }
    for (int i_1 = 4; i_1 < 12;i_1 += 1)
    {
        var_20 = (max(var_20, (((-((max(var_9, 69))))))));
        arr_6 [i_1] [i_1] = (((arr_4 [i_1] [i_1]) ? ((((arr_5 [i_1 - 4]) ? (arr_4 [i_1] [i_1]) : (arr_4 [i_1] [i_1])))) : (((arr_4 [i_1] [i_1]) ? 1 : (arr_5 [i_1 - 4])))));
        arr_7 [i_1] = (arr_5 [i_1]);
    }
    var_21 = (((var_9 + 98) ? var_15 : var_8));
    #pragma endscop
}
