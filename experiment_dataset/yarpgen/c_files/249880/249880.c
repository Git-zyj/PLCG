/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249880
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249880 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249880
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = (min(var_13, (!var_0)));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_14 = (((arr_5 [i_0 + 1] [i_0 + 1] [i_0 + 2]) ? ((((!(arr_7 [i_3] [i_2] [i_1] [i_0 + 3]))))) : ((-(arr_1 [i_0 + 2])))));
                            arr_11 [i_0 + 2] [i_0 + 2] [i_2] [i_3] = ((-(arr_7 [i_0 - 2] [i_0 + 1] [i_0 + 3] [i_0 - 2])));
                            arr_12 [i_3] [i_1] [i_0] |= (arr_5 [i_0] [i_0] [i_1]);
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 10;i_4 += 1)
                {
                    for (int i_5 = 1; i_5 < 9;i_5 += 1)
                    {
                        {
                            var_15 = var_9;
                            var_16 = (min(var_16, var_0));
                            arr_17 [i_0] [i_0] [1] = (((((-(arr_6 [i_0 + 1] [i_0 + 1] [i_0 + 1] [i_5 - 1])))) ? var_1 : (arr_3 [i_5 + 1] [i_0 + 1] [i_4])));
                        }
                    }
                }
            }
        }
    }
    var_17 &= var_8;
    var_18 += var_0;
    var_19 = var_1;
    #pragma endscop
}
