/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184378
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184378 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184378
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((~(!var_12))) % var_9);
    var_18 = 32654;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((~var_1) && var_2);
                arr_7 [i_0] [16] = 32864;
                arr_8 [i_0] = ((((((arr_3 [i_0] [i_1 - 1] [i_0]) ? (arr_2 [i_0]) : -868234666))) ? var_2 : (((-(arr_3 [i_0] [i_0] [i_0]))))));
            }
        }
    }
    var_19 = var_14;

    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        /* LoopNest 2 */
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 12;i_4 += 1)
            {
                {
                    arr_18 [i_2] [i_3] [i_4] [i_4] = var_15;
                    arr_19 [i_4] [i_3] [i_4] &= (((arr_3 [i_4] [i_3] [i_3]) & (arr_3 [i_4] [i_4] [i_4])));
                }
            }
        }
        arr_20 [i_2] [i_2] = (!-var_8);
        arr_21 [i_2] = (((arr_17 [i_2] [i_2]) > (arr_13 [i_2])));
    }
    #pragma endscop
}
