/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93061
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93061 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93061
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = max(var_11, (0 ^ 17));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_7 [i_1] [i_1] [i_2] = ((((min((var_9 || 1), ((var_7 ? var_2 : var_7))))) ? ((((min(var_2, var_6))) ? var_4 : (max(var_12, 18446744073709551592)))) : (((1 ? var_9 : 12582912)))));
                    arr_8 [i_1] [i_1] = var_4;
                    var_15 = (((!var_10) ? var_10 : (max(var_7, var_6))));
                }
            }
        }
    }
    #pragma endscop
}
