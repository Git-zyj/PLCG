/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101568
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101568 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101568
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, ((min((max(var_0, var_10)), (max(var_4, -2237070176)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_1] = (max((arr_0 [i_1]), (!var_8)));
                var_13 = ((((((((((arr_0 [i_0]) ? (arr_0 [i_1]) : (arr_3 [i_0] [i_1] [i_1])))) || 1)))) & ((min(var_0, (arr_0 [i_1]))))));
                var_14 = var_5;
            }
        }
    }
    #pragma endscop
}
