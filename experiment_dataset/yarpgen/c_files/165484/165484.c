/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = 42996;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 13;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_1] [15] [i_0] = (((((arr_3 [i_0] [i_1 + 1]) / var_11))) ? (((arr_3 [i_0] [i_2]) * (arr_3 [i_0] [i_0]))) : ((((arr_3 [i_0] [i_2 + 1]) ? 22539 : var_10))));
                    arr_7 [i_0] [i_1] = (((~22539) ? ((min((arr_0 [i_1 - 2]), (arr_0 [i_1 - 2])))) : ((max((arr_0 [i_1 - 1]), (arr_0 [i_1 - 2]))))));
                }
            }
        }
    }
    #pragma endscop
}
