/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219160
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219160 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219160
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max((((var_15 | var_4) ? var_13 : (min(-7809050948197168059, var_12)))), (((var_13 - (var_15 ^ var_12))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                arr_6 [16] = ((((((arr_3 [i_1 - 2] [i_1 + 3]) & (arr_2 [i_1])))) || (arr_0 [i_1])));
                arr_7 [i_0 + 1] = (arr_2 [i_0 - 2]);
            }
        }
    }
    var_19 = ((var_5 % ((max((max(var_7, 50291)), (max(var_13, var_12)))))));
    #pragma endscop
}
