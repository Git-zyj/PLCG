/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237720
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237720 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237720
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = var_0;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_0 - 1] = (var_7 / 14743);
                    var_12 += ((!(((var_6 - (arr_1 [i_2] [17]))))));
                    var_13 = (min(var_13, var_1));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_14 = (arr_4 [i_0 - 1]);
                                var_15 = (max(var_15, ((((arr_11 [i_0 - 1] [5] [1] [i_0 - 1] [i_0]) ? (arr_3 [i_0 - 1] [i_0 - 1]) : var_9)))));
                            }
                        }
                    }
                }
            }
        }
        var_16 = (min(var_16, (arr_4 [i_0 - 1])));
    }

    /* vectorizable */
    for (int i_5 = 0; i_5 < 13;i_5 += 1)
    {
        arr_17 [i_5] = ((-60 ? 3692194956 : -14743));
        var_17 = (((arr_16 [i_5]) ? (arr_16 [i_5]) : (arr_16 [i_5])));
        var_18 = var_10;
        arr_18 [i_5] = ((0 + (arr_11 [i_5] [i_5] [i_5] [i_5] [i_5])));
    }
    var_19 = (min(var_19, var_7));
    #pragma endscop
}
