/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173175
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173175 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173175
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [1] [i_1] |= (max((((+((((-127 - 1) >= var_3)))))), ((1 ? (((1 ? 1 : 1))) : ((137 ? 135178865 : 131071))))));
                var_20 *= ((((((min(var_4, var_14))) && ((max(var_18, (arr_2 [16] [i_1])))))) ? ((~(min((arr_4 [i_0] [10]), (arr_2 [6] [i_1]))))) : (((1 ? 1 : -97)))));
            }
        }
    }
    var_21 -= ((((((-1983118388 ? 1 : 1)))) && ((((((var_4 ? 255 : 127))) ? ((255 ? 18446744073709551615 : 1)) : (var_10 && 1))))));
    #pragma endscop
}
