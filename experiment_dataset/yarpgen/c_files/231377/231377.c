/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231377
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231377 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231377
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_20 = (min(var_20, ((((((~((min(var_18, var_2)))))) ? (max(((var_3 ? (arr_2 [i_0] [i_0] [i_0]) : (arr_3 [i_0]))), var_14)) : (arr_0 [i_1]))))));
                var_21 = (-((max((arr_0 [i_0 - 1]), (!var_2)))));
            }
        }
    }
    var_22 ^= var_16;
    var_23 ^= (max(var_2, ((((min(var_12, 1))) ? var_19 : (~var_12)))));
    #pragma endscop
}
