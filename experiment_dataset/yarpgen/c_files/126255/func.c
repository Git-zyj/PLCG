/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126255
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126255 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126255
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) ((var_12) >= (var_12)));
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 21; i_1 += 2) 
        {
            {
                var_21 = ((((/* implicit */_Bool) ((arr_3 [i_1] [i_1] [i_1]) >> (((-4261505484630346867LL) + (4261505484630346886LL)))))) ? (var_9) : (((long long int) arr_3 [i_0] [i_0] [i_1])));
                var_22 = ((/* implicit */short) min((var_22), (((/* implicit */short) var_4))));
                /* LoopSeq 1 */
                /* vectorizable */
                for (long long int i_2 = 0; i_2 < 21; i_2 += 4) 
                {
                    var_23 = ((/* implicit */long long int) var_15);
                    var_24 = ((/* implicit */short) ((((/* implicit */unsigned long long int) ((/* implicit */int) arr_1 [i_0]))) & (var_12)));
                }
                arr_9 [i_1] [i_1] [i_0] = ((/* implicit */unsigned char) var_1);
            }
        } 
    } 
    var_25 = ((/* implicit */short) var_5);
    var_26 *= ((/* implicit */int) (_Bool)1);
}
