/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152018
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            {
                var_14 = ((/* implicit */unsigned short) max((((/* implicit */int) max(((unsigned short)64376), ((unsigned short)32767)))), (((int) (unsigned char)215))));
                var_15 = ((/* implicit */long long int) (+(4294967292U)));
                var_16 = ((/* implicit */unsigned long long int) max((var_16), (((/* implicit */unsigned long long int) (+((+(1013701152))))))));
            }
        } 
    } 
    var_17 = ((/* implicit */unsigned int) var_9);
}
