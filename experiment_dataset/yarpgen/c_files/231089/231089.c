/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231089
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231089 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231089
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((var_4 <= (!var_19))) ? ((max(var_8, var_16))) : -4888273306750977830);
    var_21 = var_17;
    var_22 = (max(var_22, var_3));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_23 = (min(var_23, (arr_1 [1])));
        arr_4 [i_0] [i_0] = ((((!(arr_2 [i_0]))) ? (max(173, (arr_3 [i_0]))) : (((~(arr_2 [i_0]))))));
        arr_5 [i_0] = ((var_6 % (((arr_2 [i_0]) ? (((min(var_19, (arr_1 [i_0]))))) : (min((arr_3 [i_0]), 1))))));
    }
    #pragma endscop
}
