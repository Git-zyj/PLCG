/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75321
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75321 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75321
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((var_2 == var_3) << (var_3 - 2605287012));
    var_21 &= ((var_13 >> (((1 < 247) ^ (var_5 ^ var_8)))));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_22 = (max(var_22, (arr_6 [i_0] [i_0] [i_0] [i_0])));
                    arr_9 [i_0 - 2] [i_0 - 2] [i_2] = (arr_4 [i_0 - 4] [i_1 - 2] [8]);
                    var_23 = ((((((var_8 < var_10) && (var_1 / var_7)))) >> (var_15 - 2613117775890221867)));
                }
            }
        }
    }
    var_24 = (max(var_24, var_0));
    #pragma endscop
}
