/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81796
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81796 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81796
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        var_11 = (max(var_11, ((+(min((arr_1 [i_0]), (arr_2 [i_0] [i_0])))))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 16;i_3 += 1)
                {
                    {
                        var_12 = ((21 ? ((((arr_5 [i_0 - 1]) ? 1 : (arr_3 [i_0 + 2])))) : (arr_5 [i_0 + 3])));
                        var_13 = (arr_5 [i_0]);
                        var_14 = (max(var_14, ((max(151, (max((arr_8 [i_3 + 1] [i_3 - 1] [i_0 + 1] [i_0 + 1]), 1508002461)))))));
                    }
                }
            }
        }
    }
    var_15 = (min(var_15, var_10));
    #pragma endscop
}
