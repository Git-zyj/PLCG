/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198279
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198279 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198279
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 20; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */unsigned long long int) max((var_18), (((unsigned long long int) max((max((8147486830498102338ULL), (18446744073709551588ULL))), (((/* implicit */unsigned long long int) (-(((/* implicit */int) arr_1 [i_1]))))))))));
                var_19 = var_0;
                var_20 = arr_2 [i_0];
                arr_5 [i_1 - 1] [i_1] [i_1] = ((/* implicit */unsigned char) max((10299257243211449277ULL), (max((((unsigned long long int) arr_0 [i_0])), (((/* implicit */unsigned long long int) arr_4 [i_0] [i_1 + 1]))))));
            }
        } 
    } 
    var_21 = ((/* implicit */_Bool) var_0);
    var_22 = ((/* implicit */_Bool) var_11);
    var_23 += ((/* implicit */unsigned long long int) var_7);
}
