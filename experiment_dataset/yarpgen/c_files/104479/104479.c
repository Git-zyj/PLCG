/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104479
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104479 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104479
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_17 = (min(((((0 ? (arr_0 [i_0]) : 0)) & (((arr_1 [i_0] [i_0]) ? 1 : (arr_0 [i_0]))))), (((((arr_1 [i_0] [i_0]) * 1)) - ((~(arr_0 [i_0])))))));
        arr_2 [i_0] = (((arr_0 [i_0]) ? (arr_0 [i_0]) : ((1 ? (arr_0 [i_0]) : (arr_0 [i_0])))));
        var_18 = var_7;
    }
    var_19 = (((((min(var_11, var_1)) ? ((max(1, 1))) : var_15)) + ((1 ? (var_1 - 1) : ((var_8 ? 1 : 1))))));
    var_20 = 1;
    #pragma endscop
}
