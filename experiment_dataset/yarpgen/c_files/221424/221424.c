/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221424
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221424 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221424
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((((var_14 + var_3) / var_13))) && ((min(((101 ? 77 : var_5)), (var_3 + var_9))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [1] = ((-4 || (((var_3 | ((1078013227143288500 ? -1078013227143288496 : var_7)))))));
                    arr_8 [i_2] [i_1] [3] = ((-(((max(-1078013227143288509, var_2)) ^ (~-1078013227143288505)))));
                }
            }
        }
    }
    var_16 = ((var_10 ^ (!-var_1)));
    #pragma endscop
}
