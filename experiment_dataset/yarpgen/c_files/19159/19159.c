/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19159
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19159 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19159
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((((min(2339721019, var_3)) << (var_2 != var_2)))) ? ((~(var_14 + var_12))) : ((((min(var_6, var_15))))));
    var_18 &= (((var_14 * (var_9 * var_11))) - (((((var_4 ? var_12 : var_7))) ? (1955246278 ^ var_5) : (~var_13))));

    for (int i_0 = 1; i_0 < 11;i_0 += 1) /* same iter space */
    {
        var_19 = (max(var_19, var_6));
        var_20 = max(((1089974555 ? var_11 : var_10)), var_10);
        arr_2 [i_0 + 2] = var_16;
        var_21 = var_0;
    }
    for (int i_1 = 1; i_1 < 11;i_1 += 1) /* same iter space */
    {
        var_22 = (var_8 ? (arr_0 [i_1] [i_1]) : (((~(arr_1 [i_1 + 2])))));
        arr_5 [i_1] [i_1] = (var_1 > var_11);
        arr_6 [i_1] [i_1 + 3] = ((var_2 != (((var_6 <= (arr_1 [i_1 - 1]))))));
        arr_7 [i_1] [i_1] = ((((min(2727904255, (arr_4 [i_1])))) || -var_14));
        arr_8 [i_1 + 1] [i_1] = arr_4 [i_1];
    }
    var_23 = var_6;
    var_24 = var_12;
    #pragma endscop
}
