/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65573
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65573 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65573
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned short) min((var_19), (((/* implicit */unsigned short) (+(min((((((/* implicit */_Bool) (short)-28914)) ? (var_4) : (((/* implicit */int) var_7)))), ((+(((/* implicit */int) var_11)))))))))));
    var_20 = ((/* implicit */int) max((10471887342339692368ULL), (((/* implicit */unsigned long long int) (+(var_3))))));
    /* LoopNest 2 */
    for (unsigned short i_0 = 2; i_0 < 11; i_0 += 1) 
    {
        for (unsigned int i_1 = 2; i_1 < 11; i_1 += 3) 
        {
            {
                var_21 = ((/* implicit */unsigned long long int) var_9);
                var_22 = ((/* implicit */_Bool) (-(max((max((var_3), (((/* implicit */unsigned int) 15)))), (((/* implicit */unsigned int) (!(((/* implicit */_Bool) var_0)))))))));
            }
        } 
    } 
}
