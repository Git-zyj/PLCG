/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200849
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200849 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200849
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0 + 1] [8] |= (max((((((((arr_1 [i_1 - 1] [i_1 - 1]) >= var_5)))) & 3371693499)), var_12));
                    arr_7 [i_2] [i_1] [i_2] [i_2] = (((((-(arr_1 [i_0 + 2] [i_1 + 1])))) ? var_2 : 51));
                    arr_8 [i_0] [i_2] [1] [i_0] = 1;
                }
            }
        }
    }
    var_14 = (!-9223372036854775796);
    #pragma endscop
}
