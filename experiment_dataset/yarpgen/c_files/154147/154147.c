/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154147
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154147 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154147
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((-6553 ? 24426 : -1240292558));
    var_13 = ((min((min(var_7, var_7)), var_3)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = ((var_4 ? var_3 : 1498512152));
                var_14 = ((~(arr_3 [i_1 + 1] [1])));
                var_15 += (max((min((arr_2 [i_1] [i_1 - 1] [i_1 - 1]), (arr_0 [i_0 - 1]))), ((~(arr_2 [i_1] [i_1 - 2] [i_1 - 3])))));
            }
        }
    }
    #pragma endscop
}
