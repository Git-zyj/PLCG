/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (((((((-20288 ? -7828861786962435231 : 20288))) || (arr_5 [i_0]))) ? (((((max((arr_2 [i_0] [i_1]), var_4))) == -var_7))) : (((((arr_4 [i_0] [i_1] [i_0]) << (((arr_3 [i_0] [20] [i_1 + 1]) + 12064)))) >> (((!(arr_2 [i_0] [i_0]))))))));
                arr_7 [i_0] [i_1] [i_0] = var_6;
                arr_8 [i_0] = min((arr_1 [i_0]), (-30170 >= -20274));
            }
        }
    }
    var_20 = var_7;
    #pragma endscop
}
