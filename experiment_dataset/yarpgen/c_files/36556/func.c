/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36556
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36556 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36556
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 16; i_1 += 2) 
        {
            for (unsigned short i_2 = 1; i_2 < 13; i_2 += 2) 
            {
                {
                    var_19 = ((/* implicit */unsigned char) (((-(((/* implicit */int) ((unsigned short) arr_0 [i_0]))))) % ((-(((/* implicit */int) (unsigned short)49749))))));
                    var_20 = ((/* implicit */signed char) (unsigned short)515);
                }
            } 
        } 
    } 
    var_21 = ((/* implicit */short) ((652235083154795847LL) % (((/* implicit */long long int) ((((/* implicit */int) (signed char)-100)) + (((/* implicit */int) var_12)))))));
    var_22 = ((/* implicit */unsigned short) var_14);
}
