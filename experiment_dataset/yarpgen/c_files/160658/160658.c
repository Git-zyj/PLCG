/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160658
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160658 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160658
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((((min(25836, 25853)) ? (((var_9 + 9223372036854775807) << (((var_13 + 25096) - 39)))) : (~var_16)))));
    var_20 = ((25829 << (var_3 - 3949893561)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = (((((76 | (arr_0 [i_1])))) ? (~-25836) : (arr_0 [i_0])));
                arr_5 [i_0] |= (max(1, 25807));
                var_21 = (((arr_0 [i_1]) != ((((arr_0 [i_0]) < (var_3 & var_1))))));
            }
        }
    }
    #pragma endscop
}
