/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142745
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142745 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142745
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [i_2] = -var_11;
                    arr_9 [i_0] [i_0] [i_2] [6] = (((arr_5 [i_2 - 3] [i_2 - 3] [1] [1]) ? ((((-(arr_6 [i_0] [i_1] [i_2]))))) : ((37 ? 65520 : 1125899906842624))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [1] [i_2] [i_0] [i_0] [1] = (!-874288981);
                                var_15 ^= (-1 * 1125899906842645);
                                var_16 = (((!(arr_4 [i_1] [i_2 - 1]))));
                                var_17 -= var_12;
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = ((((~1125899906842608) ? var_3 : var_5)));
    #pragma endscop
}
