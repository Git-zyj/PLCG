/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112388
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned long long int) var_11);
    /* LoopNest 2 */
    for (unsigned short i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        for (short i_1 = 0; i_1 < 19; i_1 += 4) 
        {
            {
                arr_4 [13LL] = ((/* implicit */short) (~(((/* implicit */int) var_14))));
                arr_5 [(unsigned char)2] [i_1] &= ((((/* implicit */_Bool) ((min((16514008045249670861ULL), (((/* implicit */unsigned long long int) arr_3 [i_0])))) * (((/* implicit */unsigned long long int) ((/* implicit */int) arr_0 [(short)1])))))) ? (arr_2 [16] [i_0 - 1] [i_0]) : (((/* implicit */unsigned long long int) (+(((/* implicit */int) (signed char)-85))))));
            }
        } 
    } 
}
