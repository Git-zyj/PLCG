/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187832
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187832 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187832
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */short) (+(((/* implicit */int) var_6))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        for (unsigned int i_1 = 3; i_1 < 9; i_1 += 4) 
        {
            {
                arr_5 [i_1] = ((/* implicit */_Bool) ((signed char) (+(((((/* implicit */_Bool) (unsigned char)253)) ? (((/* implicit */int) var_7)) : (((/* implicit */int) arr_3 [i_1])))))));
                var_18 *= ((/* implicit */short) (~(arr_2 [i_0])));
            }
        } 
    } 
    var_19 = ((/* implicit */signed char) max((var_19), (((/* implicit */signed char) ((((/* implicit */int) (short)(-32767 - 1))) != (((((/* implicit */_Bool) (~(((/* implicit */int) var_1))))) ? (((/* implicit */int) var_8)) : (((/* implicit */int) var_5)))))))));
}
