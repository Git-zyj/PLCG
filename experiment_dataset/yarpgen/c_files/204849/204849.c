/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204849
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204849 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204849
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, 112));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_11 = (max((((((arr_3 [i_1]) ? 1 : (arr_6 [i_0] [i_0] [i_2]))) * (((21 << (-3575595067408383325 + 3575595067408383338)))))), var_7));
                    var_12 &= (~var_4);
                    arr_8 [1] [4] [i_2] [i_1] = var_5;
                }
            }
        }
    }
    var_13 = (max(var_13, -97));
    var_14 &= ((+(((var_4 & var_5) + -3575595067408383333))));
    #pragma endscop
}
