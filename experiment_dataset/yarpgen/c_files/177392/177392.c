/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177392
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_19 = var_0;
                arr_4 [i_1] [1] [i_0] = ((~(((arr_1 [i_1]) & 1))));
                var_20 = arr_3 [i_0] [i_0] [i_0];
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 15;i_3 += 1)
        {
            {
                var_21 = (min(var_21, 32158870903222238));
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 12;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 15;i_5 += 1)
                    {
                        {
                            var_22 = -27;
                            arr_15 [i_2] [i_2] [8] [i_2] [i_2] &= (((((min((arr_13 [i_2] [i_3] [0] [2] [i_5]), -112)))) ? (arr_8 [i_2] [i_2] [i_4 + 3]) : (min(((min((arr_2 [i_3]), 26))), ((0 ? -1060646721291303872 : var_18))))));
                            arr_16 [i_5] = (min((arr_12 [i_4 + 3] [i_4 - 1] [i_4 + 2]), var_2));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
