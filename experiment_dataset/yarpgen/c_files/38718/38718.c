/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38718
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38718 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38718
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((2741942854 < (var_10 / var_11)));
    var_15 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_5 [24] [i_0] [i_0] = ((-59 ? (max((max((arr_4 [i_0] [i_0] [i_0]), -122)), ((min((arr_3 [i_1] [i_1] [i_0]), 1258894879))))) : ((max(-var_4, (arr_0 [18]))))));
                arr_6 [i_0] = (min((arr_0 [i_0]), 1553024440));
                arr_7 [i_0] [i_0] = var_7;
                arr_8 [i_0 + 2] [i_0] = (max(1553024440, 22801));
            }
        }
    }
    #pragma endscop
}
