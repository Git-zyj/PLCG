/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190177
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_7;
    var_15 = (((var_12 ? var_7 : var_10)));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    {
                        var_16 = max((((((-(arr_8 [i_0] [i_1] [i_0])))) ? (((-(arr_10 [i_0] [i_0] [i_3])))) : (arr_8 [i_0] [i_1] [i_2]))), ((((arr_1 [i_0] [i_0]) / (arr_3 [i_0] [i_0] [i_3])))));
                        arr_11 [i_0] [i_0] [i_2] = (arr_10 [i_0] [i_0] [i_0]);
                    }
                }
            }
        }
        var_17 = (!var_4);
    }
    #pragma endscop
}
