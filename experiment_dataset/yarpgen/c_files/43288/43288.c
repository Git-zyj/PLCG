/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43288
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43288 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43288
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((max(var_7, 1329320162)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_11 = (max(var_11, (arr_4 [8])));
                arr_6 [12] = ((-((1329320158 ? var_1 : (arr_3 [i_0] [i_0])))));
                arr_7 [i_1] |= 1329320133;
                var_12 = (min(((((arr_3 [i_1] [i_0]) ? (arr_3 [i_0] [i_1]) : var_3))), ((var_6 - (arr_3 [i_0] [i_0])))));
            }
        }
    }
    #pragma endscop
}
