/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148971
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148971 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148971
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        var_11 = ((/* implicit */int) -4172427946729447696LL);
        arr_3 [i_0] = ((/* implicit */unsigned short) ((unsigned long long int) -3013379765693631370LL));
    }
    var_12 -= min(((+(((/* implicit */int) var_1)))), (((int) (unsigned char)219)));
}
