/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182091
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182091 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182091
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_16;
    var_18 = (max(var_18, var_5));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_19 = (max(var_19, (((((max((arr_3 [i_0] [i_1] [4]), (((var_14 ? var_6 : var_10)))))) ? ((((((arr_4 [11]) ? 2260 : var_12))))) : var_2)))));
                    arr_8 [i_0] = ((var_9 + (((arr_5 [i_1] [i_0]) ? var_8 : ((min((arr_2 [i_2]), var_14)))))));
                }
            }
        }
    }
    var_20 += ((~((var_6 ? var_8 : ((2248 ? var_6 : var_14))))));
    #pragma endscop
}
