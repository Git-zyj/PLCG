/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51879
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 4; i_0 < 14; i_0 += 2) 
    {
        var_11 = ((/* implicit */int) var_0);
        var_12 = ((/* implicit */unsigned short) ((arr_1 [i_0 + 2]) > (((/* implicit */unsigned int) ((/* implicit */int) min((((/* implicit */short) arr_0 [i_0 + 1])), ((short)-23848)))))));
    }
    var_13 = ((/* implicit */short) max((var_9), ((+(7733503429294600117LL)))));
}
