/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (-((((134 ? var_8 : var_3)) >> ((((max(var_3, var_9))) - 27806)))));
    var_15 = (((~31) && var_1));
    var_16 = (min(var_16, (((((~var_9) ? (var_0 / var_6) : (((min(var_9, 58))))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_17 = (arr_1 [i_0]);
        var_18 = (((((arr_0 [i_0] [i_0]) || (arr_0 [i_0] [i_0]))) ? ((~(arr_0 [i_0] [i_0]))) : (arr_0 [i_0] [i_0])));
        arr_2 [i_0] = (((arr_1 [i_0]) - (arr_0 [i_0] [i_0])));
    }
    #pragma endscop
}
