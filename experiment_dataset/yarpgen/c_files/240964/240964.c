/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240964
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240964 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240964
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_13 ? ((var_2 ? ((var_1 ? -21856 : 43545)) : var_2)) : (min((~var_7), (-21856 * -1)))));

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_15 = (min(var_15, (((-21856 ? 1 : 63208959)))));
        var_16 = (arr_1 [i_0]);
    }
    var_17 = var_0;
    var_18 = (((((var_0 ? (!var_10) : var_3))) ? ((min(-15, var_9))) : ((((max(var_9, 1))) ? -1 : (!var_4)))));
    #pragma endscop
}
