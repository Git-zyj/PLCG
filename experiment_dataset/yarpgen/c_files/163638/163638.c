/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163638
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163638 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163638
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        var_17 = (arr_0 [i_0]);
        var_18 = ((-5905912329805299024 - (arr_1 [i_0 - 2])));
    }

    for (int i_1 = 3; i_1 < 21;i_1 += 1)
    {
        arr_7 [i_1] = (((arr_5 [i_1 - 1]) ? (arr_5 [i_1 - 1]) : (arr_5 [i_1 - 3])));
        var_19 = (-(max((((arr_4 [i_1]) * (arr_3 [i_1 - 1]))), (arr_5 [i_1 - 3]))));
        arr_8 [12] = arr_6 [i_1] [i_1];
        var_20 = (~(max((arr_3 [i_1 - 2]), (((arr_1 [i_1]) % (arr_1 [i_1]))))));
    }
    #pragma endscop
}
