/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107808
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107808 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107808
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_16 = (((~var_9) ? 0 : (((var_10 ? (arr_5 [i_0] [i_1]) : (~var_15))))));
                arr_6 [i_0] [i_1] [i_1] = (min(((~(max((arr_0 [i_0] [i_0]), (arr_2 [i_1]))))), (arr_5 [i_1] [i_0])));
            }
        }
    }
    var_17 = (((min(((var_6 >> (var_15 + 9058849062089729219))), (((var_9 ? var_14 : var_7))))) | (((((var_5 ? var_5 : var_2)))))));
    #pragma endscop
}
