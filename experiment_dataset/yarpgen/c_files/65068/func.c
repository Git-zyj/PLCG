/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65068
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65068 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65068
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) min((((/* implicit */unsigned int) (unsigned short)49785)), (max((var_0), (((/* implicit */unsigned int) (unsigned short)8373)))))))));
    /* LoopNest 3 */
    for (long long int i_0 = 1; i_0 < 11; i_0 += 1) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            for (long long int i_2 = 0; i_2 < 15; i_2 += 4) 
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15; i_3 += 4) 
                    {
                        for (unsigned short i_4 = 2; i_4 < 12; i_4 += 2) 
                        {
                            {
                                var_13 = ((/* implicit */unsigned long long int) var_7);
                                var_14 = ((/* implicit */_Bool) 9223372036854775779LL);
                                var_15 |= ((/* implicit */_Bool) ((int) var_0));
                                var_16 += ((/* implicit */unsigned int) ((long long int) arr_1 [i_0] [i_0]));
                            }
                        } 
                    } 
                    var_17 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) min((((/* implicit */int) arr_2 [i_0] [i_1 - 1] [i_1 - 1])), (2147221504))))));
                }
            } 
        } 
    } 
}
