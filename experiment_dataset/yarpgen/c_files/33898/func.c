/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33898
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33898 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33898
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 |= ((/* implicit */unsigned int) var_4);
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 12; i_1 += 1) 
        {
            for (int i_2 = 0; i_2 < 13; i_2 += 1) 
            {
                {
                    var_16 = ((/* implicit */unsigned char) max((var_16), (((/* implicit */unsigned char) ((((/* implicit */unsigned long long int) ((/* implicit */int) ((unsigned char) ((unsigned long long int) arr_4 [(_Bool)0]))))) | (var_12))))));
                    var_17 = ((/* implicit */signed char) ((int) ((((/* implicit */_Bool) arr_3 [i_1 - 1] [i_1] [i_0])) || (((/* implicit */_Bool) 11834707463395188663ULL)))));
                }
            } 
        } 
    } 
}
