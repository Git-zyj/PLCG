/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242369
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242369 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242369
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (var_11 / (((max(var_6, var_3)))));
    var_15 = ((var_6 ? ((~((var_6 ? 0 : 4137092145401353912)))) : var_3));

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_16 += ((!((!((((arr_0 [i_0] [i_0]) ? var_5 : (arr_1 [2]))))))));
        arr_2 [i_0] = ((+((((((arr_1 [9]) ? var_8 : var_4)) >= -20)))));
    }
    var_17 = (((((1 ? var_7 : var_8) % -var_10))));
    #pragma endscop
}
