/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153477
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153477 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153477
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (((-(arr_3 [i_1 - 1] [i_1 - 1]))) == var_12);
                var_19 = (((arr_5 [i_0]) ? (((((var_17 ? var_6 : var_2))) ? (((arr_0 [0]) ? (arr_1 [i_1 - 1]) : (arr_0 [i_1]))) : (arr_1 [i_0]))) : 1));
            }
        }
    }
    var_20 = ((~((var_10 - (min(243, 1738734166542542638))))));
    #pragma endscop
}
