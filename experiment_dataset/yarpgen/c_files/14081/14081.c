/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14081
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14081 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14081
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 -= var_6;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [i_1] [i_1] |= var_8;
                    var_15 = (~var_0);
                    var_16 = (var_13 ? var_5 : ((var_4 ? (arr_3 [i_2 - 3] [i_0]) : (arr_5 [i_0] [i_0] [i_2]))));
                    arr_9 [19] [i_0] [17] [i_2 - 1] = ((~var_9) ? (((((var_3 ? (arr_3 [i_0] [i_0]) : var_8))) ? ((var_13 ? var_5 : var_12)) : (((arr_7 [i_0]) ? var_3 : var_4)))) : (arr_7 [i_0]));
                }
            }
        }
    }
    var_17 -= (~var_2);
    #pragma endscop
}
