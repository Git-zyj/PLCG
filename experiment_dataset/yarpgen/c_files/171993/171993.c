/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171993
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171993 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171993
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((var_7 | (((~var_4) & (((var_6 << (-2579728767364236809 + 2579728767364236817))))))));
    var_14 = var_1;

    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((-((((max((arr_0 [i_0]), 1))) ? 3470 : (arr_0 [i_0])))));
        var_15 = (((((((((!(arr_1 [i_0]))))) & ((~(arr_0 [i_0])))))) ? ((var_10 ? (arr_0 [i_0]) : (~125))) : (arr_1 [i_0])));
        arr_3 [i_0 + 1] = (arr_1 [i_0]);
        var_16 = 65526;
        arr_4 [i_0] = var_4;
    }
    #pragma endscop
}
