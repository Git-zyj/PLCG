/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77541
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77541 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77541
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_17;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1 - 1] [i_1 + 1] = min((((!(((4546 ? (arr_2 [i_0] [i_1] [i_1]) : 0)))))), (max(var_5, 2277139471)));
                var_20 = 32;

                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    var_21 = ((var_18 ? (arr_1 [i_2]) : var_10));
                    var_22 ^= (arr_1 [i_0]);
                }
                var_23 ^= ((269242084 && (((((4608 ? 4294967295 : var_17)) ^ (((max(var_17, (arr_7 [0]))))))))));
            }
        }
    }
    #pragma endscop
}
