/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62421
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62421 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62421
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 9; i_0 += 1) 
    {
        for (long long int i_1 = 2; i_1 < 12; i_1 += 2) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_14 &= ((/* implicit */int) ((_Bool) 10271668474340226758ULL));
                    arr_7 [i_0 + 2] [0ULL] [i_2] [4ULL] = ((/* implicit */int) ((((/* implicit */_Bool) -66669179)) ? (558446353793941504LL) : (558446353793941497LL)));
                    var_15 *= ((/* implicit */unsigned long long int) var_11);
                    arr_8 [i_1] &= ((/* implicit */unsigned long long int) (((~(arr_4 [i_0] [i_1 - 1] [i_1 + 1]))) >> (((((((/* implicit */int) (unsigned char)199)) << (((((/* implicit */int) var_3)) + (67))))) - (50917)))));
                }
            } 
        } 
    } 
    var_16 |= ((/* implicit */int) ((var_5) / (558446353793941517LL)));
}
