/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94077
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94077 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94077
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_14 = ((((((arr_6 [i_3]) < var_3))) << (((((((var_1 + 2147483647) << (((((((-2147483647 - 1) - -2147483640)) + 21)) - 13))))) && (~-6096))))));
                            var_15 = ((((((((arr_9 [i_0] [i_0 - 1]) < -6096)) ? 1 : (min(var_1, (arr_2 [i_0])))))) & var_11));
                            var_16 = 4611686018427387903;
                            var_17 = (arr_7 [i_0] [i_0] [i_0] [i_0]);
                        }
                    }
                }
                var_18 = (arr_8 [i_0] [i_1 - 1] [6] [i_1]);
                var_19 += (((((-9223372036854775807 ^ (((2132198342 | (arr_7 [i_1 + 2] [i_1] [i_0] [i_0]))))))) & (((((arr_8 [i_1] [i_1] [i_0 - 1] [i_0]) & var_9)) ^ (~var_6)))));
            }
        }
    }
    var_20 = var_2;
    var_21 = var_3;
    #pragma endscop
}
