/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11660
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min((((var_3 ? 1 : 155))), ((-23708 ? -1 : 3065646838))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_11 = (arr_4 [i_0] [i_2] [i_2]);
                                var_12 = min((!0), ((-852108681 ? -7 : 3244)));
                                var_13 = (max(var_13, ((((((-(arr_8 [i_0] [i_1])))) > ((27 + (arr_8 [i_0] [i_1]))))))));
                            }
                        }
                    }
                    arr_17 [i_2] = (arr_9 [i_0] [i_0] [i_0] [i_0] [i_0]);
                }
            }
        }
    }
    var_14 = ((!((((var_0 ? var_5 : -43))))));
    var_15 = (~var_6);
    var_16 = var_1;
    #pragma endscop
}
