/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162291
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162291 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162291
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_10 ^= (((arr_2 [i_0 - 1]) ? var_5 : (arr_2 [i_0 - 1])));
        var_11 = (((arr_1 [i_0 - 1] [i_0 - 1]) ? var_1 : (arr_0 [i_0] [i_0])));
        var_12 = -98;
    }
    for (int i_1 = 0; i_1 < 15;i_1 += 1)
    {
        var_13 = (min(((615519110 << (615519110 - 615519110))), (((var_3 < (!-750843191))))));
        arr_5 [9] = (arr_3 [i_1]);
        arr_6 [i_1] = ((~(((0 >= ((((!(arr_4 [i_1]))))))))));
    }
    var_14 += (min(32501, 44514));
    #pragma endscop
}
