/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144040
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144040 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144040
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_19 = (arr_0 [i_0]);
                    var_20 *= var_10;
                    arr_6 [21] [21] [i_2] = ((((var_16 ? 65393 : (arr_4 [i_2] [i_2] [i_1] [i_2]))) <= (max(((min((arr_1 [i_1] [i_1]), var_8))), ((var_0 ? 0 : (arr_0 [1])))))));
                }
            }
        }
    }
    var_21 &= ((55197 << (((max(-6667961915134828342, 11549))))));
    var_22 = ((((-((min(5110, 124)))))) ? 1 : 3683015615);
    #pragma endscop
}
