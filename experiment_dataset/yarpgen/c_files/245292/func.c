/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245292
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245292 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245292
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */unsigned long long int) var_10);
    var_20 = ((/* implicit */signed char) var_11);
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_21 = ((/* implicit */signed char) (!(((/* implicit */_Bool) (-(arr_1 [i_0]))))));
        var_22 = ((/* implicit */short) max((var_22), (((/* implicit */short) min((min(((!(((/* implicit */_Bool) (unsigned char)132)))), ((!(((/* implicit */_Bool) var_0)))))), (((var_16) >= (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned char)132))))))))));
    }
}
