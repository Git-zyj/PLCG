/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37001
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37001 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37001
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = -var_2;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_14 = var_0;
        var_15 = (((arr_0 [i_0] [i_0]) ? var_9 : var_10));
        arr_2 [i_0] = ((!(arr_0 [i_0] [i_0])));
        arr_3 [i_0] = 254;
        var_16 = var_9;
    }
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        var_17 = 65281;
        var_18 = (((~254) ? 15461150206080456999 : (arr_0 [i_1] [i_1])));
        var_19 = (max(var_19, (((((+(((arr_0 [i_1] [i_1]) ? (arr_1 [i_1]) : (arr_0 [i_1] [13]))))) == var_1)))));
        var_20 *= var_10;
    }
    var_21 = (min(var_21, ((max(var_6, var_9)))));
    var_22 = ((var_6 & (min(var_7, var_8))));
    #pragma endscop
}
