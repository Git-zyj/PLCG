/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = -var_7;
    var_12 = (max(var_12, var_1));
    var_13 = var_0;
    var_14 = 3490;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        var_15 = ((var_3 ? (((arr_1 [i_3]) ? (arr_0 [i_0]) : (arr_5 [i_1]))) : (arr_9 [i_0] [i_1] [i_2] [i_3] [i_1])));
                        var_16 &= (!(arr_7 [i_0] [i_1] [i_2]));

                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            var_17 = ((!((((var_8 + 2147483647) >> 0)))));
                            arr_13 [i_0] = (((arr_8 [i_3] [i_3] [i_2] [i_0]) ? (arr_8 [i_0] [i_1] [i_2] [i_3]) : (arr_8 [i_0] [i_1] [i_2] [i_4])));
                            arr_14 [i_1] [i_3] [i_4] = (arr_12 [i_0] [i_1] [i_0] [i_3] [i_4] [i_2]);
                            var_18 = var_0;
                        }
                    }
                    var_19 = ((~(arr_3 [i_1])));
                }
            }
        }
    }
    #pragma endscop
}
