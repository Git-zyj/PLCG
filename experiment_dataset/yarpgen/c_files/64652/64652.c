/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_20 ^= (((((-24 ? (((arr_7 [i_1] [i_0] [i_0]) * var_11)) : var_0))) ? (((var_18 ? var_12 : 417979459399539937))) : (arr_7 [i_0] [i_1] [i_1])));
                arr_8 [i_0] [i_0] [i_1] = ((+((-((((arr_6 [i_1] [i_0]) <= var_5)))))));
            }
        }
    }
    var_21 = ((min(-var_1, (min(var_10, 720049585)))));
    var_22 = (min(var_1, -var_17));
    #pragma endscop
}
