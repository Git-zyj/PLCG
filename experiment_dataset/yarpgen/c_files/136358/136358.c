/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136358
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136358 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136358
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((~(((!((max(45424, 20133))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((var_17 != (arr_1 [i_0])));
                arr_5 [i_0] [i_1] = (max(((67108848 - ((67108848 ? (arr_0 [i_1]) : (arr_0 [i_0]))))), ((((-6 >= (arr_3 [i_0] [i_1 + 2] [i_0]))) ? ((((arr_2 [i_0] [8] [i_1]) ? (arr_3 [i_0] [i_0] [i_0]) : 2630525320))) : (arr_0 [i_1 - 1])))));
            }
        }
    }
    #pragma endscop
}
