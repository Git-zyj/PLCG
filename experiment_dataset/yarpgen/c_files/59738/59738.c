/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59738
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59738 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59738
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (~var_12);
    var_14 = ((((max(-112, -93))) ? 92 : var_7));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_15 = ((var_3 ? ((((((!(arr_4 [i_0] [13] [i_2])))) << (var_3 - 112)))) : (arr_3 [i_1] [i_0])));
                    var_16 = (min((((arr_1 [i_2] [i_1]) ? var_6 : (arr_5 [i_1]))), (max((89 >= var_12), (max(11933980950355107150, (arr_0 [11])))))));
                }
            }
        }
    }
    #pragma endscop
}
