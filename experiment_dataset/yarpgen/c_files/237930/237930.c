/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237930
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237930 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237930
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = (((((var_17 ? ((max(255, 3731286786))) : (((arr_1 [i_0]) ? 2841690448624417750 : var_13))))) ? (((var_3 ? var_16 : var_14))) : var_1));
                var_20 = (!var_12);
                var_21 -= var_17;
                arr_6 [i_0] = ((27 >> (3731286786 - 3731286784)));
                var_22 = ((~((~((127 ? 16135156298263254110 : -117))))));
            }
        }
    }
    var_23 *= var_1;
    #pragma endscop
}
