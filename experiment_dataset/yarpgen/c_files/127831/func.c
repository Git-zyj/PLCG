/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127831
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127831 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127831
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */short) var_2);
    var_11 = ((/* implicit */signed char) (short)14847);
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 4) 
    {
        for (long long int i_1 = 2; i_1 < 12; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 1; i_2 < 12; i_2 += 1) 
            {
                {
                    var_12 = ((/* implicit */_Bool) (~(max((((/* implicit */unsigned int) (_Bool)1)), (2502085109U)))));
                    var_13 = ((/* implicit */int) max((((/* implicit */long long int) (_Bool)1)), (-8539947681892274092LL)));
                }
            } 
        } 
    } 
}
