/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39594
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39594 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39594
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (var_10 && var_2);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    arr_11 [i_0] [i_1] [i_0] [i_0] = (((max(var_4, var_7)) & var_5));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_16 = var_4;
                                arr_16 [i_4] [i_1 - 2] [i_2 + 2] [i_2] [i_4] = (((!var_9) >> (var_9 + 2141508034)));
                                arr_17 [i_0] [i_1 - 2] [i_2 + 1] [i_1 - 2] [i_4] [i_1 - 2] = (128 + 1908388801);
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
