/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78653
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78653 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78653
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, (((!(~var_6))))));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_12 = ((-(~16400871379348841036)));

        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {

            /* vectorizable */
            for (int i_2 = 0; i_2 < 23;i_2 += 1) /* same iter space */
            {
                var_13 = (+-19);
                var_14 = (max(var_14, (((-(arr_2 [i_0] [i_1]))))));
                var_15 = ((((!(arr_5 [i_1] [i_1])))));
                arr_11 [i_0] = (((-(arr_1 [i_0]))));
                var_16 *= ((~(!var_9)));
            }
            for (int i_3 = 0; i_3 < 23;i_3 += 1) /* same iter space */
            {
                var_17 = (!var_6);
                arr_16 [i_3] = ((!((((~(arr_15 [i_0] [i_0] [i_0] [21])))))));
                var_18 = (!-var_9);
                arr_17 [i_0] |= (-(--1));
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 23;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 1;i_5 += 1)
                    {
                        {
                            var_19 |= ((!(~var_3)));
                            arr_23 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = ((-((-(~1)))));
                        }
                    }
                }
            }
            for (int i_6 = 0; i_6 < 23;i_6 += 1) /* same iter space */
            {
                var_20 = ((-(--22)));
                var_21 = 1;
                var_22 ^= ((-((-((-(arr_18 [i_0] [i_0] [i_6])))))));
                var_23 = ((-((-((-(arr_24 [i_0] [i_0] [8])))))));
            }
            var_24 = ((-(((!(((~(arr_9 [4] [i_1] [1] [i_1])))))))));
            arr_28 [i_0] [i_0] = ((!(((-(((!(arr_22 [i_0] [i_0] [i_0])))))))));
            var_25 = (max(var_25, (((~((-(~var_3))))))));
        }
        var_26 = ((-(~var_2)));
    }
    for (int i_7 = 0; i_7 < 15;i_7 += 1)
    {
        arr_33 [i_7] [i_7] = ((!((((~(arr_22 [i_7] [i_7] [i_7])))))));
        var_27 = (!var_8);
    }
    var_28 = ((-(~var_5)));
    #pragma endscop
}
