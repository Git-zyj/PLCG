/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72114
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72114 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72114
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [4] [i_1] = var_9;
                var_10 = (((3487759266013089349 | 65534) + ((((min(1, 2))) ? (min(3487759266013089354, 106)) : (((6070576100243570321 ? 95 : var_5)))))));
            }
        }
    }
    var_11 = (var_3 ^ var_2);
    var_12 = ((((((15393 - var_2) / (50143 - -922996718822906530)))) ? (min(var_7, ((1485611036 ? var_3 : 51)))) : (2 - var_7)));
    #pragma endscop
}
