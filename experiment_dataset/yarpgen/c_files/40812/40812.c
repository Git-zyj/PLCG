/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40812
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40812 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40812
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_10, ((-((var_10 ? 1 : var_13))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_15 = (min(var_15, 4294967295));
                                arr_14 [i_3] [i_1] [i_2] [i_3] [i_4] = (((((1 - (var_1 || var_5)))) ? (((25 ? ((4080351824 ? 3089417920279684315 : -3089417920279684316)) : 65535))) : var_2));
                                arr_15 [i_0] [i_0] [i_2] [i_3] [i_0] = (arr_4 [i_0] [i_0]);
                            }
                        }
                    }
                }
                arr_16 [i_0] [i_1] [i_1] = (arr_12 [i_0] [i_1] [i_1] [i_0]);
            }
        }
    }
    var_16 = (min(var_16, var_1));
    #pragma endscop
}
