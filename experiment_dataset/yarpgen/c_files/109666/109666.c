/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109666
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109666 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109666
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] = (arr_2 [i_1]);
                arr_6 [i_0] = var_0;
            }
        }
    }
    var_19 = (max(-1902444593, 0));
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            {
                arr_11 [i_2] [4] = ((((((arr_8 [i_3]) ? 1048448 : (arr_9 [3] [i_2] [i_2]))) != (max((arr_3 [i_2] [i_2] [i_2]), (arr_2 [i_3]))))));
                var_20 ^= (((((arr_0 [i_2]) ? (max((arr_0 [i_2]), (arr_10 [i_3]))) : -1048428)) + (((((max(var_15, 1)) < ((max(-1048425, var_0)))))))));
            }
        }
    }
    var_21 |= (max(var_13, var_16));
    var_22 = (max(((max(2305843009205305344, var_14))), -1048425));
    #pragma endscop
}
