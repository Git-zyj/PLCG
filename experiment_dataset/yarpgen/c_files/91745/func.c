/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91745
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91745 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91745
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 17; i_0 += 3) 
    {
        var_17 = ((/* implicit */long long int) ((((/* implicit */unsigned long long int) (+(((long long int) arr_0 [i_0] [i_0]))))) < (8070450532247928832ULL)));
        arr_2 [i_0] = ((/* implicit */_Bool) (short)-14424);
    }
    var_18 = ((/* implicit */long long int) (-(((/* implicit */int) (short)-14424))));
}
