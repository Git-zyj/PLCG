/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 59625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=59625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/59625
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_15;
    var_17 = ((((((0 / var_4) ? var_4 : ((var_6 ? var_4 : var_4))))) ? var_12 : var_10));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_18 = ((((((arr_4 [4] [i_1 + 1] [4]) ? ((var_13 ? 255 : (arr_3 [i_0] [i_0]))) : (((arr_2 [i_0] [i_0]) ? var_2 : (arr_3 [6] [i_0])))))) ? (((!(var_9 ^ var_11)))) : ((-(((arr_4 [i_0] [i_1] [i_2]) + (arr_5 [i_0] [i_1] [i_2])))))));
                    var_19 = arr_1 [i_0] [i_0];
                }
            }
        }
    }
    #pragma endscop
}
