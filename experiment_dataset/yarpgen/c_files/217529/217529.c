/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217529
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217529 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217529
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_12;

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_18 = ((((-80 ? 1 : 14342)) > ((max((arr_0 [i_0]), (arr_1 [i_0]))))));
        arr_3 [i_0] = (max(((((arr_0 [i_0]) ? (max((arr_2 [19]), (arr_2 [i_0]))) : (max((arr_2 [19]), (arr_2 [i_0])))))), ((((max(var_8, (arr_2 [i_0])))) ? ((var_0 ? (arr_1 [i_0]) : var_4)) : (((0 ? var_11 : (arr_2 [i_0]))))))));
    }
    var_19 = (min(var_10, var_4));
    #pragma endscop
}
