/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159169
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159169 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159169
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 23; i_0 += 1) 
    {
        for (_Bool i_1 = 1; i_1 < 1; i_1 += 1) 
        {
            {
                var_17 &= ((/* implicit */signed char) max(((+(((/* implicit */int) arr_1 [i_1 - 1] [i_1 - 1])))), (max(((+(((/* implicit */int) arr_1 [i_0] [i_1])))), (((((/* implicit */_Bool) arr_4 [i_1])) ? (((/* implicit */int) arr_3 [(short)16])) : (((/* implicit */int) (unsigned char)17))))))));
                var_18 = arr_1 [i_1 - 1] [i_1 - 1];
            }
        } 
    } 
    var_19 = ((/* implicit */signed char) ((((/* implicit */_Bool) (-(((/* implicit */int) min((var_15), (var_12))))))) ? (((/* implicit */int) var_13)) : (((/* implicit */int) var_12))));
    var_20 = ((/* implicit */signed char) var_13);
    var_21 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_7))));
}
