/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89315
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89315 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89315
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((min((var_11 | 3783776016), (((5928251013888133627 ? (-2147483647 - 1) : var_15))))) >= var_15));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_9 [i_1] = var_9;
                    arr_10 [i_2] [i_1] [6] [i_2] = (min((min((min(585396445, var_4)), var_4)), (((!(arr_6 [i_2] [i_1] [i_2]))))));
                }
            }
        }
    }
    var_18 = (((!(-8293628694535426750 & var_13))));
    var_19 = ((min(4032, 4042)));
    #pragma endscop
}
