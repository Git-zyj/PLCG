/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166432
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166432 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166432
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_10 |= (min(var_6, -1561162474));
                var_11 = (!65526);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 16;i_3 += 1)
                    {
                        {
                            var_12 = (max(var_12, 17));
                            var_13 = (max(var_13, (((-869298018 > (((((22 / -12) && var_7)))))))));

                            for (int i_4 = 0; i_4 < 17;i_4 += 1)
                            {
                                var_14 = (187 / 1);
                                var_15 ^= (((!198) > (((!161) ? 185 : (~27227)))));
                            }
                            var_16 += (max(1, ((198 >= ((124 ? var_4 : 7464))))));
                        }
                    }
                }
            }
        }
    }
    var_17 = min((23249 < 121), var_8);
    #pragma endscop
}
