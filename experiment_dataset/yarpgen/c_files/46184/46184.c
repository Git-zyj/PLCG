/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46184
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 13;i_2 += 1)
            {
                {
                    var_19 = (min(var_19, ((((arr_1 [7]) >> ((((((93 && (arr_2 [i_0] [10]))) ? var_3 : (var_3 * var_14))) - 18446744072121555928)))))));
                    arr_7 [i_0] [i_0] [i_1] |= ((+(((arr_5 [i_2 + 3] [i_2 + 4] [i_2 + 3]) - -62))));
                    var_20 = (!1);
                }
            }
        }
    }
    var_21 = (max(var_21, (((((min(205, 4537832222946884117))) - var_15)))));
    #pragma endscop
}
