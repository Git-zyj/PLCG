/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181159
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181159 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181159
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_14 = (((min(255, (arr_0 [i_0]))) & ((max((arr_1 [i_0]), (min(var_13, (arr_1 [i_0]))))))));
        var_15 = ((max(32767, (arr_2 [i_0]))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_6 [i_1] [i_1] = (arr_3 [i_1]);
        var_16 = (((max((arr_4 [i_1]), (arr_3 [i_1])))));
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 24;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 24;i_4 += 1)
            {
                {
                    var_17 = 111;
                    var_18 = arr_10 [i_2] [i_3] [18];
                }
            }
        }
    }
    var_19 = ((18 ? var_9 : (max(var_12, (~var_3)))));
    #pragma endscop
}
