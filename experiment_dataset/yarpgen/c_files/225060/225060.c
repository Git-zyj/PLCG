/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 225060
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=225060 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/225060
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 10;i_4 += 1)
                        {
                            {
                                var_16 = ((12 ? 9851010817850363158 : 2590231281));
                                var_17 = 1484;
                                arr_12 [7] [i_1] [i_1] [i_1] [i_1] [i_0] [i_1 - 2] = (max(-1484, -1493));
                            }
                        }
                    }
                    var_18 = 37;
                    var_19 = -19;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_5 = 0; i_5 < 14;i_5 += 1)
    {
        for (int i_6 = 4; i_6 < 12;i_6 += 1)
        {
            {
                var_20 = ((11420 ? 2 : 1492));
                var_21 = 35;
                var_22 *= 1;
            }
        }
    }

    for (int i_7 = 4; i_7 < 22;i_7 += 1)
    {
        var_23 = (max(((min(var_14, -1484))), (7725094498545634664 | -30)));
        arr_20 [i_7] = -1;
        arr_21 [8] [i_7] = -1;
        arr_22 [i_7] = ((((max(87, 29))) >> ((((-14402 ? var_14 : -1493)) - 59439))));
    }
    #pragma endscop
}
