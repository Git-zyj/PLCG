/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79301
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((min(9223372036854775807, var_5))) ? ((var_1 ? 23508 : 5979203617673347672)) : var_8)) < var_2);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_0] [i_2] = (arr_1 [i_0 + 1] [i_0]);
                    var_12 = (min(((min(273296934, (-2147483647 - 1)))), (max((~411397271), (((-273296945 ? (arr_5 [i_2]) : var_10)))))));
                }
            }
        }
    }
    var_13 = (max(175, (!-273296934)));
    #pragma endscop
}
