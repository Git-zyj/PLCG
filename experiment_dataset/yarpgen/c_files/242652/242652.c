/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    arr_12 [i_2] [i_1] [i_2] &= ((3828042625304766488 == (((-2004100484 / (arr_10 [i_0 - 1] [i_0] [i_2 - 1] [i_0]))))));
                    var_18 -= var_17;
                }
            }
        }
    }
    var_19 = (~1);
    var_20 = (((1 || 2732687933) && ((min(var_8, ((-8840 ? var_4 : 2147483647)))))));
    var_21 = (min((18446744073709551604 || var_8), ((max((max(var_9, var_6)), var_5)))));
    #pragma endscop
}
