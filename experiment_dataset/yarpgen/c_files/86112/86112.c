/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86112
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86112 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86112
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 *= ((var_17 ? 2303591209400008704 : var_12));
    var_19 = var_4;
    var_20 = (((((min(var_9, var_5)))) >= (min((4398046511104 & 0), (~var_10)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_21 = min(((11218304990074736728 ? (arr_1 [i_0]) : (arr_1 [i_0]))), (((arr_1 [i_0]) * (arr_1 [i_0]))));
                var_22 = (((-1 + 2147483647) >> ((var_7 / (max(2145386496, 0))))));
                arr_5 [i_0] [i_1] = (arr_0 [i_0]);
            }
        }
    }
    #pragma endscop
}
