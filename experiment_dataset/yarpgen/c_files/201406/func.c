/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201406
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201406 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201406
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 += ((/* implicit */int) var_4);
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 8; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 12; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */long long int) (((~(var_3))) != (((/* implicit */unsigned long long int) ((/* implicit */int) (!(((/* implicit */_Bool) -705770242))))))));
                arr_6 [i_0] = (-(((((/* implicit */_Bool) (((((((-2147483647 - 1)) + (2147483647))) + (2147483647))) >> (((1113507797) - (1113507770)))))) ? (((/* implicit */int) ((signed char) arr_2 [(signed char)9] [i_1] [(signed char)4]))) : (((/* implicit */int) (unsigned short)0)))));
            }
        } 
    } 
}
