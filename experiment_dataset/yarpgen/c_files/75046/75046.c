/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75046
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75046 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75046
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= (((var_10 | (~var_12))));
    var_17 ^= ((-4711299231956129602 ? var_13 : ((var_6 + ((641582649 ? var_6 : var_15))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_18 = (min(var_18, (((var_8 ? (((((65520 ? var_15 : (arr_0 [14])))) - (min(3137557541, var_8)))) : (arr_0 [18]))))));
                var_19 = ((((((69 + 1157409765) | (arr_5 [i_1 - 1] [i_1 - 1] [i_1])))) ? (arr_3 [i_1]) : (!207)));
            }
        }
    }
    #pragma endscop
}
