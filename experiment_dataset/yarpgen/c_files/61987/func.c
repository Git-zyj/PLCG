/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61987
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61987 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61987
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */signed char) (+(((((/* implicit */_Bool) ((0LL) | (((/* implicit */long long int) ((/* implicit */int) var_15)))))) ? (((/* implicit */int) var_4)) : ((((_Bool)1) ? (((/* implicit */int) var_8)) : (((/* implicit */int) var_16))))))));
    /* LoopSeq 1 */
    for (long long int i_0 = 2; i_0 < 19; i_0 += 4) 
    {
        arr_4 [i_0 - 2] [i_0] |= ((/* implicit */unsigned char) arr_3 [i_0 - 1] [i_0]);
        var_21 *= ((/* implicit */unsigned int) (unsigned char)89);
    }
    var_22 = ((/* implicit */unsigned char) var_13);
}
