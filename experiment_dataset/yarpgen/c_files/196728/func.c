/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196728
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196728 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196728
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) (~(max((((/* implicit */int) (short)2032)), (((((/* implicit */_Bool) 4143441998U)) ? (((/* implicit */int) (_Bool)0)) : (-505369411)))))));
    var_16 = ((/* implicit */long long int) min((var_16), (((/* implicit */long long int) var_5))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 12; i_0 += 1) 
    {
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned int) arr_1 [i_0]);
        var_17 = ((/* implicit */unsigned char) max((var_17), (((/* implicit */unsigned char) (-(((/* implicit */int) arr_1 [i_0])))))));
    }
}
