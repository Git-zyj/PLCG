/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131653
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131653 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131653
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                var_17 = var_14;
                var_18 = (((max(var_3, (arr_3 [i_1 - 2])))) ? (max((!24583), (((((arr_5 [i_0] [i_1] [i_1]) + 2147483647)) << (((arr_2 [i_1]) - 234)))))) : (((arr_6 [i_1 - 2] [i_1] [i_1]) ? ((13661782899448089810 ? (arr_1 [i_1]) : (arr_6 [i_1] [i_1] [i_0]))) : ((13661782899448089810 ? var_13 : 255)))));
            }
        }
    }
    #pragma endscop
}
