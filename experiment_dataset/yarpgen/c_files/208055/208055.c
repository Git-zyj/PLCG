/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208055
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = 0;
                var_17 -= var_6;
                /* LoopNest 3 */
                for (int i_2 = 4; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 13;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_0] [i_2] [i_3] [i_4] = (arr_11 [i_0]);
                                var_18 = (((arr_9 [i_4 + 3] [i_4 + 2] [i_4] [i_4 - 1] [i_4 + 1] [i_4]) * ((((arr_9 [i_4 + 3] [i_4 + 2] [i_4 + 3] [i_4] [i_4 - 1] [i_4]) * (arr_13 [i_4 + 3] [i_0] [i_4] [i_4 + 1] [i_4] [i_4] [i_4 + 3]))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = var_8;
    #pragma endscop
}
