/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106013
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106013 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106013
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((~(var_1 | var_15))) | ((((~var_14) | var_9)))));
    var_17 = var_0;
    var_18 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_19 = (((var_2 ? (((2975607669810866594 ? (arr_1 [i_0] [i_0]) : (arr_3 [i_0] [i_0] [i_0])))) : (min((arr_0 [10]), 1615959088)))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                arr_15 [10] [i_1] = (((((var_5 ? var_7 : (arr_1 [i_4] [i_4])))) > ((-(((arr_6 [i_0] [i_1] [0]) * var_15))))));
                                var_20 += 1;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
