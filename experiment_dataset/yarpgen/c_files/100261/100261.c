/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100261
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100261 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100261
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (!953002015);
    var_13 = (min(var_13, -var_0));

    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            arr_5 [i_0] [i_0] = 1;
            var_14 = (min((arr_4 [i_0]), (arr_1 [i_0])));

            /* vectorizable */
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                arr_9 [i_0] [i_0] = (arr_1 [i_0]);
                arr_10 [i_0] [i_0] = (((arr_7 [i_1] [i_2]) ? (arr_0 [i_0]) : (arr_7 [i_1] [i_0])));
            }
            arr_11 [i_0] [i_0] = ((((arr_2 [i_0 - 1]) ? ((((arr_7 [i_0] [i_0]) * (arr_2 [i_0])))) : (arr_6 [i_0] [i_1] [i_0] [i_1]))));
        }
        arr_12 [i_0] = (min((((!(!953002029)))), (arr_4 [0])));
        var_15 = ((+((((var_1 || 8379) || 1)))));
        var_16 = (arr_1 [i_0]);
    }
    var_17 = (min(((((var_1 && var_7) && ((max(1385446226, 819661004)))))), (((var_5 <= 953002015) - var_8))));
    var_18 = 3317662047;
    #pragma endscop
}
