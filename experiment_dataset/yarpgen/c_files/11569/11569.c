/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11569
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11569 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11569
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (!-49962);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_19 = ((((min((arr_3 [i_0 + 1] [i_2]), (-1 <= var_5)))) | 49975));
                    arr_9 [i_0] [i_1] [i_0] [i_0 + 1] = max((((var_8 < (min((arr_6 [i_0] [i_0] [15]), (arr_7 [i_0] [16] [i_1] [i_0])))))), 16777215);
                    arr_10 [i_0] = var_9;
                    var_20 = ((var_12 ? (arr_4 [i_0] [i_0 - 1]) : 16777238));
                }
            }
        }
    }
    #pragma endscop
}
