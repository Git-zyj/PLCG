/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18962
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18962 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18962
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((min(((var_4 ? var_5 : var_12)), ((var_9 ? var_13 : var_11)))) / ((6346 ? var_6 : ((18270 ? var_9 : var_3))))));
    var_15 = -183700239;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_16 = var_0;
                    var_17 = (min(var_17, (max((!1294602890), (arr_4 [i_0] [i_0])))));
                }
            }
        }
    }
    var_18 = max(var_13, (max((47266 & 50282), (!var_3))));
    #pragma endscop
}
