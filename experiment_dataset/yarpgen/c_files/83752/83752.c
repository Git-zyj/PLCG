/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83752
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83752 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83752
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (min((arr_2 [i_1]), (((((-526879252 / (arr_0 [i_1])))) ? ((var_7 ? (arr_2 [i_1]) : (arr_1 [i_0]))) : var_1))));
                arr_6 [i_0] = ((((max((arr_4 [i_0] [i_1 + 1] [i_0]), (arr_4 [i_1] [i_1 + 1] [i_0])))) ? var_8 : (((arr_4 [i_1] [i_1 - 1] [i_1 - 1]) ? ((var_9 ? 18 : -82)) : (arr_1 [i_0])))));
            }
        }
    }
    var_10 = 57;
    var_11 = -80;
    #pragma endscop
}
