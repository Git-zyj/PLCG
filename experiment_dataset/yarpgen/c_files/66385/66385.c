/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66385
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66385 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66385
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((19 ? 209 : 260096));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            {
                var_12 = (arr_4 [3]);
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_13 *= (((min(((var_7 ? var_5 : var_3)), (((arr_10 [1] [i_3]) + (arr_10 [i_0] [i_0]))))) + (((arr_10 [i_2 - 1] [i_1 + 1]) ? (arr_10 [i_2 - 1] [i_1 - 1]) : (arr_10 [i_2 + 2] [i_1 - 1])))));
                            var_14 = (((-(arr_9 [i_0] [i_1 - 1] [i_2] [i_2] [i_3]))));
                            var_15 = (max(var_15, ((max(var_0, ((~((~(arr_0 [i_3]))))))))));
                        }
                    }
                }
            }
        }
    }
    var_16 = var_7;
    #pragma endscop
}
