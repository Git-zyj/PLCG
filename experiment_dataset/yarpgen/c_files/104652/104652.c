/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((max((3687251313598673320 - var_2), ((min(var_6, var_1))))) > (-3687251313598673321 | var_16)));

    /* vectorizable */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        var_20 = (min(var_20, (((~((-(arr_1 [i_0 - 2]))))))));
        arr_2 [i_0] [0] &= (((arr_0 [i_0 - 1]) >> (((arr_1 [i_0]) + 6350))));
    }
    var_21 = (min((((var_5 * var_12) ? (195 - -3687251313598673341) : var_0)), var_14));
    var_22 *= var_3;
    #pragma endscop
}
