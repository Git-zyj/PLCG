/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215146
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215146 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215146
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 = (max(var_12, (((((min(var_7, (arr_0 [i_0])))))))));
                arr_7 [i_0] [1] [i_1] &= ((939524096 >> (((arr_2 [i_0] [i_1]) + 1612324811692366995))));
                arr_8 [i_0] = (arr_6 [i_0] [i_0] [i_1]);
                var_13 = (max(var_13, ((min((arr_1 [4] [2]), (((arr_1 [2] [i_1]) | (arr_0 [i_0]))))))));
            }
        }
    }
    var_14 = (min(var_14, var_4));
    var_15 = (min(var_15, var_4));
    #pragma endscop
}
