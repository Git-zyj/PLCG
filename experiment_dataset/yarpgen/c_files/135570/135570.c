/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135570
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135570 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135570
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_13 = (((var_9 ? (arr_0 [i_0] [i_0]) : (((arr_2 [i_0] [i_0]) ? var_9 : 55198)))));
        arr_4 [i_0] = ((+(max((arr_1 [i_0] [i_0]), 18446744073709551615))));
    }
    var_14 = var_5;
    var_15 = (((min(22886, ((var_4 >> (var_10 - 189)))))) ? ((var_2 ? var_4 : var_10)) : ((((((55196 << (42631 - 42619)))) ? var_9 : ((var_9 ? 0 : var_4))))));
    #pragma endscop
}
