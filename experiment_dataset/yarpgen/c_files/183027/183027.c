/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183027
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183027 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183027
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (max((((52411 | var_11) >> (((max(var_10, 576460752299229184)) - 9919905341659472424)))), var_7));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_19 = (max(var_19, (arr_3 [i_2] [i_1])));
                    var_20 = (((~(var_8 & 2786097261))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_21 = (~255);
                                var_22 += 1;
                            }
                        }
                    }
                    var_23 |= var_10;
                    var_24 -= ((max((arr_0 [0]), (-64 > 17870283321410322432))));
                }
            }
        }
    }
    #pragma endscop
}
