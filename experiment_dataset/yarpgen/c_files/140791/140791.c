/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140791
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140791 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140791
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 13;i_1 += 1)
        {
            {
                var_19 = var_3;
                var_20 |= (((((-(min(14499, var_5))))) ? ((min(((-21589 ? var_11 : (arr_2 [10]))), (arr_4 [4] [i_1 - 2])))) : (max((min(4214020492074040741, (arr_2 [i_0]))), 1))));
                var_21 = ((+(((((var_9 ? (arr_2 [i_0]) : (arr_2 [i_1]))) >= var_5)))));
                var_22 = (min((((~(arr_4 [i_0] [i_1 - 3])))), ((((arr_3 [i_0] [i_1 - 3] [i_0]) >= (arr_2 [i_0]))))));
            }
        }
    }
    var_23 = var_4;
    #pragma endscop
}
