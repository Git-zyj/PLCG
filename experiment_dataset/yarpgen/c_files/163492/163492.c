/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163492
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163492 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163492
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 6;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 9;i_4 += 1)
                        {
                            {
                                var_12 = var_5;
                                var_13 = ((((!(((~(arr_8 [1] [i_1 - 1])))))) && (((((arr_2 [i_3 + 2] [i_2]) ? (arr_7 [9] [i_3] [i_2] [i_1] [i_0]) : var_8))))));
                            }
                        }
                    }
                    var_14 = ((((((~(arr_9 [i_0] [i_1 - 1] [i_1 + 1] [i_2] [i_2] [i_2])))) ? ((-(arr_6 [i_1] [i_0]))) : var_11)));
                }
            }
        }
    }
    var_15 = (((((max(var_11, -5468))) / -var_9)));
    var_16 = ((~(!16383)));
    var_17 = (min(var_4, var_0));
    var_18 = var_2;
    #pragma endscop
}
