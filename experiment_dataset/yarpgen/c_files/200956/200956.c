/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200956
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200956 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200956
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_12 = (((((((-552727883998669681 || (arr_5 [i_1] [i_1] [i_2])))) & (arr_5 [i_2] [i_0 + 1] [i_0 - 2])))) ? var_7 : (arr_3 [i_0 - 1]));
                    var_13 *= (((arr_6 [i_0] [i_1] [i_2] [i_1]) ? -552727883998669659 : 1653669055));
                    var_14 = (min(var_14, ((max((arr_3 [i_0]), (max((arr_1 [i_1] [i_1]), (max((arr_0 [i_1]), var_8)))))))));
                }
            }
        }
    }
    var_15 = var_7;
    #pragma endscop
}
