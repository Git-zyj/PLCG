/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((((min(var_15, var_1)) * var_15))));
    var_20 = (((var_13 != -119) ? (max((max(var_11, -10700)), -1)) : var_15));
    var_21 &= 1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_22 *= var_3;
                arr_4 [i_1] [i_1] = ((((((arr_1 [i_0]) | (arr_0 [i_1] [i_0])))) ? ((min(-103, 1))) : (((!(arr_3 [i_1]))))));
                var_23 = var_15;
                var_24 = (((((~(!var_8)))) ^ ((~(min(var_12, (arr_1 [i_0])))))));
            }
        }
    }
    #pragma endscop
}
