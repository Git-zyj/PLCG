/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44056
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44056 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44056
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_16 = (max((((-127 - 1) - 1079985079)), (((!(((1041416555 % (arr_2 [i_0])))))))));
        var_17 = ((((arr_0 [i_0]) * var_8)));
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        var_18 = ((((0 || (arr_3 [i_1]))) ? var_1 : ((((arr_4 [i_1]) ? var_14 : var_12)))));
        var_19 = ((var_2 ? (((arr_4 [i_1]) ? (((arr_0 [i_1]) - (arr_1 [i_1]))) : (var_12 + var_9))) : (var_8 + var_0)));
        var_20 = ((!(((((1 - (arr_3 [i_1]))) * var_8)))));
    }
    var_21 = var_1;
    #pragma endscop
}
