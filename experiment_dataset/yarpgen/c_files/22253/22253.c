/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22253
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22253 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22253
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_6 [4] [i_1] [i_2] [i_2] = (arr_3 [i_2]);
                    arr_7 [i_2] [0] [i_0] = (arr_4 [1] [i_1]);
                }
            }
        }
    }
    var_13 = ((var_9 ? (((((var_2 ? var_9 : 0))) ? ((15244 ? 62 : var_3)) : var_3)) : ((min(-9223372036854775805, var_12)))));
    var_14 = ((--29445) ? var_2 : (max((28499 % 5808684109089432312), -var_4)));
    #pragma endscop
}
