/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148506
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((((+(((-9223372036854775807 - 1) ^ var_3)))) & (((-6607073331691855310 | var_4) - (var_7 / var_16)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                var_21 = (((((arr_4 [i_0]) + var_8)) == ((((-9223372036854775807 - 1) > (arr_2 [i_1 + 3]))))));
                var_22 = ((!(var_14 | 17720699710409408776)));
            }
        }
    }
    #pragma endscop
}
