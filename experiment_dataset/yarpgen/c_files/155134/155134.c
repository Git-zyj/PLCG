/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155134
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = ((~((384 ? 0 : 1388241686))));
                var_14 = ((!(arr_2 [i_1])));
                var_15 = ((((((arr_5 [0]) ? (arr_0 [i_0] [i_1]) : (arr_0 [i_0] [i_1])))) || -21691));
                var_16 = (max(var_16, ((max((((4080 == (arr_0 [i_0] [i_0])))), (((arr_0 [i_0] [i_1]) + (arr_0 [i_0] [i_1]))))))));
            }
        }
    }
    var_17 = var_0;
    var_18 = var_10;
    var_19 = (max(var_19, (((var_8 ? var_8 : 1)))));
    #pragma endscop
}
