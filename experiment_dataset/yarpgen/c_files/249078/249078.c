/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249078
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249078 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249078
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 += var_12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                var_16 = (min(var_1, 19524));
                var_17 = 15231192608813829259;
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 10;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 12;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 1;i_4 += 1)
            {
                {
                    arr_13 [i_2] [i_2] = (((((((arr_12 [i_3] [i_3]) ? 4276105016564604885 : (arr_12 [i_3] [i_3]))))) && (((((arr_8 [i_2 + 2] [i_2]) <= (arr_5 [i_2] [i_2])))))));

                    for (int i_5 = 3; i_5 < 11;i_5 += 1)
                    {
                        var_18 ^= (arr_14 [2] [10]);
                        arr_17 [i_2] [i_3] [i_2] [i_3] = var_6;
                    }
                    for (int i_6 = 0; i_6 < 12;i_6 += 1)
                    {
                        var_19 = (max(var_19, (((arr_2 [i_4]) <= var_13))));
                        var_20 &= (arr_6 [i_2]);
                    }
                }
            }
        }
    }
    var_21 = var_8;
    #pragma endscop
}
