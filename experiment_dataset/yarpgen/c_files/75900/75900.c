/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75900
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75900 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75900
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 1509718255;

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_19 |= (((((-(~3880346151)))) ? (arr_0 [i_0] [i_0]) : ((3880346144 ? 1 : 3880346139))));
        var_20 -= (min(var_11, (max(0, 1))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_21 = (max(var_21, var_12));
        arr_4 [i_1] = (min(((((var_6 ? 9268 : var_16)))), (((arr_1 [i_1]) ? (max((arr_1 [i_1]), (arr_2 [i_1]))) : (~var_17)))));
    }
    #pragma endscop
}
