/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85502
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85502 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85502
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 ^= ((/* implicit */int) var_4);
    var_19 = ((/* implicit */unsigned int) (-((+(((/* implicit */int) var_4))))));
    var_20 *= ((/* implicit */unsigned int) ((unsigned long long int) var_2));
    var_21 = ((/* implicit */unsigned int) (+(((/* implicit */int) var_5))));
    var_22 = ((/* implicit */short) max((var_22), (((/* implicit */short) var_16))));
}
