/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191015
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191015 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191015
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((max(((max(var_3, var_3))), var_9))) ? (min(((-173409695 ? 173409694 : 119)), -var_8)) : (var_3 == var_7)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    var_11 ^= (((!1) ? 18446744073709551607 : (arr_6 [i_0] [21])));
                    var_12 = (min(var_12, (arr_1 [i_1])));
                    arr_7 [i_0] = ((((((((4294967295 && (arr_4 [i_1])))) & (65512 & 173409724)))) ? var_8 : (max((arr_0 [i_0]), (max(4275, 945615659396122713))))));
                    var_13 = 4294967295;
                }
            }
        }
    }
    #pragma endscop
}
