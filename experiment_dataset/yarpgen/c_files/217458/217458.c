/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217458
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217458 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217458
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_19 = (((var_3 < -2007326298) ? var_14 : ((((((arr_1 [i_0]) + 2007326314))) ? ((max((arr_5 [i_0] [i_0]), var_9))) : (min((arr_5 [i_0] [i_0]), (arr_2 [i_0] [i_0])))))));
                    var_20 = (min(var_20, ((((-2007326319 + 2147483647) >> (!1696879321999983651))))));
                }
            }
        }
    }
    var_21 = var_15;
    var_22 -= ((~((-((var_4 ? var_7 : var_11))))));
    #pragma endscop
}
