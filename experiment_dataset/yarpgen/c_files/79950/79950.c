/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79950
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79950 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79950
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 |= var_2;
    var_17 = var_13;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_18 = (min(var_18, (((arr_0 [i_0]) >= -18446744073709551615))));
        var_19 = (((arr_1 [i_0] [i_0]) ? (0 == var_5) : ((min(var_4, (arr_3 [i_0]))))));
        var_20 = var_1;
        var_21 = 18446744073709551598;
        arr_4 [i_0] [i_0] = (((!(arr_0 [i_0]))));
    }
    for (int i_1 = 1; i_1 < 8;i_1 += 1)
    {
        var_22 = var_13;
        var_23 = ((var_4 >> ((((((0 ? var_1 : var_4)))) - 25799))));
        arr_8 [i_1 + 2] = (arr_0 [i_1]);
    }
    for (int i_2 = 3; i_2 < 23;i_2 += 1)
    {
        arr_12 [i_2] = ((((min(0, 6294507767777253561))) ? (arr_2 [i_2 - 3] [i_2 - 2]) : 2));
        var_24 = 6294507767777253561;
    }
    var_25 = (max(var_25, var_1));
    #pragma endscop
}
