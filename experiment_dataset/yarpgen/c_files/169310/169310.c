/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169310
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169310 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169310
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 &= (((((var_6 ? var_10 : ((max(1, 1)))))) ? var_14 : var_0));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_16 = (arr_0 [i_1] [i_0]);
                    var_17 = (min(var_17, (max((((0 ? var_9 : (((arr_7 [i_2] [i_2] [i_1] [i_0]) ? var_7 : (arr_6 [i_1] [i_1] [i_1] [i_1])))))), (((arr_3 [i_0] [i_0]) ? (arr_3 [i_0] [i_0]) : (arr_3 [i_1] [i_0])))))));
                    var_18 = ((-((-(arr_8 [i_2] [13] [i_0] [i_0])))));
                    var_19 = ((-1048576 ? 123 : 1408));
                }
            }
        }
    }
    #pragma endscop
}
