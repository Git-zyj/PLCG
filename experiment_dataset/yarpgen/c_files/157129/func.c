/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157129
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157129 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157129
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 21; i_1 += 1) 
        {
            for (unsigned short i_2 = 1; i_2 < 19; i_2 += 2) 
            {
                {
                    var_18 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) (((!(((/* implicit */_Bool) (short)508)))) ? (arr_7 [i_0] [i_2] [i_2 + 2] [i_1]) : (((/* implicit */long long int) ((/* implicit */int) (_Bool)1))))))));
                    var_19 -= ((max((arr_7 [i_2 + 2] [i_2 + 1] [i_2 + 2] [i_2 + 1]), (arr_7 [i_2 + 2] [i_2 - 1] [(_Bool)1] [i_2 - 1]))) / (((/* implicit */long long int) ((((/* implicit */unsigned int) ((/* implicit */int) (unsigned short)8737))) | (var_17)))));
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */unsigned long long int) var_9);
    var_21 = ((/* implicit */int) (!(((/* implicit */_Bool) var_5))));
}
