/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42005
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    var_12 = ((max((((!(arr_0 [i_2])))), ((1 ? (arr_0 [i_1]) : (arr_4 [i_0] [i_0] [i_0] [i_0]))))) + ((~(((arr_1 [i_0] [i_1]) ? var_5 : var_5)))));
                    var_13 = (max(var_13, ((max((((arr_3 [i_1]) - var_11)), (arr_2 [i_0 - 2]))))));
                }
            }
        }
    }
    var_14 = (max(var_14, ((-((var_6 ? (~var_3) : var_0))))));
    #pragma endscop
}
