/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39396
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39396 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39396
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((min((~var_2), ((var_12 ? var_4 : var_12))))) || var_1);
    var_15 = (min(var_15, ((((((var_5 - var_0) >> (((var_2 | -15) + 65)))) * (((~((min(37228, var_6)))))))))));
    var_16 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                arr_8 [i_1] [i_1 - 1] = var_12;
                var_17 = 55562;
            }
        }
    }
    var_18 = max((((((max(1, var_7)))) & ((-75 ? var_13 : 12400)))), (((((max(var_6, var_4))) | ((max(53133, 0)))))));
    #pragma endscop
}
