/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_0;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_3 [i_0] = ((1 ? 1 : (0 < -2)));
        arr_4 [i_0] [i_0] = (arr_0 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [1] = ((1 ? (((arr_7 [i_0] [i_0] [11]) ? 1 : var_9)) : (arr_6 [i_0])));
                    var_11 = 0;

                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        arr_12 [i_0] [1] [i_2] [i_3] = ((-(var_4 >> 1)));
                        arr_13 [i_0] [i_1] [i_2] [i_1] = (((((var_8 * (arr_1 [i_3])))) ? var_0 : ((((arr_5 [i_0]) <= 0)))));
                        var_12 = ((arr_7 [i_0] [i_0] [i_0]) > (arr_7 [i_0] [i_0] [i_0]));
                        var_13 = ((1 ? (-1 == 1) : (arr_5 [1])));
                    }
                }
            }
        }
    }
    var_14 = ((var_7 | (((!(var_6 == 1))))));
    var_15 = var_9;
    var_16 |= (!var_1);
    #pragma endscop
}
