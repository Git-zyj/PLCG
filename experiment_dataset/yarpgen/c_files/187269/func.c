/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187269
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned char) var_13);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 4; i_0 < 19; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 3; i_1 < 21; i_1 += 4) 
        {
            {
                var_15 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) arr_1 [i_1]))));
                arr_5 [i_0 - 3] = (!(((/* implicit */_Bool) max((arr_1 [i_0]), (((/* implicit */signed char) (!(((/* implicit */_Bool) arr_1 [i_0])))))))));
                var_16 = min((((/* implicit */unsigned long long int) (+(((((/* implicit */int) arr_1 [i_0])) * (((/* implicit */int) arr_2 [i_0 - 3] [i_0 - 3]))))))), ((~(max((var_13), (((/* implicit */unsigned long long int) arr_2 [i_0 + 3] [i_0 + 3])))))));
            }
        } 
    } 
}
