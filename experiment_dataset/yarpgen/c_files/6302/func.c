/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6302
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6302 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6302
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 23; i_0 += 2) 
    {
        arr_2 [10U] = ((/* implicit */signed char) arr_1 [i_0] [i_0]);
        arr_3 [22] = ((/* implicit */_Bool) ((arr_1 [1U] [i_0]) ? (arr_0 [(short)2] [i_0]) : (((/* implicit */long long int) ((((/* implicit */int) (short)5837)) | (((/* implicit */int) (unsigned short)13)))))));
    }
    var_16 = ((/* implicit */signed char) (short)5837);
}
