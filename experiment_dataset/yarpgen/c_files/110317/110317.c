/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110317
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110317 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110317
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_9;
    var_19 = ((var_9 ? (((((var_4 ? var_17 : var_10))) ? var_10 : (~var_8))) : var_2));

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        var_20 = (min(var_20, ((((((((var_8 * (arr_0 [i_0] [i_0])))) ? (arr_0 [i_0] [i_0]) : (((arr_1 [i_0]) ? (arr_0 [i_0] [i_0]) : var_12)))) << (((min(var_1, var_7)) - 111)))))));
        arr_2 [i_0] [i_0] = (arr_0 [i_0] [i_0]);
        arr_3 [i_0] = var_5;
    }
    for (int i_1 = 2; i_1 < 23;i_1 += 1)
    {
        var_21 = (min(var_21, (arr_4 [i_1 - 2] [i_1 - 1])));
        var_22 = (-6 * 6);
    }
    var_23 = var_0;
    #pragma endscop
}
