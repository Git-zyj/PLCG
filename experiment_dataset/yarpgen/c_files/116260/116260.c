/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116260
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116260 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116260
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_19 = 18446744073709551615;
                    arr_8 [i_0] [i_0] [i_0] = (arr_0 [i_0 - 1]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 14;i_4 += 1)
                        {
                            {
                                var_20 = (((max(var_11, 0)) / ((max((((arr_2 [i_1]) ? (arr_11 [i_0] [i_3] [1] [i_0]) : 1)), ((min((arr_2 [i_3]), var_3))))))));
                                var_21 = (((max(1, (1 != -116)))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_22 = ((((max(67, (223 >= -125)))) ? ((-125 ? ((var_18 ? var_2 : var_12)) : ((max(var_10, 1))))) : 1));
    #pragma endscop
}
