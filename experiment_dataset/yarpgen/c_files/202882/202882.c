/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202882
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202882 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202882
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        var_20 = (min(var_20, -111));
        var_21 = (min(var_21, var_5));
    }
    /* LoopNest 3 */
    for (int i_1 = 3; i_1 < 17;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 1;i_3 += 1)
            {
                {
                    arr_9 [i_3] [i_3] [i_3] = ((-(~var_19)));
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 18;i_4 += 1)
                    {
                        for (int i_5 = 4; i_5 < 17;i_5 += 1)
                        {
                            {
                                arr_16 [i_1] [i_1] [i_3] [i_3] [i_5] = ((~((((((arr_11 [i_1] [i_2] [i_1] [i_1]) ? (arr_12 [i_1] [i_3] [i_3] [i_4] [i_4]) : 13794))) / -13782))));
                                var_22 = -4518;
                            }
                        }
                    }
                    arr_17 [i_1] [i_1] [i_2] [i_1] = var_15;
                }
            }
        }
    }
    #pragma endscop
}
