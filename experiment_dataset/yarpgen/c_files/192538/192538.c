/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 192538
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=192538 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/192538
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_15 += (min((1287222537 - 511), (arr_6 [0])));
                            var_16 = (var_10 ? ((~((var_5 ? (arr_8 [i_0] [i_1]) : var_7)))) : -60);
                            var_17 += var_7;
                            var_18 += (~(!253));
                        }
                    }
                }
                var_19 = (max((arr_13 [i_0] [4] [i_0] [i_0]), (((arr_1 [9]) ? (var_7 && 65012) : (((-81 > (arr_7 [i_0] [i_0]))))))));
            }
        }
    }
    var_20 = ((((max(var_6, ((var_8 ? var_4 : var_11))))) || var_13));
    #pragma endscop
}
