/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18786
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18786 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18786
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_7;
    var_13 += max(-1, var_6);
    var_14 += var_6;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 11;i_2 += 1)
            {
                {
                    var_15 = (arr_1 [i_1]);
                    var_16 = (((arr_6 [i_1] [i_1] [i_1] [i_2]) ? (((3788 > (arr_4 [i_1] [i_1 + 1])))) : (arr_6 [i_1] [i_1] [i_1 + 1] [3])));
                    arr_10 [i_1] = (min(((((arr_2 [i_1] [i_1]) <= var_6))), (((arr_7 [i_0] [i_1 - 1] [i_2] [i_1 - 1]) >> (((!(arr_1 [4]))))))));
                    arr_11 [i_0] [i_0] [i_0] [i_2 - 1] &= (arr_4 [i_0] [i_0]);
                }
            }
        }
    }
    var_17 = -64;
    #pragma endscop
}
