/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217171
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217171 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217171
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = ((~((((240317837 & 157543723) <= (arr_3 [i_0] [i_1] [i_1]))))));
                arr_6 [i_1] [i_1] = (((((!((max(var_7, (arr_3 [i_0] [i_1] [4]))))))) != (arr_4 [i_1] [i_0])));

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    arr_9 [10] [i_1] [i_1] = (max(-2855132520287340796, 433811321));
                    var_12 -= -675144476;
                }
                arr_10 [i_1] = (max(675144476, (arr_8 [i_0])));
                arr_11 [i_0] = -675144477;
            }
        }
    }
    #pragma endscop
}
