/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199120
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199120 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199120
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, var_11));
    var_20 = -var_14;
    var_21 = (~var_1);

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_3 [i_0] |= (((arr_0 [i_0] [i_0]) / (arr_2 [i_0] [i_0])));
        var_22 = (min(var_22, ((((((min(39377, 438986511)) ? 17432 : -28072)))))));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = -17432;
        var_23 = ((((-(arr_4 [i_1]))) | (arr_4 [i_1])));
        var_24 = (((arr_5 [i_1]) ? 7 : 28072));
        var_25 = ((((((arr_5 [i_1]) | (arr_5 [i_1])))) ^ var_14));
        var_26 ^= 17407;
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 23;i_2 += 1) /* same iter space */
    {
        var_27 = var_18;
        var_28 = (min(var_28, var_8));
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 23;i_3 += 1) /* same iter space */
    {
        var_29 = ((17432 ? 471557051 : (arr_8 [i_3])));
        arr_11 [i_3] = (~13427918935574358862);
        var_30 = ((-28072 ? (arr_5 [i_3]) : var_3));
    }
    #pragma endscop
}
