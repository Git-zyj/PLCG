/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227824
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227824 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227824
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    var_12 = (((max((var_8 | -18252), ((min(var_4, 0))))) * ((-(-2 <= 0)))));
                    var_13 = (min(var_13, (((((((~0) & -1))) ? (26259 ^ var_8) : ((var_10 ? (var_11 & 8) : -2128)))))));
                }
            }
        }
    }
    var_14 -= (max((min((var_2 || 12804), var_0)), var_2));
    var_15 = (var_7 == var_4);
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 25;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 24;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 25;i_5 += 1)
            {
                {
                    var_16 = ((((max((min(var_1, var_8)), var_8))) <= 3));
                    var_17 += var_11;
                    arr_18 [i_3] = (!30);
                    arr_19 [i_5] [i_4] = -var_9;
                }
            }
        }
    }
    #pragma endscop
}
