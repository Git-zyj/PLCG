/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231747
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231747 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231747
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned long long int) (+(((/* implicit */int) ((signed char) ((((/* implicit */_Bool) 1012573382)) ? (577772798U) : (((/* implicit */unsigned int) ((/* implicit */int) var_0)))))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 3; i_0 < 7; i_0 += 1) 
    {
        for (unsigned int i_1 = 2; i_1 < 8; i_1 += 2) 
        {
            {
                arr_7 [i_0] [i_0] = ((/* implicit */unsigned char) (-(((unsigned long long int) arr_0 [i_0 - 2] [i_0]))));
                var_17 ^= ((/* implicit */short) var_8);
            }
        } 
    } 
    var_18 = ((/* implicit */_Bool) var_3);
    var_19 = ((/* implicit */unsigned int) var_2);
    var_20 = ((/* implicit */short) var_12);
}
