/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40337
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40337 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40337
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        var_20 = ((/* implicit */unsigned short) min((var_20), (((/* implicit */unsigned short) arr_1 [i_0]))));
        arr_2 [i_0] = ((/* implicit */short) max(((-((-(((/* implicit */int) (unsigned short)20932)))))), (((/* implicit */int) (unsigned short)20932))));
    }
    var_21 ^= ((/* implicit */int) (-(var_8)));
    var_22 = ((/* implicit */unsigned char) ((var_18) > (((/* implicit */unsigned long long int) max((var_9), ((-(((/* implicit */int) (unsigned short)23273)))))))));
}
