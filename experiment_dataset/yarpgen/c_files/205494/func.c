/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205494
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 |= ((/* implicit */unsigned char) (!(((/* implicit */_Bool) (short)(-32767 - 1)))));
    var_17 = ((/* implicit */unsigned long long int) max((var_17), (((/* implicit */unsigned long long int) var_8))));
    /* LoopSeq 2 */
    /* vectorizable */
    for (unsigned char i_0 = 3; i_0 < 16; i_0 += 2) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned char) (((_Bool)1) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_0 [i_0]))) : (((((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)149))) % (var_14)))));
        var_18 = ((/* implicit */unsigned short) var_11);
    }
    /* vectorizable */
    for (unsigned char i_1 = 0; i_1 < 21; i_1 += 1) 
    {
        var_19 = ((/* implicit */signed char) ((((/* implicit */_Bool) var_10)) ? (((/* implicit */long long int) ((/* implicit */int) ((-6187720089521776746LL) == (((/* implicit */long long int) var_4)))))) : (((long long int) var_7))));
        arr_6 [i_1] = (!(((/* implicit */_Bool) arr_5 [i_1] [i_1])));
    }
    /* LoopSeq 1 */
    for (unsigned int i_2 = 0; i_2 < 20; i_2 += 4) 
    {
        var_20 -= ((/* implicit */unsigned long long int) var_5);
        arr_11 [i_2] = ((/* implicit */_Bool) (-(((((/* implicit */_Bool) arr_7 [i_2] [i_2])) ? (11096979155610680878ULL) : (((((/* implicit */unsigned long long int) arr_4 [19LL] [19LL])) % (var_10)))))));
    }
}
