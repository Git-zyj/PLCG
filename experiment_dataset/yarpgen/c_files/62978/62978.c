/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62978
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62978 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62978
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    var_13 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = (((((((((arr_1 [i_0] [i_1]) || 1)))) >= ((65535 ? (arr_1 [i_0] [7]) : var_0)))) ? (((((max((arr_2 [1] [i_0]), var_9)) == ((min(0, var_9))))))) : (arr_3 [i_0])));
                var_14 = (min((arr_2 [i_0] [i_0]), ((((arr_4 [i_0] [5]) ? 128 : 4623435657154964545)))));
            }
        }
    }
    var_15 = var_5;
    #pragma endscop
}
