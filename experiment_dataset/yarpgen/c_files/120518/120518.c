/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120518
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120518 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120518
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 |= ((((max(var_8, (var_7 > var_6)))) >> ((((~var_4) != (~1162040092))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_7 [16] [i_1] [i_1] |= (((~(arr_5 [i_0 - 1] [i_0]))));
                var_13 *= (var_7 ? var_1 : (min((var_4 - 1), 1)));
                var_14 += min(var_5, (((~(((!(arr_4 [i_0]))))))));
            }
        }
    }
    var_15 |= var_6;
    var_16 = ((~(max((max(var_5, var_2)), var_1))));
    var_17 = (!var_4);
    #pragma endscop
}
