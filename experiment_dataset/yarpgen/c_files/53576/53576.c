/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53576
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53576 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53576
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    var_12 = (max(var_12, var_6));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_0] = (min(((var_5 ? -3240395695613045853 : (arr_0 [i_0 - 1]))), ((((arr_0 [i_1]) && (((var_1 ? var_9 : var_3))))))));

                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    arr_9 [8] [i_1] [i_1] = (((((min(var_0, (max((arr_7 [i_0 - 2] [i_1] [i_1] [i_2]), var_9)))))) - (max(-4656361858101394871, (arr_3 [i_1] [i_1])))));
                    arr_10 [i_1] = (!1);
                }
            }
        }
    }
    #pragma endscop
}
