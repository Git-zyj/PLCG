/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174914
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174914 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174914
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = (+(var_10));
    /* LoopNest 3 */
    for (unsigned short i_0 = 2; i_0 < 17; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 19; i_1 += 4) 
        {
            for (unsigned short i_2 = 0; i_2 < 20; i_2 += 1) 
            {
                {
                    var_19 ^= ((/* implicit */signed char) arr_5 [i_0] [i_1]);
                    var_20 = ((/* implicit */long long int) min((var_20), (((/* implicit */long long int) min((((/* implicit */int) ((unsigned short) arr_0 [i_0 + 2]))), (max((arr_1 [i_0 + 2]), (arr_3 [i_0] [i_0] [i_0]))))))));
                    var_21 += ((/* implicit */unsigned short) (+((+(-1LL)))));
                    var_22 = ((signed char) (+(arr_1 [i_0 - 1])));
                    arr_8 [i_0] [i_1] [i_1] [i_2] = ((/* implicit */long long int) 16ULL);
                }
            } 
        } 
    } 
    var_23 = ((/* implicit */unsigned short) ((short) var_3));
}
