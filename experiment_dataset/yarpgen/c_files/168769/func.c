/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168769
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168769 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168769
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */_Bool) var_2);
    var_16 = ((/* implicit */unsigned short) ((short) ((((/* implicit */_Bool) min(((unsigned short)35837), (((/* implicit */unsigned short) (unsigned char)235))))) ? (((/* implicit */int) var_6)) : (((/* implicit */int) min((var_4), ((_Bool)1)))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        var_17 = ((/* implicit */int) max((var_17), (((/* implicit */int) var_11))));
        arr_2 [(unsigned short)2] [i_0] = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) arr_1 [i_0]))));
    }
}
