/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109766
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109766 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109766
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    var_12 -= 0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_1] = (min((arr_3 [i_1] [i_1]), (arr_1 [i_0])));
                var_13 = (min(var_13, ((((((var_4 > (arr_3 [i_1] [i_1]))) ? (arr_2 [14]) : (arr_1 [i_1])))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 14;i_3 += 1)
        {
            {
                arr_11 [i_2] = ((!(((-(arr_3 [i_2] [i_3 + 2]))))));
                arr_12 [i_3 + 1] = ((+((((max((arr_3 [i_2] [i_3 + 2]), var_6))) ? (arr_7 [i_2]) : (((arr_6 [6]) ? var_4 : var_5))))));
            }
        }
    }
    var_14 = var_10;
    #pragma endscop
}
