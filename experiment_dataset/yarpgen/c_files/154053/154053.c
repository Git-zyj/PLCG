/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154053
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154053 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154053
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((!var_3) ? (!var_6) : (var_11 < var_7)))) ? ((min(var_2, (24 == var_3)))) : (((!(((var_2 ? var_7 : var_8))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    var_13 -= ((((((((6075 ? 16777215 : -6076))) ? (12199269467205566437 < 25) : ((min(6075, (arr_7 [i_0] [i_1] [i_2] [i_1]))))))) <= 1347495231114543305));
                    arr_8 [10] [i_1] [i_2] = 47;
                }
                var_14 ^= var_1;
                var_15 = (((!3368465919) >> (((arr_0 [i_0] [i_0]) + 5585216064868746344))));
            }
        }
    }
    #pragma endscop
}
