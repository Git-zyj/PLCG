/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246439
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246439 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246439
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((((var_1 & -102) ? ((50 ? var_7 : var_14)) : var_8))) ? var_8 : (min((~3739271214), 1))));

    for (int i_0 = 1; i_0 < 17;i_0 += 1) /* same iter space */
    {
        var_16 = ((((1 == ((37 ? -37 : -2494375993846005111)))) ? var_6 : (((((arr_2 [i_0 + 1] [i_0]) && 1))))));
        var_17 = ((((((arr_0 [i_0 - 1] [i_0 - 1]) ? (arr_0 [i_0 - 1] [i_0 - 1]) : (arr_0 [i_0 + 1] [i_0 + 2])))) ? (arr_0 [i_0 + 1] [i_0 - 1]) : ((((arr_0 [i_0 + 2] [i_0 + 1]) || (arr_0 [i_0 + 1] [i_0 - 1]))))));
        var_18 = (((!var_3) ? ((((arr_1 [i_0] [i_0 - 1]) ? 127 : (arr_0 [i_0 + 2] [i_0 + 2])))) : (max((arr_1 [i_0] [i_0]), (arr_2 [i_0] [14])))));
    }
    for (int i_1 = 1; i_1 < 17;i_1 += 1) /* same iter space */
    {
        var_19 = (arr_3 [i_1] [i_1]);
        var_20 ^= 6978247540650761758;
        var_21 = (arr_2 [1] [i_1 - 1]);
    }
    var_22 = var_8;
    #pragma endscop
}
