/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35945
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35945 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35945
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        var_11 = (arr_0 [i_0]);
        var_12 = ((((arr_1 [i_0 + 1]) ? (arr_1 [i_0]) : (((arr_0 [i_0 + 1]) ? (arr_2 [i_0]) : var_3)))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_7 [i_1] = ((arr_2 [i_1]) * (((!(arr_5 [i_1] [i_1])))));
        var_13 = (((((-(!var_0)))) ? ((((min((arr_0 [i_1]), var_1)) > (var_9 >= var_10)))) : (arr_6 [i_1])));
    }
    var_14 = ((+((var_0 ? (var_0 && var_0) : ((var_7 ? var_8 : var_7))))));
    var_15 &= var_5;
    #pragma endscop
}
