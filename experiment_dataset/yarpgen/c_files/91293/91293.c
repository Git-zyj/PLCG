/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91293
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91293 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91293
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_17 = (57 < 47881);
                                var_18 = ((-78 ? (((((-(arr_8 [i_0] [i_0] [i_0] [i_0])))) ? (arr_7 [i_0] [i_2] [15] [2]) : 78)) : (arr_0 [i_0] [i_1])));
                            }
                        }
                    }
                    var_19 = ((((((((arr_11 [0] [i_1] [i_1] [13]) * -128))) ? (arr_3 [i_2]) : (~803755992))) != (arr_8 [i_0] [17] [i_1] [17])));
                    var_20 = (arr_3 [i_2]);
                }
            }
        }
        var_21 = (arr_10 [4] [i_0] [i_0]);
        var_22 ^= 5010505408945665884;
    }
    var_23 = var_4;
    #pragma endscop
}
