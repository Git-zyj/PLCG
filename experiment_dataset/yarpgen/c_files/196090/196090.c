/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_9;

    for (int i_0 = 3; i_0 < 20;i_0 += 1)
    {
        var_19 &= (min(var_11, (var_10 >= var_5)));
        arr_2 [12] &= var_16;
        var_20 = (max(var_20, -25));
        var_21 ^= (((max((max(var_10, var_4)), var_4))) && (var_2 - var_4));
        var_22 = (min(var_22, ((max(10353, -10362)))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_23 = var_2;
        /* LoopNest 2 */
        for (int i_2 = 2; i_2 < 14;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 14;i_3 += 1)
            {
                {
                    var_24 = (max(var_24, var_3));
                    var_25 += (min(((min(var_1, -10353))), 7484004688182790418));
                    var_26 &= var_12;
                }
            }
        }
        arr_11 [i_1] = var_13;
        var_27 = var_2;
    }
    #pragma endscop
}
