/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102005
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((-((max(var_8, var_3)))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    var_11 ^= (~1);
                    var_12 = (max(0, -1));
                    var_13 = (min(var_13, var_4));
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 12;i_4 += 1)
                        {
                            {
                                arr_15 [i_3] = ((-((-(arr_10 [i_1] [i_3] [i_1] [i_1] [i_2 + 1] [i_1])))));
                                arr_16 [i_3] [i_3] = (((min(((-127 + (arr_1 [i_3]))), (!1))) > var_8));
                            }
                        }
                    }
                    var_14 = ((var_0 ? (((((((arr_14 [i_0] [i_0] [i_0] [i_0] [i_0]) + 1714192380)) <= ((((!(arr_13 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0]))))))))) : (((var_4 * 0) - var_5))));
                }
            }
        }
    }
    var_15 = (min(var_15, var_7));
    #pragma endscop
}
