/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84935
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84935 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84935
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((~((var_3 ? var_14 : (var_12 + var_2)))));
    var_16 = ((((((-48 ? 5423772631264800018 : 16911433728)) ^ ((min(63881, -5423772631264800019))))) & var_1));
    var_17 = var_10;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_18 = ((+((+((((arr_1 [i_0]) != (arr_0 [i_0]))))))));
        arr_3 [i_0] = ((+(((((((arr_0 [i_0]) ? (arr_1 [5]) : (arr_0 [16])))) && (((arr_1 [13]) || (arr_0 [6]))))))));
    }
    #pragma endscop
}
