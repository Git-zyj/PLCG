/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99422
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99422 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99422
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_16 = (min(var_16, (min((arr_7 [i_1 - 1] [i_1]), (~190)))));
                    arr_9 [i_1] [i_1 + 1] [i_2] = max(-var_5, (((!(!114)))));
                    arr_10 [i_1] [i_1] = 255;
                    var_17 = ((~((~(arr_6 [i_1 - 1] [i_1] [i_1 + 1] [i_1 - 1])))));
                }
            }
        }
    }
    var_18 = (max(((((~-7) >> ((var_13 ? 0 : var_2))))), (max(-var_14, var_3))));
    #pragma endscop
}
