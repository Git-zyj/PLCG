/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138901
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138901 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138901
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_14;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = ((-(((arr_2 [i_0]) / (arr_5 [i_0] [i_1])))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_12 [i_3] [i_1] [i_0] = (((~(arr_6 [i_2] [i_1]))));
                            arr_13 [i_0 - 1] [i_1] [i_1] [i_1] [i_0 - 1] = -var_9;
                            arr_14 [i_0 - 1] [i_1] = (arr_2 [i_3 + 1]);
                        }
                    }
                }
                arr_15 [i_1] = 1;
                arr_16 [i_1] = ((!((((arr_9 [4] [i_1 - 1] [i_1 - 1] [i_0]) * (arr_9 [i_0] [i_1 - 1] [i_1 - 1] [i_0]))))));
            }
        }
    }
    #pragma endscop
}
