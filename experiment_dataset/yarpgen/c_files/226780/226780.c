/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226780
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226780 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226780
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_10;

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        arr_3 [i_0] = var_9;
        var_12 = (arr_1 [i_0] [i_0]);
        arr_4 [i_0] [i_0] = max(((var_4 ? 214 : (arr_0 [i_0]))), (((-((var_10 / (arr_1 [i_0] [i_0])))))));
    }
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        var_13 = (max(var_13, ((min(((((((arr_1 [i_1] [i_1]) && var_9)) ? (~34964) : ((((arr_0 [9]) != 41)))))), 6975099480995549961)))));
        var_14 = (max((arr_5 [10]), ((((-(arr_2 [i_1]))) * ((var_4 ? (arr_5 [1]) : -32739))))));
    }
    #pragma endscop
}
