/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233273
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(((((34922 ? 111 : var_3)) + var_1)), (!var_3)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (((((max(var_0, (arr_3 [11])))) ? var_8 : ((-94 ? 34943 : (arr_2 [i_0]))))));
                var_14 = ((!((!((((arr_1 [i_0] [i_1]) ? 30593 : 1)))))));
                arr_7 [i_0] [i_0] = var_5;
            }
        }
    }

    for (int i_2 = 0; i_2 < 11;i_2 += 1)
    {
        var_15 = (min(var_15, ((((((var_2 % var_9) <= (arr_9 [i_2]))) ? var_6 : (arr_4 [i_2] [8]))))));
        arr_10 [i_2] = (arr_9 [i_2]);
    }
    var_16 &= max(var_0, (min(((var_3 >> (255 - 228))), var_11)));
    #pragma endscop
}
