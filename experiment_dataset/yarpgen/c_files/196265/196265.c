/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196265
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196265 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196265
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_13 = (-(((arr_0 [i_0]) ? (arr_1 [i_0] [i_0]) : (arr_0 [18]))));
        var_14 ^= ((!(arr_1 [i_0] [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        var_15 = (arr_1 [i_1] [i_1]);
        var_16 = (arr_0 [i_1]);
        var_17 = ((arr_0 [i_1]) ? ((((arr_1 [i_1] [i_1]) ? (arr_0 [i_1]) : 0))) : (~16777215));
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            for (int i_3 = 1; i_3 < 1;i_3 += 1)
            {
                for (int i_4 = 2; i_4 < 16;i_4 += 1)
                {
                    {
                        var_18 = arr_4 [i_4];
                        var_19 = (+-1);
                        var_20 ^= (((-(arr_6 [6] [i_2] [i_4]))) % ((((arr_2 [i_1] [i_1]) || (-2147483647 - 1)))));
                    }
                }
            }
        }
    }
    var_21 = ((((1 ^ 8167939297936213014) & var_2)));
    var_22 = var_8;
    #pragma endscop
}
