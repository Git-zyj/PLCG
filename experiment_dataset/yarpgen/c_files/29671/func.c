/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29671
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29671 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29671
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 *= ((/* implicit */short) var_5);
    /* LoopNest 3 */
    for (short i_0 = 2; i_0 < 14; i_0 += 3) 
    {
        for (signed char i_1 = 4; i_1 < 13; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 16; i_2 += 3) 
            {
                {
                    arr_8 [i_0] [i_1] [(unsigned char)9] [i_2] = ((/* implicit */unsigned char) min((((/* implicit */unsigned long long int) max((var_12), (var_7)))), (var_9)));
                    var_14 = ((/* implicit */long long int) max((((/* implicit */int) max((((/* implicit */short) var_7)), (((short) var_12))))), ((+(((((/* implicit */_Bool) var_6)) ? (((/* implicit */int) var_12)) : (((/* implicit */int) var_7))))))));
                    var_15 = (!(var_7));
                }
            } 
        } 
    } 
}
