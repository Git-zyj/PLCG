/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34082
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34082 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34082
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max((((var_0 == (min(var_11, var_8))))), (((-153523771524509343 ? -6206799609089472295 : 30395)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = ((-((((((arr_2 [i_0 - 2]) ? var_2 : (arr_0 [i_0] [1]))) == 335986283)))));
                var_17 += ((((max((arr_4 [i_1]), (arr_4 [i_1])))) || (!53582)));
                arr_6 [i_0] = (arr_2 [i_0]);
                var_18 = ((~(arr_1 [i_0 - 2] [i_0])));
            }
        }
    }
    #pragma endscop
}
