/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51748
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51748 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51748
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(((max(63, -63))), ((var_2 ? ((-1237222101 ? 0 : 524955561102391643)) : var_10))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_13 |= ((-((min(var_2, -72)))));
                var_14 = (max((max(((-1985885166069265971 & (arr_4 [i_1] [2] [i_1]))), (arr_0 [i_0] [i_0]))), ((min(-2656, 2147483647)))));
                arr_5 [i_0] [i_0] [i_1] = (min(((2147483647 ? 26 : 63)), (arr_1 [1])));
            }
        }
    }
    #pragma endscop
}
