/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191813
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191813 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191813
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_13, var_2));
    var_14 -= (max((max(((0 ? 7535 : var_3)), var_2)), 1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_15 = ((var_0 ? (((min((arr_0 [i_0]), (arr_0 [i_0]))))) : (min((arr_3 [i_0]), (arr_1 [i_0])))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_16 = (max(var_16, ((min((((min(var_8, 3968)))), (arr_3 [1]))))));
                            var_17 = (~1);
                            var_18 = (max(((max((arr_6 [i_0] [19] [i_2] [i_2] [i_3] [17]), (min((arr_0 [i_2]), var_3))))), ((var_2 ? ((max(var_11, var_2))) : ((min(31, 1)))))));
                        }
                    }
                }
            }
        }
    }
    var_19 = var_11;
    #pragma endscop
}
