/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33804
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33804 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33804
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    var_13 = (min((min((var_4 - var_6), (((0 ? -21904 : 21903))))), (var_7 + 1)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_1] [i_2] = (min(var_7, 21903));
                    var_14 = (min((((-((~(arr_8 [2])))))), (((var_10 <= 21887) ? (((min(-21904, (arr_1 [i_0] [i_0]))))) : 17592184995840))));
                    arr_10 [i_0] [i_1] [i_1] [i_2] = (arr_4 [i_0] [i_1] [i_2]);
                }
            }
        }
    }
    var_15 = var_10;
    #pragma endscop
}
