/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82751
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82751 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82751
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((var_8 ? (min(var_5, var_6)) : 25396)) / -var_3));
    var_12 = ((-4502423898550347193 ? 4 : 4502423898550347193));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_13 = -var_7;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            var_14 = (max(var_14, var_3));
                            var_15 |= (((-(arr_3 [i_0 - 2] [i_0 + 4] [i_0 + 1]))) & 8064);
                        }
                    }
                }
                arr_11 [18] [i_0] [i_1] = ((+(((arr_7 [i_0 + 4] [i_0 + 3] [i_0] [i_0]) - (arr_10 [i_0 - 1] [i_0 - 1] [i_0] [1])))));
            }
        }
    }
    #pragma endscop
}
