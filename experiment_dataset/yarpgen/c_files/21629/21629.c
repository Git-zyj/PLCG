/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21629
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21629 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21629
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 23;i_2 += 1)
            {
                {
                    var_11 = ((((min((arr_2 [i_1 - 3] [i_1 - 3]), (arr_2 [i_1 + 1] [i_1 + 1])))) != ((max(var_6, (arr_4 [i_1 - 2] [i_1 - 3]))))));
                    arr_7 [17] [i_1] [i_2 - 1] = 3644166359;
                    var_12 = (((((239 ? 1 : 234))) ? (min(-30418, ((((arr_0 [i_0]) ? var_9 : var_10))))) : ((((((arr_1 [i_0] [i_0]) ? 8191 : 57345)) | -1060919915)))));
                }
            }
        }
    }
    var_13 -= ((var_0 << (var_2 - 1)));
    #pragma endscop
}
