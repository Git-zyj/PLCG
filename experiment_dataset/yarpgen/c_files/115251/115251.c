/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115251
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115251 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115251
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 |= var_6;
    var_11 = (-4503597479886848 < var_3);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_12 = (min(var_12, -4503597479886848));
                arr_6 [i_0] = (min(4503597479886848, ((min(-var_7, (var_8 == -9223372036854775793))))));
                arr_7 [i_0] [i_0] [i_0] = -21;
                var_13 = (min(var_13, ((min(((!(arr_4 [i_0] [i_1]))), (4503597479886867 && 4503597479886867))))));
            }
        }
    }
    #pragma endscop
}
