/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29635
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29635 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29635
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_14 |= (((arr_0 [i_0] [i_0]) && (arr_0 [i_0] [i_0])));
        var_15 = 0;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 10;i_3 += 1)
                {
                    {
                        var_16 = (((((108 != (arr_5 [i_0] [i_1] [i_3])))) != (arr_10 [i_1] [i_2] [i_3])));
                        var_17 = var_1;
                        arr_12 [i_0] [i_0] [i_2] [i_0] [i_2] [i_0] = 1652740650;
                    }
                }
            }
        }
    }
    var_18 = max((min(var_0, -49)), var_11);
    var_19 = (max(((var_9 ? var_2 : var_0)), var_12));
    var_20 *= (min(var_9, ((((var_4 != var_11) != ((max(var_2, var_8))))))));
    #pragma endscop
}
