/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148895
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) ((unsigned char) (!(((/* implicit */_Bool) ((((/* implicit */int) var_4)) / (((/* implicit */int) var_13))))))));
    /* LoopNest 3 */
    for (unsigned char i_0 = 3; i_0 < 18; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 21; i_1 += 3) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_15 ^= ((/* implicit */unsigned short) (!((((!(var_0))) && (((var_3) || (((/* implicit */_Bool) var_9))))))));
                    var_16 = ((/* implicit */unsigned char) (((~(((/* implicit */int) arr_5 [i_0 - 1])))) & (((/* implicit */int) var_8))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned long long int) var_11);
    var_18 *= ((/* implicit */short) ((((/* implicit */int) var_7)) + (((/* implicit */int) ((_Bool) var_1)))));
    var_19 = ((/* implicit */unsigned char) var_13);
}
