/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226741
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226741 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226741
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = max(9827701102740315167, 0);
    var_15 = (var_6 ? (min((!15), var_2)) : ((~(~var_7))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [11] [3] [i_1] |= (min(((max((arr_3 [i_0 + 1] [i_0 + 1] [i_1]), (arr_3 [i_0 + 1] [i_0 + 1] [i_0 - 1])))), 6963113808716511657));
                arr_5 [i_0] [i_1] = (arr_1 [12]);
            }
        }
    }
    var_16 = (var_11 >= var_11);
    #pragma endscop
}
