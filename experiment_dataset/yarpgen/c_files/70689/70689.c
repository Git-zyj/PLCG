/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70689
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70689 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70689
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 |= ((-1180434203 ? (127 % var_5) : ((var_12 ? (9203415920007921694 > 1310869240) : (!var_5)))));
    var_20 = ((~(!1180909298)));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_21 = (~((~(arr_1 [i_0 + 1] [i_0 - 2]))));
                var_22 = ((~(min(((2567966066 ? (arr_6 [1] [1]) : 4039048871)), (min(2145530700, var_12))))));
            }
        }
    }
    var_23 = var_7;
    #pragma endscop
}
