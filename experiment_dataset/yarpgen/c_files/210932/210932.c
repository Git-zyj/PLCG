/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210932
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210932 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210932
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_0] [6] = (((var_4 <= (arr_5 [i_2] [i_0] [i_0]))));
                    arr_9 [i_2] [i_2] [i_2] [14] = ((+(((arr_2 [i_1] [i_1] [i_2]) & ((7641 ? var_2 : (arr_3 [i_0] [i_1 - 1])))))));
                    arr_10 [i_0] [i_2] = (((arr_5 [8] [i_1 - 1] [8]) ? (((arr_5 [i_0] [i_1 - 1] [i_1 - 1]) ? (arr_5 [i_2] [i_1 - 1] [i_1]) : (arr_5 [i_0] [i_1 - 1] [i_2 - 1]))) : (arr_5 [i_1] [i_1 - 1] [i_1])));
                }
            }
        }
    }
    #pragma endscop
}
