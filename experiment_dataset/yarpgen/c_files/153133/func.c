/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153133
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153133 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153133
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */signed char) (-(((((/* implicit */int) ((short) var_3))) - ((-(((/* implicit */int) var_7))))))));
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 18; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 4; i_1 < 17; i_1 += 2) 
        {
            for (short i_2 = 1; i_2 < 16; i_2 += 4) 
            {
                {
                    var_12 = min((((/* implicit */unsigned long long int) ((int) ((((/* implicit */_Bool) var_2)) && (((/* implicit */_Bool) var_6)))))), (min((((/* implicit */unsigned long long int) ((((/* implicit */int) var_7)) - (((/* implicit */int) var_4))))), (((((/* implicit */unsigned long long int) ((/* implicit */int) var_7))) - (var_8))))));
                    arr_8 [i_0] [i_2] [i_0] = ((/* implicit */int) (-(var_8)));
                }
            } 
        } 
    } 
    var_13 = var_5;
    var_14 = ((/* implicit */unsigned short) ((long long int) var_2));
}
