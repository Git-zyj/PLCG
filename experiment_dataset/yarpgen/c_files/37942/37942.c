/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 37942
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=37942 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/37942
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 += ((((((var_12 != var_6) ? ((max(56, 36559))) : var_9))) ? var_3 : 255));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_15 ^= (581464124 < 0);
                var_16 -= (arr_3 [i_0]);
                var_17 |= (!0);
                var_18 -= (((((var_4 ? -1552618796 : (((arr_2 [i_0] [i_1] [i_1]) ? 6 : var_3))))) || ((max(((581464132 ? (arr_3 [i_1]) : 581464124)), (!3713503172))))));
            }
        }
    }
    var_19 = ((((max(-var_12, -14887))) ? var_2 : var_10));
    var_20 = (((~((-14954 ? 0 : 1)))));
    var_21 += ((~(((((min(1, var_0))) && 1)))));
    #pragma endscop
}
