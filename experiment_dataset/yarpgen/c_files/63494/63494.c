/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    var_17 = (max(var_17, ((((arr_3 [i_0] [i_0] [i_2]) ? (arr_6 [i_0] [i_1] [i_2]) : (arr_0 [i_0]))))));
                    arr_7 [i_2] [i_1] [i_0] = ((arr_4 [i_0] [i_0]) ? (arr_1 [i_0]) : (max(((var_10 ? (arr_5 [i_0]) : (arr_1 [i_0]))), (arr_3 [i_1] [i_0] [i_0]))));
                }
            }
        }
    }
    var_18 = (min(var_18, var_13));
    var_19 = (((~var_11) != var_16));
    var_20 = (max(var_20, var_12));
    var_21 = ((~(-127 - 1)));
    #pragma endscop
}
