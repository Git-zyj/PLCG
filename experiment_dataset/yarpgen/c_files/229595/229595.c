/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229595
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229595 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229595
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_12 &= (min(((((-9223372036854775807 - 1) ? (12 == var_0) : (var_9 - -32747)))), (((arr_4 [0]) / (arr_3 [i_0])))));
                var_13 = (((((!(arr_3 [i_0])))) & ((((var_1 ? var_1 : 26586))))));
                var_14 = (((74 == 88) != ((((-2147483647 - 1) && (arr_2 [i_0]))))));
                arr_7 [i_1] [i_0] = ((+((32736 & (((arr_3 [i_0]) & (arr_2 [i_0])))))));
            }
        }
    }
    var_15 = 32758;
    #pragma endscop
}
