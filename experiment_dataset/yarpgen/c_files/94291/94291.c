/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94291
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94291 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94291
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_17 = (((arr_3 [i_0] [i_1] [i_1]) ? (arr_0 [i_1]) : (max((max(1, (arr_4 [i_1] [7] [i_0]))), ((max(-3699, 48062)))))));
                var_18 = (max(var_18, 967100163));
            }
        }
    }
    var_19 = ((((((((var_5 ? 5611 : var_2))) ? 1 : var_9))) ? var_11 : (max(-23139, 13915046224663353685))));
    #pragma endscop
}
