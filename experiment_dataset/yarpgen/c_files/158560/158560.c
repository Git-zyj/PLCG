/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158560
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158560 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158560
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_15 = (max(((min((arr_3 [i_1]), (((arr_2 [i_0]) ? (arr_0 [i_1] [i_1]) : (arr_0 [i_1] [i_0])))))), (((arr_1 [i_0]) ? (((1 ? -201843788 : 1))) : (arr_1 [i_0])))));
                arr_4 [i_1] = (arr_3 [i_1]);
                arr_5 [i_0] [i_1] = (max(((-((16093394055423575904 ? 582427148225632052 : 582427148225632052)))), ((((arr_3 [i_0]) ? var_1 : (arr_3 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
