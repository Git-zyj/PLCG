/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77962
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77962 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77962
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_11 = (+(((arr_3 [i_0] [i_1]) ? (max(var_10, var_9)) : -5198736975969434406)));
                var_12 = (min(var_12, ((min((((!(((-(arr_3 [i_0] [i_0]))))))), var_1)))));
                var_13 = (max(var_13, (((((arr_1 [i_1]) ? (~var_9) : ((min((arr_3 [i_0] [i_0]), var_1)))))))));
            }
        }
    }
    var_14 = var_1;
    var_15 |= -109;
    var_16 = var_9;
    #pragma endscop
}
