/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108585
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108585 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108585
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [i_0] = 1095769845;
        arr_3 [i_0] = (max((arr_1 [i_0] [i_0]), (0 >> 51)));
    }
    var_12 = 78;
    var_13 = var_1;
    var_14 = 0;
    var_15 = ((((min(-var_2, var_0))) ? (max((max(var_4, var_9)), ((min(var_4, -3239524491659475264))))) : ((8191 | (min(8796093022207, var_7))))));
    #pragma endscop
}
