/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222434
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222434 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222434
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] = (arr_0 [i_0 + 1]);
        arr_3 [i_0 - 1] [i_0 - 1] = (min(1915349016, var_5));
        var_15 = (max(var_15, (((((max(2379618278, var_9))) >= ((((((arr_0 [i_0]) ? var_0 : 2379618279))) ? (1073741824 * 2379618279) : var_10)))))));
    }
    var_16 = (((min(2379618279, 1)) > var_9));
    var_17 = (max(var_17, (((((max(1, 1))))))));
    #pragma endscop
}
