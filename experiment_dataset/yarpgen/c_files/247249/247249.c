/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247249
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247249 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247249
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_3;
    var_12 = (var_8 ? ((((var_9 ? 31206 : 134217727)) >> (var_1 != 6))) : (((!(((1055713162 ? var_10 : 0)))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_0] = (arr_0 [i_0] [i_0]);
                arr_8 [i_0] [i_0] = (arr_0 [i_0 + 1] [i_0]);
                arr_9 [i_0] [i_1] [i_0] = (arr_2 [i_0 - 1]);
                var_13 = (arr_3 [i_0] [i_0]);
                arr_10 [i_0] [i_0 + 1] [i_1] = ((arr_5 [i_1] [6]) ? ((((1 < (arr_5 [i_0] [1]))))) : ((((16 >> 1) >= (((((arr_3 [i_1] [i_1]) != (arr_2 [i_1])))))))));
            }
        }
    }
    #pragma endscop
}
