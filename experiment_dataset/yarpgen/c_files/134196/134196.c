/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134196
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134196 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134196
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (var_1 * var_8);

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_8 [i_1] = ((((((max(var_6, var_0))) ? ((4294967294 ? 3027086812 : 1)) : (min((arr_1 [i_1 + 2] [i_2]), var_7)))) <= ((var_1 ? (arr_0 [i_0] [i_1 - 1]) : (arr_0 [i_1 + 1] [i_1 + 1])))));
                    var_17 = ((!(arr_4 [i_1 + 2] [i_1] [i_1 - 1])));
                }
            }
        }
        arr_9 [i_0] = ((var_3 || ((min(36, ((16231442607371733034 ? 1447 : 1465)))))));
        arr_10 [17] = ((var_3 / var_8) ? ((-((var_0 ? var_14 : var_10)))) : (~190));
    }
    #pragma endscop
}
