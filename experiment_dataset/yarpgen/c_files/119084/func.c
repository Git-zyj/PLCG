/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119084
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119084 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119084
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */int) ((((/* implicit */_Bool) max(((-(9007199221186560LL))), (((/* implicit */long long int) ((int) var_7)))))) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (!(var_3))))) : ((-((-(var_6)))))));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 17; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 17; i_1 += 2) 
        {
            {
                arr_5 [i_0] [i_1] = ((/* implicit */unsigned int) (~((-(((/* implicit */int) var_3))))));
                var_12 = (!(((/* implicit */_Bool) var_1)));
            }
        } 
    } 
    var_13 = (+(((/* implicit */int) var_10)));
}
