/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166176
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166176 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166176
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) var_1);
    var_16 ^= ((/* implicit */unsigned short) ((unsigned long long int) (!(((/* implicit */_Bool) ((15728640U) % (((/* implicit */unsigned int) ((/* implicit */int) var_14)))))))));
    /* LoopNest 2 */
    for (short i_0 = 2; i_0 < 18; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 19; i_1 += 2) 
        {
            {
                arr_8 [i_0] [(unsigned short)5] = var_0;
                var_17 &= ((/* implicit */unsigned char) (+(((/* implicit */int) max((arr_4 [i_0 - 2]), (arr_4 [i_0 - 2]))))));
                var_18 ^= ((/* implicit */unsigned int) 7628270467900979384ULL);
                var_19 = ((/* implicit */long long int) min((var_19), (arr_3 [7U])));
                var_20 += ((/* implicit */unsigned short) ((long long int) var_12));
            }
        } 
    } 
}
