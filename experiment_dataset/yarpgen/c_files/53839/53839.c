/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53839
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53839 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53839
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_10 = (max(((-((var_4 ? 2931214934 : (arr_2 [i_0]))))), ((((arr_0 [i_0]) % 1008)))));
        arr_3 [i_0] = (arr_2 [i_0]);
    }
    for (int i_1 = 1; i_1 < 20;i_1 += 1)
    {
        var_11 = (arr_5 [i_1]);
        var_12 = ((((((min(var_0, var_0))) + (arr_4 [i_1]))) > var_6));
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 23;i_3 += 1)
            {
                {
                    var_13 = var_7;
                    var_14 &= ((var_0 > (max(-var_7, (((1 >= (arr_5 [i_2]))))))));
                    var_15 = (((((((var_4 || -1) >= (min(2122703810, (arr_6 [i_1] [i_1] [i_3]))))))) == (arr_7 [i_1])));
                    arr_12 [i_2] [i_1] = (max(2122703811, -2122703810));
                }
            }
        }
    }
    var_16 = (max(var_16, var_5));
    var_17 = (max(-0, var_8));
    #pragma endscop
}
