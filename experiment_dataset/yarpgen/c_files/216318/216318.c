/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 216318
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=216318 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/216318
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((~(((var_16 ^ var_9) | (var_0 & var_1)))));
    var_18 = (var_5 - var_2);

    for (int i_0 = 4; i_0 < 15;i_0 += 1)
    {
        var_19 = 0;
        var_20 *= ((-((((arr_0 [i_0 + 2]) != 1)))));
    }
    for (int i_1 = 3; i_1 < 12;i_1 += 1)
    {
        arr_6 [i_1] [1] = min((((arr_2 [i_1]) || (var_8 % 65535))), ((!(arr_3 [i_1]))));
        var_21 = (((min((arr_2 [i_1]), var_12)) >= ((((var_7 != (arr_0 [i_1 - 2])))))));
        var_22 = (max(var_22, (((0 * (((var_5 ? (arr_5 [1]) : 1))))))));
    }
    #pragma endscop
}
