/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60177
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(var_15, var_5));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((((var_0 ? (min((arr_0 [i_0]), (-2147483647 - 1))) : ((min(-2247, (arr_1 [i_0])))))) ^ ((-2147483625 ? (arr_0 [i_0]) : (arr_0 [i_0])))));
        var_16 = ((((max((((arr_1 [i_0]) ? var_14 : var_10)), (((1 ? var_11 : (arr_0 [i_0]))))))) ? (~var_2) : 32767));
        var_17 = (max(var_17, (arr_0 [i_0])));
        arr_3 [i_0] [i_0] = 4096;
    }
    #pragma endscop
}
