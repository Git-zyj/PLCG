/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        var_13 = (min(var_13, ((((var_10 ? (arr_0 [0] [i_0 + 1]) : (arr_0 [i_0] [12])))))));
        arr_2 [1] = arr_0 [i_0 + 2] [i_0];
    }
    var_14 = (743830016 ^ 6482);
    var_15 = (min(var_15, var_10));

    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        var_16 = (((min(-7377411601335054609, var_8)) - ((((arr_4 [i_1]) ? (arr_5 [6]) : ((((arr_4 [i_1]) / (arr_3 [i_1])))))))));
        var_17 = (arr_5 [10]);
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 16;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 19;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 19;i_4 += 1)
                {
                    {
                        arr_15 [i_1] [i_2 + 3] = var_7;
                        var_18 = ((arr_6 [0] [4]) ? ((((var_8 == (((((arr_7 [i_2]) != 0)))))))) : (((arr_6 [i_2 + 2] [i_2 + 3]) ? (max(1, (arr_8 [1]))) : (arr_5 [1]))));
                        var_19 = (min(var_19, (var_6 <= 1)));
                    }
                }
            }
        }
    }
    #pragma endscop
}
