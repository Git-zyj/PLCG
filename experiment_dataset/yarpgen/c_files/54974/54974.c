/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54974
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54974 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54974
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_0] = (!8176);
                var_14 += (((!(arr_4 [i_0] [i_1 - 1] [1]))) ? ((-(arr_1 [i_1 - 1]))) : 8176);
                var_15 = (min((~8188), (!13261595789452336182)));
            }
        }
    }
    var_16 |= ((-((-8177 ? (-1 || 13744419726215809470) : ((2122986663 ? -78 : var_2))))));
    var_17 = (min(0, -15));
    var_18 ^= ((945045753 && ((((max(58, -8177)) % 62292)))));
    #pragma endscop
}
