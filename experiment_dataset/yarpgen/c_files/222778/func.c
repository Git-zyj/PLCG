/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222778
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222778 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222778
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 *= ((/* implicit */unsigned long long int) var_4);
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 3; i_0 < 18; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 21; i_1 += 4) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_13 *= ((/* implicit */short) min((((((/* implicit */_Bool) ((var_11) | (((/* implicit */int) arr_9 [i_0] [(_Bool)1] [i_2]))))) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (short)-5370))) : (arr_1 [i_0 - 1]))), (((/* implicit */unsigned long long int) ((int) arr_0 [i_0] [i_0])))));
                    var_14 = (~(((/* implicit */int) arr_3 [i_0 - 2] [i_0 - 2] [i_0 - 2])));
                }
            } 
        } 
    } 
}
