/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_0] = ((-5 ? (17557826306048 != 1245705314) : 60));
                var_13 = (min(var_13, -6626078616996137163));
                var_14 = (max(-9223372036854775789, 1));
            }
        }
    }
    var_15 = (max(var_3, ((max(1245705340, 1245705314)))));
    var_16 = (((var_11 ^ 545377094) ? var_2 : -1563602278267146353));
    #pragma endscop
}
