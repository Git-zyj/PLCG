/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63127
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63127 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63127
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        var_15 = (var_12 > (((max((var_5 <= 2663224582), ((4152191000454296249 <= (arr_0 [i_0 + 1] [i_0]))))))));
        arr_2 [i_0] = 0;
        arr_3 [i_0] [i_0 + 1] = 120;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 14;i_3 += 1)
                {
                    {
                        var_16 = 75;
                        var_17 = (((((65 ? var_8 : 11168))) <= ((var_12 ? (arr_10 [10] [i_3 + 1] [1] [i_3]) : (arr_10 [i_3] [i_3 + 2] [14] [i_3])))));
                    }
                }
            }
        }
    }
    var_18 ^= ((((var_3 ? -var_12 : var_8)) + (((var_9 ? 108 : var_2)))));
    var_19 = -288229826395897856;
    #pragma endscop
}
