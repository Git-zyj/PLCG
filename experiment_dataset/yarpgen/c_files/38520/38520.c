/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38520
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38520 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38520
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((min(var_8, var_7))));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_17 = ((((min((arr_3 [i_0]), var_3)) >> var_2)));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    {
                        var_18 = (min(((((arr_10 [i_0] [i_1]) ? (arr_10 [2] [2]) : var_2))), ((((min(var_0, var_11))) ? ((var_14 >> (var_10 - 29259))) : var_10))));
                        var_19 = (((((arr_1 [i_0]) - (arr_0 [i_0] [i_0]))) - (min((min(var_1, var_6)), (var_1 & var_2)))));
                    }
                }
            }
        }
        var_20 += (((-2147483647 - 1) ? 8025 : 32766));
    }
    #pragma endscop
}
