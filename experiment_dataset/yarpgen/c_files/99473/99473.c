/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99473
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99473 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99473
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, ((((((var_3 != var_5) ? (min(var_8, var_7)) : var_4)) % ((12 ? var_1 : (((var_0 ? 12 : 5918))))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_4 [i_0] = (((arr_0 [i_0]) ^ (arr_0 [i_0])));

        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            var_11 = 1989142611;
            var_12 = (arr_5 [i_1 + 1] [i_1 + 1]);
            var_13 = ((arr_5 [i_0] [i_1 + 1]) ? var_9 : (arr_6 [i_0] [i_1 - 1] [1]));
        }
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            var_14 = (((arr_3 [i_0]) & (arr_3 [i_2])));
            var_15 = (arr_6 [i_0] [i_0] [i_0]);
        }
        var_16 = ((!((((arr_1 [i_0]) ^ (arr_6 [16] [16] [i_0]))))));

        for (int i_3 = 0; i_3 < 22;i_3 += 1)
        {
            arr_12 [i_3] [i_0] = ((-(!244)));
            arr_13 [i_0] [i_3] [i_0] = (((arr_2 [i_0]) == (arr_3 [i_0])));
            arr_14 [i_0] [i_3] = (arr_1 [i_3]);
        }
    }
    var_17 = (-6186303896404202410 < -287461091);
    var_18 = var_0;
    #pragma endscop
}
