/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181115
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181115 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181115
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((((-442559371 / 1) ? (min(var_0, 49655)) : 3429))) ? 14269 : 48));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [i_1] [i_2] [i_4] [i_4] = (min(((((41 ? -2909479619595125496 : 1)))), 3429));
                                var_12 = (arr_8 [i_1] [1] [6] [5]);
                                arr_15 [3] [i_2 - 1] [i_2 - 1] [9] [i_3] = (min(-434545974, 0));
                                arr_16 [i_0] [i_1] = 1;
                            }
                        }
                    }
                    var_13 |= 2075559246;
                    arr_17 [i_0] &= 3417;
                }
            }
        }
    }
    #pragma endscop
}
