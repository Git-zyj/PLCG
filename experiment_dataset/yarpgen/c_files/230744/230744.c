/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230744
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230744 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230744
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 *= (((min(1, 1)) ? (((var_6 + var_10) ? -var_4 : ((max(var_11, var_0))))) : (var_14 / 20603)));
    var_18 = (1 ^ 97);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_1] [i_0 - 1] &= ((-((min(var_11, (!var_2))))));
                    var_19 = (max(var_19, (((~((((max(-32747, 131071)) >= -32747))))))));
                }
            }
        }
    }

    /* vectorizable */
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        var_20 += (((arr_4 [i_3] [i_3] [i_3]) ? (arr_4 [i_3] [i_3] [i_3]) : (arr_4 [20] [i_3] [i_3])));
        var_21 = var_10;
    }
    #pragma endscop
}
