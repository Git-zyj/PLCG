/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139795
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139795 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139795
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (arr_0 [i_0]);
                arr_7 [i_0] = ((!((!(arr_0 [i_0])))));
            }
        }
    }
    var_20 = ((29741 ? 7752 : 0));
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 1;i_3 += 1)
        {
            {
                arr_15 [i_3] [i_2] = (((min(var_18, (((-(arr_14 [i_3] [i_3 - 1])))))) <= (((-(var_0 < var_1))))));
                arr_16 [i_2] [14] &= (min(1, -2075937522));
                arr_17 [i_2] [i_3] [i_3] = (((((var_10 ? (arr_8 [i_3 - 1] [i_3 - 1]) : (arr_8 [i_3 - 1] [i_3 - 1])))) ? (((~(arr_8 [i_3 - 1] [i_3 - 1])))) : var_11));
            }
        }
    }
    var_21 = (max(var_2, -var_10));
    #pragma endscop
}
