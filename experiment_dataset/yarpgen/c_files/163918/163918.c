/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163918
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163918 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163918
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((!(((9223372036854775807 ? -1231018051 : -1231018040)))));
    var_11 = ((!(((var_7 && var_4) || (((var_7 ? var_3 : -1231018066)))))));
    var_12 = (var_7 >= var_7);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_13 = (max(var_6, (((var_6 ? ((~(arr_3 [i_0] [6]))) : (~var_8))))));
                var_14 = ((max(((-1231018040 ? (arr_1 [i_1]) : var_5)), ((((arr_2 [i_0] [i_0]) * (arr_0 [i_0])))))));
            }
        }
    }
    #pragma endscop
}
