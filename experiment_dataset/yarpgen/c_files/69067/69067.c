/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69067
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69067 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69067
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 17;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                arr_16 [i_0] [i_2] [7] [i_3] [i_4] = (arr_13 [i_0] [i_1] [6] [i_1] [i_4]);
                                arr_17 [0] [i_1] [i_1] [0] [i_2] [7] [i_4] = (min(((min((arr_11 [i_2 - 1]), var_1))), ((var_3 ? var_0 : (arr_11 [i_2 - 2])))));
                            }
                        }
                    }
                    var_12 = ((255 ? ((((arr_1 [i_1]) ? (arr_14 [i_2] [i_2]) : var_3))) : var_5));
                }
            }
        }
    }
    var_13 = (1 != var_1);
    #pragma endscop
}
