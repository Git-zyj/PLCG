/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6104
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6104 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6104
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */signed char) max((0ULL), (((/* implicit */unsigned long long int) -270163624))));
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 9; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            {
                arr_6 [i_0] [i_0] = ((/* implicit */long long int) (((!(((/* implicit */_Bool) arr_3 [i_0 + 1] [i_0])))) && (((/* implicit */_Bool) arr_3 [i_0 - 1] [i_0]))));
                arr_7 [i_0] [i_0] = arr_3 [i_0] [i_1];
            }
        } 
    } 
    var_20 = ((/* implicit */unsigned char) var_1);
}
