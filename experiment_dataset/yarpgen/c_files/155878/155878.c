/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155878
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155878 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155878
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [1] [10] = 127;
                arr_6 [i_0] [1] [i_0] = (((arr_3 [i_0]) ? 3315 : (((-9 ? 9 : 8)))));
                var_13 = ((var_10 ? (((-32750 + 5) & (arr_3 [i_1]))) : (arr_4 [1] [i_1] [i_1])));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 0;i_2 += 1)
                {
                    arr_9 [i_0] [13] [1] = -32765;
                    arr_10 [5] [i_1] [i_0] = ((var_11 ? (arr_3 [i_2 + 1]) : var_12));
                    var_14 *= var_9;
                    arr_11 [i_2] [i_2] [0] = (arr_2 [i_2]);
                }
            }
        }
    }
    var_15 = ((((((~11) != ((1596860225 ? -77 : var_4))))) % var_0));
    var_16 = var_4;
    #pragma endscop
}
