/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3380
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3380 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3380
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_19 = (!((min((((!(arr_3 [i_0] [i_1] [i_1])))), (min(36046, 29494))))));
                var_20 = (min((((!(~var_18)))), ((-(arr_1 [i_0])))));
                var_21 |= (min((max((~var_4), -19680)), (max(((min(86, (arr_0 [i_1] [i_0])))), (~var_4)))));
            }
        }
    }
    var_22 = ((~(((!((min(var_16, var_15))))))));
    #pragma endscop
}
