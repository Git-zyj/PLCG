/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41472
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41472 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41472
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= ((((min(var_8, var_6))) > ((((min(var_11, var_7))) ? 58 : (var_0 >= 114)))));
    var_15 |= ((((127 ? -120 : -64))) - ((var_10 - (max(var_7, 17)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_16 *= (~-756682608);
                                arr_13 [i_0] [i_1] [i_2] [i_3] [i_4] = var_0;
                            }
                        }
                    }
                    var_17 = (max((max(127, ((var_1 ? var_2 : -115)))), (((!(((var_7 ? 756682608 : var_9))))))));
                }
            }
        }
    }
    #pragma endscop
}
