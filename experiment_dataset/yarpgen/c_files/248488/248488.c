/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248488
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248488 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248488
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                var_15 = var_6;
                arr_6 [i_0] [i_1] = -6405;
                var_16 |= ((((min((arr_1 [i_1]), (min((arr_1 [i_1]), 126))))) ? (((!979074697564742793) * (var_4 * var_5))) : var_8));
            }
        }
    }
    var_17 = var_4;
    var_18 = (((max((~var_7), var_11))) ? ((((((6409 ? var_5 : -117))) ? var_9 : (!-3121061389804881095)))) : ((-((-21435 ? var_11 : var_11)))));
    var_19 = var_4;
    #pragma endscop
}
