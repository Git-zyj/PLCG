/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185175
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185175 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185175
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_17 = ((((arr_7 [i_0]) <= (arr_5 [2] [i_0] [2]))));
                var_18 = (min(((min((arr_3 [i_1]), (arr_1 [i_1] [i_0])))), (max((arr_4 [i_0] [i_0] [i_1]), (max((arr_7 [i_0]), (arr_5 [i_0] [i_0] [i_1])))))));
            }
        }
    }
    var_19 = (!var_8);
    var_20 = ((1588641100 ? -1588641084 : 208));
    var_21 |= ((((var_15 % 1246808911) - 8281)));
    #pragma endscop
}
