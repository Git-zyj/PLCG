/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213142
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213142 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213142
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_16 = (min((arr_3 [i_0]), ((((var_6 ? -1215211591 : (arr_1 [i_0]))) & (var_1 | var_5)))));
                var_17 = (min(var_9, 255));
            }
        }
    }

    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        arr_9 [i_2] = 2047;
        arr_10 [i_2] = (((arr_7 [i_2]) ? (((0 ? (arr_7 [i_2]) : (arr_7 [i_2])))) : (((arr_7 [i_2]) ? 18446744073709551615 : (arr_7 [i_2])))));
    }
    #pragma endscop
}
