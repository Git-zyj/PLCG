/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245793
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245793 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245793
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned short) (unsigned char)255);
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 11; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 7; i_1 += 2) 
        {
            {
                arr_4 [i_1] = ((/* implicit */unsigned char) max(((-(((/* implicit */int) arr_3 [i_1 - 1] [(unsigned char)2] [i_1 - 2])))), (((/* implicit */int) max((((/* implicit */signed char) arr_3 [i_1 - 1] [i_1 - 1] [(short)2])), (var_4))))));
                /* LoopNest 2 */
                for (unsigned short i_2 = 0; i_2 < 11; i_2 += 1) 
                {
                    for (signed char i_3 = 0; i_3 < 11; i_3 += 1) 
                    {
                        {
                            var_21 ^= ((/* implicit */_Bool) (unsigned char)104);
                            var_22 = ((/* implicit */signed char) ((_Bool) (_Bool)1));
                            var_23 = ((/* implicit */short) (!(((/* implicit */_Bool) (signed char)-1))));
                        }
                    } 
                } 
            }
        } 
    } 
}
