/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159630
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159630 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159630
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_3;
    var_13 -= (((min(((min(var_10, var_10))), var_5))) ? var_1 : var_4);
    var_14 = (max(var_14, ((((((min(var_1, var_3) < ((min(var_6, 20876)))))))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] = ((((min(((min(127, 127))), (arr_0 [i_0])))) || var_6));
                var_15 = min((min(127, 4096)), -10483);
            }
        }
    }
    #pragma endscop
}
