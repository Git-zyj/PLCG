/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160027
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160027 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160027
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned long long int) var_6);
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        var_19 = (+((-(((((/* implicit */_Bool) (unsigned char)0)) ? (((/* implicit */int) (unsigned char)183)) : (((/* implicit */int) var_6)))))));
        var_20 = arr_1 [i_0] [i_0];
        var_21 = ((/* implicit */unsigned int) min((var_21), (2940023774U)));
        var_22 = min((2931675161877432476ULL), (((/* implicit */unsigned long long int) min((((/* implicit */int) arr_1 [i_0] [i_0])), (arr_0 [i_0])))));
    }
}
