/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 &= (((!var_9) / ((min((min(var_7, var_9)), (min(var_8, var_6)))))));

    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        arr_3 [i_0] = (!var_2);
        var_12 = var_3;
        var_13 += max((((((max(var_7, var_0))) || (~var_0)))), (max(var_4, var_9)));

        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            var_14 = (max(var_14, (((max(((min(var_9, var_10))), (min(var_4, var_9)))) * ((max((min(var_1, var_8)), var_0)))))));
            arr_7 [i_1] [14] [i_0] = var_9;
            arr_8 [i_0 - 4] = var_3;
        }
    }
    #pragma endscop
}
