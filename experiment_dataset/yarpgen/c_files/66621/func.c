/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66621
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    /* vectorizable */
    for (unsigned int i_0 = 3; i_0 < 14; i_0 += 4) 
    {
        var_20 += ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_3)) && (((/* implicit */_Bool) (~(((/* implicit */int) var_12)))))));
        var_21 = ((/* implicit */_Bool) arr_2 [i_0]);
    }
    for (unsigned int i_1 = 1; i_1 < 8; i_1 += 2) 
    {
        arr_6 [i_1] = ((/* implicit */short) (~(((/* implicit */int) arr_2 [i_1]))));
        var_22 = ((/* implicit */unsigned int) var_10);
    }
    var_23 = ((/* implicit */_Bool) ((((/* implicit */int) (unsigned char)228)) / (((/* implicit */int) (unsigned short)60151))));
    var_24 = ((/* implicit */short) var_16);
}
