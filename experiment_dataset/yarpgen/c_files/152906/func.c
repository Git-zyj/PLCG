/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152906
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152906 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152906
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */signed char) var_2);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 10; i_0 += 2) 
    {
        for (int i_1 = 3; i_1 < 9; i_1 += 4) 
        {
            {
                arr_5 [i_1] = ((/* implicit */signed char) var_10);
                arr_6 [i_0] [i_0] [i_0] = ((/* implicit */unsigned long long int) arr_3 [i_1 - 1] [(signed char)1] [(signed char)1]);
                /* LoopNest 3 */
                for (unsigned char i_2 = 0; i_2 < 10; i_2 += 2) 
                {
                    for (unsigned short i_3 = 0; i_3 < 10; i_3 += 2) 
                    {
                        for (int i_4 = 3; i_4 < 7; i_4 += 1) 
                        {
                            {
                                arr_15 [i_0] [0] [0] [i_0] [i_4] |= ((/* implicit */_Bool) ((signed char) 0));
                                var_14 = ((/* implicit */signed char) var_0);
                                arr_16 [i_4] = var_7;
                                var_15 = ((/* implicit */unsigned long long int) var_1);
                            }
                        } 
                    } 
                } 
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 10; i_5 += 2) 
                {
                    for (int i_6 = 4; i_6 < 8; i_6 += 1) 
                    {
                        {
                            var_16 = ((/* implicit */unsigned int) max((((/* implicit */unsigned long long int) ((signed char) ((var_7) < (((/* implicit */unsigned long long int) ((/* implicit */int) arr_4 [i_5] [i_5]))))))), (min((((((/* implicit */unsigned long long int) -2082450527)) * (4897658807118569675ULL))), (((/* implicit */unsigned long long int) ((((/* implicit */unsigned int) ((/* implicit */int) arr_13 [(_Bool)1]))) * (arr_9 [i_6 - 1] [i_5] [(signed char)9] [i_0]))))))));
                            var_17 = ((/* implicit */int) (short)8467);
                        }
                    } 
                } 
            }
        } 
    } 
}
