/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210677
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210677 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210677
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_14 = (((arr_0 [i_0 - 2]) + (((((var_5 ? (arr_2 [i_1] [2]) : (arr_3 [i_1])))) ? (min(var_12, (arr_3 [i_0]))) : (!13752281065216094831)))));

                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    var_15 = (max((((((!(arr_5 [i_0])))))), var_9));
                    var_16 ^= (arr_0 [i_1]);
                    var_17 = var_8;
                }
                var_18 = (((((((arr_4 [1] [i_0] [i_0]) - (arr_1 [i_0] [i_0]))))) ? (((((min((arr_0 [i_0]), var_1)))) ^ (arr_0 [i_0 + 2]))) : (((-(arr_5 [i_0]))))));
            }
        }
    }
    var_19 ^= (min(var_9, var_10));
    #pragma endscop
}
