/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151801
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151801 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151801
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */short) var_1);
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 12; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 12; i_1 += 4) 
        {
            {
                arr_5 [i_1] |= ((/* implicit */unsigned long long int) (-(min((((/* implicit */int) arr_0 [i_1])), ((+(((/* implicit */int) var_12))))))));
                var_17 = ((/* implicit */unsigned long long int) max((var_17), (((/* implicit */unsigned long long int) arr_2 [i_1]))));
                arr_6 [i_0] = ((/* implicit */unsigned long long int) max((((/* implicit */unsigned char) ((((/* implicit */int) var_12)) < (((/* implicit */int) arr_2 [i_0]))))), (((unsigned char) -2147483639))));
                arr_7 [i_0] [i_0] [i_0] = ((/* implicit */unsigned int) 4555681535442201346ULL);
            }
        } 
    } 
    var_18 ^= ((/* implicit */unsigned int) var_4);
}
