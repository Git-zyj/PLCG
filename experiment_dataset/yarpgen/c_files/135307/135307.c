/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135307
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135307 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135307
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((239 ? ((((arr_0 [i_1 + 1] [i_1]) ? (arr_0 [i_1 - 3] [17]) : var_10))) : (10174281871451062891 % 28427)));
                var_12 = (arr_4 [i_1] [i_1 - 2] [i_1]);
            }
        }
    }
    var_13 = ((((((4158910633486266726 || var_3) | (min(-1, var_3))))) || 10));
    var_14 -= (min(753469886, 4096));
    var_15 = var_6;
    #pragma endscop
}
