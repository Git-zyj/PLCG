/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24123
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 &= ((/* implicit */short) var_0);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 22; i_1 += 2) 
        {
            {
                var_13 = ((/* implicit */short) arr_1 [i_0]);
                var_14 = ((/* implicit */short) 17256769031637297109ULL);
                /* LoopNest 3 */
                for (unsigned short i_2 = 1; i_2 < 24; i_2 += 3) 
                {
                    for (short i_3 = 0; i_3 < 25; i_3 += 3) 
                    {
                        for (unsigned long long int i_4 = 0; i_4 < 25; i_4 += 3) 
                        {
                            {
                                var_15 = ((/* implicit */signed char) ((((/* implicit */int) (signed char)-25)) * (((((/* implicit */_Bool) (short)23829)) ? (((/* implicit */int) arr_0 [i_2 - 1])) : (((/* implicit */int) (signed char)-10))))));
                                arr_13 [i_0] [(signed char)2] [i_1] [i_3] [i_2] = ((/* implicit */int) var_3);
                            }
                        } 
                    } 
                } 
                arr_14 [i_0] [4ULL] [i_1] = ((/* implicit */signed char) (~(((unsigned long long int) max((((/* implicit */unsigned long long int) (unsigned short)50722)), (var_0))))));
            }
        } 
    } 
}
