/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165226
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165226 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165226
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_1;
    var_15 = ((((min(0, 7762))) && var_12));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    var_16 = (min((max((arr_6 [i_1] [i_1] [i_2 + 1]), var_11)), 9223372036854775807));
                    var_17 *= (arr_3 [i_0 + 2] [i_0 + 2]);
                    arr_7 [i_0] [i_2] [i_2 + 1] = (((((arr_2 [i_2] [i_2 + 2]) ? (arr_2 [i_2 - 2] [i_2 - 2]) : (arr_2 [i_2] [i_2 + 2])))) % 15914599228840963261);
                }
            }
        }
    }
    #pragma endscop
}
