/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24761
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24761 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24761
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(((~(~40))), ((min(var_7, var_9)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 16;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_1] [i_0] = (min(((min(var_10, var_5))), (arr_2 [i_2 - 4] [i_1])));
                    var_16 *= (max((~40), (max(((var_8 ? 40 : var_6)), ((min(var_8, 205)))))));
                    var_17 *= var_5;
                }
            }
        }
    }
    var_18 = ((((min((max(var_6, var_3)), ((var_0 ? var_9 : var_11))))) ? (max(51, 965075717604268993)) : var_2));
    #pragma endscop
}
