/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 55327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=55327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/55327
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_16 = (max(7, 1));
        var_17 = (arr_1 [i_0]);
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        arr_5 [i_1] [i_1] = 248;
        arr_6 [i_1] [i_1] = (min((min((((-144626534 + 2147483647) << (var_7 - 166))), var_14)), ((((8190 | 0) < ((-1 ? -50 : 0)))))));
        var_18 = ((!((min(((-(arr_4 [i_1]))), (((arr_3 [i_1] [i_1]) - (arr_3 [i_1] [i_1]))))))));
        arr_7 [0] |= (arr_2 [i_1]);
        var_19 = ((-var_4 ? var_9 : 4294836224));
    }
    var_20 = -1;
    #pragma endscop
}
