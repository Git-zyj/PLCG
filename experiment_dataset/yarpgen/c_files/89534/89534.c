/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89534
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89534 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89534
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (var_4 / var_11);
    var_14 = (min(var_14, var_6));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_15 = (max(var_15, var_8));
        arr_3 [i_0] [i_0] |= ((((~(arr_2 [i_0])))) ? (((min((arr_2 [i_0]), (arr_0 [i_0]))) / ((var_1 >> (((arr_2 [i_0]) - 857124050)))))) : (~var_8));
        var_16 = var_9;
        arr_4 [i_0] = min(((~(((arr_0 [14]) / (arr_1 [i_0]))))), (((((-200662629 ? -1031096242 : (arr_2 [i_0])))) ? (var_7 && var_6) : ((var_10 ? var_2 : var_3)))));
    }
    var_17 = ((((-((var_0 ? var_11 : var_1))))) ? ((((var_10 % var_0) && (420082256 & 193845961)))) : -var_7);
    var_18 = (min(var_18, var_3));
    #pragma endscop
}
