/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197117
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197117 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197117
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_11;
    var_17 = var_11;
    var_18 = min(-8623145552041802935, -1);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_19 = 4864628613768108859;
                    arr_8 [i_0 + 3] [i_2] [i_0] [i_0] = (((max(3713666645767348994, ((min((arr_4 [i_2]), (arr_4 [i_1])))))) <= (min((!var_6), ((var_11 ? 4503599627370496 : var_7))))));
                }
            }
        }
    }
    #pragma endscop
}
