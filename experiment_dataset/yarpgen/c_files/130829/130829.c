/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130829
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130829 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130829
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = ((-(arr_0 [i_0] [i_0])));
        var_14 = var_5;
    }
    var_15 ^= var_6;
    /* LoopNest 3 */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        for (int i_2 = 3; i_2 < 8;i_2 += 1)
        {
            for (int i_3 = 2; i_3 < 8;i_3 += 1)
            {
                {
                    var_16 = arr_9 [i_2 - 3] [i_2 + 2];
                    arr_13 [i_3] [i_3] [i_3 + 1] = ((((((arr_9 [i_2 - 3] [i_3 + 1]) ? (arr_9 [i_2 + 1] [i_3 + 1]) : (arr_9 [i_2 - 2] [i_3 + 2])))) ? ((-(arr_9 [i_2 - 3] [i_3 + 2]))) : (((arr_9 [i_2 - 2] [i_3 + 2]) % (arr_9 [i_2 + 2] [i_3 - 2])))));
                    var_17 = (min(var_17, (arr_10 [i_1] [i_3])));
                }
            }
        }
    }
    #pragma endscop
}
