/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 9;i_2 += 1)
            {
                {
                    var_16 += var_11;
                    arr_10 [i_0] [i_2] [10] = (min(((min(var_7, 66060288))), (((!var_10) ? (min((arr_3 [i_0] [i_1] [i_2]), (arr_1 [i_0]))) : (arr_3 [i_0 - 2] [i_1] [i_1])))));
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 8;i_4 += 1)
                        {
                            {
                                var_17 = (max(var_13, (((!(arr_6 [1]))))));
                                arr_16 [i_1] [i_1] [i_3 + 1] [i_1] = ((((!(arr_6 [i_2 - 1]))) ? (((!(arr_6 [i_2 + 1])))) : (arr_6 [i_2 - 2])));
                                var_18 -= (arr_2 [i_1] [i_0]);
                                var_19 = -4242592729435535834;
                            }
                        }
                    }
                }
            }
        }
    }
    var_20 = ((+(((var_1 || var_9) << (var_2 - 55621)))));
    #pragma endscop
}
