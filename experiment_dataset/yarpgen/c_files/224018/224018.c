/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= var_1;
    var_13 = var_1;
    var_14 *= var_0;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 7;i_4 += 1)
                        {
                            {
                                arr_15 [i_0] [i_1] [i_3] = (arr_13 [i_0] [i_0] [i_2] [i_3] [i_0]);
                                var_15 &= (((((max(var_1, var_4)) ^ var_1)) - var_9));
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 10;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 10;i_6 += 1)
                    {
                        {
                            var_16 ^= 1;
                            var_17 = (((((arr_14 [i_0 - 2] [i_0 - 1] [i_0 - 1] [i_0 - 1] [3]) * ((((arr_6 [i_5] [i_1] [i_0 + 1]) > (arr_10 [4] [i_0] [i_0] [0])))))) & (((var_5 || var_11) << (((min(var_11, var_2)) - 37959))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
