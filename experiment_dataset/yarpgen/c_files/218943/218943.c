/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0 + 1] [i_1 + 1] [i_1] [i_2] = min((max(-1, -var_2)), ((var_17 ? (arr_3 [5] [i_1 + 1]) : var_14)));
                    arr_7 [2] &= (((arr_1 [i_0 + 1]) != (arr_1 [i_0 - 1])));
                    var_18 = (max(var_18, (((((((((var_16 ? 65256 : -1352717013)) == (((-2147483647 - 1) / 279)))))) / var_12)))));
                }
            }
        }
    }
    var_19 = ((var_10 ? (!var_9) : var_17));
    #pragma endscop
}
