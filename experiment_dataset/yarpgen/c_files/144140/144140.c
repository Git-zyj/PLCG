/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144140
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144140 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144140
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((var_3 / var_10) * ((((min(var_13, var_6))) ? var_2 : var_12)));
    var_20 = var_18;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = (~13);
                var_21 = (max(var_21, -3004858800));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 15;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [1] [i_3] [9] [i_3 - 3] = (arr_3 [i_0] [i_2]);
                            arr_10 [i_3] = ((~(arr_2 [i_1 - 1] [i_1 - 1])));
                            var_22 = (max(var_22, (((-(max((arr_5 [i_0] [1] [i_1 - 1] [i_3 - 3]), (arr_5 [i_0] [12] [i_2] [2]))))))));
                            var_23 = 1;
                            arr_11 [i_3] [i_3] = -115;
                        }
                    }
                }
                var_24 = var_17;
            }
        }
    }
    #pragma endscop
}
