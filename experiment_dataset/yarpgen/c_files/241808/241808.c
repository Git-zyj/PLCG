/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241808
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241808 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241808
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((min(6280569315768260532, (0 > 960))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_21 = (max(var_21, 1));
                var_22 = ((((max(((33212 << (16156532297833626512 - 16156532297833626507))), 7187))) > (((max(58357, 2)) + ((7198 ? 2290211775875925104 : 7184))))));
                var_23 += 1;
                arr_5 [i_0] [i_0] [i_1] = 968445118;
            }
        }
    }
    #pragma endscop
}
