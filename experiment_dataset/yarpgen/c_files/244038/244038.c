/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244038
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244038 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244038
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_3 ? -12337 : (108 == var_4)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [i_2] [i_0] [i_3] = (arr_8 [7] [2] [i_3]);
                            arr_12 [i_0] [i_0] [i_2] = ((((((arr_7 [i_1]) <= 65535))) << (((arr_7 [i_2]) - 21374))));
                            arr_13 [i_0] [0] = ((((min((!17179868672), var_1))) ? 1 : var_3));
                        }
                    }
                }
                arr_14 [i_0] = var_11;
            }
        }
    }
    var_15 = var_7;
    var_16 = ((!(var_4 / 1)));
    var_17 = (var_13 & var_13);
    #pragma endscop
}
