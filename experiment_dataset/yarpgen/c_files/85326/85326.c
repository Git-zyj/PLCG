/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85326
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85326 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85326
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((!((max(100, var_8))))) || var_10);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_13 = (((((arr_2 [i_1] [i_0]) && (arr_2 [i_1] [i_1]))) ? (arr_0 [i_0]) : (((((arr_4 [i_1] [i_0]) != (!43)))))));
                arr_5 [i_0] [i_1] = ((arr_2 [i_1] [i_0]) - 0);
                var_14 = 65534;
            }
        }
    }
    var_15 = (var_6 | (((min(-30943, var_7)) ^ (~var_8))));
    #pragma endscop
}
