/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196423
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196423 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196423
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 2147483647;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = 1790451139880443113;
                arr_7 [6] [i_1] [i_1] = (min(var_0, (((arr_2 [i_0]) ? (((max(var_12, (arr_4 [i_0] [i_1] [i_0]))))) : 10329601071795281510))));
            }
        }
    }
    var_15 = (max(var_15, 926710928));
    var_16 = (min(var_16, (~415211765080306801)));
    #pragma endscop
}
