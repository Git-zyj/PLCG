/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36828
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36828 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36828
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        arr_4 [i_0] [i_0 + 4] = (arr_0 [i_0]);
        var_13 = 0;
    }
    /* LoopNest 3 */
    for (int i_1 = 1; i_1 < 22;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 4; i_3 < 20;i_3 += 1)
            {
                {
                    var_14 = (max(var_8, (~var_8)));
                    /* LoopNest 2 */
                    for (int i_4 = 1; i_4 < 22;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 23;i_5 += 1)
                        {
                            {
                                arr_15 [i_1] [i_1] [i_1] [i_4 - 1] [12] [i_2] = (((((var_9 - ((~(arr_8 [i_1])))))) ? var_8 : ((~(arr_5 [i_1 - 1])))));
                                var_15 = (arr_13 [i_1] [i_2]);
                                arr_16 [i_1] = min((arr_12 [1] [i_2] [1] [i_4] [i_5] [10]), (~var_4));
                            }
                        }
                    }
                    var_16 = ((!(((var_4 ? ((((arr_14 [i_1]) || 11))) : (arr_6 [i_1]))))));
                }
            }
        }
    }
    #pragma endscop
}
