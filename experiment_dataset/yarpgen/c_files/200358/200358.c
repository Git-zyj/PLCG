/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200358
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200358 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200358
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_3 [i_0] = 144;
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_12 [i_1] = (max((min(var_15, var_12)), var_15));
                    arr_13 [10] [i_1] [i_0] [10] &= (min(10688, ((min(120, var_4)))));
                }
            }
        }
    }
    var_17 = ((((var_7 ? var_3 : ((max(var_16, 1)))))) ? var_6 : (((((var_0 ? var_16 : var_15))) ? var_9 : 8853145834225025487)));
    #pragma endscop
}
