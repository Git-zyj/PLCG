/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160933
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160933 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160933
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 12;i_0 += 1)
    {
        var_15 = ((~((-(arr_0 [i_0 - 2])))));
        var_16 = (min(var_16, ((((arr_0 [i_0 + 1]) ? 74 : (arr_1 [i_0] [i_0]))))));

        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            arr_4 [i_1] = (((((arr_0 [i_1]) + 2147483647)) << (((!(((arr_1 [i_0] [9]) && 1)))))));
            var_17 = (arr_3 [i_0] [i_0]);
            var_18 = (max(var_5, ((-((var_4 ? 0 : var_0))))));
            var_19 *= ((((((((arr_0 [i_0]) ? (arr_3 [1] [i_0]) : 29522))) ? (35 - var_0) : -5180025024946126171)) - ((((((var_12 ? var_12 : 1))) ? 29 : 5)))));
        }
        arr_5 [i_0] [i_0] = arr_0 [i_0];
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 16;i_3 += 1)
        {
            {
                var_20 *= ((((arr_7 [i_2]) ? var_6 : var_7)));
                arr_10 [i_2] [i_2] = (-((121 ? 1 : (((1 ? var_2 : 37))))));
            }
        }
    }
    #pragma endscop
}
