/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56409
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56409 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56409
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_19 = (((((arr_2 [i_0]) ^ var_0)) ^ ((((arr_2 [i_0]) ? (arr_2 [i_0]) : var_18)))));
        var_20 = (arr_0 [6] [i_0]);
        var_21 ^= ((var_14 | ((((!(((112 ? (arr_0 [i_0] [8]) : 18446744073709551613)))))))));
        arr_3 [i_0] [i_0] = var_12;
        arr_4 [i_0] = ((-(-3253687080018593268 <= 43)));
    }
    var_22 = (min(var_11, ((var_0 ? -5815218448588724220 : (var_12 == var_4)))));
    #pragma endscop
}
