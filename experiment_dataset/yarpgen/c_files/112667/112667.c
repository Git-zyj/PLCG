/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112667
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112667 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112667
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((var_10 ? var_2 : var_3))) ? (var_0 & 2147483647) : var_6)));
    var_13 ^= (((max(var_7, ((min(var_8, 32765)))))) ? (-32767 - 1) : (max(var_2, 2)));
    var_14 |= var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (min(((min((arr_3 [6]), (arr_0 [i_0] [i_0])))), (arr_2 [i_1] [i_0])));
                var_15 = (arr_2 [i_1] [i_0]);
                arr_5 [i_0] [i_1] [i_1] = (((((-32767 - 1) | 18446744073709551613)) > (min((((1312 ? (arr_3 [i_0]) : (arr_1 [i_0])))), 3))));
            }
        }
    }
    #pragma endscop
}
