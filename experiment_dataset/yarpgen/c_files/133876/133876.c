/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133876
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133876 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133876
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((((min(((119 ? var_4 : 3804)), ((0 ? var_12 : var_16))))) != (min((((205 ? var_12 : var_18))), (min(var_16, var_1))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_2] |= (min((((arr_7 [i_2] [23] [i_0] [i_0]) ? (arr_7 [i_0] [i_1] [i_2] [i_0]) : (arr_7 [i_2] [23] [23] [i_0]))), (219 * 1332255286)));
                    var_21 = ((var_8 == ((-69 ? 5141246878231035573 : 0))));
                }
            }
        }
    }
    #pragma endscop
}
