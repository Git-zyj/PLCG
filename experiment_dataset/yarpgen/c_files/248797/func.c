/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248797
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248797 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248797
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 2; i_0 < 12; i_0 += 1) 
    {
        var_16 = ((/* implicit */int) max((var_16), (((/* implicit */int) (_Bool)1))));
        arr_3 [i_0] = (!(((/* implicit */_Bool) arr_1 [i_0 - 2] [i_0 - 2])));
        var_17 = ((/* implicit */short) ((((/* implicit */long long int) ((/* implicit */int) arr_1 [i_0 - 1] [i_0 - 2]))) & (-8672834037206778336LL)));
    }
    var_18 = ((/* implicit */unsigned char) max((((/* implicit */unsigned long long int) var_7)), (((unsigned long long int) var_10))));
}
