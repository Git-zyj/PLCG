/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211608
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211608 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211608
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((max(var_2, -59)));
    var_17 = ((((min((-2147483647 - 1), (max(-60, var_9))))) ? (min((max(var_7, 50176)), var_3)) : ((((((var_0 ? 59 : 66))) && var_2)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_18 = max((((arr_3 [i_1]) ? (arr_3 [i_1]) : 50176)), (!-4));
                arr_7 [i_1] = ((!((((((var_12 ? 17809194240870975935 : 60))) ? (arr_1 [i_0]) : (-2506803061533353962 & 69))))));
            }
        }
    }
    #pragma endscop
}
