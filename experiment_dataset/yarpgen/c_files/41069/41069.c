/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41069
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_14 = ((((-42 & (-32762 ^ var_7))) + var_13));
                var_15 = (min((((max(32756, 135305106)))), (-32762 >= var_9)));
                var_16 = (((-32007 < -32762) && (6789382957814512794 - var_0)));
                var_17 += (((((var_10 < (arr_2 [i_1 - 1] [i_1 - 1])))) << (((arr_2 [i_1 + 2] [i_1 + 2]) - (arr_2 [i_1 + 1] [i_1 + 1])))));
                var_18 = ((38745 >> (32756 - 32739)));
            }
        }
    }
    var_19 = var_5;
    #pragma endscop
}
