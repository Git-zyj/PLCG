/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32215
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32215 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32215
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                var_15 = 239;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [i_1] = var_7;
                            arr_12 [i_0] [i_0] [i_1] [i_1] [1] [i_1] = -2;
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 22;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 22;i_5 += 1)
        {
            {
                arr_20 [i_4] [i_4] [i_5] = var_3;
                arr_21 [i_4] [i_4] = 1;
            }
        }
    }
    var_16 = (max(var_1, ((((377074412909499924 == 1) ? ((18858 ? -107 : 12288)) : (!var_3))))));
    var_17 = ((-21478 ? (((((1 ? -3994 : -1148899245))) ? (min(1752350456, var_12)) : 1)) : ((((!var_2) + var_4)))));
    #pragma endscop
}
