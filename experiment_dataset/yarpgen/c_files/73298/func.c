/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73298
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73298 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73298
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */signed char) ((var_5) << (((((/* implicit */int) var_6)) - (19700)))));
    /* LoopSeq 1 */
    for (int i_0 = 2; i_0 < 13; i_0 += 1) 
    {
        var_21 = ((/* implicit */unsigned short) 688628684U);
        var_22 = ((/* implicit */unsigned int) ((((/* implicit */int) arr_2 [i_0])) == (((/* implicit */int) min((arr_2 [i_0]), (arr_2 [i_0]))))));
    }
}
