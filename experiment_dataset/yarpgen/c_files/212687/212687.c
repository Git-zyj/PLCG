/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212687
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212687 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212687
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_13;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_2 + 1] [i_2] = (((((-30170 + (!61948)))) ? 11851656677651763330 : ((max((min((arr_5 [i_2] [i_2] [i_0] [i_0]), (arr_5 [i_0] [i_1] [i_1] [i_2 + 1]))), (arr_0 [9] [i_1]))))));
                    arr_8 [i_2] = 0;
                }
            }
        }
    }
    var_20 = (min((((~(var_16 >= var_9)))), 6354437805073347668));
    #pragma endscop
}
