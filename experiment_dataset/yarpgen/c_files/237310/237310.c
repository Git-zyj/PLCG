/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237310
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237310 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237310
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((+((max((40 == 40), (arr_2 [i_1] [i_1]))))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            arr_11 [i_1] = 0;
                            arr_12 [14] |= (((~var_3) ^ 0));
                            arr_13 [i_1] [i_2] [i_1] = 255;
                            var_16 = (i_1 % 2 == 0) ? (((arr_5 [i_2] [i_1] [i_2] [i_1]) >> (23736 - 23712))) : (((((arr_5 [i_2] [i_1] [i_2] [i_1]) + 9223372036854775807)) >> (23736 - 23712)));
                        }
                    }
                }
            }
        }
    }
    var_17 = var_14;
    var_18 = -var_2;
    var_19 = var_8;
    #pragma endscop
}
