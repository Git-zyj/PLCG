/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111862
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111862 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111862
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                var_13 = (((arr_3 [i_0] [i_0] [i_1 - 1]) >> (((arr_0 [i_0 + 1]) - 3503091437534356508))));
                var_14 = (((((((((arr_1 [i_0]) ? (arr_2 [i_0] [i_0] [7]) : (arr_1 [i_0])))) ? 251 : (arr_0 [i_1])))) ? (1 || 1) : (!1)));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 3; i_2 < 6;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 10;i_3 += 1)
        {
            {
                var_15 = (arr_0 [16]);
                arr_10 [i_2] = (max(1, ((var_4 ? ((~(arr_0 [i_3]))) : ((max((arr_6 [3] [i_2]), var_7)))))));
                arr_11 [i_2 - 1] [i_2] [i_2] = ((-((var_2 ? (arr_4 [i_3] [i_2 + 1]) : (((arr_7 [3] [i_3]) ? (arr_1 [i_2]) : 0))))));
                var_16 -= 1;
            }
        }
    }
    #pragma endscop
}
