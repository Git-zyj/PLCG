/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6242
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6242 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6242
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_10 = (((var_0 ^ var_9) ? (((4294967295 | ((146 ? 1838732747 : 15))))) : (((arr_1 [i_0] [i_0]) ^ 13412476325197658407))));
        var_11 = (arr_0 [i_0] [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 14;i_1 += 1) /* same iter space */
    {
        arr_5 [i_1] = ((!(((var_9 ? 70368744177663 : var_6)))));
        arr_6 [i_1] [i_1] = (((var_9 ? (arr_0 [9] [i_1]) : -95)));
        arr_7 [i_1] = var_4;
        var_12 = (-1682213827 != var_1);
    }
    var_13 ^= (~(((var_9 <= (((252201579132747776 ? 94 : var_8)))))));
    var_14 = (max(var_14, var_2));
    #pragma endscop
}
