/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] &= ((~(~-3937248780980726345)));
                var_19 = ((var_3 * ((max((max(var_7, -23882)), (((!(arr_5 [i_1] [9] [9])))))))));
            }
        }
    }
    var_20 &= ((((min(var_16, (min(93, 31))))) ? var_18 : (((!1603) / var_0))));
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 21;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 20;i_3 += 1)
        {
            {
                var_21 = (max(var_21, (((var_8 || ((((arr_2 [i_3 - 1] [i_3 - 1]) ? -94 : var_14))))))));
                arr_13 [i_2] = var_14;
                arr_14 [i_3] [i_2] [14] = arr_0 [i_3];
            }
        }
    }
    var_22 = (max(var_22, var_4));
    #pragma endscop
}
