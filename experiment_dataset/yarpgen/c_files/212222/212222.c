/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212222
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212222 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212222
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_2 ? (min(((min(var_1, 0))), var_9)) : 88));
    var_12 = ((~((-((var_1 ? 0 : var_10))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 9;i_1 += 1)
        {
            {
                arr_5 [i_1] = (min((max(2871163550824489490, 13)), 20068));
                arr_6 [i_0] [i_0] [i_0] = ((14741079608953015393 ? (((~(min(var_5, -17311))))) : (max(-1, -795))));
            }
        }
    }
    #pragma endscop
}
