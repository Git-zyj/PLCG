/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171165
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171165 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171165
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        var_16 = ((/* implicit */int) var_5);
        arr_2 [i_0] [i_0 + 1] = ((/* implicit */long long int) (unsigned short)65535);
    }
    var_17 |= ((/* implicit */_Bool) ((max(((-(((/* implicit */int) var_4)))), (var_3))) / ((~(((/* implicit */int) max(((unsigned short)14), (((/* implicit */unsigned short) (signed char)-32)))))))));
}
