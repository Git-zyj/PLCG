/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24903
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24903 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24903
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_3 [15] = (((((var_8 ? var_1 : var_0) + -var_4))));
        arr_4 [i_0] = ((var_9 << (-var_8 + 129)));
    }
    var_10 = ((var_1 - ((var_3 ? ((31 ? 4860227780155318622 : 1)) : (((var_5 ? var_1 : var_6)))))));
    var_11 = (((var_5 ^ (~var_9))) * (((((var_9 ^ var_1) && ((min(-1168424818, 1))))))));
    var_12 = var_5;
    var_13 *= ((+((((var_9 ^ var_2) || (var_9 - var_7))))));
    #pragma endscop
}
