/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22694
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22694 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22694
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max((((max(157, var_9)))), var_12));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_3 [i_0] [i_0] |= (((var_7 * (~2429534878))));
        arr_4 [i_0] = min((arr_0 [i_0] [i_0]), ((0 ? (arr_1 [i_0]) : (arr_1 [i_0]))));
        arr_5 [i_0] [i_0] |= (var_11 ? (max(var_6, var_12)) : (arr_2 [i_0]));
        arr_6 [i_0] [i_0] = ((var_1 ? (~-32761) : 3345778808));
    }
    #pragma endscop
}
