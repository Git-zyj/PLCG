/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138123
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (arr_0 [i_0] [i_0]);
        var_15 = (((((((arr_1 [i_0] [i_0]) % 18446744073709551615))) ? ((18446744073709551615 ? 18446744073709551615 : 259886408)) : -1803386996)));
        arr_4 [i_0] [6] = 17;
    }
    var_16 = (min((min((max(var_14, 13017082556671545585)), (((410801582 ? 40 : var_0))))), var_6));
    var_17 = (((((122 <= (min(2042392952, 2130533896))))) > var_12));
    var_18 = (min(var_18, -61));
    #pragma endscop
}
