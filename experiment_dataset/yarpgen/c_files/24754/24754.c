/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24754
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24754 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24754
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(((min(var_14, (!var_2)))), var_12));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_18 -= (((((((((arr_2 [i_1] [i_0]) | var_15))) ? var_2 : var_14))) ? (((arr_5 [i_0 - 2] [i_1 + 2]) ? 1 : -14)) : (!var_0)));
                    arr_8 [i_0 - 1] [i_0 - 1] [i_0] = ((var_12 ? ((~((-(arr_7 [i_0] [i_0]))))) : var_7));
                    var_19 = 3;
                    var_20 = var_9;
                }
            }
        }
    }
    var_21 = (!var_13);
    #pragma endscop
}
