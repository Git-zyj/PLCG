/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104197
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104197 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104197
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_5 [i_0] = (min(((((arr_2 [i_1]) & 229461752))), (((!(arr_1 [i_0 + 1]))))));
                var_17 = ((((min((arr_1 [i_1]), (arr_1 [i_0 - 3])))) ? (max(5425907620131234256, 8577437274692033362)) : (-9223372036854775807 - 1)));
                arr_6 [i_0 - 4] = (arr_0 [i_1] [i_0]);
                var_18 = (max(var_18, ((46 ? 29 : 0))));
            }
        }
    }
    #pragma endscop
}
