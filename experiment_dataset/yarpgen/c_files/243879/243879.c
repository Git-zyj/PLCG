/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243879
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243879 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243879
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_11 = (min((arr_0 [i_0 - 1]), (arr_0 [i_0 + 2])));
                                var_12 = (min(4026258397619799213, 8732));
                            }
                        }
                    }
                    var_13 = ((max(7694, (arr_1 [i_0 - 2]))));
                    var_14 = (min((((arr_3 [i_0 - 3] [i_1 - 1]) && (arr_11 [i_1 - 1] [i_1 + 1] [i_0 + 2] [i_0] [i_0 - 2] [i_0] [i_0]))), ((!(arr_3 [i_0 + 2] [i_1 + 1])))));
                }
            }
        }
    }
    var_15 = var_1;
    #pragma endscop
}
