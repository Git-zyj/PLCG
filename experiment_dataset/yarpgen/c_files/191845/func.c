/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191845
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned short) var_11);
    var_21 = ((/* implicit */unsigned short) min((var_21), (((/* implicit */unsigned short) (-(var_15))))));
    var_22 += ((/* implicit */unsigned short) (!(var_17)));
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (signed char i_2 = 1; i_2 < 21; i_2 += 2) 
            {
                {
                    var_23 = ((/* implicit */unsigned int) min((var_23), (((/* implicit */unsigned int) var_16))));
                    var_24 ^= ((/* implicit */signed char) min(((+(var_0))), (((/* implicit */unsigned long long int) ((((/* implicit */int) var_13)) / (((/* implicit */int) var_12)))))));
                    var_25 = ((/* implicit */unsigned int) min((var_25), (((/* implicit */unsigned int) ((((/* implicit */_Bool) (+(((/* implicit */int) var_19))))) && (((/* implicit */_Bool) (-(((/* implicit */int) min((((/* implicit */signed char) var_1)), (var_12))))))))))));
                }
            } 
        } 
    } 
    var_26 = ((/* implicit */_Bool) min((var_26), (((/* implicit */_Bool) (-((-(var_8))))))));
}
