/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81805
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 4; i_0 < 14; i_0 += 4) 
    {
        for (int i_1 = 2; i_1 < 15; i_1 += 1) 
        {
            for (int i_2 = 3; i_2 < 16; i_2 += 4) 
            {
                {
                    arr_9 [i_1] = 1759977938U;
                    var_15 = ((/* implicit */unsigned long long int) ((max(((_Bool)1), (((((/* implicit */_Bool) var_12)) || (((/* implicit */_Bool) var_10)))))) || ((!(arr_2 [i_1 - 2])))));
                    var_16 = ((/* implicit */unsigned long long int) ((((((/* implicit */int) (unsigned char)0)) * (((/* implicit */int) (signed char)-28)))) < (((/* implicit */int) var_6))));
                    var_17 = ((/* implicit */_Bool) (-(((/* implicit */int) (signed char)28))));
                }
            } 
        } 
    } 
    var_18 |= ((/* implicit */unsigned short) var_9);
    var_19 = var_10;
}
