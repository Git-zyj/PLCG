/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239756
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239756 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239756
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_12 = (max((((94 && ((((arr_0 [i_0] [i_1]) ? (arr_0 [i_0] [i_0]) : var_10)))))), (!var_2)));
                var_13 = (((((((((arr_2 [i_0] [13]) - (arr_3 [i_0])))) - (min((arr_3 [i_1]), 1218407696))))) ? (((((min((arr_3 [i_0]), (arr_3 [i_0])))) - var_9))) : ((var_6 * (arr_0 [i_0] [3])))));
            }
        }
    }
    var_14 = (!-var_2);
    #pragma endscop
}
