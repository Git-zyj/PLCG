/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47197
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47197 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47197
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (-((-(var_1 + var_8))));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_18 = (max(var_18, (((~((~(arr_1 [i_0]))))))));
        arr_3 [i_0] = (max(1, 0));
    }

    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {

        for (int i_2 = 0; i_2 < 24;i_2 += 1)
        {
            var_19 = ((((min((!4294967295), (!var_8)))) | (((((min((arr_8 [i_1]), (arr_8 [i_2])))) && var_8)))));
            var_20 = 1;
        }
        for (int i_3 = 3; i_3 < 23;i_3 += 1)
        {
            arr_13 [i_1] [i_1] = (((max(1023, (arr_11 [i_3 + 1] [i_3 - 2]))) & (arr_11 [i_3 + 1] [i_3 + 1])));
            var_21 += ((65535 ? (arr_11 [i_3 - 3] [i_3]) : (((min(1, 213))))));
        }
        for (int i_4 = 0; i_4 < 24;i_4 += 1)
        {
            var_22 = (max(var_22, (arr_14 [i_1] [i_4] [i_1])));
            var_23 *= (min(-97, (min((((arr_15 [i_4]) << 1)), ((min((arr_7 [i_1]), 15956)))))));
        }
        var_24 = (arr_8 [i_1]);
        var_25 *= 1023;
    }
    #pragma endscop
}
