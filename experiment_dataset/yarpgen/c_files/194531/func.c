/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194531
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194531 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194531
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 11; i_0 += 2) 
    {
        var_20 = var_14;
        var_21 -= (-(((/* implicit */int) (unsigned char)11)));
        var_22 = (~(((unsigned long long int) var_2)));
    }
    var_23 = ((/* implicit */_Bool) ((((_Bool) ((((/* implicit */long long int) ((/* implicit */int) var_4))) & (var_5)))) ? (((/* implicit */int) (!(((/* implicit */_Bool) ((short) 5680875115491988344ULL)))))) : (((/* implicit */int) (unsigned char)127))));
}
