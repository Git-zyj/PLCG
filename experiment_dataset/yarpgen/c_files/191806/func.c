/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191806
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191806 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191806
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 3; i_0 < 20; i_0 += 1) 
    {
        arr_3 [i_0 - 1] = ((/* implicit */int) var_5);
        var_19 += ((/* implicit */short) max((((/* implicit */int) min((var_0), (var_5)))), (max((var_13), (var_15)))));
        /* LoopSeq 1 */
        for (int i_1 = 2; i_1 < 19; i_1 += 3) 
        {
            arr_6 [i_0] [i_1] = max((var_6), (((/* implicit */unsigned int) max((((/* implicit */signed char) var_0)), (var_8)))));
            arr_7 [i_1] = ((/* implicit */long long int) min((((/* implicit */unsigned int) max((((/* implicit */short) var_8)), (min((var_1), (((/* implicit */short) var_8))))))), (var_14)));
        }
    }
    var_20 = ((/* implicit */unsigned long long int) var_13);
    var_21 = var_6;
    var_22 = ((/* implicit */unsigned long long int) var_17);
}
