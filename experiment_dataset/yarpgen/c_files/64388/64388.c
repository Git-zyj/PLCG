/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_9;
    var_16 = ((var_10 ? 8 : ((var_12 ? var_7 : var_3))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_17 = (max(var_17, (arr_4 [i_0] [8])));
                var_18 = ((-1550386693278469520 ? ((((var_0 << (((arr_0 [i_0 - 1]) - 5955014019967999693)))))) : 24537));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_13 [i_0 + 1] [i_1] [i_1] [i_3] [i_4] [i_3] [i_3] = 2147483647;
                                arr_14 [i_0] [6] [i_2] [i_1] [i_3] [8] = var_10;
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = 56890;
    #pragma endscop
}
