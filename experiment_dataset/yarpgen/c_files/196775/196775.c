/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196775
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196775 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196775
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        arr_2 [i_0] = var_0;
        arr_3 [i_0] [i_0] = ((((((arr_1 [i_0]) | (arr_0 [i_0 + 2])))) ? 108 : (107 & 125)));
        arr_4 [2] = ((-108 ? ((107 | ((max(93, 109))))) : (((((-(arr_1 [5]))) <= (!122))))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        var_19 = (max(var_19, var_6));
        var_20 = (!108);
    }
    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        var_21 = var_14;
        var_22 = var_17;
        var_23 = ((((min((arr_8 [i_2] [i_2]), (min(var_13, 102))))) ? ((93 ? 107 : var_4)) : (((((((var_14 ? 107 : (arr_8 [i_2] [i_2])))) <= (arr_7 [1] [8])))))));
        arr_10 [i_2] [i_2] = var_18;
    }
    var_24 = (~var_16);
    #pragma endscop
}
