/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47791
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47791 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47791
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -1385178527;
    var_13 = (max(((var_7 ? var_11 : ((var_3 ? var_3 : var_11)))), (var_2 | var_4)));
    var_14 = var_5;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_15 = (min(var_15, (arr_1 [6] [i_0])));
        arr_2 [i_0] = (max((arr_0 [i_0]), ((((arr_0 [i_0]) < 19285)))));
        var_16 = ((((~(arr_0 [i_0]))) | (min((arr_0 [i_0]), (arr_0 [i_0])))));
    }
    #pragma endscop
}
