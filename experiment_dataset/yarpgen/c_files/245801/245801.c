/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245801
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245801 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245801
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (!var_17);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_10 [i_0] [4] [i_1 - 1] [i_1 - 1] = ((-((var_0 ? (max((arr_3 [i_0] [i_0]), var_18)) : var_2))));
                    arr_11 [i_0] [i_1 + 1] [i_0] = (((4956749366842459151 / -4955014067448611363) != (!127)));
                    var_20 = (max(var_20, (((-(max(((min(var_4, 249))), (max((arr_3 [i_1] [i_0]), var_6)))))))));
                }
            }
        }
    }
    #pragma endscop
}
