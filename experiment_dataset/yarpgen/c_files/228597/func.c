/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228597
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228597 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228597
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        for (unsigned int i_1 = 1; i_1 < 13; i_1 += 4) 
        {
            for (unsigned int i_2 = 0; i_2 < 16; i_2 += 4) 
            {
                {
                    var_12 -= ((/* implicit */unsigned long long int) arr_5 [i_1] [i_1] [5U]);
                    arr_10 [i_1] = ((/* implicit */_Bool) ((max((arr_1 [i_0 - 1]), (arr_1 [i_0 - 1]))) ^ (((/* implicit */long long int) ((/* implicit */int) ((((/* implicit */int) (unsigned char)116)) > (((/* implicit */int) ((_Bool) (_Bool)1)))))))));
                }
            } 
        } 
    } 
    var_13 = ((((/* implicit */int) var_0)) < ((~(((/* implicit */int) (_Bool)1)))));
}
