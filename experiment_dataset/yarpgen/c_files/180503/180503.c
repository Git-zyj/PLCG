/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180503
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180503 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180503
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_8;
    var_16 = (0 ? 127 : (-127 - 1));
    var_17 = (min((-127 - 1), var_12));
    var_18 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_19 = -111;

                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    arr_9 [15] [i_1] [i_0] [i_0] = (arr_6 [i_1] [i_1] [i_0] [i_1]);
                    var_20 = (min(var_20, 32767));
                }
                for (int i_3 = 0; i_3 < 18;i_3 += 1)
                {
                    var_21 = (arr_7 [i_0] [i_1]);
                    arr_14 [i_0] = 32;
                }
                /* vectorizable */
                for (int i_4 = 2; i_4 < 17;i_4 += 1)
                {
                    var_22 = -6;

                    for (int i_5 = 0; i_5 < 18;i_5 += 1)
                    {
                        var_23 ^= ((arr_8 [i_5] [i_4 - 1] [1]) ^ (arr_17 [i_4 - 1] [i_4] [i_4 - 2]));
                        var_24 &= ((var_2 > (arr_8 [i_4 - 2] [i_4 - 2] [i_4])));
                        var_25 -= 24576;
                    }
                }
                for (int i_6 = 2; i_6 < 17;i_6 += 1)
                {
                    var_26 -= (((arr_12 [i_6] [i_6 - 2] [i_1]) - (min(-83, 10))));
                    arr_23 [i_6 - 1] [i_1] [i_0] = var_14;
                }
                var_27 += (var_6 | 248);
            }
        }
    }
    #pragma endscop
}
