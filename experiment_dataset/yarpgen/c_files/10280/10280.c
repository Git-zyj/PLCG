/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10280
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = min((--16038737102242218395), ((max((var_6 && 3428941054), var_0))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_19 = (3428941054 ? ((4396635114952734817 - ((3195302159 ? 0 : (arr_3 [i_0]))))) : ((((~var_10) ^ 0))));

                for (int i_2 = 2; i_2 < 17;i_2 += 1)
                {
                    var_20 = (min(var_20, var_1));

                    /* vectorizable */
                    for (int i_3 = 1; i_3 < 17;i_3 += 1)
                    {
                        var_21 = (max(var_21, (((1 ? -15 : (arr_5 [i_0] [i_2 - 2]))))));
                        arr_11 [i_2] = 11941;
                    }
                }
            }
        }
    }
    #pragma endscop
}
