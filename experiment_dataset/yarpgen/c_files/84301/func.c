/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84301
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 *= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_8))));
    var_16 = ((/* implicit */signed char) (~(var_12)));
    var_17 = (-(((/* implicit */int) (!(((/* implicit */_Bool) 4503591037435904LL))))));
    /* LoopSeq 1 */
    for (short i_0 = 3; i_0 < 22; i_0 += 4) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned short) ((((/* implicit */_Bool) (~(((/* implicit */int) (unsigned short)53024))))) ? (((/* implicit */int) (!(((/* implicit */_Bool) arr_0 [i_0 - 1] [i_0 - 1]))))) : (((/* implicit */int) var_13))));
        arr_4 [i_0] [i_0] = ((/* implicit */unsigned int) (unsigned char)243);
        var_18 = ((/* implicit */unsigned long long int) max((var_18), (var_11)));
    }
}
