/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151454
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151454 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151454
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = -var_4;
    var_19 = ((0 ? ((max((-127 - 1), (~var_0)))) : (((var_17 | var_13) | var_11))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            {
                var_20 = (((min((((arr_3 [i_0] [i_0]) == 9223372036854775796)), ((!(arr_0 [i_0]))))) ? (arr_0 [i_1]) : 9223372036854775796));
                var_21 = (max((arr_0 [i_1 + 2]), -var_13));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 19;i_3 += 1)
                    {
                        {
                            var_22 |= (-(var_1 - var_14));
                            arr_8 [14] [i_1] [i_1] [14] [i_3] = var_9;
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
