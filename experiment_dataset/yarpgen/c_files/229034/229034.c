/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229034
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229034 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229034
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(((min((!var_11), var_10))), (((~23439) ? (max(56338, 14834126620806653667)) : ((min(var_15, var_9)))))));
    var_17 = var_1;
    var_18 = (max(7772452237987749639, (max(-7772452237987749639, (var_7 && 1072888272)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_19 = (arr_2 [i_0]);
                var_20 = (((((arr_2 [i_0]) < var_13)) ? 58 : (max((arr_1 [i_0] [i_1]), (arr_1 [i_0] [i_1])))));
            }
        }
    }
    var_21 = (((var_1 || (!var_15))));
    #pragma endscop
}
