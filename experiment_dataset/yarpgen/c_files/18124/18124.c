/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18124
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18124 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18124
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_2, ((var_4 << (((var_8 ^ var_0) - 6165986103613335342))))));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((var_5 ? ((~(arr_0 [i_0 + 1] [i_0]))) : (arr_0 [0] [i_0 + 1])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                {
                    var_14 = (arr_8 [i_0 + 1] [i_0 + 1]);
                    arr_9 [i_0] = (arr_1 [i_0 - 1]);
                }
            }
        }
        arr_10 [i_0] = ((arr_8 [i_0 - 1] [i_0 - 1]) ? ((var_1 ? var_10 : (arr_7 [i_0] [3] [i_0]))) : (arr_0 [i_0 + 1] [i_0 - 1]));
        var_15 = (max(var_15, (((var_2 ? var_8 : var_6)))));
    }
    #pragma endscop
}
