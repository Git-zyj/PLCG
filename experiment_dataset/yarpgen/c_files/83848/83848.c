/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83848
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83848 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83848
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((var_18 ? var_9 : var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 7;i_1 += 1)
        {
            {
                var_20 -= (!-70);
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 8;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_21 -= ((9223372036854775780 >> (((!(((127 ? var_2 : var_7))))))));
                            var_22 -= (((-9223372036854775780 ? ((8848774020409786666 ? -9223372036854775781 : var_17)) : var_3)));
                            arr_12 [i_0] [i_3] [i_1] [i_2] = ((!(((-(arr_7 [i_0] [i_1] [i_2] [i_2]))))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
