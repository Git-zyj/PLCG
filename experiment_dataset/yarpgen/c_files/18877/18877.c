/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18877
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18877 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18877
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= var_2;
    var_17 = ((48 ? -40 : -40));

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0 - 2] = (((((((max(var_13, var_6))) ? (arr_0 [i_0]) : (arr_0 [i_0 - 1])))) ? 40 : (arr_1 [i_0 + 2])));
        var_18 = (arr_1 [i_0 + 2]);
        var_19 ^= (((((arr_0 [i_0 + 2]) & (arr_0 [i_0 - 2])))) ? (max((arr_0 [i_0 + 2]), (arr_0 [i_0 + 1]))) : (((arr_1 [i_0 - 1]) ? (arr_1 [i_0 - 1]) : var_13)));
    }
    #pragma endscop
}
