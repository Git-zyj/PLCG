/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243515
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243515 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243515
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((24333 ? var_2 : (!24333)))) ? (~1) : (max((!1), (min(10, var_14)))));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_6 [i_0] [4] [4] [i_2] = (max(1, 1));
                    var_17 = var_12;
                    var_18 = (~-117);
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_19 -= ((~((max((((!(arr_1 [i_2] [i_3])))), (min(89, 1)))))));
                                var_20 = max((~9223372036854775807), var_5);
                                var_21 = ((((min((-7635 / 41203), 20))) || ((((arr_0 [i_0]) ? var_14 : 1)))));
                            }
                        }
                    }
                    var_22 = (min(var_22, (((1 * (~var_14))))));
                }
            }
        }
    }
    #pragma endscop
}
