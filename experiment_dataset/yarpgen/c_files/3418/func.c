/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 3418
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=3418 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/3418
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */_Bool) var_16);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned char i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */_Bool) (unsigned char)240);
                var_19 ^= (~(((/* implicit */int) ((((/* implicit */int) arr_4 [i_1])) == (((/* implicit */int) var_5))))));
            }
        } 
    } 
    /* LoopNest 3 */
    for (short i_2 = 2; i_2 < 22; i_2 += 1) 
    {
        for (unsigned int i_3 = 0; i_3 < 24; i_3 += 2) 
        {
            for (long long int i_4 = 0; i_4 < 24; i_4 += 4) 
            {
                {
                    var_20 = ((/* implicit */long long int) (+(((/* implicit */int) arr_0 [i_2]))));
                    arr_14 [i_2] [i_3] [i_2] = ((/* implicit */unsigned short) var_10);
                }
            } 
        } 
    } 
}
