/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108371
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108371 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108371
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 = (((!-6) ? 20187 : (((!((min(5215174874388669389, (arr_5 [i_0])))))))));
                    arr_10 [i_1] [i_1] [i_1] [i_0] = ((((((0 || (arr_2 [i_0 + 3]))) ? (-2147483647 - 1) : ((max((arr_4 [i_1] [i_1]), 43))))) - (arr_3 [i_0])));
                    var_13 = ((((((((!(-2147483647 - 1)))) == (!1)))) + ((((arr_3 [i_1 + 3]) == (arr_9 [i_1] [i_1] [i_2]))))));
                }
            }
        }
    }
    var_14 = var_8;
    var_15 = ((((!((min(214, var_4))))) || ((52 && (!43)))));
    #pragma endscop
}
