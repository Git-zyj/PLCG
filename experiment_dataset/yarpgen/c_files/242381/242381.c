/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 242381
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=242381 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/242381
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_12 = (8590300314691605324 ? ((-(arr_2 [i_0]))) : ((max((arr_1 [i_0] [14]), (((30 <= (arr_4 [i_0] [i_0] [i_1]))))))));
                arr_6 [i_0] [i_0] = ((+(((((var_9 ? (arr_4 [i_0] [i_0 + 1] [i_1]) : (arr_1 [i_0] [i_0])))) ? var_5 : (arr_3 [i_0] [i_1])))));
            }
        }
    }
    var_13 = (max(var_13, ((min(var_5, 65528)))));
    #pragma endscop
}
