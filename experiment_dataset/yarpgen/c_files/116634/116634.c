/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116634
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116634 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116634
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_12 = 838254385;
                                arr_17 [i_0] [i_1] [i_2] [i_3] [i_4] [i_4] = 1;
                                arr_18 [i_0] [i_1 - 4] [i_2] [i_4] [i_4] = (((~-8171806677919406728) >> (-8934546852049857829 + 8934546852049857882)));
                            }
                        }
                    }
                }
                var_13 = (max(var_13, ((max((((~56) / ((8171806677919406699 >> (-56 + 59))))), 12240)))));
            }
        }
    }
    var_14 = -74;
    var_15 *= var_1;
    #pragma endscop
}
