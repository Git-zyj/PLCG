/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33929
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33929 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33929
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, var_8));

    for (int i_0 = 0; i_0 < 12;i_0 += 1) /* same iter space */
    {
        var_13 = (((((((!(arr_1 [i_0])))) % ((~(arr_1 [i_0])))))));
        arr_2 [i_0] &= (max(((((arr_1 [i_0]) * (((((arr_0 [1] [i_0]) > 231))))))), (arr_0 [i_0] [i_0])));
        arr_3 [i_0] [i_0] = ((((((arr_1 [i_0]) ? (arr_1 [i_0]) : var_3))) ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 12;i_1 += 1) /* same iter space */
    {
        arr_7 [i_1] = var_3;
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 11;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 12;i_3 += 1)
            {
                for (int i_4 = 2; i_4 < 11;i_4 += 1)
                {
                    {
                        arr_14 [11] [i_4 + 1] [i_4 + 1] [i_1] [i_4] [i_4] = 110;
                        arr_15 [i_1] [i_2] [10] [1] = (arr_8 [i_1]);
                    }
                }
            }
        }
        var_14 *= var_11;
        arr_16 [i_1] = ((var_5 ^ (!8098182945854560952)));
    }
    var_15 = (max(var_0, var_10));
    #pragma endscop
}
