/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247376
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247376 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247376
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        var_16 = (arr_1 [i_0 + 1] [i_0 - 1]);
        /* LoopNest 3 */
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 17;i_3 += 1)
                {
                    {
                        arr_10 [i_0] [i_1] [i_0] = (max(((((arr_7 [i_0 - 1] [i_0] [0] [i_0]) <= (arr_7 [i_0 - 1] [i_0] [i_0] [i_0])))), ((-(arr_7 [i_0 - 1] [i_0] [i_0 - 1] [i_0 - 1])))));
                        arr_11 [i_1] [i_0] [i_1] [i_1] = (max((arr_1 [i_0] [12]), (((-(arr_8 [i_2 + 2] [i_1] [i_1] [i_0]))))));
                    }
                }
            }
        }
        /* LoopNest 2 */
        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 17;i_5 += 1)
            {
                {
                    var_17 = arr_1 [i_0] [i_5];
                    var_18 = ((-8842858498550089534 ? (arr_6 [i_4] [1] [i_4]) : (arr_15 [i_5])));
                }
            }
        }
    }
    var_19 |= -1;
    #pragma endscop
}
