/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248654
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248654 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248654
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_19 = (max(var_19, ((((~var_6) ? (arr_1 [i_0]) : (!13328233786478039528))))));
                var_20 = (max(var_20, var_0));
                arr_6 [i_0] [i_0] [i_1] = (((arr_0 [i_1]) ? (~var_3) : (!60)));
                var_21 = (min(var_21, (((+((((arr_2 [i_1] [i_1]) > var_17))))))));
            }
        }
    }
    var_22 = (max(var_22, (((var_13 ? ((((var_0 - var_16) ? var_4 : -var_3))) : (((max(4182539557, 1099511627775)) * (var_17 / var_2))))))));
    #pragma endscop
}
