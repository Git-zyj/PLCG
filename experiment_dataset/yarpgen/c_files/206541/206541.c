/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206541
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206541 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206541
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((((!(var_9 <= var_4)))) >> (var_13 - 1610500400)));
    var_15 = var_2;

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (arr_1 [i_0]);
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 12;i_4 += 1)
                        {
                            {
                                arr_13 [9] [i_1] [2] [9] [i_4] = var_5;
                                var_16 = (max(var_16, (arr_0 [i_4])));
                                var_17 = (!((max((max((arr_8 [i_0] [i_0] [i_0] [i_0] [i_0]), var_8)), var_9))));
                                var_18 *= ((((!(118 <= 150))) ? -7982169720977817463 : ((max(1592038222, 138)))));
                            }
                        }
                    }
                    var_19 -= (arr_0 [i_0]);
                }
            }
        }
    }
    var_20 = var_12;
    var_21 = var_11;
    #pragma endscop
}
