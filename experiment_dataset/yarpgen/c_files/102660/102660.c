/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102660
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((1 <= var_12) ? (((var_16 < (var_14 || 1)))) : (((var_11 < var_16) ? (1 <= 1) : var_6))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_18 *= ((((max((589475736 >= 1), 20948))) << ((((-1 | (((1043681758 && (arr_0 [i_1])))))) + 9))));
                var_19 = ((var_3 == (min(((-(arr_1 [4]))), ((min(2332858922, 1561000908)))))));
            }
        }
    }
    #pragma endscop
}
