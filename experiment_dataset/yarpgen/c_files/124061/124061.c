/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124061
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124061 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124061
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_18 = ((((65520 * ((35 / (arr_2 [i_0]))))) << ((((30 ? (var_10 & var_0) : (~var_12))) - 576487151874613253))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_19 = var_10;
                            arr_9 [16] [i_1] [19] [i_2] [16] = (max(((min(65535, var_1))), 4294967295));
                        }
                    }
                }
            }
        }
    }
    var_20 ^= ((var_13 * (min((var_16 + var_4), var_4))));
    var_21 = var_2;
    #pragma endscop
}
