/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81886
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81886 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81886
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [5] = (arr_1 [i_0]);
        var_10 = (min(var_10, ((min((12272129171336288268 && 930642358785522485), (((arr_0 [i_0]) & -1465998163)))))));
    }
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        var_11 ^= ((-((((-1958888072 != (arr_1 [i_1])))))));
        arr_7 [i_1] = (((((var_9 / (arr_5 [7] [i_1]))) > (arr_0 [i_1]))) ? var_4 : ((1958888094 ? (var_9 > var_5) : (arr_6 [15] [i_1]))));
    }
    var_12 = var_3;
    var_13 = -var_2;
    var_14 = (-712559821 <= var_0);
    var_15 = var_1;
    #pragma endscop
}
