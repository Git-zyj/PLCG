/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99519
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99519 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99519
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_11 ? var_7 : ((((var_6 ? var_0 : var_8))))));
    var_15 = (((((var_5 ? (((var_1 ? var_13 : var_4))) : ((var_9 ? var_2 : var_9))))) ? ((var_13 ? ((var_8 ? var_4 : var_2)) : var_1)) : ((((((var_5 ? var_0 : var_7))) ? ((var_7 ? var_4 : var_9)) : var_8)))));
    var_16 = var_1;

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_17 = ((((arr_1 [i_0]) ? var_10 : (arr_1 [i_0]))));
        var_18 = (arr_1 [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        var_19 = (arr_3 [i_1]);
        var_20 = var_2;
    }
    #pragma endscop
}
