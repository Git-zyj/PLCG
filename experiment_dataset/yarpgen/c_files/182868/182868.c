/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182868
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182868 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182868
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 += (max(var_10, -1252966024));

    for (int i_0 = 1; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_19 = (!1279916577641791329);
        var_20 = (((-(arr_0 [i_0 + 4]))));
        var_21 = (((((var_2 ? var_3 : -218337883)))) << (((arr_0 [i_0 + 4]) + 16889)));
        var_22 = -17259595555634348385;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 15;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        arr_12 [i_0 + 2] [i_0 + 2] [0] [13] = -98;
                        arr_13 [i_0] [i_3] [i_2] = var_14;
                    }
                    var_23 = (max((var_16 + 504), (min((arr_3 [i_2]), 2147483647))));
                    var_24 = -19323;
                }
            }
        }
    }
    for (int i_4 = 1; i_4 < 14;i_4 += 1) /* same iter space */
    {
        var_25 = var_17;
        var_26 = var_4;
    }
    var_27 = ((~((max(var_12, var_17)))));
    #pragma endscop
}
