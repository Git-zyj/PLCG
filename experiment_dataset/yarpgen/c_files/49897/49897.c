/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (0 ? -6490355980978932793 : var_4);
    var_17 = (max(var_9, ((~(3 > -1)))));

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_18 = (arr_0 [i_0 - 1]);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 1;i_3 += 1)
                {
                    {
                        var_19 = ((!(((54050 * (arr_6 [i_2] [i_1] [i_1] [i_3 - 1] [i_1]))))));
                        arr_9 [i_3] [i_3] [i_2] [i_3] = var_9;
                        var_20 &= (arr_1 [i_3] [i_1]);
                        var_21 = ((((var_8 ? (arr_6 [i_3] [i_3 - 1] [i_0 - 1] [i_0 - 1] [i_0 - 1]) : var_5)) << var_9));
                    }
                }
            }
        }
    }
    var_22 = -var_9;
    #pragma endscop
}
