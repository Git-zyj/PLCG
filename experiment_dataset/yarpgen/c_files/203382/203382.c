/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203382
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203382 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203382
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = min(var_15, (min((var_10 & var_3), ((-1160252369 ? var_5 : var_4)))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_2] = (arr_7 [i_2] [i_2] [i_2] [i_2]);
                    arr_10 [i_0 + 1] [i_0 + 1] [i_0 + 1] = ((((((arr_3 [i_0 + 1]) ? var_12 : (arr_3 [i_0 - 1])))) ? (arr_2 [i_0 + 1]) : (((((arr_7 [4] [i_1] [i_1] [i_2]) && (((arr_3 [i_2]) <= -2140341164))))))));
                }
            }
        }
    }
    #pragma endscop
}
