/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168642
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168642 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168642
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */_Bool) var_11);
    var_15 ^= var_0;
    /* LoopSeq 2 */
    for (int i_0 = 0; i_0 < 16; i_0 += 3) 
    {
        arr_2 [i_0] = ((/* implicit */_Bool) arr_1 [i_0]);
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned char) arr_0 [i_0]);
    }
    for (unsigned char i_1 = 0; i_1 < 13; i_1 += 2) 
    {
        arr_8 [i_1] [i_1] = ((/* implicit */unsigned char) ((((((int) arr_5 [i_1])) % (min((arr_1 [i_1]), (((/* implicit */int) arr_6 [i_1])))))) == (((((/* implicit */_Bool) arr_1 [i_1])) ? (arr_1 [i_1]) : (((/* implicit */int) arr_6 [i_1]))))));
        arr_9 [i_1] [i_1] = max((arr_5 [i_1]), (((((/* implicit */_Bool) arr_5 [i_1])) ? (((/* implicit */int) arr_0 [i_1])) : (arr_5 [i_1]))));
    }
}
