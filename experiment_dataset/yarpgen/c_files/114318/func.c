/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114318
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114318 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114318
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) (short)1024);
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 18; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 16; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */long long int) arr_1 [i_0]);
                var_18 = ((/* implicit */long long int) ((((/* implicit */_Bool) ((((_Bool) arr_1 [i_0])) ? (min((((/* implicit */long long int) arr_2 [5ULL] [5ULL])), (3317692692738061954LL))) : (((/* implicit */long long int) (~(((/* implicit */int) var_4)))))))) ? (((((/* implicit */unsigned long long int) var_0)) ^ (max((arr_1 [i_0]), (((/* implicit */unsigned long long int) var_12)))))) : (((1384208184244988509ULL) * (17062535889464563106ULL)))));
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned char) max((var_19), (((/* implicit */unsigned char) var_0))));
}
