/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83234
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83234 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83234
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            var_20 ^= var_14;
            arr_6 [i_0] = ((-(arr_3 [i_0] [i_1])));
            var_21 = var_4;
            arr_7 [i_1] [i_0] [i_1] = (((arr_3 [i_1] [i_1]) <= (arr_4 [i_0] [i_0] [i_0])));
            arr_8 [i_1] [i_0] = (((arr_5 [i_0]) < (arr_3 [i_0] [10])));
        }
        var_22 = (-567030302 && 501841295);
    }
    var_23 = var_12;
    var_24 = (max(((((max(1807643795, -23355)) / ((-23355 ? 23346 : var_5))))), (max(var_8, (max(var_12, var_7))))));
    var_25 = var_8;
    #pragma endscop
}
