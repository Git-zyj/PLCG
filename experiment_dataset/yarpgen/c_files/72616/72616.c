/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72616
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72616 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72616
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_14;
    var_21 = (((~776556857494825235) ? 20747 : var_19));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] [i_2] = 8690934202138438117;
                    arr_10 [i_0] = (max(var_9, var_18));
                    arr_11 [i_2] [i_1] = var_9;
                    arr_12 [i_0] [i_2] [i_2] [i_2] &= (arr_5 [i_2] [11] [i_0]);
                }
            }
        }
    }
    #pragma endscop
}
