/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 191105
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=191105 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/191105
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 2; i_0 < 18; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 20; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 20; i_2 += 1) 
            {
                {
                    arr_7 [i_0] [i_2] [i_0 - 2] = ((/* implicit */long long int) ((((/* implicit */int) ((unsigned char) var_3))) <= (((/* implicit */int) (!(((/* implicit */_Bool) var_6)))))));
                    var_15 = ((/* implicit */unsigned char) min((((/* implicit */unsigned long long int) ((signed char) (unsigned char)243))), (min((var_10), (((/* implicit */unsigned long long int) ((((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)7))) ^ (var_6))))))));
                }
            } 
        } 
    } 
    var_16 = ((/* implicit */unsigned short) var_0);
    var_17 = ((/* implicit */unsigned int) var_12);
}
