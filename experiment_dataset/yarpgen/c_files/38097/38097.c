/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38097
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38097 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38097
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        var_17 = ((4701566035510497791 ? 0 : 255));
        arr_2 [i_0] = (max((arr_1 [i_0 + 1]), (arr_1 [i_0 - 1])));

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            /* LoopNest 3 */
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 21;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 21;i_4 += 1)
                    {
                        {
                            arr_15 [i_0] [i_1] [i_3] [i_3] [i_3] = 15612249862350418076;
                            var_18 = ((-((var_3 >> (((arr_14 [i_0] [i_0] [i_1] [i_1] [18] [i_3] [i_4]) ? var_1 : var_12))))));
                        }
                    }
                }
            }
            var_19 = ((+(((((arr_8 [i_0] [i_0 - 1] [i_0] [i_1]) ^ var_14))))));
        }
    }
    var_20 = var_13;
    #pragma endscop
}
