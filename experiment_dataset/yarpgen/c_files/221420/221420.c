/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221420
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221420 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221420
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_6 ? ((((var_9 - var_6) + var_9))) : var_5));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_13 = (max(var_13, (((((((arr_0 [i_1]) ? (arr_6 [i_2]) : (arr_6 [i_2])))) && (arr_4 [i_0 - 4] [i_2]))))));
                    var_14 = (arr_5 [i_1] [2]);
                    arr_11 [i_0] [i_2] [i_1] [i_0] = (((4825805618472880454 ? (arr_3 [i_0 + 1] [14]) : (arr_3 [i_0 + 1] [6]))));
                }
            }
        }
    }
    var_15 = var_7;
    #pragma endscop
}
