/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219575
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219575 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219575
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 += var_6;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((-((((max((arr_3 [13] [i_0] [i_0]), -32755))) ? (min(18446744073709551615, var_3)) : ((((arr_3 [i_0] [i_0] [i_1]) ? var_0 : 2223437455752010907)))))));
                var_16 = (((((arr_1 [i_0 - 1] [i_0]) ? (arr_1 [i_0 - 1] [i_0]) : (arr_1 [i_0 - 1] [i_0]))) + ((((!(arr_1 [i_0 - 1] [i_0])))))));
            }
        }
    }
    #pragma endscop
}
