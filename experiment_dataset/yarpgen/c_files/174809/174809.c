/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174809
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174809 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174809
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0] = ((~(arr_0 [i_0])));
        arr_4 [i_0] = (max((arr_0 [i_0]), (arr_2 [i_0])));
        arr_5 [i_0] = ((((var_0 >= (arr_2 [i_0]))) ? (((arr_2 [i_0]) & (arr_2 [i_0]))) : (((arr_2 [i_0]) / (arr_2 [i_0])))));
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1)
    {
        var_10 = (((max(var_1, (arr_6 [i_1] [i_1]))) % (((45644 ? var_7 : var_3)))));
        arr_8 [i_1] = var_9;
    }
    var_11 = (min(var_11, ((((var_7 != -27044) && (((min(var_0, 204867091)))))))));
    var_12 = (!var_0);
    #pragma endscop
}
