/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = -var_11;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_19 = (max(((((((var_4 ? var_6 : (arr_3 [i_0] [i_0] [i_0])))) || 10173549440825120870))), ((((((arr_3 [i_0] [i_0] [i_0]) ? (arr_2 [i_0]) : var_7))) ? ((-5812753224495506038 ? 12375132191610968898 : 192)) : ((6071611882098582717 & (arr_1 [i_0])))))));
                var_20 = var_5;
            }
        }
    }
    #pragma endscop
}
