/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154668
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154668 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154668
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        var_14 = (~var_4);
        arr_3 [i_0] [i_0] = ((var_1 ? ((var_7 >> (var_13 + 6970520572205310039))) : (arr_2 [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_15 = 1;
                                arr_15 [i_0] [i_0] [i_0] [i_0] [i_2] [i_0] [22] = (((((arr_9 [i_0] [i_1] [i_2] [i_3]) ? var_12 : var_13))) ? var_10 : (1 * var_8));
                            }
                        }
                    }
                    arr_16 [i_0] [i_0] [i_0] = var_4;
                    var_16 = var_2;
                    var_17 += ((var_3 >> (var_9 + 5396494034906282828)));
                }
            }
        }
    }

    for (int i_5 = 0; i_5 < 20;i_5 += 1)
    {
        var_18 = var_13;
        arr_19 [i_5] [i_5] = var_7;
    }
    #pragma endscop
}
