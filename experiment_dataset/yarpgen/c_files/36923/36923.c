/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36923
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36923 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36923
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((!-881852468) / ((((((var_15 ? var_11 : var_2))) ? var_5 : ((min(var_14, var_15))))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = (((-881852471 >= 881852499) ? (arr_0 [i_1]) : (((((881852468 ? -881852468 : 1)) * (arr_1 [i_0]))))));
                arr_4 [i_0] = (max((max((((var_5 ? 5 : 24470))), 9186513225244052711)), (arr_2 [i_0] [i_1])));
            }
        }
    }
    #pragma endscop
}
