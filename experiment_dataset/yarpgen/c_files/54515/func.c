/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54515
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54515 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54515
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 |= ((/* implicit */unsigned char) max((((/* implicit */unsigned long long int) var_9)), ((+(max((var_14), (((/* implicit */unsigned long long int) var_1))))))));
    /* LoopNest 3 */
    for (short i_0 = 3; i_0 < 14; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 16; i_1 += 4) 
        {
            for (long long int i_2 = 2; i_2 < 17; i_2 += 2) 
            {
                {
                    var_18 = ((/* implicit */unsigned int) min(((-2147483647 - 1)), (((/* implicit */int) (unsigned short)23359))));
                    arr_7 [16LL] [16LL] [i_0] = ((/* implicit */int) arr_1 [i_1]);
                }
            } 
        } 
    } 
    var_19 *= ((/* implicit */_Bool) (+(18446744073709551615ULL)));
}
