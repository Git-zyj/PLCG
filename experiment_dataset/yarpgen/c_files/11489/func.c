/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11489
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11489 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11489
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */int) var_6);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        for (int i_1 = 0; i_1 < 25; i_1 += 3) 
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 24; i_2 += 4) 
                {
                    for (short i_3 = 2; i_3 < 22; i_3 += 3) 
                    {
                        {
                            arr_9 [i_3] [i_0] = ((/* implicit */long long int) arr_4 [i_0] [i_2] [i_3]);
                            var_17 = ((/* implicit */short) (~(((/* implicit */int) ((((/* implicit */_Bool) arr_8 [i_3 + 3] [i_1] [i_2] [i_3 + 2])) || (((/* implicit */_Bool) var_0)))))));
                            var_18 = ((/* implicit */long long int) (~(((/* implicit */int) (_Bool)1))));
                        }
                    } 
                } 
                arr_10 [i_0] [i_1] = ((/* implicit */unsigned char) ((((((/* implicit */_Bool) arr_7 [i_1] [i_1] [i_0] [i_0])) && (((/* implicit */_Bool) 7267978983220116642ULL)))) ? (max((7267978983220116642ULL), (((/* implicit */unsigned long long int) arr_1 [i_0])))) : (var_14)));
            }
        } 
    } 
    var_19 = ((/* implicit */int) (+(11178765090489434973ULL)));
    var_20 = ((/* implicit */unsigned int) var_0);
}
