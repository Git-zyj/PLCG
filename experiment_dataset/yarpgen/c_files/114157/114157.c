/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114157
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114157 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114157
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= 17388586236957733586;
    var_11 = ((((!(2343834221 && var_2))) ? (((var_8 ? (var_3 != var_3) : 122))) : var_9));
    var_12 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] = (((!-122) ? (arr_1 [i_0 - 2]) : 127));
                var_13 = ((arr_3 [i_0] [i_0] [i_1]) ? ((((!(!0))))) : (max((arr_0 [i_0 - 1]), (arr_2 [i_1] [i_1] [i_1 - 2]))));
                var_14 = (min(var_14, (arr_0 [i_1 + 1])));
                var_15 = (min(var_15, (((!(arr_0 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
