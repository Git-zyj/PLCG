/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183055
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 &= ((/* implicit */unsigned char) var_13);
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 15; i_0 += 1) 
    {
        for (int i_1 = 4; i_1 < 13; i_1 += 1) 
        {
            for (int i_2 = 2; i_2 < 14; i_2 += 1) 
            {
                {
                    var_17 = ((/* implicit */signed char) min((var_17), (((/* implicit */signed char) var_3))));
                    arr_7 [i_0] [i_1] [i_0] = (~(((/* implicit */int) ((((/* implicit */unsigned int) -1764665477)) > (3168592217U)))));
                    arr_8 [i_0] [i_1] [i_2 - 1] = ((/* implicit */unsigned long long int) (~((-(1108003049)))));
                    var_18 = ((/* implicit */unsigned char) 3168592217U);
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */unsigned short) var_12);
    var_20 = ((/* implicit */unsigned short) min((var_20), (((/* implicit */unsigned short) var_15))));
    var_21 &= ((/* implicit */long long int) 3168592234U);
}
