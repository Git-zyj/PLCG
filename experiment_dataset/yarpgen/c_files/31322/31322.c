/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31322
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31322 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31322
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(((220 ? var_4 : (max(var_9, var_6)))), var_4));
    var_12 = var_10;
    var_13 = var_1;
    var_14 = (max(var_14, (((((((((35 ? 60 : 10229))) ? var_8 : ((var_7 ? 0 : var_2))))) ? (var_7 + var_1) : var_7)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_15 += ((-((var_2 ? (((60 ? -32756 : 131))) : (var_3 % 1)))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_16 *= (max(33, (((3 >= ((48 ? 175 : 179)))))));
                                var_17 += (((((var_10 ? var_8 : var_8))) == (max(var_7, var_9))));
                                var_18 = (min(var_18, var_7));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
