/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208049
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208049 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208049
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                var_13 = ((!(((((max(var_0, var_11))) ? (arr_0 [i_0]) : (arr_2 [i_1]))))));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        {
                            var_14 = (arr_6 [i_0] [i_0]);
                            var_15 = (arr_5 [i_0] [i_1] [i_3]);
                            var_16 ^= ((((var_11 ? var_7 : (arr_8 [i_0] [12] [i_2] [4])))));
                            var_17 = min((((arr_8 [i_1 - 2] [i_1 + 1] [i_1] [i_1]) - (arr_7 [i_1 - 1] [i_3 + 1] [i_1]))), var_7);
                            var_18 = var_2;
                        }
                    }
                }
            }
        }
    }
    var_19 = var_10;
    #pragma endscop
}
