/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                arr_7 [i_1 - 1] [i_1 - 1] [i_1 - 1] = (((arr_0 [i_1 + 1] [i_1 + 2]) >= (((((arr_0 [i_1 + 2] [i_1 + 3]) + 2147483647)) << (((((arr_0 [i_1 + 2] [i_1 + 3]) + 7454)) - 10))))));
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            arr_13 [i_2] [i_1] [i_2 - 1] [i_3] = (((~(arr_4 [i_3] [i_3] [i_2 - 2]))) >= (((arr_3 [i_3] [i_3] [i_2 + 4]) ? (arr_4 [i_3] [i_3] [i_2 - 2]) : (arr_4 [i_3] [i_2 - 2] [i_2 + 2]))));
                            arr_14 [i_0] [i_2] [i_2] [i_2] = (~4294967295);
                        }
                    }
                }
                /* LoopNest 3 */
                for (int i_4 = 0; i_4 < 23;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 1;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 23;i_6 += 1)
                        {
                            {
                                var_17 = var_6;
                                arr_24 [i_0] [i_5] [i_5] = var_11;
                                arr_25 [i_0] [i_5] [i_5] [i_5] = var_8;
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = (((((var_13 ? (!var_15) : (!var_15)))) ? var_13 : var_1));
    var_19 = var_6;
    #pragma endscop
}
