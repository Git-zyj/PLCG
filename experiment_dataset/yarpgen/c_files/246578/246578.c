/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246578
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                var_11 = ((!((!(17179869183 << 1)))));
                arr_6 [i_0] [i_0] |= ((((((arr_5 [i_1 - 2] [i_1] [i_1]) ? 9926 : (arr_5 [i_1 - 1] [10] [4])))) ? (((arr_5 [i_1 - 2] [1] [2]) ? -654652499 : 1)) : 1));
            }
        }
    }
    var_12 = (8035212405141760439 && 1);
    var_13 = (-502429935 ? -8035212405141760439 : var_5);
    #pragma endscop
}
