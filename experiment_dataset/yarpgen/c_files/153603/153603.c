/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 153603
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=153603 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/153603
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_8 [i_1] = ((32766 * (((arr_0 [i_0] [i_0]) * (max(6423115718950847983, 4145113462))))));
                var_15 = (((0 && (arr_4 [i_0] [i_0] [i_0]))));
            }
        }
    }
    var_16 = 42820;
    var_17 = (max((~var_7), (((var_3 > var_3) ? (max(-3529229414123503205, 87)) : (((var_0 ? var_3 : var_3)))))));
    #pragma endscop
}
