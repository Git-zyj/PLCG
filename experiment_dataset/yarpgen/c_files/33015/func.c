/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33015
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33015 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33015
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 1; i_0 < 19; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            {
                var_12 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (unsigned char)248))));
                var_13 += (~(((((/* implicit */long long int) 2364956286U)) & (-9201535061711108362LL))));
                var_14 = (+((~(arr_4 [i_0 + 1] [i_0 + 4]))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23; i_2 += 1) 
                {
                    for (signed char i_3 = 1; i_3 < 20; i_3 += 1) 
                    {
                        {
                            var_15 = ((/* implicit */int) (!(((/* implicit */_Bool) (unsigned char)248))));
                            arr_11 [i_1] [i_1] [i_1] [i_1] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) (~(7936))))));
                        }
                    } 
                } 
                /* LoopNest 3 */
                for (unsigned short i_4 = 2; i_4 < 21; i_4 += 1) 
                {
                    for (long long int i_5 = 0; i_5 < 23; i_5 += 2) 
                    {
                        for (unsigned short i_6 = 0; i_6 < 23; i_6 += 2) 
                        {
                            {
                                var_16 = ((/* implicit */unsigned short) ((9201535061711108370LL) + (-9201535061711108362LL)));
                                var_17 &= ((/* implicit */unsigned long long int) -7923);
                                var_18 = ((/* implicit */int) arr_13 [i_0] [i_0] [i_0]);
                            }
                        } 
                    } 
                } 
            }
        } 
    } 
    var_19 *= ((/* implicit */long long int) ((((/* implicit */int) (short)12288)) != (7936)));
}
