/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165412
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165412 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165412
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        var_13 = (max(var_13, (arr_0 [i_0 + 1])));
        arr_2 [i_0] [i_0] = ((((-(arr_1 [i_0 - 3]))) / (1 << 0)));
    }
    var_14 = var_10;

    for (int i_1 = 2; i_1 < 9;i_1 += 1)
    {
        var_15 *= 0;
        arr_5 [i_1] = var_5;

        /* vectorizable */
        for (int i_2 = 1; i_2 < 9;i_2 += 1)
        {
            var_16 = (min(var_16, (((-(arr_3 [0] [0]))))));
            var_17 = var_3;
        }
        for (int i_3 = 2; i_3 < 10;i_3 += 1)
        {

            /* vectorizable */
            for (int i_4 = 3; i_4 < 9;i_4 += 1)
            {
                var_18 |= ((1 >> (1962361434 - 1962361428)));
                arr_13 [i_1] [i_3] [i_1] = (((arr_9 [i_1] [i_3] [i_1]) + var_8));
                arr_14 [i_1] = (arr_12 [i_1] [i_3] [i_4]);
            }
            arr_15 [i_1] [i_3 + 1] [i_1] = (min(1415508178, 123812097211638618));
        }
    }
    #pragma endscop
}
