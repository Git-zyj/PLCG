/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80175
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80175 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80175
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_14 = (min(var_14, ((max((arr_2 [i_0 - 1]), (arr_2 [i_0 + 1]))))));
                var_15 = (((min(var_0, var_7))));
            }
        }
    }
    var_16 = (max((((!1901858636) ? ((var_7 ? 65 : 6545180875930266209)) : (1 != 1))), ((min(var_6, (var_1 - 82))))));
    var_17 = (((((max(var_2, var_5)) & var_8))));
    var_18 = ((-((var_12 / (var_1 * -9)))));
    #pragma endscop
}
