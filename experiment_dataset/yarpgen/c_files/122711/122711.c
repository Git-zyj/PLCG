/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [i_1] [12] = (((arr_4 [i_0] [i_1] [i_1]) ? (arr_4 [i_0] [i_1] [i_0]) : ((((arr_4 [i_0] [i_1] [i_1]) == var_15)))));
                arr_6 [i_1] = (((((var_0 ? 12228 : 24) * (((arr_4 [i_0] [i_1] [i_1]) ? 24 : var_7))))));
            }
        }
    }
    var_20 = (((((-6 ? var_15 : var_10))) ? (var_19 == var_9) : ((~((1798106353 ? var_9 : var_19))))));
    var_21 = var_2;
    var_22 = ((-2120 ? 30746 : -5469));
    var_23 = var_15;
    #pragma endscop
}
