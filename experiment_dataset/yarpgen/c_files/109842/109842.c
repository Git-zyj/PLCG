/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109842
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109842 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109842
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_1;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            arr_4 [i_0] [i_0] = ((~(((!(((-(arr_2 [i_0] [i_1])))))))));

            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                arr_8 [i_0] [i_2] [i_0] [0] = var_0;
                var_12 = ((((((!(arr_7 [0])))) >> (((max(-7166, 725721766)) - 4294960116)))));
                arr_9 [i_2] [i_1] [i_2] = ((+((((arr_6 [i_2]) ? var_5 : var_9)))));
            }
            for (int i_3 = 0; i_3 < 1;i_3 += 1)
            {
                var_13 = (min(var_13, var_10));
                var_14 = (!var_4);
            }
        }
        arr_14 [i_0] = (~7166);
        var_15 = ((-(min((arr_11 [i_0] [i_0] [8] [i_0]), var_0))));
        var_16 = (((14041 - 3) % (((min(var_4, var_7))))));
    }
    var_17 = ((var_9 ? (((var_6 ? var_0 : -var_2))) : ((-1 ? (min(var_9, var_4)) : var_8))));
    #pragma endscop
}
