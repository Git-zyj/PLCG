/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94153
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94153 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94153
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) var_5);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 2; i_0 < 18; i_0 += 3) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) 847243154123344731LL)) ? (((/* implicit */long long int) arr_0 [i_0] [i_0 + 1])) : (-2825093513349633288LL)));
        var_15 = ((/* implicit */short) ((((/* implicit */_Bool) 8646911284551352320ULL)) && (((/* implicit */_Bool) 7036328118362754245ULL))));
        var_16 = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) (unsigned short)6865))));
    }
    var_17 = ((/* implicit */unsigned int) var_9);
}
