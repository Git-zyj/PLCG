/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18815
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 17;i_2 += 1)
            {
                {
                    var_15 = ((!((!((min((arr_7 [i_0] [i_1] [i_2 - 2]), var_10)))))));
                    arr_9 [i_0] [i_0] [i_2] = (min(var_5, (max(var_3, -var_13))));
                    arr_10 [i_0] [i_2] = ((+(((arr_2 [i_1 - 2] [i_1 - 1]) ? (arr_5 [i_2 - 1] [i_2 + 1] [13] [i_1 - 2]) : (min((arr_5 [i_0] [15] [i_1] [i_2 - 1]), var_7))))));
                }
            }
        }
    }
    var_16 = (min(var_16, -14925));
    var_17 |= (40642 * 0);
    #pragma endscop
}
