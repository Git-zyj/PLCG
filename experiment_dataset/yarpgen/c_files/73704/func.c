/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73704
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73704 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73704
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned long long int) ((min(((-(var_4))), (((/* implicit */unsigned int) var_3)))) & (((/* implicit */unsigned int) ((/* implicit */int) var_6)))));
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        for (long long int i_1 = 2; i_1 < 12; i_1 += 3) 
        {
            {
                arr_6 [i_0] [i_1 + 2] = var_9;
                var_12 = ((/* implicit */unsigned long long int) ((signed char) ((6283347242719064705ULL) * (((/* implicit */unsigned long long int) ((/* implicit */int) (((_Bool)1) || (((/* implicit */_Bool) arr_5 [i_1 - 2])))))))));
            }
        } 
    } 
}
