/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170176
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170176 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170176
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_15 &= ((!((min((arr_1 [i_0]), (((arr_1 [i_0]) + 177)))))));
        arr_2 [i_0] = 2022377519;
        arr_3 [i_0] = (((!(((arr_1 [i_0]) || (arr_1 [i_0]))))));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        arr_13 [i_3] [i_2] [i_1] [i_1] [i_0] = -91;
                        arr_14 [i_0] [i_1] [i_2] [i_2] [i_3] [1] = (arr_1 [i_0]);
                        arr_15 [i_0] [i_2] [5] [i_3] = (((~(((arr_5 [i_0] [4]) ? (arr_1 [i_2]) : (arr_9 [i_0] [i_1] [i_1] [i_2]))))));
                    }
                }
            }
        }
    }
    var_16 = 32767;
    var_17 ^= var_12;
    #pragma endscop
}
