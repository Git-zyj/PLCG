/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5973
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5973 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5973
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) min((((/* implicit */int) (signed char)56)), (var_4)));
    var_11 = ((/* implicit */unsigned short) (signed char)69);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        for (unsigned short i_1 = 3; i_1 < 23; i_1 += 2) 
        {
            {
                var_12 = ((/* implicit */signed char) (+(((/* implicit */int) min(((unsigned short)65514), (((/* implicit */unsigned short) (signed char)-78)))))));
                var_13 = ((/* implicit */unsigned short) max((var_13), (((/* implicit */unsigned short) var_0))));
            }
        } 
    } 
}
