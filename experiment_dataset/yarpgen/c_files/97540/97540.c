/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97540
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97540 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97540
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 23;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                var_15 += var_5;
                                var_16 *= min((((arr_6 [i_3 - 2] [i_3] [i_3 - 2] [i_3]) ? 1 : -8675)), 1);
                            }
                        }
                    }
                    var_17 = ((((min(8685, 1))) ? (arr_14 [i_0] [i_0] [i_0 + 1] [i_0 - 1] [8] [i_0 - 1]) : (((var_4 + var_3) ? var_3 : (arr_0 [i_0])))));
                }
            }
        }
        var_18 += ((arr_3 [i_0 + 1]) <= -8675);
    }
    for (int i_5 = 1; i_5 < 14;i_5 += 1)
    {
        var_19 = (max(var_19, ((max((((((arr_10 [i_5] [i_5] [23] [i_5]) + 1)) + (arr_3 [i_5 + 3]))), -var_6)))));
        var_20 -= 8674;
    }
    var_21 = var_9;
    #pragma endscop
}
