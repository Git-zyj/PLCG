/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196887
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (arr_4 [i_0]);
                arr_7 [i_0] [i_1] = var_0;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 17;i_4 += 1)
                        {
                            {
                                var_11 = ((~(arr_13 [2] [i_1] [i_2] [i_1] [i_4])));
                                arr_16 [i_0] [i_4] [i_2] [2] [6] [i_4] [i_3] = (((~-1082339741) != (arr_5 [i_1] [4] [i_2])));
                                arr_17 [9] [9] [i_2] [i_3] [i_4] = ((-(arr_10 [i_0])));
                            }
                        }
                    }
                }
                var_12 -= (max((arr_3 [i_1]), (arr_3 [i_0])));
            }
        }
    }
    var_13 &= var_0;
    #pragma endscop
}
