/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134990
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134990 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134990
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 22;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0 + 1] = ((!(!var_1)));
        var_11 ^= ((((!(~-3521298462279445654)))));
    }
    for (int i_1 = 1; i_1 < 22;i_1 += 1) /* same iter space */
    {

        for (int i_2 = 0; i_2 < 23;i_2 += 1) /* same iter space */
        {
            var_12 = -63;
            var_13 *= ((~((-(((!(arr_4 [0]))))))));
            arr_7 [i_1 - 1] [i_1] [i_2] = ((~(((!(((~(arr_4 [i_1])))))))));
            var_14 &= ((!((((~(arr_5 [i_1 + 1] [i_2])))))));
        }
        /* vectorizable */
        for (int i_3 = 0; i_3 < 23;i_3 += 1) /* same iter space */
        {
            var_15 = (~var_9);
            var_16 = ((~(~var_7)));
            arr_12 [i_1] [i_3] = ((!(~-78)));
            var_17 = ((~(arr_3 [i_3])));
        }
        for (int i_4 = 0; i_4 < 23;i_4 += 1)
        {
            arr_15 [i_1] = (~var_6);
            arr_16 [i_1] = ((((!(((~(arr_8 [i_1 + 1] [i_4] [i_4]))))))));
        }
        var_18 = ((~(!78)));
        arr_17 [i_1] = ((-((~(~var_7)))));

        for (int i_5 = 0; i_5 < 23;i_5 += 1)
        {
            arr_20 [i_1] [i_5] = ((!(((-(!var_9))))));
            arr_21 [i_1] = ((~((~(~-69)))));
        }
    }
    /* LoopNest 2 */
    for (int i_6 = 0; i_6 < 18;i_6 += 1)
    {
        for (int i_7 = 3; i_7 < 17;i_7 += 1)
        {
            {
                var_19 = ((~((~(~var_10)))));
                var_20 = (min(var_20, (((((!((!(arr_6 [22] [i_7 - 2]))))))))));
                arr_29 [i_7 - 3] [i_7 - 1] [i_6] = -17151;
                /* LoopNest 2 */
                for (int i_8 = 0; i_8 < 18;i_8 += 1)
                {
                    for (int i_9 = 0; i_9 < 18;i_9 += 1)
                    {
                        {

                            for (int i_10 = 1; i_10 < 16;i_10 += 1)
                            {
                                arr_39 [i_9] = (arr_14 [i_10 - 1]);
                                var_21 = (~-9223372036854775789);
                                var_22 *= ((~((~(~var_5)))));
                            }
                            arr_40 [i_6] [i_9] [i_6] [i_6] [i_6] = -146;
                            var_23 = 1963776344;
                            arr_41 [i_6] [i_9] [i_8] [i_9] = (!var_7);
                            arr_42 [i_6] [i_7 - 3] [i_8] [i_9] [i_9] = ((!((!(arr_14 [i_6])))));
                        }
                    }
                }
                var_24 = ((!(((~(~-63))))));
            }
        }
    }
    var_25 = ((~((-(!var_5)))));
    #pragma endscop
}
