/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168257
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168257 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168257
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, var_2));
    var_12 = (max(((min(49, 8))), (~var_2)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 = ((((((((var_4 ? (arr_3 [i_0] [i_0]) : var_8))) && 9223372036854775807))) & ((var_3 << (8 & 18446744073709551615)))));
                var_14 = (min(var_14, ((((((var_5 <= var_7) <= ((~(arr_3 [i_0] [8]))))) && ((max(-var_8, var_0))))))));
            }
        }
    }
    #pragma endscop
}
