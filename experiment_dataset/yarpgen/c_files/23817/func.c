/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23817
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23817 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23817
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned char) var_7);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 16; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 16; i_1 += 4) 
        {
            {
                var_16 *= ((/* implicit */short) ((((/* implicit */_Bool) arr_0 [i_0])) || ((!(((/* implicit */_Bool) arr_3 [i_0] [i_0]))))));
                var_17 = ((/* implicit */int) max((var_17), (((/* implicit */int) arr_0 [i_0]))));
                var_18 &= ((/* implicit */short) ((long long int) min((arr_0 [i_1]), (((/* implicit */long long int) arr_2 [(signed char)12])))));
            }
        } 
    } 
}
