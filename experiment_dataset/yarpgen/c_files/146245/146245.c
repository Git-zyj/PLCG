/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146245
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146245 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146245
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        var_16 |= ((-(arr_0 [i_0] [i_0])));
        var_17 = -908139690;
    }
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        var_18 = (max(var_18, ((((((arr_3 [6] [6]) >= (((arr_1 [i_1]) ? -1806331443 : (arr_0 [i_1] [i_1]))))) ? 0 : (((((1 / (arr_2 [i_1])))) ? (arr_3 [1] [1]) : (((arr_3 [4] [4]) ? var_4 : (arr_2 [i_1]))))))))));
        var_19 *= ((((max(((var_0 ? (arr_1 [i_1]) : var_1)), (arr_2 [i_1])))) && (arr_3 [0] [0])));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 22;i_2 += 1)
    {
        var_20 = (arr_1 [i_2]);
        var_21 *= ((var_12 < (arr_4 [i_2] [i_2])));
    }
    var_22 = (min(var_22, var_14));
    #pragma endscop
}
