/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166607
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned short i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        /* LoopNest 2 */
        for (short i_1 = 0; i_1 < 23; i_1 += 3) 
        {
            for (unsigned int i_2 = 1; i_2 < 22; i_2 += 3) 
            {
                {
                    var_14 = ((/* implicit */unsigned char) arr_3 [i_1]);
                    var_15 = (+(arr_5 [i_2 + 1] [i_2 - 1]));
                    var_16 = ((/* implicit */unsigned char) arr_0 [i_2 - 1]);
                }
            } 
        } 
        var_17 = ((/* implicit */long long int) var_11);
        /* LoopSeq 1 */
        for (int i_3 = 0; i_3 < 23; i_3 += 2) 
        {
            /* LoopNest 2 */
            for (long long int i_4 = 0; i_4 < 23; i_4 += 2) 
            {
                for (unsigned int i_5 = 0; i_5 < 23; i_5 += 3) 
                {
                    {
                        arr_15 [i_0] [i_3] [i_4] [i_4] = ((/* implicit */short) arr_4 [i_5] [i_0] [i_0] [i_0]);
                        arr_16 [i_0] [i_3] = ((/* implicit */long long int) ((((/* implicit */int) (!(((/* implicit */_Bool) arr_12 [i_3]))))) >= (((/* implicit */int) arr_7 [i_3]))));
                    }
                } 
            } 
            var_18 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) arr_1 [i_0]))));
        }
        var_19 = ((/* implicit */long long int) ((((/* implicit */_Bool) (-(((/* implicit */int) (_Bool)1))))) ? (((/* implicit */unsigned long long int) ((/* implicit */int) var_7))) : (((((/* implicit */_Bool) arr_0 [i_0])) ? (arr_0 [i_0]) : (((/* implicit */unsigned long long int) ((/* implicit */int) arr_14 [14LL] [i_0] [i_0] [19ULL])))))));
    }
    var_20 = ((/* implicit */unsigned char) var_13);
    var_21 &= ((/* implicit */unsigned long long int) var_9);
    var_22 = ((/* implicit */unsigned long long int) var_4);
}
