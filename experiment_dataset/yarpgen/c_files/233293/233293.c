/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233293
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233293 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233293
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(((min(0, (!41)))), (((var_14 ? var_5 : var_12)))));
    var_17 &= (((var_15 ? (!var_4) : var_14)));
    var_18 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = (min((((var_5 >> (((arr_0 [i_0]) - 138))))), (max((var_11 & var_10), ((var_9 ? (arr_3 [i_0] [i_1] [i_0]) : var_8))))));
                var_20 += (((0 ? 1 : (arr_4 [i_0] [i_1] [i_1]))));
            }
        }
    }
    #pragma endscop
}
