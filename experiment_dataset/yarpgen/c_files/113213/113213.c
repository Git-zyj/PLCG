/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113213
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113213 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113213
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_2;
    var_14 = (~var_4);
    var_15 = (((max(((max(1, var_3))), ((var_0 ? var_11 : var_0))))) / ((((var_2 ? var_10 : var_2)) & var_11)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_16 = (min(var_16, ((((((max(1, 1)))) ? (min((min(var_7, var_12)), ((max((arr_4 [21] [23]), 1))))) : ((((((-1 ? (arr_0 [0] [i_1]) : -1))) ? (((var_5 ? var_7 : var_4))) : (max((arr_1 [6] [i_1]), var_11))))))))));
                var_17 = (min(var_17, ((min(1, 1)))));
            }
        }
    }
    #pragma endscop
}
