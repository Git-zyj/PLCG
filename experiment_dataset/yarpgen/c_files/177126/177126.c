/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177126
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177126 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177126
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -398105353;
    var_11 = ((-(min(1, ((var_1 ? (-32767 - 1) : var_8))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_1] [i_2] [i_0] = ((-((1 ? (arr_4 [i_0] [i_1] [i_2]) : (arr_4 [i_0] [i_0] [i_0])))));
                    arr_9 [i_1] [i_0] = ((7959549208325159698 ? ((((min(3896861943, (arr_5 [i_0] [i_1] [i_2])))) ? -var_2 : ((min(var_2, var_5))))) : 27));
                    var_12 = (max(1, var_5));
                    arr_10 [i_1] = ((!(arr_0 [i_1] [i_0])));
                }
            }
        }
    }
    #pragma endscop
}
