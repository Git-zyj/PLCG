/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20458
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20458 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20458
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 |= ((/* implicit */short) (unsigned char)176);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 12; i_0 += 3) 
    {
        for (long long int i_1 = 0; i_1 < 12; i_1 += 1) 
        {
            {
                var_16 -= ((/* implicit */unsigned long long int) var_10);
                /* LoopNest 2 */
                for (_Bool i_2 = 1; i_2 < 1; i_2 += 1) 
                {
                    for (long long int i_3 = 1; i_3 < 11; i_3 += 1) 
                    {
                        {
                            arr_9 [i_0] [i_0] = ((/* implicit */unsigned char) (!(((/* implicit */_Bool) var_2))));
                            var_17 = (~(arr_7 [i_1] [i_1] [i_1] [i_0]));
                            arr_10 [(signed char)4] [(unsigned char)5] [(signed char)4] [i_3] [i_3] [i_0] = max((((/* implicit */long long int) ((((/* implicit */int) arr_3 [i_0] [i_0])) == ((-(((/* implicit */int) arr_0 [i_0]))))))), (((long long int) ((signed char) 2106330082750915684LL))));
                        }
                    } 
                } 
                arr_11 [i_0] [i_0] [i_0] = ((/* implicit */short) ((signed char) (-(10052715645540443625ULL))));
            }
        } 
    } 
    var_18 = ((/* implicit */unsigned char) min((var_18), (((/* implicit */unsigned char) (-(((/* implicit */int) var_0)))))));
    var_19 = ((/* implicit */int) (short)-30010);
    var_20 = ((/* implicit */unsigned short) var_5);
}
