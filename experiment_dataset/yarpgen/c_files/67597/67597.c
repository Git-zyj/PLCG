/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67597
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67597 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67597
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        arr_2 [1] = var_11;
        arr_3 [i_0] [i_0] = var_11;
        var_20 = 79;
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_21 = (min((min(((71 ? -27252 : (arr_1 [i_1]))), -106)), ((((2783141834 << (-1 + 7)))))));
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 10;i_3 += 1)
            {
                {
                    var_22 = 66;
                    arr_12 [i_1] [8] [i_1] = (max(939524096, ((-(arr_6 [i_1])))));
                    arr_13 [9] [i_1] [i_3] = (((((max(1, -119))) & (~184))));
                    arr_14 [i_1] [i_1] [i_3] = (max(119, (((arr_11 [i_1] [i_1]) ^ 76))));
                }
            }
        }
    }
    for (int i_4 = 0; i_4 < 1;i_4 += 1)
    {
        var_23 = var_8;
        var_24 = (max(-103, ((2783141827 ? 184 : 81))));
        arr_17 [2] [2] |= 32766;
    }
    var_25 = var_17;
    var_26 = 24;
    var_27 = ((~((var_8 ? var_19 : var_1))));
    #pragma endscop
}
