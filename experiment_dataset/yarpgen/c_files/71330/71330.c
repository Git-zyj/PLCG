/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71330
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71330 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71330
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_0 ? (var_0 + 5899) : ((var_1 ? var_2 : var_11))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = (min((((!(arr_3 [i_0] [i_0])))), (((arr_0 [i_1]) - (arr_2 [i_1])))));
                arr_5 [i_1] [i_1] [i_1] = 11636;
            }
        }
    }

    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        var_13 *= ((+((var_9 - ((min((arr_3 [i_2] [9]), (arr_0 [i_2]))))))));
        var_14 = ((((((arr_0 [i_2]) ? (arr_1 [i_2]) : (arr_0 [i_2])))) ? var_9 : var_3));
    }
    #pragma endscop
}
