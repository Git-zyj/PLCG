/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67161
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67161 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67161
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 10; i_0 += 2) 
    {
        var_15 = ((/* implicit */unsigned int) var_8);
        arr_2 [i_0] = ((/* implicit */int) ((max((((/* implicit */unsigned long long int) ((_Bool) (unsigned char)14))), (arr_0 [i_0]))) < (((/* implicit */unsigned long long int) ((var_3) ? (((/* implicit */int) var_1)) : (((/* implicit */int) arr_1 [6ULL])))))));
    }
    var_16 = ((/* implicit */int) var_2);
}
