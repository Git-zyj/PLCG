/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117007
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117007 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117007
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            arr_4 [i_0] [i_1] [i_1] = (((arr_0 [i_1]) ? (arr_0 [i_0]) : (arr_0 [i_1])));
            var_19 = 10675439759114939458;
            var_20 -= (arr_1 [i_0] [i_0]);
            arr_5 [i_1] = (arr_2 [i_1] [i_0] [i_1]);
        }
        var_21 = (max(var_21, ((((arr_3 [i_0] [i_0] [i_0]) ? 1 : ((41330 ? (arr_3 [i_0] [i_0] [i_0]) : (arr_0 [i_0]))))))));
    }
    var_22 = var_15;
    #pragma endscop
}
