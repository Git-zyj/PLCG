/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204382
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204382 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204382
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    arr_10 [i_1] [i_1] &= (((arr_5 [i_1 + 1] [i_1 - 1]) && (((-11324 ? var_0 : ((max(20128, 1))))))));
                    arr_11 [i_0] [i_1] [i_0] = ((((32767 << (((arr_2 [i_1 - 2]) - 4269933887011562609)))) != ((((184 ^ 55456) >= 32785)))));
                }
                arr_12 [i_0] = (((((65 >= (arr_8 [i_1 + 1] [i_0] [i_1 + 1] [i_1 - 2])))) > -24592));
            }
        }
    }
    var_13 = 45401;
    #pragma endscop
}
