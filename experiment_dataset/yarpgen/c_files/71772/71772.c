/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71772
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71772 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71772
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [7] [2] = (max(17293822569102704640, (10420599280005998553 * 17293822569102704640)));
                    var_10 = min((((+((max((arr_1 [0] [0]), (arr_5 [i_0] [i_1] [i_2]))))))), (arr_4 [i_2] [i_0] [i_0]));
                }
            }
        }
    }
    var_11 = ((var_7 && ((max(var_2, (min(var_5, var_6)))))));
    #pragma endscop
}
