/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 55568
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=55568 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/55568
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 4; i_0 < 12; i_0 += 1) 
    {
        /* LoopNest 2 */
        for (unsigned int i_1 = 0; i_1 < 14; i_1 += 4) 
        {
            for (_Bool i_2 = 1; i_2 < 1; i_2 += 1) 
            {
                {
                    var_11 = ((/* implicit */unsigned short) (-(((/* implicit */int) arr_0 [i_0 + 1]))));
                    var_12 = (-(arr_8 [i_2 - 1] [i_2 - 1] [i_2 - 1] [i_0 - 4]));
                    /* LoopNest 2 */
                    for (long long int i_3 = 1; i_3 < 13; i_3 += 1) 
                    {
                        for (signed char i_4 = 0; i_4 < 14; i_4 += 4) 
                        {
                            {
                                var_13 += ((/* implicit */signed char) ((((/* implicit */_Bool) arr_3 [i_0])) || (((/* implicit */_Bool) ((((/* implicit */int) (signed char)-13)) - (((/* implicit */int) (short)8)))))));
                                var_14 = ((int) arr_10 [i_2 - 1] [i_2 - 1] [i_3] [i_2 - 1]);
                            }
                        } 
                    } 
                }
            } 
        } 
        var_15 = ((/* implicit */unsigned char) arr_3 [i_0 - 1]);
    }
    var_16 = ((/* implicit */unsigned long long int) max((var_10), (((/* implicit */unsigned short) var_5))));
}
