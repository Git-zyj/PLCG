/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42506
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 12; i_0 += 4) 
    {
        var_15 = ((/* implicit */unsigned char) (~(4189234694U)));
        var_16 = ((/* implicit */_Bool) (-((~((-2147483647 - 1))))));
        var_17 ^= ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) ((unsigned long long int) 8970762374610599450ULL))) ? (((/* implicit */int) arr_3 [i_0] [i_0])) : (((/* implicit */int) (_Bool)1))));
        arr_4 [i_0] = ((/* implicit */_Bool) (-((+(((/* implicit */int) (signed char)0))))));
        arr_5 [i_0] = (((~(var_9))) <= (126100789566373888ULL));
    }
    var_18 = ((/* implicit */unsigned short) var_8);
}
