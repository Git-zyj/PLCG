/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244461
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244461 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244461
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_1] [i_0] = (((arr_1 [i_0 - 1] [i_0 - 1]) != (arr_1 [i_0 + 3] [i_0 + 4])));
                    var_10 = ((!(((-(arr_2 [i_2] [i_1]))))));
                }
            }
        }
    }
    var_11 = (min((((var_0 ? var_7 : 0))), (((((72057594037911552 >> (var_3 + 107)))) ? (var_1 * var_4) : (min(var_0, var_6))))));
    var_12 = var_3;
    var_13 *= var_6;
    #pragma endscop
}
