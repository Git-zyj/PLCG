/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((-((max(var_9, 65515)))));

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_13 = (((((var_9 ? var_9 : var_7)) << (((~0) + 49)))));
        var_14 = (--1);
        arr_3 [i_0] [i_0] = (((98 % 1) - ((((arr_2 [i_0]) || var_11)))));
        arr_4 [i_0] = (arr_0 [i_0]);
    }
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        var_15 = (min(var_15, 1));
        arr_8 [i_1] [21] = (((min(((1 ? var_8 : var_9)), (max(127, var_11))))) * (((4092 << 0) ? (arr_7 [i_1] [i_1]) : 1023)));
        var_16 = arr_6 [11] [11];
    }
    #pragma endscop
}
