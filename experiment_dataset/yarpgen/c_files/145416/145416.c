/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145416
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145416 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145416
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((((1 ? var_12 : 18446744073709551615)) ? var_7 : var_12))));
    var_16 = (min(var_16, -43396));
    var_17 += ((max(0, 36831)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_18 ^= 58362;

                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    var_19 = (arr_1 [i_0 - 1] [i_0 - 1]);
                    arr_9 [i_2] = ((((((((-127 - 1) || 1)) ? (max(43396, -8267606342805214107)) : -30081))) && ((max(2145386496, (!-23068))))));
                    arr_10 [i_1] = ((~(((!(arr_4 [i_0 - 1]))))));
                }
                var_20 = (min(var_20, (((((arr_3 [i_0 - 1]) ? (arr_3 [i_0 + 3]) : (arr_3 [i_0 - 1])))))));
                var_21 = ((~((-(arr_3 [i_0 - 1])))));
            }
        }
    }
    var_22 = (546415727 + 1);
    #pragma endscop
}
