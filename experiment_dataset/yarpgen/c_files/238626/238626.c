/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238626
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238626 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238626
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_11 = (arr_1 [i_0] [11]);
                var_12 = (arr_3 [8] [8] [i_1]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 4; i_3 < 9;i_3 += 1)
                    {
                        {
                            var_13 ^= ((max((arr_5 [i_1 + 2] [i_3 + 2] [i_1 + 2] [i_3 - 4]), (arr_7 [i_0]))));
                            var_14 -= (((((1709733393 ? 3405462156 : var_6) % -var_2))));
                            arr_11 [i_0] [i_0] = (arr_6 [i_3 + 3] [i_1 - 1] [i_2] [i_3]);
                        }
                    }
                }
            }
        }
    }
    var_15 = var_6;
    var_16 -= ((11208318329962395077 << (var_9 + 671247782)));
    #pragma endscop
}
