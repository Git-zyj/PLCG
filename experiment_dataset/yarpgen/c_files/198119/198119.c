/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198119
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198119 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198119
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (85 == -27811);

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_15 = 1655;
        var_16 = (max(var_16, ((max((arr_1 [i_0]), (arr_1 [i_0]))))));
    }
    for (int i_1 = 0; i_1 < 0;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 15;i_3 += 1)
            {
                {
                    arr_13 [i_1] [i_1] [i_1 + 1] = (((((arr_1 [i_3]) || (arr_4 [i_1]))) && (((arr_10 [i_1] [i_1] [i_3]) && (arr_10 [11] [i_2] [i_1 + 1])))));
                    var_17 = (min(0, ((-((min(65535, var_5)))))));
                }
            }
        }
        var_18 = (max(var_18, (((~((((arr_1 [i_1 + 1]) == (arr_1 [i_1 + 1])))))))));
        var_19 += (~-670342887);
        var_20 = ((-((min(var_1, var_12)))));
    }
    var_21 -= -28001;
    #pragma endscop
}
