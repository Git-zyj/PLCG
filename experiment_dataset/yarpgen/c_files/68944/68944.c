/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_18 = (((((-((var_10 ? var_1 : 1))))) && (var_2 && 1)));
                                var_19 = var_6;
                                var_20 -= var_4;
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 18;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 18;i_6 += 1)
                    {
                        {
                            arr_19 [i_0] [i_1] [i_5] [i_1] = ((-(((!(((0 ? 0 : 13534187366887209165))))))));
                            arr_20 [i_0] [i_1] [i_1] [i_6] = ((6454583174231813704 / ((-53180 ^ (4912556706822342465 + 2)))));
                        }
                    }
                }
            }
        }
    }
    var_21 = (var_9 && ((((7625943607429913325 ? 1 : 5024)) >= (1 * 0))));
    #pragma endscop
}
