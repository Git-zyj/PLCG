/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10077
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10077 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10077
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (1677678110 & 141002176);
    var_12 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 8;i_1 += 1)
        {
            {
                var_13 = ((!((min((arr_0 [i_0]), (arr_0 [i_1 + 1]))))));

                /* vectorizable */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    arr_7 [i_2] [i_1 - 2] [i_2] = (arr_4 [i_1 + 1] [i_1 + 1] [i_2 + 1]);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_12 [i_2] [i_2 + 1] [i_2 + 1] [i_2 + 1] = 0;
                                arr_13 [i_3] [i_2] = ((16 ? (!var_5) : (arr_9 [i_0] [i_0] [i_2 + 1] [i_2] [i_4])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
