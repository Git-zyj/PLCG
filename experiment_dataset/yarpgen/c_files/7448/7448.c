/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7448
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7448 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7448
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_12;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 14;i_2 += 1)
            {
                {
                    var_18 = (((arr_3 [i_2 + 4] [i_1]) != var_0));
                    arr_9 [i_2] = ((((((~var_9) * 24))) | (min((min(163396292, 49)), (((~(arr_4 [i_0] [i_1 + 1] [9]))))))));
                    arr_10 [i_0] [i_2] [i_2] = (32767 == 13336498517616658560);
                    arr_11 [i_2] [9] = var_12;
                }
            }
        }
    }
    var_19 = (((6 < -1) >> (var_7 - 155)));
    #pragma endscop
}
