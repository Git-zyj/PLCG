/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131370
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131370 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131370
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 16966702793175295299;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 23;i_2 += 1)
            {
                {
                    var_17 = var_10;
                    var_18 = (max(var_18, (((((((((var_3 ? var_15 : var_4))) ? ((var_12 ? var_12 : var_13)) : var_15))) || var_8)))));
                    var_19 *= ((((((arr_5 [1] [i_1]) >= ((-1049022784 ? 16966702793175295299 : 1879680348))))) < (-13 == 2016)));
                    var_20 = (arr_1 [i_2] [i_0]);
                    var_21 = 0;
                }
            }
        }
    }
    #pragma endscop
}
