/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35121
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35121 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35121
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_0] [i_1] [i_0] |= (max(((~(arr_4 [i_0] [i_2]))), 1763630436));
                    var_17 *= (max(var_0, ((((arr_2 [i_1] [i_2]) ? 9586269736874735457 : 47)))));
                }
            }
        }
    }
    var_18 = ((var_1 ? ((((min(var_15, var_16))) ? var_9 : 933562814068844807)) : var_0));
    #pragma endscop
}
