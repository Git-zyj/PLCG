/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43604
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43604 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43604
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    arr_7 [i_1] [6] [i_2 - 2] [11] = min(var_2, ((((arr_0 [i_1] [i_1]) ? var_5 : (arr_3 [7] [i_1])))));
                    arr_8 [i_0] = var_18;
                    var_19 = (min(var_19, ((((~195) ? -14345 : (((arr_3 [i_2 - 2] [11]) ? (arr_3 [i_2 + 1] [4]) : (arr_3 [i_2 - 2] [1]))))))));
                }
            }
        }
    }
    var_20 = var_1;
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 17;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            {
                var_21 = ((!((max((arr_14 [i_4] [6] [i_3]), 1)))));
                arr_15 [i_3] = ((var_5 ? (arr_13 [i_3] [i_4]) : (arr_11 [3] [1])));
                arr_16 [8] = ((~(~-14123)));
                arr_17 [i_3] [i_4] = var_7;
            }
        }
    }
    #pragma endscop
}
