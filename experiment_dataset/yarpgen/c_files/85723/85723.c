/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85723
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85723 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85723
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 ^= var_6;
    var_16 = ((-((max(11287277822761770280, -92769276)))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_1] = ((-964738388 ? ((((!(arr_2 [i_0] [i_0 + 1]))))) : -92769276));
                var_17 = (max((arr_1 [i_0 - 2]), (min((arr_1 [i_0 - 1]), 13456245781115134607))));
            }
        }
    }
    var_18 = (min(var_14, (((!((max(11287277822761770280, var_6))))))));
    #pragma endscop
}
