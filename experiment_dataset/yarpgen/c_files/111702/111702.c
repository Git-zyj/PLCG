/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111702
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111702 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111702
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_11 = (max(var_11, (((-((-(~-3173452123629101508))))))));
                var_12 = var_9;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_13 = (min(var_13, (((-351932026 ? 2147483634 : 96)))));
                            var_14 = (min(var_14, (((((((18358 ? 70 : -113)) << (((~var_9) - 7064279828003505125)))) ^ var_10)))));
                            var_15 -= (!1);
                        }
                    }
                }
                var_16 = (max(var_16, ((min((min(240, 95)), -2147483634)))));
                var_17 = (max(var_17, var_1));
            }
        }
    }
    var_18 -= ((var_3 ? (((var_9 ? -1 : var_0))) : 962819458));
    #pragma endscop
}
