/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93558
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93558 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93558
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((~(max((~130), (max(254, var_7))))));
    var_11 *= max((max(((var_1 ? var_0 : 4969)), 106)), 2147483647);

    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        var_12 &= ((+(((arr_1 [i_0 + 1] [i_0]) ? (arr_0 [4]) : (((-127 - 1) ? 12 : 243))))));
        arr_2 [i_0] = ((((((arr_1 [i_0 + 1] [i_0 + 1]) ? -38 : (arr_1 [i_0 + 1] [i_0 + 1])))) ? (arr_1 [i_0 + 1] [i_0 + 1]) : ((max((arr_0 [i_0]), (arr_1 [i_0 + 1] [i_0 + 1]))))));
    }
    var_13 = var_4;
    #pragma endscop
}
