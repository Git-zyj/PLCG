/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24820
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24820 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24820
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        var_12 = 4989920507767787824;
        arr_3 [i_0] = (!5099200002348497196);
        arr_4 [i_0 - 1] = var_10;
    }
    var_13 = (min(var_13, var_4));
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 18;i_2 += 1)
        {
            {
                var_14 -= (9416358682379346090 % 1795545240);
                arr_9 [i_2] [i_1] = ((((((arr_5 [i_2]) / (arr_5 [i_2])))) || (arr_5 [i_1])));
                arr_10 [i_1] [i_2] = ((-((((arr_6 [i_2]) || (arr_6 [i_1]))))));
                arr_11 [i_2] = (((min((arr_0 [i_1]), (arr_0 [i_1]))) % (arr_0 [i_1])));
                var_15 |= (arr_2 [i_2] [i_2]);
            }
        }
    }
    #pragma endscop
}
