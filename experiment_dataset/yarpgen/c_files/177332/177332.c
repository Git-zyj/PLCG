/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 *= var_16;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_19 = (max(var_19, (((~(((var_7 < (arr_4 [i_0] [i_1] [i_2])))))))));
                            var_20 = arr_5 [i_0] [i_1] [i_2];
                            var_21 = (var_5 | 33554431);
                        }
                    }
                }
                var_22 = (min((((12769 / (arr_3 [i_0] [i_0] [i_0])))), (min((arr_4 [i_0] [i_1] [i_0]), (arr_4 [i_1] [i_1] [i_0])))));
                var_23 ^= (+((10226679960893109137 ? (!2124740709147307484) : 15)));
            }
        }
    }
    #pragma endscop
}
