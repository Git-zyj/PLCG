/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100125
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100125 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100125
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] = (min((min((min((arr_2 [6]), (arr_5 [i_0]))), (arr_4 [i_1] [i_1]))), var_7));
                var_20 = (min((min(((((arr_3 [9] [i_1]) > 16102088641992307351))), (((arr_0 [1] [1]) & (arr_1 [i_0] [i_0]))))), ((((arr_5 [i_1]) ? (arr_4 [i_0] [i_1]) : (arr_4 [i_0] [i_1]))))));
            }
        }
    }
    var_21 *= ((var_15 ? ((((max(1, var_5))))) : var_1));
    #pragma endscop
}
