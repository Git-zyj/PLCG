/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182511
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182511 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182511
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */int) 33554432U);
    var_11 ^= (+(((/* implicit */int) var_6)));
    var_12 ^= ((/* implicit */signed char) ((((/* implicit */_Bool) -1)) ? ((-(((/* implicit */int) (unsigned short)46827)))) : ((+(((/* implicit */int) (!(((/* implicit */_Bool) var_7)))))))));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 16; i_0 += 4) 
    {
        for (int i_1 = 0; i_1 < 17; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 17; i_2 += 3) 
            {
                {
                    var_13 = 900787208;
                    arr_8 [i_0] [i_0] [i_1] [i_2] = (+(((/* implicit */int) ((min((arr_1 [10U] [13U]), (((/* implicit */unsigned long long int) (unsigned short)16383)))) >= (arr_1 [i_0] [i_0 - 2])))));
                }
            } 
        } 
    } 
}
