/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43703
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43703 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43703
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_8;
    var_15 = (min(var_15, (((!(var_12 > var_10))))));
    var_16 = (((((90961683 % (((-5320926080990050049 ? 1816848652 : var_2)))))) ? ((((!((min(var_0, var_6))))))) : (((~var_0) ? 2015369375 : (max(-5320926080990050049, 8810110824899985432))))));
    var_17 = (min(var_17, var_13));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_1] = var_12;
                var_18 = (min(var_18, var_8));
                var_19 = -var_10;
                arr_5 [i_1 - 1] = (max(((-(max(var_13, var_7)))), (max((arr_1 [i_0 - 2]), (arr_1 [i_0 + 2])))));
                var_20 ^= (max((max((-8810110824899985432 * var_5), 0)), ((((arr_3 [i_0 - 2] [i_1 + 1]) ? (arr_3 [i_0 - 2] [i_1 + 1]) : (arr_3 [i_0 - 2] [i_1 + 1]))))));
            }
        }
    }
    #pragma endscop
}
