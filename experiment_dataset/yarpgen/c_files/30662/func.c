/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30662
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30662 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30662
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */short) ((((/* implicit */int) (short)248)) != (((/* implicit */int) var_18))));
    var_21 = (~(((/* implicit */int) var_7)));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 21; i_1 += 1) 
        {
            {
                arr_5 [i_1] = ((/* implicit */short) (~(max((((/* implicit */unsigned long long int) var_3)), (arr_1 [i_0])))));
                arr_6 [i_1] [i_1] = ((/* implicit */short) (~(min((22U), (3002796734U)))));
            }
        } 
    } 
    var_22 = ((/* implicit */short) (~(var_19)));
    var_23 = ((/* implicit */unsigned int) var_13);
}
