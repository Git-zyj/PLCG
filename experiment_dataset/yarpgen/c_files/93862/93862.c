/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93862
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93862 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93862
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 &= var_11;
    var_18 = var_15;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] = ((-(((arr_5 [i_0] [i_1]) % (((43247 ? -32751 : 4982)))))));
                arr_7 [11] [5] [i_1] = ((((((arr_4 [i_0] [i_0] [i_1]) >> (((arr_4 [i_0] [i_1] [i_1]) - 363936557)))) < (min((-32767 - 1), 446906642)))));
                var_19 = (-(min((arr_3 [i_0]), (arr_4 [i_0] [i_0] [i_0]))));
            }
        }
    }
    #pragma endscop
}
