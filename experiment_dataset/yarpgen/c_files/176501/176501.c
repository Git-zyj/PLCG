/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176501
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176501 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176501
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                var_20 = (max(var_20, (((((min((arr_6 [i_1 + 2] [i_1 + 2]), (arr_7 [i_0])))) && var_7)))));
                arr_8 [i_1] = ((((max((arr_2 [i_0] [i_1]), 11138888885450528845)) * (((var_5 ^ (arr_7 [i_1])))))));
                var_21 = var_5;
            }
        }
    }
    var_22 = ((((((var_11 / var_14) | (((14369010098716260100 ? -1 : 0)))))) ? var_9 : var_5));
    #pragma endscop
}
