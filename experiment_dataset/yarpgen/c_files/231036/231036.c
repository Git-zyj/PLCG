/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231036
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231036 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231036
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 ^= -var_9;
    var_11 = (((((max(15369, var_1)))) >> var_5));
    var_12 = var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_2] [i_0] = (~448);
                    var_13 = (+(((arr_4 [i_0] [i_1] [i_2]) ? 15365 : (arr_1 [i_1]))));
                    var_14 = (max(var_14, ((((arr_7 [i_2] [8] [7]) <= (max(15369, var_4)))))));
                }
            }
        }
    }
    var_15 = (max(var_15, ((max((((!((max(var_8, var_5)))))), var_5)))));
    #pragma endscop
}
