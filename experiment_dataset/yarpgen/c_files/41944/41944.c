/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, var_1));

    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        arr_3 [i_0 - 2] [i_0] = ((~(arr_2 [i_0])));
        arr_4 [i_0] = (min(((((((arr_2 [i_0]) << (var_3 - 17698192659733578674)))) && (arr_0 [1]))), (((((var_9 ? (arr_1 [i_0] [i_0]) : var_13))) > (arr_2 [i_0])))));
    }
    var_19 &= (((((var_13 ? var_15 : (((var_13 << (var_11 - 101))))))) ? (var_0 >= var_17) : var_9));
    #pragma endscop
}
