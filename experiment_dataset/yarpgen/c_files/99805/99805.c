/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99805
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99805 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99805
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= ((!((((max(var_7, var_0)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_12 = (!4234);
                arr_4 [i_1] [i_0] [i_0] = (min((((65514 ? 32089 : 90))), (max((!var_3), var_10))));
                arr_5 [i_1] [i_1] [i_0] = (max(((max(((0 ? var_8 : var_5)), ((max(var_3, 32085)))))), ((-((var_10 ? 136 : 0))))));
            }
        }
    }
    var_13 = ((-(max(32767, var_0))));
    var_14 = (max(((max(var_4, (~-32088)))), (~1)));
    #pragma endscop
}
