/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113212
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113212 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113212
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] [i_0] = ((-524287 * (arr_1 [i_0])));
        var_12 += (arr_1 [i_0]);
        arr_4 [i_0] [1] = 5862313261529385432;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        var_13 = (max(var_13, (((((!(arr_0 [4]))) ? ((524287 - (arr_7 [1] [i_1]))) : (arr_1 [i_1]))))));
        arr_9 [i_1] [i_1] = (arr_5 [i_1]);
    }
    var_14 = (min(var_1, ((((4294443008 ^ 877873828) < (!var_2))))));
    var_15 = (var_9 / var_1);
    #pragma endscop
}
