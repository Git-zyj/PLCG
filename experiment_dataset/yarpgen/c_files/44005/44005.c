/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44005
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (~var_0);
    var_20 *= var_18;
    var_21 = (min(var_21, var_17));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((-(((arr_1 [1]) ? (arr_0 [i_0]) : (((((arr_1 [i_0]) + 2147483647)) << (((((arr_2 [i_0] [i_0] [i_0]) + 3406073788021778962)) - 23))))))));

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    var_22 = arr_1 [i_1];
                    arr_7 [10] [i_1] = ((~(((arr_3 [i_0]) ? (arr_6 [i_0]) : (arr_1 [i_1])))));
                    var_23 = ((((((((arr_5 [0] [i_0] [i_0]) * (arr_3 [7]))) - (arr_3 [i_1])))) ? (((arr_1 [i_0]) ? (arr_2 [i_0] [i_1] [i_2]) : ((((!(arr_0 [i_2]))))))) : (((~(arr_0 [i_0]))))));
                }
            }
        }
    }
    var_24 = (max(var_24, var_7));
    #pragma endscop
}
