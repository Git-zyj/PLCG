/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128993
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128993 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128993
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 0;i_4 += 1)
                        {
                            {
                                var_15 = arr_9 [i_4] [i_3] [i_0];
                                arr_14 [i_0] [i_0] [i_0] [i_2] [i_3] [i_3] [i_4 + 1] = (arr_0 [i_0]);
                                var_16 = arr_3 [1] [i_1] [13];
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 0; i_5 < 1;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 1;i_6 += 1)
                    {
                        {

                            for (int i_7 = 3; i_7 < 24;i_7 += 1) /* same iter space */
                            {
                                arr_25 [i_0] [i_1] [1] [1] [i_0] = (arr_8 [i_0] [11] [i_0] [1]);
                                var_17 *= arr_4 [i_1] [i_1];
                            }
                            for (int i_8 = 3; i_8 < 24;i_8 += 1) /* same iter space */
                            {
                                var_18 &= arr_22 [i_0] [i_1] [1] [i_5] [i_5] [i_8 - 3];
                                var_19 = arr_19 [i_0] [1] [i_5] [1] [i_0];
                                var_20 = (min(var_20, (arr_0 [i_0])));
                            }
                            var_21 ^= arr_23 [1] [i_1] [i_0] [1] [i_5] [i_0];
                        }
                    }
                }
            }
        }
    }
    var_22 = var_7;
    #pragma endscop
}
