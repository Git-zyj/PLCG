/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49308
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49308 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49308
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            {
                var_15 = (min(var_15, ((((((max(var_8, (arr_4 [i_0 - 1]))))) ? 2476525841 : (arr_4 [i_0]))))));
                var_16 = var_11;
                var_17 = var_6;
            }
        }
    }

    for (int i_2 = 0; i_2 < 12;i_2 += 1)
    {
        arr_9 [i_2] = (((((arr_5 [i_2] [i_2]) || ((max((arr_4 [i_2]), (arr_1 [i_2])))))) ? (arr_5 [i_2] [i_2]) : var_0));
        var_18 = (min(var_18, (max((((-(arr_4 [i_2])))), (max((arr_4 [i_2]), (arr_8 [i_2])))))));
    }
    #pragma endscop
}
