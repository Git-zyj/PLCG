/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [i_0]);
        var_14 = 0;
        var_15 = var_8;

        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            arr_7 [i_0] [17] [i_1] = (((arr_0 [i_1] [i_0]) ? (arr_0 [18] [i_0]) : var_12));
            var_16 = (arr_3 [13] [i_1] [i_1]);
            /* LoopNest 3 */
            for (int i_2 = 2; i_2 < 24;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 25;i_3 += 1)
                {
                    for (int i_4 = 2; i_4 < 24;i_4 += 1)
                    {
                        {
                            arr_14 [i_0] [i_3] [i_3] [1] = var_8;
                            arr_15 [i_0] [i_1] [i_2 - 1] [i_3] [i_3] [i_3] = ((((1 > (arr_5 [i_3] [i_2] [i_0]))) ? (var_6 != var_5) : (arr_9 [i_0] [i_2] [i_4])));
                        }
                    }
                }
            }
        }
    }
    var_17 = var_2;
    #pragma endscop
}
