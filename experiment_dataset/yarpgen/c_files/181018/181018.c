/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((((var_10 << (var_4 + 618261047)))) ? var_0 : var_5))) ? (~var_8) : ((~(~var_9))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_12 = ((((((arr_2 [i_0 + 1]) ? (arr_2 [i_0 + 1]) : (arr_2 [i_0 + 1])))) || var_9));
                var_13 &= ((((!(!var_8))) ? (((var_4 || 46721) ? (127 || 127) : (((-127 - 1) ? var_9 : var_1)))) : (((((2425499795 && (arr_0 [2] [i_0]))) || var_3)))));
            }
        }
    }
    #pragma endscop
}
