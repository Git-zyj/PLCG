/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102968
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102968 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102968
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (((((var_7 >> 0) * var_16)) != (var_3 / 18446744073709551604)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_20 = ((((arr_4 [i_1]) < (arr_4 [i_0]))));
                    var_21 = (arr_5 [i_0] [9] [i_2]);
                }
            }
        }
    }
    var_22 -= ((((var_5 ? (max(18446744073709551615, var_4)) : -1)) <= var_16));
    var_23 = (((1 || 1) ? (var_4 || var_17) : var_6));
    #pragma endscop
}
