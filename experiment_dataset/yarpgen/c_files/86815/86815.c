/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86815
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_13 ^= (min((((var_0 >= (arr_0 [i_0] [i_0])))), (((arr_3 [i_0]) ? ((min(1, var_4))) : ((min((arr_3 [i_0]), var_9)))))));
                var_14 = ((-6811170692545548601 ? (max((arr_4 [i_0] [i_0]), (((arr_0 [i_0] [i_0]) ? var_7 : -10705)))) : (arr_3 [i_1])));
            }
        }
    }
    var_15 = (min(var_15, var_1));
    var_16 = -123;
    var_17 = var_12;
    #pragma endscop
}
