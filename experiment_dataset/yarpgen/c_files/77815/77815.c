/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77815
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_14 = (((((((!(arr_6 [i_0] [i_2] [i_0] [i_0])))))) ? (arr_2 [i_1] [i_0] [i_2]) : (!-55679034)));
                    arr_8 [i_2] [i_0] [i_2] [i_2] = ((min(((-1331919902 ? 622797585 : -1307228296162671668)), (~var_13))));
                    var_15 = ((~(max(-110, 1295265227))));
                    var_16 = var_9;
                }
            }
        }
    }
    var_17 = ((((((((var_13 ? var_9 : (-2147483647 - 1)))) - 1295265227))) ? var_12 : var_9));
    #pragma endscop
}
