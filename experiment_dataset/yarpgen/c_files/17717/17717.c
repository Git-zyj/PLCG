/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 17717
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=17717 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/17717
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                var_11 = (((arr_2 [i_0 + 2]) ? (max((arr_2 [i_0 + 3]), (arr_0 [i_1 + 3]))) : (arr_0 [i_1 - 1])));
                var_12 = ((((((arr_5 [i_0] [i_0 + 1] [i_1 + 3]) ? (arr_5 [10] [i_0 + 2] [i_1 + 1]) : var_1))) ? (arr_4 [i_0 - 1]) : (((arr_0 [i_0 + 3]) ? (arr_4 [i_0 + 1]) : (arr_1 [i_0 + 3])))));
            }
        }
    }
    var_13 = (max(var_0, (8830761371187492281 == 8830761371187492305)));
    #pragma endscop
}
