/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193788
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193788 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193788
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */_Bool) min((max((((/* implicit */long long int) max((var_5), (var_16)))), (((((/* implicit */_Bool) var_14)) ? (var_17) : (((/* implicit */long long int) ((/* implicit */int) var_16))))))), (((/* implicit */long long int) ((var_13) ? (((/* implicit */int) var_6)) : ((-(((/* implicit */int) var_7)))))))));
    var_19 = ((/* implicit */unsigned char) var_6);
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            for (unsigned char i_2 = 0; i_2 < 25; i_2 += 2) 
            {
                {
                    var_20 = ((/* implicit */long long int) max((var_20), (((/* implicit */long long int) var_16))));
                    arr_11 [i_0] = ((/* implicit */unsigned short) max((((((/* implicit */_Bool) var_17)) ? (var_8) : (var_12))), (((((/* implicit */_Bool) var_4)) ? (var_15) : (((/* implicit */unsigned int) ((/* implicit */int) var_6)))))));
                }
            } 
        } 
    } 
}
