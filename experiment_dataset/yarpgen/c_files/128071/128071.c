/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128071
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128071 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128071
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -var_9;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_11 = (max((min((((arr_3 [3] [3]) ? (arr_0 [i_1]) : (arr_2 [i_0]))), (((arr_4 [i_0] [i_0]) ? var_7 : (arr_4 [5] [i_0]))))), ((min((arr_5 [i_0] [i_1]), (arr_0 [i_0]))))));
                var_12 -= ((((min(62921, (arr_3 [i_0 + 1] [i_0 + 1])))) + ((max(var_8, ((62537 + (arr_1 [8] [13]))))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 3; i_2 < 22;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 23;i_3 += 1)
        {
            {
                var_13 = (min(var_13, (arr_9 [i_2])));
                arr_11 [i_2] = var_2;
                /* LoopNest 2 */
                for (int i_4 = 3; i_4 < 22;i_4 += 1)
                {
                    for (int i_5 = 2; i_5 < 21;i_5 += 1)
                    {
                        {
                            var_14 = (max(var_14, ((((arr_8 [i_2 - 3] [i_3] [i_4 + 1]) & var_8)))));
                            var_15 = (arr_14 [i_4] [i_4]);
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
