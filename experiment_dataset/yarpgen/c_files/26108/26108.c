/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26108
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26108 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26108
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += (((((((var_9 ? var_2 : var_5))) & ((var_0 ? var_13 : var_14)))) ^ var_12));
    var_20 *= ((((min((max(var_1, var_17)), var_4))) ? ((~(var_9 ^ var_17))) : var_2));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_21 ^= arr_4 [i_0] [i_1];
                arr_5 [i_0] [i_1] = ((-(((min((arr_3 [i_1] [i_1] [i_1]), (arr_1 [i_0]))) & ((min((arr_2 [i_0]), (arr_3 [i_1] [i_0] [i_0]))))))));
            }
        }
    }
    #pragma endscop
}
