/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249830
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249830 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249830
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((min(268369920, 51859)) - var_8));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_12 = (((((-19149 ? (arr_3 [i_0]) : 1561281161))) ? ((-(arr_3 [i_0]))) : 18640));
        arr_4 [5] = (arr_2 [i_0] [i_0]);
        var_13 = (min(var_13, ((((((min(47, -18641))) ? (((51915 ? (arr_1 [i_0]) : (arr_3 [i_0])))) : (max(-39, var_1))))))));
    }
    for (int i_1 = 0; i_1 < 20;i_1 += 1)
    {
        var_14 = ((-(arr_5 [i_1] [i_1])));
        var_15 ^= (var_3 ^ ((max(-18629, 18640))));
    }
    var_16 = ((((~1) >> (var_0 - 89))));
    #pragma endscop
}
