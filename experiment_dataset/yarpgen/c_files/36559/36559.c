/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36559
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36559 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36559
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = 576179277326712832;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_11 [i_0] = (((9223372036854775798 ^ 0) ? (min((~var_2), 111)) : 88));
                    arr_12 [i_2] [i_1] [i_1] [i_0] = (((((((var_5 ? (arr_10 [i_2] [i_2] [i_1 + 3] [i_0]) : 1))) ? ((1 ? 88 : 62981)) : (1 / 62981))) << (max(1, -1))));
                }
            }
        }
    }
    var_16 = var_2;
    #pragma endscop
}
