/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230541
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230541 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230541
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 -= ((/* implicit */unsigned short) min((((/* implicit */unsigned int) var_6)), (var_16)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_19 -= ((/* implicit */unsigned short) arr_2 [i_0]);
        var_20 = ((/* implicit */unsigned short) (signed char)-82);
        var_21 = ((/* implicit */_Bool) min((var_21), (((((/* implicit */_Bool) (unsigned char)146)) && (((/* implicit */_Bool) (signed char)-64))))));
    }
}
