/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16513
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16513 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16513
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 *= var_2;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 21;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        var_12 = (min((arr_0 [i_2]), ((var_5 ? var_4 : (arr_3 [i_0 + 3] [i_0] [i_0 - 2])))));
                        arr_10 [i_0] [i_3] [i_2] [i_3] = 235;
                    }
                    var_13 = (max(var_13, 255));
                    arr_11 [i_0] [i_0] [i_2 - 1] = (max(((218 + (arr_2 [i_0 - 2]))), ((((((248 ? var_0 : (arr_3 [20] [i_1] [i_2])))) ? ((~(arr_7 [i_0] [i_1] [i_2]))) : var_7)))));
                }
            }
        }
    }
    #pragma endscop
}
