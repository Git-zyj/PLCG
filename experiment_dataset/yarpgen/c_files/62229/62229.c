/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62229
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = 5;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_14 = (((arr_6 [i_1]) ? (arr_1 [i_0] [i_2]) : (-2086359381163441598 / 1)));
                    var_15 = ((((27458 ? (arr_4 [i_1 + 3] [2]) : 30867)) * (arr_4 [i_1 + 3] [i_1 + 3])));
                    arr_8 [i_1] = (max(((27467 | (arr_4 [i_1 - 3] [i_1 - 1]))), (arr_4 [i_1 + 2] [i_1 + 1])));
                }
            }
        }
    }
    var_16 = ((var_1 ? (((!var_6) ? (min(var_9, 19241)) : (~var_0))) : (-89 >= -52)));
    #pragma endscop
}
