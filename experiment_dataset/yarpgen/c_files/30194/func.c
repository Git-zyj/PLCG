/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30194
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30194 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30194
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned char) -750253560203142231LL);
        var_11 -= ((/* implicit */unsigned long long int) min(((unsigned short)47806), (min(((unsigned short)47808), (((/* implicit */unsigned short) var_10))))));
        var_12 -= ((/* implicit */int) var_0);
    }
    var_13 = ((/* implicit */unsigned char) min((var_6), (((/* implicit */long long int) -64114364))));
}
