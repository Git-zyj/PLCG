/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12280
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_11 = ((var_0 ? ((min((((-2147483647 - 1) + (arr_4 [i_0] [i_1 - 1] [i_2]))), var_3))) : ((~(min((arr_3 [i_0] [i_0]), var_10))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_12 = (min(((((var_0 | var_6) ? -18 : var_9))), ((-var_2 & (((((arr_14 [14] [i_4] [i_2] [i_4] [i_0]) || var_8))))))));
                                var_13 = arr_0 [i_0];
                                var_14 = var_0;
                            }
                        }
                    }
                    var_15 &= ((!(36698 != 159132216242476886)));
                }
            }
        }
    }
    var_16 = -var_4;
    #pragma endscop
}
