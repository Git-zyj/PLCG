/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66681
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66681 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66681
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */int) min((var_18), ((~(var_10)))));
    var_19 = ((/* implicit */_Bool) -398519663);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        var_20 = ((/* implicit */signed char) max((var_20), (((/* implicit */signed char) arr_0 [(unsigned short)16]))));
        var_21 = ((/* implicit */int) 18446744073709551598ULL);
        var_22 = ((((/* implicit */_Bool) (~(((/* implicit */int) var_14))))) || ((_Bool)0));
    }
}
