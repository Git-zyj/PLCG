/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121654
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121654 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121654
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= 40;
    var_12 = (min(var_12, ((max(var_7, var_5)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [0] |= (max((((65535 >= (arr_3 [i_0 - 1] [i_0 - 1] [i_0 + 2])))), (((arr_3 [i_0 - 1] [i_0 + 1] [i_0 + 2]) ? 1 : (arr_3 [i_0 - 1] [i_0 + 1] [i_0 + 2])))));
                var_13 &= ((-(((max(var_7, (arr_4 [i_1])))))));
            }
        }
    }
    #pragma endscop
}
