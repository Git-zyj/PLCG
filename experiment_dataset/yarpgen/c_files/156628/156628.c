/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156628
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156628 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156628
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_12, (!31103)));
    var_13 -= (((!var_4) >= (((((var_1 ? var_6 : var_9)) < var_1)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [1] [1] |= -1;
                var_14 = (((((arr_1 [i_1]) ? (arr_1 [i_0]) : var_1))) ? ((((!(arr_2 [3]))))) : ((((max((arr_0 [i_0] [i_1]), (arr_2 [i_0])))) ? (arr_1 [i_0]) : var_1)));
                arr_6 [i_0] = (min((arr_2 [i_0]), ((((arr_2 [i_0]) < ((850 ? (arr_0 [i_1] [i_0]) : (arr_0 [i_0] [17]))))))));

                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    var_15 = (((~((var_11 ? var_6 : 875668836)))));
                    var_16 = arr_8 [i_0] [i_2];
                }
            }
        }
    }
    var_17 = (min(var_17, var_2));
    var_18 = -var_1;
    #pragma endscop
}
