/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66770
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66770 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66770
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_6 <= var_3) / (max((var_13 + var_8), (var_6 & var_0))));

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_16 = (max((((arr_0 [i_0] [i_0]) ? 112 : (arr_0 [i_0] [i_0]))), (arr_0 [i_0] [i_0])));
        var_17 = (max((((arr_1 [i_0]) + (arr_1 [i_0]))), (((~(arr_2 [i_0]))))));
    }
    for (int i_1 = 2; i_1 < 12;i_1 += 1)
    {
        var_18 = (max(var_18, (!var_2)));
        var_19 = ((var_6 <= ((-(arr_5 [i_1 + 2])))));
        arr_6 [7] = (((((110 + (((arr_4 [i_1 + 2]) + (arr_3 [i_1])))))) ? (min(var_10, (arr_5 [i_1 - 2]))) : (~-20491)));
        var_20 += (arr_5 [i_1]);
        var_21 = ((-((-(arr_4 [i_1 - 2])))));
    }
    #pragma endscop
}
