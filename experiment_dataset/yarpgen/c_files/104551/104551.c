/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_7;
    var_15 = var_0;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_16 = (arr_7 [i_1] [i_2] [i_3]);
                                var_17 += ((-(max((min((arr_5 [10]), 5963662308633588133)), (!4294967295)))));
                            }
                        }
                    }
                    arr_12 [i_0] [i_0] [i_0] [i_0] = ((((12483081765075963482 ^ (~var_9)))) ? 6393769194539590808 : (((((41726 << (-37 + 38)))) ? 35993612646875136 : (~var_10))));
                    arr_13 [i_0] [i_0] [i_2] = ((!var_3) || ((((arr_7 [i_2] [i_1] [i_0]) / 79))));
                }
            }
        }
    }
    #pragma endscop
}
