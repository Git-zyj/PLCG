/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180895
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) (short)2694);
    var_21 = ((/* implicit */long long int) (+(((/* implicit */int) (_Bool)1))));
    /* LoopSeq 2 */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 2) /* same iter space */
    {
        arr_4 [i_0] = ((/* implicit */_Bool) arr_2 [i_0]);
        var_22 = ((/* implicit */_Bool) ((((/* implicit */int) (_Bool)1)) << (((((((/* implicit */_Bool) (+(6715192892558413329LL)))) ? ((~(arr_1 [i_0] [(unsigned short)19]))) : (((/* implicit */long long int) (+(((/* implicit */int) (short)-3374))))))) + (1725580559149180390LL)))));
    }
    /* vectorizable */
    for (unsigned int i_1 = 0; i_1 < 24; i_1 += 2) /* same iter space */
    {
        arr_7 [i_1] [i_1] = ((/* implicit */_Bool) var_14);
        arr_8 [i_1] = ((/* implicit */unsigned short) (((_Bool)0) ? (((/* implicit */int) arr_5 [i_1])) : (((/* implicit */int) arr_5 [i_1]))));
        var_23 = ((/* implicit */int) min((var_23), (((/* implicit */int) arr_5 [(signed char)22]))));
    }
}
