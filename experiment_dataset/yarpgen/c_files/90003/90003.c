/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90003
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90003 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90003
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 += (!var_7);
    var_14 -= var_5;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_15 = (((-100676551 * (arr_4 [i_0]))));
                var_16 = (max(((156 ? var_1 : (max(640247756, var_1)))), -2147483647));
                var_17 += ((((100676566 + (((arr_0 [i_1]) ? var_7 : -8834005971479966315)))) << (((~var_10) - 6168963778327714354))));
            }
        }
    }
    #pragma endscop
}
