/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50636
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50636 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50636
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_10 = 6672870583235260563;
        arr_2 [i_0] = (((((-3 ? -3 : -6672870583235260554))) ? (~-3) : ((-4 ? -6672870583235260564 : 6672870583235260547))));
    }

    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_11 = 7751269417979071995;
        arr_7 [i_1] = 90;
    }
    for (int i_2 = 0; i_2 < 11;i_2 += 1)
    {
        var_12 = (~-6672870583235260538);
        arr_10 [5] = 6672870583235260547;
        var_13 = -18;
        arr_11 [7] = 3;
    }
    var_14 = (~1544637632721325271);
    #pragma endscop
}
