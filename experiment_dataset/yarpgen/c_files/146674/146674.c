/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146674
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146674 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146674
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 19;i_3 += 1)
                    {
                        {

                            for (int i_4 = 0; i_4 < 20;i_4 += 1)
                            {
                                var_13 = ((arr_3 [i_1 + 4] [i_1 + 4]) || (arr_0 [i_0]));
                                var_14 = (max(var_14, 10));
                            }
                            var_15 = (min(var_15, ((min(var_4, ((max((max(1, var_9)), (arr_11 [i_1 + 4] [i_1] [i_1 + 1] [i_1])))))))));
                        }
                    }
                }
                /* LoopNest 3 */
                for (int i_5 = 3; i_5 < 16;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 20;i_6 += 1)
                    {
                        for (int i_7 = 0; i_7 < 20;i_7 += 1)
                        {
                            {
                                var_16 = (((var_0 * (arr_17 [i_0] [i_1]))));
                                var_17 ^= var_10;
                            }
                        }
                    }
                }
                arr_22 [i_0] [i_0] [i_1] = (arr_20 [i_0] [i_0] [i_0] [i_1] [i_1] [i_1]);
            }
        }
    }
    var_18 = var_2;
    var_19 = (((var_10 && var_8) ? var_10 : (var_0 - 1)));
    #pragma endscop
}
