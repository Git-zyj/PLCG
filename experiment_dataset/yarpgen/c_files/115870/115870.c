/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115870
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115870 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115870
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] = ((((min((arr_4 [i_1 + 1]), (arr_4 [i_1 + 1])))) ? ((max((arr_4 [i_1 - 1]), (arr_4 [i_1 + 1])))) : ((387172574173744327 - (arr_4 [i_1 - 1])))));
                arr_6 [8] |= (((((~(min((arr_2 [4]), (arr_1 [i_1] [i_0])))))) ? (((arr_0 [i_1 + 1]) + (((arr_1 [i_0] [i_1]) ? -536870912 : 18446744073709551615)))) : ((((!((max(var_8, (arr_1 [i_0] [i_1]))))))))));
            }
        }
    }
    var_18 &= var_5;
    #pragma endscop
}
