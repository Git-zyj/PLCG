/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159769
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159769 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159769
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = -var_4;
        var_11 = (max((arr_0 [i_0] [i_0]), (((arr_1 [i_0]) ? var_1 : (!var_9)))));
        arr_3 [i_0] = var_6;
        arr_4 [i_0] = var_6;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {

        for (int i_2 = 0; i_2 < 24;i_2 += 1)
        {

            for (int i_3 = 2; i_3 < 22;i_3 += 1) /* same iter space */
            {
                var_12 = var_3;
                var_13 = (((arr_7 [i_2] [i_3 + 1] [i_3]) ? var_9 : var_3));
            }
            for (int i_4 = 2; i_4 < 22;i_4 += 1) /* same iter space */
            {
                arr_14 [i_1] [i_1] [1] [i_2] = (arr_8 [i_4 - 2] [i_2] [i_4 - 1]);
                var_14 |= ((var_8 ? ((var_1 ? (arr_8 [i_1] [i_1] [i_4]) : (arr_8 [i_4] [i_1] [i_4]))) : var_0));
                var_15 = (arr_12 [i_1]);
                arr_15 [i_1] [22] [8] |= (arr_8 [i_1] [i_1] [i_4 - 1]);
            }
            var_16 = ((~(arr_6 [i_1] [i_2])));
            var_17 *= ((var_8 ? (arr_10 [i_1]) : var_3));
        }
        var_18 = ((var_0 ? (arr_7 [i_1] [i_1] [i_1]) : (arr_7 [i_1] [i_1] [i_1])));
        var_19 = var_8;
    }
    var_20 = ((!((((max(var_8, var_6)))))));
    #pragma endscop
}
