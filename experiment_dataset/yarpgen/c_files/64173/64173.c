/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64173
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64173 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64173
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= (max((-var_11 || var_4), var_6));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                var_17 = (!((((((arr_1 [i_1] [i_0 - 1]) ? var_3 : (arr_0 [i_0 - 1]))) - var_7))));

                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    arr_8 [i_0] [i_0] [i_0] [i_0 + 1] &= 5999;
                    var_18 = (min(var_18, ((min((min(38, 15783)), (37 >= -4700))))));
                }
            }
        }
    }
    var_19 = (var_1 < ((((~var_4) > var_7))));
    #pragma endscop
}
