/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195737
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195737 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195737
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = -var_0;
    var_18 = ((((min((((4154607605355202578 ? var_7 : var_11))), (max(var_5, var_0))))) ? var_1 : var_14));
    var_19 = (min(var_7, ((((!var_13) && (var_3 && var_0))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_20 -= (((arr_0 [i_0]) ? ((var_14 ? (arr_0 [i_0]) : (arr_1 [i_0] [i_0]))) : (arr_1 [i_0] [i_0])));
        arr_2 [i_0] = var_3;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_5 [i_1] = (~(arr_3 [i_1]));
        arr_6 [i_1] = ((arr_0 [i_1]) >= (arr_0 [i_1]));
    }
    #pragma endscop
}
