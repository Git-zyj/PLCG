/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219926
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219926 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219926
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = min((((((var_5 ? var_1 : -4))) ? (((var_5 ? var_9 : -19))) : var_2)), (min((min(-19, var_2)), var_8)));
    var_11 = ((-(!var_5)));

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_12 |= var_8;
        arr_3 [i_0] [i_0] = (min(((max((arr_2 [i_0] [i_0]), (arr_2 [i_0] [i_0])))), ((~(arr_1 [i_0] [i_0])))));
        var_13 = ((-7 < ((((min(18, (arr_0 [i_0] [i_0])))) ? 16 : (min(32, 0))))));
    }
    #pragma endscop
}
