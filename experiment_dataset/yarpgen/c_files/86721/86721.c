/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86721
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86721 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86721
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += 0;
    var_17 = var_2;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = ((((((arr_0 [i_0]) * (arr_0 [i_0])))) ? ((-(arr_0 [i_0]))) : ((var_5 ? (arr_0 [i_0]) : var_2))));
        var_18 = -var_0;
    }
    for (int i_1 = 1; i_1 < 22;i_1 += 1)
    {
        var_19 &= ((((min(8176, (arr_0 [i_1])))) << (((arr_3 [i_1]) - 9259131956064271594))));
        var_20 = (!var_14);
    }
    var_21 = ((~((4174166438731500810 ? 16252928 : 1))));
    #pragma endscop
}
