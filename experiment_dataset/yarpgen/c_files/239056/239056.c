/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239056
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239056 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239056
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_12, var_12));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 18;i_3 += 1)
                    {
                        {
                            arr_10 [i_0] [i_1] [i_1] [i_1] [i_1] = (min(4255310858, (~var_4)));
                            var_17 = (max(var_17, ((max((((!((!(arr_9 [i_0] [i_2] [i_2] [i_3 - 1])))))), -23810)))));
                            var_18 = ((0 ? (((!((!(arr_6 [i_0] [i_1] [i_3])))))) : (1 || 0)));
                        }
                    }
                }
                var_19 = (max(var_19, (arr_1 [12])));
            }
        }
    }
    #pragma endscop
}
