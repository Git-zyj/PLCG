/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179400
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179400 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179400
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_13 [i_1] = var_4;
                                var_12 = (max(var_12, (((var_6 ? 2382994501096102734 : var_3)))));
                                var_13 &= var_3;
                            }
                        }
                    }
                    var_14 = 0;
                    arr_14 [i_1] [i_1] [i_2] = var_10;
                    var_15 = (max(var_15, (((1 ? ((-2147483631 ? var_8 : var_7)) : 1)))));
                }
                var_16 = var_8;
                var_17 = var_1;
                arr_15 [i_0 - 1] [i_1] = var_10;
                var_18 = (max(var_18, (((-2147483631 ? var_10 : var_1)))));
            }
        }
    }
    var_19 = ((var_8 ? var_1 : var_8));
    #pragma endscop
}
