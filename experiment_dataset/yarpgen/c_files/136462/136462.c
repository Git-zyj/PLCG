/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136462
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136462 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136462
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 23;i_3 += 1)
                    {
                        {
                            var_15 = (arr_2 [i_0] [i_1] [i_2]);
                            var_16 |= -34359738367;
                            var_17 = ((var_9 ? (-34359738357 & -2130377567097251096) : (((-34359738367 + 9223372036854775807) >> (1423338922 - 1423338921)))));
                        }
                    }
                }
                var_18 = var_12;
            }
        }
    }
    var_19 = ((((max(643687187161231740, (min(40763, 34359738379))))) ? (max(var_3, ((var_1 ? -1 : var_4)))) : (((min(10, var_10)) | var_4))));
    #pragma endscop
}
