/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 212263
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=212263 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/212263
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */_Bool) var_7);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 10; i_1 += 1) 
        {
            {
                var_14 = ((/* implicit */unsigned short) var_11);
                var_15 = (!(((/* implicit */_Bool) ((signed char) 1141172638U))));
                arr_6 [i_1] = ((/* implicit */_Bool) (-((~(((/* implicit */int) arr_2 [i_0 - 1] [i_1]))))));
                arr_7 [i_1] = ((/* implicit */short) arr_3 [i_0] [i_1] [i_1]);
                var_16 ^= ((/* implicit */int) ((unsigned long long int) min((((/* implicit */int) ((_Bool) var_9))), ((~(((/* implicit */int) arr_4 [i_0] [i_1])))))));
            }
        } 
    } 
    /* LoopSeq 1 */
    for (int i_2 = 0; i_2 < 21; i_2 += 1) 
    {
        var_17 = ((/* implicit */short) min((var_17), (((/* implicit */short) (unsigned short)29951))));
        arr_10 [i_2] = ((/* implicit */signed char) arr_9 [i_2]);
        var_18 |= ((/* implicit */unsigned int) var_0);
        var_19 = ((/* implicit */unsigned char) max((var_19), (((/* implicit */unsigned char) var_8))));
        arr_11 [i_2] &= ((/* implicit */_Bool) (-(((int) min((((/* implicit */unsigned long long int) (_Bool)0)), (0ULL))))));
    }
}
