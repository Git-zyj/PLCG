/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117822
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117822 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117822
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_19 = ((((max(65515, 1286500389)) == var_18)));
        arr_3 [i_0] [i_0] = (((((~(arr_1 [i_0] [i_0])))) | (((arr_2 [i_0]) ? (arr_1 [i_0] [i_0]) : (arr_2 [i_0])))));
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        arr_6 [i_1] = (~65515);
        arr_7 [i_1] = ((((max((arr_1 [1] [i_1]), (arr_1 [i_1] [i_1])))) ? (((arr_1 [i_1] [i_1]) ? 65515 : (arr_1 [i_1] [i_1]))) : (arr_1 [i_1] [i_1])));
    }
    var_20 = -1208843908;
    var_21 = var_11;
    #pragma endscop
}
