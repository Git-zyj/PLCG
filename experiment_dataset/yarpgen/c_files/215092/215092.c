/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215092
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215092 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215092
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_1;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((((((76 + (arr_0 [i_0] [i_1])))) ? ((-55 ? -5033 : 80)) : (-5030 <= 84))) < (arr_3 [i_0] [i_1] [i_0])));
                var_18 = ((((~-1) ? 5942951351157793285 : (~268435455))) & 274877906943);
                arr_6 [i_0] = (arr_1 [i_0]);
                arr_7 [i_0] [i_0] = (((~(arr_4 [i_0] [i_0] [i_0]))));
                arr_8 [i_0] [i_0] [i_0] = ((15419102278597637179 & (((~(arr_0 [i_0] [i_1]))))));
            }
        }
    }
    var_19 = var_8;
    #pragma endscop
}
