/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72043
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72043 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72043
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = 2324594869;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_13 = (((arr_4 [11] [i_0]) / (min(var_6, (((arr_0 [i_4] [i_0]) ? var_11 : (arr_1 [i_2] [i_1])))))));
                                var_14 = (min(var_14, var_9));
                            }
                        }
                    }
                    var_15 = var_7;
                    var_16 = (max((arr_3 [i_2]), (0 ^ 22586)));
                    var_17 = var_1;
                    var_18 = ((~((min((!3861559776), (!2926489290))))));
                }
            }
        }
    }
    var_19 = ((var_7 ? var_4 : (1368478001 != var_8)));
    var_20 = var_8;
    var_21 = (max(var_21, ((+(((~var_7) >> (var_0 - 4013968559)))))));
    #pragma endscop
}
