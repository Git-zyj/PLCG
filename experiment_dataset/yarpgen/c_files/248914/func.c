/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248914
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248914 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248914
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned long long int) var_7);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11; i_0 += 2) 
    {
        for (unsigned int i_1 = 3; i_1 < 13; i_1 += 4) 
        {
            {
                var_16 -= ((/* implicit */short) ((unsigned char) min(((unsigned char)255), ((unsigned char)1))));
                var_17 = ((/* implicit */_Bool) ((((/* implicit */int) (_Bool)1)) | (((((/* implicit */int) (unsigned char)1)) << (((((/* implicit */int) (unsigned char)255)) - (244)))))));
                arr_6 [i_0 + 3] [i_0 + 3] = ((/* implicit */unsigned int) (short)27718);
                var_18 = ((/* implicit */_Bool) max((var_18), (((/* implicit */_Bool) (short)27717))));
            }
        } 
    } 
}
