/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165513
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165513 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165513
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_20 = (((max(-372302607, (arr_8 [i_3 - 1] [i_3 + 1] [i_3] [i_1 - 1] [i_0])))) ? (!247) : 6);
                                var_21 = (min(var_21, (arr_7 [i_4])));
                                var_22 = var_8;
                            }
                        }
                    }
                    var_23 = (max(3, ((126 ? (arr_9 [i_1 - 2] [8] [i_1 + 1] [i_1 + 1]) : var_11))));
                    arr_12 [i_0] [i_0] [i_0] &= 9;
                }
            }
        }
    }
    var_24 = ((-((8 ? -2052407868 : 222))));
    #pragma endscop
}
