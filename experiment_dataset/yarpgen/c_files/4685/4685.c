/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4685
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4685 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4685
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_18 = ((17494365906328583040 ? ((0 ? 17494365906328583008 : 4031)) : 4020));
                var_19 = ((((arr_4 [i_1] [i_1] [7]) ? (((var_1 ? 3916740398 : 127))) : var_8)));
                var_20 = (((((arr_2 [i_1] [i_0]) && var_5)) ? (((arr_1 [i_0] [i_1]) ? var_15 : var_3)) : (arr_4 [i_0] [i_1] [i_1])));
                var_21 = ((!(~var_8)));
            }
        }
    }
    var_22 = var_9;
    #pragma endscop
}
