/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132301
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132301 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132301
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = max((((/* implicit */unsigned int) (_Bool)1)), (max((((unsigned int) var_3)), (((/* implicit */unsigned int) max((var_11), ((_Bool)1)))))));
    /* LoopNest 3 */
    for (long long int i_0 = 3; i_0 < 18; i_0 += 1) 
    {
        for (long long int i_1 = 0; i_1 < 20; i_1 += 2) 
        {
            for (unsigned short i_2 = 0; i_2 < 20; i_2 += 3) 
            {
                {
                    var_16 |= ((/* implicit */unsigned char) max((((((/* implicit */unsigned long long int) ((/* implicit */int) (_Bool)1))) + (6646225668154604072ULL))), (((/* implicit */unsigned long long int) (+(var_4))))));
                    var_17 = ((/* implicit */int) (!(((/* implicit */_Bool) (~(var_4))))));
                }
            } 
        } 
    } 
    var_18 = ((/* implicit */signed char) var_9);
    var_19 = ((/* implicit */unsigned int) var_0);
    var_20 = ((/* implicit */_Bool) (+(((/* implicit */int) var_7))));
}
