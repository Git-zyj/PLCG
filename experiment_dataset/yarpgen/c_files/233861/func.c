/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233861
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233861 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233861
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned int) ((((/* implicit */int) var_11)) >= (((/* implicit */int) var_11))));
    /* LoopNest 2 */
    for (signed char i_0 = 2; i_0 < 17; i_0 += 1) 
    {
        for (unsigned short i_1 = 3; i_1 < 17; i_1 += 1) 
        {
            {
                arr_6 [i_0] = ((/* implicit */unsigned long long int) var_6);
                arr_7 [i_0] [i_1] [i_1] = ((unsigned long long int) arr_5 [i_0] [i_1 + 1]);
            }
        } 
    } 
    var_19 = ((/* implicit */unsigned long long int) var_1);
    var_20 = ((/* implicit */int) ((((unsigned int) min((1005633613U), (676812960U)))) >= (((/* implicit */unsigned int) 2147221504))));
    var_21 = ((/* implicit */unsigned long long int) var_2);
}
