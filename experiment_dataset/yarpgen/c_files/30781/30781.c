/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30781
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30781 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30781
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_3 [i_0] = ((((((1 || 77) && ((((-2147483647 - 1) ? 14844324068103140783 : 4391201612424048817)))))) == (((var_1 > ((~(arr_0 [i_0]))))))));
        var_17 = (((((var_4 - (arr_0 [i_0])))) == (max((arr_0 [i_0]), 4391201612424048820))));
    }
    /* LoopNest 2 */
    for (int i_1 = 1; i_1 < 7;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 11;i_2 += 1)
        {
            {
                var_18 = (max(var_18, (((!(104 == -4463985068913990590))))));
                var_19 = ((+((((arr_8 [i_1 + 2] [i_1 + 4]) && var_16)))));
            }
        }
    }
    #pragma endscop
}
