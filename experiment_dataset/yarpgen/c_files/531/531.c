/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 531
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=531 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/531
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_1] = (((((-(~255)))) + 22));
                var_11 = (max(((((((arr_2 [i_1] [i_0 + 3] [i_1]) | 25))) ? -24377 : (((min(95, 233)))))), (((arr_3 [i_0]) / (max(var_1, 11938984667201368129))))));
            }
        }
    }
    var_12 = (max((((16286290457668792190 && ((min(-11, -11)))))), (((var_5 < var_7) ^ (139 < var_4)))));
    #pragma endscop
}
