/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27402
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27402 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27402
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_12 = 1;
                                arr_12 [i_0] [i_1] [i_3] [1] [i_3] [i_4] = ((!((((arr_9 [i_0] [i_0] [i_1] [i_1] [i_2] [i_3] [i_4]) ? (arr_7 [i_0] [i_1]) : -2)))));
                            }
                        }
                    }
                    arr_13 [i_0] [i_0] [i_1] [i_2] = ((~(max((arr_4 [i_2] [i_1] [i_0]), 43547))));
                }
            }
        }
    }
    var_13 = (max(var_13, (((((max(var_4, var_5))))))));
    var_14 = (((!1) - (max(((max(1, 255))), 1))));
    #pragma endscop
}
