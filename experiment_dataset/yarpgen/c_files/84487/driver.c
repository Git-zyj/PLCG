#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned char var_1 = (unsigned char)252;
unsigned char var_3 = (unsigned char)143;
unsigned int var_4 = 3305387995U;
unsigned int var_6 = 3107913546U;
unsigned int var_7 = 2153202881U;
signed char var_8 = (signed char)-76;
unsigned char var_9 = (unsigned char)16;
unsigned int var_10 = 1343699868U;
unsigned int var_11 = 2666200542U;
int zero = 0;
unsigned char var_12 = (unsigned char)69;
_Bool var_13 = (_Bool)1;
int var_14 = 3338644;
unsigned int var_15 = 786410033U;
unsigned int var_16 = 3272780011U;
_Bool var_17 = (_Bool)0;
signed char arr_0 [24] ;
unsigned int arr_2 [24] [24] ;
_Bool arr_4 [24] [24] [24] ;
unsigned int arr_6 [24] ;
void init() {
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        arr_0 [i_0] = (signed char)-99;
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        for (size_t i_1 = 0; i_1 < 24; ++i_1) 
            arr_2 [i_0] [i_1] = (i_0 % 2 == 0) ? 3527470793U : 1523022398U;
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        for (size_t i_1 = 0; i_1 < 24; ++i_1) 
            for (size_t i_2 = 0; i_2 < 24; ++i_2) 
                arr_4 [i_0] [i_1] [i_2] = (i_1 % 2 == 0) ? (_Bool)1 : (_Bool)1;
    for (size_t i_0 = 0; i_0 < 24; ++i_0) 
        arr_6 [i_0] = 12002526U;
}

void checksum() {
    hash(&seed, var_12);
    hash(&seed, var_13);
    hash(&seed, var_14);
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
