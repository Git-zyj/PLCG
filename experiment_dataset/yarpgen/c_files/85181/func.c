/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85181
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85181 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85181
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) ((((/* implicit */long long int) ((/* implicit */int) var_1))) <= (var_6)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (short i_0 = 2; i_0 < 12; i_0 += 3) 
    {
        arr_2 [4ULL] [i_0] = ((/* implicit */signed char) (~(((/* implicit */int) (!(((/* implicit */_Bool) var_6)))))));
        var_15 = ((/* implicit */short) (!(((((/* implicit */_Bool) (short)-27776)) && (((/* implicit */_Bool) var_1))))));
    }
    var_16 = ((/* implicit */long long int) var_10);
}
