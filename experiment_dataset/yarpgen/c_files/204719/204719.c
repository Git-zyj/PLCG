/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204719
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204719 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204719
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_1] [1] = ((+(((!var_4) ? ((51187 ? var_2 : var_2)) : -2091682193441056564))));
                arr_6 [i_1] = ((max(-1951493723, (max(2091682193441056564, var_6)))));
            }
        }
    }
    var_11 = ((~((((var_1 > var_1) || (((50395 ? 16355061880268495056 : -513788271))))))));
    #pragma endscop
}
