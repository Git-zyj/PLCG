/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131027
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131027 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131027
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_0] [i_2] = (((((min(-69, (arr_3 [i_0] [i_1] [i_2]))))) | ((~(arr_2 [9] [i_2])))));
                    arr_7 [i_0] [i_2] [i_2] = (((((-(arr_2 [i_0] [i_1])))) ? (arr_2 [i_0] [i_0]) : (min(3465317470915675236, (arr_2 [i_0] [i_1])))));
                    var_11 = (min(var_11, (arr_0 [i_1])));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_13 [i_2] [i_3] [i_2] [i_1] [i_2] = -69;
                                arr_14 [i_0] [9] [i_0] [i_3] [i_4 + 1] [i_2] [i_0] = var_10;
                            }
                        }
                    }
                }
            }
        }
    }
    var_12 = var_9;
    #pragma endscop
}
