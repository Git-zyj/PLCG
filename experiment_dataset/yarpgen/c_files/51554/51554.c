/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51554
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51554 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51554
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    arr_7 [i_1] [i_1] = arr_5 [i_0] [i_1] [i_2];
                    var_13 = (max(var_13, (((((var_9 != (!var_10))))))));
                }
                /* LoopNest 3 */
                for (int i_3 = 1; i_3 < 21;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 22;i_4 += 1)
                    {
                        for (int i_5 = 1; i_5 < 21;i_5 += 1)
                        {
                            {
                                arr_18 [9] [i_1] [i_3] [14] [i_4] [i_1] = (((arr_16 [i_1]) ? (((((((~(arr_11 [i_3])))) && (arr_15 [i_0 - 1]))))) : ((var_5 ? var_2 : var_3))));
                                var_14 = ((-(!var_5)));
                            }
                        }
                    }
                }
                arr_19 [11] [i_1] = ((((((var_0 | var_5) | (arr_12 [i_0] [i_0 + 3] [i_0] [i_0 + 3])))) || ((min(var_10, var_9)))));
            }
        }
    }
    var_15 |= ((((max((var_3 >= var_5), var_1))) ? ((min(var_7, (!var_4)))) : var_6));
    #pragma endscop
}
