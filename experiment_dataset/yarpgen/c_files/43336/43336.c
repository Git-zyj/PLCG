/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43336
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43336 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43336
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(var_15, var_5));

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = ((-(max((arr_0 [i_0]), (arr_1 [i_0])))));
        var_16 -= (max((((arr_0 [i_0]) ? var_4 : (arr_0 [i_0]))), (arr_0 [i_0])));
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        arr_7 [i_1] = (((min((arr_1 [19]), (arr_1 [i_1]))) ? (((-(arr_1 [i_1])))) : 14450548316247276177));
        arr_8 [i_1] [i_1] = (((!((max(143953074716486206, 44422))))));
    }
    var_17 = var_8;
    var_18 = ((127 ? (((21091 ? 105 : 92))) : ((var_8 ? var_11 : (max(var_12, var_8))))));
    var_19 = -1086198103;
    #pragma endscop
}
