/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106094
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = 0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_16 -= (min(((min((5401 ^ 0), (-1 && 1206703904)))), (max((166 / 5631854667454751916), (2409240108 >= -5018697692753227477)))));
                var_17 = 2114393448;
                arr_4 [i_1] [i_0 + 2] = ((-1 > (((0 + 5112585448549748349) / (10833 | 1)))));
            }
        }
    }
    var_18 = (max(var_18, (((187 ^ ((max((max(0, 204)), 116))))))));
    #pragma endscop
}
