/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64157
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64157 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64157
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (short i_0 = 2; i_0 < 11; i_0 += 2) 
    {
        for (short i_1 = 2; i_1 < 11; i_1 += 3) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    arr_6 [i_2] = ((/* implicit */long long int) ((unsigned short) (_Bool)1));
                    arr_7 [i_2] [i_1] [i_2] = ((/* implicit */signed char) (~(((((((/* implicit */int) arr_5 [i_2] [i_2] [i_2] [i_0 - 1])) + (2147483647))) >> (((((/* implicit */int) var_8)) + (29839)))))));
                }
            } 
        } 
    } 
    var_13 ^= ((/* implicit */short) (+((+(((/* implicit */int) var_9))))));
    var_14 = ((/* implicit */_Bool) min((max((var_6), (((/* implicit */short) var_12)))), (((/* implicit */short) (!(((/* implicit */_Bool) var_2)))))));
    var_15 = ((/* implicit */signed char) var_10);
}
