/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138980
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138980 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138980
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_17 = ((((((8792964487724788065 ? 252 : -27886)))) ^ ((~(~127)))));
        arr_4 [3] = (arr_2 [i_0]);
        var_18 += ((0 * (((((var_1 == -27900) != (arr_2 [i_0])))))));
        var_19 = (max(var_19, (((var_13 == ((((((arr_0 [i_0]) ? var_1 : var_16))))))))));
    }
    var_20 = var_5;
    /* LoopNest 2 */
    for (int i_1 = 4; i_1 < 23;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 23;i_2 += 1)
        {
            {
                arr_11 [i_2] [i_2] = var_11;
                arr_12 [i_2] [i_1] [i_2] = (arr_10 [i_2] [i_2]);
            }
        }
    }
    #pragma endscop
}
