/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65095
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65095 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65095
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0] = var_12;
        var_14 = (arr_0 [i_0] [i_0]);
        var_15 -= (((arr_0 [i_0] [i_0 - 2]) & (arr_0 [i_0] [i_0 + 1])));
        arr_3 [i_0] &= ((max((arr_0 [i_0] [i_0 - 2]), (min((arr_1 [i_0]), var_0)))));
    }
    for (int i_1 = 3; i_1 < 9;i_1 += 1) /* same iter space */
    {
        arr_7 [i_1] [i_1] = (arr_0 [i_1] [i_1]);
        arr_8 [i_1] [i_1 - 2] = (arr_4 [i_1] [i_1]);
        var_16 ^= ((!(arr_0 [15] [i_1])));
        arr_9 [i_1] [i_1] = ((((max((arr_0 [i_1 + 2] [i_1 + 2]), (arr_0 [i_1 + 2] [i_1 + 2])))) ? (arr_5 [i_1 + 1]) : (min((arr_4 [i_1 - 2] [i_1]), (arr_4 [i_1 - 1] [i_1 - 3])))));
        var_17 = (arr_0 [i_1 - 1] [i_1]);
    }
    for (int i_2 = 3; i_2 < 9;i_2 += 1) /* same iter space */
    {

        for (int i_3 = 2; i_3 < 9;i_3 += 1)
        {
            var_18 += 89309727;
            var_19 = (arr_1 [i_3]);
        }
        arr_14 [i_2] [i_2] = ((min((arr_0 [i_2] [i_2 + 2]), (arr_11 [i_2] [i_2 + 1]))));
    }
    var_20 = var_9;
    #pragma endscop
}
