/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157715
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */short) var_5);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 7; i_1 += 2) 
        {
            {
                var_13 = ((/* implicit */unsigned int) min((var_13), (((/* implicit */unsigned int) (!(((/* implicit */_Bool) ((unsigned char) var_3))))))));
                /* LoopSeq 1 */
                for (unsigned int i_2 = 0; i_2 < 10; i_2 += 4) 
                {
                    var_14 = ((/* implicit */int) ((((/* implicit */_Bool) max((arr_2 [i_1 - 2] [i_1 + 2]), (arr_2 [i_1 - 2] [i_1 + 3])))) ? (((/* implicit */unsigned long long int) var_0)) : (((unsigned long long int) var_7))));
                    var_15 = ((/* implicit */unsigned long long int) max((var_15), (((/* implicit */unsigned long long int) var_6))));
                }
            }
        } 
    } 
    var_16 = ((/* implicit */long long int) ((unsigned int) var_4));
}
