/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198710
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198710 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198710
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 ^= (-((-(min((4568612391780573007ULL), (var_13))))));
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 10; i_1 += 1) 
        {
            for (unsigned int i_2 = 1; i_2 < 10; i_2 += 1) 
            {
                {
                    arr_8 [i_0] [i_0] [i_2] = ((/* implicit */long long int) ((unsigned int) ((long long int) ((((/* implicit */_Bool) var_9)) ? (var_13) : (((/* implicit */unsigned long long int) var_1))))));
                    var_16 = ((/* implicit */int) min((var_16), (((/* implicit */int) (signed char)0))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */int) 18446744073709551615ULL);
    var_18 = ((/* implicit */unsigned short) var_9);
}
