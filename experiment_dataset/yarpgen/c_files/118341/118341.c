/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118341
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118341 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118341
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_0;

    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = (max(112, 0));
        arr_4 [i_0] = ((((((arr_0 [i_0]) ? (((!(-9223372036854775807 - 1)))) : (!var_2)))) ? (((((31 ? (arr_1 [i_0]) : 132)) ^ -21))) : (-9223372036854775807 - 1)));

        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            arr_8 [i_0] = ((-((~(!var_5)))));
            var_14 = (((((arr_5 [i_0 + 1]) ? 3 : (arr_5 [i_0 + 1]))) - 57601));
        }
    }
    #pragma endscop
}
