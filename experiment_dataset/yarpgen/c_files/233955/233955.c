/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233955
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233955 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233955
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((!((((max(var_3, 15307)) << (var_15 + 72))))));
    var_17 = (max(var_17, var_8));
    var_18 = (max(var_18, 234));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                var_19 = ((((!(arr_0 [i_1 + 2]))) ? (arr_0 [i_1 + 1]) : (((arr_2 [i_1 + 1] [i_1 + 3] [11]) ? (arr_0 [i_1 - 1]) : (arr_2 [i_1 - 2] [i_1 - 1] [i_1 - 1])))));
                var_20 = (max((((arr_0 [i_1 - 1]) ? 234 : 111)), 108));
            }
        }
    }
    var_21 = var_6;
    #pragma endscop
}
