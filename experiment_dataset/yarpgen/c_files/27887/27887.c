/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27887
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27887 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27887
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 13;i_2 += 1)
            {
                {
                    arr_6 [i_0] [4] [i_2] [13] = arr_1 [i_0];
                    var_17 += ((((((34787 ? (arr_5 [i_0] [i_0] [i_0]) : (arr_3 [i_2]))) + (((max((arr_0 [i_0]), var_8)))))) / (((((var_7 ? (arr_5 [i_0] [i_0] [i_2]) : (arr_0 [i_0])))) ? (((arr_5 [i_0] [i_1] [i_2]) ? (arr_0 [i_0]) : (arr_3 [i_0]))) : (((arr_1 [i_0]) / var_1))))));
                }
            }
        }
    }
    var_18 = ((-(var_8 & var_11)));
    var_19 = ((-((~(var_4 * var_10)))));
    #pragma endscop
}
