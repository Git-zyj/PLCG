/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15387
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15387 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15387
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((var_10 ? (((min(2147483647, 117)) - var_3)) : ((var_3 ? var_0 : -var_1))));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_20 = (max(var_20, (((~(((!-512) << (((min(var_13, 33)) + 2040578135)))))))));
        arr_4 [i_0] [i_0] = (min((((arr_0 [i_0] [i_0]) ? var_12 : 2147483647)), -6242507857707630437));
    }
    #pragma endscop
}
