/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136189
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136189 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136189
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_0 - 3] = (((((!(~2)))) & (arr_4 [i_0] [i_0] [i_0 - 2])));
                arr_7 [i_0] = (max(((((min((arr_3 [9] [5]), 1978896041)) + ((((arr_5 [i_0]) / -22848)))))), ((45230 ? (arr_4 [i_0 - 2] [i_1] [i_1]) : (((arr_4 [i_0 - 3] [i_0 - 3] [i_0]) + (arr_0 [i_1])))))));
            }
        }
    }
    var_16 = ((var_3 ? var_10 : var_11));
    var_17 &= (max(17194, 2269620816));
    #pragma endscop
}
