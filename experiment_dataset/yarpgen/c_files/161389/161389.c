/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 161389
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=161389 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/161389
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 1;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_4 [i_0 + 1] [16] [i_0] = (max((max((min(3539968420, 2021760261)), -8662)), (((-((min(var_6, var_10))))))));
                var_17 = (max(((-(arr_3 [i_0 + 2]))), (((arr_0 [i_0 - 1]) ? var_15 : (arr_3 [i_0])))));
                var_18 = (((((~(var_9 ^ var_5)))) ? 6379526806296580072 : var_11));
                arr_5 [i_0 - 1] [10] [i_1] = var_14;
            }
        }
    }
    #pragma endscop
}
