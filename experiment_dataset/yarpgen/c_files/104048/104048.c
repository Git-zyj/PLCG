/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104048
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((var_11 * ((max(var_4, var_3)))));
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        {
                            var_13 = -197;
                            var_14 = ((-(~0)));
                        }
                    }
                }
                var_15 = -89;
                var_16 &= ((~((min((arr_2 [i_0 - 4] [i_0 + 1]), (arr_2 [i_0 - 4] [i_1]))))));
                var_17 &= (arr_2 [i_0] [i_0]);
                /* LoopNest 3 */
                for (int i_4 = 3; i_4 < 13;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 16;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 16;i_6 += 1)
                        {
                            {
                                var_18 += (((arr_8 [8] [i_0]) >> (((arr_1 [i_5]) + 9921))));
                                var_19 = ((((max((arr_10 [i_4 + 1] [i_4 + 1] [i_4]), (arr_5 [i_0 - 3])))) ^ (arr_10 [i_4 + 1] [i_4 + 1] [i_4 + 2])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
