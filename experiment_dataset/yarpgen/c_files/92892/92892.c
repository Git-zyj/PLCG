/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92892
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92892 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92892
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 21;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1 - 1] [i_2] = (max(((-(arr_7 [i_2] [i_0] [i_2 - 2]))), (((!(arr_4 [i_1 + 3] [i_1 - 1]))))));
                    var_18 += ((((((arr_6 [i_2]) ? (max(var_5, (arr_8 [i_0] [i_0] [16] [i_0]))) : var_1))) && (((4032 ? 18446744073709551601 : 1)))));
                }
            }
        }
    }
    var_19 = (min(var_19, ((max(var_7, var_5)))));
    var_20 = (min(var_20, var_13));
    #pragma endscop
}
