/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178271
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178271 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178271
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (arr_3 [i_0]);
                arr_7 [i_0] [i_1] = (arr_2 [i_1]);
                var_10 = ((!((min((arr_1 [i_0 - 1] [i_0]), var_2)))));
            }
        }
    }
    var_11 = (max(var_11, (!var_7)));
    var_12 = var_6;
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 12;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 15;i_3 += 1)
        {
            {
                arr_12 [i_2] [i_3] = ((!(((((arr_11 [i_3] [i_2 + 3] [i_2 + 3]) ? var_5 : (arr_11 [i_2] [i_2 - 2] [i_3])))))));
                arr_13 [i_2 + 3] [i_3] = 3382190245;
                arr_14 [i_3] = (max((arr_3 [i_3]), (arr_10 [i_2] [i_3] [i_3])));
            }
        }
    }
    #pragma endscop
}
