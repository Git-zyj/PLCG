/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172132
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_9, var_4));
    var_11 = ((var_4 ? -717083455 : 1));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 15;i_1 += 1)
        {
            {
                var_12 = ((!((!(arr_4 [i_0 + 3] [i_1 - 3] [i_1])))));
                arr_5 [i_0] [i_1] [i_1] = ((~(((arr_3 [i_0 + 1]) << (-473350467 + 473350486)))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 17;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            var_13 = ((min(var_1, (arr_9 [i_0 + 2] [i_1 - 3]))));
                            var_14 = -518675292;
                            arr_10 [i_1 - 3] [i_3] = (max(var_1, ((518675291 ? -518675292 : var_3))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
