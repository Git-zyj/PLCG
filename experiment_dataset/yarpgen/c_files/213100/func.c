/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213100
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213100 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213100
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 9; i_0 += 4) 
    {
        for (short i_1 = 1; i_1 < 7; i_1 += 1) 
        {
            for (unsigned short i_2 = 3; i_2 < 8; i_2 += 4) 
            {
                {
                    var_20 = ((/* implicit */int) min((var_20), (((/* implicit */int) ((unsigned char) (-((+(((/* implicit */int) arr_3 [i_0 - 1] [i_0 - 1] [i_0 - 1]))))))))));
                    var_21 = ((/* implicit */unsigned int) arr_2 [i_1]);
                }
            } 
        } 
    } 
    var_22 = ((/* implicit */signed char) (-(((((/* implicit */unsigned int) ((((/* implicit */_Bool) var_19)) ? (((/* implicit */int) var_4)) : (((/* implicit */int) var_2))))) + ((+(var_3)))))));
}
