/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182484
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182484 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182484
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((1 ? 1 : 40768));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 16;i_1 += 1)
        {
            {
                var_17 ^= ((((((arr_2 [i_1 + 1]) ? (arr_2 [i_1 - 2]) : (arr_2 [i_1 - 1])))) ? ((var_7 ? ((max(var_10, var_8))) : 145)) : ((((max(var_0, (arr_0 [i_0 - 2])))) ? -var_14 : ((min((arr_1 [i_1 - 3]), 1)))))));
                var_18 = (max(((max((var_8 || 4698925884066564758), (arr_3 [i_0] [i_1])))), (max(9189, ((var_1 ? (-9223372036854775807 - 1) : var_0))))));
            }
        }
    }
    #pragma endscop
}
