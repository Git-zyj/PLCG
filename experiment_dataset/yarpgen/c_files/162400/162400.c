/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162400
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162400 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162400
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = ((7986277099468440451 ? 4264365573 : 4016652551));
                var_17 += ((-(arr_3 [i_0 - 1] [i_0 - 1] [i_0 - 1])));
                var_18 = (arr_1 [i_0 - 1]);
                var_19 = (max(var_19, 34260));
            }
        }
    }
    var_20 = ((max(var_6, (min(var_7, var_2)))));
    var_21 = (min(var_6, 2923590392));
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 22;i_3 += 1)
        {
            {
                var_22 = (((min(((var_2 ? 4016652534 : (arr_5 [i_2]))), var_5)) < -19463));
                var_23 = (arr_5 [i_3]);
            }
        }
    }
    #pragma endscop
}
