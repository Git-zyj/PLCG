/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88427
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88427 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88427
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        arr_4 [i_0] [i_0] = ((/* implicit */unsigned long long int) ((unsigned char) max((((/* implicit */unsigned long long int) var_14)), (((((/* implicit */_Bool) var_18)) ? (8796025913344ULL) : (((/* implicit */unsigned long long int) ((/* implicit */int) arr_1 [i_0]))))))));
        var_19 = ((/* implicit */unsigned long long int) (-((-(((/* implicit */int) (unsigned char)63))))));
        var_20 = (_Bool)1;
    }
    var_21 = ((/* implicit */_Bool) var_15);
    var_22 = ((/* implicit */unsigned char) var_14);
    var_23 = ((/* implicit */short) var_2);
}
