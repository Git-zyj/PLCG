/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114876
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114876 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114876
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_15 = 251;
        var_16 = (((((var_12 ? var_9 : var_9))) ? (((-(arr_2 [i_0] [i_0])))) : (((((arr_1 [i_0] [i_0]) + 9223372036854775807)) >> (((arr_1 [i_0] [i_0]) + 3175494211969312719))))));
        var_17 |= ((((4 ? 2060468831 : (arr_0 [i_0]))) + (18519 % 4)));
        var_18 = ((!((max(var_3, ((var_3 ? var_3 : 125)))))));
    }
    var_19 = (((var_8 ? var_10 : ((var_8 ? var_12 : 0)))));
    #pragma endscop
}
