/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220057
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220057 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220057
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */long long int) 8380416ULL);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        var_13 = ((/* implicit */unsigned long long int) max((((long long int) arr_0 [i_0])), (((var_0) - (((/* implicit */long long int) ((/* implicit */int) (short)-32767)))))));
        var_14 ^= ((/* implicit */long long int) 2097150);
    }
}
