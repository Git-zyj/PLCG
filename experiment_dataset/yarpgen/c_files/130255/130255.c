/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130255
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130255 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130255
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [i_0] [i_3] [i_0] [i_0] = ((((max((arr_7 [i_1 - 1] [i_1 - 1] [i_1 + 1] [i_1 + 1]), (arr_1 [i_1 + 1])))) ? (min((arr_7 [i_1 - 1] [i_1 + 1] [i_1 + 1] [i_1 + 1]), (arr_7 [i_1 + 1] [i_1] [i_1 + 1] [i_1 + 1]))) : (((var_17 ? 6947 : 1)))));
                            arr_10 [i_0] [i_3] = ((-(1 * 1775)));
                        }
                    }
                }
                var_19 = (max(var_19, var_1));
                arr_11 [i_0] [i_0] = var_9;
            }
        }
    }
    var_20 = 1;
    #pragma endscop
}
