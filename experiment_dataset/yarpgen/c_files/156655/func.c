/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156655
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156655 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156655
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */signed char) var_13);
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 2; i_0 < 8; i_0 += 1) 
    {
        for (short i_1 = 1; i_1 < 8; i_1 += 1) 
        {
            for (unsigned int i_2 = 2; i_2 < 11; i_2 += 4) 
            {
                {
                    var_19 = ((/* implicit */long long int) ((((/* implicit */_Bool) (+(max((2057546542), (((/* implicit */int) (_Bool)1))))))) ? (((/* implicit */unsigned int) ((/* implicit */int) var_11))) : (3737863674U)));
                    arr_7 [i_2] [i_0] [i_0] [i_0] = ((((((/* implicit */_Bool) var_17)) ? (arr_0 [i_0] [i_0]) : (var_6))) < (((/* implicit */int) var_15)));
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */long long int) var_12);
}
