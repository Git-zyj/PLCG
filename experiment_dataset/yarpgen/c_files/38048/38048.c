/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38048
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 4; i_0 < 18;i_0 += 1) /* same iter space */
    {
        var_18 = (max(var_18, 57));
        arr_2 [i_0] [i_0] = (((((~(arr_1 [i_0 + 1] [i_0 - 3])))) == var_5));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 17;i_2 += 1)
            {
                {
                    var_19 -= 6484;
                    var_20 = ((3268066580074115664 ? (min(((var_2 ? var_15 : var_5)), (arr_5 [i_0 + 1] [i_2 - 1] [i_2]))) : (((-(arr_4 [i_2 - 1] [i_2] [i_2]))))));
                }
            }
        }
    }
    /* vectorizable */
    for (int i_3 = 4; i_3 < 18;i_3 += 1) /* same iter space */
    {
        arr_11 [i_3] [i_3] = (((arr_10 [i_3] [9]) + (arr_0 [i_3 - 3])));
        var_21 = (~49);
        arr_12 [8] = (arr_7 [i_3 - 2] [i_3] [i_3 + 1]);
        arr_13 [i_3] [i_3] = (((((arr_1 [i_3] [i_3]) || 49)) ? (arr_9 [i_3]) : (!var_1)));
    }
    var_22 |= (((~var_10) ? (~var_13) : ((((249 ? var_14 : 198)) >> ((((min(var_0, var_16))) - 44472))))));
    #pragma endscop
}
