/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230287
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230287 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230287
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) var_8);
    var_11 = ((/* implicit */int) ((((((/* implicit */_Bool) 1158631774U)) ? (((/* implicit */int) (short)32761)) : ((-(1102627603))))) <= (((/* implicit */int) (_Bool)1))));
    /* LoopSeq 1 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_12 = ((/* implicit */unsigned char) max((var_12), (((/* implicit */unsigned char) max((arr_1 [i_0 - 1] [i_0 - 1]), (min((((/* implicit */unsigned short) var_9)), (arr_1 [i_0] [i_0 - 1]))))))));
        arr_3 [i_0] = ((/* implicit */_Bool) arr_1 [i_0] [i_0]);
    }
}
