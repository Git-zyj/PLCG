/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44270
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44270 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44270
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_12));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 13;i_2 += 1)
            {
                {
                    var_15 *= (max((((arr_6 [i_2 + 1] [7] [i_2]) ? var_1 : (arr_6 [i_2 + 1] [i_1] [7]))), (max(var_8, (((var_4 ? 1 : 1889044923)))))));
                    var_16 = (max(var_16, (arr_1 [i_0])));

                    for (int i_3 = 0; i_3 < 0;i_3 += 1)
                    {
                        var_17 = var_12;
                        var_18 = (+(max((((arr_12 [i_1]) % -1283437149)), (6839778117969290884 <= 94))));
                        var_19 = -var_0;
                    }
                    for (int i_4 = 0; i_4 < 14;i_4 += 1) /* same iter space */
                    {
                        var_20 = (min(((var_12 - (max(var_4, var_3)))), var_6));
                        var_21 -= 2658;
                        var_22 = max((min((arr_15 [i_2 + 1] [i_2] [i_2 - 1]), var_11)), var_12);
                    }
                    for (int i_5 = 0; i_5 < 14;i_5 += 1) /* same iter space */
                    {
                        var_23 *= (arr_3 [i_5] [i_5] [i_5]);
                        var_24 = (min(var_24, ((((var_1 + var_10) ? var_7 : var_2)))));
                    }
                }
            }
        }
    }
    #pragma endscop
}
