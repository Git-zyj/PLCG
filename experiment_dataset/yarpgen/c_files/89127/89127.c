/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89127
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89127 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89127
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, var_6));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (min(((((-(arr_1 [i_0]))))), ((((min(var_14, (arr_3 [20] [19] [20])))) - (1291734939 + 38821)))));
                var_17 *= (((576460752303423456 & 1) ? ((max(((-978129855 & (arr_2 [i_0]))), (min(117, var_9))))) : ((~(~576460752303423456)))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        for (int i_3 = 2; i_3 < 8;i_3 += 1)
        {
            {
                arr_11 [3] [3] = ((!(((~(~var_5))))));
                var_18 = (94 - var_12);
                arr_12 [i_3 - 1] [i_2] = var_7;
                var_19 *= (((((min(3003232356, 576460752303423456))) ? (((var_2 ? 113 : (arr_7 [0])))) : 0)));
            }
        }
    }
    #pragma endscop
}
