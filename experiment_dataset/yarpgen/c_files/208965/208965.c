/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208965
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208965 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208965
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((max(var_6, (-1231117539 / var_3))));
    var_18 = ((var_5 ? (max(var_7, (max(1231117538, -1231117539)))) : ((var_9 & ((max(var_4, var_10)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_0] = (min((((((var_13 + (arr_1 [i_1] [i_0])))) ? var_1 : (max(var_10, (arr_1 [i_0] [i_1]))))), (arr_1 [i_1] [i_0])));
                arr_7 [i_0] [i_0] [i_0] = var_13;
            }
        }
    }
    #pragma endscop
}
