/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141065
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141065 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141065
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned short) 1042358417);
    var_16 = ((/* implicit */unsigned long long int) ((((/* implicit */int) var_0)) >= (((/* implicit */int) var_12))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 3) 
    {
        for (unsigned short i_1 = 1; i_1 < 19; i_1 += 4) 
        {
            {
                arr_4 [i_0] = ((/* implicit */_Bool) (~(((/* implicit */int) max((var_14), (((/* implicit */unsigned char) var_8)))))));
                var_17 = ((/* implicit */_Bool) (-(((/* implicit */int) arr_2 [i_0] [i_1 + 1]))));
            }
        } 
    } 
    var_18 ^= ((/* implicit */short) var_2);
}
