/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51504
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51504 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51504
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = (arr_3 [i_1]);
                var_19 = ((((((min((arr_5 [i_1] [i_0] [i_0 - 2]), var_9)) * ((49023 + (arr_5 [i_0] [i_0] [2])))))) ? ((~(((arr_3 [i_1 + 2]) ? (arr_1 [i_0]) : (arr_4 [i_0] [i_1] [i_1]))))) : (((~16750) ? (((arr_4 [6] [i_1] [i_1 - 1]) ? (arr_0 [i_0]) : 0)) : (var_3 + var_16)))));
            }
        }
    }
    var_20 = (((var_5 == (min(var_2, 94)))));
    #pragma endscop
}
