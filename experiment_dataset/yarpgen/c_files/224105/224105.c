/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224105
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224105 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224105
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        var_16 = (arr_0 [i_0]);
        var_17 -= (158 > 1);
        var_18 = var_4;
        arr_4 [i_0] [i_0] = (arr_1 [i_0]);
        var_19 = ((216 && (((112 ? (arr_3 [i_0] [i_0]) : 4294967279)))));
    }
    for (int i_1 = 0; i_1 < 19;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = (((1 | 41357) >> (((((arr_5 [i_1] [i_1]) >> (var_7 - 17762012765769633083))) - 705965361050989))));
        arr_8 [i_1] = (arr_5 [i_1] [i_1]);
        var_20 = (min(var_20, var_14));
    }
    var_21 = (min(var_21, (((max(15871, -1029035507884243146))))));
    var_22 = -3686287277590657921;
    #pragma endscop
}
