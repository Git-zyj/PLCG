/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71420
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71420 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71420
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (((var_10 ? var_3 : ((min(var_11, var_14))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_16 = var_1;
                var_17 = var_5;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 15;i_3 += 1)
        {
            {
                arr_9 [i_2] [i_3] [i_3] = (min((((((arr_8 [i_3]) > var_9)) ? var_8 : (arr_7 [i_2]))), (var_6 & var_1)));
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 1;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 15;i_5 += 1)
                    {
                        {
                            var_18 = var_6;
                            arr_14 [i_5] [i_3] [i_3] [i_3] [i_3] [i_3] = var_8;
                            arr_15 [i_5] [i_3] [i_4] [i_5] = ((-(min((min((arr_7 [i_2]), (arr_6 [5]))), (((-5715374182629702331 ? var_11 : var_2)))))));
                        }
                    }
                }
                var_19 = (~18446744073709551592);
            }
        }
    }
    #pragma endscop
}
