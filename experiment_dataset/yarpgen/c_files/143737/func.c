/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143737
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143737 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143737
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = var_10;
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 1; i_0 < 8; i_0 += 2) 
    {
        var_21 = ((/* implicit */unsigned short) ((unsigned char) arr_0 [i_0]));
        var_22 = ((/* implicit */short) (unsigned short)4);
    }
    var_23 = ((/* implicit */short) ((min((((((/* implicit */_Bool) (unsigned short)12101)) ? (((/* implicit */int) var_9)) : (((/* implicit */int) var_2)))), (((/* implicit */int) max((var_18), (var_14)))))) + (((/* implicit */int) (short)8176))));
}
