/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162844
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162844 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162844
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        var_12 = (max((((arr_1 [i_0 - 1] [i_0 - 1]) / (arr_1 [i_0 + 1] [i_0 - 2]))), (max((arr_1 [i_0 + 1] [i_0 + 1]), (arr_1 [i_0 - 1] [i_0 - 2])))));
        arr_2 [i_0] = (((arr_1 [i_0 + 1] [i_0 + 1]) * 2));
    }
    var_13 = ((-(((((1 ? var_4 : var_3))) ? var_6 : var_4))));
    var_14 = var_4;
    var_15 = ((!1) | ((((max(68718428160, -90))) ? var_0 : var_3)));
    #pragma endscop
}
