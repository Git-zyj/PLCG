/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33042
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33042 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33042
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 -= (((((arr_0 [i_0]) ? (max((arr_0 [i_0]), (arr_0 [i_0]))) : (((min(-60, 13))))))) ? (arr_2 [i_1] [i_1] [i_0]) : (arr_0 [i_0]));
                arr_4 [14] = (((((((max(var_9, var_5))) % -var_1))) ? (((max(-43, 26482)))) : (max((min(var_8, (arr_0 [i_1]))), (arr_3 [i_0] [i_0] [i_0])))));
                var_16 = (arr_0 [i_0]);
            }
        }
    }
    var_17 = ((~(((var_8 % var_2) / ((var_13 ? var_10 : var_6))))));
    #pragma endscop
}
