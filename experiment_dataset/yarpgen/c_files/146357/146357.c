/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146357
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146357 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146357
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        var_15 = (((arr_1 [i_0 + 1] [i_0]) ? 79 : var_13));
        var_16 = (arr_2 [i_0] [i_0 - 1]);

        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            var_17 = 115;
            arr_5 [i_0] = (9223372036854775807 && var_6);
        }
        for (int i_2 = 1; i_2 < 19;i_2 += 1)
        {
            arr_8 [i_0] [i_0] [i_2] = var_4;
            /* LoopNest 2 */
            for (int i_3 = 1; i_3 < 19;i_3 += 1)
            {
                for (int i_4 = 1; i_4 < 18;i_4 += 1)
                {
                    {
                        var_18 = ((32979 ? -1245947076 : var_1));
                        var_19 = (1 % -86);
                    }
                }
            }

            for (int i_5 = 0; i_5 < 20;i_5 += 1)
            {
                arr_17 [i_0] [i_0] [7] [i_5] = var_7;
                arr_18 [i_0] [i_2 + 1] [i_2] [i_0] = (arr_2 [i_0] [i_2]);
                var_20 = ((!(arr_10 [i_0] [i_2 + 1] [i_5])));
                arr_19 [i_0] = (!41674);
            }
        }
        for (int i_6 = 0; i_6 < 20;i_6 += 1)
        {
            arr_22 [i_0] [i_0] = 0;
            arr_23 [i_0] [i_6] [i_0] = (((arr_4 [i_0]) - (((arr_6 [i_0 - 1]) - var_13))));
        }
        for (int i_7 = 0; i_7 < 20;i_7 += 1)
        {
            var_21 = arr_16 [i_7] [i_0] [i_0];
            arr_26 [i_0] [i_7] = var_2;
            arr_27 [i_0] = (3434 ^ 15765);
        }
    }
    for (int i_8 = 0; i_8 < 11;i_8 += 1)
    {
        arr_31 [i_8] = 0;
        arr_32 [i_8] = ((-115 ? 41 : -5697813004748076373));
    }
    var_22 = (min(var_22, var_12));
    #pragma endscop
}
