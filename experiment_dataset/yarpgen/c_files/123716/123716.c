/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123716
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123716 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123716
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((min(((var_12 ? var_7 : var_8)), var_9)) - (((var_9 ? 1551190969 : var_16)))));
    var_18 *= (((var_7 != 9223372036854775807) <= var_13));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [8] [i_1] = var_15;
                var_19 *= ((0 ? 33 : 32742));
                var_20 ^= var_4;
            }
        }
    }
    var_21 = (min(var_21, (((var_10 ? (((((31744 ? var_1 : var_4))) ? (var_11 + var_5) : 1)) : var_9)))));
    #pragma endscop
}
