/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 234110
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=234110 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/234110
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 &= ((var_8 ? ((1486 ? 3642095369050428229 : 117)) : var_10));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_16 = min(var_14, (arr_0 [i_1]));
                    arr_12 [i_0] [i_0] [i_1] = ((!((((arr_11 [i_0 - 1] [11]) ? (arr_11 [i_0 + 2] [i_2]) : (arr_11 [i_0 - 1] [i_1]))))));
                    arr_13 [i_2] [i_1] [i_2] |= (min(-5678, 893549419));
                    var_17 = 57344;
                }
            }
        }
    }
    var_18 = (min(var_7, var_13));
    #pragma endscop
}
