/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104201
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104201 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104201
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (unsigned short i_0 = 0; i_0 < 20; i_0 += 4) 
    {
        var_15 = ((/* implicit */_Bool) arr_0 [i_0]);
        arr_2 [(unsigned short)9] = ((/* implicit */_Bool) arr_0 [(unsigned char)5]);
        var_16 = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) (((!(((/* implicit */_Bool) arr_0 [i_0])))) ? (arr_0 [i_0]) : (((/* implicit */long long int) ((arr_1 [i_0]) >> (((arr_1 [i_0]) - (646853393)))))))))));
    }
    /* vectorizable */
    for (long long int i_1 = 0; i_1 < 15; i_1 += 3) 
    {
        var_17 = ((/* implicit */signed char) arr_3 [i_1] [i_1]);
        var_18 = ((/* implicit */unsigned char) arr_1 [(unsigned char)7]);
    }
    var_19 = ((/* implicit */int) min((var_19), (((/* implicit */int) (-(var_10))))));
    var_20 = ((/* implicit */long long int) (+((+(((((/* implicit */_Bool) var_0)) ? (((/* implicit */int) var_8)) : (((/* implicit */int) var_13))))))));
}
