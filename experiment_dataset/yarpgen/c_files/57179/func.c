/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 57179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=57179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/57179
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (signed char i_1 = 0; i_1 < 25; i_1 += 1) 
        {
            {
                arr_6 [i_0] [16] = ((/* implicit */signed char) ((((/* implicit */int) arr_0 [i_1])) < (((/* implicit */int) ((((/* implicit */_Bool) max(((unsigned char)37), (((/* implicit */unsigned char) arr_2 [(_Bool)1]))))) || (((/* implicit */_Bool) ((unsigned int) (_Bool)1))))))));
                arr_7 [i_0] [6] [i_1] |= ((/* implicit */signed char) (!((!(((/* implicit */_Bool) ((int) var_13)))))));
            }
        } 
    } 
    var_14 = ((/* implicit */_Bool) var_13);
}
