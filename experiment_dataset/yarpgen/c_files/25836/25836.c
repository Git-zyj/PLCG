/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25836
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25836 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25836
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_20 += (((2024867130 >= var_13) & (max(((min(var_12, var_15))), 46))));
                    var_21 = (max((max(0, ((32767 ? (arr_5 [i_2] [i_1] [i_2]) : (arr_6 [0] [i_0 + 1] [i_0]))))), ((-(arr_6 [i_0] [i_1] [i_0])))));
                    var_22 *= (((max(var_12, var_9)) > (arr_6 [i_0 + 1] [i_2] [i_2])));
                    var_23 = var_6;
                }
            }
        }
    }
    var_24 = var_9;
    #pragma endscop
}
