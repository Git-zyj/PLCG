/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 6;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_11 ^= ((((max(15946489929771515415, ((min(var_0, var_7)))))) ? ((var_9 & ((min(var_3, var_6))))) : (((127 && (var_6 / var_6))))));
                    arr_7 [i_0] [i_1 - 2] [2] |= (max(1, 4294967283));
                }
            }
        }
    }
    var_12 = (((min(var_10, (max(var_8, 9714796689588197520)))) >= (((max((!var_2), (!var_4)))))));
    #pragma endscop
}
