/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115607
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned char) (~(((/* implicit */int) var_2))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 1; i_0 < 13; i_0 += 4) 
    {
        var_19 = ((/* implicit */unsigned long long int) ((unsigned short) (signed char)127));
        /* LoopSeq 1 */
        for (unsigned char i_1 = 2; i_1 < 13; i_1 += 3) 
        {
            var_20 = ((signed char) ((long long int) (unsigned short)34008));
            var_21 = ((/* implicit */signed char) ((((/* implicit */int) arr_1 [i_0 + 1] [i_1 - 2])) << (((((/* implicit */int) arr_1 [i_0 - 1] [i_1 - 1])) - (36)))));
        }
        var_22 = ((/* implicit */long long int) ((short) arr_1 [i_0] [i_0]));
        var_23 = ((/* implicit */signed char) (unsigned short)1023);
    }
}
