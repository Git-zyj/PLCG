/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190756
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190756 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190756
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 14; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 17; i_1 += 1) 
        {
            {
                var_13 = ((/* implicit */short) min((((/* implicit */unsigned short) (signed char)64)), ((unsigned short)64037)));
                var_14 = ((/* implicit */_Bool) max((var_14), (var_4)));
            }
        } 
    } 
    var_15 = ((/* implicit */short) ((signed char) var_2));
    var_16 = ((/* implicit */_Bool) max((((/* implicit */int) (unsigned short)57628)), (var_3)));
    var_17 = ((/* implicit */signed char) (~((+(((/* implicit */int) (unsigned short)7907))))));
}
