/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122774
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122774 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122774
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 17;i_2 += 1) /* same iter space */
                {
                    var_12 *= ((((~var_4) == (arr_1 [i_0 + 1] [i_0 - 1]))));
                    var_13 = min(((((((844091625 ? var_10 : var_11))) || (((-2 ? -31811 : 844091625)))))), var_5);
                }
                for (int i_3 = 0; i_3 < 17;i_3 += 1) /* same iter space */
                {
                    var_14 = (min(((844091614 ? var_8 : (arr_3 [i_1]))), (arr_0 [i_3] [i_0 + 1])));
                    arr_10 [i_0] [i_0 - 1] [i_1] [i_0 - 1] = (((arr_2 [i_0 - 2] [i_0 - 1]) | var_3));
                }
                var_15 = (arr_7 [i_0 - 1] [i_1] [i_1] [i_0]);
                var_16 = -675754756;
            }
        }
    }
    var_17 = var_4;
    #pragma endscop
}
