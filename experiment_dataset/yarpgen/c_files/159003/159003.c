/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159003
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159003 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159003
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((-(((!((min(var_4, var_6))))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = ((((var_2 < var_9) | var_13)) == ((((arr_3 [i_1] [i_0 + 3]) >= var_9))));
                var_17 = (max((~var_6), ((var_10 << (((var_7 + 87) - 22))))));
                var_18 = (((2147483647 ? 413591208888163414 : 18033152864821388201)));
                var_19 = var_3;
                var_20 = (var_7 != var_14);
            }
        }
    }
    var_21 = -var_4;
    #pragma endscop
}
