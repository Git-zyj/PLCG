/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56987
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56987 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56987
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_5 ? var_1 : (1 - 60403)));
    var_11 = (min((1 % 60403), (((-2147483647 - 1) ? (208 == var_4) : var_1))));
    var_12 &= (1 ? (((((~1) + 2147483647)) << ((((((var_3 ? -1 : var_2)) + 21)) - 20)))) : (((((1 - var_5) + 2147483647)) << (var_8 - 3519400550))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                {

                    for (int i_3 = 2; i_3 < 14;i_3 += 1)
                    {
                        arr_14 [i_0 + 1] [i_1] [i_2] [i_3] [i_3] [i_1] = (arr_2 [i_0 + 2]);
                        var_13 = (min(var_13, var_7));
                    }
                    var_14 = (2147483647 | 1610079330);
                }
            }
        }
    }
    #pragma endscop
}
