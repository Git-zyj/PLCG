/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172662
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172662 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172662
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 -= var_18;
    var_21 = (max(var_21, var_6));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_22 = (((((arr_6 [i_1 - 1] [i_1] [i_1 - 1]) / (arr_4 [i_1 + 1] [i_1 + 1] [1]))) * (max(-1, var_2))));
                    var_23 ^= (max(((((arr_3 [i_1 - 1] [i_1] [i_2]) ? var_6 : 133))), (12628 * 0)));
                    var_24 = (((((21 ? (((var_4 && (arr_2 [i_1])))) : ((var_15 ? var_4 : var_13))))) ? (17261 || -123) : (max((((68169720922112 <= (arr_2 [i_0])))), var_12))));
                }
            }
        }
    }
    #pragma endscop
}
