/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7086
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7086 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7086
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, (((((((-32763 | var_2) <= var_3))) <= var_13)))));
    var_18 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_10 [i_1] [0] [i_1] [i_1] = arr_9 [i_0] [i_0] [i_0];
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_19 *= (max((min(777384900845072319, (arr_5 [i_3 - 1] [i_3 - 1] [i_3]))), ((~(max(var_5, 17669359172864479273))))));
                                var_20 = (-((var_1 ? var_5 : (arr_0 [i_3 - 1]))));
                                var_21 = -122;
                            }
                        }
                    }
                    var_22 *= ((var_14 || ((((((var_3 ? var_9 : 62))) ? (arr_4 [i_0] [i_0] [i_2]) : (((arr_0 [i_2]) & (arr_3 [i_1]))))))));
                    var_23 = ((((arr_1 [i_1]) > 128)));
                }
            }
        }
    }
    #pragma endscop
}
