/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247136
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247136 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247136
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_7;
    var_21 = ((((((var_5 ? var_17 : var_8)))) ? var_13 : ((((var_19 - var_7) ? var_7 : var_12)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_22 ^= ((-((((arr_1 [i_0]) ? (arr_3 [i_0] [i_1]) : var_13)))));
                var_23 = max(((((arr_3 [i_0] [i_0]) <= (((arr_0 [i_0] [i_0]) ? var_3 : (arr_0 [i_0] [i_1])))))), (max(var_14, (~var_4))));
                var_24 = (~var_18);
                var_25 -= (~1);
            }
        }
    }
    #pragma endscop
}
