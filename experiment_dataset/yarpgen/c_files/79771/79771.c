/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_2 - 1] [i_1] [i_1] [i_0] = ((((~(!var_9))) + (((arr_5 [i_0]) ? (arr_3 [i_2] [i_1]) : 57))));
                    var_11 *= (min(var_3, 189));
                    arr_10 [i_0] [i_0 + 2] [i_1] [i_2 - 2] = (((((88 == -9223372036854775796) > (~0))) ? (arr_4 [i_0 + 4]) : (2 || 4294967270)));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 3; i_3 < 14;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            {
                arr_16 [i_4] = (~178);
                arr_17 [i_3] [i_4] = ((!((max(32767, 75)))));
            }
        }
    }
    #pragma endscop
}
