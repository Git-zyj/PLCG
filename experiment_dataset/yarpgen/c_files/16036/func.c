/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16036
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16036 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16036
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((((/* implicit */_Bool) var_1)) ? (10662901386993627873ULL) : (((/* implicit */unsigned long long int) 2073213737)));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 22; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (unsigned short i_2 = 1; i_2 < 21; i_2 += 2) 
            {
                {
                    arr_7 [i_0] [i_0] [i_2] [18LL] = ((/* implicit */unsigned short) (-(((/* implicit */int) ((_Bool) 8272497671823883852ULL)))));
                    var_16 = ((/* implicit */int) ((3852326853U) ^ (((/* implicit */unsigned int) ((/* implicit */int) var_9)))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */int) (~(var_11)));
}
