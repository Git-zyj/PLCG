/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144100
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144100 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144100
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) (!(((/* implicit */_Bool) -2124313076))));
    /* LoopNest 2 */
    for (signed char i_0 = 0; i_0 < 21; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 17; i_1 += 3) 
        {
            {
                var_21 = ((/* implicit */short) min((var_21), (((/* implicit */short) (signed char)105))));
                arr_7 [(signed char)3] [i_0] [14ULL] = ((/* implicit */signed char) arr_1 [i_0]);
            }
        } 
    } 
}
