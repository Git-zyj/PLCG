/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200882
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200882 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200882
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_8;
    var_14 = ((((var_9 & (var_8 == var_4))) * (((205 == (min(var_1, 65515)))))));
    var_15 += ((var_6 ? ((((var_11 + 2147483647) << (((max(var_11, var_3)) - 4294967187))))) : (max(((var_1 ? var_0 : var_9)), (max(4008682289, 13554998292328582611))))));

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        arr_2 [i_0] [i_0] &= arr_0 [i_0];
        var_16 &= ((((~0) ? ((8388608 / (arr_1 [i_0] [i_0]))) : (((18465 ? 40900 : 0))))));
    }
    #pragma endscop
}
