/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177846
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177846 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177846
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                var_20 = var_1;
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 22;i_3 += 1)
                    {
                        {
                            var_21 = (min(var_21, ((max(-5, 1)))));
                            var_22 *= (!-var_15);
                            arr_9 [i_0] [i_1 - 1] [i_0] [i_3 - 2] = (arr_4 [i_1] [i_1] [i_1 - 2]);
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 20;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 23;i_5 += 1)
                    {
                        {
                            var_23 = (max(var_23, var_6));
                            var_24 = ((-var_4 ? (((-(min(-2147483640, var_14))))) : (min(var_11, (arr_14 [i_1 + 1] [i_4 + 2])))));
                            var_25 += -2032234912;
                        }
                    }
                }
            }
        }
    }
    var_26 = (-(min((max(var_3, 0)), var_13)));
    var_27 = var_18;
    #pragma endscop
}
