/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236443
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236443 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236443
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min((var_2 || var_0), (!var_15)));
    var_19 *= 60458;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {
                var_20 = 0;
                arr_4 [i_1] [i_1] = ((+(((-5932863717142476320 + 9223372036854775807) << (var_4 - 3749391988323975802)))));
                var_21 = (max(var_21, ((min((((arr_1 [i_0]) ? (arr_2 [2] [i_1]) : var_12)), -var_3)))));
            }
        }
    }
    var_22 = (max(var_22, var_8));
    var_23 = var_8;
    #pragma endscop
}
