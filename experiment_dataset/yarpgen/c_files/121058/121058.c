/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121058
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121058 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121058
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 54;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = ((var_6 ? ((-54073 ? (min(var_18, 9410262869452468020)) : (((var_11 ? (arr_3 [i_0] [6]) : (arr_0 [i_1])))))) : ((((arr_0 [i_1]) ? (((arr_1 [i_1]) / var_17)) : ((max((arr_1 [i_0]), var_15))))))));
                var_20 *= ((+((max((arr_1 [i_1]), var_17)))));
                arr_5 [i_0] = -16;
            }
        }
    }
    #pragma endscop
}
