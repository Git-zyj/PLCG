/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202790
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202790 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202790
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 ^= ((/* implicit */unsigned int) var_2);
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned short i_0 = 4; i_0 < 11; i_0 += 4) 
    {
        var_15 += ((/* implicit */unsigned int) (-(((/* implicit */int) var_8))));
        arr_3 [i_0 + 1] = ((/* implicit */unsigned char) ((((/* implicit */int) var_8)) >> (((((/* implicit */int) (signed char)-8)) + (20)))));
    }
    var_16 = ((/* implicit */int) (unsigned char)66);
    var_17 = ((/* implicit */short) 2059289058);
}
