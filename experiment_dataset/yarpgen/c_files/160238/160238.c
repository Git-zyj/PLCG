/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160238
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_0;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_0] [7] [i_2] = 1;
                    arr_8 [i_0] [i_0] [i_2 + 1] [i_2] = var_3;
                    var_20 = ((18318 ? -32005 : 562140057));
                    var_21 = (min((((arr_5 [14] [i_0 + 3] [i_2 - 1]) + ((var_8 ? var_15 : (arr_2 [i_0] [i_0 + 3] [i_0 + 2]))))), (((562140068 ? (((arr_0 [i_0]) ? var_11 : 562140050)) : 32249)))));
                }
            }
        }
    }
    #pragma endscop
}
