/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146098
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146098 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146098
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_12 = (max(var_12, ((max((((var_0 == (arr_0 [i_0])))), (max((arr_1 [i_0] [i_0]), (arr_2 [i_0] [i_0]))))))));
        var_13 = (max(-430492649, (~-7)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] = (!var_11);
                    var_14 = var_7;
                    var_15 = ((max(((var_11 | (arr_2 [i_1] [i_0])), ((max(var_4, var_0)))))));
                }
            }
        }
    }
    var_16 -= ((~((((var_0 == var_0) == var_7)))));
    var_17 ^= 218;
    var_18 = var_4;
    #pragma endscop
}
