/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149862
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149862 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149862
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                var_13 = (max(((max((!3184517506146699726), var_5))), (((arr_5 [i_0 - 2] [i_1 - 2] [i_1]) ? (arr_5 [i_0 - 3] [i_1 - 1] [i_1]) : (arr_5 [i_0 - 2] [i_1 + 1] [i_1])))));
                var_14 = (min(((((arr_3 [i_1] [i_1]) < (arr_3 [i_1] [i_1])))), ((((2047 ? 1672010367276262122 : 16774733706433289498)) | (-12570 && var_9)))));
            }
        }
    }
    var_15 = var_10;
    var_16 = var_8;
    var_17 += (!var_11);
    var_18 = var_4;
    #pragma endscop
}
