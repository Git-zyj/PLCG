/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157408
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157408 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157408
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 14; i_0 += 4) 
    {
        for (unsigned short i_1 = 2; i_1 < 13; i_1 += 2) 
        {
            for (unsigned char i_2 = 0; i_2 < 15; i_2 += 3) 
            {
                {
                    arr_8 [i_2] [i_0] [i_2] [i_2] = ((/* implicit */short) 13468718721292759765ULL);
                    arr_9 [i_0] [i_2] = ((/* implicit */int) arr_4 [i_0] [i_0 - 1]);
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned long long int) ((unsigned char) (unsigned char)28));
    var_15 = ((/* implicit */int) 2468319317058772081ULL);
}
