/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26815
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((((/* implicit */int) var_2)) != (((/* implicit */int) ((var_1) != (((/* implicit */long long int) (+(((/* implicit */int) var_9)))))))));
    /* LoopNest 3 */
    for (signed char i_0 = 2; i_0 < 10; i_0 += 2) 
    {
        for (int i_1 = 2; i_1 < 10; i_1 += 3) 
        {
            for (long long int i_2 = 1; i_2 < 10; i_2 += 1) 
            {
                {
                    var_16 &= ((/* implicit */long long int) arr_1 [i_0] [i_0]);
                    var_17 -= ((/* implicit */signed char) ((((((/* implicit */int) (!(((/* implicit */_Bool) arr_4 [i_0] [i_1] [i_2]))))) != (((/* implicit */int) (!(((/* implicit */_Bool) -9223372036854775803LL))))))) ? ((+(min((var_1), (((/* implicit */long long int) var_8)))))) : (((/* implicit */long long int) ((/* implicit */int) var_7)))));
                }
            } 
        } 
    } 
}
