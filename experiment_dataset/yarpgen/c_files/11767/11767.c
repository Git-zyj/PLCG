/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11767
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11767 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11767
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    arr_6 [11] [11] = ((-((min(4279, 1)))));
                    arr_7 [i_1] = min(31527, ((((((3638061420 << (109 - 94)))) < (max(17500672362983362625, -31528))))));
                    arr_8 [i_2] [15] [i_0] [i_0] = 31527;
                }
            }
        }
    }
    var_14 = (min(var_14, (((-1985427924 & (min(19818, 6)))))));
    #pragma endscop
}
