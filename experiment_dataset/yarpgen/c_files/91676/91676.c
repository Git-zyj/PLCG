/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91676
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91676 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91676
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 18896;
    var_12 &= ((max(18886, -18916)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [18] [18] [i_1] = ((-(((7076932028839682840 + 128) ? 7097 : (-32767 - 1)))));
                var_13 = (max(var_13, (((((58 ? -81 : (arr_3 [i_0] [i_0]))))))));
                arr_6 [i_1] = ((((min(0, -81))) ? (arr_2 [i_1] [i_1] [i_1]) : (((arr_1 [i_0]) ? (arr_2 [i_0] [i_0] [i_0]) : (arr_2 [0] [7] [7])))));
            }
        }
    }
    var_14 = var_7;
    #pragma endscop
}
