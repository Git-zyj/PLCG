/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222118
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222118 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222118
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] = max(((var_11 ? (arr_3 [i_0 + 1] [i_1]) : var_13)), ((((min(15076871750537454614, (arr_1 [i_0])))) ? var_2 : var_9)));
                arr_5 [i_0] [i_1] = (max((arr_0 [i_0 - 1] [i_1]), ((min((arr_1 [i_0 + 1]), (((arr_3 [i_0 + 1] [i_1]) * 20382)))))));
            }
        }
    }
    var_16 = var_10;
    var_17 = var_2;
    #pragma endscop
}
