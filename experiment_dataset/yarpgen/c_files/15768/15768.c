/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15768
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 ^= (var_2 && var_17);
    var_20 = -var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_21 = var_0;
                var_22 = (arr_5 [i_0]);

                for (int i_2 = 1; i_2 < 13;i_2 += 1) /* same iter space */
                {
                    arr_8 [i_2] [i_1] [i_2] [1] = (arr_6 [i_1]);
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 16;i_4 += 1)
                        {
                            {
                                var_23 += (arr_6 [i_3 - 1]);
                                var_24 = ((((arr_4 [i_0] [i_1] [i_3 + 1]) | (((((arr_12 [11] [11] [i_2 + 1] [1] [i_2 - 1]) != var_3)))))) != (((((arr_3 [i_1] [0] [i_1]) & var_15)) | (arr_4 [i_2] [i_2] [4]))));
                                var_25 = (min(var_25, (((((arr_0 [14]) - ((-(arr_12 [i_0] [i_0] [i_2] [i_3 - 1] [i_4])))))))));
                            }
                        }
                    }
                }
                for (int i_5 = 1; i_5 < 13;i_5 += 1) /* same iter space */
                {
                    var_26 = (((arr_0 [i_5 - 1]) && (arr_0 [i_5 - 1])));
                    var_27 *= (!(((-(arr_5 [12])))));
                }
                /* vectorizable */
                for (int i_6 = 1; i_6 < 13;i_6 += 1) /* same iter space */
                {
                    var_28 = (min(var_28, var_1));
                    var_29 = ((+((((arr_0 [i_0]) < (arr_0 [7]))))));
                    /* LoopNest 2 */
                    for (int i_7 = 0; i_7 < 16;i_7 += 1)
                    {
                        for (int i_8 = 0; i_8 < 1;i_8 += 1)
                        {
                            {
                                var_30 += (((arr_16 [i_7] [i_8]) <= var_11));
                                arr_24 [i_8] [i_7] [i_6] [i_1] [i_0] = var_15;
                                arr_25 [i_0] [i_7] [8] [i_1] [i_0] = (arr_3 [i_1] [i_6 - 1] [i_8]);
                                var_31 = (arr_23 [i_0] [i_1] [i_6 - 1] [i_7] [i_7] [i_8] [i_8]);
                                var_32 = arr_12 [i_0] [i_1] [i_6 + 1] [i_7] [i_0];
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
