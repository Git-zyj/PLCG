/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244149
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244149 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244149
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((((((var_10 ? 26 : var_6)) * var_14))) ? var_5 : var_1));
    var_21 = (max(var_21, var_6));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_22 = ((((!(~1117103813820416))) ? (arr_2 [8]) : 13682259224013560535));
                    arr_10 [i_1] [i_1] [i_1] [i_0] = ((~(var_1 / 35759518)));
                    arr_11 [i_0] [1] [i_1] = ((((!(arr_6 [i_0]))) || (arr_2 [i_1])));
                }
            }
        }
    }
    var_23 = (max(var_23, var_15));
    var_24 = 1058180858;
    #pragma endscop
}
