/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21921
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_10;
    var_20 = ((-(2147483647 && var_13)));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_21 = (min(((+((max((arr_0 [i_0]), (arr_0 [i_0])))))), (arr_1 [i_0] [i_0])));
        var_22 = ((max(((var_16 << (((arr_1 [i_0] [i_0]) - 64)))), var_18)));
        arr_2 [i_0] = (min((arr_0 [1]), var_13));
        var_23 = ((max(((var_18 ? 2147483647 : var_8)), ((max(var_3, (arr_0 [i_0])))))));
    }
    #pragma endscop
}
