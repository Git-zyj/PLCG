/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143778
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143778 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143778
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_14 &= ((-(arr_2 [i_0 + 2])));
                    var_15 = ((+(((arr_4 [i_0 - 2] [i_0 + 1] [i_0 + 1]) ? (arr_6 [i_0 + 3]) : (1307932188 + 33554)))));
                }
            }
        }
    }
    var_16 = (min(var_2, ((((min(var_6, 87))) ? (((var_4 ? 241307925 : 16631))) : var_1))));
    var_17 = (min(var_17, (((((var_3 - 41) ? var_5 : var_5))))));
    var_18 = 48904;
    #pragma endscop
}
