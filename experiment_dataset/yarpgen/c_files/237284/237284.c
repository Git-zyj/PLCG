/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237284
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237284 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237284
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (min((((min(var_7, var_9)))), (!var_5)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (((242 ? 281337537757184 : var_12)));
                arr_6 [i_0] [i_0] = ((~(max(-8471076848050199960, ((max(var_12, (arr_2 [i_0] [i_0]))))))));
                var_16 ^= ((~(max((!1), (arr_1 [6])))));
            }
        }
    }
    #pragma endscop
}
