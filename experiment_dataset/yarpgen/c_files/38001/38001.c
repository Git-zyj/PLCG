/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 38001
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=38001 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/38001
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max((~var_14), ((max(2, var_9)))));

    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        var_18 = (arr_0 [i_0]);
        var_19 = (((((((-(arr_1 [i_0] [i_0])))) ? var_12 : (arr_0 [i_0]))) != (((arr_0 [i_0]) ? ((var_7 ? 16 : 16)) : ((max((arr_0 [i_0]), var_9)))))));
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 11;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 11;i_2 += 1)
        {
            {
                var_20 = ((((max(4120400512653801878, (arr_4 [i_2] [i_1])))) ? ((((arr_4 [i_1] [i_2]) >= var_0))) : (arr_4 [i_1] [i_2])));
                var_21 = (min(var_9, (arr_5 [i_1])));
            }
        }
    }
    #pragma endscop
}
