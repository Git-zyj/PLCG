/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = (min(var_17, ((((arr_4 [i_0 - 1] [i_0 - 1]) ? (arr_4 [i_0 - 1] [i_1]) : ((~((var_13 ? 21357 : (arr_3 [i_0]))))))))));
                var_18 = ((-(((arr_3 [i_0]) ? var_6 : (arr_0 [i_0] [i_1])))));
                arr_6 [i_1] [i_0] [i_0 - 1] = var_15;
                var_19 = (!var_15);
            }
        }
    }
    var_20 = 12004029465189853158;
    var_21 = var_5;
    #pragma endscop
}
