/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156192
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156192 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156192
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */signed char) ((var_6) * ((+((+(((/* implicit */int) var_8))))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 4; i_0 < 21; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 21; i_1 += 4) 
        {
            {
                arr_5 [i_0] [i_1 - 2] [i_0 - 3] = ((/* implicit */unsigned short) min((arr_0 [i_0 - 1]), (((/* implicit */unsigned int) ((int) (unsigned short)35884)))));
                /* LoopNest 2 */
                for (unsigned char i_2 = 0; i_2 < 22; i_2 += 3) 
                {
                    for (unsigned int i_3 = 3; i_3 < 21; i_3 += 2) 
                    {
                        {
                            var_12 = ((/* implicit */_Bool) max((var_12), (((/* implicit */_Bool) arr_0 [i_2]))));
                            var_13 = ((/* implicit */unsigned char) (signed char)51);
                            var_14 = arr_6 [i_1] [i_1] [i_1] [i_0 - 4];
                            arr_10 [i_0] |= ((/* implicit */unsigned int) (signed char)-26);
                            var_15 -= ((/* implicit */unsigned int) (short)511);
                        }
                    } 
                } 
                var_16 = ((/* implicit */unsigned short) arr_1 [i_1]);
            }
        } 
    } 
}
