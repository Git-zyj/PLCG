/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214282
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214282 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214282
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_16 = var_8;
        arr_2 [i_0] = ((((211956772 ? (arr_1 [i_0] [i_0]) : (arr_1 [i_0] [i_0]))) | (((((arr_1 [i_0] [i_0]) <= (arr_1 [i_0] [i_0])))))));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        arr_6 [i_1] = ((arr_5 [i_1]) - (var_12 == 7572130891067637689));
        arr_7 [i_1] = var_6;
    }
    var_17 = ((((((var_10 ? var_14 : var_12)) << (var_7 & -7772))) + var_2));
    var_18 = (max(8191, var_15));
    #pragma endscop
}
