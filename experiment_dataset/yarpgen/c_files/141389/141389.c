/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141389
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141389 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141389
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 = (max(var_15, var_1));
                    var_16 = var_8;
                    arr_6 [i_0] [i_2] [i_1 - 1] [i_2] = (min(var_4, (arr_4 [i_0] [i_0] [i_0] [i_2])));
                    arr_7 [i_2] [i_2] [i_0] = (((((1 ? (arr_4 [i_0] [i_0] [i_0] [i_0]) : ((-27931 ? (arr_3 [i_0]) : var_13))))) ? (max((1 + 36355), var_10)) : ((max(1, (min(var_14, 49279)))))));
                    arr_8 [i_2] [i_2] [i_2] [i_0] = ((40950 ? -1 : 1));
                }
            }
        }
    }
    var_17 = var_13;
    var_18 = ((var_9 >> (((min((~var_2), 27936)) + 8))));
    #pragma endscop
}
