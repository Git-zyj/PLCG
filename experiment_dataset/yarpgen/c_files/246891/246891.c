/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246891
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246891 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246891
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_16 -= ((-(max((var_6 % var_15), var_11))));
        arr_2 [i_0] = (-238430355 - 511);
        arr_3 [i_0] [8] = ((min(var_6, var_12)));
    }
    /* LoopNest 2 */
    for (int i_1 = 3; i_1 < 9;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            {
                var_17 = ((((!((min(var_3, var_3))))) ? (((var_3 ? var_11 : var_10))) : ((((var_15 != var_0) && (((var_2 ? var_14 : var_4))))))));
                var_18 = ((!((((56 ? 56 : 1))))));
                var_19 -= (!var_3);
            }
        }
    }
    var_20 = (((max((max(83, 65488)), (max(1023, 38))))) < 38);
    #pragma endscop
}
