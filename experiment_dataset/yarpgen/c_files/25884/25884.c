/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25884
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25884 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25884
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 = (max((max(((((arr_4 [2] [i_1] [i_1] [8]) || (arr_1 [i_0])))), 18446744073709551615)), ((((~var_2) ? var_2 : (min((arr_1 [i_0]), var_6)))))));
                    var_13 = (min(var_13, (arr_2 [i_0 + 3] [1] [i_2])));
                }
            }
        }
    }
    var_14 *= ((((((var_0 ? var_4 : var_11)) >= var_7))) < -114);
    #pragma endscop
}
