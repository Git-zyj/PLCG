/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162207
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162207 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162207
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, (~var_10)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_1] = ((~(arr_4 [i_0] [i_1])));
                var_12 = (max(var_12, (((((arr_2 [8] [8]) >= (arr_1 [i_0]))) ? var_9 : ((~(max(8551918860086148395, 6))))))));
                arr_7 [i_1] [i_1] [i_1] = (~(((arr_3 [i_0] [i_1] [i_0]) ? (((min(var_4, 4395)))) : (arr_4 [i_0] [i_0]))));
            }
        }
    }
    var_13 = 2;
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 21;i_2 += 1)
    {
        for (int i_3 = 2; i_3 < 18;i_3 += 1)
        {
            {
                var_14 = (min(((-(arr_10 [i_3]))), var_10));
                var_15 = ((~((((arr_10 [i_2]) && var_2)))));
                var_16 -= var_9;
            }
        }
    }

    /* vectorizable */
    for (int i_4 = 0; i_4 < 18;i_4 += 1)
    {
        var_17 += var_0;
        arr_15 [i_4] = var_4;
        var_18 *= ((-(((arr_9 [i_4]) ? (arr_11 [i_4]) : 0))));
    }
    #pragma endscop
}
