/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21375
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21375 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21375
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_8 [i_2] [i_2] [i_0] [i_2] = (min((((!(!-8952333927100595480)))), (arr_2 [1] [i_1])));
                    arr_9 [i_1] [i_1] [i_0] = ((8952333927100595460 ? 288230376151709696 : 8952333927100595479));
                }
            }
        }
    }
    var_18 = ((var_7 > (var_6 - var_10)));
    #pragma endscop
}
