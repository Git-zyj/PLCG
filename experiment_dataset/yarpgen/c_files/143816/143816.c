/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 |= (var_15 >= 93);

    /* vectorizable */
    for (int i_0 = 2; i_0 < 13;i_0 += 1) /* same iter space */
    {
        var_20 ^= var_13;
        arr_2 [i_0 - 1] = (((((-1 & (arr_1 [i_0] [i_0])))) ? var_1 : ((var_7 ? var_15 : 16777216))));
    }
    /* vectorizable */
    for (int i_1 = 2; i_1 < 13;i_1 += 1) /* same iter space */
    {
        var_21 &= (((arr_4 [i_1 - 2] [i_1]) ? (((arr_4 [i_1] [i_1]) - 0)) : (((var_16 ? (arr_3 [1]) : (arr_3 [i_1 + 1]))))));

        for (int i_2 = 0; i_2 < 14;i_2 += 1)
        {
            var_22 = (min(var_22, var_17));
            arr_8 [i_1] [i_1] = (!3120591572568540195);
        }
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            /* LoopNest 3 */
            for (int i_4 = 1; i_4 < 12;i_4 += 1)
            {
                for (int i_5 = 0; i_5 < 14;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 14;i_6 += 1)
                    {
                        {
                            var_23 = (arr_0 [i_1 + 1]);
                            var_24 = -var_3;
                            var_25 |= ((!(((-16777197 ? (arr_0 [i_4 + 1]) : (arr_4 [i_3] [i_5]))))));
                            arr_19 [i_1] [i_1] [i_4] [i_1] [i_6] |= (!38689);
                            var_26 = ((11530338278394837716 ? -1679853961 : 255));
                        }
                    }
                }
            }
            arr_20 [i_3] [i_3] [i_3] = -1;
        }
        arr_21 [i_1] [i_1] = var_2;
        arr_22 [i_1] = 18446744073709551600;
    }
    #pragma endscop
}
