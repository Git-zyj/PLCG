/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156660
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156660 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156660
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_20 = (min(var_20, (arr_3 [i_0 - 2] [i_0 + 1])));
                                var_21 = (2771 & ((-(!var_1))));
                                var_22 *= var_4;
                            }
                        }
                    }
                    var_23 = (max(var_23, (arr_5 [i_0 + 2] [i_0 + 1] [i_0 + 1] [i_2 + 1])));
                }
            }
        }
    }
    var_24 = (max((((var_7 / -1780263826) ? var_5 : var_1)), var_0));
    var_25 = (max(var_25, ((~(((var_14 - var_16) ^ var_15))))));
    #pragma endscop
}
