/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143412
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143412 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143412
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = var_14;
    /* LoopNest 2 */
    for (unsigned short i_0 = 2; i_0 < 20; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 22; i_1 += 3) 
        {
            {
                arr_6 [i_0 - 2] [12LL] [i_0 - 2] = ((/* implicit */short) ((((/* implicit */int) (unsigned char)18)) > (((/* implicit */int) (unsigned char)13))));
                var_21 -= ((/* implicit */int) (!(((/* implicit */_Bool) 18446744073709551605ULL))));
            }
        } 
    } 
}
