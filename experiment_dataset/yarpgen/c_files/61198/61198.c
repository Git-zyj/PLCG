/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61198
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61198 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61198
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_1] [i_0] = (((((+((62 ? (arr_7 [i_0] [i_0] [3]) : (arr_5 [11] [i_1] [10] [10])))))) ? ((((arr_3 [i_0] [i_2]) < (arr_3 [i_1] [i_0])))) : (((!(arr_0 [i_1]))))));
                    var_12 = (((((~(arr_5 [i_2] [i_1] [i_2] [i_1])))) ? ((40455 >> (((arr_5 [i_0] [i_1] [i_1] [i_2]) + 959860934)))) : ((204 << (((arr_5 [i_0] [i_2] [i_0] [i_0]) + 959860938))))));
                }
            }
        }
    }
    #pragma endscop
}
