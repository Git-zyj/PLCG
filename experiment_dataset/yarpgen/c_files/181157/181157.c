/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181157
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181157 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181157
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = (min(var_16, (((max(2773760157, -727026790))))));
                arr_5 [i_0] [i_0] = max(((((arr_0 [i_0 + 1]) ? (arr_0 [i_0 + 1]) : (arr_0 [i_0 + 1])))), ((18446744073709551615 << (9223372036854775786 - 9223372036854775774))));
            }
        }
    }
    var_17 = var_4;
    #pragma endscop
}
