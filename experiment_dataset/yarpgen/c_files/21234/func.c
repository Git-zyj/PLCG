/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21234
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21234 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21234
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned char i_1 = 0; i_1 < 19; i_1 += 2) 
        {
            {
                arr_5 [i_1] [i_1] [i_1] = ((/* implicit */unsigned char) (-((~((~(-795769335)))))));
                var_15 = (+((+((-(((/* implicit */int) var_13)))))));
                arr_6 [i_0] [i_0] [i_0] = ((/* implicit */unsigned long long int) (~((-((+(((/* implicit */int) var_14))))))));
                var_16 = ((/* implicit */unsigned short) max((var_16), (((/* implicit */unsigned short) (+((+((-(((/* implicit */int) var_13)))))))))));
            }
        } 
    } 
    var_17 = ((/* implicit */signed char) (~((-((+(((/* implicit */int) var_5))))))));
}
