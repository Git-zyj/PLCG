/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193631
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193631 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193631
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 *= ((/* implicit */unsigned short) (!(((/* implicit */_Bool) var_8))));
    /* LoopSeq 1 */
    for (long long int i_0 = 3; i_0 < 11; i_0 += 1) 
    {
        arr_2 [i_0] [i_0] = ((unsigned short) ((((/* implicit */unsigned int) arr_1 [i_0])) == (4294967273U)));
        var_16 = ((/* implicit */signed char) (~(((/* implicit */int) (!(((/* implicit */_Bool) min((((/* implicit */unsigned long long int) arr_1 [i_0 + 1])), (2305843009213693952ULL)))))))));
        arr_3 [i_0] = ((/* implicit */unsigned long long int) (+(arr_0 [i_0])));
        arr_4 [i_0] [i_0] = ((/* implicit */unsigned int) 16140901064495857674ULL);
        var_17 = ((/* implicit */unsigned char) max((var_17), (((/* implicit */unsigned char) (-(((/* implicit */int) (short)4094)))))));
    }
}
