/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182522
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182522 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182522
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 -= min(0, ((var_0 >= (1 != var_10))));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_21 = (!var_19);
        var_22 = ((+((max((arr_2 [i_0] [1]), (arr_2 [i_0] [i_0]))))));
        var_23 += (((min(1, 1))) <= (((arr_1 [i_0]) ? ((max(1, var_1))) : (var_10 != 1))));
        var_24 = min(1, (arr_0 [1] [i_0]));
        arr_4 [0] [i_0] = (((arr_0 [i_0] [i_0]) ? (((!(arr_0 [i_0] [i_0])))) : (max((arr_2 [i_0] [i_0]), (0 - 1)))));
    }
    var_25 = var_6;
    #pragma endscop
}
