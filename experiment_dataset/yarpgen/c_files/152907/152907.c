/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152907
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152907 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152907
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((1 && (1 ^ -2021925540)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_11 = 1;
                    var_12 = (((-635419870 < 1) ? (arr_0 [1] [i_2]) : ((((((arr_1 [i_0]) ? var_7 : 1))) ? -2021925563 : 3249162013))));
                    var_13 = ((+(((arr_5 [i_0] [i_1 + 1] [i_2]) ? (arr_5 [9] [i_1 - 1] [i_2]) : (arr_5 [i_0] [i_1 + 2] [i_2])))));
                }
            }
        }
    }
    var_14 = min(((-((4026289895 ? 1045805261 : 268677374)))), 1);
    #pragma endscop
}
