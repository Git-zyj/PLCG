/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66826
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66826 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66826
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((var_0 ? var_2 : var_3))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    var_11 = 0;
                    arr_9 [11] [1] [i_1] = ((((+(((arr_0 [i_1]) / var_8)))) * (max(var_6, -var_6))));
                    var_12 ^= ((min(((var_2 | (arr_5 [i_2] [i_2] [i_2])), var_1))));
                    arr_10 [i_0] [i_1] [i_2] = 64;
                    var_13 = ((-(((arr_8 [i_2] [i_1]) + -var_1))));
                }
            }
        }
    }
    var_14 ^= var_9;
    var_15 ^= (max(((min(((min(var_4, 32))), (~var_6)))), var_7));
    #pragma endscop
}
