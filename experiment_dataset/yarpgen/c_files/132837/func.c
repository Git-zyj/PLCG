/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132837
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132837 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132837
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */_Bool) var_7);
    var_13 = ((/* implicit */unsigned short) ((_Bool) ((618003231649411728LL) << (((676292100998787596LL) - (676292100998787596LL))))));
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 0; i_0 += 1) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */int) var_0);
        var_14 = ((/* implicit */unsigned char) (~(((long long int) (_Bool)1))));
        arr_4 [(unsigned char)14] [(unsigned short)14] &= arr_0 [(unsigned char)5] [i_0];
        var_15 = ((/* implicit */unsigned short) -676292100998787604LL);
    }
}
