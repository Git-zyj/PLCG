/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 35616
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=35616 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/35616
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 18; i_0 += 4) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (int i_2 = 1; i_2 < 16; i_2 += 4) 
            {
                {
                    var_12 = ((/* implicit */int) 3342534889U);
                    var_13 ^= ((/* implicit */int) var_5);
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned long long int) (~(((/* implicit */int) var_11))));
    /* LoopNest 3 */
    for (unsigned char i_3 = 0; i_3 < 20; i_3 += 4) 
    {
        for (unsigned char i_4 = 0; i_4 < 20; i_4 += 1) 
        {
            for (signed char i_5 = 0; i_5 < 20; i_5 += 2) 
            {
                {
                    arr_13 [i_5] [i_5] = ((/* implicit */unsigned int) max((max((((/* implicit */int) ((unsigned short) var_2))), (((int) 952432422U)))), ((+(((/* implicit */int) var_10))))));
                    arr_14 [i_3] [7ULL] [i_5] = ((/* implicit */unsigned short) (((_Bool)0) ? (max((((/* implicit */long long int) (!(((/* implicit */_Bool) 3342534907U))))), (arr_8 [i_3] [i_3] [5]))) : (((/* implicit */long long int) ((/* implicit */int) ((((((/* implicit */_Bool) var_4)) || (((/* implicit */_Bool) (unsigned char)64)))) && (((/* implicit */_Bool) var_10))))))));
                    var_15 = ((/* implicit */int) arr_6 [i_3]);
                    var_16 ^= ((/* implicit */unsigned int) var_6);
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned char) var_11);
}
