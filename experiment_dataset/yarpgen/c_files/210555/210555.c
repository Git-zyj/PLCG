/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210555
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210555 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210555
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    var_19 = (((-(arr_1 [i_2 + 1]))));
                    arr_7 [i_0] [i_1] [7] = (arr_5 [i_2 - 1] [i_2 - 1] [i_2 - 1] [i_2]);
                    arr_8 [i_1] [1] [1] [i_1] = ((~(((5411121379167742573 && (arr_0 [i_2 - 1]))))));
                }
            }
        }
    }
    var_20 = var_4;
    var_21 = 152;
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 20;i_3 += 1)
    {
        for (int i_4 = 3; i_4 < 18;i_4 += 1)
        {
            {
                var_22 = ((arr_12 [i_3] [i_3] [6]) | (arr_11 [8]));
                var_23 &= (max((((5411121379167742545 || (arr_14 [i_3] [i_4 - 2] [i_3])))), ((~(arr_14 [i_4] [i_3] [i_3])))));
            }
        }
    }
    #pragma endscop
}
