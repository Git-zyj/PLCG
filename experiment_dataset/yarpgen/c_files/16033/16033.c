/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16033
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16033 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16033
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (((((0 ? 262144 : -262149))) ? (((1 ? -1863617301 : -8624627844128630454))) : (((((arr_1 [i_0]) | (arr_3 [i_0] [i_0 - 1] [i_1]))) | var_3))));
                arr_5 [i_0] [i_1] = ((((!(arr_2 [i_0] [i_1]))) ? ((((!((max((arr_2 [i_0] [i_1]), (arr_3 [i_0] [i_0] [i_0 + 1])))))))) : (max((((arr_0 [9]) / (arr_3 [i_1] [i_1] [i_1]))), (((112 ? 433273778 : 3346158138)))))));
            }
        }
    }
    var_10 |= var_2;
    #pragma endscop
}
