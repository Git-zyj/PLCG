/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110662
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110662 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110662
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 16; i_0 += 4) 
    {
        /* LoopSeq 1 */
        for (unsigned int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            arr_4 [i_0] [i_0] = ((/* implicit */unsigned int) (~(((/* implicit */int) (short)-16025))));
            arr_5 [i_0] [1] [1] = ((/* implicit */int) ((((/* implicit */unsigned long long int) ((((/* implicit */int) var_14)) & (((/* implicit */int) var_8))))) & ((~(arr_0 [i_0 + 1] [i_0 - 1])))));
        }
        var_16 = ((/* implicit */short) var_0);
    }
    /* LoopSeq 1 */
    for (unsigned long long int i_2 = 0; i_2 < 20; i_2 += 1) 
    {
        arr_8 [i_2] = ((/* implicit */unsigned short) var_12);
        arr_9 [i_2] = (+(((/* implicit */int) ((unsigned short) var_7))));
    }
}
