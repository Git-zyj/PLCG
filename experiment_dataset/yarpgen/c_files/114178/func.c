/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114178
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114178 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114178
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned int) var_6);
    var_11 |= ((/* implicit */unsigned short) var_2);
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        for (unsigned int i_1 = 2; i_1 < 9; i_1 += 3) 
        {
            {
                var_12 = ((/* implicit */unsigned int) ((int) (short)7015));
                var_13 = ((10215633330057407163ULL) == (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)18261))));
                arr_4 [i_0] [i_1 + 1] [i_0] = ((/* implicit */signed char) var_5);
            }
        } 
    } 
    var_14 = ((/* implicit */int) var_3);
}
