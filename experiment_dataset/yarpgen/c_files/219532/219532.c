/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219532
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219532 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219532
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((252 ? var_14 : (((139 ? var_15 : var_2)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_17 = (min(var_17, var_5));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_18 = 3;
                                arr_15 [i_0] [i_3 - 1] [i_2] [i_3 - 1] = (((arr_13 [i_0] [i_2] [i_1 - 3] [i_0] [i_0]) ? (((min(var_5, 62720)))) : (((((43430 ? 3 : var_5))) ? (((var_12 ? var_5 : (arr_9 [i_4])))) : (min(var_12, (arr_5 [i_3] [i_4])))))));
                                arr_16 [i_0] = -2147483642;
                                arr_17 [20] [i_1] [12] [0] &= arr_3 [i_1 - 3] [i_1 - 3];
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
