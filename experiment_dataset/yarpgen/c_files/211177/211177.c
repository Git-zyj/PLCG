/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211177
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211177 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211177
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 13;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = ((((min((arr_3 [i_1 + 2]), (arr_2 [i_0] [i_1 + 2] [i_0])))) != (((arr_1 [i_1] [i_1 + 1]) ? (arr_1 [i_1] [i_1 + 3]) : (arr_1 [i_1] [i_1 - 1])))));
                arr_5 [14] = (!9199768788493175777);
                var_13 = (((108 << (((arr_2 [i_1 + 3] [i_1 + 3] [i_1 + 3]) + 27)))));
                arr_6 [i_0] [i_1] [i_1] = (((77 ? var_5 : var_1)));
            }
        }
    }
    var_14 *= ((~((max(var_8, var_10)))));
    var_15 = var_7;
    #pragma endscop
}
