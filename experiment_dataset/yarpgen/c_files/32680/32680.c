/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32680
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32680 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32680
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_10;

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_14 = (max(var_14, ((((min(((((arr_2 [i_0]) && 65517))), (arr_1 [i_0] [i_0]))) | ((-(min(var_9, var_10)))))))));
        var_15 = (min(var_15, ((max((((max(var_3, 0)))), ((1 ? ((((-725651368 + 2147483647) >> (((arr_0 [i_0] [i_0]) - 30912))))) : 5482015837048911973)))))));
    }
    var_16 = (max(var_16, var_1));
    var_17 = ((var_6 ? ((var_12 ? var_2 : (~var_1))) : var_6));
    #pragma endscop
}
