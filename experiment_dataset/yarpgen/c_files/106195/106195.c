/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106195
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106195 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106195
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = ((~(max((arr_4 [i_0] [i_1 - 1]), var_3))));
                var_10 = ((((-(arr_2 [i_1 + 1] [i_1] [i_1]))) % var_8));
                var_11 *= var_5;
            }
        }
    }
    var_12 = (min(var_12, ((min((min(var_6, var_3)), -var_1)))));
    var_13 = (max(var_13, ((max((~var_5), var_0)))));
    var_14 = (min(var_4, var_4));
    #pragma endscop
}
