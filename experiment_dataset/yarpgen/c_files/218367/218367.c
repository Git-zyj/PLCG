/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218367
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218367 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218367
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                var_12 -= var_5;
                arr_4 [i_1] [i_0] [i_0] = ((arr_1 [i_0] [i_1]) & (min((max((arr_0 [i_0]), (arr_0 [i_0]))), (arr_3 [i_0] [i_1 - 1]))));
                var_13 &= (((arr_3 [i_1 + 1] [i_1 - 1]) <= (arr_3 [i_1 + 1] [i_1 - 1])));
            }
        }
    }
    var_14 = (((var_11 ? (((min(var_1, var_8)))) : (min(18446744073709551615, var_0)))));
    var_15 = ((var_6 << ((((-(max(var_5, var_8)))) + 97))));
    #pragma endscop
}
