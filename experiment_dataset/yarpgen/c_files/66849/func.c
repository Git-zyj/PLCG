/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66849
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66849 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66849
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */_Bool) min((((/* implicit */unsigned long long int) var_3)), (var_1)));
    /* LoopNest 2 */
    for (unsigned int i_0 = 4; i_0 < 19; i_0 += 3) 
    {
        for (int i_1 = 3; i_1 < 18; i_1 += 3) 
        {
            {
                var_13 = ((/* implicit */short) ((unsigned int) ((_Bool) min((((/* implicit */short) (signed char)37)), (var_10)))));
                arr_5 [i_1] = ((/* implicit */unsigned char) (+(var_6)));
                arr_6 [i_1] [i_0 + 1] [i_1] = ((/* implicit */unsigned long long int) min((((/* implicit */long long int) ((unsigned int) var_3))), (-9223372036854775803LL)));
            }
        } 
    } 
    var_14 = ((/* implicit */unsigned int) var_11);
    var_15 = ((/* implicit */short) (+(var_1)));
    var_16 += ((/* implicit */short) (+(((/* implicit */int) (_Bool)1))));
}
