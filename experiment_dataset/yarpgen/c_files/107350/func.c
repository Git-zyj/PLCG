/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107350
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */int) var_2);
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        for (unsigned short i_1 = 2; i_1 < 16; i_1 += 3) 
        {
            {
                var_14 = ((/* implicit */unsigned int) min((var_14), (((/* implicit */unsigned int) ((((int) arr_3 [(short)16] [(short)16] [i_1 + 1])) >= ((-(2147483641))))))));
                arr_4 [(unsigned short)0] [i_0] [(_Bool)1] = ((/* implicit */unsigned char) arr_0 [i_1 - 2] [i_1]);
            }
        } 
    } 
}
