/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113589
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113589 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113589
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 21;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = (arr_1 [i_0]);
        var_18 = (arr_0 [i_0 + 1] [i_0]);
        var_19 = (min((min((min(9, var_4)), (~-9))), (((-((min(var_16, 30830))))))));
    }
    for (int i_1 = 1; i_1 < 21;i_1 += 1) /* same iter space */
    {
        var_20 = (max(-4, (((!(arr_4 [i_1 - 1]))))));
        var_21 = (max(((((max((arr_1 [i_1 + 1]), (arr_0 [i_1 + 1] [10])))) ? var_6 : 1)), (((((min(var_3, var_11))) ? var_0 : (arr_3 [i_1] [i_1]))))));
    }
    var_22 = var_1;
    var_23 = (max((((-((6768168540701412637 ? 1 : 1))))), var_7));
    #pragma endscop
}
