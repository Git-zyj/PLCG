/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141344
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141344 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141344
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_5;
    var_13 = var_9;
    var_14 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 19;i_3 += 1)
                    {
                        {

                            /* vectorizable */
                            for (int i_4 = 1; i_4 < 21;i_4 += 1)
                            {
                                arr_14 [i_0] [5] [i_3] [6] [19] = var_9;
                                var_15 *= var_4;
                                arr_15 [i_1] [i_4] [i_2] [i_3] [1] [i_3] [8] = var_2;
                            }
                            arr_16 [i_0] = -1601474121;
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 1; i_5 < 21;i_5 += 1)
                {
                    for (int i_6 = 3; i_6 < 20;i_6 += 1)
                    {
                        {
                            arr_22 [i_1] = 4294967295;
                            arr_23 [i_0] [i_1] [i_1] = var_1;
                            arr_24 [i_0] [i_0] [i_0] = var_5;
                        }
                    }
                }
                var_16 = var_11;
                var_17 = (arr_3 [i_0] [i_1]);
            }
        }
    }
    var_18 = var_8;
    #pragma endscop
}
