/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26415
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26415 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26415
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((var_0 == (-1 + var_16))) ? var_13 : ((var_14 | ((max(var_7, var_0)))))));
    var_18 = 8092;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_8 [9] [1] [2] = (arr_1 [i_0]);
                    var_19 = (min(var_19, ((((((((1022 ? (arr_3 [i_1]) : (arr_6 [i_2] [i_1] [i_0])))) ? (min(2, (arr_2 [i_0]))) : ((((21307 < (arr_5 [i_2])))))))) ? (((min((1 < 18918), -1)))) : (min((arr_2 [i_1]), ((((arr_7 [i_2] [i_1] [5] [i_0]) / 41)))))))));
                }
            }
        }
    }
    #pragma endscop
}
