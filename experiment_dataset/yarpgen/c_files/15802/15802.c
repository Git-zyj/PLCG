/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (var_5 + var_9);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {

                    for (int i_3 = 4; i_3 < 7;i_3 += 1)
                    {
                        arr_12 [i_0 - 1] [i_1] [i_0 + 1] [5] = -116;
                        var_13 = ((((((arr_9 [i_2] [i_1] [i_0]) ? (arr_5 [i_3 - 1] [i_1] [i_0 - 1]) : var_10))) ? (((var_5 + 6808) ? (arr_6 [i_0 + 1] [i_0 + 1]) : 3342841752)) : (arr_1 [i_1])));
                    }
                    var_14 = (arr_6 [i_0] [i_0 - 1]);
                    var_15 = 42358;
                }
            }
        }
    }
    var_16 = ((~(max(1, 4294967295))));
    var_17 = (!var_6);
    #pragma endscop
}
