/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183517
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183517 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183517
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_15;
    var_20 = ((8703 ? 4294967289 : (((((0 ? 1 : var_9))) ? (2521265572 & 128) : var_13))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_21 = (max(((max(24, (arr_3 [i_1 + 2] [i_1])))), (arr_2 [i_1 - 1] [i_1 - 1] [i_1])));
                arr_5 [i_1] = ((-1 ? (1 / 1) : 1));
                var_22 = ((var_2 ? ((((min(var_4, (arr_0 [i_0])))) ? (arr_0 [i_0]) : (max(var_13, var_17)))) : ((((((!(arr_2 [i_0] [i_0] [i_1]))) && ((max(var_9, 0)))))))));
            }
        }
    }
    #pragma endscop
}
