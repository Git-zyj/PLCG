/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47334
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = (signed char)0;
    /* LoopSeq 1 */
    for (signed char i_0 = 2; i_0 < 17; i_0 += 2) 
    {
        /* LoopNest 3 */
        for (signed char i_1 = 1; i_1 < 18; i_1 += 3) 
        {
            for (signed char i_2 = 0; i_2 < 19; i_2 += 1) 
            {
                for (signed char i_3 = 3; i_3 < 17; i_3 += 3) 
                {
                    {
                        arr_10 [i_0] [i_1] [(signed char)4] [i_1] = ((/* implicit */signed char) (-(((((/* implicit */int) arr_2 [i_1 - 1] [i_3 - 3])) * (((/* implicit */int) arr_2 [i_1 + 1] [i_3 + 1]))))));
                        arr_11 [i_1] [i_1] [i_2] [i_3] = ((/* implicit */signed char) ((max((((((/* implicit */int) (signed char)127)) * (((/* implicit */int) var_5)))), (((/* implicit */int) var_9)))) * (((/* implicit */int) ((signed char) arr_8 [i_1])))));
                    }
                } 
            } 
        } 
        var_13 = min((arr_1 [i_0 - 2]), (max((arr_2 [i_0 + 2] [i_0 + 1]), (arr_4 [i_0] [i_0] [i_0 - 2] [i_0]))));
    }
    var_14 = var_7;
    var_15 = var_5;
}
