/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 146089
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=146089 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/146089
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 11;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = ((~(((var_1 ^ var_11) & ((1160056350 ? (arr_0 [i_0]) : var_11))))));
                arr_7 [i_0] [i_1 + 1] = -66;
                arr_8 [i_1 + 2] [1] [i_1] = (((((+(min((arr_1 [i_0]), 0))))) ? ((+((var_12 ? (arr_5 [i_0] [i_0] [7]) : (arr_3 [i_0] [i_1]))))) : (((1208066828 ? (arr_5 [i_0] [i_1 + 1] [i_0]) : (arr_4 [i_0]))))));
            }
        }
    }
    var_15 = (((((-1818762574 ? var_13 : var_8))) >= var_6));
    #pragma endscop
}
