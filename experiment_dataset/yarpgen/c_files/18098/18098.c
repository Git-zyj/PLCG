/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18098
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18098 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18098
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((var_10 ? var_12 : (max((min(var_7, var_0)), (-23053 / var_0)))));
    var_19 = var_7;

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0] |= ((!(((arr_1 [i_0] [i_0]) && (arr_1 [i_0] [i_0])))));
        var_20 = max(((6372 ? (min((arr_0 [1]), var_3)) : (((var_8 ? 0 : var_0))))), ((((arr_0 [i_0]) ? ((-(arr_2 [i_0] [16]))) : (arr_2 [i_0] [i_0])))));
    }
    var_21 = (max(var_21, (((var_14 ? (min((max(var_6, var_2)), var_13)) : ((((min(var_4, var_15)) < var_0))))))));
    var_22 = ((var_0 ? var_3 : var_9));
    #pragma endscop
}
