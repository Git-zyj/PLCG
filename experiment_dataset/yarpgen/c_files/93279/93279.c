/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93279
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93279 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93279
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 12;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((!((min(13137, 39778)))));
        arr_3 [i_0] [i_0 - 1] = var_12;
        var_17 *= (arr_0 [0]);
    }
    for (int i_1 = 1; i_1 < 12;i_1 += 1) /* same iter space */
    {
        var_18 = (min(-6, 4));

        /* vectorizable */
        for (int i_2 = 0; i_2 < 14;i_2 += 1)
        {
            var_19 = 1;
            var_20 = ((-22 ? (arr_7 [i_1 - 1]) : (arr_5 [i_1] [i_1 + 1])));
            arr_10 [i_2] [i_2] [i_1] = (((var_6 ? var_2 : (arr_4 [i_1]))));
        }
        for (int i_3 = 2; i_3 < 11;i_3 += 1)
        {
            var_21 += (min((min(var_15, ((var_3 ? var_13 : 4294967293)))), (arr_12 [i_3 + 2] [i_3 + 2])));
            var_22 = ((-(min((arr_6 [i_1 - 1] [i_3]), (arr_5 [i_1 + 2] [i_3 + 3])))));
            arr_13 [i_1] [i_3 + 2] = (min(((((~var_16) ? var_4 : var_3))), -3353731682207226861));
        }
        var_23 = arr_5 [i_1 + 1] [i_1];
        var_24 = (max(var_24, (min(45, 2))));
    }
    var_25 = var_16;
    #pragma endscop
}
