/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49426
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((max(var_11, var_5)) <= 6)) ? var_15 : var_4);
    var_18 = -2146;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 7;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_7 [2] [i_1] [i_1] = (~var_14);
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 10;i_4 += 1)
                        {
                            {
                                arr_12 [i_4] [i_3] [i_1] [i_1] [i_1] [i_0 + 1] = (arr_4 [i_3]);
                                var_19 = ((10477281481226744932 & (((~(arr_0 [i_0 + 1]))))));
                            }
                        }
                    }
                    var_20 = (max(var_20, (arr_6 [i_0] [i_0] [i_2])));
                    var_21 = ((+(min((((var_16 ? (arr_0 [i_1]) : 2146))), (((arr_4 [i_0]) & (arr_6 [5] [i_1 - 2] [i_0])))))));
                    var_22 = (max(var_22, -23518));
                }
            }
        }
    }
    var_23 = (min(var_23, var_11));
    #pragma endscop
}
