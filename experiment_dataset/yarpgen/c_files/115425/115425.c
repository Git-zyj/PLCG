/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115425
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115425 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115425
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(((var_14 >> (-var_12 + 52))), ((max((!var_10), var_13)))));
    var_19 = (max(var_9, (((var_0 + var_6) ? (var_10 - var_12) : var_2))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_20 = (min(var_20, var_2));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 16;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_21 = ((100 ? (var_17 || var_16) : var_10));
                                var_22 = var_4;
                            }
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_5 = 2; i_5 < 15;i_5 += 1)
                {
                    for (int i_6 = 0; i_6 < 18;i_6 += 1)
                    {
                        {
                            var_23 = (min((arr_11 [i_0] [i_1]), var_7));
                            var_24 -= ((((((max(var_5, var_14))) - (var_8 != var_4))) != (((((24 ? 96 : 25)) < ((min(177, 160))))))));
                        }
                    }
                }
                var_25 = arr_4 [i_0] [i_1] [i_1];
                arr_20 [i_0] [i_1] [i_0] = arr_2 [i_0] [i_0];
            }
        }
    }
    var_26 = var_4;
    #pragma endscop
}
