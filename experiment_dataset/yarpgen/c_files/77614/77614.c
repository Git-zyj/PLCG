/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77614
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77614 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77614
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((((((((var_19 > (arr_1 [i_0 - 1]))))) / (min(var_3, (arr_0 [i_1]))))) * (arr_3 [i_1] [i_0 - 1])));
                var_20 = (arr_3 [i_0] [i_0]);
                var_21 *= ((~(arr_2 [2])));
                arr_5 [i_0] [i_0] [6] = (min((max(2147483645, ((~(arr_3 [16] [i_0]))))), (arr_2 [i_0])));
            }
        }
    }
    var_22 = (min((1134406156 <= 3160561139), var_14));
    var_23 = (((!var_5) << (((!((min(var_16, var_12))))))));
    var_24 = var_9;
    #pragma endscop
}
