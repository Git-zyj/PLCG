/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 210009
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=210009 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/210009
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_13, var_8));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_14 -= 183;
        var_15 = (arr_0 [i_0]);
        arr_2 [i_0] = (arr_1 [i_0] [i_0]);
    }
    for (int i_1 = 1; i_1 < 16;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 20;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 20;i_3 += 1)
            {
                {
                    arr_12 [i_2] [i_2] [i_2] [i_2] = ((((((var_12 ? (arr_9 [i_2] [i_2] [i_2]) : (arr_10 [i_1] [i_2])))) + (arr_5 [i_1]))));
                    var_16 += var_10;
                    var_17 = (arr_11 [3] [i_2] [i_2]);
                }
            }
        }
        var_18 -= var_5;
        var_19 = (((((arr_7 [1] [i_1 - 1]) ? var_3 : (arr_3 [2] [i_1 + 1])))) ? (arr_3 [i_1] [i_1 - 1]) : ((((arr_3 [i_1] [i_1 + 1]) == (arr_5 [i_1])))));
    }
    #pragma endscop
}
