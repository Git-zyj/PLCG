/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102274
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102274 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102274
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 13;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_2] = ((((min((((arr_2 [i_0]) ? var_1 : 1)), (arr_5 [i_1] [i_2 - 1] [i_2 + 1])))) ? -118 : ((var_1 ? (max(var_1, var_3)) : (arr_5 [i_0] [7] [11])))));
                    var_10 = (arr_2 [i_2 + 1]);
                }
            }
        }
    }
    var_11 = (min(var_11, ((min((!var_1), (min(var_6, (min(1331162602, var_5)))))))));
    var_12 = var_4;
    var_13 = (max((max((min(var_1, var_4)), (!var_3))), (((!(((var_1 ? var_8 : var_0))))))));
    #pragma endscop
}
