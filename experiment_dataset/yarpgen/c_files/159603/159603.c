/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159603
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159603 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159603
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_17 = (arr_4 [i_1]);
                arr_6 [i_1] [i_1] = ((-(((arr_1 [i_1] [i_0]) ? (((arr_5 [i_0] [i_0]) ? (arr_4 [i_1]) : (arr_0 [i_1]))) : (((-32767 - 1) ? 144 : (arr_5 [i_1] [i_1])))))));
                arr_7 [i_1] = (((min(((5541 ? var_16 : 12458)), ((max((arr_2 [i_1]), 21590))))) >> (((arr_1 [i_0] [i_0]) - 206))));
                var_18 = (arr_1 [i_1] [i_1]);
            }
        }
    }
    var_19 = (max(var_1, ((-(4092 > 38610)))));
    var_20 = ((15 ? 1 : 247));
    #pragma endscop
}
