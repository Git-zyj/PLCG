/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29856
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29856 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29856
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0 + 2] = (((max(((2147483647 ? var_9 : (arr_0 [i_1]))), (max(1792, var_4)))) ^ var_3));
                var_13 ^= 1840107793;
            }
        }
    }
    var_14 = (min((25 / var_12), ((min(231, 0)))));
    var_15 = (min((((var_11 / var_9) / (((min(24, 14501)))))), ((min(((max(37, 232))), var_6)))));
    var_16 = (max(var_16, var_8));
    #pragma endscop
}
