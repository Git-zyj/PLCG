/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135269
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) var_6);
    /* LoopSeq 1 */
    for (unsigned char i_0 = 2; i_0 < 21; i_0 += 1) 
    {
        /* LoopNest 2 */
        for (unsigned int i_1 = 3; i_1 < 22; i_1 += 1) 
        {
            for (unsigned int i_2 = 4; i_2 < 22; i_2 += 3) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned char i_3 = 3; i_3 < 19; i_3 += 3) 
                    {
                        for (unsigned char i_4 = 1; i_4 < 22; i_4 += 3) 
                        {
                            {
                                var_12 = ((/* implicit */unsigned char) (~(((unsigned int) var_9))));
                                var_13 |= ((/* implicit */unsigned char) ((((((/* implicit */_Bool) var_5)) ? (((/* implicit */unsigned int) ((/* implicit */int) arr_2 [i_1 - 3] [i_1 - 2] [i_1]))) : (min((var_1), (((/* implicit */unsigned int) (_Bool)0)))))) * (((/* implicit */unsigned int) ((((/* implicit */int) var_4)) & ((((_Bool)1) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) var_7)))))))));
                            }
                        } 
                    } 
                    var_14 = ((/* implicit */unsigned char) var_8);
                    var_15 = ((/* implicit */unsigned char) (_Bool)1);
                }
            } 
        } 
        var_16 = ((/* implicit */long long int) (~(((/* implicit */int) (_Bool)1))));
    }
}
