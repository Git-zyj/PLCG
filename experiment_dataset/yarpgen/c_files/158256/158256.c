/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 158256
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=158256 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/158256
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_8;
    var_12 = (min(var_12, ((((((var_8 >= (max(var_9, var_3))))) ^ 48326)))));
    var_13 = -var_0;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 15;i_3 += 1)
                {
                    {
                        arr_11 [i_0] [i_0] [i_0] [i_0] = (arr_10 [i_0] [i_1] [i_2] [7]);
                        var_14 = (((!(arr_3 [i_1] [i_2]))));
                        var_15 = (((1162520038 ^ (arr_0 [i_2]))) * ((~(arr_6 [i_3] [1] [i_0] [i_0]))));
                    }
                }
            }
        }
        /* LoopNest 2 */
        for (int i_4 = 0; i_4 < 15;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 15;i_5 += 1)
            {
                {
                    var_16 = (max(var_16, var_0));
                    arr_16 [3] [3] [i_5] [i_5] = ((arr_15 [i_5] [i_4] [i_5]) / (arr_4 [i_0] [6]));
                }
            }
        }
        var_17 = arr_14 [i_0] [i_0] [i_0] [i_0];
        var_18 = var_7;
        /* LoopNest 3 */
        for (int i_6 = 0; i_6 < 15;i_6 += 1)
        {
            for (int i_7 = 1; i_7 < 14;i_7 += 1)
            {
                for (int i_8 = 0; i_8 < 15;i_8 += 1)
                {
                    {
                        var_19 = ((((arr_7 [i_8]) <= (arr_4 [i_0] [i_6]))));
                        arr_23 [i_6] = (arr_15 [i_6] [i_6] [i_7 + 1]);

                        for (int i_9 = 0; i_9 < 15;i_9 += 1)
                        {
                            var_20 = var_3;
                            var_21 = arr_12 [i_7 - 1] [i_6] [i_6];
                            arr_26 [i_6] = (((!1684290062) < var_4));
                        }
                        var_22 = arr_24 [i_6] [i_7] [i_7 + 1] [i_7] [i_8];
                    }
                }
            }
        }
    }
    for (int i_10 = 0; i_10 < 25;i_10 += 1)
    {
        var_23 = ((2370 >> (17232 - 17225)));
        arr_30 [i_10] = ((var_0 > (max((arr_27 [i_10]), (arr_27 [i_10])))));
    }
    #pragma endscop
}
