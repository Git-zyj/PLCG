/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198586
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198586 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198586
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_8;

    /* vectorizable */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (!var_3);
        var_15 = ((14767004849759191061 ? (var_0 % var_5) : var_4));
        arr_5 [i_0] [i_0] = ((-(39176 >> 0)));
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 13;i_2 += 1)
        {
            {
                arr_10 [i_2] = (((5676384869156066704 <= -1) ? 1 : (max(3679739223950360554, 14767004849759191061))));
                arr_11 [i_2] = var_12;
            }
        }
    }
    var_16 = (((((var_8 ? (var_0 < var_13) : (~var_8)))) ? 25329 : (((min(var_0, var_9)) + var_7))));
    #pragma endscop
}
