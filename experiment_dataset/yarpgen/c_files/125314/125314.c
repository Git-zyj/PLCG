/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125314
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125314 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125314
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_19 = (arr_1 [i_0] [i_2]);
                    arr_7 [i_0] [i_1 - 2] [i_0] [i_2] = ((((((max(0, 1)) || (arr_5 [i_0 - 1] [i_2])))) << (min((-7786184415711996768 && 3659762152), -18446744073709551598))));
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 3; i_3 < 12;i_3 += 1)
    {
        for (int i_4 = 1; i_4 < 12;i_4 += 1)
        {
            for (int i_5 = 1; i_5 < 13;i_5 += 1)
            {
                {
                    var_20 = 18446744073709551615;
                    var_21 = (~((((((arr_6 [i_3] [i_3 - 1] [i_3 - 1] [i_3]) ? var_7 : 0))) ? (arr_3 [i_5]) : (arr_15 [i_3 - 2] [i_4 + 1] [i_3 - 2] [i_5]))));
                    var_22 = (((arr_8 [i_4]) ? (arr_16 [i_5] [i_5 + 1] [i_4 - 1] [i_3 + 2]) : (((3659762152 | 65535) ^ ((17 >> (var_12 - 76)))))));
                    var_23 |= (~-1);
                }
            }
        }
    }
    #pragma endscop
}
