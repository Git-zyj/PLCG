/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 65000
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=65000 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/65000
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_14 = (arr_3 [i_0] [i_1]);
                var_15 += (((((arr_1 [i_0]) * var_0))));
                var_16 = (((-25841 | 4294705152) >> (-25844 - 39667)));
                var_17 *= (((((arr_1 [i_0]) && 23897))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 2; i_2 < 14;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_4 = 0; i_4 < 15;i_4 += 1)
                {
                    for (int i_5 = 3; i_5 < 12;i_5 += 1)
                    {
                        for (int i_6 = 1; i_6 < 12;i_6 += 1)
                        {
                            {
                                arr_16 [i_6 + 3] [i_6 + 3] [i_6 + 3] [i_6 + 3] [i_5] = 1;
                                var_18 ^= var_0;
                                arr_17 [i_2] [i_6] [i_5] = var_8;
                                var_19 += ((var_10 > (-25839 | 82)));
                                arr_18 [i_2 - 1] [i_2] [i_2] [i_5] [11] [i_6 + 1] = -30;
                            }
                        }
                    }
                }
                arr_19 [i_2] [i_2] = ((100 | (25 >= var_8)));
                var_20 = -6;
            }
        }
    }
    var_21 = (44596 || -32760);
    var_22 = (-9223372036854775807 - 1);
    #pragma endscop
}
