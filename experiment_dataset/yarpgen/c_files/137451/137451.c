/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137451
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137451 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137451
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 ^= var_2;
    var_16 = (min(var_16, var_10));
    var_17 = (var_6 ? ((((max(var_4, var_6))) ? var_7 : (var_11 + -1257466372))) : (var_1 + var_4));
    var_18 = var_10;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_9 [i_2] [1] [i_2] = ((!((((~(arr_6 [i_0] [i_1] [i_2])))))));
                    arr_10 [i_0] = ((7256330420918684792 ? 7256330420918684793 : var_11));
                }
            }
        }
    }
    #pragma endscop
}
