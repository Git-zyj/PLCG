/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169466
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169466 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169466
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = -3;
    var_18 = (((var_6 % 3) ? -3 : 220));

    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        var_19 = (min(((var_2 ? (arr_2 [i_0 - 2] [i_0 - 1]) : (arr_0 [i_0] [i_0]))), (arr_1 [i_0])));
        arr_3 [i_0] = (arr_2 [i_0] [i_0]);
        var_20 = ((((min(((1 ? -7 : 1)), 2))) / (((((arr_2 [1] [i_0]) ? (arr_2 [i_0] [i_0]) : var_13)) / var_7))));
        arr_4 [i_0] = ((((min(((~(arr_1 [i_0 - 1]))), (arr_0 [i_0 - 1] [i_0 - 1])))) / ((~(((arr_2 [i_0 - 2] [i_0 + 1]) ? 6 : var_6))))));
    }
    #pragma endscop
}
