/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64987
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64987 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64987
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 ^= ((((min(var_9, 5592065280858513119))) ? var_4 : var_13));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_18 = ((var_13 ? 1422311083381570685 : ((((var_15 != (min(1422311083381570685, (arr_7 [i_0] [i_1])))))))));
                    var_19 *= (+(max((arr_4 [i_0 - 2] [i_0 + 1] [i_0 - 1]), 18901)));
                }
            }
        }
    }
    var_20 = var_13;
    #pragma endscop
}
