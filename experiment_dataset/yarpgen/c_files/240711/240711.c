/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, var_2));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_15 = (min(var_15, (~8886911617795559821)));
        var_16 = (max(var_16, (((max((min(-1, (arr_0 [i_0]))), (23770 && 41756)))))));
        /* LoopNest 2 */
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_17 = ((((!(57 & 41766))) ? (arr_0 [i_1]) : 57));
                    var_18 &= 75;
                    var_19 = (((((-((75 ? 10176889957737661050 : (arr_4 [i_1] [i_2])))))) ? 4486542461277295359 : (arr_3 [i_0] [i_1] [i_2])));
                }
            }
        }
    }
    #pragma endscop
}
