/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163319
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_20, var_18));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                var_21 = (min(var_21, (((((~(arr_1 [15])))) || ((max(((min(-26, -26))), (min(-39, (arr_5 [i_0]))))))))));
                                var_22 ^= ((((26 ? 38 : 30410))) ? (arr_10 [i_1] [i_1] [6] [i_1] [i_1] [i_0]) : ((((arr_1 [i_0]) ? -39 : 20))));
                            }
                        }
                    }
                    arr_14 [i_2] [i_1] [i_0] = ((((min(127, -36))) ? (arr_4 [i_2] [i_1] [i_0]) : (min((var_8 + -26), ((var_12 ? 126 : var_10))))));
                    var_23 = (arr_2 [i_2]);
                }
            }
        }
    }
    var_24 += ((var_10 | (min(var_8, (min(var_1, 65515))))));
    #pragma endscop
}
