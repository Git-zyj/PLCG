/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41444
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41444 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41444
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (arr_0 [i_0 + 3] [i_0]);
        var_18 = (min(var_18, var_10));
        var_19 = ((((var_3 <= (((arr_3 [i_0]) ? var_3 : 5535)))) ? (((~((min(5535, var_1)))))) : var_17));
    }
    var_20 = (((var_9 != var_11) ? ((var_10 ? ((max(0, var_12))) : var_5)) : (((var_17 < (((5535 ? var_3 : var_12))))))));
    var_21 = var_5;
    #pragma endscop
}
