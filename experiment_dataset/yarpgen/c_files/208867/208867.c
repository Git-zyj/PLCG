/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208867
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, ((min(0, 8355)))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    var_18 = (max(var_18, (((((((-(arr_5 [i_0] [i_1] [i_1] [i_1])))) ? (((((arr_2 [i_1]) < 22)))) : (((arr_2 [i_1]) ? (arr_3 [i_0] [i_0] [i_2]) : 8246))))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_19 = (-5270726360971916963 + 9396);
                                var_20 *= ((max(62, (arr_4 [i_4] [i_3]))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_21 = (~var_0);
    #pragma endscop
}
