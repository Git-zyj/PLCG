/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (~0);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_2] = (((((273800639 ? (((arr_4 [i_2] [i_1] [i_0]) ? -9223372036854775787 : (arr_2 [i_0] [i_0]))) : ((((var_2 >= (arr_3 [i_1])))))))) ? (((arr_1 [i_0] [i_2 + 1]) - (arr_1 [i_0] [i_2 + 4]))) : ((~(arr_4 [i_2 + 3] [i_2 - 1] [i_2 + 1])))));
                    var_15 &= -2238;
                    var_16 = (max((!18446744073709551595), (arr_1 [i_1] [i_2])));
                }
            }
        }
    }
    var_17 -= var_8;
    #pragma endscop
}
