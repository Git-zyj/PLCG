/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233357
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233357 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233357
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 17; i_0 += 2) 
    {
        for (unsigned int i_1 = 2; i_1 < 14; i_1 += 4) 
        {
            for (int i_2 = 0; i_2 < 17; i_2 += 4) 
            {
                {
                    var_18 = ((/* implicit */_Bool) arr_6 [i_2]);
                    arr_8 [i_0] [i_0] [i_1 + 1] [i_1 + 1] = ((/* implicit */long long int) arr_2 [i_2]);
                    arr_9 [i_0] [i_1] [i_2] = ((/* implicit */unsigned int) ((long long int) (!(((/* implicit */_Bool) min((var_2), (((/* implicit */long long int) var_11))))))));
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */_Bool) ((((/* implicit */_Bool) var_13)) ? (((/* implicit */int) ((_Bool) ((unsigned int) -922462322)))) : (((/* implicit */int) min((var_12), ((!(((/* implicit */_Bool) (signed char)-44)))))))));
}
