/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89205
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89205 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89205
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(((16895838252376514102 ? var_4 : var_12)), var_3));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_15 |= ((var_9 ? (min(4817, (arr_3 [i_0 + 1]))) : ((max((min(-315083636, var_7)), (((arr_4 [i_0 + 2] [i_1] [22]) ? var_7 : var_0)))))));
                arr_5 [i_0] = ((+(max(((var_12 ? (arr_4 [i_1] [i_1] [i_0]) : var_11)), (min((arr_4 [i_0 + 1] [i_0] [i_0]), var_3))))));
            }
        }
    }
    #pragma endscop
}
