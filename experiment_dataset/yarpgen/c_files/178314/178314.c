/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178314
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178314 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178314
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        var_15 = ((((!(-127 - 1))) ? ((var_4 + (arr_0 [i_0]))) : (!31)));
        arr_3 [i_0] [i_0] = (i_0 % 2 == 0) ? (max((arr_1 [i_0] [i_0]), ((49041 >> (((arr_1 [i_0] [i_0]) + 23873)))))) : (max((arr_1 [i_0] [i_0]), ((49041 >> (((((arr_1 [i_0] [i_0]) + 23873)) - 45891))))));
        arr_4 [i_0] = (max(-53, var_5));
    }
    var_16 = (max(var_16, var_2));
    var_17 = (-var_5 * var_12);
    #pragma endscop
}
