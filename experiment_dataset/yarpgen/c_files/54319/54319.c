/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54319
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((((min(67108863, var_4))) ? ((max(var_3, var_11))) : 0)) * ((max(((var_7 ? var_3 : var_4)), var_3)))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 15;i_0 += 1)
    {
        arr_3 [i_0 + 2] = (((arr_1 [i_0]) * var_4));
        var_13 |= (((arr_1 [i_0]) ? (((arr_0 [i_0 - 1]) / var_3)) : 0));
    }
    var_14 = (((((((var_0 ? var_8 : 18446744073642442753))) ? var_10 : (min(var_7, var_9)))) * ((((var_4 ^ var_8) ^ var_7)))));
    #pragma endscop
}
