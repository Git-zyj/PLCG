/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 24;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_19 |= ((((!(arr_11 [i_0] [i_1] [i_2] [i_0] [i_3] [i_1])))));
                            var_20 -= 29364;
                            arr_12 [i_0] [i_0] [i_0] = ((!(((-(arr_4 [i_0] [i_0 - 1] [i_1]))))));
                            var_21 = -2076578858;
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 0; i_4 < 1;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 24;i_5 += 1)
                    {
                        {
                            arr_17 [i_0] [i_4] = 10;
                            var_22 += 3167777262;
                        }
                    }
                }
                var_23 = (max(-3, (max((arr_10 [14] [i_0 + 1] [i_1] [i_0 + 1] [i_1]), ((~(arr_10 [i_1] [i_1] [i_1] [i_0] [i_0])))))));
                var_24 = (max(var_24, 2076578858));
            }
        }
    }
    var_25 &= var_15;
    var_26 = var_2;
    #pragma endscop
}
