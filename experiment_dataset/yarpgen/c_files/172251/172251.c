/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172251
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172251 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172251
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_3;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] = -8249568391784833537;
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                for (int i_3 = 3; i_3 < 14;i_3 += 1)
                {
                    {
                        arr_12 [i_0] [i_1 + 4] [i_2] [i_3] [i_3 + 1] [i_3] = var_3;
                        arr_13 [i_1] [i_1] [i_1] [i_3] [i_3] [i_0] = ((var_7 ? 13328352257938746630 : (~var_7)));
                        arr_14 [i_0] [i_0] [i_0] [i_0] = 13328352257938746643;
                    }
                }
            }
        }
        arr_15 [i_0] [i_0] = 118;
    }
    #pragma endscop
}
