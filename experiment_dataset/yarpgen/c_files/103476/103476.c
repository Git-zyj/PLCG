/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103476
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103476 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103476
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_2;

    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        var_14 = ((min(1155045843, (arr_2 [i_0]))));
        /* LoopNest 2 */
        for (int i_1 = 3; i_1 < 8;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_15 ^= (!1602461891);
                    var_16 = (max(((107970110 ? var_0 : (arr_6 [i_0 + 1] [i_1 - 1]))), ((max((min(var_5, (arr_7 [i_0 - 2] [3] [3] [i_0 - 2]))), (((-111 >= (arr_4 [i_0] [i_2] [i_2])))))))));
                    var_17 = (arr_8 [i_1 - 1] [i_1 - 3]);
                }
            }
        }
    }
    var_18 = (min(var_1, (!var_0)));
    var_19 = var_7;
    #pragma endscop
}
