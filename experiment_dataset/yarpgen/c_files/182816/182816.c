/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (((~var_7) ? ((-var_4 ? (-37 / -5063001985908166761) : 244)) : (var_8 / var_5)));

    for (int i_0 = 1; i_0 < 6;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = ((17060217732882781431 ? -25869 : (((var_4 ? (arr_1 [i_0]) : 1377057832)))));
        var_19 = ((((-33 ? 1971137464 : 42011)) + (arr_1 [i_0 + 2])));
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        var_20 = -2694260097794324018;
        arr_7 [i_1] [i_1] = (arr_4 [i_1]);
        var_21 = (((((-(arr_6 [i_1]))) - (65535 - -1733987749))));
    }
    #pragma endscop
}
