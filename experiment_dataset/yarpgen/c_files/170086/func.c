/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170086
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170086 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170086
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned long long int) var_5);
        var_10 = ((/* implicit */_Bool) ((unsigned char) (+((+(((/* implicit */int) var_3)))))));
        var_11 = ((/* implicit */int) min((((/* implicit */short) var_4)), (var_7)));
    }
    var_12 = ((/* implicit */_Bool) max((var_12), (((/* implicit */_Bool) ((((/* implicit */_Bool) var_0)) ? (((/* implicit */long long int) (+(3118815594U)))) : (var_1))))));
}
