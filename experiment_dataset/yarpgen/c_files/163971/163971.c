/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163971
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163971 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163971
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_10 = ((((((arr_0 [i_0]) < (arr_0 [i_0 - 1])))) != ((((((var_3 ? (arr_0 [i_0]) : (arr_0 [i_0 - 1]))) + 2147483647)) << (((((arr_0 [i_0 - 1]) + 526273371)) - 29))))));
        arr_2 [i_0] = (65523 && (arr_1 [i_0]));
        arr_3 [i_0 - 1] [i_0] = (((-616924794 ? -616924799 : 2928)));
    }

    /* vectorizable */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        var_11 = (min(var_11, (((~(arr_1 [i_1]))))));
        arr_6 [i_1] = var_8;
        var_12 ^= var_9;
    }
    #pragma endscop
}
