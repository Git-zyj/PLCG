/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229458
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229458 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229458
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_15 = (max((max((max(2047, -5130440475264958905)), (arr_0 [i_0 + 3]))), (((((max((arr_1 [i_0 - 1]), -1))) ^ -5130440475264958906)))));
                var_16 = (39863 && ((!(arr_3 [i_0] [i_0]))));
            }
        }
    }
    var_17 = (((((max(var_4, var_8)) - 32760)) - ((((min(var_11, var_0)))))));
    #pragma endscop
}
