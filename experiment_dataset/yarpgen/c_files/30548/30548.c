/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30548
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30548 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30548
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 18;i_2 += 1)
            {
                {
                    arr_7 [i_1] [i_2] = 8295;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_14 = (min(var_14, ((min((((arr_1 [i_0 + 1] [i_0 + 1]) ? -20311 : (arr_1 [i_0 - 2] [i_0 + 1]))), ((((arr_11 [i_0] [i_3] [i_2] [i_2] [i_2]) <= (var_12 ^ var_1)))))))));
                                var_15 = var_1;
                                var_16 *= (((1 ? 20315 : 255)));
                            }
                        }
                    }
                    arr_13 [i_1 - 1] [i_1] = 1;
                    var_17 = (~-var_10);
                }
            }
        }
    }
    var_18 = (min(var_18, (((var_8 ? (((~var_7) ? (((var_12 ? 4294967283 : 10))) : var_13)) : (18446744073709551615 || var_1))))));
    #pragma endscop
}
