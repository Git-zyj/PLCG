/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198786
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198786 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198786
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 8;i_3 += 1)
                    {
                        {
                            var_14 = ((189 ? 189 : (((77 < (9572 || 0))))));
                            var_15 = var_3;
                        }
                    }
                }

                for (int i_4 = 2; i_4 < 9;i_4 += 1)
                {
                    var_16 = ((var_11 ? ((var_0 ? ((var_8 ? var_13 : var_0)) : (((max(67, 22255)))))) : var_13));
                    var_17 = var_11;
                }
                arr_15 [i_0] [i_0] = ((((1 ? 1 : 65535)) + (((!var_13) ? (var_1 + var_4) : var_1))));
                var_18 = var_8;
                var_19 = (((var_12 && var_0) ? (!var_4) : (!var_13)));
            }
        }
    }
    var_20 = ((-((((var_6 > var_9) && var_0)))));
    var_21 = (var_4 - var_6);
    #pragma endscop
}
