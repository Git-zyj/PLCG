/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119221
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119221 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119221
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 -= ((/* implicit */signed char) (+(300339366)));
    var_15 = ((/* implicit */unsigned long long int) var_6);
    /* LoopSeq 1 */
    for (unsigned int i_0 = 2; i_0 < 11; i_0 += 4) 
    {
        var_16 = ((/* implicit */unsigned char) (((!(((/* implicit */_Bool) 300339379)))) ? (((/* implicit */unsigned int) 300339379)) : (((unsigned int) arr_1 [i_0 - 2]))));
        var_17 = ((/* implicit */short) 1265249588);
    }
}
