/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171020
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171020 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171020
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 1;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 8;i_1 += 1)
        {
            {
                arr_4 [4] |= (((arr_2 [i_0]) % (((arr_3 [i_1]) ^ ((32767 << (var_1 - 4245912089)))))));

                for (int i_2 = 3; i_2 < 9;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 11;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1] [i_2] [3] [i_3] [i_0] [i_4] = (((((-(arr_5 [i_2 - 3] [i_2 - 3] [i_2])))) / (arr_7 [i_0] [10])));
                                arr_14 [i_0] [i_0] [10] [10] [i_4] = ((+((-((max((arr_5 [i_0] [i_0] [i_0]), var_4)))))));
                            }
                        }
                    }
                    var_12 &= 32767;
                }
                /* vectorizable */
                for (int i_5 = 0; i_5 < 1;i_5 += 1)
                {
                    arr_17 [i_0] [i_0] [i_0] = (((arr_1 [i_0] [i_0]) ? -10549 : var_2));
                    arr_18 [i_0] [i_0] = (~96);
                }
            }
        }
    }
    #pragma endscop
}
