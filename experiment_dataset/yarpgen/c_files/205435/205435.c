/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205435
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205435 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205435
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_17 *= ((!(arr_3 [i_0] [i_1])));
                var_18 = (max(var_18, (((((!(arr_1 [i_0]))))))));
                var_19 = ((((((arr_0 [i_1]) ? var_12 : (~var_1)))) ? -15982 : (min(var_3, (arr_2 [i_0])))));
                var_20 = ((-(((62801 > (arr_3 [i_0] [i_1]))))));

                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    var_21 = var_12;
                    var_22 ^= var_1;
                    var_23 = (min(var_23, -1159991380));
                }
            }
        }
    }
    var_24 = -var_15;
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 23;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 23;i_4 += 1)
        {
            {
                var_25 = (max(var_25, (((!(((-var_1 ? 38156 : ((~(arr_6 [i_3])))))))))));
                var_26 *= ((((-6772 == ((var_11 ? (arr_8 [i_3]) : 0)))) ? ((var_1 ? -22844 : (arr_6 [i_3]))) : var_1));
            }
        }
    }
    #pragma endscop
}
