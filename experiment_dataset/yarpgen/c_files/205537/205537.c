/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205537
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205537 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205537
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (arr_0 [i_0] [i_1]);
                var_17 ^= ((((((arr_2 [i_0 - 3] [i_1]) ? (!var_3) : (((arr_2 [i_0] [i_0]) ? var_3 : var_14))))) ? (arr_3 [8] [i_0 + 1] [i_1]) : ((((arr_1 [i_0 - 1]) <= ((((arr_4 [i_1]) != (arr_1 [i_0 - 2])))))))));
                var_18 = ((((min((arr_1 [i_0 - 2]), var_4))) ? (!var_4) : (min((((arr_3 [i_0] [i_1] [i_1]) - var_9)), ((826190885 ? 17805 : var_14))))));
                var_19 ^= (arr_0 [i_0] [i_0]);
                arr_6 [i_0] [i_0] = (min(((((arr_4 [i_0]) || ((min(1, 0)))))), (min((arr_2 [i_0] [i_0]), (arr_2 [2] [i_0])))));
            }
        }
    }
    var_20 = (!var_4);
    #pragma endscop
}
