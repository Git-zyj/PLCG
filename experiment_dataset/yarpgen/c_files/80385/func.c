/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80385
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80385 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80385
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned long long int) var_13);
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_15 ^= ((/* implicit */unsigned char) (_Bool)1);
        arr_2 [i_0] [0LL] &= ((/* implicit */int) ((unsigned int) max((arr_1 [i_0]), (arr_1 [i_0]))));
    }
    for (signed char i_1 = 0; i_1 < 18; i_1 += 4) 
    {
        var_16 = ((/* implicit */signed char) ((((/* implicit */_Bool) (+(((((/* implicit */_Bool) var_11)) ? (((/* implicit */unsigned long long int) var_0)) : (13426108667935997194ULL)))))) || (((/* implicit */_Bool) ((unsigned char) ((((/* implicit */unsigned int) var_1)) == (var_5)))))));
        arr_6 [i_1] [i_1] = ((/* implicit */long long int) -1940782381);
    }
}
