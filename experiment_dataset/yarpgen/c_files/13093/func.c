/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 13093
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=13093 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/13093
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 ^= ((/* implicit */short) 281474943156224ULL);
    /* LoopNest 3 */
    for (long long int i_0 = 4; i_0 < 16; i_0 += 2) 
    {
        for (int i_1 = 3; i_1 < 17; i_1 += 1) 
        {
            for (int i_2 = 0; i_2 < 19; i_2 += 4) 
            {
                {
                    arr_7 [i_1] = (-(18446462598766395392ULL));
                    arr_8 [i_1] [i_1 - 2] [i_1] = ((/* implicit */unsigned long long int) arr_0 [i_0]);
                    var_13 = ((/* implicit */long long int) ((unsigned char) (-(((/* implicit */int) ((((/* implicit */int) arr_6 [i_0 - 3] [i_1] [i_2] [i_2])) <= (((/* implicit */int) var_6))))))));
                    var_14 = ((/* implicit */unsigned short) var_7);
                }
            } 
        } 
    } 
}
