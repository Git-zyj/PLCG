/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104123
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_2;
    var_20 = ((var_5 <= ((max(var_12, 1)))));
    var_21 = (((~1) ? (((((var_0 ? var_5 : -64))) ? var_3 : 1)) : ((~((var_12 ? 1 : var_8))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_22 = (116 == 1);
                var_23 = 63;
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 0;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_16 [i_3] [i_1] [18] = 40;
                                arr_17 [i_0] [i_1] [i_2] [i_1] [i_4 + 2] = 1;
                                arr_18 [i_1] = (arr_14 [i_2 + 1] [i_4 - 1] [i_4] [i_4] [2] [i_4]);
                            }
                        }
                    }
                }

                /* vectorizable */
                for (int i_5 = 0; i_5 < 1;i_5 += 1)
                {
                    var_24 = 6;
                    arr_21 [i_1] [i_1] [i_1] = 1;
                }
                arr_22 [i_0] [16] |= 1;
            }
        }
    }
    #pragma endscop
}
