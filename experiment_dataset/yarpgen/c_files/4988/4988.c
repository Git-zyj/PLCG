/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4988
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4988 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4988
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 *= (min(-0, 1));
    var_19 = (var_13 | var_13);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_20 = (var_1 >> ((((min(4055615679, 1))) ? ((max(1, 0))) : 1091931089)));
                    var_21 = (((((((min(var_16, 0)))) == (arr_8 [i_0] [i_1] [i_2] [i_2]))) ? ((max(1, 1))) : (max((arr_5 [i_0] [i_1] [i_2]), (arr_5 [i_0] [i_0] [i_0])))));
                }
            }
        }
    }
    var_22 = (~(max((max(1, var_11)), (0 || 1))));
    var_23 = 0;
    #pragma endscop
}
