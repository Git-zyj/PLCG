/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244899
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244899 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244899
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned char) max((var_16), (((/* implicit */unsigned char) var_1))));
    var_17 = var_2;
    /* LoopNest 3 */
    for (signed char i_0 = 3; i_0 < 23; i_0 += 3) 
    {
        for (int i_1 = 0; i_1 < 24; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 24; i_2 += 3) 
            {
                {
                    var_18 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) ((short) (signed char)-90))) ? (((((/* implicit */_Bool) ((signed char) (short)0))) ? (((/* implicit */int) arr_1 [i_0] [i_0 - 2])) : (((/* implicit */int) arr_6 [i_0] [i_0] [i_0])))) : (((((/* implicit */_Bool) ((signed char) -807960746))) ? (((((/* implicit */_Bool) var_10)) ? (((/* implicit */int) (signed char)-3)) : (((/* implicit */int) (unsigned char)40)))) : (((/* implicit */int) ((signed char) arr_4 [i_0] [i_1] [i_2])))))));
                    var_19 = var_8;
                }
            } 
        } 
    } 
}
