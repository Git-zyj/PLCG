/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76077
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76077 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76077
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((min(var_11, (!19)))) ? ((((!(1 * var_6))))) : var_1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((!(((var_9 + (-1691669657 + 0))))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_11 [i_1] [i_1] [i_1] [i_3] = ((!((min((arr_10 [i_0] [i_0] [i_2] [i_2] [i_4 - 1] [i_1]), var_8)))));
                                var_13 *= ((!(((-(((var_1 && (arr_9 [1] [1] [i_2] [i_1] [1] [i_0])))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
