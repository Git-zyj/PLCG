/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117726
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117726 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117726
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 3; i_0 < 9;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 7;i_2 += 1)
            {
                {
                    var_17 &= ((~((((min(4591, 60945))) ? -4 : (arr_6 [0] [0] [i_2 - 3] [i_2 - 3])))));
                    var_18 = (min(var_18, (((((((!var_4) - var_7))) ? 1 : ((var_16 - (arr_3 [i_1] [i_1] [i_2]))))))));
                    var_19 -= (min((~var_9), (arr_5 [i_0 - 2])));
                }
            }
        }
        var_20 = (~var_0);
        arr_9 [i_0] = (arr_6 [i_0] [i_0] [i_0] [i_0]);
    }
    var_21 = ((((((((var_16 ? var_9 : var_3))) ? var_1 : var_1))) ? (max(-18770, var_5)) : -8323046451480013669));
    #pragma endscop
}
