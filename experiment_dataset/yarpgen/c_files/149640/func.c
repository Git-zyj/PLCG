/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149640
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149640 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149640
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 ^= ((/* implicit */unsigned long long int) var_11);
    var_19 = ((/* implicit */_Bool) (signed char)(-127 - 1));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 3) 
    {
        var_20 += var_17;
        var_21 *= ((/* implicit */unsigned char) ((_Bool) (-(arr_1 [i_0]))));
        arr_3 [i_0] &= ((/* implicit */signed char) ((((/* implicit */_Bool) (signed char)-16)) ? (arr_0 [i_0] [i_0]) : (arr_0 [i_0] [i_0])));
        var_22 = ((/* implicit */unsigned int) (+(arr_0 [i_0] [i_0])));
    }
    var_23 = ((/* implicit */unsigned long long int) max((var_23), (((/* implicit */unsigned long long int) ((134217726) >= (((/* implicit */int) var_7)))))));
}
