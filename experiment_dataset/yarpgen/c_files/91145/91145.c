/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91145
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91145 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91145
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [3] = (((((max(var_3, (arr_1 [i_0])))) ? (arr_3 [i_0] [i_0]) : ((var_1 ? var_7 : (arr_0 [i_0]))))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [12] [i_2] [i_3] [i_3] [i_0] [i_0] = var_1;
                                var_17 *= (~var_10);
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 = var_16;
    var_19 = (((min(((-750036647 ? var_8 : 9223372036854775807)), ((var_3 << (var_7 + 97))))) == (min(var_2, (max(var_2, var_15))))));
    #pragma endscop
}
