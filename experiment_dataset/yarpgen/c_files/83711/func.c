/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83711
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 4) 
    {
        for (unsigned short i_1 = 2; i_1 < 21; i_1 += 3) 
        {
            {
                var_12 -= ((13283787426036797864ULL) * (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)0))));
                arr_4 [i_0] [i_1] = ((/* implicit */unsigned int) arr_0 [i_1]);
                arr_5 [i_0] [i_1] = ((/* implicit */signed char) (+(var_1)));
            }
        } 
    } 
    var_13 += ((/* implicit */short) (-(16368962961028481200ULL)));
}
