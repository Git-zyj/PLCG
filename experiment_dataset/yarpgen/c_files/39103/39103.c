/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39103
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39103 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39103
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((min(9099, 452746077))) ? (~-4247441582810010936) : var_14));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        var_18 = (min(var_18, ((-3058 ? (arr_2 [i_0] [i_0] [i_3]) : (arr_2 [i_0] [i_1] [10])))));
                        arr_9 [i_0] [1] [9] [i_1] [i_3] [i_3] = ((1 == (((arr_5 [i_1]) ? (arr_8 [i_0] [i_0] [i_1] [i_2] [i_3] [i_3]) : var_8))));
                        var_19 = var_6;
                    }
                }
            }
        }
        arr_10 [i_0] = ((!(arr_3 [i_0])));

        for (int i_4 = 2; i_4 < 17;i_4 += 1)
        {
            arr_13 [i_4] = ((!(arr_1 [i_4 - 1] [i_4 - 1])));
            var_20 = var_11;
            var_21 ^= (!-3843);
        }
    }
    #pragma endscop
}
