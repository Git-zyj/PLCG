/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181711
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181711 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181711
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 &= var_9;
    var_12 = var_0;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = 1767752949268571496;
        arr_3 [i_0] = (arr_1 [13]);
    }
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        arr_8 [i_1] = (max((arr_7 [i_1] [i_1]), ((((arr_5 [i_1]) < ((max(var_9, 206))))))));
        arr_9 [21] = (((arr_6 [i_1]) ? (arr_6 [i_1]) : ((var_8 ? var_4 : (arr_6 [i_1])))));
    }
    var_13 = (max(var_13, ((var_6 >> ((((max(var_0, 52))) - 491089990))))));
    #pragma endscop
}
