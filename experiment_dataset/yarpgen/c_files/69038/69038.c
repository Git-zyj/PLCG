/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69038
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69038 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69038
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((var_4 * (!var_15))) == (~65535)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_17 = (max(var_17, ((min(((20 ? 1752979632 : 18446744073709551608)), var_6)))));
                arr_5 [i_0] [i_0] [i_1] = (min(((-(arr_1 [4] [i_1]))), (((arr_1 [i_0] [i_1]) ? (arr_1 [i_1] [i_1]) : (arr_1 [i_0] [i_1])))));
            }
        }
    }
    #pragma endscop
}
