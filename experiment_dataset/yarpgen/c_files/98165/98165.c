/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98165
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98165 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98165
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] = ((((max((min(15433, 159)), (max(var_9, -15844))))) >= (max((arr_1 [i_0]), (((arr_1 [i_0]) ? (arr_1 [i_0]) : -987))))));
        arr_3 [i_0] = ((((((arr_1 [i_0]) & (3419648097 & -9223372036854775785)))) ? (min((arr_0 [i_0]), var_4)) : ((-((-(arr_0 [i_0])))))));
    }
    /* LoopNest 2 */
    for (int i_1 = 3; i_1 < 24;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 24;i_2 += 1)
        {
            {
                var_19 = 554165955437968706;
                arr_8 [i_1 - 1] [1] = -9223372036854775784;
            }
        }
    }
    var_20 = (max(var_20, var_9));
    var_21 = 9;
    #pragma endscop
}
