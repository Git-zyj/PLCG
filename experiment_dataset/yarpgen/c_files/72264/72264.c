/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72264
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72264 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72264
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 -= (max(((4503569505911855601 ? 0 : -1)), (~-var_13)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = (((!var_0) ? (~108) : (arr_5 [i_1] [i_1 - 1])));
                arr_7 [i_1] = (((((-((var_17 ? var_6 : (arr_5 [i_0] [i_0])))))) ? (arr_1 [i_1]) : ((~(arr_5 [i_1 + 1] [i_1 - 1])))));
                arr_8 [i_1] [i_1] [i_0] = (arr_2 [i_0]);
            }
        }
    }
    #pragma endscop
}
