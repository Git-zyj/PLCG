/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((var_0 + (max((!63), var_1))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            arr_10 [i_1] = -6760884998853036793;
                            arr_11 [i_1] [i_2] [i_0] = -1816559817;
                            arr_12 [i_0] [i_0] [i_0 - 2] [i_2] [i_0 + 1] = var_2;
                        }
                    }
                }
                arr_13 [i_1] = (min((71 <= var_7), (((var_8 % (arr_5 [i_0] [i_1 + 2]))))));
            }
        }
    }
    #pragma endscop
}
