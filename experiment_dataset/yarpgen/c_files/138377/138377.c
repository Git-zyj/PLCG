/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138377
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138377 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138377
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((-61 > -35) ? ((((6441083328499431444 ? 17159811527735165658 : var_13)) / var_6)) : ((var_12 ? var_4 : var_9))));

    for (int i_0 = 2; i_0 < 23;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((min((arr_0 [i_0 - 1]), var_12)));
        arr_3 [i_0] = var_4;
    }
    for (int i_1 = 2; i_1 < 23;i_1 += 1) /* same iter space */
    {
        var_15 = (((56 - ((6441083328499431444 ? 16256 : var_4)))));
        arr_8 [23] = (max((((1405952194 ? 4369 : 62))), (max((arr_5 [i_1 - 2] [i_1 - 2]), (arr_4 [i_1 - 2] [i_1 - 2])))));
    }
    #pragma endscop
}
