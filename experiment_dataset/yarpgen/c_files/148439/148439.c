/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148439
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148439 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148439
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_10;
    var_21 = var_17;

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_3 [i_0] = (max((((~-4744916646974748264) ? ((max(3, -6))) : 31)), 224));
        arr_4 [i_0] = (max((arr_1 [i_0] [i_0]), var_13));
        arr_5 [i_0] = (((arr_2 [i_0]) ? (min(-562949953421311, (((arr_1 [i_0] [i_0]) * -6)))) : (((((var_18 ? 6 : (arr_0 [i_0]))) * 9)))));
    }
    var_22 = var_15;
    #pragma endscop
}
