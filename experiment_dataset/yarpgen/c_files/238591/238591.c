/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238591
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238591 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238591
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += 10087083744711552184;
    var_20 &= var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_21 = (min(var_21, 4211263847465000922));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_22 += (min((((1 ? -252828799 : var_9))), (((arr_13 [i_0] [i_1] [i_4 - 1] [i_3] [2]) >> (((arr_2 [i_4 + 1] [i_4 - 1]) - 1582923786))))));
                                var_23 = (min(var_23, 16452));
                            }
                        }
                    }
                    var_24 &= 28;
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 19;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 18;i_6 += 1)
                        {
                            {
                                var_25 = var_1;
                                var_26 = var_3;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
