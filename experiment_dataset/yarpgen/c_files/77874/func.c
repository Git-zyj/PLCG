/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77874
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77874 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77874
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */unsigned short) (_Bool)1);
    var_19 = ((/* implicit */unsigned short) ((unsigned int) 0ULL));
    var_20 = ((/* implicit */unsigned int) ((((((/* implicit */unsigned long long int) ((var_1) ^ (((/* implicit */unsigned int) ((/* implicit */int) var_8)))))) <= ((+(var_6))))) ? (((/* implicit */unsigned long long int) 3336396985U)) : (((((18446744073709551614ULL) & (7447453293011652612ULL))) & (max((((/* implicit */unsigned long long int) (_Bool)1)), (var_6)))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 1; i_0 < 13; i_0 += 1) 
    {
        for (short i_1 = 2; i_1 < 13; i_1 += 4) 
        {
            {
                var_21 ^= ((((/* implicit */_Bool) var_12)) || (((/* implicit */_Bool) 7079715886978166770ULL)));
                var_22 = ((/* implicit */short) min((var_22), (((/* implicit */short) arr_0 [i_0]))));
                var_23 = ((/* implicit */unsigned int) max((var_23), (((/* implicit */unsigned int) var_17))));
            }
        } 
    } 
}
