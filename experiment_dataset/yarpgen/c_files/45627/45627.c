/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45627
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45627 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45627
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] = ((((!((!(arr_0 [i_0])))))));
        var_15 = (max(((max(1, (((arr_1 [i_0]) - 4242263633))))), 17292830322593922758));
    }
    var_16 = (((-1 ? 1 : -18859)));
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            {
                arr_7 [i_2] = (arr_3 [i_2] [2]);
                var_17 = (min(var_17, (((1 * 0) <= ((((((arr_0 [i_2]) | (arr_1 [i_2])))) ? ((1153913751115628858 | (arr_4 [i_1] [i_1]))) : (arr_4 [i_2] [i_1])))))));
            }
        }
    }
    var_18 = var_14;
    #pragma endscop
}
