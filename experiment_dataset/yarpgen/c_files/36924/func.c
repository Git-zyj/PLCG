/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36924
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36924 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36924
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        for (unsigned char i_1 = 3; i_1 < 19; i_1 += 4) 
        {
            {
                arr_5 [10U] [(unsigned short)18] = ((/* implicit */int) (unsigned char)243);
                arr_6 [4] [i_1] [i_0] = (+((-(((((/* implicit */_Bool) (unsigned short)15291)) ? (((/* implicit */int) (_Bool)0)) : (2147483647))))));
                arr_7 [8] = ((/* implicit */unsigned long long int) ((((/* implicit */int) var_8)) | (((/* implicit */int) ((_Bool) (unsigned char)253)))));
            }
        } 
    } 
    var_14 = ((/* implicit */short) var_10);
}
