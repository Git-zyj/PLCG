/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73817
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73817 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73817
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((((((0 ? (-127 - 1) : 96))) ? (var_4 != -268435456) : (min(var_6, var_2)))) * (9223372036854775807 || var_3)));
    var_12 |= var_5;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 20;i_3 += 1)
                    {
                        {
                            var_13 = 46492;
                            var_14 = (max(262143, 871928215144296869));
                        }
                    }
                }
                var_15 = (min((-9223372036854775807 - 1), 127));
                var_16 ^= (arr_10 [11] [3] [i_1] [i_1] [i_1]);
            }
        }
    }
    var_17 = var_0;
    #pragma endscop
}
