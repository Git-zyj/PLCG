/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28277
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28277 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28277
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = (max(((((min(1, var_4))) * var_4)), (1 | -4178)));
    var_16 = var_4;
    var_17 = ((var_6 >> (var_13 - 15953374815774618896)));
    var_18 = (-(((3780019874640292887 + var_2) + var_7)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_6 [i_0] [1] [i_0] = (min((max(((~(arr_5 [i_0] [i_0] [1]))), var_7)), var_0));
                arr_7 [i_0] [i_1] [i_1] = (arr_2 [i_0]);
            }
        }
    }
    #pragma endscop
}
