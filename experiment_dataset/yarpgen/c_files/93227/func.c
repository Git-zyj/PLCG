/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93227
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93227 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93227
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = (signed char)0;
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 24; i_0 += 3) 
    {
        for (signed char i_1 = 0; i_1 < 24; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 24; i_2 += 2) 
            {
                {
                    var_17 = ((/* implicit */signed char) ((((/* implicit */_Bool) arr_0 [i_0])) ? (((/* implicit */int) var_0)) : (((/* implicit */int) (signed char)(-127 - 1)))));
                    arr_8 [i_0] [i_1] [i_0] = min(((signed char)59), (arr_1 [i_0]));
                }
            } 
        } 
    } 
    var_18 &= (signed char)-96;
    var_19 = min((var_15), (((signed char) (signed char)(-127 - 1))));
}
