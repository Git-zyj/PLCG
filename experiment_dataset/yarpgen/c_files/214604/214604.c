/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214604
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214604 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214604
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 24;i_4 += 1)
                        {
                            {
                                arr_17 [i_0] [i_0] [i_2] = var_3;
                                var_14 = (((((arr_1 [i_1]) ? var_3 : 1))));
                                var_15 = (max(var_15, (((-(448453587083095432 / -var_1))))));
                            }
                        }
                    }
                    var_16 = var_8;
                    var_17 = (max((((!((((arr_4 [i_1] [i_1] [i_1]) ? var_9 : var_5)))))), ((75 ? 38075 : (arr_1 [i_0])))));
                }
            }
        }
    }
    var_18 = (((((min(0, var_1)) >= 12619)) ? 5676262730830098730 : var_0));
    #pragma endscop
}
