/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99578
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99578 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99578
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = var_8;
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    var_16 ^= max((arr_6 [i_2] [1] [i_0]), ((((arr_3 [i_0]) ? var_2 : var_10))));
                    arr_11 [i_1] [i_0] = ((var_14 * (min((arr_3 [i_1]), (((arr_3 [13]) ? var_2 : 1))))));
                }
            }
        }
    }
    var_17 = (((((var_4 ? (319552843 - var_13) : (((var_6 ? 1 : 1)))))) ? var_14 : (((1 ? var_13 : var_2)))));
    var_18 = var_6;
    var_19 += (max(((((-32767 - 1) > 1))), 1));
    var_20 = var_13;
    #pragma endscop
}
