/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172227
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172227 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172227
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max((!14203868959804245271), var_17));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_0] [16] [i_0] = var_14;
                    var_21 = (max(var_21, (((var_6 >> (((max(var_19, (((-9223372036854775807 - 1) ? (-127 - 1) : 262143)))) - 4294967147)))))));

                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        var_22 ^= (!-var_19);
                        arr_9 [3] [i_1] [i_1] [21] [i_1] = ((((65535 ? 109 : 2505278545)) != ((((!(arr_7 [i_0] [i_0] [i_1] [i_2] [0] [i_3])))))));
                    }
                }
            }
        }
    }
    #pragma endscop
}
