/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45671
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45671 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45671
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((var_9 < (((-1796549280 ? -617914418 : 2147483644))))) ? var_5 : var_9));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_12 = (max((617914417 < var_4), ((-(arr_2 [i_0])))));
                    var_13 -= (min((arr_7 [i_0] [i_2]), var_9));
                    var_14 |= var_6;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 22;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 22;i_4 += 1)
        {
            {
                var_15 = ((((min(var_3, (arr_4 [i_3])))) ? ((~(arr_4 [i_4]))) : ((min(var_6, var_1)))));
                var_16 *= 2147483645;
            }
        }
    }
    #pragma endscop
}
