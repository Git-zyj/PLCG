/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 203664
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=203664 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/203664
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 16; i_0 += 3) 
    {
        for (int i_1 = 4; i_1 < 13; i_1 += 3) 
        {
            for (int i_2 = 4; i_2 < 13; i_2 += 1) 
            {
                {
                    var_20 = ((/* implicit */long long int) min((var_20), (((/* implicit */long long int) max(((-(var_13))), ((-(((/* implicit */int) arr_10 [i_0] [(_Bool)1] [i_2])))))))));
                    arr_11 [i_0] [i_0] [i_2] = ((/* implicit */int) (short)26961);
                }
            } 
        } 
    } 
    var_21 = ((/* implicit */short) min((var_21), (((/* implicit */short) var_18))));
}
