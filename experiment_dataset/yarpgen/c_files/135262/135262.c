/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135262
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135262 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135262
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((min(var_3, 105))) && (((var_4 ? ((min(2029245195, var_10))) : ((211 ? 6574572860146632389 : 1)))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (max(((((arr_3 [i_0] [1] [i_0]) < ((var_4 ? 0 : var_10))))), var_3));
                arr_6 [i_0 + 2] [i_1] [6] = (max(((((arr_3 [9] [9] [i_0 + 3]) == var_1))), (((~-118) - ((var_0 & (arr_3 [i_0] [i_0] [i_1])))))));
                var_16 ^= 0;
            }
        }
    }
    var_17 = var_14;
    #pragma endscop
}
