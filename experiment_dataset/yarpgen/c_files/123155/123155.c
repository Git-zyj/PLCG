/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123155
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123155 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123155
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_9;
    var_13 = (10515464577545166806 ? var_2 : 1);
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_14 = (max(var_14, ((((((arr_5 [i_0 - 1] [i_0 - 1] [2] [i_4]) / (arr_5 [i_0 - 1] [i_1] [i_2] [i_4]))) % (arr_5 [i_0 - 1] [i_1] [i_0 - 1] [i_1]))))));
                                var_15 = var_5;
                                var_16 += (arr_0 [i_4]);
                            }
                        }
                    }
                    var_17 *= (arr_8 [i_1]);
                    var_18 += (((arr_8 [i_2]) / ((-(~-2549403458078530504)))));
                }
            }
        }
    }
    #pragma endscop
}
