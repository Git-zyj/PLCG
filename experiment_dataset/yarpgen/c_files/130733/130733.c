/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130733
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130733 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130733
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, ((((min(var_8, (var_3 - 127))) << (((max(var_8, (((131 ? 131 : 1))))) - 517506193853446607)))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_11 += (((arr_1 [i_0]) ? (~var_7) : (~18446744073709551615)));
        arr_2 [19] [i_0] = var_1;
        var_12 -= (arr_1 [i_0]);
    }
    for (int i_1 = 3; i_1 < 14;i_1 += 1)
    {
        arr_6 [i_1] [i_1 + 1] = (arr_0 [i_1]);
        arr_7 [i_1] = (max(var_5, (10 / 18446744073709551611)));
    }
    var_13 = var_1;
    #pragma endscop
}
