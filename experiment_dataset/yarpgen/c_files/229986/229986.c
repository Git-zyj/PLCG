/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229986
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229986 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229986
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_10;
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [i_1] = var_11;
                var_17 = ((((((min(var_0, var_12))) ? (max(-24601, var_13)) : (arr_3 [i_0 + 1] [i_1] [i_1]))) - ((((((var_5 ? (arr_3 [i_0] [i_0] [i_0]) : (arr_1 [i_1 - 4]))) != var_12))))));
                var_18 = (!-24582);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 7;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 10;i_3 += 1)
        {
            {
                var_19 = (((((max((arr_0 [i_2 + 2]), var_2)))) | (((-(arr_2 [i_2] [i_2]))))));
                var_20 = (((((var_7 | (arr_4 [i_2 + 3] [i_2 + 3])))) ? (~32767) : ((var_12 * (arr_4 [i_2 - 1] [i_3])))));
            }
        }
    }
    #pragma endscop
}
