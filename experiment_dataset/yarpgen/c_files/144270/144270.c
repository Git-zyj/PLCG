/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144270
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144270 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144270
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_18 |= ((((159 << (((((-2147483647 - 1) - -2147483621)) + 32)))) >> ((((-164783792 | (arr_5 [i_0] [i_1]))) + 152182813))));
                var_19 += (~(((!(arr_3 [i_0] [i_1] [i_0])))));
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        {
                            arr_13 [i_1] [i_1] |= arr_9 [i_0] [i_1] [i_1];
                            var_20 = (max(var_20, ((~(!var_15)))));
                            var_21 = max((+-250897305), ((((arr_3 [0] [i_1] [i_2]) != (((arr_2 [0] [6] [3]) ? -250897324 : (arr_0 [i_2])))))));
                            var_22 = var_13;
                        }
                    }
                }
            }
        }
    }
    var_23 = ((max(var_7, var_16)));

    for (int i_4 = 0; i_4 < 24;i_4 += 1)
    {
        var_24 = (arr_15 [i_4]);
        arr_17 [i_4] = (((arr_14 [i_4] [i_4]) / (((arr_14 [i_4] [i_4]) / (arr_14 [i_4] [i_4])))));
        var_25 = (max(var_25, (~var_4)));
        var_26 *= (((76 && 148) > ((((arr_16 [i_4]) || (arr_15 [i_4]))))));
    }
    #pragma endscop
}
