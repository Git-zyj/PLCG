/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217537
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217537 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217537
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = ((!(((((min(90, (arr_1 [i_0])))) ? (arr_0 [i_0]) : (arr_3 [i_1] [i_1]))))));
                var_19 ^= (((((~(arr_4 [i_1] [i_0] [i_0])))) ? (((arr_4 [i_0] [i_0] [i_1]) % (arr_4 [i_0] [i_1] [i_1]))) : (arr_4 [i_1] [i_1] [i_1])));
                var_20 *= (arr_0 [i_1]);
                var_21 ^= (min((((arr_4 [i_0] [i_1] [0]) ? (((arr_1 [3]) + (arr_2 [17] [i_0]))) : 85)), (~1409351298)));
            }
        }
    }
    #pragma endscop
}
