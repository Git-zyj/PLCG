/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217768
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 1; i_0 < 22; i_0 += 4) 
    {
        var_13 += ((/* implicit */unsigned int) (+(arr_1 [i_0 - 1] [i_0 + 2])));
        var_14 = min(((~(arr_2 [i_0 + 2]))), (((/* implicit */int) min((((/* implicit */unsigned short) (_Bool)1)), ((unsigned short)15605)))));
        var_15 = ((((/* implicit */long long int) 1873975978)) % (((((/* implicit */_Bool) (short)-26688)) ? (((/* implicit */long long int) -1873975992)) : (-306613672572091077LL))));
    }
    var_16 = ((/* implicit */signed char) var_10);
}
