/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218407
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218407 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218407
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 -= var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_13 = ((((max(((761074455 ? (arr_0 [i_0]) : var_11)), -170))) & ((373745911 ? 3533892841 : 761074455))));
                arr_4 [i_1] [i_1] [i_1] = 307643869;
                arr_5 [i_1] = 0;
                arr_6 [i_1] = (max((((1 ? 3771840599246028257 : 105))), ((max((var_11 || 1), 394167085)))));
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        var_14 = 1;
        arr_10 [i_2] = (arr_8 [i_2]);
    }
    #pragma endscop
}
