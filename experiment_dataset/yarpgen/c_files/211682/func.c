/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211682
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211682 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211682
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */signed char) (short)32767);
    var_14 |= ((/* implicit */int) (signed char)-75);
    /* LoopSeq 1 */
    for (signed char i_0 = 1; i_0 < 18; i_0 += 3) 
    {
        arr_3 [i_0] = ((/* implicit */short) (unsigned char)2);
        var_15 = ((/* implicit */unsigned int) (signed char)127);
        var_16 = ((/* implicit */int) (short)-15717);
        var_17 = ((/* implicit */unsigned int) (unsigned short)54765);
        var_18 = ((/* implicit */short) 2726360508U);
    }
}
