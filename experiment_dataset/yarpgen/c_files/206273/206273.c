/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206273
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206273 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206273
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((((((((!(-2147483647 - 1)))) >> ((((167 ? 82546782 : -1285889579)) - 82546754))))) ? (min((max(4187809746, var_4)), var_5)) : (!var_11)));
    var_14 = (!-63814);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    var_15 = (((arr_4 [i_0] [i_1] [i_0 - 1]) | 826032201));
                    arr_8 [i_0] [13] [i_1] [i_2] = arr_2 [i_0];
                }
                arr_9 [i_0] = 4187809749;
            }
        }
    }
    #pragma endscop
}
