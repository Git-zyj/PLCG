/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152291
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152291 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152291
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            {
                arr_8 [i_1 - 1] [i_1 - 1] [i_0] = ((((max(22489, var_14))) - ((-(arr_7 [i_1 + 1] [i_1 + 1])))));
                arr_9 [i_0 + 1] [i_0 + 1] = ((((arr_4 [i_0 - 1] [i_0 - 1] [i_0]) ? var_14 : 13365)));
                var_18 = (min((-6537 + -15), (max(((14756 ? -9285 : var_3)), 1792))));
            }
        }
    }
    var_19 = (max(var_19, var_10));
    #pragma endscop
}
