/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143130
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143130 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143130
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 10;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [i_2] = ((+(((arr_5 [i_2 + 1] [i_1] [i_0] [i_0]) ? (arr_5 [i_2 + 2] [i_2 + 2] [i_0] [i_0]) : (arr_5 [i_2 + 2] [i_1] [i_0] [i_2 - 1])))));
                    var_18 ^= ((arr_3 [i_0]) ? (((var_11 - var_5) ? ((24 ? 11135449246778227439 : 33)) : 1)) : ((((arr_1 [10]) / (arr_4 [i_0] [i_1])))));
                }
            }
        }
    }
    var_19 = 10;
    #pragma endscop
}
