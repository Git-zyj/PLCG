/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135466
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135466 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135466
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 25; i_0 += 2) 
    {
        for (unsigned char i_1 = 0; i_1 < 25; i_1 += 2) 
        {
            {
                arr_6 [i_1] = ((/* implicit */int) var_2);
                var_10 = ((/* implicit */unsigned int) min((var_10), (max((min((arr_2 [i_0]), (arr_2 [i_0]))), (max((((/* implicit */unsigned int) 304592807)), (arr_2 [i_0])))))));
            }
        } 
    } 
    var_11 = ((/* implicit */unsigned long long int) -304592807);
}
