/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [1] [1] [i_0] = ((-7280448823244715726 - (0 > var_9)));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 14;i_4 += 1)
                        {
                            {
                                arr_12 [i_4] [i_0] [i_2] [i_0] [i_0] = (arr_10 [i_4 + 1] [i_4] [i_4 - 1] [i_4] [i_4] [i_0]);
                                var_16 = (min((((-1626344076 > 144) >> (-9953 + 9968))), -55573));
                                arr_13 [i_0] [i_1] [i_1] [i_3] [i_4] &= (!var_14);
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 *= 9953;
    var_18 = var_2;
    #pragma endscop
}
