/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227387
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227387 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227387
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */long long int) var_8);
    var_14 = ((/* implicit */long long int) var_12);
    var_15 = var_1;
    var_16 += ((/* implicit */int) var_6);
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (unsigned short i_2 = 1; i_2 < 12; i_2 += 4) 
            {
                {
                    var_17 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) var_4)) || (((/* implicit */_Bool) ((((/* implicit */_Bool) (((_Bool)1) ? (15304477099936730335ULL) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_11)))))) ? (0U) : (min((0U), (((/* implicit */unsigned int) var_4)))))))));
                    arr_7 [i_0] [i_2 + 2] |= ((/* implicit */unsigned char) arr_4 [6ULL] [i_1] [i_2 - 1]);
                    arr_8 [i_1] = ((/* implicit */unsigned long long int) (unsigned char)64);
                }
            } 
        } 
    } 
}
