/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99828
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99828 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99828
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_6;
    var_15 = ((4144958219 ? 9309434292400060180 : 1637958970));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_16 &= (!var_0);
                    var_17 *= 9309434292400060192;
                    arr_8 [i_0] [i_0] [3] = ((!(arr_2 [i_0])));
                }
            }
        }
    }
    var_18 = ((!(((9309434292400060175 ? (-5771006484024092291 >= var_1) : (min(var_5, var_0)))))));
    #pragma endscop
}
