/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                {
                    var_14 = ((((max((min((arr_1 [i_1] [i_0]), var_5)), (((var_2 > (arr_3 [i_1]))))))) ? 8489193767842836480 : (3248703104451402978 || 1418582796)));
                    arr_8 [i_0] [i_1] [i_2] = (((((arr_5 [i_0 + 2]) ? 3001096446 : (arr_5 [i_0 + 2]))) != (((max(2141075844, var_8))))));
                }
            }
        }
    }
    var_15 = var_2;
    #pragma endscop
}
