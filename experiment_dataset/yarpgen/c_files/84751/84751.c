/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 84751
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=84751 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/84751
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = var_11;
                var_15 = ((((max((((arr_2 [i_1]) ? var_7 : var_11)), var_0))) || (arr_3 [i_0] [i_0] [i_1 - 2])));
            }
        }
    }
    var_16 = var_0;
    var_17 -= (var_12 ? (!var_10) : (((var_6 && var_1) ? (min(5523612129730716554, var_5)) : var_8)));
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 25;i_3 += 1)
        {
            {
                var_18 = arr_10 [i_3] [i_3] [i_2 - 1];
                arr_11 [i_2 - 1] [i_3] = (((arr_8 [i_2 - 1]) & ((((!(arr_9 [1] [i_3] [1])))))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_4 = 0; i_4 < 20;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 20;i_5 += 1)
        {
            for (int i_6 = 0; i_6 < 20;i_6 += 1)
            {
                {
                    var_19 |= arr_6 [i_5];
                    var_20 &= var_2;
                    var_21 = (((((arr_10 [i_4] [i_5] [i_6]) ? (arr_10 [i_6] [i_5] [i_4]) : (arr_10 [i_4] [i_5] [i_4]))) >> (-var_9 - 48190014)));
                }
            }
        }
    }
    #pragma endscop
}
