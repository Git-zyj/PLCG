/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202963
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202963 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202963
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        arr_3 [i_0] = ((/* implicit */int) ((((/* implicit */unsigned long long int) ((/* implicit */int) ((_Bool) ((long long int) 18446744073709551615ULL))))) - (min((max((var_9), (((/* implicit */unsigned long long int) (signed char)72)))), (((/* implicit */unsigned long long int) (~(3205710916U))))))));
        var_11 += ((/* implicit */_Bool) var_6);
    }
    var_12 += ((/* implicit */long long int) (_Bool)1);
}
