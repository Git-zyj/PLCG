/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87441
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87441 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87441
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((-((max(var_0, (var_7 < var_3))))));
    var_17 = (~var_10);

    for (int i_0 = 3; i_0 < 15;i_0 += 1)
    {
        arr_2 [16] [i_0] = (var_15 ? (var_7 ^ 221) : (min((((220 | (arr_0 [i_0] [i_0 + 2])))), (min((arr_0 [i_0] [i_0]), 18446744073709551612)))));
        var_18 = (max(var_18, ((((arr_1 [10] [i_0]) >= ((((max((arr_0 [14] [i_0]), var_14))) ? ((16279029277002548479 ? (arr_0 [0] [i_0]) : (arr_0 [2] [2]))) : ((max(185, (arr_1 [8] [1])))))))))));
    }
    var_19 = (max(var_19, var_10));
    var_20 = var_7;
    #pragma endscop
}
