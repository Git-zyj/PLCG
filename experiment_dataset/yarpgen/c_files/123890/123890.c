/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 15;i_4 += 1)
                        {
                            {
                                var_20 = ((((((((87 ? 9068410869158679936 : 65))) ? 9068410869158679936 : (var_11 <= 95)))) ? ((114 ? -57 : -3174172285720302210)) : 9223372036854775807));
                                var_21 = 18090;
                                arr_11 [i_0] [12] [i_0] = 1;
                                arr_12 [i_1] = var_3;
                            }
                        }
                    }
                    arr_13 [i_1] = 1;
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_5 = 2; i_5 < 10;i_5 += 1)
    {
        for (int i_6 = 2; i_6 < 12;i_6 += 1)
        {
            for (int i_7 = 1; i_7 < 12;i_7 += 1)
            {
                {
                    var_22 = -9068410869158679930;
                    var_23 -= 2997722029657069319;
                    arr_21 [6] &= 1;
                }
            }
        }
    }
    #pragma endscop
}
