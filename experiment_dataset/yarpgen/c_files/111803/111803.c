/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111803
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((!((min((max(var_1, 1)), -26217)))));
    var_19 = var_6;
    var_20 = 18446744073709551606;

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] = (((max((max((arr_1 [14]), (arr_0 [i_0] [i_0]))), (!1))) >> (((!(arr_1 [i_0]))))));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    arr_8 [i_1] = ((((((((arr_7 [i_0] [i_0] [19] [19]) || var_1)) ? 123 : (~26188)))) ? (arr_6 [i_1]) : ((((26216 / (arr_5 [i_0] [i_1] [i_2 + 1])))))));
                    var_21 = ((-(min((arr_6 [i_0]), (arr_1 [i_1])))));
                }
            }
        }
    }
    #pragma endscop
}
