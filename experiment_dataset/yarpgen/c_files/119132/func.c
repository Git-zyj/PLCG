/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119132
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        for (unsigned char i_1 = 0; i_1 < 11; i_1 += 4) 
        {
            for (unsigned int i_2 = 0; i_2 < 11; i_2 += 2) 
            {
                {
                    var_17 = ((/* implicit */unsigned char) max((var_17), (((/* implicit */unsigned char) var_14))));
                    var_18 = ((/* implicit */unsigned long long int) min((var_18), (max((max((((/* implicit */unsigned long long int) arr_2 [i_0])), (var_9))), (((/* implicit */unsigned long long int) min((4294967292U), (((/* implicit */unsigned int) arr_2 [i_0])))))))));
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */unsigned char) ((signed char) var_0));
    var_20 = ((/* implicit */_Bool) ((long long int) 21U));
}
