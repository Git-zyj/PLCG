/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247958
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247958 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247958
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (arr_1 [i_0]);
        arr_3 [i_0] [i_0] = (112 * 119);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_16 = (((arr_5 [i_1]) | (arr_5 [i_1])));
        /* LoopNest 3 */
        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 15;i_3 += 1)
            {
                for (int i_4 = 3; i_4 < 14;i_4 += 1)
                {
                    {
                        arr_12 [i_1] [i_1] [12] [i_4] = (!(1901819972 || var_0));
                        arr_13 [i_1] [14] [i_3] [i_4] = ((3992556236569359954 ? 127 : 119));
                    }
                }
            }
        }
    }
    var_17 = (max(((-7 ? var_15 : (var_4 > var_2))), ((max(var_11, var_6)))));
    var_18 = var_1;
    #pragma endscop
}
