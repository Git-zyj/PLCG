/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224617
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224617 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224617
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = 15084728206247942241;

    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        var_15 += (((arr_0 [i_0 + 1]) | ((~(arr_1 [i_0])))));
        var_16 *= 7299496711270822621;

        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            var_17 = ((((((arr_0 [i_0 + 2]) - (arr_0 [i_0 + 1])))) * (((arr_3 [i_0 - 2] [i_0 - 2]) + (arr_0 [i_0 + 2])))));
            /* LoopNest 2 */
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 16;i_3 += 1)
                {
                    {
                        var_18 = (arr_2 [i_3 - 2]);
                        var_19 = (arr_2 [i_0 - 1]);
                    }
                }
            }
        }
        var_20 -= 193663526405705741;
    }
    var_21 = (max(var_21, (var_12 ^ var_9)));
    #pragma endscop
}
