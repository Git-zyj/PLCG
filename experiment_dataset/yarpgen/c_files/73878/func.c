/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73878
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73878 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73878
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 4; i_0 < 18; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 20; i_1 += 3) 
        {
            for (unsigned short i_2 = 1; i_2 < 19; i_2 += 2) 
            {
                {
                    arr_8 [i_0] [i_1] [i_2 - 1] &= ((/* implicit */short) (unsigned char)15);
                    /* LoopNest 2 */
                    for (long long int i_3 = 3; i_3 < 17; i_3 += 3) 
                    {
                        for (unsigned long long int i_4 = 0; i_4 < 20; i_4 += 3) 
                        {
                            {
                                arr_16 [i_2] = (!(((/* implicit */_Bool) (unsigned char)15)));
                                var_14 = ((/* implicit */short) min((var_14), (((/* implicit */short) var_1))));
                                var_15 = ((/* implicit */_Bool) (+(((/* implicit */int) var_0))));
                                var_16 -= ((/* implicit */_Bool) var_1);
                            }
                        } 
                    } 
                    arr_17 [i_0] [i_0] [i_0] [i_0] |= ((/* implicit */unsigned short) (+(((((/* implicit */_Bool) var_8)) ? (((((/* implicit */_Bool) var_2)) ? (((/* implicit */int) (_Bool)1)) : (((/* implicit */int) var_13)))) : (((((/* implicit */_Bool) var_5)) ? (((/* implicit */int) var_5)) : (((/* implicit */int) var_3))))))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned char) min((var_17), (((/* implicit */unsigned char) (~(((/* implicit */int) var_11)))))));
}
