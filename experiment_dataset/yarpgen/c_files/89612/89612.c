/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89612
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89612 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89612
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (min(var_19, var_17));

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_20 = (max(var_20, (((((((arr_2 [i_0]) ? 1 : (((var_10 && (arr_0 [i_0]))))))) ? (((!((!(arr_1 [i_0])))))) : 7225)))));
        var_21 = (min(var_21, -7211));
    }
    var_22 = ((((max(((var_2 ? var_6 : var_1)), var_13))) ? ((-30150 ? (var_8 != 7210) : (~var_10))) : ((-var_3 - (var_10 | var_11)))));
    #pragma endscop
}
