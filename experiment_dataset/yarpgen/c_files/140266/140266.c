/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 140266
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=140266 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/140266
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_10 = (~var_6);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 19;i_3 += 1)
                    {
                        {
                            var_11 = (((min((85 + 83), -2268693258753103891)) < (--65535)));
                            var_12 = (arr_8 [i_2] [i_1]);
                        }
                    }
                }
                var_13 = (min((((arr_5 [i_1] [i_1] [14]) >> (((8 * 65520) - 524150)))), ((-((var_9 ? 47 : (arr_2 [i_0] [i_0] [i_0])))))));
            }
        }
    }
    var_14 = (max((min(var_5, -var_2)), (((((var_8 ? var_8 : var_1)) & var_3)))));
    #pragma endscop
}
