/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                arr_8 [i_0] [i_0 + 1] [i_1] = ((-((((((arr_0 [i_1]) ? var_8 : (arr_7 [i_1] [i_1] [i_1])))) ? -16663736913746719831 : ((((arr_4 [i_1 + 1]) + (arr_6 [i_1] [i_1]))))))));

                for (int i_2 = 1; i_2 < 15;i_2 += 1)
                {
                    var_11 &= (arr_7 [i_0] [i_1 + 1] [i_1 + 1]);
                    var_12 -= (arr_7 [i_0] [i_2] [i_2]);
                }
                var_13 = (((arr_10 [4] [i_1 + 1] [i_1] [i_1]) / (((2145386496 | ((var_3 ^ (arr_5 [i_1] [i_1]))))))));
            }
        }
    }
    var_14 = 3520298382;
    #pragma endscop
}
