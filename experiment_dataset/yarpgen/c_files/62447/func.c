/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62447
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62447 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62447
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) var_6);
    var_21 = var_17;
    /* LoopNest 3 */
    for (signed char i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (int i_2 = 2; i_2 < 11; i_2 += 4) 
            {
                {
                    var_22 = ((/* implicit */short) ((int) ((((/* implicit */int) arr_7 [i_1] [i_2 - 2] [(_Bool)1] [i_2])) + (((/* implicit */int) arr_6 [i_2 + 1] [i_2] [i_2 + 1])))));
                    var_23 = ((/* implicit */unsigned short) ((((((/* implicit */_Bool) (unsigned char)192)) && (((/* implicit */_Bool) arr_0 [i_2 - 1] [i_1])))) && (((/* implicit */_Bool) max((arr_0 [i_2 + 1] [i_1]), (((/* implicit */unsigned short) (unsigned char)40)))))));
                }
            } 
        } 
    } 
}
