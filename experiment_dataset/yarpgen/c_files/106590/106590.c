/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        arr_3 [4] = var_11;
        arr_4 [10] = var_11;
        arr_5 [14] [14] = var_12;

        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {

            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {

                for (int i_3 = 3; i_3 < 17;i_3 += 1) /* same iter space */
                {
                    arr_16 [i_0] [i_0] [19] [19] [i_0] [i_0] = var_0;
                    arr_17 [i_0 + 2] [i_1] [i_0] [i_0 + 2] = var_3;
                    arr_18 [i_0 + 1] [i_1] [8] [i_0] = var_13;
                }
                for (int i_4 = 3; i_4 < 17;i_4 += 1) /* same iter space */
                {
                    arr_23 [i_2] [i_2] = var_14;
                    arr_24 [i_0] [i_1] [i_2] [i_2] [2] = var_5;
                    arr_25 [i_2] = var_6;
                    arr_26 [i_0] [i_0] [i_0] [i_1] [7] [i_4] = var_1;
                    arr_27 [8] [i_2] &= var_1;
                }
                arr_28 [i_2] = var_1;
                arr_29 [i_0] [i_0 - 2] [i_0] [i_0] = var_13;
            }
            for (int i_5 = 3; i_5 < 18;i_5 += 1)
            {
                arr_32 [i_0 - 2] [i_0] = var_11;
                arr_33 [i_0 - 1] [i_1] [8] |= var_6;
                arr_34 [i_0] [i_1] = var_10;
            }
            arr_35 [i_0] = var_4;
            arr_36 [0] = var_0;
        }
        arr_37 [10] [i_0] = var_10;
    }
    var_15 |= var_1;
    #pragma endscop
}
