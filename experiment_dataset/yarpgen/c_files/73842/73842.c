/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73842
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73842 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73842
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [9] [i_1] [i_1] = -5563001547684460542;
                arr_5 [11] [12] [i_0] = max(((~((-25927 ? (arr_3 [i_0]) : -1)))), (arr_3 [i_0]));
                var_14 = (1 == -221);
                arr_6 [i_0] [i_1] = (min(-5563001547684460558, 4171835851));
            }
        }
    }
    var_15 = ((min(var_1, -1)));
    var_16 = ((((!-107) && (((var_8 ? var_5 : 287171576))))));
    var_17 = (min(var_17, var_8));
    #pragma endscop
}
