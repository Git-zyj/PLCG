/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125556
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125556 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125556
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_21 = max(((+(((arr_0 [i_1] [i_0]) ? var_3 : (arr_0 [i_1] [i_1]))))), ((-(arr_2 [i_0] [i_0] [i_0]))));
                arr_4 [i_0] [i_0] [i_1] = arr_0 [i_0] [i_0];
                arr_5 [i_1] [i_1] [i_1] &= ((~(((arr_0 [i_0] [i_1]) ? var_5 : (arr_3 [i_0] [i_1])))));
                var_22 -= 2446004140;
                var_23 *= ((~(min(((min(var_0, var_15))), (arr_2 [i_1] [i_1] [i_0])))));
            }
        }
    }
    #pragma endscop
}
