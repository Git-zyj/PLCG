/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 89073
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=89073 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/89073
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 8;

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_20 = (min(var_20, (((-1 > (min(-1128530503, 1)))))));
        arr_2 [i_0] = (min(((1 ? ((min(1, (arr_0 [i_0] [i_0])))) : var_11)), (max((((arr_1 [i_0] [i_0]) ? var_6 : 230)), (arr_0 [i_0] [i_0])))));
    }
    var_21 = (min(var_9, 1));
    var_22 = (min(var_13, (min((-21 / var_11), ((var_1 ? var_10 : 1))))));
    #pragma endscop
}
