/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74483
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74483 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74483
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */signed char) max((((/* implicit */short) ((var_1) < (max((((/* implicit */long long int) (signed char)-83)), (2053200518140149745LL)))))), (var_2)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        for (short i_1 = 3; i_1 < 21; i_1 += 1) 
        {
            {
                var_11 = ((/* implicit */unsigned long long int) min((var_11), (((/* implicit */unsigned long long int) min((min(((-(arr_2 [16LL]))), (((long long int) var_4)))), (((/* implicit */long long int) ((int) max(((signed char)83), (((/* implicit */signed char) arr_0 [i_0])))))))))));
                var_12 = ((/* implicit */unsigned short) (_Bool)1);
            }
        } 
    } 
    var_13 = ((/* implicit */unsigned short) (_Bool)1);
    var_14 += ((/* implicit */unsigned int) var_9);
}
