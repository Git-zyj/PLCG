/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32418
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32418 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32418
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned long long int) var_2);
    /* LoopSeq 2 */
    for (unsigned long long int i_0 = 1; i_0 < 15; i_0 += 2) /* same iter space */
    {
        arr_2 [i_0] [i_0] = ((/* implicit */unsigned char) (-(arr_0 [i_0])));
        var_13 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_0 [i_0])) && (((/* implicit */_Bool) arr_0 [i_0]))));
        var_14 = ((/* implicit */unsigned long long int) max((var_14), ((-(max((((/* implicit */unsigned long long int) arr_1 [i_0 + 2] [i_0 + 1])), (((((/* implicit */_Bool) var_6)) ? (var_3) : (arr_0 [6ULL])))))))));
    }
    for (unsigned long long int i_1 = 1; i_1 < 15; i_1 += 2) /* same iter space */
    {
        var_15 = ((/* implicit */unsigned char) 2099985657698902923ULL);
        arr_5 [i_1] = arr_1 [i_1] [i_1];
        arr_6 [i_1] = ((/* implicit */unsigned long long int) ((((/* implicit */int) (unsigned char)60)) % (((/* implicit */int) (unsigned char)63))));
    }
}
