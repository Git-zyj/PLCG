/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162610
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162610 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162610
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_12 [i_0 - 2] [i_1] [i_2] [i_1] [i_4] = 1;
                                arr_13 [0] [i_0] [i_2] [i_2] [i_1] [i_0] [i_0] = ((~(arr_9 [i_0] [i_0 - 2] [1] [i_3] [i_1])));
                            }
                        }
                    }
                }
                arr_14 [i_0] = ((~-1) ? ((((-64 || (arr_2 [i_1]))) ? (arr_4 [i_0] [i_0 - 4]) : (3304709907008302836 / 22535))) : (((~(arr_7 [i_0] [i_0] [7] [i_1])))));
            }
        }
    }
    var_11 = var_9;
    #pragma endscop
}
