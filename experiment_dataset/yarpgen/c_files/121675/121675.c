/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121675
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121675 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121675
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] = (arr_4 [i_1]);
                    var_12 = (arr_3 [i_0 + 2] [i_1] [i_2]);
                    var_13 = (max(var_13, (arr_5 [i_0] [i_1])));
                    var_14 = (max((((arr_8 [i_2]) ? var_4 : (arr_8 [i_0 + 2]))), (((3935252404962132976 ? -32752 : 59243)))));
                }
            }
        }
    }
    var_15 = -26674;
    #pragma endscop
}
