/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75891
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75891 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75891
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_1] = ((((min(var_10, (arr_3 [i_1])))) ? ((((arr_2 [i_1] [i_0] [i_0]) ? (arr_2 [i_1] [i_1] [i_1]) : (arr_0 [i_0])))) : (arr_3 [i_0])));
                var_15 |= (max((max((((arr_3 [i_1]) ? (arr_0 [i_1]) : (arr_2 [8] [6] [8]))), (min((arr_2 [0] [i_1] [12]), var_9)))), (max(var_13, (((arr_2 [16] [16] [i_1]) ? (arr_0 [5]) : 0))))));
            }
        }
    }
    #pragma endscop
}
