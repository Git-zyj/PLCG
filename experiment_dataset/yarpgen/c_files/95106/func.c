/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95106
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95106 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95106
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        var_14 = ((/* implicit */unsigned long long int) (unsigned short)7321);
        var_15 -= ((/* implicit */unsigned long long int) arr_0 [i_0]);
        var_16 |= ((/* implicit */unsigned short) (!((_Bool)1)));
    }
    var_17 ^= ((/* implicit */short) max((((int) -179413405)), (((int) ((var_10) ? (((/* implicit */int) var_0)) : (((/* implicit */int) (signed char)-85)))))));
}
