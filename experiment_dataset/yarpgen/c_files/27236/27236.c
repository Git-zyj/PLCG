/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27236
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27236 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27236
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        {
                            var_20 = ((var_17 + (((0 * var_1) / var_14))));

                            for (int i_4 = 1; i_4 < 13;i_4 += 1)
                            {
                                var_21 = (max(var_21, (((((max(var_0, 0))) && 1)))));
                                var_22 = (((min(var_7, (max(var_5, 1422697365)))) == ((~((var_6 ? var_5 : (arr_4 [i_0])))))));
                                arr_15 [i_0] [i_0] [i_0] [i_0] [i_0] = ((((min(var_13, var_1))) ? var_11 : var_4));
                            }
                            arr_16 [i_0] &= 14;
                        }
                    }
                }
                var_23 *= max((min(0, 2190399925)), 1);
            }
        }
    }
    var_24 = var_4;
    #pragma endscop
}
