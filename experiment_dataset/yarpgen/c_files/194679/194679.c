/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194679
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194679 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194679
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (((~var_0) % var_0));
    var_12 = var_3;
    var_13 = var_4;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    var_14 = ((((9223372036854775796 ? 65535 : 1859937328641922641)) * ((((((((arr_1 [i_0 - 1]) < var_10))) <= (((var_1 && (arr_3 [i_0] [i_1]))))))))));
                    arr_8 [i_0] [2] [i_2] [i_2] = (((((arr_6 [i_1]) < (arr_0 [i_0]))) || ((!((min((arr_4 [i_2]), var_9)))))));
                }
            }
        }
    }
    #pragma endscop
}
