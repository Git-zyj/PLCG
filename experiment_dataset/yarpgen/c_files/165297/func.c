/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165297
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165297 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165297
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 2; i_0 < 13; i_0 += 2) 
    {
        var_16 = ((/* implicit */int) arr_3 [i_0] [i_0]);
        var_17 = ((/* implicit */unsigned char) ((((/* implicit */int) ((unsigned char) var_11))) ^ (((/* implicit */int) arr_1 [i_0 - 1]))));
        var_18 = arr_0 [i_0] [i_0];
    }
    var_19 = ((/* implicit */unsigned long long int) var_13);
    /* LoopNest 2 */
    for (unsigned char i_1 = 3; i_1 < 10; i_1 += 2) 
    {
        for (_Bool i_2 = 0; i_2 < 0; i_2 += 1) 
        {
            {
                var_20 = ((/* implicit */unsigned char) var_12);
                arr_9 [(unsigned char)10] [i_2 + 1] [i_1] = ((/* implicit */short) arr_3 [i_1] [i_2 + 1]);
            }
        } 
    } 
    var_21 -= ((/* implicit */unsigned char) ((unsigned long long int) ((((/* implicit */_Bool) max((((/* implicit */unsigned long long int) var_3)), (var_12)))) ? (((((/* implicit */unsigned long long int) ((/* implicit */int) var_15))) + (var_6))) : (((/* implicit */unsigned long long int) ((512U) | (((/* implicit */unsigned int) ((/* implicit */int) (short)(-32767 - 1))))))))));
}
