/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124505
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */int) max((var_11), (((/* implicit */int) var_8))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 21; i_0 += 1) 
    {
        for (int i_1 = 1; i_1 < 19; i_1 += 2) 
        {
            {
                var_12 &= ((/* implicit */signed char) ((((/* implicit */int) min((((/* implicit */signed char) var_3)), (var_10)))) * (((/* implicit */int) var_9))));
                var_13 = ((/* implicit */_Bool) min((var_13), (((/* implicit */_Bool) ((unsigned int) (!(((/* implicit */_Bool) arr_1 [i_0] [i_0]))))))));
            }
        } 
    } 
}
