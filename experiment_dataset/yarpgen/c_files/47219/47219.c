/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 47219
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=47219 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/47219
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_14 = var_7;
                                var_15 = ((+(((arr_15 [i_4 - 2] [i_4] [i_4 + 1] [i_4 - 2] [i_4 - 1] [i_2] [i_4]) ? 0 : var_8))));
                            }
                        }
                    }
                }
                arr_16 [i_0] [i_0] = ((var_2 ? ((((max((arr_6 [i_1]), var_8))))) : (max((((var_8 | (arr_15 [i_0] [i_1] [i_1] [i_0] [i_0] [i_0] [i_1])))), (arr_13 [i_0] [i_0])))));
                arr_17 [i_0] = ((+((max((arr_7 [i_0] [i_0] [i_0] [i_1]), var_8)))));
            }
        }
    }
    #pragma endscop
}
