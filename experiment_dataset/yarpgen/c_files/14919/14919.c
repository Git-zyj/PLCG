/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14919
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14919 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14919
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((-2813037397354223143 ? 1276373082 : 14));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_2] = (((5322851668684683219 / 1) ? ((540431955284459520 ^ (arr_1 [i_1]))) : (min(var_3, var_5))));
                    var_12 *= ((!((((arr_3 [i_0]) ? (arr_3 [i_0]) : (arr_4 [i_0] [i_1]))))));
                }
            }
        }
    }
    var_13 = (max(var_13, 1));
    var_14 ^= var_6;
    #pragma endscop
}
