/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79367
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79367 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79367
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += (((var_4 & var_2) && (((-15382 ? 20 : 16719704570831069896)))));
    var_11 = var_8;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_12 = (min((arr_1 [i_0]), (((((arr_0 [i_0]) || var_4)) ? -8977 : var_9))));

        /* vectorizable */
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            arr_5 [i_0] [i_0] = (arr_0 [i_0]);
            var_13 = (((24551 * 0) && (arr_1 [i_0])));
        }
        for (int i_2 = 1; i_2 < 14;i_2 += 1)
        {
            arr_10 [i_0] [13] [i_0] = var_4;
            arr_11 [i_0] = arr_4 [i_0] [i_2 + 1] [i_2 - 1];
            var_14 = (arr_4 [i_0] [i_2 + 3] [i_0]);
        }
        arr_12 [i_0] [i_0] = (max((((arr_9 [i_0] [i_0] [i_0]) & var_5)), (min((((arr_1 [i_0]) * (arr_0 [16]))), (arr_2 [i_0])))));
    }
    var_15 = ((~(var_5 + var_4)));
    #pragma endscop
}
