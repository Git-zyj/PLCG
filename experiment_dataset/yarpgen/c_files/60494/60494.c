/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 60494
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=60494 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/60494
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 &= max((min(((var_2 ? 1 : 5424519627505150422)), ((min(-1883773801, var_11))))), ((((~-916522103) <= (min(var_3, -38))))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        var_13 = (max(var_13, (arr_0 [i_0] [i_0])));
        var_14 = (((((arr_0 [i_0] [i_0]) ? 43527 : -45)) & ((-45 ? -2 : var_6))));
    }
    for (int i_1 = 0; i_1 < 12;i_1 += 1)
    {
        var_15 = (arr_5 [i_1]);
        var_16 = (max(var_16, (((~(arr_1 [i_1]))))));
        arr_6 [4] = arr_5 [i_1];
        var_17 = (((((~(arr_1 [13]))))));
        arr_7 [9] = (max(((~(arr_0 [i_1] [i_1]))), ((~((max((arr_3 [i_1]), 0)))))));
    }
    var_18 = (((((var_7 ? var_6 : -6338186008259494922))) || ((((min(var_5, var_6))) && -var_3))));
    #pragma endscop
}
