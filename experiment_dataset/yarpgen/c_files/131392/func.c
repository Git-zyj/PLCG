/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131392
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 1; i_0 < 19; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 21; i_1 += 2) 
        {
            for (unsigned int i_2 = 0; i_2 < 23; i_2 += 2) 
            {
                {
                    arr_10 [i_0] [i_0] [i_0] = max((arr_5 [i_0] [i_0]), (((/* implicit */unsigned long long int) min((((/* implicit */unsigned int) 134086656)), (4153121938U)))));
                    var_10 = ((/* implicit */short) (signed char)(-127 - 1));
                }
            } 
        } 
    } 
    var_11 &= ((/* implicit */int) var_6);
}
