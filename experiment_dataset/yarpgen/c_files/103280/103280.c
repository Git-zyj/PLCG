/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103280
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103280 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103280
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_2;
    var_15 = var_6;
    var_16 = (min(((((((var_2 ? var_6 : var_10))) && var_6))), ((32767 >> (!-1016868369728747665)))));

    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_17 *= ((((max((max((arr_5 [2] [i_1] [i_2]), 4552767819399957233)), (!var_9)))) + ((min(4611686018427387903, 1)))));
                    arr_9 [i_2] = var_11;
                    arr_10 [i_2] [i_1] [i_2] = var_7;
                }
            }
        }
        arr_11 [i_0] = (-var_6 + var_10);
    }
    #pragma endscop
}
