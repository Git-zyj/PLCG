/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235390
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235390 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235390
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 -= (((var_2 <= var_8) | ((var_9 ? (min(var_0, var_4)) : var_7))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_12 = (min(var_12, ((min(((-(arr_4 [19]))), ((((((arr_0 [0]) ? (arr_4 [i_0]) : var_3)) > ((((arr_1 [i_0 - 2]) & (arr_0 [i_0]))))))))))));
                arr_5 [i_0] [i_0] [i_0] = ((-(arr_2 [i_0])));
                arr_6 [i_1] [i_0 + 1] [i_0] = (min((min(8132140670765707892, -975492938)), var_10));
                var_13 = (min((((-(arr_2 [i_0 + 2])))), (((arr_3 [i_0] [17] [i_1]) | var_7))));
            }
        }
    }
    var_14 = var_8;
    #pragma endscop
}
