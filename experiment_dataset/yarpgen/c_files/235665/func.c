/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235665
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235665 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235665
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 18; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 18; i_1 += 1) 
        {
            {
                var_15 = ((/* implicit */_Bool) var_12);
                var_16 = ((/* implicit */signed char) (-((+(((/* implicit */int) arr_0 [i_0]))))));
                /* LoopSeq 1 */
                /* vectorizable */
                for (short i_2 = 0; i_2 < 18; i_2 += 4) 
                {
                    arr_7 [i_0] = ((/* implicit */_Bool) ((unsigned short) arr_0 [6]));
                    arr_8 [i_0] [i_1] = ((/* implicit */unsigned long long int) (~(var_3)));
                    var_17 = (((_Bool)1) ? (((/* implicit */unsigned int) ((/* implicit */int) (unsigned char)89))) : (4232596218U));
                }
            }
        } 
    } 
    var_18 = ((/* implicit */int) (!(((/* implicit */_Bool) (+(((/* implicit */int) var_9)))))));
    var_19 = ((/* implicit */short) var_0);
    var_20 = ((/* implicit */int) (!(((/* implicit */_Bool) 12ULL))));
}
