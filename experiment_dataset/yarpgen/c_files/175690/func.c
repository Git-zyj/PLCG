/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175690
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175690 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175690
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 2; i_0 < 16; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            {
                arr_8 [i_1] = ((/* implicit */unsigned short) (short)-32761);
                arr_9 [i_0] [i_0] = ((/* implicit */unsigned short) (signed char)41);
                arr_10 [(unsigned char)16] = ((/* implicit */long long int) ((((int) arr_2 [i_0 - 2])) <= (((/* implicit */int) var_3))));
            }
        } 
    } 
    var_13 = ((/* implicit */unsigned long long int) (-(((/* implicit */int) (short)-7982))));
}
