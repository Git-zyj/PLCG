/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80607
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 ^= (min((max(var_9, var_11)), (((((var_8 ? var_8 : var_12))) ? var_12 : var_2))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 20;i_1 += 1)
        {
            {
                arr_7 [i_1] = 1097445152;
                var_14 = var_8;
                var_15 = (max(var_15, (arr_6 [3])));
            }
        }
    }
    var_16 *= (((-0 == (~1097445152))));
    var_17 = ((var_0 / (min(9223372036854775807, ((var_7 ? var_5 : 2897952962))))));
    #pragma endscop
}
