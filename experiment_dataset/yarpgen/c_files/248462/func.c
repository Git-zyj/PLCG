/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 248462
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=248462 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/248462
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            {
                arr_5 [i_1] [i_0] [i_0] = min((min((min((2360965065U), (3801274011U))), (((((/* implicit */_Bool) var_15)) ? (arr_2 [i_0] [i_1]) : (34506688U))))), (((/* implicit */unsigned int) ((arr_0 [i_1] [i_1]) >= (arr_2 [i_0] [i_0])))));
                arr_6 [i_0] [i_0] &= ((/* implicit */unsigned int) ((var_10) >= (min((0U), (2591201846U)))));
            }
        } 
    } 
    var_18 = var_3;
}
