/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67678
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67678 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67678
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 16;i_0 += 1) /* same iter space */
    {
        var_17 = (min((((arr_1 [i_0 + 2]) || (arr_1 [i_0 + 2]))), ((!(arr_0 [i_0 + 1])))));
        var_18 = ((+(((arr_2 [i_0 + 1]) / (arr_0 [i_0 + 1])))));
        var_19 = (min((((((var_13 ? (arr_2 [i_0]) : var_11))) ? ((var_1 << (var_2 - 2850926757))) : ((min(var_4, (arr_0 [1])))))), ((((arr_0 [i_0 - 1]) <= (arr_0 [i_0 - 1]))))));
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 16;i_1 += 1) /* same iter space */
    {
        var_20 ^= ((!(arr_5 [i_1 - 1])));
        var_21 = (max(var_21, var_15));
    }
    var_22 &= var_11;
    var_23 = var_4;
    var_24 ^= min((((var_5 > ((var_11 ? var_1 : var_6))))), ((((min(var_5, var_12))) ? (((var_1 ? var_4 : var_5))) : (min(var_9, var_15)))));
    #pragma endscop
}
