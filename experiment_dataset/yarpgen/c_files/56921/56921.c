/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56921
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56921 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56921
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_7;
    var_16 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 12;i_2 += 1)
            {
                {
                    var_17 = var_8;
                    var_18 = var_3;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 13;i_4 += 1)
                        {
                            {
                                var_19 = (arr_3 [i_0]);
                                var_20 = ((~((((var_5 ? var_7 : var_4)) | (var_2 ^ -1280700292)))));
                                arr_13 [1] [i_1] [7] [i_1] [i_0] = (arr_7 [i_0]);
                                arr_14 [i_0] [1] [i_2 - 2] [i_3] [i_1] = var_12;
                                var_21 = ((-1903071733 ? ((+((var_1 ? (arr_7 [i_0]) : (arr_8 [i_1] [i_1]))))) : (arr_0 [i_2 + 1])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
