/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136556
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136556 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136556
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) var_9);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 15; i_1 += 4) 
        {
            {
                /* LoopNest 2 */
                for (_Bool i_2 = 1; i_2 < 1; i_2 += 1) 
                {
                    for (unsigned short i_3 = 0; i_3 < 15; i_3 += 3) 
                    {
                        {
                            var_17 = ((/* implicit */unsigned char) -6711306548051064792LL);
                            arr_10 [11LL] [i_2] [i_2 - 1] [i_3] = ((/* implicit */unsigned short) (short)11640);
                            arr_11 [i_0] [i_0] [(short)6] [i_2] = ((/* implicit */unsigned int) (short)18790);
                            var_18 = ((/* implicit */long long int) (short)-22319);
                        }
                    } 
                } 
                var_19 = ((/* implicit */unsigned int) (_Bool)1);
                var_20 = ((/* implicit */unsigned short) 1838205949U);
            }
        } 
    } 
    /* LoopSeq 1 */
    for (_Bool i_4 = 1; i_4 < 1; i_4 += 1) 
    {
        arr_14 [i_4] = ((/* implicit */unsigned char) 312716223);
        var_21 = ((/* implicit */unsigned long long int) -1880363544418588001LL);
    }
    var_22 = ((/* implicit */signed char) var_15);
}
