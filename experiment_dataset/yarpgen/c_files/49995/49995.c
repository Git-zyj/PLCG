/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49995
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49995 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49995
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_14 ^= (var_6 || (((var_3 ? 3938796939 : (((max(21503, var_3))))))));
                var_15 = (~-47);
                var_16 = (min(var_16, ((((!1) ? (~777495785) : (arr_4 [i_0 - 1]))))));
            }
        }
    }
    var_17 = ((!((((((var_7 ? var_0 : var_4))) ? ((var_7 ? 4294967288 : var_9)) : ((4294967288 ? var_12 : var_3)))))));
    #pragma endscop
}
