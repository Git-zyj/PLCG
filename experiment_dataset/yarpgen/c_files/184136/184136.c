/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184136
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184136 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184136
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 18;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_2] [11] = 11513191722939069205;
                    var_13 = (max(var_13, 5433894413008548634));
                    var_14 = (max(var_14, ((max(var_12, (((max(-489503321, -794129794)) % -1937230789)))))));
                    var_15 ^= 489503323;
                }
            }
        }
    }
    var_16 = (max(var_16, (((!((((((6933552350770482411 ? var_0 : var_7))) ? 794129793 : var_6))))))));
    #pragma endscop
}
