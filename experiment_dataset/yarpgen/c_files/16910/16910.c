/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16910
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16910 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16910
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_9, var_2));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_12 = ((((min(234, -111))) << ((((~(arr_4 [i_0 - 1]))) + 2120786160))));
                    arr_9 [i_0] [i_0] [0] [i_0] = (max(((((arr_5 [i_0 - 1]) || var_9))), (~65535)));
                    var_13 = var_3;
                    arr_10 [i_0] [0] [0] [i_0] &= (!var_10);
                    var_14 = (min(var_14, (((-((((arr_4 [i_0 + 1]) || (arr_3 [12] [12])))))))));
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 1; i_3 < 1;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 19;i_4 += 1)
        {
            {
                arr_17 [i_3] [i_3] = (arr_1 [i_3]);
                arr_18 [i_3] = var_1;
            }
        }
    }
    #pragma endscop
}
