/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125002
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125002 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125002
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((((min(var_4, (var_9 - -16747)))) || ((min((min(1125625028935680, 131071)), var_7)))));
    var_12 += -24488;

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] [i_0] |= (((arr_1 [i_0]) - (min(223380641802368419, 223380641802368420))));
        var_13 = ((!((min(1493342607, 18223363431907183197)))));
    }
    var_14 = ((~(-var_9 + var_6)));
    #pragma endscop
}
