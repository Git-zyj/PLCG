/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 139622
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=139622 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/139622
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_10;

    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        var_16 = (((((!165) ? (arr_1 [i_0 - 1]) : 1)) ^ var_10));
        var_17 = -52;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        arr_5 [i_1] [i_1] = (var_13 >> (-71 + 71));
        var_18 = (((arr_3 [i_1] [i_1]) != (!var_8)));
    }
    for (int i_2 = 0; i_2 < 17;i_2 += 1)
    {
        var_19 = (((max(var_4, var_3)) >> ((((((!(arr_9 [i_2] [i_2])))) == (arr_1 [i_2]))))));
        var_20 = var_8;
    }
    for (int i_3 = 0; i_3 < 11;i_3 += 1)
    {
        var_21 -= ((var_9 * (max((arr_6 [i_3]), (arr_8 [i_3])))));
        arr_14 [i_3] [i_3] = (((arr_1 [i_3]) != ((1 ? var_11 : (((!(arr_2 [i_3]))))))));
    }
    #pragma endscop
}
