/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23472
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23472 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23472
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        var_10 = (((((((arr_1 [i_0 + 1] [i_0 + 1]) ? var_2 : (arr_0 [i_0]))) ^ var_5)) != ((-(arr_0 [i_0 + 1])))));
        var_11 = (min((max((arr_1 [i_0 + 2] [i_0 - 1]), (arr_0 [i_0 - 1]))), ((~(arr_1 [i_0 - 1] [i_0 - 1])))));
    }
    var_12 += ((((var_7 ? ((var_0 ? var_9 : var_4)) : var_7)) - ((var_1 ? var_0 : (220 <= 99)))));
    var_13 = (~((max((var_0 != var_8), (min(-71, var_2))))));
    #pragma endscop
}
