/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72972
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72972 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72972
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_11 = ((((~(~var_10)))) ? (arr_6 [6] [i_1] [12] [i_2]) : (max(((var_5 - (arr_7 [i_1] [9] [i_1]))), (max(var_5, (arr_5 [i_0]))))));
                    arr_8 [i_0] [i_1] [i_2] [i_2] = (((~(arr_6 [i_2] [1] [i_0] [i_0]))));
                    var_12 -= ((~((max((arr_2 [i_2]), (arr_0 [i_0]))))));
                }
            }
        }
    }

    /* vectorizable */
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        var_13 = (max(var_13, var_7));

        for (int i_4 = 1; i_4 < 1;i_4 += 1) /* same iter space */
        {
            var_14 = (((arr_9 [i_4 - 1]) ? (arr_9 [i_3]) : (arr_2 [i_3])));
            arr_14 [i_3] = var_0;
        }
        for (int i_5 = 1; i_5 < 1;i_5 += 1) /* same iter space */
        {
            var_15 |= (~((~(arr_4 [i_3] [i_3] [19]))));
            var_16 += ((((var_8 ? (arr_6 [i_3] [i_3] [i_3] [i_5]) : (arr_16 [7]))) == (arr_5 [i_5])));
        }
        for (int i_6 = 1; i_6 < 1;i_6 += 1) /* same iter space */
        {
            var_17 *= ((~(arr_18 [i_6 - 1] [i_6 - 1] [i_6])));
            var_18 = (~(arr_18 [i_6 - 1] [i_6 - 1] [i_3]));
            var_19 = (max(var_19, var_8));
        }
    }
    /* vectorizable */
    for (int i_7 = 0; i_7 < 22;i_7 += 1)
    {
        arr_22 [i_7] = (((((~(arr_20 [i_7] [i_7])))) < (((arr_20 [i_7] [i_7]) ? var_7 : var_8))));
        arr_23 [i_7] = ((~(arr_21 [i_7])));
        arr_24 [i_7] [i_7] = var_8;
        /* LoopNest 2 */
        for (int i_8 = 0; i_8 < 22;i_8 += 1)
        {
            for (int i_9 = 0; i_9 < 22;i_9 += 1)
            {
                {
                    var_20 = ((~((~(arr_25 [i_7])))));
                    var_21 = (arr_29 [i_7] [i_7] [i_8] [i_7]);
                }
            }
        }
    }
    var_22 = (~var_6);
    #pragma endscop
}
