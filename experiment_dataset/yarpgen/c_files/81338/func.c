/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81338
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81338 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81338
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */_Bool) var_0);
    var_15 = (-(-134217728));
    var_16 = ((/* implicit */short) (+(((/* implicit */int) (!(((/* implicit */_Bool) max((var_11), (var_6)))))))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 7; i_0 += 1) 
    {
        for (unsigned short i_1 = 3; i_1 < 9; i_1 += 3) 
        {
            {
                var_17 = ((/* implicit */signed char) (unsigned short)511);
                arr_7 [i_0 - 1] [i_1] = ((/* implicit */unsigned long long int) (!(((/* implicit */_Bool) var_12))));
            }
        } 
    } 
}
