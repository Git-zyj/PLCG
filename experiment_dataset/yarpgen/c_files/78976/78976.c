/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78976
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78976 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78976
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_12 = (((0 != 13) > (((arr_1 [i_0] [i_0]) ? 1961360191 : (max(var_6, 1961360167))))));
        arr_3 [8] |= (((var_9 - 26824) * (~44)));
        var_13 += (((var_10 * (((arr_1 [i_0] [10]) * (arr_1 [i_0] [6]))))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 13;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 13;i_3 += 1)
            {
                {
                    var_14 |= ((27 ? (199 && 1961360201) : var_9));
                    arr_11 [i_3] [i_1] = (((1696 && var_11) && (((arr_9 [i_1] [i_2] [i_1]) || var_2))));
                }
            }
        }
        var_15 |= var_10;
    }
    var_16 = (var_0 + var_7);
    #pragma endscop
}
