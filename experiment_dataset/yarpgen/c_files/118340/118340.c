/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118340
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118340 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118340
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 ^= 1727932740285453341;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1 + 1] [i_2] = (!1727932740285453341);
                    var_15 = (max(var_15, ((((arr_5 [12]) | (((min((arr_4 [16] [i_2]), (arr_6 [i_2] [i_2] [i_2]))) & ((var_7 ? var_5 : (arr_6 [i_2] [i_1 + 1] [i_0]))))))))));
                }
            }
        }
    }
    #pragma endscop
}
