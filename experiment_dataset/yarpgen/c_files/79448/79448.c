/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 79448
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=79448 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/79448
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 |= 2575579273525618445;
    var_14 = var_4;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {
                arr_6 [i_1 - 1] [i_0] [i_0] = 2575579273525618425;
                var_15 = (((var_12 ? 183 : (arr_5 [i_0]))));
            }
        }
    }
    var_16 = (max(var_16, (((var_11 ? (((2575579273525618424 ? -86 : var_0))) : var_2)))));
    #pragma endscop
}
