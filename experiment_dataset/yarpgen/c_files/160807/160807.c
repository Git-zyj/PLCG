/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160807
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160807 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160807
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(12963, var_7));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 21;i_2 += 1)
            {
                {
                    var_12 = (((((min(1395171789854012107, 17461)) > (~1))) ? -12963 : var_3));
                    var_13 -= (arr_5 [i_0] [i_1] [i_1]);
                    var_14 = ((((((arr_0 [i_1]) & (arr_2 [i_0])))) ? (2147482624 != 0) : ((7357254136398811747 ? 12963 : 12989))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_15 = (arr_4 [i_1]);
                                var_16 = (max(var_8, ((((arr_10 [i_2 - 1] [i_2 + 1] [i_2 + 1]) != 0)))));
                                var_17 = 229;
                            }
                        }
                    }
                    var_18 = 4294967295;
                }
            }
        }
        var_19 = (max(232, 4294967295));
    }
    var_20 = (min(var_8, 12956));
    #pragma endscop
}
