/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 45934
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=45934 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/45934
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_20 = ((max((arr_0 [i_0 + 4]), (arr_0 [i_1 - 1]))));
                    var_21 = (min(var_21, ((((((((arr_4 [i_0] [6] [i_2] [i_2]) ? var_16 : (arr_1 [i_1] [16])))) ? ((var_1 ? 2072918099565256363 : var_5)) : var_14))))));
                    var_22 = -3401467187122334362;
                }
            }
        }
    }
    var_23 = var_12;
    var_24 |= var_5;
    #pragma endscop
}
