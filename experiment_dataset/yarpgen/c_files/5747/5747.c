/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5747
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5747 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5747
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        arr_3 [i_0] [i_0] = ((33337 >> (384803273 == 27152)));
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    var_12 = (min(var_12, (((51724 ? 125 : ((max(8980, 32198))))))));
                    var_13 = ((((max((!3418131217), ((var_0 || (arr_1 [i_0])))))) < (((!(var_3 == 115))))));
                }
            }
        }
    }
    var_14 = (min(var_14, ((var_8 ? (var_7 % var_5) : var_11))));
    var_15 = ((0 ? (((min((min(11663, 59)), var_9)))) : var_5));
    #pragma endscop
}
