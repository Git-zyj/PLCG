/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48848
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48848 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48848
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned short) (+(min(((-(var_2))), (((/* implicit */unsigned long long int) max((var_0), (-5))))))));
    /* LoopSeq 1 */
    for (int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */unsigned long long int) max(((-((~(((/* implicit */int) (unsigned char)142)))))), (5)));
        var_16 = ((/* implicit */long long int) (short)28741);
        var_17 = ((/* implicit */unsigned long long int) min((var_17), (((/* implicit */unsigned long long int) (signed char)127))));
    }
    var_18 = ((/* implicit */short) ((((/* implicit */unsigned long long int) ((unsigned int) var_13))) > (var_3)));
    var_19 = ((/* implicit */long long int) var_6);
}
