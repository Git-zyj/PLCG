/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114786
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114786 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114786
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    var_11 = ((((var_2 ? var_4 : (arr_1 [i_0 - 2]))) ^ (!196)));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_0] [i_2] [i_0] [i_0] [i_0] = var_4;
                                arr_14 [16] [1] [i_2] [i_3] [i_4] [i_4] &= (((((46036 ? 204 : 44))) ? ((min(1, -51))) : var_6));
                                var_12 = (max(var_12, var_1));
                            }
                        }
                    }
                }
            }
        }
    }
    var_13 = ((1 >> ((((var_7 ? var_9 : (max(var_6, var_3)))) - 182))));
    #pragma endscop
}
