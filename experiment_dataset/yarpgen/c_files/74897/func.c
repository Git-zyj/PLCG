/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74897
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 *= ((/* implicit */unsigned int) ((((/* implicit */_Bool) ((((/* implicit */int) ((((/* implicit */_Bool) var_7)) || (((/* implicit */_Bool) (unsigned char)107))))) - (((/* implicit */int) ((((/* implicit */_Bool) var_7)) || (((/* implicit */_Bool) var_0)))))))) || ((!(((/* implicit */_Bool) (unsigned char)107))))));
    var_16 = ((/* implicit */unsigned short) var_11);
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        for (short i_1 = 3; i_1 < 23; i_1 += 2) 
        {
            {
                var_17 = ((/* implicit */unsigned char) arr_3 [i_1]);
                var_18 = ((/* implicit */unsigned char) max(((+(((/* implicit */int) (_Bool)1)))), (((((/* implicit */int) arr_1 [i_0] [i_1 - 3])) + (((/* implicit */int) (unsigned char)130))))));
            }
        } 
    } 
}
