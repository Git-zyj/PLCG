/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197662
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197662 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197662
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (~0);

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_15 = (max(var_7, ((max((arr_0 [i_0]), (arr_0 [i_0]))))));
        arr_2 [i_0] [i_0] = (max(((493656859 ? var_8 : ((((arr_1 [i_0] [i_0]) ? 23508 : var_10))))), var_9));
        var_16 = (((arr_1 [16] [i_0]) % ((min(23508, 23514)))));
    }
    var_17 = var_7;
    var_18 = var_4;
    var_19 = (min((~182), (1 | 23508)));
    #pragma endscop
}
