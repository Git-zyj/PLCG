/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238243
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238243 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238243
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, -19541));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_15 = ((((!(arr_1 [i_0] [i_2]))) ? (((max(((min(-96, (arr_1 [i_1] [i_1])))), (min((arr_2 [i_0]), (arr_3 [i_0] [i_1]))))))) : ((111 ? -58 : 756772735))));
                    arr_8 [i_0] [i_2] [i_0] = ((~((-(arr_4 [i_1 - 2] [i_1 + 2] [i_1 - 2])))));
                }
            }
        }
    }
    var_16 &= ((!(((((min(-96, var_5))) ? (~var_10) : (max(var_10, var_13)))))));
    var_17 ^= var_8;
    #pragma endscop
}
