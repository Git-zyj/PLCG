/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 137173
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=137173 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/137173
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_10 = ((((~((var_5 ? (arr_1 [i_1] [i_0]) : (arr_0 [i_0]))))) * var_7));
                var_11 = (((5 & 356316164) ? (((arr_0 [i_1]) >> (arr_0 [i_0]))) : (max(var_3, 2080186201))));
                arr_5 [i_0] [i_0] [i_1] = ((((-(min((arr_4 [i_0] [i_0] [i_0]), var_1)))) != var_8));
            }
        }
    }
    var_12 = (min(((~(var_1 / 187))), 186));
    #pragma endscop
}
