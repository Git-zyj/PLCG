/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48020
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48020 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48020
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = (min(var_16, ((((((~var_14) | (arr_5 [i_0 + 1]))) >= (arr_2 [18] [i_1 - 1] [i_1 + 3]))))));
                arr_6 [i_0] [i_1] = (((arr_2 [i_1] [i_1 - 1] [i_1 + 2]) ? (arr_2 [i_1] [i_1 + 1] [i_1 - 1]) : (arr_2 [i_1] [i_1 - 1] [i_1 + 2])));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 13;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            for (int i_4 = 4; i_4 < 11;i_4 += 1)
            {
                {
                    var_17 = var_7;
                    arr_15 [i_2 + 1] [11] [12] = (~4294967295);
                    arr_16 [i_4 + 3] = var_6;
                }
            }
        }
    }
    #pragma endscop
}
