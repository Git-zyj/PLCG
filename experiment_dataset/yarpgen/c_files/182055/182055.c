/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182055
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_0] = (arr_5 [i_0] [i_0] [i_2 + 2]);
                    var_18 ^= max(((!(arr_0 [i_1]))), ((((var_0 / (arr_0 [i_1]))) == (((1 ? (arr_2 [i_0 + 3] [i_1] [i_1]) : 251))))));
                    arr_7 [i_2] [i_0] [i_0] [i_0] = ((min(2036, 65533)));
                }
            }
        }
    }
    var_19 = ((!((max(var_3, var_5)))));
    var_20 = (((!-5326876595338267320) * -24));
    #pragma endscop
}
