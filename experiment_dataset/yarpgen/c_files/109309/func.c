/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 109309
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=109309 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/109309
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */short) var_10);
    var_12 += ((/* implicit */_Bool) (+(((/* implicit */int) var_0))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        var_13 *= ((((/* implicit */int) var_9)) > (((/* implicit */int) arr_1 [i_0])));
        var_14 ^= ((/* implicit */_Bool) min((((unsigned long long int) (short)25213)), (((/* implicit */unsigned long long int) (short)-25213))));
        var_15 = ((((((/* implicit */int) arr_1 [i_0])) == (((/* implicit */int) var_1)))) ? (((((/* implicit */_Bool) var_7)) ? (((/* implicit */int) (short)-25201)) : (((/* implicit */int) arr_1 [i_0])))) : ((~(((/* implicit */int) arr_1 [i_0])))));
    }
    var_16 = ((/* implicit */unsigned int) var_8);
}
