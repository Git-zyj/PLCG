/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185829
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185829 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185829
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_15 = ((((max(3688872971, (var_12 / var_12)))) && ((min(((var_5 ? 9223372036854775783 : 9223372036854775783)), (max(var_11, -7856165640747625148)))))));
                var_16 &= (min((~var_9), (min(var_10, (var_1 || var_2)))));
            }
        }
    }
    var_17 &= ((~(max(((var_5 ? var_11 : 18446744073709551615)), ((max(var_14, var_9)))))));
    #pragma endscop
}
