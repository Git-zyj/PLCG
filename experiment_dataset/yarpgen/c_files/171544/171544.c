/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171544
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171544 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171544
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        var_16 += (min((max((arr_1 [10] [i_0 + 1]), (arr_0 [6] [i_0 + 1]))), (arr_0 [i_0] [i_0 + 1])));
        var_17 -= ((+(((arr_0 [i_0 - 1] [i_0 + 1]) ? (arr_1 [i_0 + 1] [i_0 - 1]) : (arr_0 [i_0 + 1] [i_0 + 1])))));
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        arr_4 [4] = (((((arr_2 [i_1] [i_1]) | (arr_3 [i_1]))) <= ((-(arr_3 [i_1])))));
        var_18 = (max(var_18, 110));
    }
    var_19 = (max(var_19, (((((!(!var_2))) ? 31 : -3775414748371534365)))));
    #pragma endscop
}
