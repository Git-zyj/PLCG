/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114392
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114392 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114392
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, 178));
    var_19 ^= (((min((0 & var_10), ((4646 ? 79 : var_14)))) % var_2));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 17;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_0] = (((arr_3 [i_0]) ? (((arr_5 [i_1 + 1]) ? (arr_5 [i_1 - 1]) : (arr_5 [i_1 - 1]))) : (arr_7 [i_0] [8] [i_2])));
                    var_20 += 78;
                    arr_10 [i_0] [i_0] [i_1] [i_2] = (((((var_1 ? (~235) : (arr_8 [8])))) ? (arr_0 [i_0]) : (!1)));
                }
            }
        }
    }
    #pragma endscop
}
