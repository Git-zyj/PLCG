/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 40500
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=40500 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/40500
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] [10] = (((((~(max((arr_1 [i_0] [2]), (arr_3 [5] [i_1] [i_1])))))) ? (((arr_2 [0] [i_1] [0]) ? (var_2 ^ var_0) : var_5)) : ((((((min(var_19, (arr_2 [i_0] [i_1] [5]))))) - ((255 ? (arr_0 [12]) : (arr_3 [i_0] [i_1] [i_1]))))))));
                arr_5 [12] [i_1 + 1] [i_1] = -var_12;
                var_20 = ((~(arr_2 [i_0] [i_0] [12])));
            }
        }
    }
    var_21 = var_4;
    var_22 = var_2;
    #pragma endscop
}
