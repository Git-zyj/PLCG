/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61500
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61500 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61500
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (max(var_1, ((max(223, 223)))));

    for (int i_0 = 2; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_14 = (max((!27), (max((var_5 ^ 0), var_11))));
        arr_3 [1] = (max(((-((max((arr_2 [i_0]), (arr_1 [i_0] [i_0])))))), ((var_3 ? var_8 : ((32 ? 30 : 177))))));
        var_15 = (min(var_15, (((((((arr_1 [i_0] [i_0 + 1]) - ((min(-78, (arr_2 [12]))))))) ? (~0) : 226)))));
    }
    for (int i_1 = 2; i_1 < 14;i_1 += 1) /* same iter space */
    {
        var_16 += 16;
        var_17 = ((22 ? ((~(min(var_0, var_8)))) : 1));
    }
    var_18 = (((~18446744073709551615) & var_6));
    var_19 = (((max(var_12, ((var_4 ? 74 : 4921944177331717551)))) >> (var_2 - 84)));
    var_20 = (min(var_20, var_0));
    #pragma endscop
}
