/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83021
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83021 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83021
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 22; i_0 += 3) 
    {
        var_10 = ((/* implicit */short) min((var_10), (((/* implicit */short) min((((/* implicit */int) arr_1 [(short)10] [(short)10])), ((~(((/* implicit */int) var_0)))))))));
        arr_3 [i_0] = ((/* implicit */unsigned char) ((((/* implicit */_Bool) arr_2 [i_0])) && (((/* implicit */_Bool) min((((/* implicit */int) (short)(-32767 - 1))), (133169152))))));
    }
    var_11 = 130023424;
    var_12 = ((/* implicit */signed char) (-(((/* implicit */int) var_9))));
}
