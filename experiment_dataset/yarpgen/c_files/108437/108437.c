/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 108437
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=108437 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/108437
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (3602667717 < var_10);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 21;i_1 += 1)
        {
            {
                var_14 &= (max(3940649673949184, (arr_3 [i_0] [i_1 - 3] [i_1])));
                var_15 = (min(var_15, (6254550500557170642 / 1827664725)));
                var_16 = (max((((arr_1 [i_1 + 4]) < (arr_2 [i_1 - 2] [i_1 - 3] [i_1 + 2]))), (((arr_3 [i_1 + 4] [i_1 + 4] [21]) && (arr_3 [i_1 - 3] [i_1] [i_0])))));
            }
        }
    }
    #pragma endscop
}
