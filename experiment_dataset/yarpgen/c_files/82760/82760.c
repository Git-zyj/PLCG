/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82760
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82760 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82760
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0 + 1] [12] [i_1] |= 2624049394;
                var_19 = (!((((arr_2 [i_0 - 1] [i_0 - 1] [i_0 + 1]) ^ (arr_2 [i_0 + 1] [i_0 + 1] [i_0 - 1])))));
            }
        }
    }
    var_20 = var_4;
    var_21 = min(((var_3 - (var_14 == var_1))), var_18);
    var_22 = var_15;
    var_23 = (max(var_6, (((max(var_18, 2624049400)) >> (var_7 >= var_12)))));
    #pragma endscop
}
