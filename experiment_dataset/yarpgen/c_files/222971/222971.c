/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222971
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222971 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222971
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_3));

    /* vectorizable */
    for (int i_0 = 1; i_0 < 17;i_0 += 1) /* same iter space */
    {
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_8 [i_1] [i_1] [i_1] [i_1] &= -var_15;
                    var_18 = (arr_6 [10] [i_1] [i_2]);
                }
            }
        }
        var_19 = (((arr_6 [8] [i_0] [i_0 + 3]) ? (arr_6 [i_0] [i_0] [i_0 - 1]) : (arr_1 [i_0])));
        arr_9 [i_0] = 9223372036854775807;
        var_20 ^= ((-(arr_4 [10])));
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 17;i_3 += 1) /* same iter space */
    {
        var_21 = (max(var_21, (((((((~(arr_1 [8]))) + 2147483647)) << (var_1 - 54442))))));
        var_22 -= ((arr_5 [i_3 + 3] [i_3 + 3]) ? 4664593433739834816 : -1962178345);
    }
    #pragma endscop
}
