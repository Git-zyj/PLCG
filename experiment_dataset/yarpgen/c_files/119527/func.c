/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119527
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119527 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119527
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 13; i_0 += 2) 
    {
        var_16 &= ((/* implicit */unsigned char) ((long long int) var_11));
        arr_2 [i_0] = ((/* implicit */int) ((((/* implicit */long long int) (-(max((3914553904U), (3914553909U)))))) ^ (min((((var_2) / (((/* implicit */long long int) 3914553904U)))), (max((-881201041166002538LL), (((/* implicit */long long int) (_Bool)1))))))));
    }
    var_17 |= ((/* implicit */unsigned short) var_10);
}
