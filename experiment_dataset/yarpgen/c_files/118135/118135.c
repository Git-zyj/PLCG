/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118135
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118135 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118135
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_12 = (max(var_12, (((((-(max((arr_2 [i_0] [i_0]), (arr_5 [i_0] [i_0] [i_0]))))) != (min(-2445474911704730662, 1999852913)))))));
                arr_6 [i_1] [i_0] [i_0] = (((((~-5969623145801705087) / -106)) <= ((((((((var_1 < (arr_5 [i_0] [i_1] [i_1]))))) < (((arr_3 [i_0]) << var_1))))))));
            }
        }
    }
    var_13 = var_5;
    var_14 = (min((((!(var_1 / 246)))), (((!var_4) * (var_8 * var_2)))));
    #pragma endscop
}
