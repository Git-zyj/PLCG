/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199452
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199452 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199452
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 &= (max((11585151532541092157 <= 6861592541168459435), (max(-1, (min(52992, -6503565053872227868))))));
    var_19 = (-(min(var_8, ((min(var_5, var_3))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_20 *= ((~2852395570223745427) - (arr_1 [i_0]));
                var_21 = ((!((min((arr_4 [i_0] [i_0]), (arr_0 [i_0] [0]))))));
            }
        }
    }
    var_22 = -var_16;
    #pragma endscop
}
