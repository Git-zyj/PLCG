/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 110314
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=110314 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/110314
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((!var_9) ? var_1 : var_9)) > (min(var_7, var_1))));
    var_13 = ((((!3083021988) > (var_5 && var_10))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 8;i_3 += 1)
                    {
                        {
                            arr_10 [i_0] [i_1] [i_0] [i_3] = arr_0 [i_2];
                            arr_11 [i_0] [i_0] [i_2] [i_3] &= var_11;
                            var_14 = ((~((~(arr_8 [i_0] [i_0] [i_0])))));
                        }
                    }
                }

                for (int i_4 = 0; i_4 < 12;i_4 += 1)
                {
                    var_15 = (((1901528687 ? (arr_6 [i_0] [i_0]) : ((1211945307 ? (arr_3 [i_1] [i_1]) : var_10)))));
                    arr_14 [i_0] [i_1] [i_4] &= (arr_6 [i_0] [i_0]);
                    arr_15 [i_0] [i_1] [i_1] = (arr_1 [i_1] [i_1]);
                }
                arr_16 [i_1] [i_0] = (((arr_8 [i_1] [i_1] [i_0]) + (arr_8 [i_0] [i_0] [i_0])));
            }
        }
    }
    #pragma endscop
}
