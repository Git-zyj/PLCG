/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221067
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221067 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221067
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 20; i_0 += 3) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) arr_0 [i_0] [i_0])) || (((/* implicit */_Bool) ((unsigned char) arr_0 [i_0] [i_0])))));
        var_16 = ((/* implicit */unsigned short) ((arr_0 [i_0] [i_0]) - (arr_0 [i_0] [i_0])));
        var_17 = ((/* implicit */unsigned char) min((var_17), (((/* implicit */unsigned char) (short)5801))));
        var_18 = ((/* implicit */unsigned short) (~(arr_2 [i_0])));
    }
    var_19 = ((/* implicit */short) (+(var_2)));
}
