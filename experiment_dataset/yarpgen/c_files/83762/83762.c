/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83762
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83762 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83762
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = max(var_7, (~var_10));
    var_13 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_14 = (((arr_3 [i_0]) ? ((-(((arr_7 [i_0] [i_0 - 1] [0] [0]) / var_7)))) : (((arr_1 [i_0]) ? (var_2 > 0) : var_11))));
                    arr_9 [i_0] = ((((arr_4 [i_2] [i_1] [i_0 - 1]) % -8797071909390420712)) < (((((arr_7 [i_0 - 1] [i_1] [i_2] [i_2]) == (arr_4 [i_2] [i_1] [i_2]))))));
                }
            }
        }
    }
    var_15 = (max(var_15, ((max(57060, (!-var_8))))));
    #pragma endscop
}
