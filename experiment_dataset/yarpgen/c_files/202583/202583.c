/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202583
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202583 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202583
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_15 ^= var_11;
                    var_16 = (max(var_16, (((!(((arr_3 [i_0] [i_1] [i_1 + 2]) >= var_12)))))));
                }
            }
        }
    }
    var_17 = ((((0 ? ((1 ? 0 : 1)) : 0)) - var_10));
    var_18 += var_0;
    var_19 = (min(var_19, (((!(((var_0 ? 30719 : ((var_3 ? 127 : 1))))))))));
    var_20 = (max(var_20, (((30719 < ((33188 ? 34817 : (var_5 >= -31754))))))));
    #pragma endscop
}
