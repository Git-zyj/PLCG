/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36618
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36618 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36618
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        var_16 = var_0;
        arr_2 [11] = (((((max(var_15, (arr_1 [i_0] [i_0]))) + 2147483647)) >> (var_7 - 100)));
        var_17 = (min((((!(arr_1 [i_0] [i_0])))), -18));
    }
    var_18 = (((((var_10 ? (!var_11) : 6962004266185115959))) ? var_5 : ((~((var_8 ? var_3 : var_7))))));
    var_19 = ((-((((min(-121, -14))) ? var_9 : -29))));
    var_20 = (min(var_20, (((var_14 + (max(var_9, (!-20))))))));
    #pragma endscop
}
