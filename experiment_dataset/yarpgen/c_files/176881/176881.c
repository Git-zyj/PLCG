/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176881
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176881 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176881
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 &= ((((min(((var_6 ? var_14 : 165)), var_8))) ? -560311466 : (~var_12)));
    var_20 += (min(var_10, (min((max(var_8, 90)), (max(var_5, var_6))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_4 [i_0] [0] = (((!((max((arr_0 [10]), (arr_0 [i_0])))))));
                arr_5 [10] [i_0] = (((min(141, -410296016))));
            }
        }
    }
    #pragma endscop
}
