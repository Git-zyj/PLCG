/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240852
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240852 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240852
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (~var_3);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_21 = var_3;

                            for (int i_4 = 0; i_4 < 15;i_4 += 1)
                            {
                                var_22 += (arr_6 [i_0] [i_0] [i_0] [i_0] [i_3]);
                                var_23 = (1 > -3035290506419106285);
                            }
                            var_24 = ((~((7027662795317419942 ? var_9 : (min(var_0, var_8))))));
                        }
                    }
                }
                arr_13 [i_0 + 1] [i_1] [i_1] = var_14;
                var_25 = (min((((11320 > var_11) / 11818114042424798465)), (((arr_9 [5] [i_0 + 1] [i_0 + 1] [i_0 + 1]) ? var_12 : var_16))));
            }
        }
    }
    #pragma endscop
}
