/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231666
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231666 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231666
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */int) var_10);
    var_12 = ((/* implicit */int) var_7);
    var_13 -= (short)-5314;
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned char i_0 = 0; i_0 < 11; i_0 += 3) 
    {
        arr_2 [(signed char)7] = ((/* implicit */int) ((3085206023350792440ULL) / (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)42185)))));
        var_14 = ((/* implicit */unsigned char) arr_0 [i_0]);
    }
    var_15 = ((/* implicit */unsigned char) ((unsigned short) (unsigned short)63090));
}
