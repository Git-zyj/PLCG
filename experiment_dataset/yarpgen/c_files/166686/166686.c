/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166686
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166686 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166686
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 += var_7;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_2] [9] = (min((max((max(-1424612367, var_2)), (arr_2 [i_0]))), (max((max((arr_4 [i_0] [i_0] [9] [0]), 1424612368)), ((max(var_0, (arr_1 [i_0]))))))));
                    var_11 -= 104;
                    arr_9 [1] |= ((max((((arr_7 [i_0] [i_1 - 2] [i_2]) >> (arr_4 [i_0] [6] [i_1 - 1] [i_2]))), 1424612375)));
                    arr_10 [i_0] [i_1 - 1] = (~15451449065066525880);
                }
            }
        }
    }
    #pragma endscop
}
