/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159867
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_4;

    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        var_21 ^= (min((!var_12), ((((!(arr_3 [i_0]))) ? (arr_1 [i_0 + 3] [i_0 + 1]) : (!3280863991)))));

        /* vectorizable */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            var_22 = (((((var_0 ? var_1 : (arr_3 [i_0 - 1])))) & (arr_1 [i_0] [i_1])));
            arr_6 [i_0] [i_0] [14] = ((var_6 ? (arr_4 [i_0 - 1]) : (31457280 >= var_6)));
        }
    }
    var_23 = (max(var_23, var_17));
    var_24 = (max(var_15, ((((28 ? var_11 : -67)) | var_14))));
    #pragma endscop
}
