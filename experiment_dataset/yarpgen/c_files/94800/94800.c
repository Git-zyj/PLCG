/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94800
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94800 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94800
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] = (((((((max(var_3, var_2))) ? (((min(var_2, (arr_0 [i_0] [i_0]))))) : (max((arr_1 [i_0]), var_0))))) ? (max((((arr_0 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0]))), (arr_1 [i_0]))) : (((arr_0 [i_0] [i_0]) ? (arr_1 [i_0]) : var_14))));
        var_19 *= var_10;
        var_20 *= (arr_1 [i_0]);
    }

    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        arr_5 [i_1] = (min((~var_16), var_7));
        var_21 = (min(var_21, var_0));
    }
    for (int i_2 = 0; i_2 < 20;i_2 += 1)
    {
        arr_10 [i_2] [i_2] = (arr_9 [i_2]);
        var_22 = (arr_9 [i_2]);
    }
    var_23 = var_11;
    #pragma endscop
}
