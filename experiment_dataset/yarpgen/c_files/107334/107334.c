/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_8;
    var_18 = (min(var_18, (((~((-(var_8 || var_9))))))));

    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        arr_3 [i_0] [1] = (max((arr_0 [i_0] [i_0]), (arr_0 [i_0 + 1] [i_0 + 1])));
        var_19 -= (arr_1 [i_0]);
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 8;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 8;i_3 += 1)
                {
                    {
                        arr_12 [i_0] [i_2] [i_3] = (min((var_0 * -0), (arr_0 [5] [i_3 + 1])));
                        var_20 = (!-106);
                    }
                }
            }
        }
        var_21 = 1;
    }
    for (int i_4 = 2; i_4 < 21;i_4 += 1)
    {
        arr_16 [i_4] = ((max(-var_12, (arr_15 [i_4 + 1] [i_4]))));
        var_22 = (min((-32767 - 1), 32767));
    }
    #pragma endscop
}
