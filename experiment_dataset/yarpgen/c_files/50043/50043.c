/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50043
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50043 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50043
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] [4] [13] = (((((var_2 ? (arr_3 [i_1]) : (arr_3 [i_0])))) ? var_11 : (max(25403867, var_15))));
                arr_7 [i_0] = max((arr_4 [i_0]), ((((max((arr_1 [i_0] [i_1]), (arr_5 [i_0] [4] [i_0])))) ? (arr_3 [i_1]) : (arr_5 [i_0] [8] [i_0]))));
            }
        }
    }
    var_18 = ((var_4 || ((min((max(-448918258, -797945814)), var_15)))));
    #pragma endscop
}
