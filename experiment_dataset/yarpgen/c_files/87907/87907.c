/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87907
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87907 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87907
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_13 = (((((arr_3 [i_1]) > (arr_0 [i_0] [i_1]))) ? ((((arr_0 [i_0] [i_0]) > (arr_3 [i_0])))) : (!0)));
                var_14 = (max(var_14, ((((arr_3 [i_0]) << (((((((arr_1 [i_0]) ? -4611686018427387904 : 244)) + 4611686018427387924)) - 18)))))));
                arr_6 [i_0] [9] [i_1] = (((var_5 ? 10624991025786908480 : (arr_4 [i_0]))));
            }
        }
    }
    #pragma endscop
}
