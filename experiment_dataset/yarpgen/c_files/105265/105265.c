/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105265
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105265 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105265
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        arr_4 [i_0] = arr_3 [i_0];
        /* LoopNest 3 */
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        arr_13 [i_0] [i_0] [i_2] [i_3] = (max((((arr_3 [i_0 - 1]) ? (arr_3 [i_0 + 1]) : (arr_3 [i_0 + 1]))), (((arr_3 [i_0 + 1]) / (arr_3 [i_0 - 1])))));
                        arr_14 [i_0] [i_1] [i_2] [i_3] = ((-936059351015074315 ? 86 : 6580865253690668443));
                        var_11 ^= (arr_10 [15] [i_1] [i_2] [15] [1]);
                    }
                }
            }
        }
        var_12 = (min(var_12, var_4));
    }
    var_13 = (((((var_3 | -89) - (max(1035819823, -96)))) <= var_6));
    var_14 = var_0;
    #pragma endscop
}
