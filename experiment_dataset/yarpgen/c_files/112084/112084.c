/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112084
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112084 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112084
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= var_8;
    var_12 = ((-((var_7 ? var_0 : (max(var_9, var_3))))));

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_13 = (arr_0 [i_0]);
        var_14 = (((max((var_6 ^ var_3), ((~(arr_1 [i_0] [i_0]))))) == (max((var_6 < 65523), var_2))));
        var_15 = 1204725972;
        arr_2 [i_0] &= ((((~(arr_1 [i_0] [i_0])))) ? (((((214 ? var_3 : (arr_1 [8] [i_0])))) ? ((max(var_7, (arr_0 [i_0])))) : ((var_1 ? (arr_0 [8]) : var_8)))) : 65522);
    }
    for (int i_1 = 1; i_1 < 21;i_1 += 1)
    {
        var_16 = var_2;
        var_17 ^= ((-(min(-74, (arr_4 [6])))));
    }
    #pragma endscop
}
