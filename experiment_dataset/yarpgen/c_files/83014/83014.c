/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 83014
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=83014 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/83014
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 |= ((var_17 == (((76 ? var_8 : var_13)))));
    var_21 = var_12;
    var_22 = (min((~var_13), ((min(var_9, var_15)))));

    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        var_23 = (((1125899906826240 ? 1885998254 : (arr_1 [i_0 - 2]))));

        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            var_24 = var_3;
            var_25 ^= 1;
            var_26 = ((~((~(arr_3 [i_0 + 2] [i_1] [i_0 + 2])))));
        }
        var_27 *= ((-(((((arr_4 [13] [i_0]) ? 0 : (arr_4 [i_0] [i_0 - 1])))))));
        var_28 = var_1;
    }
    #pragma endscop
}
