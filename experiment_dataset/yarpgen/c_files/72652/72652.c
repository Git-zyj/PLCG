/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72652
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72652 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72652
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [i_0 + 1] [i_0 + 1] [i_1] |= ((3 >> (((max(3228371998, 0)) - 3228371983))));
                var_14 += (((min(((var_13 || (arr_3 [i_0] [4])), (max((arr_0 [i_1] [i_0]), (arr_3 [i_1] [i_1]))))))));
            }
        }
    }
    var_15 = (max(-var_2, (((max(var_2, var_8)) >> ((((var_12 ? var_13 : var_6)) - 7355355312358120188))))));
    #pragma endscop
}
