/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 9446
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=9446 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/9446
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 1; i_0 < 9; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            for (_Bool i_2 = 0; i_2 < 1; i_2 += 1) 
            {
                {
                    var_16 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) (short)16383)) ? (((/* implicit */int) (unsigned short)63)) : (((/* implicit */int) (short)-16385))));
                    /* LoopNest 2 */
                    for (int i_3 = 4; i_3 < 8; i_3 += 1) 
                    {
                        for (unsigned long long int i_4 = 0; i_4 < 11; i_4 += 3) 
                        {
                            {
                                arr_16 [i_1] [i_1] [i_2] [i_3 + 3] [i_2] = ((/* implicit */int) (short)-16385);
                                var_17 = ((/* implicit */unsigned short) var_1);
                            }
                        } 
                    } 
                    var_18 = ((/* implicit */unsigned char) (~(3091671348U)));
                }
            } 
        } 
    } 
    var_19 *= ((/* implicit */unsigned int) min((((/* implicit */unsigned short) var_0)), ((unsigned short)65474)));
    var_20 = ((/* implicit */unsigned long long int) var_5);
    var_21 = var_8;
}
