/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80856
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80856 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80856
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) (short)6146);
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 11; i_0 += 1) 
    {
        for (long long int i_1 = 0; i_1 < 11; i_1 += 2) 
        {
            {
                var_21 = ((/* implicit */_Bool) ((((/* implicit */_Bool) (((_Bool)1) ? (((/* implicit */int) (unsigned short)23245)) : (((/* implicit */int) (unsigned short)27335))))) ? (((((/* implicit */_Bool) arr_3 [i_0] [i_1] [i_0])) ? (94517494) : (((/* implicit */int) (unsigned short)3639)))) : ((+(((/* implicit */int) arr_3 [i_0] [i_0] [i_0]))))));
                var_22 *= ((/* implicit */unsigned int) var_12);
            }
        } 
    } 
    var_23 = ((/* implicit */long long int) min((var_23), (((/* implicit */long long int) var_7))));
}
