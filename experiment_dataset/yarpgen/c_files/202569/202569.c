/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 202569
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=202569 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/202569
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_11;
    var_14 = (max(-var_7, (min(var_3, 0))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_2] [i_2] = ((32765 ? (((((arr_4 [i_0] [i_0] [i_1 - 1] [i_1 - 1]) && (arr_4 [i_0] [i_0] [i_1 - 1] [5]))))) : ((-(arr_1 [i_1 + 1] [i_1 + 1])))));
                    arr_7 [i_1] = (((1 / 65535) ? var_4 : (((((min(19, (arr_4 [2] [i_1] [2] [1]))) < 8))))));
                    var_15 = (arr_4 [i_0] [i_1 - 1] [i_2] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
