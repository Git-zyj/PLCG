/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172240
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172240 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172240
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_0 = 0; i_0 < 13; i_0 += 1) 
    {
        var_13 = ((/* implicit */short) ((((/* implicit */_Bool) arr_1 [i_0] [11LL])) ? (((/* implicit */unsigned long long int) (-(arr_0 [i_0])))) : (((13877108472064690940ULL) - (((/* implicit */unsigned long long int) 2423070345483329419LL))))));
        var_14 = ((/* implicit */signed char) ((((/* implicit */_Bool) (unsigned char)255)) ? (arr_1 [i_0] [i_0]) : (arr_1 [i_0] [i_0])));
    }
    var_15 = ((/* implicit */unsigned long long int) var_5);
}
