/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61134
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 ^= (var_8 & var_5);
    var_13 = ((((var_8 | 65535) & 15414)) == (((var_7 % -7917) % var_9)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_14 |= 7696581394432;

        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {

            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                var_15 = arr_2 [i_0];
                var_16 = -9131;
            }
            var_17 *= (((arr_6 [i_0] [i_0] [i_1] [i_1]) < (arr_3 [i_1])));
            arr_9 [i_0] = (arr_1 [i_0] [i_0]);
            var_18 = ((+((((arr_7 [i_1]) >= (arr_5 [3] [i_0] [i_1]))))));
        }
    }
    /* vectorizable */
    for (int i_3 = 1; i_3 < 22;i_3 += 1)
    {

        for (int i_4 = 0; i_4 < 24;i_4 += 1)
        {
            var_19 -= (arr_11 [i_3]);
            var_20 = ((-(arr_12 [i_3 + 1])));
            var_21 = ((255 + (arr_11 [i_3 + 1])));
            var_22 = (arr_12 [4]);
        }
        var_23 = (((arr_11 [i_3 + 1]) / (arr_11 [i_3 + 2])));
    }
    var_24 ^= 67108800;
    var_25 = (max(var_25, (((!(var_7 + var_9))))));
    #pragma endscop
}
