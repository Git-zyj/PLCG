/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120069
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, var_15));
    var_18 = var_0;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                var_19 = (((max((((var_11 ? (arr_1 [i_1]) : var_5))), (((arr_0 [i_0]) ^ (arr_7 [i_0] [i_1] [i_0]))))) ^ (((max((arr_3 [i_1 - 4] [i_0 - 1]), (arr_3 [i_1 - 2] [i_0 - 1])))))));
                var_20 = (max(var_20, (((+((((max((arr_7 [i_0] [i_1] [i_1]), var_2))) ? (arr_1 [i_1 - 3]) : (((arr_5 [i_0] [i_1] [1]) ? var_14 : var_10)))))))));
            }
        }
    }
    #pragma endscop
}
