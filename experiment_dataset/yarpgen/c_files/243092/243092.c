/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 243092
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=243092 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/243092
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((((min(var_10, var_13))) > 3709021426076123305)) ? ((((!((min(var_0, var_11))))))) : var_1));
    var_17 |= var_11;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_1] = (min(1924853336, -2877));
                var_18 = (arr_2 [i_1]);
            }
        }
    }
    var_19 = var_12;
    var_20 = (min(var_12, ((18446744073709551615 * (13458934885332283674 && -1632778367)))));
    #pragma endscop
}
