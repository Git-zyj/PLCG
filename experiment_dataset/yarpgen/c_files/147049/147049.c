/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147049
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147049 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147049
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((((!(~var_10))) ? ((((((17179869182 ? var_0 : -61))) ? (!61) : var_2))) : (((((1 ? var_4 : 61))) ? var_11 : (!var_1)))));
    var_13 = ((var_5 ? ((min(((var_1 ? var_2 : var_1)), (!var_1)))) : (((!-596) ? var_3 : var_7))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = ((~((6533303109406004296 - ((7 ? (arr_3 [i_0] [i_0] [12]) : -45))))));
                arr_7 [i_0 + 1] [i_0] [i_0] = ((arr_5 [i_0 + 1] [i_0 - 1]) ? ((~(arr_5 [i_0 - 1] [i_0 + 2]))) : -62);
                var_14 = arr_0 [i_0] [i_0];
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_15 &= (arr_1 [i_4]);
                                var_16 = 2703893680;
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 &= var_0;
    #pragma endscop
}
