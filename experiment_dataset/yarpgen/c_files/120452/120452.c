/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120452
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120452 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120452
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 *= var_9;
    var_20 = (var_9 && 14588518480543211293);
    var_21 = (max(var_1, 0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 10;i_3 += 1)
                    {
                        {
                            var_22 = ((((((var_9 * var_6) + (max(var_7, var_7))))) ? (min((max(var_10, var_8)), 26)) : (((~((max(1, var_1))))))));
                            arr_12 [i_3] [i_1] [i_3] [i_1] [i_1] = 1;
                            var_23 = ((var_0 ? 1 : -16800));
                        }
                    }
                }
                arr_13 [i_0] [i_1] [i_1] = var_11;
            }
        }
    }
    #pragma endscop
}
