/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70238
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 |= ((/* implicit */_Bool) var_14);
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        arr_2 [i_0] = ((/* implicit */unsigned char) ((signed char) ((((/* implicit */int) (short)-30145)) | (((/* implicit */int) (unsigned short)960)))));
        arr_3 [i_0] |= ((/* implicit */long long int) arr_1 [i_0]);
        var_16 ^= ((/* implicit */signed char) var_0);
    }
    var_17 += var_0;
    var_18 = ((/* implicit */short) (!(((((/* implicit */_Bool) ((int) (unsigned short)938))) && (((/* implicit */_Bool) (unsigned char)135))))));
}
