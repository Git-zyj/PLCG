/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43730
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43730 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43730
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_17 = (arr_1 [i_0]);
                var_18 = (min(var_18, ((((~6272) ? (~var_11) : (((arr_5 [i_0]) ? (arr_2 [i_1]) : (arr_0 [i_1] [i_0]))))))));
            }
        }
    }
    var_19 = var_12;
    var_20 &= (min(((min(var_4, (var_14 >= var_16)))), (min((var_3 | var_0), (!var_1)))));
    var_21 = ((((max((var_0 - var_9), (min(var_11, var_9))))) ? var_7 : (max((var_2 && -69), -68))));
    var_22 = var_12;
    #pragma endscop
}
