/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 58434
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=58434 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/58434
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 14;i_1 += 1)
        {
            {
                var_20 -= ((1 << (((((((2976737915 << (arr_3 [i_1])))) ? ((var_2 ? (arr_2 [i_0]) : var_16)) : (arr_1 [i_0]))) + 1012223400))));
                arr_4 [i_0] [i_0] = (arr_1 [i_0]);
            }
        }
    }
    var_21 = ((!((((~1073741824) ^ -24136)))));
    /* LoopNest 3 */
    for (int i_2 = 1; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 3; i_3 < 18;i_3 += 1)
        {
            for (int i_4 = 2; i_4 < 17;i_4 += 1)
            {
                {
                    arr_14 [6] [i_3 - 1] [i_3 - 1] [6] = ((((max((arr_13 [i_2 + 1]), var_14))) ? (((var_1 == (arr_13 [i_2 - 1])))) : ((max((arr_13 [i_2 + 2]), (arr_13 [i_2 - 1]))))));
                    var_22 = var_1;
                }
            }
        }
    }
    #pragma endscop
}
