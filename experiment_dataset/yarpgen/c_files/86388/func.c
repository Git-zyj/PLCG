/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 86388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=86388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/86388
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */short) min((var_12), (((/* implicit */short) var_3))));
    var_13 = ((/* implicit */signed char) max((var_13), (((/* implicit */signed char) ((((/* implicit */_Bool) var_3)) ? (((/* implicit */int) (short)4838)) : ((-(var_10))))))));
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 10; i_0 += 1) 
    {
        for (short i_1 = 3; i_1 < 9; i_1 += 4) 
        {
            for (long long int i_2 = 0; i_2 < 11; i_2 += 4) 
            {
                {
                    var_14 = var_6;
                    arr_8 [i_1] [i_1 - 3] [i_1] = ((/* implicit */short) arr_6 [i_1]);
                }
            } 
        } 
    } 
    var_15 *= ((/* implicit */_Bool) var_4);
}
