/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127424
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127424 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127424
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned char) min((var_17), (((/* implicit */unsigned char) ((var_12) * (((/* implicit */long long int) ((/* implicit */int) var_13))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 3; i_0 < 22; i_0 += 1) 
    {
        arr_3 [i_0] = arr_2 [i_0] [i_0];
        var_18 = ((/* implicit */long long int) (_Bool)1);
    }
    var_19 |= ((unsigned long long int) (-2147483647 - 1));
    var_20 = ((/* implicit */unsigned long long int) min(((((~(-1537395532))) != (((/* implicit */int) var_15)))), ((_Bool)1)));
}
