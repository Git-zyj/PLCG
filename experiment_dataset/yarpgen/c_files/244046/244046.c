/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244046
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244046 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244046
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = ((~(((var_8 != (!-312324110))))));

    for (int i_0 = 0; i_0 < 25;i_0 += 1) /* same iter space */
    {
        var_12 += -312324110;
        arr_3 [i_0] = ((((min((0 - 0), 103))) + (((arr_0 [i_0] [i_0]) - ((2210581254 ? var_2 : 403703848))))));
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1) /* same iter space */
    {
        var_13 = ((min((arr_1 [i_1]), 109)));

        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            var_14 = min(((0 ? (((var_10 >> (var_5 - 3063725580)))) : (min(var_0, 105)))), (((~((min(var_6, var_9)))))));
            var_15 -= (var_5 - 1);
        }
    }
    for (int i_3 = 0; i_3 < 25;i_3 += 1) /* same iter space */
    {
        var_16 |= var_1;
        var_17 -= (((~(~var_4))));
    }
    #pragma endscop
}
