/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209403
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209403 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209403
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((-(!-var_3)));
                arr_5 [i_0] [i_1] [i_0] = ((~((11054449250918376253 ? 18446744073709551615 : var_8))));
                arr_6 [1] = ((((var_11 ? (arr_0 [i_0 + 1]) : (var_11 <= var_4))) > (((!(((arr_3 [i_0] [i_1]) <= var_14)))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                arr_12 [i_2] [i_3] = var_8;
                var_17 = var_7;
                var_18 *= (((((max((arr_9 [13] [7]), -24822)) + 2147483647)) << (((((((var_6 ? var_3 : var_3))) ? (max(0, var_8)) : ((var_8 ? var_2 : (arr_10 [i_2] [i_2] [i_2]))))) - 28973732))));
            }
        }
    }
    #pragma endscop
}
