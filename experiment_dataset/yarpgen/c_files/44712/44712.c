/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44712
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44712 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44712
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_17 *= var_2;
                arr_7 [i_0] = (((((var_3 ? var_8 : var_9))) ? (max((arr_3 [i_0]), var_2)) : 125));
            }
        }
    }
    var_18 += (max((min(((var_12 ? -185611866054095718 : var_16)), var_14)), ((max(((var_3 ? -125 : 17127)), var_2)))));
    var_19 = (max(((var_14 ? var_3 : ((max(0, var_15))))), (((var_2 ? -125 : var_12)))));
    #pragma endscop
}
