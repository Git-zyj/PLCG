/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32949
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32949 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32949
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            {
                var_14 = (((arr_2 [i_0 + 3] [i_0] [i_0 + 3]) ? (arr_2 [i_1 + 2] [i_1 + 3] [i_1 - 1]) : var_5));
                var_15 = var_11;
                var_16 = (min((((arr_1 [i_0 + 3] [i_0]) ? var_11 : (arr_1 [i_0 - 1] [i_0]))), (arr_1 [i_0 + 1] [i_0])));
                arr_4 [i_0] [i_0] [i_1 - 2] = ((-(!var_6)));
                var_17 = (max(var_17, ((~(~0)))));
            }
        }
    }
    var_18 *= ((((!var_2) ? 1 : var_13)));
    var_19 *= var_4;
    #pragma endscop
}
