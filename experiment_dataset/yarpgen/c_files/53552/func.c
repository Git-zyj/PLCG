/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53552
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53552 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53552
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            {
                var_18 = ((/* implicit */unsigned short) var_6);
                arr_4 [i_1] [i_1] [i_0] = ((/* implicit */unsigned char) (_Bool)1);
                arr_5 [i_0] [i_0] [i_0] = ((/* implicit */unsigned char) 936607865);
                arr_6 [i_1] [i_0] = ((/* implicit */long long int) var_17);
            }
        } 
    } 
    var_19 = ((unsigned char) -936607856);
    var_20 = ((/* implicit */int) (_Bool)1);
    var_21 = ((/* implicit */int) var_12);
}
