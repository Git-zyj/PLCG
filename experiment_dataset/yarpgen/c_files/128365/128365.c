/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 128365
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=128365 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/128365
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_19 = (min(var_19, ((((min((0 / 9083183330118917537), 1)) * (((((max(29814, -1585339695356933929))) ? 112 : ((max(55541, 95)))))))))));
                var_20 = (((((5261128203221200686 ? -335649671 : 5261128203221200686))) ? ((((-1115725245 ? -2590 : 143)) * 76)) : 335649671));
            }
        }
    }
    var_21 = var_8;
    #pragma endscop
}
