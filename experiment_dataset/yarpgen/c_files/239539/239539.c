/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 239539
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=239539 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/239539
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 19;i_1 += 1)
        {
            {
                var_12 ^= (((arr_3 [i_1 + 4] [i_1 + 1] [i_1 - 1]) << (((arr_3 [i_1 - 2] [i_1 - 2] [i_1 - 2]) - 200))));
                arr_5 [12] |= (((((((var_6 ? var_10 : var_3))))) < (((((var_0 << (((var_6 + 4358) - 15))))) - (arr_2 [i_0] [i_0] [i_0])))));
                var_13 = (((((var_4 ? (arr_1 [i_0]) : var_6))) ^ (arr_2 [i_0] [i_0] [i_1])));
            }
        }
    }
    var_14 = var_10;
    var_15 = ((var_3 ? var_3 : var_4));
    #pragma endscop
}
