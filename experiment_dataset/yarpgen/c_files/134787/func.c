/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134787
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134787 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134787
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (+(((/* implicit */int) var_9))))) ? ((~(8351243214429227956LL))) : (((/* implicit */long long int) ((/* implicit */int) (_Bool)1)))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 1; i_0 < 18; i_0 += 4) 
    {
        var_11 = ((/* implicit */short) 3406165099463714360LL);
        arr_3 [i_0] = ((/* implicit */signed char) (_Bool)1);
    }
    var_12 = ((/* implicit */_Bool) (signed char)120);
}
