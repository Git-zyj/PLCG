/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 126022
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=126022 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/126022
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 17;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] = (arr_1 [i_1 - 3] [i_1 + 2]);
                    arr_8 [i_0] [i_1] [i_1] [i_1] = -96;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 0;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 18;i_4 += 1)
        {
            {
                arr_15 [i_3] [i_3] [19] = 64;
                var_19 = (max(var_19, (min(var_12, 122))));
                var_20 = 95;
                arr_16 [i_3] [i_4 + 2] = (arr_11 [i_3 + 1]);
                var_21 = var_0;
            }
        }
    }
    var_22 = ((var_0 ? ((var_4 - (0 + 15))) : var_12));
    var_23 = (max(var_23, var_6));
    #pragma endscop
}
