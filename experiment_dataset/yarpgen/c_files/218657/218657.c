/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 218657
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=218657 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/218657
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((!((min(18446744073709551615, 6063171720431993756)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_11 += ((((max(13240, 13205))) ? -1 : ((~((-13206 ? var_3 : 15689036182330450516))))));
                var_12 = (((((arr_2 [1]) < (arr_0 [i_0] [i_1]))) ? ((max(54009, 45))) : 1770571892));
                arr_6 [i_0] [i_1] = (arr_3 [i_1]);
            }
        }
    }
    var_13 = (min((1770571892 != 21), -var_4));
    #pragma endscop
}
