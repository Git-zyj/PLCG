/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213086
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213086 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213086
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    var_14 = ((((min(var_8, (((arr_4 [i_0 - 1] [i_1] [i_2]) ? (arr_0 [i_1] [i_2 - 1]) : (arr_2 [i_1] [11] [i_2])))))) ? (((arr_3 [i_2 + 1]) / (arr_3 [i_2 + 1]))) : (arr_2 [i_2 - 1] [i_0 + 1] [i_0 + 1])));
                    arr_8 [i_0] [10] [i_0] [i_2 - 2] = ((+(((((arr_0 [i_1] [i_1]) * var_6)) | (~73)))));
                }
            }
        }
    }
    var_15 = (((var_3 | var_10) ? var_7 : var_8));
    #pragma endscop
}
