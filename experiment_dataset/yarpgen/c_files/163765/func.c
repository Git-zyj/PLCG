/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163765
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163765 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163765
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = (~(var_6));
    /* LoopNest 2 */
    for (unsigned char i_0 = 2; i_0 < 18; i_0 += 4) 
    {
        for (unsigned char i_1 = 3; i_1 < 18; i_1 += 4) 
        {
            {
                var_13 = ((/* implicit */unsigned short) ((((/* implicit */long long int) ((((/* implicit */int) (unsigned char)52)) + (((/* implicit */int) (unsigned short)8570))))) - (max((9223372036854775802LL), (((/* implicit */long long int) (short)32767))))));
                arr_6 [i_1] [i_1] [i_0 - 1] = ((/* implicit */long long int) ((short) (-(min((((/* implicit */long long int) (short)16524)), (8845181707884474022LL))))));
            }
        } 
    } 
}
