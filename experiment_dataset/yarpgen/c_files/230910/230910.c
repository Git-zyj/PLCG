/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230910
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230910 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230910
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, var_13));
    var_18 = (((((-(var_6 * 32767)))) ? ((-((var_9 ? var_8 : var_15)))) : var_11));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        arr_2 [i_0] = (((!var_13) ? (((((max(var_15, 32))) << (32752 - 32727)))) : (arr_1 [i_0])));

        for (int i_1 = 4; i_1 < 19;i_1 += 1)
        {
            var_19 &= (arr_4 [i_0] [i_1 - 4] [i_1]);
            arr_5 [i_0] [15] = (arr_3 [i_0] [i_1 + 1]);
        }
        var_20 &= (!-32763);
    }
    #pragma endscop
}
