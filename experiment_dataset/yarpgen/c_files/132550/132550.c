/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132550
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132550 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132550
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_10;
    var_16 = (min(var_16, ((((((var_5 ? var_4 : var_14))) >= (~255))))));
    var_17 = (((((var_8 ? (~var_1) : var_4))) ? (var_5 & -104) : (min((var_6 & 8167553907672327755), (((var_12 >> (59 - 33))))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_18 = ((-(((arr_2 [i_2] [i_2 + 1]) - 8167553907672327748))));
                    var_19 += ((-(arr_3 [i_1 - 1] [i_1 - 1] [i_2 + 1])));
                }
            }
        }
    }
    var_20 = var_3;
    #pragma endscop
}
