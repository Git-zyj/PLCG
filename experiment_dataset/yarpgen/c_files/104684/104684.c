/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104684
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104684 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104684
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = min(-6387541041691323275, var_1);

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_20 = -32757;
        arr_2 [i_0] = (((!126100789566373888) <= (arr_0 [i_0] [i_0])));
        /* LoopNest 2 */
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_21 += (((arr_3 [i_1 + 2] [10]) % (arr_3 [i_1 + 1] [12])));
                    arr_9 [i_0] [i_2] = ((!(arr_0 [i_0] [i_1 + 2])));
                }
            }
        }
        arr_10 [i_0] = (max((((~(arr_0 [i_0] [i_0])))), var_13));
    }
    var_22 = ((-(~var_6)));
    #pragma endscop
}
