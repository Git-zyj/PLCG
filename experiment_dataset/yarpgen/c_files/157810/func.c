/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157810
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157810 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157810
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 &= ((/* implicit */unsigned long long int) min(((short)64), (((/* implicit */short) (_Bool)1))));
    var_11 = ((/* implicit */unsigned long long int) var_4);
    var_12 = ((/* implicit */unsigned long long int) ((((/* implicit */int) var_2)) | (var_0)));
    var_13 = (_Bool)1;
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 21; i_0 += 4) 
    {
        var_14 = ((/* implicit */unsigned char) (-(((/* implicit */int) (short)-64))));
        var_15 = ((/* implicit */_Bool) ((((/* implicit */int) arr_0 [i_0])) + (((/* implicit */int) (_Bool)1))));
    }
}
