/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 76560
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=76560 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/76560
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_9;
    var_15 = (max(var_15, var_8));

    for (int i_0 = 0; i_0 < 14;i_0 += 1) /* same iter space */
    {
        var_16 ^= ((((min((((arr_1 [i_0] [i_0]) ^ (arr_0 [i_0]))), (!4294967232)))) ? (max(-var_4, (((arr_0 [i_0]) * var_9)))) : (((-var_11 / (arr_1 [i_0] [i_0]))))));
        arr_2 [i_0] = (arr_1 [i_0] [i_0]);
    }
    for (int i_1 = 0; i_1 < 14;i_1 += 1) /* same iter space */
    {
        arr_7 [i_1] = var_5;
        var_17 = (!(arr_0 [i_1]));
        var_18 ^= (((arr_5 [i_1]) ? ((var_8 - ((65488 / (arr_4 [i_1] [i_1]))))) : ((max((var_2 != var_0), ((0 >= (arr_0 [i_1]))))))));
    }
    #pragma endscop
}
