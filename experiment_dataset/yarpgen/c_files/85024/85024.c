/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85024
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85024 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85024
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_6;
    var_16 = min((((var_6 && (max(var_14, var_14))))), 13898885836275296887);
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                var_17 = (max((max((min((arr_4 [i_0]), var_7)), 14641603605013281679)), (((!((max((arr_1 [i_0] [i_1]), -2013179996))))))));
                var_18 = var_12;
            }
        }
    }
    var_19 = (max((max(var_14, var_14)), -2013180017));
    #pragma endscop
}
