/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28345
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28345 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28345
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            {
                var_17 = (min(var_17, (((((max(4294967295, ((var_13 ? (arr_4 [i_0] [i_0] [i_0]) : var_12))))) ? (min(2147483646, (arr_1 [i_1 - 1]))) : (arr_5 [i_1 + 2] [i_1 + 3] [i_1 + 2]))))));
                var_18 ^= ((~(arr_3 [i_0])));
            }
        }
    }
    var_19 *= ((var_16 ? (min(var_3, ((var_7 ? var_4 : var_6)))) : var_0));
    #pragma endscop
}
