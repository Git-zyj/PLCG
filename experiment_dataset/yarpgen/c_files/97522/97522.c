/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97522
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97522 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97522
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min((var_2 / var_8), (((((min(var_6, var_5))) || var_2)))));

    for (int i_0 = 1; i_0 < 21;i_0 += 1) /* same iter space */
    {
        arr_4 [i_0] |= (max(((((arr_0 [i_0] [i_0 - 1]) + (arr_1 [i_0] [i_0])))), ((((var_8 / var_6) == (arr_2 [i_0] [i_0 + 1]))))));
        arr_5 [i_0 + 1] = (max(((~(arr_1 [5] [i_0 + 1]))), 1));
    }
    for (int i_1 = 1; i_1 < 21;i_1 += 1) /* same iter space */
    {
        arr_9 [15] |= (arr_3 [i_1 - 1] [i_1 - 1]);
        var_13 ^= 34542;
    }
    for (int i_2 = 1; i_2 < 21;i_2 += 1) /* same iter space */
    {
        var_14 = ((var_6 ? (((arr_13 [i_2] [i_2 + 1]) * ((((arr_11 [i_2]) == (arr_1 [i_2 + 1] [i_2 + 1])))))) : ((((arr_0 [i_2] [i_2 - 1]) || (arr_0 [i_2 - 1] [i_2 - 1]))))));
        var_15 = (max(var_15, (((var_4 % (arr_6 [i_2 + 1]))))));
        var_16 *= arr_6 [i_2 + 1];
    }
    #pragma endscop
}
