/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8337
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8337 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8337
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        arr_2 [i_0] = var_3;
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 14;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_18 = var_17;
                                var_19 &= var_10;
                                var_20 = (max(((-(arr_15 [i_0] [i_1] [6] [i_1 - 1] [9]))), (((arr_4 [i_0 + 1] [i_1 - 2] [i_2]) ? (arr_4 [i_0 + 1] [i_1 - 1] [8]) : var_12))));
                            }
                        }
                    }
                    var_21 *= (max((max(((max(var_14, 1))), 30885)), (((!((((arr_0 [0]) ? var_8 : (arr_0 [i_1 - 1])))))))));
                    var_22 = (!(!96));
                }
            }
        }
    }
    var_23 = (min(var_23, var_8));
    var_24 = ((-var_4 ^ (((!var_15) ? ((var_9 ? var_7 : var_4)) : (!var_4)))));

    /* vectorizable */
    for (int i_5 = 1; i_5 < 20;i_5 += 1)
    {
        var_25 = (((arr_16 [i_5]) && -91));
        var_26 = (min(var_26, (arr_17 [i_5 + 2])));
    }
    #pragma endscop
}
