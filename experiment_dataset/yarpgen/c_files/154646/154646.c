/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154646
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154646 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154646
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_14));

    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        arr_2 [i_0] = (min((max((arr_0 [i_0]), (arr_0 [i_0]))), (arr_1 [i_0 + 1])));
        var_18 = -1;
        arr_3 [i_0] [i_0] = (min(31, -2068134817));
        var_19 = 2147483647;
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        arr_8 [i_1] = min(1, 1);
        arr_9 [17] = ((((!(max(0, 1)))) ? (arr_4 [i_1] [i_1]) : (arr_4 [i_1] [i_1])));
        var_20 ^= (max((min((arr_6 [i_1]), (~var_13))), ((min(((81 ? 228 : 1)), var_5)))));
        var_21 = (((((min((arr_6 [i_1]), (arr_4 [i_1] [i_1]))))) | ((+(min((arr_6 [i_1]), 134217727))))));
    }
    var_22 = var_10;
    #pragma endscop
}
