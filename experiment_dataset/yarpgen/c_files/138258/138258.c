/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138258
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138258 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138258
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_14 = ((2837344241 ? 223 : -6709));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 8;i_3 += 1)
                    {
                        {
                            var_15 -= ((arr_7 [i_0 - 1] [i_3 + 1]) > ((+(((arr_5 [i_0] [8] [i_2] [8]) ? var_7 : var_8)))));
                            var_16 = ((!var_0) ? (arr_9 [i_0 - 1] [i_1] [i_2] [i_3] [i_0] [i_0 - 1]) : (((arr_6 [i_0] [i_1] [i_2]) >> (var_11 - 1926680075))));
                            var_17 = (((arr_4 [i_1] [i_1]) ? 0 : (max(6709, 1))));
                        }
                    }
                }
                arr_11 [i_0] [i_1] = -6709;
            }
        }
    }
    var_18 = var_5;
    #pragma endscop
}
