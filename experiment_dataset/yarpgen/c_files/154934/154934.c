/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154934
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154934 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154934
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                var_11 = (min(var_11, 2039728991));
                var_12 = (~-59);
            }
        }
    }

    for (int i_2 = 0; i_2 < 19;i_2 += 1)
    {
        var_13 = ((((var_8 + (arr_5 [i_2])))) ? (((arr_3 [i_2] [i_2]) ? var_0 : (((arr_5 [i_2]) >> (var_4 - 27973))))) : (((var_0 < (-821360065 - var_0)))));
        arr_10 [i_2] = (arr_2 [i_2] [i_2]);
    }
    var_14 = (max((max(1, var_9)), var_3));
    #pragma endscop
}
