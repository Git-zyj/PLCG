/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10393
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10393 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10393
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((((((3298 ? 3562411943 : var_5)))) && ((var_3 <= (var_10 != var_10)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 16;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_1] [0] [i_1] = ((~((min(var_9, (arr_4 [i_0] [i_1] [i_1] [i_2 + 1]))))));
                    arr_8 [i_1] [i_1] = (arr_6 [i_0] [i_1] [13]);
                }
            }
        }
    }
    var_14 |= (max((var_0 + var_11), var_7));
    var_15 = (min(var_6, ((var_7 / (max(var_7, var_12))))));
    #pragma endscop
}
