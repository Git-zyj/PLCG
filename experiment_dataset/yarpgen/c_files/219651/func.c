/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 219651
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=219651 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/219651
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = var_8;
    /* LoopNest 2 */
    for (signed char i_0 = 2; i_0 < 13; i_0 += 4) 
    {
        for (unsigned int i_1 = 0; i_1 < 14; i_1 += 2) 
        {
            {
                var_12 *= ((/* implicit */_Bool) (-(((/* implicit */int) (signed char)-104))));
                var_13 = ((/* implicit */long long int) min((var_13), (((/* implicit */long long int) arr_0 [i_0]))));
                arr_4 [i_0] [i_1] [i_0] = ((/* implicit */int) var_4);
            }
        } 
    } 
    var_14 = max((15025632049169945875ULL), (((/* implicit */unsigned long long int) (signed char)-30)));
}
