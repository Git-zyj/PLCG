/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 147032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=147032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/147032
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 2; i_0 < 9; i_0 += 2) 
    {
        for (unsigned char i_1 = 1; i_1 < 9; i_1 += 2) 
        {
            {
                var_20 = min(((!(((arr_1 [i_0]) == (((/* implicit */int) (signed char)-4)))))), ((_Bool)1));
                var_21 &= ((/* implicit */signed char) var_15);
                var_22 = ((/* implicit */unsigned long long int) min((var_22), ((-(max((((((/* implicit */_Bool) (signed char)114)) ? (((/* implicit */unsigned long long int) arr_2 [i_1] [i_1] [i_1 + 1])) : (var_19))), (((/* implicit */unsigned long long int) arr_3 [i_0] [i_0] [i_0]))))))));
            }
        } 
    } 
    var_23 = ((/* implicit */int) var_19);
}
