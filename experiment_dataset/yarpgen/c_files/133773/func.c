/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133773
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133773 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133773
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 |= ((/* implicit */int) ((var_6) + (var_16)));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned int i_0 = 0; i_0 < 21; i_0 += 2) 
    {
        var_19 = ((/* implicit */unsigned int) ((unsigned short) var_5));
        arr_2 [i_0] = ((/* implicit */short) ((arr_1 [i_0]) / (((/* implicit */long long int) ((/* implicit */int) (signed char)(-127 - 1))))));
    }
}
