/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74561
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74561 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74561
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, var_9));
    var_15 = (max(var_15, var_2));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1) /* same iter space */
    {
        var_16 = (min(var_16, (arr_1 [i_0] [i_0])));
        arr_2 [i_0] [i_0] = arr_1 [i_0] [12];
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1) /* same iter space */
    {
        var_17 = (max(var_17, (((-((min((arr_1 [i_1] [i_1]), (arr_4 [i_1] [i_1])))))))));
        var_18 = max((min((arr_0 [i_1]), var_5)), var_3);
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 22;i_2 += 1)
    {
        for (int i_3 = 1; i_3 < 21;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 22;i_4 += 1)
            {
                {
                    var_19 = min((min((arr_8 [i_3 + 1]), (arr_8 [i_3 + 1]))), var_3);
                    var_20 = min((arr_7 [i_4]), (min(var_8, (max((arr_10 [11] [i_4]), (arr_12 [i_4] [i_3] [i_2] [6]))))));
                    var_21 |= ((-((min(var_7, (arr_8 [i_4]))))));
                }
            }
        }
    }
    #pragma endscop
}
