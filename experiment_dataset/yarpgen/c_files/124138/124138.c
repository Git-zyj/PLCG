/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 124138
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=124138 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/124138
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_0;
    var_15 = var_12;
    var_16 = ((var_5 - (4294967271 || -2078)));

    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        var_17 ^= var_13;
        arr_2 [i_0] [i_0] = ((~(max((min(var_4, var_8)), (((var_11 ? (arr_1 [i_0 - 1]) : (arr_0 [i_0]))))))));
        var_18 = (min(var_18, (arr_1 [i_0 + 1])));
        var_19 += var_9;
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        var_20 = (((max((arr_3 [i_1] [i_1]), ((min(46230, 1281570671))))) >> (((!(arr_4 [i_1]))))));
        var_21 &= (((26555 * 1) ? ((-(max((arr_4 [i_1]), (arr_4 [i_1]))))) : (((-2078 ? 1 : 1)))));
    }
    #pragma endscop
}
