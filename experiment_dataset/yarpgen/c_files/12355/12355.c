/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12355
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12355 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12355
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += var_8;

    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        var_12 = (max(((max(var_0, (arr_1 [i_0])))), (((arr_0 [i_0] [i_0 - 1]) ? (((arr_1 [i_0]) ? 37 : (arr_1 [3]))) : ((max((arr_1 [i_0]), (arr_1 [i_0]))))))));
        var_13 = ((~(max((37 + var_3), (arr_1 [i_0 - 1])))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_14 = (((-27 ^ (arr_0 [10] [10]))));
        var_15 = 71;
        var_16 = (arr_0 [i_1] [i_1]);
    }
    var_17 = ((-((0 ? 71 : -38))));
    var_18 = ((-((max(((max(64, 71))), (max(106, 56)))))));
    #pragma endscop
}
