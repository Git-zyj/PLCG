/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= var_0;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                var_18 = var_3;
                /* LoopNest 2 */
                for (int i_2 = 2; i_2 < 16;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 17;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] = 52;
                            arr_12 [i_1 + 1] [i_0] [i_2] [i_1 + 1] [i_0] [i_0] = (max((max(1, (arr_7 [i_0] [i_0]))), (((max(32, (arr_0 [i_1 - 1])))))));
                            var_19 = ((224 ? -3201306594838786357 : (min((arr_0 [i_1]), (arr_3 [i_1] [i_1 - 1] [i_1 + 2])))));
                            var_20 |= (arr_3 [i_0] [i_0] [i_0]);
                            var_21 = (~var_2);
                        }
                    }
                }
                arr_13 [i_0] [i_0] = ((~(16 * 224)));
                var_22 = ((~(arr_7 [i_1 + 1] [i_0])));
            }
        }
    }
    var_23 = (max(var_23, var_11));
    var_24 = (((!(var_3 * var_2))));
    var_25 = (max((min(var_12, var_8)), var_13));
    #pragma endscop
}
