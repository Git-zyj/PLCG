/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 33942
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=33942 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/33942
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */short) min((var_11), (((/* implicit */short) (-((-((~(var_6))))))))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 1; i_0 < 12; i_0 += 1) 
    {
        var_12 = ((/* implicit */long long int) (-((~((-(285978576338026496ULL)))))));
        var_13 |= ((/* implicit */unsigned long long int) (-((~((-(-9044399286509233738LL)))))));
        /* LoopNest 3 */
        for (_Bool i_1 = 0; i_1 < 1; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 2; i_2 < 12; i_2 += 1) 
            {
                for (unsigned int i_3 = 0; i_3 < 14; i_3 += 3) 
                {
                    {
                        arr_11 [4U] &= ((/* implicit */unsigned char) (~((-((-(9044399286509233709LL)))))));
                        var_14 = ((/* implicit */_Bool) max((var_14), ((!(((/* implicit */_Bool) (+((~(-9044399286509233738LL))))))))));
                    }
                } 
            } 
        } 
        var_15 = ((/* implicit */int) (!(((/* implicit */_Bool) (~((~(((/* implicit */int) var_2)))))))));
        arr_12 [i_0] = ((/* implicit */unsigned short) (-((~((~(var_6)))))));
    }
    var_16 |= ((/* implicit */short) (~(((/* implicit */int) (!(((/* implicit */_Bool) (~(((/* implicit */int) var_1))))))))));
    var_17 = ((/* implicit */_Bool) (~((-((-(((/* implicit */int) var_2))))))));
    /* LoopNest 3 */
    for (int i_4 = 1; i_4 < 21; i_4 += 2) 
    {
        for (unsigned int i_5 = 0; i_5 < 23; i_5 += 4) 
        {
            for (short i_6 = 0; i_6 < 23; i_6 += 3) 
            {
                {
                    arr_19 [i_4] [i_4] [i_4] = ((/* implicit */unsigned long long int) (-(((/* implicit */int) (!((!(((/* implicit */_Bool) 9044399286509233725LL)))))))));
                    var_18 = (~((-((-(((/* implicit */int) arr_13 [i_4])))))));
                }
            } 
        } 
    } 
}
