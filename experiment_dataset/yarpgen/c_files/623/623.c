/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 623
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=623 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/623
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_12 = (min(var_12, ((((10 - 233) & (arr_1 [i_0] [i_1]))))));
                    arr_7 [i_2] [i_1] [11] = ((((var_2 ? -13 : ((-11 ? (arr_0 [i_0] [i_2]) : var_4)))) * ((~((max((arr_6 [i_0] [i_1] [12]), var_3)))))));
                }
            }
        }
    }
    var_13 = ((((max((var_0 || var_8), (min(var_9, var_4))))) <= (max(var_10, 10))));
    var_14 = (82 + 25);
    #pragma endscop
}
