/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116048
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = (min(var_13, (2686809777 - var_5)));
    var_14 = (min(((1 ? ((min(var_1, var_7))) : -var_5)), (((var_3 >= (4294967284 != var_0))))));

    for (int i_0 = 3; i_0 < 17;i_0 += 1)
    {
        var_15 = (min(var_15, (((var_4 | (((min(-43, -2147483635)) | (arr_1 [i_0 - 1] [i_0]))))))));
        arr_4 [i_0] [14] = ((-(((max((arr_0 [i_0]), 1))))));
    }
    #pragma endscop
}
