/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247048
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247048 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247048
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = var_2;
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 11; i_0 += 2) 
    {
        for (signed char i_1 = 3; i_1 < 9; i_1 += 2) 
        {
            {
                arr_4 [i_1] = ((/* implicit */unsigned int) arr_0 [9U]);
                arr_5 [i_1] = ((/* implicit */long long int) max((((((/* implicit */_Bool) (~(((/* implicit */int) (signed char)95))))) ? ((+(((/* implicit */int) arr_3 [i_0] [i_1] [i_1])))) : (((/* implicit */int) (signed char)-96)))), (((/* implicit */int) ((signed char) arr_0 [i_0])))));
            }
        } 
    } 
}
