/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187114
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187114 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187114
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_0] = (((((((max(-1791045411356563317, 1819997750))) ? (min(var_16, -5731797337861405301)) : (~7)))) ? ((var_1 ? ((max(var_8, (arr_2 [i_0])))) : (arr_1 [i_0]))) : var_9));
                var_19 = (max(var_19, (((((1 ? (arr_3 [10] [10]) : 5101930838793546066)) > (var_6 && 62))))));
                arr_5 [i_0] = (arr_3 [14] [i_1]);
            }
        }
    }
    var_20 = 29848;
    #pragma endscop
}
