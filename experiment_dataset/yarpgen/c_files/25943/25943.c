/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 25943
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=25943 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/25943
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = 135;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    arr_6 [i_2] [i_0] [i_0] = ((var_4 != ((((arr_0 [i_2 + 1]) ? var_3 : -158)))));
                    var_16 = ((((arr_1 [i_2 + 2] [i_2]) ? (arr_1 [i_2 + 3] [i_2 - 3]) : (arr_1 [i_2 - 3] [i_2]))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 15;i_4 += 1)
                        {
                            {
                                arr_14 [3] [i_3] [i_1] = var_1;
                                arr_15 [i_3] [i_3] [i_3] [i_3] [i_3] = -1976422028;
                                var_17 += ((var_8 >> (1976422027 - 1976422006)));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
