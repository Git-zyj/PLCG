/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24961
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24961 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24961
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((((/* implicit */_Bool) -197726402)) && (((/* implicit */_Bool) ((long long int) -197726402))));
    /* LoopSeq 1 */
    for (unsigned char i_0 = 2; i_0 < 11; i_0 += 3) 
    {
        var_12 |= ((/* implicit */short) 197726402);
        var_13 = ((/* implicit */short) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) arr_0 [i_0])) ? (3462623224U) : (((/* implicit */unsigned int) ((/* implicit */int) arr_0 [i_0]))))))));
        var_14 |= ((/* implicit */short) (signed char)-48);
        var_15 = ((/* implicit */short) var_6);
    }
    var_16 = ((/* implicit */unsigned char) max((var_16), (((/* implicit */unsigned char) var_8))));
}
