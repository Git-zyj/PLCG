/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143890
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143890 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143890
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        arr_2 [i_0] = var_8;
        arr_3 [19] = 18446744073709551612;
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 22;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 25;i_3 += 1)
                {
                    {
                        var_15 = ((!(((!57344) || 11466335953661815148))));
                        var_16 ^= max((arr_0 [i_2 - 2]), (((arr_0 [i_2 + 1]) ? 30 : (arr_0 [i_2 - 1]))));
                        arr_12 [i_0] [i_0] [i_0] [i_0] |= ((((arr_5 [i_2 + 3]) || (arr_5 [i_2 + 3]))));
                    }
                }
            }
        }
    }
    var_17 = var_1;
    #pragma endscop
}
