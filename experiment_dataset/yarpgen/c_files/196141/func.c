/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 196141
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=196141 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/196141
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */unsigned int) var_15);
    var_17 = ((/* implicit */unsigned short) var_15);
    /* LoopSeq 2 */
    /* vectorizable */
    for (unsigned short i_0 = 0; i_0 < 22; i_0 += 4) /* same iter space */
    {
        var_18 = ((/* implicit */unsigned short) max((var_18), (((/* implicit */unsigned short) ((((/* implicit */_Bool) arr_3 [i_0] [i_0])) ? (((unsigned long long int) arr_1 [(_Bool)1])) : (((/* implicit */unsigned long long int) ((/* implicit */int) (unsigned short)5620))))))));
        arr_4 [i_0] = ((/* implicit */unsigned char) arr_1 [0LL]);
    }
    for (unsigned short i_1 = 0; i_1 < 22; i_1 += 4) /* same iter space */
    {
        var_19 = ((/* implicit */int) ((((/* implicit */_Bool) (unsigned char)191)) ? (2374505228U) : (((/* implicit */unsigned int) 12032118))));
        arr_7 [20U] [i_1] = ((/* implicit */unsigned short) (_Bool)1);
        var_20 = ((/* implicit */unsigned char) var_7);
    }
    var_21 &= ((/* implicit */unsigned char) var_2);
}
