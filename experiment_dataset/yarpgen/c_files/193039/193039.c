/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193039
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193039 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193039
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 *= ((-(var_4 == var_7)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_13 = (((((-((var_0 ? var_3 : var_0))))) ? (arr_12 [i_1] [8] [8] [i_3]) : var_1));
                            var_14 = 138;
                            arr_13 [1] [i_1] [i_3] = ((((((arr_9 [i_3] [5] [i_1] [i_0]) ? (min(2886171853, var_2)) : var_0))) ? (!var_5) : (((arr_9 [i_0] [i_1] [i_2] [i_3]) ? (arr_5 [i_0 + 3] [i_1]) : 31246))));
                        }
                    }
                }
                var_15 = (min(var_15, (((min(var_7, var_7))))));
                arr_14 [i_0] [i_0] = -var_0;
            }
        }
    }
    var_16 = (max(var_2, (((-((var_7 ? var_1 : var_10)))))));
    var_17 += ((~((~(~var_11)))));
    #pragma endscop
}
