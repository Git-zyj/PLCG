/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29558
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29558 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29558
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */long long int) var_0);
    /* LoopNest 2 */
    for (unsigned char i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_18 = ((/* implicit */unsigned char) ((short) ((int) (~(5671758847264200262LL)))));
                var_19 = ((/* implicit */_Bool) (-(max((((((/* implicit */_Bool) var_11)) ? (17750547512931370011ULL) : (((/* implicit */unsigned long long int) arr_4 [11ULL] [i_1] [i_1])))), (((/* implicit */unsigned long long int) ((signed char) arr_5 [i_0] [i_1] [i_1])))))));
            }
        } 
    } 
}
