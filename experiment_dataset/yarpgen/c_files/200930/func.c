/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200930
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200930 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200930
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */signed char) max((var_14), (((/* implicit */signed char) var_5))));
    /* LoopNest 3 */
    for (short i_0 = 0; i_0 < 10; i_0 += 4) 
    {
        for (long long int i_1 = 0; i_1 < 10; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 10; i_2 += 1) 
            {
                {
                    var_15 += ((/* implicit */short) (~(((/* implicit */int) arr_5 [i_2] [i_2] [i_1] [i_0]))));
                    var_16 += ((/* implicit */int) var_1);
                    var_17 = min((((/* implicit */int) (signed char)48)), ((-((-(0))))));
                    arr_8 [i_0] [i_2] = ((/* implicit */short) 9223372036854775786LL);
                }
            } 
        } 
    } 
    var_18 -= ((/* implicit */long long int) var_7);
}
