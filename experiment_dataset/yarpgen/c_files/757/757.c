/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 757
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=757 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/757
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = -279807891;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_17 = (((min(var_12, var_9))));
                                var_18 = 124;
                                var_19 *= (((111 | 5739735124552377751) ? 6954396252170679220 : 1023));
                                var_20 *= (~(((((var_7 << (145 - 110)))) ? (((min(17494, var_15)))) : (min(255, (arr_11 [i_0] [i_0] [0] [i_4] [i_3] [i_4]))))));
                            }
                        }
                    }
                    /* LoopNest 2 */
                    for (int i_5 = 0; i_5 < 12;i_5 += 1)
                    {
                        for (int i_6 = 2; i_6 < 8;i_6 += 1)
                        {
                            {
                                var_21 *= var_13;
                                var_22 = (min((arr_3 [i_0] [i_0]), var_14));
                                arr_18 [i_1] [i_0] [i_5] [i_6 - 2] = (arr_17 [i_6] [i_6] [i_5] [i_2] [i_1] [i_1 - 1] [0]);
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
