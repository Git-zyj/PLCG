/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [1] = (((((arr_2 [i_0] [i_0]) >= ((1 ? 14659570118371613182 : var_11)))) && ((((-6629294409737707497 == var_12) ? ((2754627306450402751 ? var_1 : var_3)) : (arr_2 [5] [i_0]))))));
                arr_6 [i_0] [i_1] [14] = var_5;
            }
        }
    }
    var_13 = var_12;
    var_14 = -2754627306450402759;
    #pragma endscop
}
