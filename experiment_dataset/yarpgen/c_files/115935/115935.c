/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115935
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115935 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115935
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_14 = (+(((arr_2 [i_0] [i_0] [i_1]) ? (arr_4 [i_0] [i_0] [i_0]) : (arr_1 [i_0] [i_1]))));
                arr_5 [i_0] = (((arr_4 [i_0] [i_1] [2]) ? ((max((arr_3 [i_0]), (arr_3 [i_0])))) : (((max(18446744073709551615, (arr_2 [7] [i_0] [i_0])))))));
                var_15 = (min(var_15, ((((var_2 ? 2147483647 : (arr_3 [1])))))));
            }
        }
    }
    var_16 = (~var_11);
    #pragma endscop
}
