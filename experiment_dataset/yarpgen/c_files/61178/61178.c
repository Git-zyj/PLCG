/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61178
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61178 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61178
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((!((max(104, (max(104, var_7)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_4 [i_0] = ((((min((arr_3 [i_0]), (arr_1 [i_0] [6])))) || -106));
                var_11 = 242;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 13;i_3 += 1)
                    {
                        {
                            var_12 = (((arr_5 [i_3 - 1] [i_3 + 1]) ? (((arr_5 [i_3 - 1] [i_3 - 1]) / (arr_5 [i_3 + 1] [i_3 - 1]))) : (((arr_5 [i_3 + 1] [i_3 - 1]) ? (arr_5 [i_3 + 1] [i_3 - 1]) : 68))));
                            var_13 = ((((((~((max(106, 0))))) + 2147483647)) << (((-64 + 75) - 11))));
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
