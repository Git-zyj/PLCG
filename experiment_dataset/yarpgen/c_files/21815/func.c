/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 21815
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=21815 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/21815
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */signed char) 1073741823ULL);
    var_11 = ((/* implicit */unsigned short) ((((/* implicit */int) (short)-6486)) * (((/* implicit */int) (short)0))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        for (int i_1 = 0; i_1 < 24; i_1 += 4) 
        {
            for (unsigned short i_2 = 2; i_2 < 23; i_2 += 4) 
            {
                {
                    var_12 += ((/* implicit */int) min((((/* implicit */unsigned char) (_Bool)1)), ((unsigned char)55)));
                    var_13 = ((/* implicit */signed char) (_Bool)1);
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned long long int) ((((/* implicit */int) (signed char)62)) + (((/* implicit */int) (_Bool)1))));
}
