/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151708
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151708 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151708
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_4;
    var_12 = ((((!(~32))) ? (!var_5) : (((!var_8) * ((var_7 ? var_8 : var_0))))));
    var_13 = (max(var_13, (((1 - (((117 - var_2) ? 7912424864865916029 : var_8)))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 20;i_2 += 1)
            {
                {
                    var_14 = -7912424864865916028;
                    var_15 ^= ((var_1 ? ((((((!(arr_3 [i_0] [i_0] [i_0]))) && (((7912424864865916016 ? var_10 : 2825050695))))))) : ((((arr_2 [20]) && 9223372036853727232)))));
                }
            }
        }
    }
    var_16 = (~1023);
    #pragma endscop
}
