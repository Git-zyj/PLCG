/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 98113
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=98113 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/98113
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 += (!var_5);
    var_17 = (max(var_17, ((((var_8 ? var_12 : -1))))));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        arr_2 [i_0] = ((-(1 ^ 32762)));
        var_18 = var_9;
        /* LoopNest 2 */
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {

                    for (int i_3 = 1; i_3 < 22;i_3 += 1)
                    {
                        arr_11 [i_0] [i_3 - 1] [i_2] [i_1] [i_0] = (max(54044, (((224 > (9475 >> var_7))))));
                        arr_12 [i_0] [i_1] [i_1 - 2] [i_1] = var_0;
                    }
                    var_19 -= (max((max(231, 1018)), var_12));
                    var_20 = (arr_7 [i_0] [i_1]);
                    arr_13 [i_0] [i_0] [12] [i_0] = ((-(((arr_1 [i_0]) / var_3))));
                    arr_14 [i_0] [i_1] = (arr_7 [i_0] [i_2]);
                }
            }
        }
    }
    #pragma endscop
}
