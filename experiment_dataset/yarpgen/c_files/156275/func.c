/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156275
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156275 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156275
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */long long int) ((((/* implicit */int) (unsigned short)22352)) < (((/* implicit */int) (unsigned short)43184))));
    /* LoopNest 3 */
    for (unsigned char i_0 = 1; i_0 < 23; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 23; i_1 += 2) 
        {
            for (unsigned char i_2 = 1; i_2 < 23; i_2 += 2) 
            {
                {
                    var_18 ^= ((/* implicit */long long int) arr_7 [i_2] [0ULL] [0ULL] [i_0 + 1]);
                    arr_9 [i_0] = ((/* implicit */unsigned char) (-(((/* implicit */int) (!(((/* implicit */_Bool) arr_2 [i_0] [i_1 + 2])))))));
                    var_19 -= ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (short)29930)) || (((/* implicit */_Bool) (unsigned char)187))));
                    var_20 -= ((/* implicit */unsigned long long int) 146947729676072499LL);
                }
            } 
        } 
    } 
    var_21 ^= ((/* implicit */unsigned long long int) var_8);
    var_22 = ((/* implicit */_Bool) var_7);
}
