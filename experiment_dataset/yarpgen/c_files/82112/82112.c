/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82112
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82112 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82112
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_20 = var_19;
                arr_7 [i_0] = var_7;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                var_21 = var_11;

                for (int i_4 = 0; i_4 < 17;i_4 += 1)
                {
                    arr_16 [i_4] [i_2] [i_2] [i_2 - 1] = var_19;
                    arr_17 [i_2] = var_1;
                    arr_18 [i_2] [4] [i_3] [i_2] = var_3;
                }
                for (int i_5 = 0; i_5 < 17;i_5 += 1)
                {

                    for (int i_6 = 4; i_6 < 15;i_6 += 1) /* same iter space */
                    {
                        arr_25 [i_2] = var_3;
                        var_22 = (max(var_22, var_19));
                        var_23 = var_15;
                    }
                    /* vectorizable */
                    for (int i_7 = 4; i_7 < 15;i_7 += 1) /* same iter space */
                    {

                        for (int i_8 = 2; i_8 < 15;i_8 += 1) /* same iter space */
                        {
                            var_24 += var_14;
                            arr_30 [i_2] = var_8;
                        }
                        for (int i_9 = 2; i_9 < 15;i_9 += 1) /* same iter space */
                        {
                            var_25 = (max(var_25, var_7));
                            arr_33 [i_2] = var_5;
                            var_26 = var_10;
                            arr_34 [i_2] [i_2] [i_5] [1] [i_9 - 2] = var_9;
                        }
                        arr_35 [i_2] [i_2] = var_12;
                        var_27 = var_16;
                    }
                    arr_36 [i_2] [i_3] = var_16;
                    var_28 = var_8;
                    arr_37 [i_2] [16] [i_2] [i_2] = var_0;
                }
            }
        }
    }
    #pragma endscop
}
