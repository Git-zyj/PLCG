/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241613
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241613 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241613
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = ((~(((((~(arr_0 [i_0])))) ? (~var_3) : 0))));
        var_11 |= ((((-(arr_1 [i_0] [i_0])))) ? ((1 + (((arr_0 [i_0]) - 1)))) : ((min((arr_1 [i_0] [i_0]), 20041))));
    }
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        var_12 = (max(var_12, var_8));
        arr_5 [i_1] = 1;
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 10;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 10;i_3 += 1)
            {
                {
                    var_13 += (!27366);
                    /* LoopNest 2 */
                    for (int i_4 = 3; i_4 < 8;i_4 += 1)
                    {
                        for (int i_5 = 2; i_5 < 9;i_5 += 1)
                        {
                            {
                                var_14 -= -var_7;
                                arr_17 [i_1] [i_2] [i_2] [i_2] [i_5] [i_2] = (arr_11 [i_1] [i_1] [i_1]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_15 |= ((var_3 ? (max((~var_3), ((min(30749, 255))))) : var_1));
    var_16 = (((max(var_8, (var_6 == var_8))) ? -3466683652 : ((min(-0, var_7)))));
    #pragma endscop
}
