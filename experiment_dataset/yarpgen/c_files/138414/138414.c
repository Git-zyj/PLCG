/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138414
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138414 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138414
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = var_9;
    var_14 = (max(var_14, ((min(((-4515589031230151314 ? (max(16, 10012791608516891784)) : var_12)), var_9)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] = (max((2063640720 % 4514582639079670238), (((-(arr_5 [i_0] [i_1] [i_0]))))));
                var_15 = ((68451041280 ? ((9 >> (((2231326575 % 30850) - 7760)))) : (((arr_0 [i_0] [i_1]) % var_1))));
            }
        }
    }
    #pragma endscop
}
