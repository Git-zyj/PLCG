/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 30504
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=30504 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/30504
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (((max((min(var_15, var_0)), var_15)) & ((max(469762048, ((min(10, 255))))))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_4 [1] [i_0] |= ((~(!1073217536)));
        arr_5 [i_0] = (((!var_4) ? var_17 : (var_19 & var_17)));
        arr_6 [i_0] = (!var_1);
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_11 [i_0] [i_1] [i_1] = (102 > 12);
                    var_21 = ((var_9 == (var_12 || var_12)));
                }
            }
        }
        arr_12 [i_0] = (((~var_13) ? -576459041 : var_13));
    }
    #pragma endscop
}
