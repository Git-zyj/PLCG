/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224906
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224906 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224906
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_13 [i_0] [i_1] [i_1] [i_2] [i_3] [i_1] = ((+(min((arr_4 [i_0] [i_3] [i_3] [i_0]), (min(var_0, 536649456))))));
                                arr_14 [i_0] [i_0] [i_2] [i_3] [i_4] [i_2] [17] = 991000000277222223;
                            }
                        }
                    }
                    var_18 = ((~(((((min((arr_9 [i_0] [i_0] [i_2] [i_2]), var_17))) < var_4)))));
                }
            }
        }
    }
    var_19 |= var_1;
    var_20 = var_3;
    #pragma endscop
}
