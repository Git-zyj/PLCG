/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197845
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((26659 >> (2043440029 - 2043440002)));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 3; i_4 < 11;i_4 += 1)
                        {
                            {
                                var_11 ^= (((38868 + 2147483647) >> ((((min(26686, var_1))) - 26676))));
                                arr_16 [i_1 + 1] [i_4] [i_3] = ((38845 ? 26691 : 1694747456163976312));
                            }
                        }
                    }
                    var_12 *= (min((((arr_9 [i_1] [i_0 - 4] [i_0 - 4]) ? (arr_9 [i_0] [i_0 - 4] [i_0 - 4]) : var_3)), ((-(arr_9 [i_0] [i_0 - 2] [i_0 - 2])))));
                }
            }
        }
    }
    var_13 = var_1;
    #pragma endscop
}
