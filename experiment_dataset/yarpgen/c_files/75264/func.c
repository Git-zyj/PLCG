/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 75264
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=75264 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/75264
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 1; i_0 < 20; i_0 += 1) 
    {
        var_14 = ((/* implicit */signed char) max((min((((long long int) var_5)), (((/* implicit */long long int) var_7)))), (((/* implicit */long long int) var_3))));
        arr_2 [i_0] [i_0] = ((/* implicit */_Bool) var_10);
    }
    var_15 = ((/* implicit */short) min((((/* implicit */unsigned int) (_Bool)0)), (var_13)));
}
