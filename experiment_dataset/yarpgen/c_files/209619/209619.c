/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209619
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209619 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209619
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (((max((-9223372036854775807 - 1), var_0)) - (((((var_0 <= 26) > (max(var_11, -5))))))));
    var_15 = max((max((!var_3), (~var_13))), -27);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [i_2] = var_0;
                    arr_9 [i_2] [i_1 - 1] = ((((min(((-(arr_5 [i_0] [i_0]))), ((max((arr_3 [i_0] [i_1] [6]), var_1)))))) ? (((-1886415781 ? (arr_1 [i_2]) : (2127 < var_12)))) : (max(8, 0))));
                }
            }
        }
    }
    #pragma endscop
}
