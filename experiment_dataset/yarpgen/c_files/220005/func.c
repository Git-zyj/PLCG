/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220005
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 8; i_0 += 3) 
    {
        for (unsigned int i_1 = 2; i_1 < 11; i_1 += 1) 
        {
            {
                var_16 = ((/* implicit */unsigned long long int) ((var_11) ? ((-(503316480U))) : (((/* implicit */unsigned int) ((((/* implicit */_Bool) min((var_2), (1609773695U)))) ? (min((((/* implicit */int) arr_3 [i_0 + 1])), (1728191448))) : ((-(-1728191448))))))));
                var_17 = ((/* implicit */int) (unsigned short)65535);
            }
        } 
    } 
    var_18 = ((/* implicit */int) (((-(var_2))) * (((((/* implicit */_Bool) var_5)) ? (var_2) : (min((var_7), (((/* implicit */unsigned int) -1728191448))))))));
}
