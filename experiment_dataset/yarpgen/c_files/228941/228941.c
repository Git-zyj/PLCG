/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 228941
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=228941 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/228941
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_15 = (arr_0 [i_0]);
                arr_5 [i_0] [i_0] = var_0;
                var_16 -= ((var_14 * ((2147483648 & (((-127 - 1) ? -16478 : 2254702970))))));
                var_17 = (min(var_17, var_11));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        for (int i_3 = 4; i_3 < 10;i_3 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 12;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 14;i_5 += 1)
                    {
                        {
                            var_18 = (max(((167 ? var_8 : var_1)), var_2));
                            var_19 = var_0;
                            var_20 = ((+((0 ? (((min((-127 - 1), (arr_2 [i_2]))))) : 17698745365242466158))));
                        }
                    }
                }
                arr_15 [i_2] [i_2] [i_2] = -4094;
            }
        }
    }
    var_21 = var_8;
    #pragma endscop
}
