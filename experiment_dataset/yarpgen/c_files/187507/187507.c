/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187507
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187507 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187507
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = min((((min(var_3, var_10)) % (~var_6))), (~var_2));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1 - 1] = ((!((var_3 == (((min(var_17, var_15))))))));
                arr_5 [i_0] [15] [i_0] = ((-(min(var_10, (!var_15)))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 14;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 14;i_3 += 1)
        {
            {
                var_19 = ((min((~var_13), var_2)));
                var_20 = (!var_0);
                var_21 = (min(var_21, ((((var_15 || var_6) == -var_12)))));
                arr_12 [1] = (max((((var_17 & (~var_5)))), (716213202 && var_11)));
            }
        }
    }
    #pragma endscop
}
