/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 240252
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=240252 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/240252
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 1; i_0 < 22; i_0 += 2) 
    {
        var_17 -= ((/* implicit */long long int) (!(((/* implicit */_Bool) (unsigned char)19))));
        var_18 = ((/* implicit */_Bool) ((signed char) ((((/* implicit */_Bool) 2725589747U)) ? (((/* implicit */int) (signed char)100)) : (((/* implicit */int) (short)4095)))));
        var_19 = ((/* implicit */short) var_13);
    }
    var_20 += ((/* implicit */unsigned int) var_3);
    var_21 = ((/* implicit */short) var_14);
}
