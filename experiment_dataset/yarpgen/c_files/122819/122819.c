/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122819
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122819 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122819
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_8;
    var_11 += (min(((102 * ((161 ? 102 : 94)))), ((min(95, 168)))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_12 ^= ((var_9 ? 15746 : var_0));
                    var_13 ^= arr_3 [i_0 + 1] [i_1] [i_2];
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_14 = arr_6 [i_1] [i_2] [i_4];
                                var_15 = ((-(min((min(var_1, 791250599)), var_8))));
                            }
                        }
                    }
                }
            }
        }
    }

    /* vectorizable */
    for (int i_5 = 0; i_5 < 20;i_5 += 1) /* same iter space */
    {
        arr_16 [i_5] = var_7;

        for (int i_6 = 0; i_6 < 20;i_6 += 1)
        {
            arr_19 [i_5] [1] = (((arr_14 [i_5]) || var_5));
            var_16 = (min(var_16, 102));
        }
        var_17 = var_3;
    }
    for (int i_7 = 0; i_7 < 20;i_7 += 1) /* same iter space */
    {
        var_18 *= ((161 ? 160 : 55475));
        var_19 = var_1;
    }
    #pragma endscop
}
