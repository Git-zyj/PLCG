/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175149
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175149 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175149
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 += 134217727;

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_15 = arr_2 [i_0];
        var_16 = var_0;
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 24;i_1 += 1)
    {
        /* LoopNest 3 */
        for (int i_2 = 2; i_2 < 23;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 24;i_3 += 1)
            {
                for (int i_4 = 0; i_4 < 24;i_4 += 1)
                {
                    {
                        var_17 = (min(var_17, (arr_7 [5])));
                        var_18 = ((var_13 ? (arr_8 [i_1] [i_2] [1]) : (arr_3 [i_2 + 1] [i_2])));
                    }
                }
            }
        }
        var_19 = (min(var_19, ((~(arr_12 [i_1] [i_1] [i_1] [i_1])))));
        arr_15 [i_1] = ((65535 ? 204254734697595368 : 8589934576));
        var_20 = ((arr_5 [1] [i_1]) ? -1 : ((~(arr_10 [i_1] [i_1]))));
        arr_16 [i_1] = 1;
    }
    var_21 = 349342281;
    #pragma endscop
}
