/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 178859
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=178859 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/178859
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {

        /* vectorizable */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            arr_6 [18] = (((var_2 <= 25395) ? (arr_0 [i_1]) : (1857278607 & -75)));
            arr_7 [10] [10] = ((-75 ? ((var_7 ? (arr_3 [i_0] [i_0]) : var_0)) : (((-75 ? 62250 : -100)))));
        }
        arr_8 [1] [i_0 + 1] = (((25395 | 25395) | var_4));
    }
    var_10 = var_4;
    var_11 = (((~var_1) ? ((min(var_5, -61))) : (-25395 / var_5)));
    var_12 = (min(-var_1, (30332 >= var_1)));
    #pragma endscop
}
