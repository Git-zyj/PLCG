/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = 2147483647;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_21 = ((-1943426699240273982 << (-2532288506663816841 && var_4)));
        arr_2 [i_0] = var_17;
        arr_3 [13] = (var_3 > var_3);
        var_22 = (max(var_22, (((104 ? 830371337879512689 : 4960034533451936690)))));
    }
    var_23 = (-var_19 % 9223372036854775807);
    var_24 = var_18;
    var_25 *= (var_18 | var_5);
    #pragma endscop
}
