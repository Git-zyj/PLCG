/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 62673
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=62673 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/62673
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_0] = (arr_1 [i_0] [i_1]);
                var_19 = (max((((arr_1 [i_1 - 2] [i_1]) / (16140084911350419792 * 7524724223397072376))), ((min((arr_0 [i_1 + 1] [i_1]), (arr_0 [i_1 + 1] [i_1]))))));
            }
        }
    }
    var_20 = (((58 / 7524724223397072376) ? (((((var_3 ? -35 : var_3))) ? (var_4 | var_7) : var_12)) : 1));
    #pragma endscop
}
