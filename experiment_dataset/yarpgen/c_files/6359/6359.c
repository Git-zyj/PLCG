/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 6359
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=6359 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/6359
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min((max((max(var_12, var_13)), ((min(79, 93))))), (!var_7)));
    var_15 = 10146914718153394087;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 9;i_1 += 1)
        {
            {
                var_16 &= var_8;
                var_17 = (arr_3 [i_0] [i_1]);
                var_18 = var_11;
                var_19 &= var_13;
            }
        }
    }
    var_20 = (((((var_9 | -1) - ((var_11 ? var_12 : var_0)))) ^ ((((var_13 != var_3) ? ((var_7 ? var_1 : 1647531810)) : var_7)))));
    var_21 -= ((!((min((var_13 & var_10), -var_3)))));
    #pragma endscop
}
