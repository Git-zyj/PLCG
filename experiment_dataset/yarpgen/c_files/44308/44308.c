/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 44308
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=44308 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/44308
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 19;i_2 += 1)
            {
                {
                    var_13 = (min(((((arr_1 [i_0 - 2]) && (arr_1 [i_0 - 1])))), (255 * 244)));
                    var_14 = (min(-var_2, (((arr_0 [i_1 - 3] [i_1 + 3]) - (arr_0 [i_1 - 3] [i_1 + 3])))));
                    var_15 = (arr_0 [i_2 - 2] [i_0 + 1]);
                    var_16 = (arr_3 [i_1 - 3] [i_0 - 2]);
                }
            }
        }
    }
    var_17 = (!2535022925);
    var_18 = var_8;
    var_19 |= (171445731870996601 == var_0);
    var_20 = (min(249, (min(var_4, ((min(var_0, var_11)))))));
    #pragma endscop
}
