/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249140
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249140 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249140
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_17 = (min(var_17, (((!((!(arr_1 [i_0] [i_0]))))))));
        arr_2 [i_0] = ((((!(arr_1 [i_0] [i_0]))) && ((min((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0]))))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        var_18 |= ((!(arr_1 [i_1] [i_1])));
        arr_5 [i_1] [i_1] = (!var_7);
        arr_6 [7] = ((var_1 ? -12873 : (((arr_4 [i_1]) ? (arr_0 [i_1]) : var_3))));
    }
    var_19 |= (((((4294967296 ? -var_16 : var_11))) ? var_1 : var_10));
    var_20 = ((var_15 && ((min(((10698398293210086755 ? var_1 : var_4)), var_1)))));
    var_21 = (max(var_21, (((var_2 ? (~var_10) : (~var_10))))));
    #pragma endscop
}
