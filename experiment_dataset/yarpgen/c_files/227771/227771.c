/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227771
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227771 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227771
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_20 = (max(var_20, -5691579163231144743));
                    var_21 = ((((-(min((arr_5 [i_1] [i_1]), (arr_0 [i_0]))))) | (min(var_12, (arr_0 [i_0])))));
                    arr_8 [i_1] [i_0] [i_1] [i_2] = (max((((arr_2 [i_1] [i_2]) ? (arr_1 [i_2]) : (((arr_6 [i_2] [i_1] [i_0] [i_0]) ? (arr_7 [i_1] [i_1]) : var_10)))), (((var_8 / (arr_6 [i_2] [i_2] [i_2] [i_2]))))));
                }
            }
        }
    }
    var_22 = (0 + 57);
    #pragma endscop
}
