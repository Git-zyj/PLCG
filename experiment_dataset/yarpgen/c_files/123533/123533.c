/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123533
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123533 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123533
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_10 = (((!(-2147483647 - 1))));
                arr_4 [i_0] = (((((arr_0 [i_0]) ^ 0)) ^ 103));
                var_11 = ((106 >> (-103 + 119)));
                var_12 = (((((arr_1 [i_0 + 2] [13]) || (arr_1 [i_0 + 2] [i_1])))) <= ((min((arr_3 [i_0 + 2] [2] [i_1]), (arr_3 [i_0 + 2] [i_0 + 2] [i_0 + 2])))));
            }
        }
    }
    var_13 += (((((max(-6, var_8)))) ^ var_1));
    var_14 = (max(((var_5 ? (9223372036854775807 > var_2) : 40110)), var_3));
    var_15 = var_8;
    #pragma endscop
}
