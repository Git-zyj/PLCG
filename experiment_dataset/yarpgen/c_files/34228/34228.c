/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 34228
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=34228 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/34228
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_18 = (((arr_0 [i_0] [i_0]) * (max((((arr_0 [i_0] [i_0]) ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0]))), (arr_1 [i_0])))));
        var_19 += ((!((max((arr_0 [i_0] [i_0]), (((arr_0 [i_0] [i_0]) ? (arr_1 [i_0]) : (arr_1 [i_0]))))))));
    }
    var_20 = (max(((((10721 && 861300921385409439) && (var_16 && var_14)))), ((var_2 - (!var_0)))));
    #pragma endscop
}
