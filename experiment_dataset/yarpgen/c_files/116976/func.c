/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116976
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116976 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116976
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned int) var_12);
    /* LoopNest 2 */
    for (unsigned char i_0 = 1; i_0 < 10; i_0 += 3) 
    {
        for (long long int i_1 = 1; i_1 < 11; i_1 += 4) 
        {
            {
                var_14 |= ((/* implicit */unsigned char) (-(arr_3 [i_0 + 2])));
                arr_4 [i_0] [i_1] = ((/* implicit */short) ((((/* implicit */_Bool) max((((/* implicit */unsigned long long int) (+(((/* implicit */int) arr_0 [i_0] [i_1]))))), (((((/* implicit */_Bool) (signed char)-83)) ? (((/* implicit */unsigned long long int) ((/* implicit */int) (short)-26273))) : (arr_1 [i_0] [i_0 + 2])))))) ? (arr_1 [2LL] [i_1 + 1]) : (((/* implicit */unsigned long long int) var_12))));
                arr_5 [i_0] = ((/* implicit */_Bool) arr_3 [i_0]);
            }
        } 
    } 
}
