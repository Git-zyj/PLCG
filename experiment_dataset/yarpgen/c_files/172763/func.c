/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 172763
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=172763 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/172763
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */_Bool) var_10);
    var_13 = ((/* implicit */unsigned char) var_7);
    /* LoopSeq 1 */
    for (unsigned long long int i_0 = 2; i_0 < 13; i_0 += 2) 
    {
        var_14 = ((/* implicit */_Bool) var_11);
        var_15 = ((/* implicit */unsigned int) min((var_15), (((/* implicit */unsigned int) ((((/* implicit */int) arr_2 [i_0])) != (((/* implicit */int) arr_0 [i_0])))))));
    }
    var_16 = ((/* implicit */unsigned char) max((min((((/* implicit */long long int) min((((/* implicit */unsigned int) var_0)), (var_10)))), (var_9))), (((/* implicit */long long int) ((signed char) var_8)))));
}
