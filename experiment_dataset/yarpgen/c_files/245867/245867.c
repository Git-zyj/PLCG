/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245867
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245867 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245867
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = max(0, 5017125661466036152);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 23;i_1 += 1)
        {
            {
                arr_7 [20] [i_1 + 2] [i_1 - 2] = (((((((100 ? 232 : (arr_3 [i_1 - 1])))) ? (var_3 < var_2) : (arr_3 [i_0]))) > (arr_4 [i_0])));
                arr_8 [i_0] [i_1] = (((arr_0 [i_1]) ? ((((((arr_1 [6]) + 4016))))) : (((arr_3 [i_0]) - var_5))));
            }
        }
    }
    #pragma endscop
}
