/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221380
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221380 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221380
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 3; i_0 < 14; i_0 += 1) 
    {
        for (long long int i_1 = 3; i_1 < 15; i_1 += 2) 
        {
            for (unsigned int i_2 = 0; i_2 < 16; i_2 += 4) 
            {
                {
                    var_10 -= ((/* implicit */int) (!(((/* implicit */_Bool) ((min((var_5), (((/* implicit */int) arr_6 [i_0] [i_0] [i_2])))) ^ (((((((/* implicit */int) arr_3 [i_0] [i_0] [i_2])) + (2147483647))) << (((((/* implicit */int) arr_1 [i_1])) - (4355))))))))));
                    var_11 ^= ((((/* implicit */_Bool) var_3)) ? (((/* implicit */int) (!(((/* implicit */_Bool) var_3))))) : (((/* implicit */int) (!(((/* implicit */_Bool) var_5))))));
                }
            } 
        } 
    } 
    var_12 = var_3;
    var_13 = ((/* implicit */signed char) var_4);
}
