/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241643
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241643 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241643
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 &= ((-(((max(var_4, var_14)) - var_3))));
    var_18 |= var_7;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_19 = ((min(3, (min(-4, (arr_2 [i_1 - 1]))))));
                    arr_8 [i_1] = (arr_5 [0] [i_1 - 1] [12] [10]);
                    arr_9 [i_1] [1] [i_2] [i_0] |= (max((arr_7 [i_2]), (!-8)));
                }
            }
        }
    }
    var_20 += (min(46847, var_16));
    var_21 = (((((min((((18689 ? -4 : -32758))), -var_5)) + 9223372036854775807)) << ((var_9 * (var_13 >= var_9)))));
    #pragma endscop
}
