/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73241
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73241 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73241
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_12;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_15 ^= (!var_5);
                arr_4 [i_0] [i_0 + 2] [i_0] = ((((((max(var_13, var_1)) % var_2))) || var_7));
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 7;i_3 += 1)
                    {
                        {
                            arr_9 [i_0] [i_1] [i_1] [i_2] = (((min((((-310617760 ? 31 : var_2))), ((var_3 ? var_2 : var_11)))) / var_7));
                            arr_10 [i_0] [i_2] [i_0 + 2] [i_0 - 1] [i_0] = 9223372036854775793;
                        }
                    }
                }
            }
        }
    }
    var_16 = (~-600526747);
    #pragma endscop
}
