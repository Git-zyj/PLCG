/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164989
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164989 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164989
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */int) ((short) max((((/* implicit */int) ((unsigned char) var_7))), (var_1))));
    var_14 += ((/* implicit */unsigned long long int) var_10);
    /* LoopNest 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (int i_1 = 2; i_1 < 7; i_1 += 3) 
        {
            {
                var_15 += ((/* implicit */signed char) ((_Bool) ((((((/* implicit */_Bool) (unsigned char)190)) ? (((/* implicit */int) (unsigned char)74)) : (((/* implicit */int) (unsigned char)81)))) + (((/* implicit */int) (unsigned char)70)))));
                var_16 -= ((/* implicit */signed char) var_3);
            }
        } 
    } 
}
