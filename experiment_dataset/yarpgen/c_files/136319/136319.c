/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136319
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136319 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136319
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_0;
    var_18 = (min((((!(var_9 % 225)))), var_7));
    var_19 *= var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_7 [i_0] = ((((max((~var_16), var_11))) || var_11));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        {
                            arr_13 [5] [i_0] [i_2] [12] = ((((arr_8 [i_0] [i_2] [i_3]) ? (arr_8 [i_1] [i_1] [i_2]) : (arr_8 [i_0] [i_0] [0]))) & (arr_8 [i_0] [3] [i_0]));
                            var_20 = (arr_2 [2] [i_2]);
                        }
                    }
                }
            }
        }
    }
    var_21 = (max(var_0, (var_16 & -1317300410770544861)));
    #pragma endscop
}
