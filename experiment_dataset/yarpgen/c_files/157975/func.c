/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157975
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157975 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157975
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 -= ((/* implicit */_Bool) var_9);
    var_21 = ((unsigned short) min((((/* implicit */unsigned long long int) var_15)), (max((((/* implicit */unsigned long long int) var_18)), (var_4)))));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 2; i_0 < 18; i_0 += 2) 
    {
        var_22 = ((/* implicit */signed char) (+(((long long int) var_9))));
        var_23 = ((/* implicit */unsigned long long int) min((((/* implicit */unsigned int) min((((/* implicit */unsigned short) var_10)), (var_12)))), (var_8)));
    }
}
