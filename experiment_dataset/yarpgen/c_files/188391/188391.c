/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188391
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188391 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188391
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 += var_7;

    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        var_19 = (min(var_19, ((((((4 | 9012161313635596803) - (var_0 * 248))) == ((min((((arr_3 [i_0 - 2] [i_0 - 2]) - (arr_1 [i_0] [i_0]))), -var_12))))))));
        arr_4 [i_0] [i_0 - 2] = (arr_1 [21] [i_0]);
        var_20 = (max(var_20, 4));
        arr_5 [i_0] = ((var_14 <= (max((arr_1 [i_0] [i_0 - 2]), (arr_1 [i_0 + 1] [i_0 - 2])))));
    }
    #pragma endscop
}
