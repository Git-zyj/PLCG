/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223876
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223876 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223876
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = -11;
    var_21 = (min((var_9 + var_10), (((var_14 ? var_7 : var_6)))));
    var_22 = var_15;

    /* vectorizable */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_23 = var_19;
        var_24 = (((((-(arr_1 [i_0])))) ? (arr_1 [i_0]) : (((arr_0 [14]) ? (arr_1 [i_0]) : var_9))));
        var_25 *= ((arr_1 [i_0]) > (arr_0 [i_0]));
        var_26 = ((arr_1 [i_0]) ? (arr_1 [i_0]) : var_8);
    }
    var_27 = (((((-((var_3 ? var_6 : var_11))))) ? var_9 : (min(var_10, var_15))));
    #pragma endscop
}
