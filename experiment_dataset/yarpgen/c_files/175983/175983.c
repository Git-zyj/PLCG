/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175983
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175983 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175983
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (~-1841033935);
    var_11 = (--49934);

    /* vectorizable */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        arr_2 [1] = (arr_0 [i_0] [i_0]);
        arr_3 [0] [1] = (((arr_1 [i_0] [i_0]) ? (arr_1 [i_0] [i_0]) : var_2));
        arr_4 [i_0] [i_0] = (arr_0 [i_0] [i_0]);
        var_12 = (((arr_0 [i_0] [i_0]) ? (~var_2) : (!18446744073709551594)));
        var_13 = ((~(arr_0 [i_0] [i_0])));
    }
    var_14 = ((((var_7 ? var_3 : 15780203504029368189)) / (((((min(var_8, var_1))) ? var_2 : (1 == 1))))));
    var_15 = var_6;
    #pragma endscop
}
