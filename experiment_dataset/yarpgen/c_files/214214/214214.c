/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 214214
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=214214 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/214214
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 *= (((~((var_3 ? 0 : var_4)))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            {
                arr_8 [i_1] = (((((((0 ? (arr_2 [i_0]) : var_0))) / (var_15 + var_8))) * (arr_3 [1] [1])));
                var_17 = (min(var_17, (((~(((arr_5 [i_1 - 1] [i_1] [i_0 + 1]) / 2072230830)))))));
                arr_9 [i_0 + 1] [i_1 - 2] [i_1 - 2] = (((arr_2 [i_0 + 1]) ? (((18446744073709551615 ? (arr_2 [i_0 - 1]) : 384))) : (((arr_7 [i_0 - 2] [i_1 + 2]) ? var_7 : var_5))));
            }
        }
    }
    #pragma endscop
}
