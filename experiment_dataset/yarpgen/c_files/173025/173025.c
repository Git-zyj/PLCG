/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173025
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173025 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173025
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1 + 1] = ((((((arr_4 [i_0] [i_0]) | (arr_0 [i_1 + 1])))) || (((~(arr_5 [i_0] [i_0] [i_0]))))));
                var_18 ^= (max((~var_16), (max((min(var_2, (arr_2 [i_0]))), (((var_15 || (arr_0 [i_0]))))))));
            }
        }
    }
    var_19 = (((var_15 - var_0) > (max(2146959360, var_16))));
    var_20 = var_9;
    #pragma endscop
}
