/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187397
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187397 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187397
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */int) min((var_15), (((/* implicit */int) ((((/* implicit */int) (signed char)-51)) != (((/* implicit */int) (!(((/* implicit */_Bool) var_6))))))))));
    /* LoopSeq 1 */
    for (long long int i_0 = 0; i_0 < 19; i_0 += 2) 
    {
        var_16 = ((/* implicit */long long int) ((int) ((signed char) (~(((/* implicit */int) (unsigned char)255))))));
        arr_3 [i_0] = ((/* implicit */long long int) var_8);
        var_17 += ((/* implicit */_Bool) (-(((/* implicit */int) (short)(-32767 - 1)))));
    }
}
