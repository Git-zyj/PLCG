/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181009
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181009 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181009
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        var_15 = 10246576399191442880;
        arr_4 [4] [6] |= ((((((arr_2 [i_0] [i_0]) % var_7))) ? (((arr_2 [i_0] [i_0]) ? (arr_2 [i_0] [i_0]) : (arr_2 [i_0] [i_0]))) : (((arr_2 [i_0] [i_0]) ^ (arr_2 [i_0] [2])))));
        var_16 = ((((!(((arr_3 [i_0] [i_0]) || (arr_1 [i_0]))))) && ((max((!var_6), (max(var_14, var_12)))))));
        var_17 = (arr_0 [i_0] [i_0]);
    }
    var_18 = (var_9 | var_2);
    #pragma endscop
}
