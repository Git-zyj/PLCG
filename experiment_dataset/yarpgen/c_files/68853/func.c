/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68853
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68853 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68853
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */int) (unsigned short)31908);
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 24; i_0 += 1) 
    {
        var_21 = ((/* implicit */unsigned char) min((var_21), (((/* implicit */unsigned char) max(((+(((long long int) (unsigned short)33628)))), ((+(var_6))))))));
        arr_3 [i_0] = ((/* implicit */short) (unsigned short)33620);
    }
    var_22 = ((/* implicit */short) (+(var_12)));
}
