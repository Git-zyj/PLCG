/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237018
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 19;i_2 += 1)
            {
                {
                    var_12 = (max(var_12, ((((~778549906) < 1625113389)))));
                    var_13 = ((-778549907 ? -27390 : -27394));
                    var_14 -= (~101);
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        for (int i_4 = 1; i_4 < 22;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 24;i_5 += 1)
            {
                {
                    var_15 = 778549919;
                    arr_15 [21] [i_4] [i_5] = ((((max((arr_10 [i_4 + 2] [i_4 + 1]), 31))) ? ((((arr_12 [i_5]) && (~-124)))) : (~-1403)));
                    arr_16 [i_3] [i_4] [1] = (arr_9 [i_4 - 1]);
                    arr_17 [i_3] [i_4] [i_5] [i_5] = 1;
                }
            }
        }
    }
    #pragma endscop
}
