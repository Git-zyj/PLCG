/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151567
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151567 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151567
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = (signed char)-12;
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 11; i_0 += 1) 
    {
        var_12 *= arr_1 [i_0] [i_0];
        arr_2 [i_0] [i_0] = ((/* implicit */long long int) ((((/* implicit */int) (_Bool)1)) ^ (((/* implicit */int) (unsigned short)0))));
        var_13 = ((/* implicit */_Bool) ((((/* implicit */_Bool) (unsigned short)5550)) ? (((/* implicit */unsigned int) ((/* implicit */int) (_Bool)1))) : (arr_1 [i_0] [i_0])));
    }
    var_14 = ((/* implicit */unsigned int) var_4);
}
