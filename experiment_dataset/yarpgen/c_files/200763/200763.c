/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200763
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200763 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200763
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_10;
    var_20 = (!var_15);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 25;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 2; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 25;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_1] [i_1] [i_2] [i_3] [i_4] = (min(((((max((arr_2 [i_1]), 192))) ? var_10 : (((arr_11 [i_0] [i_0]) - (arr_11 [i_0] [1]))))), ((((var_17 ? 263949717769939926 : var_6))))));
                                var_21 = (max(var_21, (((!(!var_16))))));
                                var_22 -= var_5;
                            }
                        }
                    }
                    var_23 = var_15;
                }
            }
        }
    }

    for (int i_5 = 2; i_5 < 12;i_5 += 1) /* same iter space */
    {
        arr_15 [i_5] = (arr_7 [i_5] [22]);
        var_24 ^= ((!(((131071 ? var_16 : var_1)))));
        arr_16 [i_5] = ((((((!(!131074))))) | (arr_1 [i_5 + 1])));
        var_25 = (arr_6 [i_5] [i_5] [i_5 + 1]);
    }
    for (int i_6 = 2; i_6 < 12;i_6 += 1) /* same iter space */
    {
        var_26 *= (min((arr_17 [10]), var_1));
        arr_21 [i_6 - 2] = max((arr_7 [i_6 - 1] [i_6 - 1]), (((((-131079 ? var_15 : var_17)) > var_6))));
    }
    #pragma endscop
}
