/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170775
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170775 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170775
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_11 = ((((-2147483645 ? 2147483621 : 65535)) ^ ((((arr_0 [i_0]) || (arr_0 [i_0]))))));
        var_12 = ((((((arr_0 [i_0]) ? (arr_0 [i_0]) : ((min((arr_2 [i_0]), var_0)))))) ? (((arr_0 [i_0]) | (arr_1 [i_0]))) : (arr_0 [i_0])));
    }
    var_13 = var_8;
    var_14 = (min((!1887040875431372487), var_9));
    var_15 = ((((var_0 != (max(var_3, var_8))))));
    #pragma endscop
}
