/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14248
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14248 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14248
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned char) (~(((((/* implicit */_Bool) (~(((/* implicit */int) var_9))))) ? (((/* implicit */int) (!(((/* implicit */_Bool) (unsigned char)239))))) : (((((/* implicit */int) var_5)) ^ (((/* implicit */int) var_4))))))));
    /* LoopNest 2 */
    for (unsigned int i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        for (unsigned char i_1 = 3; i_1 < 16; i_1 += 4) 
        {
            {
                arr_5 [i_1] [i_1] [i_0] = (short)-31281;
                arr_6 [i_0] [i_0] = ((/* implicit */_Bool) arr_0 [i_0]);
            }
        } 
    } 
}
