/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177690
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177690 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177690
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        arr_2 [i_0] = (((arr_1 [i_0 + 1]) ^ ((var_1 ? var_8 : (arr_0 [i_0])))));
        var_20 = (max(var_20, (((0 ? (1 - 1) : 1)))));
        var_21 = (((1 == 1) ? -130 : var_14));
        var_22 = (max(var_22, 1));
        var_23 ^= ((193 - ((var_12 ? (arr_0 [i_0]) : var_15))));
    }
    var_24 = var_2;
    var_25 = (max(var_25, (((0 * ((var_8 >> (255 - 250))))))));
    #pragma endscop
}
