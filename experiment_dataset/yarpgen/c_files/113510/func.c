/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113510
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113510 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113510
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 21; i_0 += 3) 
    {
        var_16 = ((/* implicit */_Bool) arr_0 [i_0]);
        var_17 ^= ((/* implicit */short) (-((~(((((((/* implicit */int) arr_0 [i_0])) + (2147483647))) >> (((/* implicit */int) var_14))))))));
    }
    var_18 |= ((/* implicit */unsigned char) ((((/* implicit */_Bool) ((((/* implicit */int) (unsigned char)25)) / (((/* implicit */int) (_Bool)1))))) ? (((/* implicit */int) var_11)) : (((/* implicit */int) var_11))));
    var_19 = ((/* implicit */short) (-(((/* implicit */int) var_15))));
}
