/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193498
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193498 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193498
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */long long int) ((((/* implicit */int) min((var_4), ((!(((/* implicit */_Bool) (unsigned short)42963))))))) | (((/* implicit */int) ((signed char) var_6)))));
    /* LoopNest 2 */
    for (short i_0 = 2; i_0 < 20; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 2; i_1 < 21; i_1 += 3) 
        {
            {
                arr_4 [i_1] = ((/* implicit */_Bool) (-(((/* implicit */int) var_3))));
                var_11 = ((/* implicit */unsigned int) min((((signed char) (unsigned char)128)), (var_6)));
            }
        } 
    } 
    var_12 *= ((/* implicit */unsigned short) var_2);
    var_13 = ((/* implicit */unsigned char) (+(max((var_9), (((/* implicit */unsigned long long int) (-(var_1))))))));
}
