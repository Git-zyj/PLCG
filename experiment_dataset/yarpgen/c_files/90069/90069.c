/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90069
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_8;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_11 = (((min(1028799734091112678, -4805357152130541921))));
                    var_12 = ((-2526501079092061501 ? 15830136016962156958 : -5937666582253251712));
                }
            }
        }
    }
    var_13 = ((576460752303423487 ? var_9 : (max((33979 - var_8), (min(-1028799734091112689, var_6))))));
    var_14 = var_7;
    #pragma endscop
}
