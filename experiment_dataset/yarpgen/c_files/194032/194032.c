/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194032
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194032 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194032
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 10;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_1] [i_1] = (((-((var_3 ? 53371 : 1206917439)))));
                    arr_11 [i_0] [i_1] [i_2] [i_2] |= var_9;
                    var_10 = var_1;
                }
            }
        }
        var_11 = (((arr_0 [i_0]) >= (!6727997539933782379)));
    }
    for (int i_3 = 1; i_3 < 11;i_3 += 1)
    {
        var_12 &= ((+(((arr_13 [i_3]) + var_6))));
        var_13 = 6727997539933782379;
        arr_16 [i_3] [i_3] = ((~(((var_6 > var_4) ? (!var_9) : ((var_3 ? 1 : var_8))))));
    }
    var_14 = ((max(var_5, ((-1 ? 1 : 2396)))));
    #pragma endscop
}
