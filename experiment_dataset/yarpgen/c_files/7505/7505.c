/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7505
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7505 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7505
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 &= 3895795210;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 16;i_1 += 1)
        {
            {
                var_15 = ((6061292499222079685 ? ((((arr_0 [i_0 + 2] [i_1 - 2]) ? (arr_0 [i_0 + 1] [i_1 - 1]) : -1534861634))) : (((arr_3 [i_0] [i_1 - 1]) ^ (arr_3 [i_0] [i_1 - 1])))));
                var_16 = (arr_2 [i_0 - 1]);
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 15;i_2 += 1)
    {
        for (int i_3 = 3; i_3 < 17;i_3 += 1)
        {
            {
                var_17 = (max(var_17, (((~((~(arr_7 [i_2] [16]))))))));
                arr_8 [i_3] [i_3] [i_3] = 15384582193344359099;
            }
        }
    }
    #pragma endscop
}
