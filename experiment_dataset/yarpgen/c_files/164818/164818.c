/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164818
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164818 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164818
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_0] = ((!((max(67108863, (arr_0 [i_1 + 1]))))));
                    arr_10 [i_1] [i_1 - 3] [i_1] [i_1 + 1] = var_3;
                    arr_11 [i_1] [i_0] [i_0] [i_0] = var_10;
                    arr_12 [i_0] [1] [i_2] [i_2] = ((((((var_15 ? -107 : -107)) - (arr_2 [i_0] [i_1])))));
                }
            }
        }
    }
    var_21 = var_5;
    var_22 = 107;
    #pragma endscop
}
