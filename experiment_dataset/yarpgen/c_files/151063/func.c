/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 151063
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=151063 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/151063
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 3; i_0 < 17; i_0 += 3) 
    {
        for (unsigned int i_1 = 1; i_1 < 18; i_1 += 4) 
        {
            for (signed char i_2 = 4; i_2 < 18; i_2 += 2) 
            {
                {
                    arr_7 [(short)3] [i_1 + 1] [(unsigned short)7] [i_0] = ((/* implicit */signed char) (~(min((((/* implicit */int) arr_5 [i_0 + 3] [i_1 + 1] [i_2 - 3])), (max((((/* implicit */int) var_3)), (var_9)))))));
                    var_10 = ((/* implicit */int) max((var_10), (((/* implicit */int) var_1))));
                }
            } 
        } 
    } 
    var_11 &= ((/* implicit */unsigned long long int) var_4);
    var_12 = ((/* implicit */unsigned int) ((int) var_1));
}
