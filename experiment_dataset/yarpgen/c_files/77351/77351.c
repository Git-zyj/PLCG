/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77351
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77351 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77351
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = arr_3 [i_0] [i_0] [i_0];
                var_12 += 103;
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 1; i_2 < 9;i_2 += 1)
    {
        for (int i_3 = 4; i_3 < 8;i_3 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 7;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 10;i_5 += 1)
                    {
                        {
                            var_13 = (((((arr_2 [8]) <= ((var_8 ^ (arr_17 [7] [i_3] [i_4] [i_5]))))) ? ((((((arr_12 [i_2] [9] [9]) ? var_0 : var_8))) ? 2 : (((~(arr_0 [i_3] [i_3])))))) : var_5));
                            var_14 = (max(var_14, (arr_0 [i_2 + 1] [i_3 + 1])));
                            arr_18 [i_2] [i_3 - 3] [i_3 - 3] [i_3 - 3] [i_3 - 3] = (arr_1 [4]);
                        }
                    }
                }
                var_15 = (max(var_15, ((max((arr_17 [i_2 - 1] [i_2 - 1] [1] [7]), (arr_0 [i_2] [i_2]))))));
                var_16 += (arr_9 [5]);
            }
        }
    }
    var_17 = 76;
    #pragma endscop
}
