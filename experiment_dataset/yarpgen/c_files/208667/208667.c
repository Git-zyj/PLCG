/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 208667
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=208667 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/208667
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_11;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            var_15 = var_2;
            var_16 = min((arr_3 [1] [0]), (arr_1 [i_0]));
        }
        for (int i_2 = 0; i_2 < 1;i_2 += 1)
        {
            var_17 ^= arr_3 [i_2] [i_2];
            var_18 = arr_3 [1] [i_2];
            var_19 -= ((var_8 ? ((min((arr_3 [i_0] [i_2]), (arr_3 [i_0] [i_2])))) : 0));
        }

        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            arr_10 [i_0] = 0;
            var_20 = arr_5 [i_0] [i_0] [1];
        }

        for (int i_4 = 0; i_4 < 1;i_4 += 1)
        {
            var_21 = ((0 ? ((min((arr_12 [i_0]), (min((arr_7 [i_0] [1]), (arr_11 [1])))))) : (arr_4 [i_0] [i_4] [1])));
            arr_13 [i_0] = arr_3 [i_4] [i_0];
            var_22 = arr_2 [1] [i_4] [1];
            var_23 = min((min((arr_11 [i_0]), (arr_11 [i_0]))), (arr_11 [i_0]));
        }
    }
    var_24 = (max(var_24, var_12));
    #pragma endscop
}
