/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51630
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51630 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51630
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_16));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        var_18 = 127;
        var_19 = -2048233257;
        var_20 = (((arr_3 [i_0 + 1]) + -111));
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_21 += var_1;
        var_22 = ((171 ? (arr_4 [i_1]) : (min((!var_9), -var_6))));
        var_23 = ((var_7 ? var_9 : ((((0 < 418117626331717878) >= ((max(12602, -127))))))));
    }
    var_24 = ((((var_1 >= (52934 % 710096977643596857))) ? (((var_15 <= (min(var_4, var_6))))) : (((-116 - var_5) ? var_2 : -126))));
    var_25 |= ((-(((var_2 <= (var_2 >> var_5))))));
    var_26 = (max(var_26, var_9));
    #pragma endscop
}
