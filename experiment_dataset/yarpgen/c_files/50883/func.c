/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50883
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50883 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50883
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_18 = ((/* implicit */_Bool) var_9);
    /* LoopNest 3 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 18; i_1 += 3) 
        {
            for (unsigned long long int i_2 = 0; i_2 < 18; i_2 += 3) 
            {
                {
                    var_19 = ((/* implicit */unsigned long long int) max((var_19), (((((/* implicit */_Bool) (~(((/* implicit */int) ((_Bool) var_17)))))) ? (arr_6 [(unsigned char)2] [6ULL] [i_2]) : (((/* implicit */unsigned long long int) ((/* implicit */int) var_2)))))));
                    var_20 = ((unsigned char) 3401775806851524733ULL);
                    var_21 = ((/* implicit */unsigned char) var_14);
                }
            } 
        } 
    } 
}
