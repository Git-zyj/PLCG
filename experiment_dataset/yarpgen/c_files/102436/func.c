/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102436
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102436 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102436
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */unsigned long long int) ((((/* implicit */long long int) ((/* implicit */int) var_2))) != (4503595332403200LL)));
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 1; i_1 < 20; i_1 += 1) 
        {
            {
                var_11 -= ((/* implicit */long long int) arr_1 [i_0]);
                arr_5 [i_0] [i_0] &= ((/* implicit */long long int) min((2243003720663040ULL), (((/* implicit */unsigned long long int) ((((/* implicit */int) var_2)) != ((~(((/* implicit */int) (unsigned short)20590)))))))));
            }
        } 
    } 
}
