/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67308
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67308 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67308
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */int) ((3829869583835915923LL) % (3829869583835915930LL)));
    var_20 = ((/* implicit */signed char) var_14);
    var_21 += ((/* implicit */signed char) var_6);
    var_22 = ((/* implicit */unsigned short) min((var_22), (min((((/* implicit */unsigned short) ((short) var_4))), (var_17)))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (unsigned long long int i_0 = 0; i_0 < 10; i_0 += 3) 
    {
        var_23 = ((/* implicit */signed char) max((var_23), (arr_0 [i_0] [i_0])));
        var_24 = ((/* implicit */unsigned long long int) ((unsigned int) arr_0 [i_0] [i_0]));
    }
}
