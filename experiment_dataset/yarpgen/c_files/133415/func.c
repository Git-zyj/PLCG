/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 133415
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=133415 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/133415
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_19 = ((/* implicit */long long int) min((var_19), (((/* implicit */long long int) min((((/* implicit */unsigned int) var_9)), (var_1))))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 22; i_0 += 1) 
    {
        for (long long int i_1 = 1; i_1 < 21; i_1 += 1) 
        {
            for (unsigned int i_2 = 4; i_2 < 21; i_2 += 2) 
            {
                {
                    var_20 = ((/* implicit */long long int) arr_5 [i_0] [i_0] [i_0]);
                    arr_6 [i_0] [i_0] [i_2] = ((/* implicit */long long int) 2147483647);
                    var_21 = ((/* implicit */unsigned int) ((((arr_1 [i_0]) + (2147483647))) << (((((/* implicit */int) arr_5 [i_0] [i_1] [i_2])) - (55300)))));
                    var_22 -= ((/* implicit */long long int) arr_0 [i_1]);
                }
            } 
        } 
    } 
    var_23 = ((/* implicit */signed char) var_2);
}
