/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22848
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22848 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22848
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_20 = (min(var_20, var_9));
                var_21 = (min(-var_18, (((((((arr_0 [i_0]) * (arr_3 [i_0])))) && (arr_0 [i_0]))))));
                arr_5 [i_0] = ((~(arr_3 [1])));
            }
        }
    }
    var_22 = var_6;
    var_23 = var_9;

    for (int i_2 = 2; i_2 < 19;i_2 += 1)
    {
        var_24 = ((!(((var_1 % (arr_7 [i_2 + 2]))))));
        var_25 = ((((((42829 ^ (arr_7 [i_2])))) ? ((~(arr_7 [i_2 + 1]))) : (~90))));
        var_26 = (((((((44609 ? (arr_7 [i_2]) : var_16)) <= -59))) == (((min((arr_7 [i_2]), (arr_7 [1])))))));
    }
    #pragma endscop
}
