/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1370
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1370 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1370
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 &= ((/* implicit */_Bool) ((unsigned char) 4045165628U));
    /* LoopNest 2 */
    for (unsigned char i_0 = 4; i_0 < 23; i_0 += 4) 
    {
        for (unsigned int i_1 = 1; i_1 < 21; i_1 += 1) 
        {
            {
                var_21 = ((/* implicit */long long int) min((var_21), (((/* implicit */long long int) arr_2 [i_0] [(unsigned char)3]))));
                var_22 = ((/* implicit */long long int) arr_1 [i_0 - 4]);
            }
        } 
    } 
    var_23 = (short)2736;
    var_24 = ((/* implicit */long long int) var_13);
}
