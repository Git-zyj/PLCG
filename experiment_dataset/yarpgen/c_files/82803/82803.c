/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82803
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] = ((((((((arr_2 [i_1]) ? 0 : var_11)) | var_3))) ? (!4615301987168190443) : (((!(((~(arr_2 [i_1])))))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 15;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            arr_13 [i_1] [i_1] [i_1] [i_1] = arr_7 [i_0] [i_0] [i_0];
                            arr_14 [i_0] [i_0] [i_1] [i_0] = (min((((!(arr_9 [i_3] [i_3] [1] [i_3])))), (arr_7 [i_0] [i_0] [1])));
                        }
                    }
                }
                var_13 = ((((((arr_7 [i_0] [i_0] [i_0]) | (arr_7 [i_0] [i_0] [i_0])))) * (((arr_2 [i_1]) ? var_7 : 7636540482352123415))));
                var_14 = ((~((min((arr_4 [i_0] [i_1] [i_1]), (arr_4 [i_0] [i_1] [i_0]))))));
            }
        }
    }
    #pragma endscop
}
