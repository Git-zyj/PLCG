/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 213897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=213897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/213897
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 17;i_1 += 1)
        {
            {
                var_18 = (min(((var_4 ? ((var_3 ? var_6 : 2614243474)) : ((1385544440 ? 4121516343 : -473256911234526766)))), ((max((arr_3 [i_1 + 1] [i_0 - 1]), var_11)))));
                arr_6 [i_0] = (((~4121516343) << ((((((~(((arr_1 [i_1]) + var_6)))) + 76401692928229981)) - 19))));
                var_19 = (min(((((var_7 || var_12) ? (~var_7) : (173450953 > 54534)))), -var_10));
            }
        }
    }
    #pragma endscop
}
