/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113451
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113451 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113451
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */long long int) (signed char)-42);
    /* LoopNest 2 */
    for (long long int i_0 = 2; i_0 < 21; i_0 += 2) 
    {
        for (long long int i_1 = 0; i_1 < 24; i_1 += 1) 
        {
            {
                arr_7 [(signed char)16] [(signed char)16] [(signed char)16] = ((_Bool) ((signed char) arr_6 [i_0 - 2] [(unsigned char)5]));
                var_13 = ((/* implicit */signed char) (+(arr_4 [i_0] [(_Bool)1] [15])));
            }
        } 
    } 
}
