/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209782
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209782 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209782
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_4;

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        var_21 ^= ((-(((((arr_1 [i_0]) ? (arr_2 [8] [i_0]) : 18446744073709551615)) + (((var_10 ? var_4 : 1)))))));
        arr_3 [i_0] = ((!(arr_0 [i_0] [i_0])));
        arr_4 [i_0] = (18446744073709551615 + -18995);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 22;i_1 += 1)
    {
        arr_7 [i_1] = (((arr_2 [i_1] [i_1]) + (arr_2 [1] [i_1])));
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 22;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 22;i_3 += 1)
            {
                {
                    arr_12 [i_1] [i_2] [i_1] = ((-(arr_10 [i_1] [i_2] [0] [i_3])));
                    var_22 = (((arr_5 [i_1]) ? var_0 : (arr_5 [i_1])));
                }
            }
        }
    }
    #pragma endscop
}
