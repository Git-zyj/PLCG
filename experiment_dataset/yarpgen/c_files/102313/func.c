/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 102313
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=102313 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/102313
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */signed char) var_15);
    var_21 = ((/* implicit */long long int) var_1);
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 25; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 25; i_1 += 1) 
        {
            for (unsigned char i_2 = 1; i_2 < 22; i_2 += 4) 
            {
                {
                    var_22 = ((/* implicit */unsigned short) (short)(-32767 - 1));
                    arr_8 [i_0] [i_1] [i_2] [i_2] = ((/* implicit */signed char) (short)(-32767 - 1));
                    arr_9 [i_2] [i_1] [i_1] [i_2] = ((/* implicit */unsigned char) (short)-32750);
                    arr_10 [(unsigned char)4] [i_0] [i_1] [i_2] = ((/* implicit */int) (short)32762);
                    arr_11 [i_2] [(unsigned short)15] [i_0] [i_2] = (unsigned char)5;
                }
            } 
        } 
    } 
    var_23 = ((/* implicit */unsigned int) var_5);
}
