/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 69621
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=69621 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/69621
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 ^= ((/* implicit */short) var_13);
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 0; i_0 < 15; i_0 += 4) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) (~(((/* implicit */int) (signed char)-125))));
        var_15 = ((/* implicit */unsigned short) ((((/* implicit */_Bool) arr_1 [i_0])) ? (var_0) : (((/* implicit */long long int) ((/* implicit */int) var_2)))));
    }
}
