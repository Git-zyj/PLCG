/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27694
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27694 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27694
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = var_15;
        var_19 = (max(var_19, (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_3 [i_0])))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 2; i_2 < 13;i_2 += 1)
        {
            for (int i_3 = 3; i_3 < 15;i_3 += 1)
            {
                {
                    var_20 *= 1;
                    arr_14 [i_1] [i_2] [i_3] [i_3] = ((9247757665643368894 / (((var_15 ? 1 : var_13)))));
                }
            }
        }
        arr_15 [i_1] = var_6;
        var_21 = (arr_8 [i_1]);
        var_22 += (var_17 + var_4);
        /* LoopNest 2 */
        for (int i_4 = 3; i_4 < 14;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 16;i_5 += 1)
            {
                {
                    var_23 = 12564;
                    /* LoopNest 2 */
                    for (int i_6 = 1; i_6 < 12;i_6 += 1)
                    {
                        for (int i_7 = 1; i_7 < 15;i_7 += 1)
                        {
                            {
                                var_24 &= ((1 ? (arr_0 [i_6 - 1]) : 1));
                                var_25 = (arr_8 [i_6 - 1]);
                            }
                        }
                    }
                    var_26 = ((var_14 % (arr_17 [i_1] [i_1] [i_1])));
                }
            }
        }
    }
    var_27 = var_6;
    var_28 = ((((min((~1), var_8))) ? (max((max(var_6, var_15)), var_12)) : var_5));
    var_29 = var_10;
    #pragma endscop
}
