/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46896
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46896 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46896
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 &= (max((max(1042443669, var_10)), (-959985124 ^ 769309203)));
    var_18 = var_5;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                var_19 = var_16;
                var_20 = 3525658092;
                arr_6 [i_0] [i_0] = (((max((arr_0 [i_1]), (arr_3 [i_0] [i_1] [i_0]))) & ((((arr_5 [i_0]) ? ((769309174 ? var_3 : 2544515584)) : 3525658108)))));
                var_21 = (max(var_21, (((1750451695 / (arr_3 [i_1] [i_1] [i_0]))))));
            }
        }
    }
    #pragma endscop
}
