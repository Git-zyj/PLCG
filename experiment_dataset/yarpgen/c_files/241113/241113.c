/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241113
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241113 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241113
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_18 |= (arr_4 [i_0]);
                                var_19 = min((arr_8 [i_0]), -var_1);
                            }
                        }
                    }
                }
                var_20 = var_10;
                var_21 ^= ((((min(10024, (arr_2 [i_1] [i_0 - 2])))) ? (((arr_9 [i_1] [14] [i_0] [i_0] [i_0 - 2] [i_0]) ? -52 : (arr_9 [0] [i_1] [i_0] [i_0] [i_0 - 2] [i_0 + 1]))) : ((min((arr_1 [i_1]), (arr_2 [i_1] [i_0 - 1]))))));
            }
        }
    }
    var_22 = var_12;
    #pragma endscop
}
