/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231599
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231599 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231599
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_19 = (min(var_19, 917926739726409691));
        arr_3 [i_0] [i_0] = (((((~((~(arr_2 [i_0])))))) ? ((min(-31, 201595407))) : (((arr_0 [i_0] [i_0]) - ((((30067 != (arr_0 [i_0] [1])))))))));
        var_20 = (!(((1899956408 ? (arr_0 [i_0] [i_0]) : 80))));
    }
    var_21 = (min(var_21, ((var_15 && (354760013 != var_3)))));
    var_22 = var_5;
    #pragma endscop
}
