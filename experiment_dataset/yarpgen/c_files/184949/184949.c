/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184949
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184949 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184949
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((var_9 + 4281354683) ? ((max(58720256, var_5))) : var_6)));
    var_11 = (-32 ? (max(((max(var_2, var_7))), var_2)) : (max(var_3, var_6)));
    var_12 = (((((-58720261 ? -14 : 169) & 1))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_13 = (-32 && ((((max(0, 31))) || (((~(arr_2 [i_0])))))));
                var_14 = (max(var_14, var_8));
                arr_5 [i_0] [i_1] = max((max(var_5, 1)), 4281354687);
            }
        }
    }
    #pragma endscop
}
