/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15055
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned short i_0 = 3; i_0 < 19; i_0 += 1) 
    {
        var_20 = ((/* implicit */unsigned char) ((unsigned long long int) ((long long int) arr_1 [i_0 + 1])));
        arr_3 [i_0] = ((/* implicit */short) max((((/* implicit */unsigned short) (signed char)-70)), (min((((unsigned short) (short)23110)), (((/* implicit */unsigned short) (short)17664))))));
    }
    var_21 = ((/* implicit */unsigned short) (-(var_19)));
}
