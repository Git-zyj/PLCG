/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164309
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164309 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164309
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((min(var_7, var_7)));
    var_16 = (min(((var_11 ? ((var_5 ? var_7 : var_13)) : (var_6 ^ var_10))), ((max(((var_6 ? var_4 : 4294967286)), -var_13)))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 *= (arr_4 [i_0] [i_0]);
                    var_18 = (((max((arr_5 [i_0] [i_2]), var_7))));
                    var_19 = var_12;
                }
            }
        }
    }
    var_20 = (min(((min((var_7 && var_8), ((14825385881698569848 ? var_1 : var_13))))), (((var_14 / var_5) ? var_6 : (var_13 * var_14)))));
    #pragma endscop
}
