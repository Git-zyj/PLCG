/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 43363
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=43363 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/43363
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = 1772208131U;
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_18 = ((/* implicit */long long int) min((var_18), (((/* implicit */long long int) min(((unsigned short)63758), (((/* implicit */unsigned short) (short)-21603)))))));
        var_19 -= ((/* implicit */unsigned int) (((((-(((/* implicit */int) arr_0 [2U])))) + (2147483647))) >> (((var_7) - (912955324)))));
    }
    var_20 = ((/* implicit */short) var_2);
    var_21 = ((/* implicit */_Bool) var_1);
}
