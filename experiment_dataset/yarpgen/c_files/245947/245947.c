/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 245947
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=245947 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/245947
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 += var_11;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 10;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 8;i_4 += 1)
                        {
                            {
                                var_21 += (((((-12754 ? (((((((-9223372036854775807 - 1) + 9223372036854775807)) + 9223372036854775807)) >> (194 - 150))) : (((10 ? (arr_1 [i_2]) : var_9)))))) == (min(((180562058 ? -2429 : 18446744073709551606)), (arr_3 [i_2] [i_3 + 1])))));
                                var_22 = -841971809;
                                var_23 = ((+(((arr_12 [i_0] [i_1] [i_2] [i_3 - 1] [i_4 - 1]) / 1))));
                            }
                        }
                    }
                    var_24 = (arr_0 [i_0] [i_2]);
                    arr_13 [i_0] = (+-1015808);
                }
            }
        }
    }
    var_25 = -var_2;
    #pragma endscop
}
