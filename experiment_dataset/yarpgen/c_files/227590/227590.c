/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227590
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227590 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227590
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_11 -= ((!(arr_3 [i_0] [i_0])));

                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        var_12 |= ((-543258388 ? 9223372036854775804 : 207));
                        arr_12 [i_0] [i_1] [i_1] [i_2] [i_3] [i_3] = (((((min(66, (arr_6 [i_0] [i_2] [15])))) * -28872)));
                    }
                    var_13 = (arr_5 [i_2] [i_1 - 2] [5] [i_2]);
                }
            }
        }
    }
    var_14 ^= ((-26970 ? ((max((~var_9), (((var_1 ? 115 : var_10)))))) : var_4));
    #pragma endscop
}
