/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 177626
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=177626 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/177626
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((!(-4586408738368858671 & -14060))) ? var_5 : -14068));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_11 = (((max((arr_0 [i_0]), (((var_8 ? (arr_3 [i_0] [i_1]) : 14076))))) ^ -14068));
                var_12 = (max((((14074 ? ((-(arr_3 [i_1] [i_1]))) : -0))), ((((var_9 ? var_3 : (arr_1 [i_1] [i_0]))) * (((var_3 ? (arr_0 [1]) : var_4)))))));
                var_13 -= (((~var_0) ^ (((min((arr_1 [i_1] [i_0]), (arr_0 [13])))))));
            }
        }
    }
    #pragma endscop
}
