/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91426
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        arr_2 [i_0] = ((((var_7 ? 0 : (arr_1 [i_0]))) - (!var_5)));
        arr_3 [i_0] = ((-(arr_0 [i_0])));
        arr_4 [i_0] [i_0] = 751602187;
    }
    for (int i_1 = 0; i_1 < 16;i_1 += 1)
    {
        arr_7 [i_1] [i_1] = var_4;
        arr_8 [i_1] = ((arr_5 [i_1] [i_1]) ? ((min(127, (arr_5 [i_1] [i_1])))) : ((var_5 ? (arr_6 [i_1]) : (arr_6 [i_1]))));
        arr_9 [i_1] = ((-var_2 ? ((var_4 ? (arr_5 [i_1] [i_1]) : 51874253968468522)) : (((var_5 ? 90 : 1023)))));
    }
    var_10 = var_2;
    var_11 = (max(var_7, (((-((var_6 ? var_5 : var_1)))))));
    #pragma endscop
}
