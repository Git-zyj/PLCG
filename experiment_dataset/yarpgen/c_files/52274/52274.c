/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 52274
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=52274 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/52274
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 |= (min((min((min(83, var_11)), ((min(var_4, var_9))))), var_9));
    var_13 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_14 = (max(var_14, ((((((~(arr_2 [i_0] [i_0])))) % var_11)))));

                for (int i_2 = 1; i_2 < 17;i_2 += 1)
                {
                    var_15 = (max((((arr_0 [i_0]) ? ((1 ? var_9 : (arr_2 [i_0] [i_2 - 1]))) : var_9)), 1));
                    var_16 = (max(var_16, (((-(!65))))));
                }
            }
        }
    }
    var_17 = max(1, (min((var_6 & 6507198381037265722), (!var_9))));
    var_18 = 98;
    #pragma endscop
}
