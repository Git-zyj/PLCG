/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 29740
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=29740 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/29740
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = -3931990654098463722;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] = var_1;
                arr_5 [i_0] = (max((var_11 < var_0), var_7));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            arr_10 [i_3] [i_0] [7] = (min(0, 4217026930));
                            arr_11 [i_3] [i_0] [i_0] [i_0] = (((((min(var_9, (arr_6 [i_0]))) ? ((((arr_9 [i_0 + 2] [i_0 + 2] [i_0 + 2] [i_0 + 2]) | var_0))) : (max(var_2, var_6))))));
                        }
                    }
                }
            }
        }
    }
    var_13 = var_1;
    #pragma endscop
}
