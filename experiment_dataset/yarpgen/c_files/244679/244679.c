/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244679
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244679 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244679
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_3;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_18 = (max(var_18, (((-6181424727605603890 ? 18 : (arr_4 [i_0]))))));
                var_19 = (((((-104073840 ? 1832450789745604459 : 1))) % (arr_3 [i_0] [i_1] [i_1])));
                var_20 = (min(var_20, var_0));
                var_21 = (min(var_21, ((min((((arr_0 [i_0]) - (arr_0 [i_1]))), ((-(arr_0 [i_0]))))))));
            }
        }
    }
    var_22 = (var_11 ? 0 : (!var_2));
    var_23 = var_8;
    #pragma endscop
}
