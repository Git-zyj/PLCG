/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 112485
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=112485 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/112485
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_18 = ((+((((arr_2 [8] [i_0 + 1] [i_0]) >= var_5)))));
                arr_5 [i_0] = ((var_8 >> (((arr_2 [i_0] [i_1] [i_1]) - 39))));
                arr_6 [i_0] = ((!(((-2104694033 ? (arr_1 [i_0 + 3] [i_0]) : (arr_1 [i_0 - 1] [i_0 - 1]))))));
            }
        }
    }
    var_19 = var_11;
    var_20 |= var_2;
    var_21 |= (((var_8 ? (max(-16427, -2104694030)) : var_10)));
    var_22 *= var_1;
    #pragma endscop
}
