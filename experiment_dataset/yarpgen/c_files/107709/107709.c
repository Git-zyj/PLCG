/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 107709
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=107709 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/107709
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_0] = ((((((min(48213, 6850342951889693981))) ? 48223 : (((-32767 - 1) ? 6850342951889693982 : (arr_0 [i_0] [i_0]))))) / (((arr_1 [i_1] [i_1]) ? 16755579998013644956 : (arr_1 [i_0] [i_1])))));
                var_15 += 1;
                arr_5 [9] [9] = (((((-31117 ? 17312 : 48223))) ? 17480 : (15 ^ 0)));
            }
        }
    }
    var_16 = 17322;
    var_17 = 17328;
    #pragma endscop
}
