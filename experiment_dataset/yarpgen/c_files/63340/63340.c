/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63340
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63340 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63340
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (max(var_11, ((((((-(max(var_5, var_8))))) ? var_2 : var_4)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 10;i_1 += 1)
        {
            {
                var_12 = (min((((arr_3 [i_1 + 2] [7] [i_1]) ? var_6 : (arr_1 [2]))), (1 || 16265)));
                var_13 = (max(((((var_1 ? -4627497034187182007 : 49270)))), (((((var_4 ? var_2 : var_0))) ? var_2 : var_5))));
            }
        }
    }
    #pragma endscop
}
