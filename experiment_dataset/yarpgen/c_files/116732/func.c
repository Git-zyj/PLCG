/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 116732
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=116732 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/116732
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 0; i_0 < 17; i_0 += 1) 
    {
        for (short i_1 = 0; i_1 < 17; i_1 += 3) 
        {
            for (int i_2 = 0; i_2 < 17; i_2 += 4) 
            {
                {
                    var_11 = ((/* implicit */unsigned int) ((((/* implicit */_Bool) ((int) min((-871621776), (var_10))))) ? (((/* implicit */unsigned long long int) 871621776)) : (min((arr_6 [i_2] [i_1] [i_2] [i_0]), (((/* implicit */unsigned long long int) var_7))))));
                    arr_8 [i_0] = ((/* implicit */unsigned int) (~(((/* implicit */int) ((((/* implicit */_Bool) arr_2 [i_0])) || (((/* implicit */_Bool) arr_2 [i_0])))))));
                }
            } 
        } 
    } 
    var_12 = ((/* implicit */int) var_3);
}
