/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 24169
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=24169 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/24169
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= -12;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 9;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    arr_9 [i_0] [i_1] = ((((((arr_1 [i_0] [7]) ? var_13 : (arr_1 [i_0] [i_0])))) ? ((~(arr_2 [i_0] [i_1]))) : ((min((max((arr_1 [i_0] [i_1 + 1]), var_3)), (arr_4 [5] [5] [i_1 - 1]))))));
                    arr_10 [i_1 + 1] [6] = 35;
                }
                /* LoopNest 3 */
                for (int i_3 = 0; i_3 < 10;i_3 += 1)
                {
                    for (int i_4 = 2; i_4 < 7;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 10;i_5 += 1)
                        {
                            {
                                arr_20 [i_5] [i_5] = (max(((((arr_14 [6] [i_4 - 1] [6] [6] [i_5] [i_1 + 1]) || -19))), (min(23, -26261))));
                                var_16 = -88;
                                var_17 -= var_14;
                            }
                        }
                    }
                }
            }
        }
    }

    for (int i_6 = 1; i_6 < 15;i_6 += 1)
    {
        arr_23 [i_6] [i_6] = arr_22 [i_6] [i_6];
        var_18 ^= var_5;
    }
    #pragma endscop
}
