/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163672
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163672 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163672
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_19 = (~(arr_0 [11]));
        /* LoopNest 2 */
        for (int i_1 = 4; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_20 = (min(var_20, -931303085));
                    var_21 ^= (min(((var_4 >> (-931303085 + 931303093))), ((~(arr_2 [18])))));
                    var_22 = ((536870911 || ((min(var_18, -931303085)))));
                }
            }
        }
        var_23 = (min(var_23, var_17));
    }
    /* vectorizable */
    for (int i_3 = 0; i_3 < 24;i_3 += 1)
    {
        arr_11 [i_3] [i_3] = var_2;
        arr_12 [3] = var_17;
        var_24 = (((arr_8 [i_3]) + (arr_10 [i_3])));
    }
    var_25 = ((~((var_9 | (((var_6 << (var_17 - 157))))))));
    var_26 += var_9;
    var_27 ^= (max(0, 5));
    #pragma endscop
}
