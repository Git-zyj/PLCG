/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 154012
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=154012 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/154012
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = var_15;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_5 [2] [i_0] [2] &= (min((((arr_4 [i_0] [i_1] [i_1]) / (arr_4 [i_0] [i_0] [3]))), (min((arr_4 [i_0] [i_1] [i_0]), var_19))));
                var_21 -= (((((var_3 ? (min(var_13, var_2)) : (((((arr_3 [i_0] [4] [i_0]) < (arr_0 [4] [i_1])))))))) % (min(((((arr_3 [2] [4] [i_1]) ? 1 : 1))), (arr_4 [i_0] [i_0] [i_0])))));
                var_22 &= (~((max(1, 1))));
            }
        }
    }
    #pragma endscop
}
