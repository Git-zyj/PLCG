/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 31359
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=31359 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/31359
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (min((var_16 || 0), var_1));
        var_19 = (min(var_19, (((((((((arr_1 [i_0]) >= 6)))) + (((arr_0 [i_0] [i_0]) ? (arr_0 [i_0] [7]) : 299833528426549789))))))));
        arr_3 [i_0] [i_0] = ((~(((arr_0 [20] [i_0]) ? ((1993307647 ? 185 : (arr_0 [i_0] [12]))) : (~2965650132)))));
        var_20 = (!1533968081);
        arr_4 [i_0] = (0 ? -1993307640 : 44315946);
    }
    var_21 = var_5;
    #pragma endscop
}
