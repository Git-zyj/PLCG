/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136132
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    var_16 ^= max((((((arr_6 [13] [i_1 + 1] [13]) ? var_8 : 16336)))), ((((((arr_5 [i_1 + 1] [i_0]) & 60))) ? ((3339826117586610240 ? (arr_1 [i_0] [i_1]) : (arr_7 [i_0] [i_0] [i_0] [i_2]))) : ((var_10 / (arr_3 [1] [i_1]))))));
                    var_17 |= (((((arr_7 [i_0] [i_0] [i_1] [i_2]) ? (arr_4 [i_1]) : 1))));
                }
            }
        }
    }
    #pragma endscop
}
