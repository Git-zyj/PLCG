/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94158
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94158 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94158
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((max((var_1 + var_7), var_7)) * (min(var_1, (var_2 >= var_1)))));
    var_11 = var_9;
    var_12 = var_1;
    var_13 = var_1;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 6;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_0] [i_1] [i_2] = ((((max((arr_2 [i_0 + 3] [i_0]), (arr_2 [i_0 + 1] [i_0])))) == (0 || var_7)));
                    arr_10 [i_0] [0] [i_0 + 4] [i_0 - 1] &= ((((min(0, (arr_3 [2])))) <= (max((max(0, var_8)), var_9))));
                }
            }
        }
    }
    #pragma endscop
}
