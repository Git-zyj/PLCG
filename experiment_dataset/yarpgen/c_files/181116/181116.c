/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181116
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181116 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181116
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = (max(var_19, (((+((var_2 ? (~var_1) : var_13)))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {

                    /* vectorizable */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        arr_11 [i_3] [2] [i_1 - 1] [i_0] = (((arr_10 [i_1] [i_1 - 1] [i_1 - 1] [i_1 - 1] [i_1 - 1] [i_1 + 1]) | (arr_10 [i_0] [i_1 - 1] [i_1] [i_1 - 1] [i_1] [i_1 + 1])));
                        var_20 = ((!((!(arr_0 [i_0] [4])))));
                        var_21 = var_0;
                    }
                    var_22 = (min(var_22, (((~(max((!var_0), (arr_4 [i_0] [i_1 - 1] [i_2]))))))));
                }
            }
        }
    }
    var_23 = -var_5;
    #pragma endscop
}
