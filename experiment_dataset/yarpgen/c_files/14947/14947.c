/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14947
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14947 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14947
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_17 |= var_5;
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 14;i_3 += 1)
                    {
                        {
                            arr_9 [i_1] [i_0] |= (((arr_6 [i_0] [i_1] [i_2 - 3] [i_2 - 3]) * 18291));
                            arr_10 [7] [7] [i_2] [i_2] = ((var_13 << (((arr_4 [i_0] [i_2 - 1] [i_3 - 1]) / var_5))));
                            var_18 = ((max(var_1, 511)));
                            arr_11 [i_1] |= 231;
                        }
                    }
                }
            }
        }
    }
    var_19 = (max((((268435455 ^ 1) ? 18446744073709551615 : var_5)), var_12));
    #pragma endscop
}
