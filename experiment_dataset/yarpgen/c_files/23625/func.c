/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23625
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23625 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23625
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_11 = ((/* implicit */short) var_3);
        var_12 = ((/* implicit */unsigned long long int) ((((/* implicit */_Bool) 3362082436U)) && (((/* implicit */_Bool) arr_1 [i_0]))));
        var_13 = ((/* implicit */signed char) max((var_13), (var_1)));
    }
    var_14 = ((/* implicit */unsigned char) min((((/* implicit */unsigned int) var_6)), (var_10)));
    var_15 = ((/* implicit */int) (signed char)79);
}
