/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 66784
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=66784 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/66784
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (max(var_20, var_16));
    var_21 = ((((max(var_9, (var_11 + var_19)))) * ((var_12 ? var_8 : var_2))));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1) /* same iter space */
    {
        var_22 = (((((var_15 & (arr_0 [i_0] [i_0])))) ? ((var_12 ? var_16 : var_1)) : (arr_1 [i_0])));
        var_23 |= (((arr_1 [i_0]) ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 15;i_1 += 1) /* same iter space */
    {
        var_24 = (((arr_0 [i_1] [i_1]) ? (arr_5 [i_1]) : var_15));
        var_25 = (min(var_25, (((var_0 ? var_10 : var_0)))));
    }
    var_26 = (~var_2);
    #pragma endscop
}
