/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32691
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32691 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32691
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_1] [i_0] = (((arr_5 [i_0] [i_1 + 1]) - 524287));
                var_10 = (max(var_10, (((!(!1))))));
            }
        }
    }

    /* vectorizable */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        var_11 = (arr_8 [i_2] [i_2]);
        /* LoopNest 3 */
        for (int i_3 = 0; i_3 < 24;i_3 += 1)
        {
            for (int i_4 = 1; i_4 < 23;i_4 += 1)
            {
                for (int i_5 = 2; i_5 < 22;i_5 += 1)
                {
                    {
                        arr_16 [i_4] [i_4] [i_4] [i_5 - 1] = ((((((arr_14 [i_2] [i_2] [i_3] [i_4 + 1] [i_2] [i_5 - 1]) ? (arr_11 [i_2] [i_2]) : (arr_12 [i_2] [i_3] [i_4] [i_4])))) & var_5));
                        var_12 = (max(var_12, ((((arr_10 [i_4 + 1] [i_4 + 1] [i_5 + 1]) | var_8)))));
                    }
                }
            }
        }
    }
    #pragma endscop
}
