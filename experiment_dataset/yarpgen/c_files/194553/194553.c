/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194553
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194553 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194553
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (~-0);
    var_11 = var_7;
    var_12 = ((var_1 ? (((!(((var_3 ? var_8 : var_5)))))) : (~var_4)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                var_13 = (min(var_13, ((((arr_3 [i_1 - 1] [i_1 - 1] [i_1]) - (min((arr_3 [i_1 - 2] [i_1 - 1] [i_1]), var_1)))))));
                arr_5 [i_1] [i_1 - 2] [i_1 + 1] = (!(((arr_2 [i_1 + 1] [1] [i_1 - 2]) != var_9)));
            }
        }
    }
    #pragma endscop
}
