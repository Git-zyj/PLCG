/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131803
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131803 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131803
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 19;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_16 = ((~((88 ? (arr_5 [i_0] [i_0] [i_0]) : 1561641834360690094))));
                                arr_10 [i_4] [i_3] [i_0] [i_0] [i_0] [i_0] = (((!0) & ((((max(var_5, 265621992))) ? (((arr_6 [i_0] [i_1] [i_0] [i_4]) - (arr_2 [i_0] [i_0] [i_0]))) : (arr_2 [i_4] [i_1] [i_2])))));
                            }
                        }
                    }
                }
                var_17 = 64;
            }
        }
    }
    var_18 = -231073857;
    #pragma endscop
}
