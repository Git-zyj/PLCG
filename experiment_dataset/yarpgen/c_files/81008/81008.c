/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 81008
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=81008 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/81008
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (!var_3);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    var_11 = (max(((max((!var_3), (arr_2 [i_0] [i_0])))), var_2));

                    for (int i_3 = 3; i_3 < 21;i_3 += 1)
                    {
                        var_12 = var_9;
                        arr_9 [1] [1] [i_2] [i_3 - 1] [4] [i_1] |= var_7;
                        var_13 = arr_4 [i_0] [i_1];
                    }
                    arr_10 [i_1] = (max(var_9, var_8));
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_4 = 0; i_4 < 22;i_4 += 1)
    {
        for (int i_5 = 3; i_5 < 20;i_5 += 1)
        {
            for (int i_6 = 2; i_6 < 21;i_6 += 1)
            {
                {
                    arr_18 [i_4] [i_5 - 1] [i_5] [i_6] = ((max(var_6, ((max((arr_0 [i_6]), (arr_16 [i_4] [i_5] [i_6 - 2])))))));
                    var_14 = arr_2 [1] [i_5];
                }
            }
        }
    }
    #pragma endscop
}
