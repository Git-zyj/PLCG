/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53426
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        arr_2 [i_0] = ((((2140746166139728344 >> (((arr_1 [i_0 + 4] [8]) - 3053521091)))) <= (arr_0 [i_0 + 1] [i_0])));
        arr_3 [i_0] = (arr_1 [15] [i_0 + 4]);
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        var_13 = (min(var_8, ((var_10 ? (arr_6 [i_1]) : ((var_7 ? (arr_6 [i_1]) : var_5))))));
        var_14 = (max(var_14, -1));
    }
    var_15 = ((((((-1330995455 + 2147483647) >> (var_2 - 1767618389)))) - var_10));
    #pragma endscop
}
