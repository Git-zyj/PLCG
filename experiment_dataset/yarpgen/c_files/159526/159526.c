/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159526
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159526 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159526
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 14;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 3; i_2 < 14;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        {
                            var_10 &= (((((var_7 ? var_1 : (arr_5 [i_2 - 2] [i_1] [i_1])))) ? ((528482304 ? 1 : 1)) : var_2));
                            arr_9 [i_0] [i_1] [i_0] [i_0] [i_0] [i_0] = ((~(arr_1 [i_1 - 1])));
                            arr_10 [i_0] [i_0] [i_2] [i_1] |= ((-(arr_5 [i_0] [i_3] [1])));
                            arr_11 [i_0] [12] [i_0] [i_3] = (~3875480700373302309);
                        }
                    }
                }
                var_11 = (min(var_11, 3875480700373302309));
                var_12 += -1;
            }
        }
    }
    var_13 = (((~(~var_7))));
    #pragma endscop
}
