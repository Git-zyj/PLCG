/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 160352
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=160352 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/160352
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned int) (+((-(((/* implicit */int) max((var_11), (var_11))))))));
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 12; i_0 += 1) 
    {
        for (int i_1 = 3; i_1 < 9; i_1 += 4) 
        {
            {
                var_15 = ((/* implicit */short) min((var_15), (((/* implicit */short) ((((/* implicit */int) var_5)) | (((/* implicit */int) ((short) ((((/* implicit */_Bool) var_7)) ? (((/* implicit */int) var_13)) : (((/* implicit */int) var_4)))))))))));
                var_16 = ((/* implicit */unsigned int) (_Bool)1);
            }
        } 
    } 
    var_17 = (-((-(((/* implicit */int) ((_Bool) (short)-26211))))));
}
