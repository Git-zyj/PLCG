/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 201268
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=201268 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/201268
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_16 = ((max((((var_14 ? (arr_5 [i_0 + 1] [i_1]) : (arr_4 [i_0])))), ((var_7 ? var_11 : var_8)))) * (var_14 % var_9));
                var_17 = (-(arr_4 [i_0 + 1]));
                var_18 |= (((((var_6 / (arr_0 [i_0 - 1])))) ? (((((var_11 ? var_0 : var_9))) ? (!97) : ((var_6 ? var_3 : var_13)))) : (((((max(var_13, var_7))) ? var_9 : ((min(var_14, var_14))))))));
            }
        }
    }
    #pragma endscop
}
