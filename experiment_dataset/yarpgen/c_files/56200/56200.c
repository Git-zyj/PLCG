/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 56200
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=56200 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/56200
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        {
                            var_20 = (min(((1 ? (-7898709861225166671 - 146) : 210)), ((min(var_2, -161)))));
                            var_21 -= ((64213 || (((-557566296 + ((157 ? 41827 : 26837)))))));
                        }
                    }
                }
                arr_10 [i_0] [i_1] = (min((((arr_5 [i_0] [i_0] [i_0]) ? (arr_5 [i_0] [i_0] [i_1]) : var_18)), 960227938));
            }
        }
    }
    var_22 = (min(var_22, ((min(9713617992749726436, var_7)))));
    #pragma endscop
}
