/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92126
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92126 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92126
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_5;
    var_18 = ((((1355137554 / var_8) != var_1)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 22;i_1 += 1)
        {
            {
                arr_5 [i_0] = ((((max((((arr_4 [i_0] [i_1 + 1]) / 255)), (arr_4 [i_0] [i_1 + 2])))) * ((1 ? (arr_0 [i_0]) : 1))));
                var_19 = (min(var_19, ((((1 || -26746837) == ((max((((arr_4 [i_0] [i_1]) && 0)), (!4492940066886643783)))))))));
            }
        }
    }
    #pragma endscop
}
