/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = ((+(((arr_0 [i_0] [i_1 - 1]) ? 0 : (arr_0 [8] [i_1 - 1])))));
                var_14 += (!var_7);
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 17;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 20;i_4 += 1)
                        {
                            {
                                var_15 = (max(10570, (max(59546, -62))));
                                arr_14 [i_0] [i_0] [i_1 - 1] [i_2] [i_3 - 1] [i_1] [i_4] = (max(5974, 17923));
                            }
                        }
                    }
                }
            }
        }
    }
    var_16 -= var_12;
    var_17 = (max(var_17, (max(59570, (min(var_9, (~-6771563228500719069)))))));
    #pragma endscop
}
