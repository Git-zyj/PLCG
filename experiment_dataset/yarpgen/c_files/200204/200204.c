/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200204
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200204 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200204
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 3; i_2 < 14;i_2 += 1)
                {
                    var_12 -= (2346688335617239135 || 1);
                    var_13 -= (arr_5 [i_2] [i_2] [i_2]);
                }
                for (int i_3 = 0; i_3 < 18;i_3 += 1)
                {
                    var_14 = (((((~(arr_1 [1])))) ? ((var_1 * ((((arr_3 [i_0] [i_1] [i_3]) == 17160553115626524260))))) : (arr_9 [6] [i_3] [0])));
                    var_15 -= -81;
                    var_16 = 0;
                }
                var_17 = (arr_6 [i_0] [i_1] [16] [i_0]);
            }
        }
    }
    var_18 = 0;
    var_19 = (max((min(((1 ? var_10 : var_10)), ((141 ? var_4 : var_3)))), var_8));
    #pragma endscop
}
