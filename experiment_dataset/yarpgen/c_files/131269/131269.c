/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 131269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=131269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/131269
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((var_6 != (min(262144, (min(var_16, var_8))))));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_18 = ((!((max((arr_1 [i_0]), var_10)))));
        var_19 = (((arr_0 [i_0] [i_0]) ^ ((((((var_14 ? (arr_0 [i_0] [i_0]) : 35))) < (((arr_0 [i_0] [i_0]) ? 0 : (arr_0 [i_0] [i_0]))))))));
        var_20 = (min(var_20, (arr_1 [8])));
    }
    #pragma endscop
}
