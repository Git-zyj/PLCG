/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 141991
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=141991 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/141991
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_18 -= (min((((((2610113702 ? 1 : 2863201910))) ? var_13 : 255)), var_0));
                                var_19 = (min(var_19, (arr_0 [4])));
                            }
                        }
                    }
                }
                var_20 = (min((arr_4 [i_0] [i_0] [i_0]), (((arr_4 [i_1 + 1] [i_0] [i_0]) ? (arr_4 [i_0] [i_0] [i_0]) : (arr_4 [i_0] [i_1 - 2] [i_1 - 2])))));
            }
        }
    }
    var_21 -= (min((((var_16 - var_10) ? (348737251 - 67) : var_13)), var_17));
    #pragma endscop
}
