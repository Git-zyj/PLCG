/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41142
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41142 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41142
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((((min(var_6, var_10)) << (var_13 - 1561316884))));
    var_21 = (min(var_4, -16633));
    var_22 ^= 9743;

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 14;i_3 += 1)
                {
                    {
                        var_23 = ((((max((arr_7 [i_0] [i_0] [i_0 - 1] [i_0 - 1]), (min(var_2, var_9))))) || (((~((~(arr_5 [i_0] [i_0] [i_2] [i_0]))))))));
                        var_24 = (min(var_24, (arr_3 [i_0] [i_0 + 1] [i_0])));
                    }
                }
            }
        }
        var_25 = 18446744073709551615;
    }
    var_26 = var_12;
    #pragma endscop
}
