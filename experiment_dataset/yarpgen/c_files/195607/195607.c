/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195607
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195607 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195607
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 += var_0;

    /* vectorizable */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (((arr_1 [i_0]) ? -1 : (arr_1 [i_0])));
        var_14 = var_9;
        var_15 += (2251765453946880 ? (arr_1 [12]) : var_7);

        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            var_16 = (min(var_16, ((((arr_4 [i_0] [i_0] [i_0 + 2]) && 0)))));
            var_17 ^= (((arr_4 [4] [i_0] [i_0]) ? (~var_8) : (var_7 != var_9)));
        }
    }
    #pragma endscop
}
