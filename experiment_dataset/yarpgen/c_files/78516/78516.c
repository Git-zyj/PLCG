/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78516
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78516 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78516
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((var_3 ? ((var_9 ? var_14 : var_4)) : var_2)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 9;i_1 += 1)
        {
            {
                var_18 = (max(((((arr_5 [i_0] [i_0] [i_0]) ? var_12 : (arr_3 [i_0] [i_1])))), (max(var_14, var_12))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        {
                            var_19 = (max(var_19, var_3));
                            arr_11 [i_0] [i_0] [i_0] [i_0] [i_0] = (arr_5 [i_2] [i_0 - 1] [i_0 - 1]);
                            var_20 ^= ((~(-9223372036854775807 - 1)));
                            arr_12 [i_0] [i_1 + 2] [i_1 + 2] [i_2] [i_3] [i_3] |= (0 >= var_2);
                        }
                    }
                }
                /* LoopNest 3 */
                for (int i_4 = 2; i_4 < 10;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 1;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 11;i_6 += 1)
                        {
                            {
                                var_21 = var_9;
                                arr_21 [i_0] [i_1] [i_0] [i_1] [i_0] = ((~(min(28656205, 0))));
                                var_22 += var_9;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
