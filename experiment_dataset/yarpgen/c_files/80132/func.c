/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 80132
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=80132 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/80132
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned char i_0 = 4; i_0 < 15; i_0 += 3) 
    {
        var_20 += ((/* implicit */unsigned short) (_Bool)1);
        arr_3 [i_0] = ((/* implicit */int) (_Bool)1);
    }
    var_21 ^= ((/* implicit */unsigned char) 627408062);
    var_22 = ((/* implicit */signed char) ((((_Bool) 67389924)) ? (((/* implicit */unsigned int) ((((((/* implicit */int) var_3)) + (((/* implicit */int) (unsigned short)6)))) / (((/* implicit */int) (unsigned char)213))))) : (2903494651U)));
}
