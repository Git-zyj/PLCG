/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159446
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159446 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159446
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (int i_0 = 1; i_0 < 17; i_0 += 4) 
    {
        arr_4 [i_0 + 4] [i_0] &= ((/* implicit */int) max((((/* implicit */unsigned int) ((arr_1 [i_0 + 4]) > (((/* implicit */long long int) ((/* implicit */int) (short)14659)))))), (((((/* implicit */unsigned int) -1964712618)) & (218448289U)))));
        var_18 = ((/* implicit */unsigned char) arr_1 [i_0]);
        arr_5 [i_0 + 1] = ((/* implicit */int) (+(arr_1 [i_0 + 2])));
    }
    /* LoopNest 3 */
    for (unsigned int i_1 = 4; i_1 < 24; i_1 += 2) 
    {
        for (unsigned int i_2 = 1; i_2 < 22; i_2 += 2) 
        {
            for (unsigned short i_3 = 0; i_3 < 25; i_3 += 3) 
            {
                {
                    arr_15 [i_3] [i_2] [(unsigned short)9] [i_1] = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) (-(((/* implicit */int) arr_10 [4U] [i_2 - 1] [i_2 - 1])))))));
                    var_19 &= arr_8 [i_3];
                    arr_16 [i_1] [i_1] [i_1] = arr_12 [i_2 + 2] [i_2 - 1] [i_2 + 3];
                }
            } 
        } 
    } 
    var_20 = ((/* implicit */unsigned short) var_7);
}
