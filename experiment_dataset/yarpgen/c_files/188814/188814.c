/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188814
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188814 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188814
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 -= ((!(((((max(var_2, var_6))) ? var_15 : var_7)))));
    var_20 = (max(((~(~var_18))), (var_10 > var_0)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_21 -= (max((arr_0 [i_0 + 1]), ((((arr_0 [i_0 + 1]) ? var_13 : ((var_17 + (arr_5 [i_1] [0]))))))));
                arr_7 [i_0] [i_1] [i_1 - 1] = ((((var_12 ? (arr_6 [i_0] [i_1 - 1] [i_1 - 1]) : (arr_1 [i_0])))));
            }
        }
    }
    #pragma endscop
}
