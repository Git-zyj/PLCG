/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148628
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148628 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148628
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        {
                            arr_8 [i_0] |= 1311123169754604016;
                            var_19 = (min(var_19, ((+(((arr_5 [i_2] [i_2] [3] [i_2 - 1]) ? (arr_5 [i_1] [7] [i_2] [i_2 - 1]) : (arr_5 [i_2 + 1] [i_2] [5] [i_2 - 1])))))));
                            var_20 = var_13;
                            var_21 = ((~(arr_1 [i_3])));
                        }
                    }
                }
                var_22 = ((~(((arr_5 [i_1] [i_1] [i_0] [i_0]) ? (arr_5 [i_0] [i_1] [i_1] [i_1]) : 243))));
                var_23 -= (arr_6 [10] [i_1]);
            }
        }
    }
    var_24 = var_8;
    var_25 = (((~var_13) % (-9223372036854775807 - 1)));
    #pragma endscop
}
