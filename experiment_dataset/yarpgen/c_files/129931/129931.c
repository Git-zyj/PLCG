/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 129931
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=129931 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/129931
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_17 = 1;
        var_18 = (max(var_18, (~var_0)));
        var_19 = ((!(((~((1 ? var_4 : (arr_1 [i_0] [i_0]))))))));
    }
    for (int i_1 = 0; i_1 < 20;i_1 += 1)
    {
        var_20 = 124;
        arr_6 [i_1] = (((((min(var_2, 1)) ? var_0 : (((min(var_5, (arr_0 [i_1] [i_1])))))))));
        arr_7 [i_1] = min(((((arr_3 [i_1] [i_1]) - var_12))), ((-1 + (min(var_3, 103)))));
    }
    var_21 = (max(var_21, -var_15));
    var_22 = ((((217 ? var_5 : var_14))));
    #pragma endscop
}
