/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148535
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148535 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148535
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    var_10 *= ((((max(31744, 3675059567)) < 3675059574)) ? 619907733 : (((arr_2 [i_2] [i_2]) & 26)));
                    /* LoopNest 2 */
                    for (int i_3 = 1; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_11 = (max(var_11, ((max((arr_9 [i_2]), (arr_3 [i_2] [i_2]))))));
                                arr_11 [17] [i_0] [8] [i_0] [12] = (max((((7 & (arr_1 [i_3 + 1] [i_1 - 2])))), (max((arr_3 [i_0] [i_0]), (arr_7 [i_0])))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_12 &= var_0;
    var_13 = ((max(((var_1 ? 1 : var_6)), -959748164)));
    var_14 = var_4;
    #pragma endscop
}
