/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 236077
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=236077 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/236077
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -1459304808513076647;
    var_11 = (max(((min(1, var_7))), (min(((min(1, 1))), (~41)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 -= (max((((arr_0 [4] [4]) ? (arr_3 [i_0] [i_0] [i_0]) : ((29 ? (-127 - 1) : (arr_3 [6] [i_1] [6]))))), var_5));
                var_13 = max((arr_2 [i_0] [i_0] [i_0]), (((arr_0 [i_0] [i_0]) ? ((var_0 ? 78 : (arr_0 [11] [11]))) : 1)));
            }
        }
    }
    #pragma endscop
}
