/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 113152
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=113152 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/113152
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 *= 10313779805951357700;
    var_13 &= ((((var_10 ? (((20 ? 784384873 : -848769905))) : ((5345797591684555333 ? 1251444277 : 0)))) / (min(13777964420084105306, (min(var_1, var_0))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_14 &= (min((((arr_2 [i_1]) ? (arr_4 [i_0]) : (arr_1 [i_1]))), (((max((arr_3 [i_1] [i_0] [i_0]), (arr_4 [i_0])))))));
                var_15 = (max(var_15, var_11));
            }
        }
    }
    #pragma endscop
}
