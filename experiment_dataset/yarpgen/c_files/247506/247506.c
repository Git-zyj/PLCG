/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247506
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_11, ((min(var_4, ((var_7 ? ((min(1, (-127 - 1)))) : -17348)))))));

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        var_12 = ((var_7 ? (((arr_0 [i_0 + 3]) ? (arr_0 [i_0 + 2]) : 15627)) : (min(var_7, -30320))));
        var_13 += var_3;
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_14 = (((((((min(128, 2))) ? -var_0 : ((9186990647705404766 ? -1 : var_1))))) ? (arr_3 [i_1]) : ((var_6 ? 524987178 : var_7))));
        arr_4 [0] [0] |= -524987194;
    }
    #pragma endscop
}
