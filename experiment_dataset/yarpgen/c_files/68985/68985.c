/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68985
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68985 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68985
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(var_10, (((~(~var_0))))));

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        var_11 = ((255 ? 255 : 1073741823));
        var_12 += (!54);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 1;i_3 += 1)
                {
                    {
                        var_13 = (min((~var_0), 15438));
                        arr_11 [i_0] = (min((arr_4 [i_0] [i_1] [i_2]), (~2)));
                    }
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_4 = 0; i_4 < 14;i_4 += 1)
    {
        for (int i_5 = 0; i_5 < 14;i_5 += 1)
        {
            {
                arr_17 [6] [13] [i_4] = ((~((((arr_8 [i_4] [i_4] [i_4] [i_4]) != (-9223372036854775807 - 1))))));
                arr_18 [i_4] = ((((max(50103, 18446744073709551609)) ^ (max((arr_14 [i_4]), 0)))));
            }
        }
    }
    var_14 = ((2577738521 ? 249 : 0));
    #pragma endscop
}
