/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125452
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125452 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125452
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 6;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 9;i_1 += 1)
        {
            {
                arr_5 [i_1] = (min(((((arr_3 [i_1] [i_0 - 1] [i_0 + 2]) && (arr_3 [i_1] [i_0 - 1] [i_0 + 2])))), ((((arr_4 [i_0]) && var_12)))));
                var_13 = (min(var_3, var_0));
                arr_6 [6] [i_1] [i_1] = ((!((min((var_6 / var_8), (arr_1 [i_1 - 1] [i_1 - 3]))))));
                var_14 = (((((var_2 * (arr_4 [i_0 + 1])))) ^ ((+(((arr_3 [i_1] [i_1] [1]) ^ (arr_4 [i_0])))))));
                var_15 = var_0;
            }
        }
    }
    var_16 = var_9;
    var_17 = var_12;
    #pragma endscop
}
