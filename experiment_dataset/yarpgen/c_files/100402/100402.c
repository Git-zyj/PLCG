/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100402
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100402 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100402
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (1 + 1);
    var_17 = ((((((((1 ? var_7 : var_15))) ? (min(1, 2248615987)) : var_10))) ? var_0 : var_0));
    var_18 = var_13;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (((117 ? ((14 ? 4294967295 : (arr_1 [i_0])) : (arr_3 [i_1])))));
                arr_6 [i_0] [i_0] = ((((1 ? (arr_0 [i_0]) : (arr_0 [i_0]))) | (1 < 1)));
                var_19 = ((2046351300 | 1) <= (!18446744073709551601));
                var_20 = 4;
            }
        }
    }
    var_21 = 19668;
    #pragma endscop
}
