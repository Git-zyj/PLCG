/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 143290
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=143290 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/143290
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = ((((((((var_1 | (arr_1 [i_0])))) < (((arr_2 [i_0] [14]) ? var_17 : var_0))))) ^ (((((var_4 ? (arr_0 [i_0] [i_0]) : (arr_3 [i_0] [i_0] [i_1]))) > (((var_13 ? (arr_1 [i_0]) : var_3))))))));
                arr_5 [i_1] [i_1] = (max((((var_17 ? -var_2 : (arr_2 [i_0] [i_0])))), (min(((min(var_6, (arr_1 [i_1])))), var_5))));
            }
        }
    }
    var_20 = var_16;
    #pragma endscop
}
