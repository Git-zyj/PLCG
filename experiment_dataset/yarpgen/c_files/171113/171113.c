/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 171113
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=171113 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/171113
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_7 [i_2] [3] [i_1] [i_0] = ((19 | (arr_0 [i_1])));
                    var_15 -= (((max((arr_3 [i_0]), (arr_4 [i_2]))) | ((((((max(var_5, 5241))) && (arr_3 [i_2])))))));
                }
            }
        }
    }
    var_16 = (((var_6 != var_14) << ((((((min(60289, var_8))) ? var_9 : var_5)) - 18446744073709550603))));
    #pragma endscop
}
