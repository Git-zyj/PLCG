/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 22014
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=22014 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/22014
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        arr_2 [i_0] = (arr_1 [0] [i_0]);

        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            var_17 ^= (min(((min((max((arr_1 [i_0] [i_1]), (arr_0 [i_0]))), ((min((arr_0 [i_0]), (arr_0 [i_1]))))))), (arr_4 [14] [14])));
            arr_5 [i_0] = (arr_0 [i_1]);
            arr_6 [i_1] = (arr_4 [i_0] [i_1]);
        }
        var_18 -= (max((arr_4 [i_0] [i_0]), ((max((max((arr_0 [i_0]), (arr_1 [i_0] [i_0]))), (arr_0 [i_0]))))));
    }
    var_19 = var_5;
    var_20 = var_13;
    #pragma endscop
}
