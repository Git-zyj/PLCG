/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 51284
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=51284 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/51284
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                {
                    var_12 = (min(var_12, ((min((((!(((var_8 ? 72 : var_4)))))), (~var_1))))));
                    var_13 -= (arr_5 [12] [i_0] [i_2]);
                    var_14 = ((~(((!(arr_4 [4] [i_2 + 2] [i_0]))))));

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_15 += var_4;
                        var_16 = (995276769 ? (arr_8 [i_0 + 2] [i_1] [i_2] [19]) : (((!((max((arr_10 [i_0] [15] [i_2] [i_2] [i_2 - 1] [i_3]), (arr_1 [i_2 + 1]))))))));
                    }
                    var_17 = (max(var_0, (((~(arr_0 [i_0] [i_2]))))));
                }
            }
        }
    }
    #pragma endscop
}
