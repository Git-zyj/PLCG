/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_1;
    var_13 = (((((var_4 ? (var_6 && 0) : ((var_5 ? var_2 : var_11))))) ? ((((var_8 >= var_9) && 1184999171))) : (!var_6)));
    var_14 = (((var_5 > var_8) ? var_6 : var_5));
    var_15 = -203337935;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                arr_7 [14] [i_1] = (arr_3 [i_0]);
                arr_8 [24] = -1134747749;
                arr_9 [i_0] [i_1] = (((((((-(arr_4 [i_0] [i_0] [i_1])))) ? -10 : ((-10 ? (arr_4 [i_1] [i_1] [15]) : 1134747768))))) ? var_0 : (arr_5 [i_1] [i_0]));
            }
        }
    }
    #pragma endscop
}
