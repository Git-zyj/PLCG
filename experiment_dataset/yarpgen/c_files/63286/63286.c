/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63286
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63286 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63286
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_15 -= (((((~((20414 ? var_14 : (arr_3 [i_0] [i_0])))))) ? (!72) : 84));
                var_16 = ((((-(arr_0 [i_0])))) ? (((arr_0 [i_1]) / (arr_1 [i_0]))) : (((!var_10) | (~-1042628104))));
                var_17 = ((!((((((-1261766162 ? 20414 : 157))) ? (!20413) : (158632907 || var_5))))));
            }
        }
    }
    var_18 = ((((var_11 ? var_13 : (!var_1)))) ? var_3 : (!45122));
    #pragma endscop
}
