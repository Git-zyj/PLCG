/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226489
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226489 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226489
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = -var_9;
    var_12 = (min(var_12, var_1));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 19;i_3 += 1)
                {
                    {
                        var_13 = (min(var_13, (((((!(((-(arr_9 [i_1] [1] [i_1] [0] [12])))))))))));
                        var_14 = (+((((arr_7 [i_0] [i_1] [i_3] [i_3]) <= (arr_9 [i_0] [i_2] [i_2] [i_3] [i_2 + 2])))));
                    }
                }
            }
        }

        for (int i_4 = 1; i_4 < 18;i_4 += 1)
        {
            var_15 = (min(var_15, (arr_9 [16] [18] [i_4 + 1] [i_4] [i_4 + 1])));
            var_16 = min(((-((~(arr_1 [i_0]))))), (((arr_2 [14] [14]) | (((!(arr_5 [i_0] [3] [i_4])))))));
            var_17 = ((((!(arr_0 [i_0] [i_4 - 1])))));
        }
        var_18 = (min(var_18, (((~(((!(((~(arr_4 [i_0]))))))))))));
    }
    #pragma endscop
}
