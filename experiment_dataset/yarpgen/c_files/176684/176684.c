/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 176684
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=176684 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/176684
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 += var_0;
    var_12 ^= ((var_1 ? (-120 == -var_4) : var_1));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 -= (((arr_4 [i_0] [i_0] [i_0]) <= ((((((arr_2 [i_0]) ? 65535 : 15))) ? 2048 : 1))));
                /* LoopNest 3 */
                for (int i_2 = 3; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                arr_14 [i_4] = ((~((min((arr_4 [i_2 - 3] [i_2 - 3] [i_2 + 2]), (arr_4 [i_2 + 1] [i_2 - 1] [i_2 - 2]))))));
                                var_14 -= (((arr_9 [i_2 + 2] [i_2 - 1]) > (arr_9 [i_2 - 3] [i_2 - 1])));
                                var_15 = (max(var_15, 1));
                                arr_15 [1] [1] [i_2] [1] [i_2] &= 0;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
