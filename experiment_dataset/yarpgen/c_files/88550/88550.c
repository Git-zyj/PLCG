/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88550
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88550 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88550
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 6;i_1 += 1)
        {
            {
                var_17 = (min(var_17, (arr_2 [i_1] [i_0])));
                var_18 = ((746579754 ? 2785577340 : 11916));
                /* LoopNest 3 */
                for (int i_2 = 1; i_2 < 9;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 9;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 8;i_4 += 1)
                        {
                            {
                                var_19 = (arr_0 [i_0] [i_0]);
                                arr_16 [i_0] [i_1] [i_0] [i_3] [i_4] = ((((max(var_8, 3548387541))) ? (4294967288 && 1490649003) : ((min((arr_8 [i_4] [i_3 + 1] [i_2 + 1] [i_1]), (arr_8 [i_0] [i_0 - 3] [i_0 - 1] [i_0]))))));
                                var_20 = 100663296;
                                var_21 = (arr_0 [i_0] [i_0]);
                                arr_17 [i_0] [i_0] [i_2] [i_3] [i_4] = ((!(((var_16 ? (max(2903793480, -1579244141280337275)) : (((max(var_4, var_15)))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_22 = 0;
    var_23 = var_10;
    #pragma endscop
}
