/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195310
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195310 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195310
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */long long int) ((((/* implicit */_Bool) var_3)) ? (((/* implicit */int) ((short) var_4))) : (((/* implicit */int) var_1))));
    /* LoopNest 2 */
    for (long long int i_0 = 1; i_0 < 11; i_0 += 4) 
    {
        for (unsigned short i_1 = 1; i_1 < 9; i_1 += 1) 
        {
            {
                arr_6 [i_1 + 3] |= ((/* implicit */unsigned char) var_9);
                var_15 = ((/* implicit */unsigned short) min((min((((((/* implicit */int) (signed char)-12)) * (((/* implicit */int) (signed char)-74)))), (((/* implicit */int) ((unsigned char) (short)10952))))), (((/* implicit */int) var_4))));
            }
        } 
    } 
}
