/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169125
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169125 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169125
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] = (((arr_6 [i_1]) != var_5));
                arr_8 [i_1] [i_1] = ((-4294967295 ? ((min((arr_4 [1] [i_1]), -5261))) : (((arr_4 [i_0] [i_1]) ? var_8 : -691044564))));
            }
        }
    }
    var_13 = (max(var_13, 18446744069414584321));
    var_14 = (max(var_14, var_7));
    var_15 = (!((((max(2065622082, var_9))) && var_8)));
    #pragma endscop
}
