/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 15402
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=15402 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/15402
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 -= var_2;
    var_13 = var_9;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 17;i_2 += 1)
            {
                {
                    arr_10 [i_2] [i_2] = (((min((arr_8 [8] [i_1] [i_2 + 1]), (arr_8 [i_0] [i_1] [i_2 - 1]))) == (((arr_8 [i_1] [i_1] [i_2 - 2]) + (arr_8 [3] [3] [i_2 - 2])))));
                    arr_11 [i_0] [i_2] [i_2] = (max(((max(var_0, (arr_9 [i_0] [i_0])))), (min(((min((arr_5 [i_1] [3]), var_11))), (arr_8 [i_2] [i_2 - 3] [i_2])))));
                }
            }
        }
    }
    #pragma endscop
}
