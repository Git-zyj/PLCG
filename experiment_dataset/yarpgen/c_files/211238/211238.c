/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211238
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        arr_2 [i_0] = var_6;
        var_12 = ((((min((arr_1 [i_0]), (arr_0 [i_0])))) ? (arr_1 [i_0]) : var_0));
        var_13 = (max(var_13, var_10));
    }
    for (int i_1 = 2; i_1 < 8;i_1 += 1)
    {
        arr_6 [i_1] = ((((((max(339967967537176790, 3))) ? 56413233 : var_10))) ? (((!(arr_1 [i_1])))) : (((((max((arr_0 [i_1]), (arr_4 [i_1 + 2] [i_1 - 2])))) >= (arr_1 [i_1 + 2])))));
        var_14 |= 18446744073709551615;
        arr_7 [i_1] = var_0;
    }
    var_15 = (max(var_8, (((!var_6) ? var_7 : var_5))));
    var_16 *= ((((max(var_4, var_1)))));
    var_17 = (min(var_17, var_5));
    #pragma endscop
}
