/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 690
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=690 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/690
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned int) var_3);
    var_21 = ((/* implicit */int) ((((var_2) + (9223372036854775807LL))) >> ((((~((-(((/* implicit */int) (short)30867)))))) - (30859)))));
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 0; i_0 < 10; i_0 += 1) 
    {
        for (short i_1 = 0; i_1 < 10; i_1 += 1) 
        {
            for (_Bool i_2 = 0; i_2 < 0; i_2 += 1) 
            {
                {
                    arr_8 [i_0] [i_0] = min((((/* implicit */unsigned long long int) (~(var_2)))), ((~(var_12))));
                    arr_9 [i_0] [i_1] [i_1] [i_0] = min((((/* implicit */unsigned long long int) (_Bool)1)), (18446744073709551615ULL));
                    var_22 = ((/* implicit */unsigned int) (+(((/* implicit */int) var_15))));
                }
            } 
        } 
    } 
}
