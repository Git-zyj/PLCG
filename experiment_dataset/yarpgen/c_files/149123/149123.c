/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149123
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149123 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149123
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((!(((2147483641 ? (!17592186044415) : 18446726481523507200)))));
    var_15 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_16 = min((((var_2 < (36508 != 18446726481523507200)))), (!var_3));
                var_17 += ((((((18446726481523507200 ? var_10 : (arr_4 [i_0] [i_1])))) || (var_6 % var_13))));
                var_18 = (max(var_18, 17592186044395));
            }
        }
    }
    var_19 -= var_7;
    #pragma endscop
}
