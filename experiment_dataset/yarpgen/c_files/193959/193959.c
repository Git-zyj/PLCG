/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 193959
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=193959 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/193959
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= (min(-31, 268435456));
    var_18 = (min(var_13, ((var_6 * (24576 || var_6)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                arr_5 [i_1] = arr_1 [i_0];
                arr_6 [i_1] = (max(((((arr_0 [i_1] [i_0 + 3]) - (arr_3 [i_1])))), (((arr_0 [i_0] [i_1]) ? ((((arr_0 [i_0 + 2] [i_1]) + (arr_3 [i_0])))) : ((var_13 / (arr_2 [i_1])))))));
            }
        }
    }
    #pragma endscop
}
