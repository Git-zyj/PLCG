#include <stdio.h>

unsigned long long int seed = 0;
void hash(unsigned long long int *seed, unsigned long long int const v) {
    *seed ^= v + 0x9e3779b9 + ((*seed)<<6) + ((*seed)>>2);
}

unsigned long long int var_0 = 10004026415458642984ULL;
unsigned char var_2 = (unsigned char)79;
unsigned int var_3 = 1366481063U;
short var_4 = (short)5505;
unsigned char var_5 = (unsigned char)192;
signed char var_6 = (signed char)103;
signed char var_8 = (signed char)-93;
signed char var_11 = (signed char)-96;
unsigned long long int var_12 = 730196813010671682ULL;
unsigned char var_13 = (unsigned char)196;
long long int var_14 = -253370877863687564LL;
int zero = 0;
unsigned int var_15 = 1005123105U;
signed char var_16 = (signed char)71;
short var_17 = (short)21252;
unsigned int var_18 = 2155008489U;
signed char var_19 = (signed char)69;
unsigned short arr_3 [18] [18] [18] ;
unsigned char arr_4 [18] ;
_Bool arr_8 [18] [18] [18] ;
void init() {
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        for (size_t i_1 = 0; i_1 < 18; ++i_1) 
            for (size_t i_2 = 0; i_2 < 18; ++i_2) 
                arr_3 [i_0] [i_1] [i_2] = (unsigned short)65379;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        arr_4 [i_0] = (i_0 % 2 == 0) ? (unsigned char)216 : (unsigned char)255;
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        for (size_t i_1 = 0; i_1 < 18; ++i_1) 
            for (size_t i_2 = 0; i_2 < 18; ++i_2) 
                arr_8 [i_0] [i_1] [i_2] = (i_2 % 2 == 0) ? (_Bool)1 : (_Bool)1;
}

void checksum() {
    hash(&seed, var_15);
    hash(&seed, var_16);
    hash(&seed, var_17);
    hash(&seed, var_18);
    hash(&seed, var_19);
    for (size_t i_0 = 0; i_0 < 18; ++i_0) 
        for (size_t i_1 = 0; i_1 < 18; ++i_1) 
            for (size_t i_2 = 0; i_2 < 18; ++i_2) 
                hash(&seed, arr_8 [i_0] [i_1] [i_2] );
}
void test();

int main() {
    init();
    test();
    checksum();
    printf("%llu\n", seed);
}
