/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23324
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23324 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23324
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_15 = ((((4549607712528045845 ? 21930 : 43590)) << (((((arr_12 [i_1 - 3] [i_1 + 2] [i_1] [i_4] [18]) ? (arr_12 [i_1 - 3] [i_1 + 2] [8] [i_3] [i_3]) : (arr_13 [i_1 - 3] [i_1 + 2] [i_1 + 2] [i_1 + 2] [i_3]))) - 2137449143))));
                                var_16 = (arr_1 [i_2]);
                            }
                        }
                    }
                    var_17 = (min(var_17, ((max((43591 - 996448247), (arr_10 [i_1]))))));
                }
            }
        }
    }
    var_18 = var_5;
    var_19 = var_5;
    #pragma endscop
}
