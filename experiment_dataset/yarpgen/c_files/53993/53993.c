/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 53993
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=53993 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/53993
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 11;i_1 += 1)
        {
            {
                var_20 = (min(var_20, ((max(((var_10 ? 62 : var_0)), 965488565287280994)))));
                var_21 = ((((arr_3 [i_0 - 2]) ? 297935102 : var_9)));
                var_22 -= min((((!var_5) ? var_0 : (arr_0 [i_1]))), 97);
                var_23 = 965488565287281010;
            }
        }
    }
    var_24 = (max(var_24, 2541890395));
    #pragma endscop
}
