/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 200206
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=200206 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/200206
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = 1;
        arr_3 [i_0] = (min(var_3, ((max(((((arr_0 [i_0] [i_0]) ? -64 : var_7))), (max(1728112841234773119, var_1)))))));
        var_13 = ((min(0, 57407)));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 18;i_1 += 1)
    {
        /* LoopNest 2 */
        for (int i_2 = 0; i_2 < 18;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 18;i_3 += 1)
            {
                {
                    var_14 |= ((var_0 > (arr_11 [i_3] [i_2] [i_1])));
                    var_15 = (min(var_15, var_9));
                }
            }
        }
        var_16 = (max(var_16, (var_7 * -8930)));
    }
    /* vectorizable */
    for (int i_4 = 4; i_4 < 17;i_4 += 1)
    {
        var_17 = 1;
        var_18 = ((((~(arr_8 [4] [i_4 - 1]))) / var_5));
        arr_14 [i_4] = var_10;
    }
    var_19 ^= ((~(((16767030698916522193 <= (-8930 * 1))))));
    var_20 = var_1;
    var_21 = var_8;
    #pragma endscop
}
