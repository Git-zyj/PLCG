/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123909
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123909 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123909
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 2 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_19 = ((/* implicit */_Bool) 18446744073709551615ULL);
        var_20 = ((/* implicit */signed char) 18446744073709551615ULL);
        arr_3 [i_0] &= ((/* implicit */int) 6728283428831540375LL);
        arr_4 [i_0] [i_0] = ((/* implicit */long long int) 1ULL);
        var_21 |= ((/* implicit */unsigned long long int) (unsigned short)16384);
    }
    for (unsigned long long int i_1 = 0; i_1 < 16; i_1 += 3) 
    {
        arr_7 [i_1] = ((/* implicit */unsigned short) 8152724706944636498ULL);
        arr_8 [i_1] [i_1] = ((/* implicit */long long int) (unsigned char)124);
        var_22 = ((/* implicit */unsigned int) 16380);
    }
    var_23 = ((/* implicit */short) var_11);
    var_24 = ((/* implicit */int) var_17);
}
