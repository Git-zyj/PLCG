/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166312
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166312 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166312
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, (((((((((var_10 ? var_14 : var_0))) ? (!32760) : var_6))) ? ((max(((var_1 ? var_9 : var_11)), (!var_8)))) : ((((max(var_5, var_13))) ? var_6 : var_8)))))));
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 11;i_2 += 1)
            {
                {
                    var_17 -= 203;

                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        var_18 = (max(var_18, ((max((((arr_10 [i_0] [i_2] [i_0] [12]) ? (~4151) : (((!(arr_9 [i_3] [0])))))), (((!((min((arr_6 [i_3] [3] [i_0 + 3]), (arr_9 [i_0] [i_0]))))))))))));
                        var_19 = (!0);
                    }
                }
            }
        }
    }
    #pragma endscop
}
