/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18847
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18847 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18847
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 22;i_1 += 1)
        {
            {
                arr_7 [i_1 + 1] [i_1 + 1] &= (min(-3630370571425984897, 249));
                arr_8 [i_1] = ((((-((min(-10032, -48))))) == ((9050 ? 1 : (min(-302512360, 31884))))));
            }
        }
    }
    var_20 &= (((((-1 ? (min(-8507173257986938010, 1)) : 7))) ? (max((min(-6579325466623627185, 24764)), (~-830667545))) : ((min((21308 | 41), 73)))));
    #pragma endscop
}
