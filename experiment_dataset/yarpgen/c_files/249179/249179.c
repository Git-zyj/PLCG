/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249179
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249179 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249179
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 1296937187;
    var_19 = (((!var_12) & var_14));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_20 = (((max(-1296937187, ((~(arr_3 [i_1] [i_0] [i_0]))))) != (((((((max((arr_1 [i_0]), (arr_4 [i_0] [i_0] [i_0]))))) != 1296937155))))));
                var_21 = (~1296937146);
                arr_7 [i_0] [i_0] = ((((((((1296937186 && (arr_0 [i_0] [i_0]))))) + 2080374784)) - ((max((arr_0 [i_0] [i_0]), 2314541383)))));
            }
        }
    }
    #pragma endscop
}
