/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207766
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207766 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207766
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = (arr_3 [i_0] [i_1]);
                arr_5 [i_0] [i_1] = 1;
            }
        }
    }
    var_15 = -var_0;
    /* LoopNest 3 */
    for (int i_2 = 2; i_2 < 10;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 13;i_3 += 1)
        {
            for (int i_4 = 2; i_4 < 11;i_4 += 1)
            {
                {
                    arr_12 [i_2] [i_2 + 3] [i_4] [i_3] = ((((((255 - var_5) * var_6))) ? (((arr_2 [i_4 - 2] [i_4 - 2] [i_2]) ? (arr_11 [12] [i_4] [i_4] [i_4]) : (arr_10 [i_2] [i_3] [i_2] [i_3]))) : ((211 ? 17931 : 127))));
                    var_16 = (min((((((-121 ? (arr_6 [i_3]) : 120))) / var_0)), (((var_9 | (-127 - 1))))));
                }
            }
        }
    }
    var_17 = ((((max(((min(-107, var_13))), var_5))) ? 3192681367 : var_11));
    var_18 = var_14;
    #pragma endscop
}
