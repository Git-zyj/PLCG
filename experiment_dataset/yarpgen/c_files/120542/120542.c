/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120542
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120542 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120542
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, (((((((min(1, -6753098997166424607))) ? (var_2 | var_3) : var_7)) | (var_5 - 59))))));
    var_11 = (min(var_11, (((var_1 ? ((~(var_9 < var_1))) : (~31))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 ^= (max(14741746451431673228, (-127 - 1)));
                var_13 = (((arr_2 [i_0] [i_1] [i_1]) - var_6));
                arr_4 [4] [i_1] [11] = ((~(arr_2 [i_1] [i_1] [i_0])));
            }
        }
    }
    #pragma endscop
}
