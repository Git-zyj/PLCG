/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217510
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217510 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217510
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(((2937140860237792849 == (((var_0 >> (34254 - 34235)))))), ((!(((var_19 ? var_10 : var_11)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 22;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = (-127 - 1);
                var_21 ^= 31816;
                arr_5 [i_0] [i_0] [i_0] = ((((!((((arr_2 [i_0] [i_0] [i_1 - 1]) ? (arr_2 [i_0] [i_0] [i_0]) : 31816))))) ? (max((min(0, (arr_2 [i_0] [i_1] [i_0]))), (arr_1 [i_0]))) : 112));
            }
        }
    }
    #pragma endscop
}
