/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 180180
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=180180 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/180180
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((((((max(19133, var_13)) | (((var_11 ? var_14 : var_2)))))) ? (max(965361035, 0)) : var_18));
    var_20 = (!var_16);
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_21 = (max(((((arr_7 [i_0] [i_1 + 2] [i_0 + 1]) | (arr_7 [i_0] [i_1 - 1] [i_0 + 2])))), var_8));
                    arr_8 [i_1] [i_0] = (((var_10 <= ((var_5 ? var_8 : (arr_2 [i_0]))))));
                }
            }
        }
    }
    var_22 += (min(var_4, (((var_18 ? var_16 : (var_0 + var_5))))));
    #pragma endscop
}
