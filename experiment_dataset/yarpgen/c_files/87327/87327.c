/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 87327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=87327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/87327
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 18;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = ((((arr_0 [i_0 + 4]) ? (arr_0 [i_0 - 2]) : (arr_0 [i_0 + 1]))));
        arr_3 [i_0] = (min((arr_0 [i_0]), (arr_0 [i_0])));
    }
    /* vectorizable */
    for (int i_1 = 2; i_1 < 18;i_1 += 1) /* same iter space */
    {
        arr_6 [i_1] = (((arr_4 [i_1 + 4]) >= (arr_5 [i_1 + 1])));
        arr_7 [i_1 + 2] [i_1] = (((((arr_4 [i_1]) ? (arr_0 [i_1 - 2]) : var_5)) == (arr_4 [i_1 - 1])));
    }
    for (int i_2 = 2; i_2 < 18;i_2 += 1) /* same iter space */
    {
        var_14 = (arr_4 [i_2 + 2]);
        arr_10 [i_2 + 2] = ((((((var_12 > (min(var_10, (arr_4 [i_2]))))))) ^ (((arr_4 [i_2]) / (arr_0 [i_2])))));
        var_15 -= (max(((var_13 % (arr_4 [i_2 - 2]))), (~var_5)));
        var_16 = (arr_9 [i_2 + 1]);
        arr_11 [1] [i_2] = ((var_6 / (min((arr_1 [i_2] [i_2]), var_10))));
    }
    var_17 = (~var_12);
    var_18 = ((-(((((var_1 ? var_11 : var_13))) ? (var_11 && var_1) : (var_2 / var_10)))));
    #pragma endscop
}
