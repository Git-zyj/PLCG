/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77757
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77757 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77757
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_16 = ((/* implicit */short) var_1);
    /* LoopNest 2 */
    for (long long int i_0 = 0; i_0 < 18; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 18; i_1 += 4) 
        {
            {
                var_17 = ((/* implicit */short) ((((/* implicit */_Bool) arr_0 [i_0])) ? (((/* implicit */int) arr_2 [i_0] [i_0] [(signed char)10])) : (((arr_2 [i_0] [i_0] [i_1]) ? (arr_0 [i_0]) : (((/* implicit */int) arr_1 [(unsigned short)9] [i_0]))))));
                var_18 -= ((/* implicit */unsigned int) var_1);
            }
        } 
    } 
    var_19 = ((/* implicit */signed char) var_3);
    var_20 = ((/* implicit */unsigned char) var_10);
    var_21 *= ((/* implicit */long long int) min((((/* implicit */unsigned long long int) max((((/* implicit */long long int) var_7)), (((-12LL) * (((/* implicit */long long int) var_9))))))), (var_12)));
}
