/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 8310
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=8310 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/8310
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_11;
    var_18 = -1;
    var_19 *= (min(var_6, (min(1, ((63 ? 4032 : -1))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_20 = ((((1 ? 1 : 9))) ? (arr_0 [i_0]) : (~17));
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 11;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_21 *= (((((((min((arr_5 [i_2] [i_0]), (arr_3 [i_0] [i_1] [i_2])))) ? ((1 ? 20 : 16131428689256754146)) : var_5))) ? (((arr_5 [i_0] [i_4]) ^ -1)) : var_7));
                                var_22 = (min(var_22, (arr_7 [i_0] [i_1 - 1] [i_2] [i_3] [i_3] [i_0])));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
