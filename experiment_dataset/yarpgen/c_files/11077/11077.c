/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11077
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11077 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11077
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 18;i_2 += 1)
            {
                {
                    var_16 = (max((min((min(137, 5018461119815220484)), 1334822197167851866)), ((max(var_7, var_5)))));
                    var_17 = var_8;
                    var_18 = var_9;
                    var_19 = 8;
                }
            }
        }
    }
    var_20 += (max(5018461119815220484, 1334822197167851866));
    var_21 = (max((max(var_1, var_15)), ((max(-1334822197167851880, 8816015649087687940)))));
    var_22 = (-2147483647 - 1);
    #pragma endscop
}
