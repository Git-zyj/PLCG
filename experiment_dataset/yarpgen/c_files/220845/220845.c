/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220845
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220845 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220845
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 = ((((-((var_13 ? 2140853429239490789 : var_12))))) ? ((((arr_2 [i_0]) ? (!2140853429239490789) : ((1 ? (arr_0 [i_0]) : var_5))))) : (min((arr_0 [i_0]), (arr_1 [i_0]))));
                /* LoopNest 3 */
                for (int i_2 = 2; i_2 < 20;i_2 += 1)
                {
                    for (int i_3 = 3; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 19;i_4 += 1)
                        {
                            {
                                var_16 = var_8;
                                var_17 = (max(var_17, ((((((!1) ? (~7881299347898368) : 1)) < (((((((arr_6 [i_0]) ? (arr_3 [i_0] [i_1] [i_2 + 1]) : 1))) ? 233 : (!1)))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
