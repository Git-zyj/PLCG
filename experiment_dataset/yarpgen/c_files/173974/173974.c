/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 173974
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=173974 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/173974
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1) /* same iter space */
    {
        arr_3 [i_0] = (arr_2 [i_0]);
        arr_4 [i_0] = (((var_3 / -83995079) ^ (arr_1 [i_0] [i_0])));
    }
    for (int i_1 = 0; i_1 < 18;i_1 += 1) /* same iter space */
    {
        arr_8 [i_1] = ((0 ? 83995079 : 13752));
        var_12 = (min(var_12, (((0 ? -83995079 : 0)))));
        var_13 -= ((!(!-124)));
        var_14 = var_9;
        arr_9 [i_1] [i_1] = (~1);
    }
    var_15 = ((min(-20, var_2)));
    var_16 = ((124 >> ((((-((max(var_5, var_11))))) + 44667))));
    var_17 = (max(83995079, 11205933681344901490));
    #pragma endscop
}
