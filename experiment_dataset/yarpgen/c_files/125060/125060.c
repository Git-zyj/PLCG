/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 125060
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=125060 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/125060
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_17 = ((((((max(20, var_10))) ? (min(918285582325257134, 250)) : (var_16 * 1643415353585112813))) < ((((var_2 != ((min(124, -1)))))))));
                arr_5 [i_0] [14] |= (~var_15);
                var_18 = (min(-var_14, (min((-2147483647 - 1), var_4))));
            }
        }
    }
    var_19 = (min((((~var_15) != (-774445809 < var_4))), (((max(var_4, var_16)) < (var_9 != var_13)))));
    #pragma endscop
}
