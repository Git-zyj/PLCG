/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 226163
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=226163 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/226163
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = max(((45 ? 100 : 27223)), ((20090 * (var_8 / var_10))));

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_4 [7] = ((((min((min(var_13, (arr_2 [i_0] [i_0]))), 8767))) ? ((var_15 ? (arr_2 [i_0] [i_0]) : (arr_0 [i_0] [i_0]))) : var_3));
        var_17 ^= (min(((min(-85, (var_4 != 27223)))), (((max(4991, 504))))));
    }
    #pragma endscop
}
