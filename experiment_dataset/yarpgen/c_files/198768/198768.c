/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198768
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198768 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198768
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((max(((var_10 ? var_0 : 1)), var_7)));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_13 = 7;
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 18;i_3 += 1)
                    {
                        {
                            var_14 = (min(var_14, var_9));
                            arr_11 [i_0] [i_2] [i_0 + 2] [i_0 + 2] [i_0] = ((((max((min(var_8, (arr_1 [i_2]))), (!var_10)))) % (arr_8 [i_2] [i_1] [i_2])));
                            var_15 ^= (((-681624694 ? 1 : 1)));
                            var_16 = (arr_6 [i_0]);
                        }
                    }
                }

                for (int i_4 = 0; i_4 < 21;i_4 += 1)
                {
                    var_17 = (max(-var_9, (((arr_10 [i_0] [i_0 + 2] [i_0 - 2] [i_0 + 1] [i_0 + 2]) ? 6057286354493418384 : (arr_10 [i_0] [i_0 - 1] [i_0 - 1] [i_0 + 1] [i_0 + 2])))));
                    var_18 |= (arr_4 [i_0] [i_0]);
                }
            }
        }
    }
    #pragma endscop
}
