/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182350
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182350 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182350
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_15 |= ((((arr_1 [i_0 + 3]) | (arr_1 [i_0 + 3]))));
                arr_6 [i_0] [i_0] [i_1] = ((~((max((arr_5 [i_0 + 3] [i_0] [i_0 + 1]), (arr_5 [i_0 + 3] [i_0] [i_0 + 1]))))));
                var_16 = ((((max((arr_4 [i_0 + 2] [i_0] [i_0]), 4))) & ((5746 + (arr_4 [i_0 + 3] [i_0] [i_0])))));
                var_17 = var_12;
            }
        }
    }
    var_18 ^= var_0;
    var_19 = var_14;
    #pragma endscop
}
