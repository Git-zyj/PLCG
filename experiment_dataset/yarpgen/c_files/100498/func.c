/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 100498
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=100498 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/100498
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */long long int) var_12);
    /* LoopSeq 1 */
    for (long long int i_0 = 1; i_0 < 11; i_0 += 1) 
    {
        var_16 = ((/* implicit */long long int) max((min((((0ULL) >> (((1483351823320432135LL) - (1483351823320432132LL))))), (((/* implicit */unsigned long long int) ((((/* implicit */_Bool) (short)7680)) || (((/* implicit */_Bool) (short)7699))))))), (((/* implicit */unsigned long long int) arr_0 [i_0 + 1] [(unsigned char)10]))));
        var_17 *= ((/* implicit */unsigned long long int) (short)7680);
    }
    var_18 = (short)-7680;
}
