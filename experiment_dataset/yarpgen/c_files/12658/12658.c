/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 12658
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=12658 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/12658
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_16 = (arr_0 [i_0]);
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        for (int i_4 = 4; i_4 < 23;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [i_1] [i_0] [i_3] [i_4] [i_0] [i_0] = ((~((((!0) && ((max(68685922304, 1))))))));
                                arr_15 [i_0 + 1] [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] = var_6;
                                var_17 = (arr_8 [i_0] [i_0] [i_0] [i_0] [i_0]);
                            }
                        }
                    }
                }
            }
        }
    }
    var_18 &= var_11;
    var_19 = (max(var_19, var_4));
    var_20 = (min(var_20, (((1 / ((((var_15 - var_14) <= 17851280892853758266))))))));
    #pragma endscop
}
