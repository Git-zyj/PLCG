/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182090
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = var_8;
    var_11 -= ((/* implicit */unsigned long long int) var_8);
    /* LoopSeq 1 */
    for (_Bool i_0 = 1; i_0 < 1; i_0 += 1) 
    {
        var_12 = ((/* implicit */unsigned int) ((((((/* implicit */long long int) ((/* implicit */int) arr_0 [i_0]))) / (arr_1 [i_0 - 1] [i_0 - 1]))) / (((/* implicit */long long int) ((4294967293U) / (2768554856U))))));
        arr_2 [i_0] [i_0 - 1] = ((((/* implicit */_Bool) max((((/* implicit */unsigned int) var_5)), (10U)))) || (((/* implicit */_Bool) arr_0 [i_0])));
    }
}
