/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223377
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223377 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223377
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, var_10));

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_18 = (arr_0 [i_0]);
        var_19 += (((min(((var_14 ? (arr_0 [i_0]) : (arr_1 [8] [i_0]))), (arr_1 [6] [i_0]))) >= (((arr_0 [i_0]) ? (arr_1 [20] [i_0]) : (arr_0 [i_0])))));
    }
    for (int i_1 = 0; i_1 < 25;i_1 += 1)
    {
        var_20 ^= ((-(((arr_4 [i_1]) * var_1))));
        var_21 = (min(var_21, (min(((min(2147483647, (arr_3 [i_1])))), ((max((arr_5 [i_1]), (arr_5 [i_1]))))))));
        arr_6 [i_1] [i_1] = ((-(arr_2 [i_1])));
    }
    var_22 = ((((min(2288441642, 0))) ? (min(((max(var_1, -1683876556))), (var_0 - var_4))) : -27));
    #pragma endscop
}
