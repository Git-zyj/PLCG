/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7820
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7820 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7820
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = min(var_1, (((var_1 * 16593713407476749866) ? (min(11088908146128954420, 7357835927580597196)) : -226)));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 23;i_2 += 1)
            {
                {
                    var_20 = (((~-122) ? 30 : (((arr_5 [7] [i_2 + 1] [i_2 + 1] [i_2 + 1]) | (arr_5 [i_2] [i_2 - 1] [i_2] [i_1])))));
                    var_21 = ((+(max((213 / var_13), -16))));
                }
            }
        }
    }
    #pragma endscop
}
