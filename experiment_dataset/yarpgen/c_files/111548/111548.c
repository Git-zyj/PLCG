/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111548
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111548 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111548
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    arr_8 [2] [i_0] [i_0] [i_0] = (9223372036854775789 ? -1602780886 : 16383);
                    arr_9 [i_0] [i_1] = (min((max((arr_4 [i_0] [i_0]), (arr_5 [i_1]))), -292485484));
                }
            }
        }
    }
    var_18 = var_3;
    var_19 = (~var_16);
    var_20 = ((min((~5704740526819730945), ((max(292485489, 127))))));
    #pragma endscop
}
