/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169030
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169030 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169030
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 23; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            {
                arr_4 [i_0] [i_0] = ((/* implicit */unsigned short) min((0), (((/* implicit */int) (short)32752))));
                var_13 = ((/* implicit */short) -1116358073);
            }
        } 
    } 
    var_14 = ((/* implicit */unsigned short) (!(((/* implicit */_Bool) -1116358081))));
}
