/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39353
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39353 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39353
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_0;
    var_19 = -18545;
    var_20 = var_2;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] = (min((arr_4 [i_1]), ((min(8128, 586649121)))));
                var_21 = var_10;
                var_22 = ((-(min((arr_3 [i_0] [i_0]), var_8))));
                var_23 = (((((arr_2 [i_0]) ? (max(var_7, -4134157358094465103)) : (arr_2 [i_0]))) == (((arr_1 [i_0] [i_0]) / (((max(var_0, var_5))))))));
            }
        }
    }
    #pragma endscop
}
