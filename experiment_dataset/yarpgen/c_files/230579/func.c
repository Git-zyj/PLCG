/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230579
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230579 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230579
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (signed char i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        var_19 = ((/* implicit */unsigned char) max(((unsigned short)47951), ((unsigned short)17593)));
        var_20 = ((/* implicit */short) (~(((/* implicit */int) min(((unsigned short)47913), (((/* implicit */unsigned short) (signed char)44)))))));
    }
    var_21 = ((/* implicit */_Bool) (~(-1456580343)));
}
