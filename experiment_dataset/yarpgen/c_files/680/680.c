/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 680
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=680 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/680
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_10 = (max(var_10, (((!((((arr_0 [i_1]) ? var_7 : (((arr_0 [i_1]) ? (arr_0 [i_1]) : var_0))))))))));
                arr_4 [i_0] = (~((~((var_3 ^ (arr_3 [7]))))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_2 = 0; i_2 < 24;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 1;i_3 += 1)
        {
            {
                arr_13 [i_2] [19] [i_2] = (min(var_1, (((((arr_8 [3]) ? 199 : (arr_9 [i_2] [9] [i_2]))) + ((min(1, 3332370059226754237)))))));
                var_11 ^= var_2;
                var_12 = var_2;
            }
        }
    }
    #pragma endscop
}
