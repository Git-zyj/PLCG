/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 101487
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=101487 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/101487
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        var_11 = ((((((max(var_1, var_1))) ? (min(16383, 268435455)) : 202)) * (((((min((arr_0 [i_0]), (arr_0 [i_0])))) ? ((var_3 >> (var_8 - 6633328816128704434))) : ((max(16383, var_2))))))));
        var_12 = (arr_0 [i_0]);
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 10;i_3 += 1)
                {
                    {
                        var_13 &= ((((!(var_0 | var_6))) ? var_6 : var_9));
                        var_14 = ((!((max(((268435455 ? 65535 : 131)), 49153)))));
                        var_15 = (arr_4 [i_1]);
                    }
                }
            }
        }
    }
    var_16 = var_1;
    #pragma endscop
}
