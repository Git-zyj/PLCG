/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 63715
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=63715 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/63715
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        var_10 *= ((arr_0 [i_0] [i_0]) < (arr_0 [i_0] [i_0]));
        arr_2 [i_0] = var_4;
        var_11 -= (-8029323528123228384 > ((1 ? var_8 : 4902155170699222859)));
        var_12 = (arr_1 [i_0]);
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 1;i_1 += 1)
    {
        var_13 += (((arr_3 [i_1]) <= (arr_3 [i_1])));
        var_14 = (min(var_14, (arr_1 [i_1])));

        for (int i_2 = 0; i_2 < 15;i_2 += 1)
        {
            var_15 = (((arr_4 [i_1] [i_1]) / 4294967292));
            var_16 = (arr_0 [i_1] [i_2]);
        }
        arr_7 [i_1] [i_1] = (((arr_3 [i_1]) * 32755));
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 1;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 17;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 17;i_5 += 1)
            {
                {
                    var_17 -= ((((((arr_16 [i_5] [i_4] [i_3]) ? 7168 : (!var_5)))) ? ((~(arr_14 [i_3]))) : (((!(arr_10 [i_5] [i_4]))))));
                    arr_20 [i_3] [i_4] [i_3] [i_3] = (max((((var_2 ? 32 : ((min(-7175, var_9)))))), (0 <= -4902155170699222859)));
                }
            }
        }
    }
    #pragma endscop
}
