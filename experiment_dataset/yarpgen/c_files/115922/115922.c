/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115922
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115922 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115922
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_15 = (max(var_15, (((max((max(-11779, 1637516952), -22509)))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_16 = (max((((arr_1 [i_1]) ? (arr_1 [i_0]) : -56)), (min((arr_1 [i_0]), (arr_1 [i_0])))));
                                var_17 = (((~(arr_10 [i_4] [i_2] [i_2]))));
                                var_18 = ((-(~18788)));
                            }
                        }
                    }
                }
            }
        }
    }
    var_19 = (max(((max(-10982, var_5))), var_11));
    #pragma endscop
}
