/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 32388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=32388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/32388
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 ^= (var_8 >= var_3);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_18 *= 48571;

                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        var_19 += (var_7 ? (((-(arr_9 [i_0] [16] [i_2] [1])))) : (48585 / var_14));
                        var_20 = var_2;
                        var_21 -= (min((arr_6 [i_0] [i_3] [i_2] [i_3]), var_13));
                        var_22 ^= (((((arr_4 [i_0] [i_2]) / var_10)) + (((12066 == (arr_4 [i_0] [i_1]))))));
                        arr_13 [0] [0] |= (arr_4 [3] [i_1]);
                    }
                }
            }
        }
    }
    var_23 = (-9223372036854775807 - 1);
    #pragma endscop
}
