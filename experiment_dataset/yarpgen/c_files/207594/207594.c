/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 207594
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=207594 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/207594
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_2;

    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {

        /* vectorizable */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            /* LoopNest 3 */
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 0; i_3 < 20;i_3 += 1)
                {
                    for (int i_4 = 0; i_4 < 20;i_4 += 1)
                    {
                        {
                            var_16 = (!(arr_0 [i_0 - 3]));
                            arr_13 [1] [1] [i_1] [i_3] [1] = arr_4 [i_1] [i_1];
                            var_17 = (min(var_17, (arr_3 [i_4] [i_2 - 1] [i_0 - 3])));
                            var_18 = (((arr_5 [i_2] [i_1]) ? (arr_0 [i_0 + 1]) : (arr_0 [i_2 - 1])));
                            var_19 &= (((arr_12 [i_0 - 2] [i_1] [i_1] [i_3] [i_3] [i_0 - 2]) % ((var_11 ? var_6 : 1))));
                        }
                    }
                }
            }
            var_20 |= var_7;
        }
        var_21 ^= ((1 ? 0 : 1));
    }
    #pragma endscop
}
