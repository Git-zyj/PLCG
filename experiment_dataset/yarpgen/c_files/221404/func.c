/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 221404
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=221404 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/221404
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */unsigned short) (((~(((var_6) + (((/* implicit */unsigned long long int) ((/* implicit */int) var_0))))))) == (((/* implicit */unsigned long long int) (~((+(5U))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (int i_0 = 1; i_0 < 14; i_0 += 1) 
    {
        arr_3 [i_0] = ((/* implicit */signed char) (-((~(var_4)))));
        var_21 += ((/* implicit */int) var_2);
        var_22 = ((/* implicit */unsigned long long int) min((var_22), (((/* implicit */unsigned long long int) ((((/* implicit */int) var_12)) <= (((/* implicit */int) (unsigned char)250)))))));
    }
}
