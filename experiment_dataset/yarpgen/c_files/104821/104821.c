/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104821
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104821 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104821
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((~(var_3 && var_3))) & (max(-var_8, var_5))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_11 = (max(var_11, (((1 ? 1 : 18446744073709551615)))));
                arr_6 [i_0] [i_1] = (arr_5 [i_0] [i_0] [i_1]);
                var_12 -= ((arr_4 [i_0] [2]) && ((max((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_1])))));
            }
        }
    }
    #pragma endscop
}
