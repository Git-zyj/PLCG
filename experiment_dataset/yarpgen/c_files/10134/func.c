/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 10134
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=10134 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/10134
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 -= ((/* implicit */int) ((((/* implicit */unsigned long long int) ((/* implicit */int) (signed char)42))) | (5983044841837006261ULL)));
    var_18 = ((/* implicit */unsigned char) var_7);
    /* LoopNest 2 */
    for (signed char i_0 = 2; i_0 < 18; i_0 += 4) 
    {
        for (int i_1 = 2; i_1 < 18; i_1 += 1) 
        {
            {
                var_19 = ((/* implicit */_Bool) arr_4 [i_1 + 2] [i_1 + 2] [i_1]);
                var_20 ^= ((/* implicit */unsigned int) arr_4 [i_1 + 2] [14U] [i_0]);
            }
        } 
    } 
    var_21 = var_8;
}
