/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 231031
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=231031 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/231031
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = var_2;
    var_19 = var_16;
    var_20 = ((-3604186393547202561 ^ (4294967295 / 2290)));

    for (int i_0 = 0; i_0 < 1;i_0 += 1) /* same iter space */
    {
        var_21 = -29;
        var_22 ^= (min((1 ^ -13757), (((((arr_2 [i_0]) | (arr_2 [i_0])))))));
        arr_3 [i_0] = var_8;
    }
    for (int i_1 = 0; i_1 < 1;i_1 += 1) /* same iter space */
    {
        var_23 = (((~38232) ? (arr_1 [i_1]) : var_4));
        var_24 ^= ((((arr_1 [i_1]) && (arr_1 [i_1]))));
        var_25 ^= (max(-2308, (arr_2 [1])));
        var_26 = var_10;
        var_27 ^= (min((!15), (-78 || -806)));
    }
    #pragma endscop
}
