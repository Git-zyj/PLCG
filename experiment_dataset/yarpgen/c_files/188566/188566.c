/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188566
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188566 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188566
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (~240);
    var_12 = (!-60197987);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] |= ((min((arr_2 [i_1 - 2]), (arr_0 [i_1 - 2]))));
                arr_5 [i_0] = (-241303215228037438 & (-2147483647 - 1));
                arr_6 [i_0] = (max(((((arr_3 [i_1 - 1]) > (arr_3 [i_1 - 2])))), (((arr_1 [i_1 + 1] [i_1]) % (arr_1 [i_1 + 1] [i_1])))));
            }
        }
    }
    #pragma endscop
}
