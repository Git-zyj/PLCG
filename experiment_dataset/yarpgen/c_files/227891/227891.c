/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227891
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227891 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227891
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = var_0;
    var_18 = (max(28672, 36864));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                var_19 = arr_5 [i_0] [12] [i_0];
                arr_6 [i_0] [i_1] [16] = (max((618718178 * 7941872637248505550), (max((var_8 / 4807058588112877148), (arr_1 [i_0])))));
                arr_7 [i_1] = 18381983757628782754;
            }
        }
    }
    var_20 = var_10;
    #pragma endscop
}
