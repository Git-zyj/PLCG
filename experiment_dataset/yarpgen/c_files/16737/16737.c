/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 16737
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=16737 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/16737
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        var_15 -= (var_0 ? var_11 : var_5);
        var_16 += (arr_1 [i_0]);
        var_17 = (max(var_17, ((((((var_8 - (arr_1 [i_0])))) ? (arr_0 [i_0]) : ((((arr_0 [i_0]) || 23707))))))));
        var_18 = ((-390 ? (arr_1 [i_0]) : var_6));
    }
    /* LoopNest 3 */
    for (int i_1 = 0; i_1 < 17;i_1 += 1)
    {
        for (int i_2 = 3; i_2 < 15;i_2 += 1)
        {
            for (int i_3 = 0; i_3 < 17;i_3 += 1)
            {
                {
                    var_19 = (min(var_7, (arr_4 [i_1])));
                    var_20 = (max(var_20, ((((((-(arr_5 [i_2])))) ? (arr_5 [i_2 - 2]) : var_4)))));
                }
            }
        }
    }
    var_21 = 958326579;
    #pragma endscop
}
