/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224864
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224864 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224864
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 *= ((((((var_7 ? var_0 : var_0)) | (min(var_2, var_5)))) >> (((min((max(var_5, var_8)), (max(105, var_1)))) - 78))));
    var_11 = (min((var_3 * var_8), var_4));
    var_12 = ((255 >> (var_5 - 1769816926)));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            var_13 = (arr_1 [i_0]);
            var_14 = ((0 ^ (arr_4 [i_0] [i_1] [i_1])));
        }
        var_15 = (max(var_15, (arr_3 [i_0])));
    }
    #pragma endscop
}
