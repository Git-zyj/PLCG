/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 206089
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=206089 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/206089
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (signed char i_0 = 3; i_0 < 14; i_0 += 3) 
    {
        for (unsigned char i_1 = 2; i_1 < 12; i_1 += 1) 
        {
            {
                var_19 += ((/* implicit */unsigned short) ((unsigned long long int) min((((((/* implicit */unsigned int) ((/* implicit */int) arr_4 [i_0] [i_0] [i_1]))) ^ (var_9))), (((/* implicit */unsigned int) max((((/* implicit */unsigned char) var_15)), (var_14)))))));
                var_20 &= arr_0 [(signed char)8];
            }
        } 
    } 
    /* LoopNest 3 */
    for (unsigned long long int i_2 = 4; i_2 < 15; i_2 += 2) 
    {
        for (short i_3 = 3; i_3 < 16; i_3 += 1) 
        {
            for (signed char i_4 = 4; i_4 < 16; i_4 += 3) 
            {
                {
                    arr_13 [i_4 - 4] [i_3] [i_2] = ((/* implicit */long long int) var_14);
                    var_21 *= ((/* implicit */unsigned short) min((var_9), (((/* implicit */unsigned int) arr_9 [i_2 + 1]))));
                }
            } 
        } 
    } 
    var_22 &= ((/* implicit */short) ((signed char) var_10));
}
