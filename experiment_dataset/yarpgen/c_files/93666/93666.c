/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 93666
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=93666 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/93666
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_13;

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        var_16 *= 6059627394141479277;
        arr_2 [i_0] [i_0] = (max(((-9223372036854775806 ? var_4 : (arr_1 [i_0] [18]))), ((max(((var_0 ? var_5 : var_8)), ((max(-1291097571, 1291097571))))))));
        var_17 = (min((((!(arr_1 [i_0] [i_0])))), var_13));
    }
    var_18 = (max(1291097571, ((((!(-127 - 1))) ? 1575194489 : var_1))));
    var_19 = -1291097571;
    #pragma endscop
}
