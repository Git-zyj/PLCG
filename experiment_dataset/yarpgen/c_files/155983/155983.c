/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 155983
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=155983 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/155983
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_3 [i_0] = ((max((arr_1 [i_0]), 366539010)));
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 13;i_2 += 1)
            {
                {
                    arr_9 [i_0] [i_1] = ((((((arr_2 [i_2 - 1]) ? -287052814173030836 : 3928428270))) ? ((((3928428276 << (30580 - 30566))) << (((arr_1 [i_2]) - 174)))) : 34956));
                    arr_10 [i_2] [i_0] [i_1] [i_0] = ((~(max(1, (arr_6 [i_0] [i_2 + 3])))));
                }
            }
        }
        arr_11 [i_0] [i_0] = 32761;
    }
    var_11 = var_3;
    #pragma endscop
}
