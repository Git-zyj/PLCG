/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14897
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14897 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14897
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned short i_0 = 0; i_0 < 15; i_0 += 3) 
    {
        for (int i_1 = 1; i_1 < 14; i_1 += 4) 
        {
            for (signed char i_2 = 0; i_2 < 15; i_2 += 3) 
            {
                {
                    var_12 ^= ((/* implicit */signed char) (unsigned short)3);
                    var_13 -= ((/* implicit */unsigned short) ((long long int) (signed char)-77));
                }
            } 
        } 
    } 
    var_14 |= (((~(((/* implicit */int) (_Bool)0)))) ^ ((~(((/* implicit */int) var_2)))));
    var_15 = ((((/* implicit */_Bool) (short)-22305)) ? (((/* implicit */long long int) ((/* implicit */int) (unsigned short)51405))) : (min((((/* implicit */long long int) max((((/* implicit */unsigned char) var_4)), ((unsigned char)95)))), (var_3))));
    var_16 = ((/* implicit */unsigned short) var_4);
}
