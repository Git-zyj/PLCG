/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 241551
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=241551 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/241551
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_19 = (arr_3 [i_0] [i_1] [i_0]);
                var_20 *= (((arr_1 [i_0]) || (arr_1 [5])));
                var_21 *= (((((arr_1 [i_1]) + (arr_2 [i_0 - 1]))) - (((((-(arr_2 [4])))) ? (arr_1 [i_0 + 2]) : (arr_1 [i_1])))));
            }
        }
    }
    var_22 *= ((-((var_11 ? var_18 : (var_15 % var_15)))));
    var_23 = ((((((var_17 || var_7) ? var_7 : ((2047 ? 2047 : 2048))))) * ((var_14 ? var_7 : var_5))));
    #pragma endscop
}
