/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 115294
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=115294 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/115294
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (((((((var_8 || var_0) && (((var_5 ? var_5 : 1))))))) <= -var_6));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        var_11 = ((~(~0)));
        arr_4 [i_0] = (arr_3 [i_0] [i_0]);
    }
    var_12 = ((var_9 ? (((max(var_7, -1812313560)) - ((var_4 ? var_2 : 1441280042)))) : ((1441280027 >> (1441280027 - 1441279998)))));
    var_13 = var_9;
    var_14 = -8529583234750151640;
    #pragma endscop
}
