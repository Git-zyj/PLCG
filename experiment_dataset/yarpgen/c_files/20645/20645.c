/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 20645
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=20645 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/20645
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((-((max((min(var_15, -4307)), var_1)))));
    var_18 *= -72;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 19;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 21;i_4 += 1)
                        {
                            {
                                var_19 = (~(max(0, (arr_10 [i_4] [i_1] [i_1] [i_1 + 2] [i_1] [i_0 + 1]))));
                                arr_13 [i_3] [i_1] [i_1] [i_3] [i_3] [i_3] = (24767 + 14509);
                            }
                        }
                    }
                }
                var_20 = ((-4294967285 > -1221794638) + -3257247390099978048);
                var_21 = (max((~3712229180290890660), (min(21, (!-124)))));
            }
        }
    }
    var_22 = (max(var_22, 4294967295));
    var_23 += (var_11 * var_15);
    #pragma endscop
}
