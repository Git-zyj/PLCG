/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67233
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67233 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67233
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 18;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 18;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 18;i_4 += 1)
                        {
                            {
                                var_14 = (((arr_10 [i_1 + 1] [i_1 + 1] [i_1 + 1] [i_1 - 2]) & ((max((arr_10 [12] [4] [i_1 - 2] [i_1 + 1]), -31715)))));
                                var_15 = 127;
                            }
                        }
                    }
                }
                var_16 = min(var_2, (((-127 + 2147483647) >> (((((arr_8 [i_0] [i_0] [i_0] [16]) ? var_1 : 230)) - 992)))));
                arr_12 [i_1] = (((min(var_12, (((127 <= (arr_9 [14] [i_1] [i_0] [i_1] [i_0])))))) | var_11));
            }
        }
    }
    var_17 = (max(var_17, -var_0));
    #pragma endscop
}
