/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179388
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179388 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179388
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (long long int i_0 = 1; i_0 < 16; i_0 += 2) 
    {
        for (short i_1 = 2; i_1 < 16; i_1 += 3) 
        {
            for (long long int i_2 = 0; i_2 < 19; i_2 += 4) 
            {
                {
                    arr_8 [i_0] [i_0] [i_2] [i_2] = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) var_8))));
                    arr_9 [i_0] = ((/* implicit */int) arr_1 [i_2]);
                    var_16 = ((((/* implicit */int) ((((/* implicit */int) arr_0 [i_0 + 1])) <= (((/* implicit */int) arr_0 [i_0 + 1]))))) <= (((/* implicit */int) min((arr_0 [i_0 + 2]), (arr_0 [i_0 + 2])))));
                }
            } 
        } 
    } 
    var_17 = ((/* implicit */unsigned char) (-((~((+(1220526352)))))));
    var_18 = ((/* implicit */_Bool) var_6);
    var_19 = ((/* implicit */long long int) var_3);
}
