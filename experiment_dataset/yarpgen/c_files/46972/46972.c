/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46972
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46972 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46972
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 ^= (min(((min((arr_4 [i_0]), (arr_4 [i_0])))), ((1099691599 ? 0 : 127))));
                arr_5 [i_0] [1] [i_0] = var_0;
                var_13 = (((((18423 ? (~0) : (1639392110226989495 + 56862)))) ? var_8 : ((-(((arr_3 [i_0] [i_0] [i_0]) ? var_8 : (arr_1 [9])))))));
            }
        }
    }
    var_14 = var_9;
    var_15 = var_9;
    #pragma endscop
}
