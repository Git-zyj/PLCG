/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 122476
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=122476 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/122476
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 21;i_2 += 1)
            {
                {
                    var_15 = (~(arr_9 [i_1 - 1] [i_1 - 1] [i_0]));
                    arr_10 [i_0] [i_0] [i_1] [i_0] = var_3;
                }
            }
        }
    }
    var_16 = ((((min((((-2147483647 - 1) ? 5791612336565662480 : 2299416785858645911)), ((min(var_5, var_5)))))) ? var_7 : ((~((var_3 ? 6416248464636910311 : 12655131737143889142))))));
    var_17 = var_0;
    #pragma endscop
}
