/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64580
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64580 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64580
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                var_11 = (min((arr_3 [i_0 - 1]), ((-(arr_3 [i_0 - 1])))));
                var_12 = var_8;
                var_13 = (((arr_1 [i_0]) ? ((min((min(var_10, var_7)), (arr_1 [1])))) : ((-(arr_3 [i_0 - 1])))));
            }
        }
    }
    var_14 = var_9;
    var_15 ^= var_2;
    var_16 = ((((max(var_5, (var_7 / var_5)))) ? ((var_9 ? (min(var_7, var_3)) : (((16792 ? var_5 : var_1))))) : (((((var_3 ? var_6 : var_0))) ? (max(var_3, var_1)) : var_5))));
    #pragma endscop
}
