/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 91714
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=91714 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/91714
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = (min(var_1, 6144));
    var_12 = (min(var_9, 2078853219));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                var_13 = (max(var_13, -var_4));
                var_14 = (arr_1 [i_0] [i_1]);
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 10;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 10;i_3 += 1)
                    {
                        {
                            arr_11 [i_2] [i_2] [i_2] [i_0] [i_2] = var_7;
                            arr_12 [i_0 - 1] [i_1] [i_2] [i_0] [i_3] [i_3] = (max(var_2, (var_7 % 9223372036854775807)));
                        }
                    }
                }
                var_15 = ((((min(0, (max((arr_7 [i_0] [i_0] [i_0] [i_0 + 1] [i_0]), var_9))))) ? (arr_8 [i_0] [i_0] [i_0] [i_0 + 1]) : (((arr_3 [i_0]) ? 1 : var_4))));
                var_16 = 0;
            }
        }
    }
    #pragma endscop
}
