/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103258
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103258 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103258
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 &= (min((((((var_1 ? -4419 : var_13))) ? ((4 ? var_14 : var_6)) : (var_11 == var_6))), 18446744073709551612));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            {
                var_17 *= (arr_0 [i_0]);
                var_18 = (min(var_18, (((-((min((((!(arr_0 [i_0])))), (arr_1 [i_0] [i_0])))))))));
            }
        }
    }
    var_19 = var_1;
    var_20 = ((var_9 ? var_9 : ((var_3 - ((var_2 ? var_1 : var_8))))));
    #pragma endscop
}
