/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 7618
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=7618 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/7618
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = -var_7;

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = ((((((var_14 << (6975503242499649029 - 6975503242499649024))) ^ (var_2 << var_1))) & ((((var_12 | (arr_0 [i_0]))) >> (((0 ^ -7152955999703564428) + 7152955999703564443))))));
        arr_3 [i_0] = ((((arr_1 [i_0]) == var_14)) || ((min((arr_0 [i_0]), ((((arr_0 [i_0]) < 6975503242499649029)))))));
    }
    #pragma endscop
}
