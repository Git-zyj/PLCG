/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123547
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123547 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123547
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 24; i_0 += 3) 
    {
        arr_4 [i_0] [i_0] = ((/* implicit */unsigned int) ((((arr_2 [i_0] [i_0]) - (arr_2 [i_0] [i_0]))) + (((((/* implicit */long long int) ((((/* implicit */int) arr_0 [i_0])) - (((/* implicit */int) arr_1 [i_0]))))) + (arr_2 [i_0] [(_Bool)1])))));
        var_10 = ((/* implicit */int) ((long long int) ((arr_2 [i_0] [i_0]) <= (((/* implicit */long long int) ((/* implicit */int) arr_1 [i_0]))))));
    }
    var_11 = ((/* implicit */signed char) var_3);
}
