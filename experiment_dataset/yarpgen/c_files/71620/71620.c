/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 71620
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=71620 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/71620
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((var_9 >> ((var_3 ? 1 : var_0))))) && 1));
    var_13 = var_11;
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_14 &= ((15 ^ 1) && ((max(var_4, ((1 ? 0 : 1))))));
                    arr_7 [i_0 + 2] [i_1] = (((((arr_5 [i_0 + 2] [1] [1] [1]) ? (((2 && (arr_4 [i_1] [i_1])))) : (((arr_5 [i_0] [i_0] [i_0] [i_0]) & 43)))) ^ var_6));
                    arr_8 [i_0] [i_0] [i_2] = ((~(arr_6 [i_0 - 1] [i_0 - 1] [i_0 + 1])));
                }
            }
        }
    }
    var_15 &= (!((min(0, -114))));
    #pragma endscop
}
