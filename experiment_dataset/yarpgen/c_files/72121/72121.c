/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72121
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72121 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72121
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_15 = max((((min(var_9, var_6)))), (((15298238367698185079 * var_14) * 8000)));
                arr_5 [i_0] [i_0] = ((1 & var_6) > (var_14 & var_10));
                var_16 = min((~853375489054316777), 17593368584655234838);
                arr_6 [i_0] [i_0] = ((+(max((!1331386876), ((164 ? 30 : var_12))))));
                arr_7 [i_0] [i_0] = var_13;
            }
        }
    }
    var_17 = 21289;
    #pragma endscop
}
