/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217593
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217593 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217593
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                var_20 = var_4;

                for (int i_2 = 2; i_2 < 16;i_2 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 19;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 17;i_4 += 1)
                        {
                            {
                                arr_13 [i_2] = var_13;
                                arr_14 [i_0] [i_0] [i_2] [i_0] [i_0] = var_9;
                            }
                        }
                    }
                    var_21 = (min(var_21, var_11));
                    var_22 = (max(var_4, (min((max(var_8, var_0)), (min(-1, -8))))));
                    var_23 ^= (min(var_5, var_19));
                    var_24 ^= (max((max(var_12, (min(var_9, var_12)))), var_3));
                }
                /* vectorizable */
                for (int i_5 = 0; i_5 < 19;i_5 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_6 = 0; i_6 < 19;i_6 += 1)
                    {
                        for (int i_7 = 0; i_7 < 0;i_7 += 1)
                        {
                            {
                                arr_23 [i_0 + 1] [2] [i_0] [i_0] [i_7] = var_5;
                                var_25 += var_11;
                            }
                        }
                    }
                    var_26 = var_11;
                    var_27 = (max(var_27, var_14));
                }
                for (int i_8 = 0; i_8 < 19;i_8 += 1)
                {
                    var_28 = (min(var_9, var_4));
                    var_29 |= (min(255, 20));
                    var_30 = (max(var_30, var_11));
                    arr_26 [i_8] = var_2;
                }
            }
        }
    }
    var_31 += (max(var_13, (min(var_7, ((max(var_4, var_4)))))));
    #pragma endscop
}
