/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 72580
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=72580 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/72580
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_0;
    var_12 = (((((-(var_1 + var_7)))) ? var_10 : (((var_10 ? var_7 : var_8)))));
    var_13 = var_7;
    var_14 = (!4294967295);
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 0;i_2 += 1)
            {
                {
                    var_15 = (min(var_15, ((((arr_3 [i_1] [i_1]) + (((arr_0 [i_0]) ? (arr_0 [i_0]) : (arr_0 [i_0]))))))));
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 12;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_16 = ((arr_5 [i_2 + 1] [i_1] [i_2] [i_1 + 1]) ? (!var_1) : (((!(arr_12 [2] [i_2 + 1] [i_2 + 1] [i_2 + 1] [4])))));
                                arr_14 [i_0] [i_3] [i_2] [i_2] [i_3] [i_2] = (~(var_6 | 1));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
