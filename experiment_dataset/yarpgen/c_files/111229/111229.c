/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 111229
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=111229 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/111229
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 19;i_1 += 1)
        {
            {
                var_16 |= 14;
                var_17 = 1;
                var_18 = (-var_3 * (!var_10));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    var_19 = (max(var_19, var_5));
                    var_20 = (var_8 ? var_15 : (arr_5 [19] [17]));
                    var_21 *= ((var_11 ? (arr_1 [18] [i_1 - 1]) : (arr_1 [i_0] [i_1 + 2])));
                    var_22 = ((!(arr_5 [i_1 - 2] [i_1 - 2])));
                }
                var_23 = ((-7194917178879978004 ? 223 : ((1 ? 41570 : 4230897713))));
            }
        }
    }
    var_24 = (min(((((250 - 17317308137472) ? var_14 : 468120501))), var_13));
    #pragma endscop
}
