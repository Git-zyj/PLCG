/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18109
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18109 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18109
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (max(var_14, var_6));
    var_15 ^= ((((max(((var_13 ? 205 : 205)), var_1))) && ((max(var_10, (var_2 || var_9))))));
    var_16 = (((!(((192 ? var_12 : 1))))));
    var_17 = (max(var_17, (var_0 * var_6)));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_18 = (((max((((var_8 != (arr_3 [i_0]))), (arr_0 [i_1]))))));
                var_19 = 1221810998;
                var_20 = 1;
                var_21 ^= (max(var_12, ((~(max(5986873118072477529, var_12))))));
            }
        }
    }
    #pragma endscop
}
