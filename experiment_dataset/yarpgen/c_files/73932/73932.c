/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 73932
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=73932 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/73932
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_1] = ((((arr_2 [i_0] [i_1 - 3] [12]) ^ (arr_2 [8] [i_1 - 2] [i_1 - 1]))));
                arr_6 [i_1] [i_1] = (((((-(arr_3 [i_0])))) ? ((max((arr_2 [i_0] [i_0] [i_1 + 1]), ((var_1 ? (arr_2 [i_0] [0] [0]) : var_5))))) : (max(62, var_0))));
                var_16 &= (arr_1 [i_0]);
                var_17 = (((((max(10126, 9506244209585235325)))) != (arr_0 [i_0])));
            }
        }
    }
    var_18 = 1442041125;
    #pragma endscop
}
