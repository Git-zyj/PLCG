/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 222587
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=222587 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/222587
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        var_11 = (var_2 > var_7);
        var_12 &= (~1809419585);
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 0;i_1 += 1)
    {
        for (int i_2 = 0; i_2 < 16;i_2 += 1)
        {
            {
                var_13 = (min(var_13, (min((((60474 != 1) & (arr_3 [i_1 + 1]))), (max(var_7, var_9))))));
                var_14 = var_3;
                var_15 = ((~(arr_7 [i_1] [i_1])));

                for (int i_3 = 0; i_3 < 16;i_3 += 1)
                {
                    /* LoopNest 2 */
                    for (int i_4 = 3; i_4 < 12;i_4 += 1)
                    {
                        for (int i_5 = 0; i_5 < 16;i_5 += 1)
                        {
                            {
                                var_16 &= arr_0 [i_2];
                                var_17 = 3964251197864421389;
                                var_18 = (((!1809419585) == 46229));
                            }
                        }
                    }
                    var_19 = var_9;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_6 = 4; i_6 < 17;i_6 += 1)
    {
        for (int i_7 = 1; i_7 < 1;i_7 += 1)
        {
            {
                var_20 = 8493093484458069295;
                var_21 &= (((((max(var_6, (arr_20 [i_6 - 4] [17])))) && (arr_21 [i_6]))));
                var_22 = (arr_0 [i_6 - 1]);
                var_23 = (arr_19 [0]);
            }
        }
    }

    /* vectorizable */
    for (int i_8 = 0; i_8 < 15;i_8 += 1)
    {
        arr_27 [5] = var_8;
        var_24 = (((arr_8 [i_8] [10]) <= (arr_15 [i_8])));
    }
    var_25 = var_6;
    #pragma endscop
}
