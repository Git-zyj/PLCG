/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238287
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238287 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238287
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = ((24741 < (!32765)));
    var_19 = -20;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_5 [i_0] = (!0);
                var_20 *= (+((((arr_2 [1]) == (arr_1 [i_0])))));
                var_21 = ((!(((-(arr_2 [i_0 - 1]))))));
                var_22 = ((((1 ? var_12 : 1))) >= (arr_2 [i_0]));
                var_23 *= (!(((0 ? (arr_2 [i_0 - 1]) : (arr_2 [i_0 + 2])))));
            }
        }
    }
    var_24 = (min(var_24, (((((1 ? var_8 : var_1))) ? 0 : (((0 ? -401189049 : ((518327813 ? 1 : -401189042)))))))));
    #pragma endscop
}
