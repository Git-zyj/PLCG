/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229797
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229797 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229797
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (((var_1 > var_1) / ((var_13 ? var_0 : (((max(var_5, var_6))))))));
    var_18 = ((-((((-75 ? 5359306840786229737 : var_11)) / (((var_2 ? var_13 : 29962)))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_0] = var_7;
                    arr_11 [i_0] [i_2] [i_2] [i_2] = ((~((9814 ? (((var_15 ? (arr_0 [i_0] [i_1]) : (arr_7 [i_2])))) : (max(var_0, var_9))))));
                }
            }
        }
    }
    var_19 = var_7;
    #pragma endscop
}
