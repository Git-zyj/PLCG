/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 244212
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=244212 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/244212
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = 102;
    var_12 = var_5;
    var_13 = (min(var_5, var_0));
    var_14 = (!13953526105846090584);

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_15 = 13953526105846090584;
        arr_2 [i_0] &= (arr_0 [i_0] [i_0]);
        var_16 = var_5;
        var_17 = (min(var_17, ((min((min(var_8, (arr_0 [i_0] [i_0]))), ((var_2 >> (((arr_0 [i_0] [i_0]) + 559708724)))))))));
    }
    for (int i_1 = 0; i_1 < 21;i_1 += 1)
    {
        var_18 = (((!13583262173682284934) / (!0)));
        var_19 = (arr_4 [i_1]);
    }
    #pragma endscop
}
