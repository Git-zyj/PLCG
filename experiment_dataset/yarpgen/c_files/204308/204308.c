/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 204308
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=204308 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/204308
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((((var_9 & var_9) || 2830)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                arr_7 [i_0] [i_1] [i_1 + 1] = ((-((222 * (((arr_0 [i_0] [i_0]) * (arr_5 [i_0] [9] [i_0])))))));
                var_18 = ((((min((max((arr_4 [i_0] [i_0]), 14392)), (arr_3 [i_0] [i_0])))) ? (min(51144, (3 * 32756))) : 532018242));
            }
        }
    }
    #pragma endscop
}
