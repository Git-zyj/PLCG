/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168627
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168627 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168627
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] [i_1] [i_1] = (((arr_1 [i_0 + 1] [i_0 - 2]) + (((arr_4 [i_0 - 1]) ? (((arr_0 [i_0] [18]) << (var_11 - 2911506673))) : (arr_3 [i_0 - 2])))));
                arr_7 [i_0] [i_1] [11] = -87;
                arr_8 [i_1] = ((((86 ? (arr_3 [i_0 - 3]) : var_0)) == (((arr_0 [i_0 - 2] [i_0 + 1]) ? (arr_0 [i_0 - 2] [i_0 + 1]) : (arr_0 [i_0 - 3] [i_0 - 2])))));
            }
        }
    }
    var_14 = ((+((var_0 << (((var_2 + 7940619991123839355) - 46))))));
    #pragma endscop
}
