/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 220808
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=220808 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/220808
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] [i_1] = (min(((((!var_2) || ((min(1, 4294967295)))))), -2520705392));
                arr_7 [i_1] |= ((((1 / (1353216130519855842 / 16))) / (((max((arr_2 [i_0] [0]), (1131 > -1353216130519855843)))))));
            }
        }
    }
    var_11 = var_5;
    var_12 = (max((((var_5 ? var_5 : var_5))), 3646738527));
    #pragma endscop
}
