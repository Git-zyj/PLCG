/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 233386
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=233386 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/233386
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 ^= (var_13 >= -16673);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_4 [i_1] = ((((min(var_12, (arr_1 [i_0] [i_0])))) >> (((((arr_1 [i_0] [i_1]) ? (arr_3 [i_0] [i_1]) : var_8)) - 37))));
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 22;i_4 += 1)
                        {
                            {
                                arr_12 [i_0] [i_1] [i_0] [i_2] [16] [i_4] &= (min((arr_1 [7] [11]), (arr_10 [i_0] [i_1] [i_0] [i_3] [1])));
                                var_19 = ((16673 - ((1 ? 174 : 78))));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
