/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136918
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136918 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136918
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */long long int) min((var_15), (var_9)));
    var_16 = ((/* implicit */long long int) min((var_3), (((/* implicit */short) (!(((/* implicit */_Bool) 4614356751246682976ULL)))))));
    /* LoopNest 2 */
    for (short i_0 = 1; i_0 < 15; i_0 += 3) 
    {
        for (short i_1 = 0; i_1 < 17; i_1 += 4) 
        {
            {
                arr_5 [i_1] &= ((/* implicit */short) max((10567342181493070893ULL), (7879401892216480721ULL)));
                var_17 = (short)-6049;
            }
        } 
    } 
}
