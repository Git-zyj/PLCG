/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42046
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42046 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42046
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        arr_3 [i_0] = ((((((arr_2 [9] [i_0]) ? var_2 : 33141))) ? 33141 : (arr_0 [i_0])));
        arr_4 [i_0] |= ((max((arr_1 [i_0]), (arr_2 [i_0] [i_0]))));
    }
    /* vectorizable */
    for (int i_1 = 0; i_1 < 10;i_1 += 1)
    {
        arr_8 [i_1] = (((arr_7 [i_1]) ? (((!(arr_1 [i_1])))) : (~33158)));
        var_10 = (min(var_10, 33158));
    }
    var_11 = 4545456450356236067;
    #pragma endscop
}
