/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64076
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64076 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64076
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= ((!((!((min(var_6, var_10)))))));
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {

                for (int i_2 = 1; i_2 < 21;i_2 += 1)
                {
                    var_15 = ((!(((((min(0, 64218))) ? ((((!(arr_0 [i_1]))))) : ((var_11 ? (arr_5 [i_0] [i_0]) : var_5)))))));
                    var_16 = (min(var_16, (arr_5 [12] [12])));
                }
                var_17 ^= (((var_12 ? ((max((arr_5 [i_0 - 2] [i_1]), (arr_2 [i_0 - 3] [i_0 + 2])))) : (arr_3 [i_0]))));
            }
        }
    }
    var_18 = ((~(max(((1317 ? -1 : 0)), (!1)))));
    #pragma endscop
}
