/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194619
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194619 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194619
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 |= ((/* implicit */signed char) ((((/* implicit */unsigned int) (~(((/* implicit */int) (!(((/* implicit */_Bool) 16078202940549776086ULL)))))))) & (((((/* implicit */_Bool) var_9)) ? (((unsigned int) var_5)) : (((/* implicit */unsigned int) ((/* implicit */int) var_6)))))));
    var_14 = ((/* implicit */unsigned int) ((((/* implicit */int) (unsigned short)65535)) - (((/* implicit */int) (unsigned short)1))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 11; i_0 += 4) 
    {
        var_15 = ((/* implicit */unsigned int) (!(((/* implicit */_Bool) var_8))));
        var_16 |= ((/* implicit */unsigned short) var_2);
    }
}
