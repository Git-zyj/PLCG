/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5006
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5006 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5006
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (var_11 || 27029);
    var_13 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 8;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 7;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0 - 1] [i_0 + 1] [i_0 + 1] = (arr_0 [i_1]);
                    arr_9 [i_0] [i_1] [i_2] = (min((116 && 4), (arr_6 [i_1] [i_1] [i_0 + 2])));
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 20;i_3 += 1)
    {
        for (int i_4 = 2; i_4 < 18;i_4 += 1)
        {
            for (int i_5 = 2; i_5 < 19;i_5 += 1)
            {
                {
                    arr_18 [i_5 - 2] [i_4] [8] [i_5] = ((((((~(arr_15 [i_4]))) & (var_3 ^ var_9)))) ? (((arr_17 [15] [i_3] [i_4 + 2] [i_5]) - 10503930845196208186)) : (min(1, (arr_17 [i_4] [i_4] [i_4 - 1] [i_5 - 1]))));
                    arr_19 [i_3] = var_5;
                    arr_20 [i_3] &= 5;
                }
            }
        }
    }
    #pragma endscop
}
