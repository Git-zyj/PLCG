/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 49598
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=49598 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/49598
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = ((/* implicit */unsigned short) var_12);
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 9; i_0 += 4) 
    {
        for (long long int i_1 = 4; i_1 < 7; i_1 += 3) 
        {
            {
                var_15 &= ((/* implicit */signed char) min(((~(((/* implicit */int) arr_1 [i_0 - 1])))), (((/* implicit */int) var_11))));
                var_16 = ((/* implicit */unsigned short) (unsigned char)110);
            }
        } 
    } 
    var_17 |= min((var_12), (((/* implicit */long long int) var_3)));
}
