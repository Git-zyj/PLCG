/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 67332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=67332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/67332
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 17;i_2 += 1)
            {
                {
                    var_11 = (min(var_11, (((var_0 ? (arr_7 [i_0 - 2] [i_1] [i_1 + 1] [i_2 - 1]) : (((arr_7 [i_2 - 3] [i_2 - 3] [i_2 - 2] [i_2 - 1]) ? -var_6 : (max(-48359196, 50499)))))))));
                    arr_9 [i_0 - 2] [i_1] = ((15037 ? 1 : 128));
                }
            }
        }
    }
    var_12 = ((var_1 ? (max((min(var_2, var_9)), (var_10 + 5347022438998566878))) : var_0));
    #pragma endscop
}
