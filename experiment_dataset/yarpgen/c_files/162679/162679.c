/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162679
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162679 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162679
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (min(var_10, var_6));
    var_11 = var_8;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_12 = (min(((11157 ? ((16254317220577736580 ? 140737488355327 : 203)) : (max((arr_1 [i_1]), (arr_3 [i_0] [i_0]))))), (~var_8)));
                var_13 = (((arr_3 [i_0 + 1] [i_0 - 1]) * ((min(var_5, var_7)))));
                arr_5 [i_1] [i_1] = ((!(arr_2 [i_0] [i_0 + 1] [i_0])));
                arr_6 [i_0] = ((!(((-(max(17, (arr_3 [i_0] [i_1]))))))));
            }
        }
    }
    #pragma endscop
}
