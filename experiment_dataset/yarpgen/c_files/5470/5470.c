/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 5470
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=5470 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/5470
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((~(((var_2 ? var_13 : -1)))));
    var_17 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                var_18 = (min((((-(((arr_0 [i_1]) ? (arr_1 [1]) : 8))))), (max(-432437307, (arr_0 [0])))));
                arr_4 [i_0] [i_0] = ((((min(432437306, (arr_1 [i_0])))) ? (arr_1 [i_1]) : (max((arr_1 [i_0]), 9634413059575692795))));
                arr_5 [i_0] [i_0] [4] = ((-(arr_1 [i_0])));
            }
        }
    }
    #pragma endscop
}
