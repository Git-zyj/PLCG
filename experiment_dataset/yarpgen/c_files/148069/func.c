/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148069
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148069 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148069
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */_Bool) min((var_11), (((/* implicit */_Bool) max((var_6), (((/* implicit */unsigned long long int) (-(1512935826)))))))));
    /* LoopSeq 1 */
    for (unsigned short i_0 = 0; i_0 < 23; i_0 += 4) 
    {
        var_12 = ((/* implicit */_Bool) max((var_12), (((/* implicit */_Bool) (~(4835754938229274869ULL))))));
        var_13 = ((/* implicit */_Bool) arr_1 [i_0]);
    }
}
