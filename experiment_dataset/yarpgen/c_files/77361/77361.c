/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77361
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77361 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77361
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 &= ((var_7 ? (((~(~var_0)))) : ((~(~var_4)))));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 19;i_2 += 1)
            {
                {
                    var_12 -= (min(-var_1, (var_0 >> var_7)));
                    arr_7 [i_0] [i_1] [i_1] [i_1] = ((((max(7968408662710000800, -7968408662710000798) >> (((((arr_2 [i_0]) % -7968408662710000822)) - 1741932354))))));
                    var_13 = (max(var_13, ((((arr_5 [i_0] [i_0] [i_0] [i_0]) ? (((~(!var_10)))) : ((-(arr_2 [i_0 - 1]))))))));
                }
            }
        }
    }
    #pragma endscop
}
