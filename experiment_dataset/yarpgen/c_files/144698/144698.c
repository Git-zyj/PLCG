/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 144698
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=144698 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/144698
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((var_14 * (((-var_3 ? (~54) : (~188))))));

    /* vectorizable */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        var_18 = (arr_0 [7]);
        arr_2 [i_0 + 2] = (arr_1 [i_0] [i_0]);
        arr_3 [i_0] = ((((((arr_1 [i_0 + 2] [i_0]) ? (arr_1 [1] [i_0]) : 4734507250636497012))) ? ((13571481020167385254 ? (arr_1 [1] [4]) : (arr_1 [i_0 + 2] [1]))) : (arr_0 [i_0 - 1])));
        arr_4 [i_0] = (((((201 ? (arr_0 [3]) : 469859645))) ? -1701875397 : 3887099052));
    }
    #pragma endscop
}
