/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118408
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118408 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118408
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_11 = (((max((((var_4 ? 41 : var_2))), (max(5896359872305718466, 7)))) * (arr_1 [i_0] [4])));
        var_12 = (((arr_1 [i_0] [i_0]) ^ var_6));
        arr_3 [i_0] [i_0] = var_8;
        var_13 = (max(18446744073709551593, (arr_0 [i_0])));
        arr_4 [i_0] = ((!((max(var_9, ((-(arr_1 [i_0] [i_0]))))))));
    }
    var_14 = ((!((max((max(993323035297360798, var_1)), ((max(var_6, var_10))))))));
    #pragma endscop
}
