/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97135
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97135 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97135
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) var_11);
    /* LoopNest 2 */
    for (unsigned long long int i_0 = 0; i_0 < 23; i_0 += 2) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 23; i_1 += 2) 
        {
            {
                var_21 = ((/* implicit */_Bool) max((max((min((((/* implicit */unsigned long long int) arr_5 [i_1] [(unsigned char)10])), (var_14))), (((/* implicit */unsigned long long int) ((((/* implicit */long long int) ((/* implicit */int) arr_2 [i_0]))) | (var_1)))))), (((unsigned long long int) min((var_8), (arr_4 [19LL] [i_1]))))));
                /* LoopSeq 1 */
                for (signed char i_2 = 1; i_2 < 22; i_2 += 3) 
                {
                    var_22 = ((/* implicit */_Bool) max((var_22), (((/* implicit */_Bool) (-(5ULL))))));
                    var_23 = arr_6 [i_0] [i_0] [i_0] [i_2];
                }
            }
        } 
    } 
}
