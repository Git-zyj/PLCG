/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68030
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68030 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68030
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [11] [10] = ((255 == (((min(var_14, (arr_1 [6] [i_0]))) * (!238)))));
                arr_8 [i_0] |= (arr_5 [i_0] [1] [6]);
                var_17 = var_13;
                var_18 = (max(var_18, -27051));
                var_19 = (min(var_10, ((((((arr_1 [i_0] [i_0]) ? 18446744073709551601 : 1)) <= (arr_5 [i_0] [i_0] [i_0]))))));
            }
        }
    }
    var_20 = (max(var_20, ((((var_12 ? -244228180 : -27048))))));
    #pragma endscop
}
