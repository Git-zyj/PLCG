/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14459
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14459 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14459
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = ((!(((-87 ? var_10 : 26320)))));

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        arr_4 [i_0] = ((~(min(((((arr_1 [i_0]) < -89))), ((~(arr_2 [4])))))));
        var_18 = (max((min((arr_0 [i_0] [i_0]), (-835 - var_8))), (arr_3 [i_0 + 1] [i_0 - 1])));
    }
    for (int i_1 = 0; i_1 < 0;i_1 += 1) /* same iter space */
    {
        var_19 *= ((22 ? 12765 : -32402));
        var_20 = (min(var_20, (-53 & 32402)));
    }
    /* vectorizable */
    for (int i_2 = 0; i_2 < 0;i_2 += 1) /* same iter space */
    {
        arr_9 [i_2] = var_8;
        var_21 *= var_4;
        arr_10 [i_2] [i_2] = (arr_5 [4]);
    }
    #pragma endscop
}
