/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 166372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=166372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/166372
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 |= var_12;
    var_15 |= ((!((max(2788693267439972429, (min(var_3, var_1)))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                arr_4 [i_1] = (min(2788693267439972429, ((min(var_13, 10255727153800760511)))));
                arr_5 [i_1] [i_1] = (((((min((arr_3 [i_0]), (arr_2 [i_0] [i_0] [19])))) ? (((min(var_13, 3725)))) : ((23886 ? var_7 : var_4)))));
                arr_6 [i_0] [1] = 0;
            }
        }
    }
    #pragma endscop
}
