/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 92778
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=92778 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/92778
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_9;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_1] = -3065503846434068293;
                arr_6 [6] = (((arr_3 [i_1] [i_0]) ? (((((-2277570917222933999 ? (arr_4 [i_0]) : 3065503846434068300))) ? (arr_4 [i_0]) : (arr_2 [i_0]))) : (((~(((arr_2 [i_1]) ? -527 : -6367)))))));
                arr_7 [i_1] = (((((-103 ? (arr_0 [i_0]) : (arr_0 [i_0])))) ? ((min((arr_0 [i_0]), (arr_0 [i_0])))) : ((-(arr_0 [i_0])))));
            }
        }
    }
    #pragma endscop
}
