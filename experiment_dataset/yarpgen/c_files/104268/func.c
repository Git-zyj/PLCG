/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 104268
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=104268 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/104268
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (short i_0 = 2; i_0 < 18; i_0 += 2) 
    {
        var_13 = ((/* implicit */int) ((short) arr_2 [i_0 - 1]));
        arr_3 [18U] = (unsigned short)18354;
        arr_4 [i_0] = ((/* implicit */short) (-(-820735863)));
        var_14 += ((/* implicit */unsigned short) ((unsigned long long int) arr_2 [i_0 - 1]));
    }
    var_15 = ((/* implicit */unsigned long long int) (-(286418137)));
}
