/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 1410
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=1410 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/1410
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 += ((var_14 ? (!var_18) : (var_2 & var_12)));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_5 [i_1] = (((max((arr_2 [i_0]), (arr_2 [i_1])))) ? ((((var_3 || (arr_2 [i_0]))))) : ((var_6 ? (arr_2 [i_1]) : var_13)));
                var_20 = (max((((arr_4 [i_0]) ? var_12 : (arr_4 [i_0]))), ((((((var_16 ? var_17 : (arr_2 [i_0])))) & (arr_3 [i_0]))))));
                var_21 -= var_0;
            }
        }
    }

    for (int i_2 = 0; i_2 < 18;i_2 += 1)
    {
        var_22 *= (arr_6 [i_2]);
        var_23 = var_8;
    }
    #pragma endscop
}
