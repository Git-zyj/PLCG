/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 181422
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=181422 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/181422
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 |= 576390383559245824;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {

                /* vectorizable */
                for (int i_2 = 0; i_2 < 22;i_2 += 1)
                {
                    var_19 = (arr_3 [i_0] [i_0]);
                    var_20 = (-2147483647 - 1);
                }
                arr_7 [i_0] [i_1] [i_1] = var_2;
                arr_8 [i_0] [i_1] [i_1] = 58787;
                var_21 = (min(var_21, ((min(((-((min(-22737, var_4))))), (-22065 > 1644330456378989203))))));
            }
        }
    }
    var_22 = 17642951592929660382;
    var_23 = ((max(var_9, ((var_15 ? -31078 : -31101)))) - (~31082));
    #pragma endscop
}
