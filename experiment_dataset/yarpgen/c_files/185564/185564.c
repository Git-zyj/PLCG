/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185564
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185564 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185564
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_11 = ((((((arr_7 [i_0] [i_1 - 1] [i_1 - 1]) ? (arr_6 [i_0 + 1] [i_1] [i_0 + 1]) : ((min((arr_6 [i_0] [i_1] [i_1]), 45)))))) > (min(-var_5, (min(var_5, var_7))))));
                var_12 = (min(var_12, ((min((((((((~(arr_1 [i_1] [i_0]))) + 2147483647)) << (((var_0 + 1312568170) - 26))))), var_4)))));
                arr_8 [i_1] = -1844275710;
                arr_9 [i_1] [i_1] [i_1] = 1693512479;
            }
        }
    }
    var_13 *= var_4;
    #pragma endscop
}
