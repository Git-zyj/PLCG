/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175103
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175103 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175103
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 = ((/* implicit */short) ((1004863235) << (((-1004863254) + (1004863254)))));
    /* LoopNest 2 */
    for (signed char i_0 = 1; i_0 < 9; i_0 += 2) 
    {
        for (short i_1 = 0; i_1 < 10; i_1 += 3) 
        {
            {
                arr_7 [i_1] = var_9;
                arr_8 [i_1] [i_1] = ((/* implicit */int) var_7);
            }
        } 
    } 
    var_11 -= ((/* implicit */unsigned char) max((((/* implicit */int) ((-1004863241) > (min((var_0), (-1004863236)))))), (var_0)));
}
