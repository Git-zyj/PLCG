/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 74655
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=74655 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/74655
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = var_4;
    var_13 = ((~(((var_11 / 1) >> (-1 + 28)))));
    var_14 = var_5;
    var_15 = var_7;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] [i_0] = ((((-(var_10 + var_2))) | -var_1));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 9;i_3 += 1)
                    {
                        {
                            var_16 = var_3;
                            arr_13 [i_0] [i_0] [i_1] [i_1] [i_2] [i_2] = var_7;
                            arr_14 [i_0] [i_1] [i_2] [i_2] = var_5;

                            /* vectorizable */
                            for (int i_4 = 1; i_4 < 9;i_4 += 1)
                            {
                                arr_17 [7] [i_1] [i_4] [i_3 + 2] [i_3] [i_4 + 3] = 1;
                                arr_18 [i_4] = (!1080378984);
                                arr_19 [i_0] [i_1 - 1] [i_2] [i_1 - 1] [i_4] = (var_3 + var_11);
                            }
                            for (int i_5 = 4; i_5 < 8;i_5 += 1)
                            {
                                arr_23 [i_0] &= -73;
                                arr_24 [i_0] [i_1] [i_2] [i_3] [i_5 + 4] &= var_9;
                                var_17 = var_3;
                            }
                        }
                    }
                }
                arr_25 [i_0] [i_0] [i_1] = -9056313009718651726;
                var_18 &= var_3;
            }
        }
    }
    #pragma endscop
}
