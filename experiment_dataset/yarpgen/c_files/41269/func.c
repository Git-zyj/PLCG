/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 41269
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=41269 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/41269
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_10 &= ((/* implicit */unsigned int) var_4);
    var_11 *= var_2;
    /* LoopSeq 2 */
    for (unsigned char i_0 = 0; i_0 < 12; i_0 += 1) 
    {
        var_12 = ((/* implicit */unsigned short) arr_1 [i_0]);
        var_13 |= ((/* implicit */short) arr_1 [i_0]);
    }
    for (unsigned int i_1 = 2; i_1 < 15; i_1 += 3) 
    {
        /* LoopSeq 3 */
        for (unsigned char i_2 = 3; i_2 < 13; i_2 += 1) /* same iter space */
        {
            arr_8 [i_1] [i_2] = ((/* implicit */int) arr_6 [i_1 - 2] [(_Bool)1]);
            var_14 = ((/* implicit */int) arr_6 [4U] [i_2]);
            var_15 *= ((/* implicit */unsigned int) arr_7 [i_2]);
            arr_9 [i_2] = arr_7 [i_2];
        }
        for (unsigned char i_3 = 3; i_3 < 13; i_3 += 1) /* same iter space */
        {
            var_16 = ((/* implicit */_Bool) arr_11 [i_1]);
            var_17 = arr_10 [4] [4] [i_3];
        }
        for (int i_4 = 2; i_4 < 15; i_4 += 2) 
        {
            var_18 = ((/* implicit */unsigned long long int) arr_14 [i_1] [i_1 - 1] [i_1]);
            var_19 = arr_11 [i_4];
            /* LoopNest 2 */
            for (unsigned int i_5 = 3; i_5 < 14; i_5 += 1) 
            {
                for (int i_6 = 1; i_6 < 13; i_6 += 1) 
                {
                    {
                        var_20 = ((/* implicit */unsigned long long int) arr_22 [i_1] [i_1] [i_1] [(unsigned short)11] [i_1] [i_1]);
                        var_21 = ((/* implicit */unsigned char) min((var_21), (((/* implicit */unsigned char) arr_16 [i_5] [i_5 - 1] [i_5]))));
                    }
                } 
            } 
            /* LoopSeq 1 */
            /* vectorizable */
            for (unsigned char i_7 = 4; i_7 < 14; i_7 += 2) 
            {
                var_22 = ((/* implicit */signed char) max((var_22), (((/* implicit */signed char) arr_11 [i_1]))));
                var_23 = ((/* implicit */signed char) arr_13 [i_1] [i_1]);
            }
            var_24 *= ((/* implicit */unsigned long long int) arr_22 [i_1] [(short)15] [i_1] [i_4] [i_4] [i_4]);
        }
        var_25 = ((/* implicit */_Bool) arr_5 [i_1] [(short)14]);
    }
    var_26 = ((/* implicit */unsigned int) var_4);
}
