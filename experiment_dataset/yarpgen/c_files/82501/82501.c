/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82501
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82501 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82501
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                var_18 = (((min((!var_5), (arr_3 [i_0])))) ? (max(var_16, (arr_6 [i_0] [i_0] [i_1]))) : (arr_5 [6] [i_1]));
                arr_7 [i_0] [i_0] = (((arr_5 [i_0] [i_1]) && (arr_4 [i_0 - 1] [i_0 - 1])));
                arr_8 [i_0] [i_0] [i_1] = (arr_3 [i_0]);
            }
        }
    }
    var_19 = var_13;
    var_20 = var_4;
    var_21 = ((((var_17 ? (var_16 || 2291846007950061463) : var_7)) ^ var_13));
    #pragma endscop
}
