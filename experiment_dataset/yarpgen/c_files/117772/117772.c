/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117772
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117772 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117772
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 13;i_2 += 1)
            {
                {
                    arr_7 [i_0 + 1] [i_1] = var_2;
                    var_16 = 17540067590105187478;

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_17 ^= (((max((!var_5), (min(3016015188396816408, (arr_5 [i_0 - 2]))))) > var_6));
                        arr_10 [i_0] [i_1] [i_2] [i_3] [i_0 + 2] [i_1] = ((((arr_1 [i_0 - 2]) ? 0 : (arr_1 [i_0 + 1]))));
                    }
                }
            }
        }
    }
    var_18 = (min(((min(var_13, var_2))), (((6 ? var_9 : 9223372036854775807)))));
    #pragma endscop
}
