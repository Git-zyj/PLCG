/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 224050
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=224050 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/224050
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 0;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_19 = min((var_12 / var_6), (arr_1 [i_0 + 1]));
                arr_7 [i_0] = min(((var_16 ? (-32763 && -104) : (~209))), ((-((~(arr_2 [i_0]))))));
                var_20 ^= (max(((var_2 ? (((arr_3 [i_0]) ? 148 : (arr_2 [i_0 + 1]))) : ((var_11 ? -1 : 148)))), 314021342));
            }
        }
    }
    var_21 = (max((var_14 < var_9), (((!((max(var_7, var_11))))))));
    var_22 ^= (var_1 >= var_1);
    #pragma endscop
}
