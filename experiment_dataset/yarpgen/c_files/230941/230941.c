/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230941
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230941 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230941
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 11;i_0 += 1)
    {
        arr_2 [i_0] = (arr_0 [i_0]);
        arr_3 [i_0] [i_0] = (min((~84), ((((arr_1 [i_0]) || (arr_1 [i_0]))))));
        var_10 = ((var_2 ^ ((var_1 ? ((max(var_6, 1))) : 14))));
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 23;i_1 += 1)
    {
        for (int i_2 = 2; i_2 < 19;i_2 += 1)
        {
            {
                var_11 = ((~(min(-62, ((-(arr_4 [i_2] [i_1])))))));
                arr_11 [i_1] [i_2] = min((arr_10 [i_2] [i_2 + 4]), (((max(var_2, (arr_4 [i_1] [2]))))));
            }
        }
    }
    var_12 = 7897923455682596834;
    #pragma endscop
}
