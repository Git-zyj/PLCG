/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26095
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26095 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26095
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 12;i_1 += 1)
        {
            {

                for (int i_2 = 0; i_2 < 16;i_2 += 1) /* same iter space */
                {

                    for (int i_3 = 1; i_3 < 14;i_3 += 1) /* same iter space */
                    {
                        var_19 = (arr_1 [i_0]);
                        var_20 = (max(var_20, (arr_8 [i_1 - 1] [i_1 + 4] [i_3 + 1] [i_3 - 1])));
                    }
                    for (int i_4 = 1; i_4 < 14;i_4 += 1) /* same iter space */
                    {
                        arr_16 [0] [i_1 + 3] [i_1 + 3] = (max((arr_5 [i_1 + 4] [i_4 + 1]), (arr_5 [i_1 + 1] [i_4 - 1])));
                        var_21 = (max((min(255, 65535)), (arr_9 [i_0] [i_0] [i_4] [i_4 + 1] [i_0] [i_4])));
                        var_22 = (max((arr_13 [i_4]), ((max((arr_9 [i_1] [i_1] [14] [i_4 - 1] [14] [5]), 1)))));
                    }
                    arr_17 [2] [i_1] [2] = ((-(arr_2 [i_0] [9] [i_0])));
                    var_23 |= (arr_8 [i_0] [i_0] [i_1] [14]);
                }
                for (int i_5 = 0; i_5 < 16;i_5 += 1) /* same iter space */
                {
                    var_24 = max((arr_19 [i_1 + 1] [i_1 - 1] [i_1 + 3]), (~-1));
                    var_25 = (arr_0 [4]);
                    var_26 = (min(var_26, (arr_5 [i_0] [i_1])));
                    var_27 = min((arr_3 [i_1 - 1]), (min((arr_3 [i_1 + 4]), (arr_3 [i_1 + 3]))));
                }
                var_28 = (arr_1 [i_0]);
                arr_22 [i_1 + 3] [i_0] [i_0] = (arr_18 [4] [9] [4]);
                arr_23 [i_0] [i_1 + 2] = (arr_13 [i_0]);
            }
        }
    }
    var_29 |= var_4;
    #pragma endscop
}
