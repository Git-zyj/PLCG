/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 159583
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=159583 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/159583
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 14;i_0 += 1)
    {
        var_10 = (((((arr_3 [i_0 + 1]) | (arr_3 [i_0 + 1]))) >> ((((arr_0 [i_0]) || ((!(arr_3 [i_0 + 2]))))))));
        arr_4 [i_0] = (arr_1 [i_0 + 3]);
        arr_5 [i_0 + 3] [i_0 + 3] = (arr_0 [i_0 - 1]);
    }
    /* LoopNest 2 */
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        for (int i_2 = 1; i_2 < 1;i_2 += 1)
        {
            {
                var_11 = ((((((arr_6 [i_2 - 1] [i_2]) != (arr_6 [i_2 - 1] [i_2])))) | (arr_6 [i_2 - 1] [i_2 - 1])));
                var_12 += (+(((arr_6 [i_2 - 1] [i_2 - 1]) | (arr_6 [i_2 - 1] [5]))));
            }
        }
    }
    #pragma endscop
}
