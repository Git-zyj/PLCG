/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 18671
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=18671 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/18671
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */_Bool) var_11);
    var_18 = ((/* implicit */signed char) ((var_13) / (((/* implicit */unsigned long long int) ((/* implicit */int) var_7)))));
    /* LoopSeq 1 */
    for (unsigned int i_0 = 2; i_0 < 14; i_0 += 3) 
    {
        var_19 = ((/* implicit */signed char) arr_0 [(_Bool)0] [i_0 - 2]);
        var_20 = ((/* implicit */unsigned short) 2520825003094432266LL);
    }
}
