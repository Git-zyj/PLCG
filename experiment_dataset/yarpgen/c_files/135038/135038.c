/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 135038
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=135038 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/135038
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_17 = var_7;
                    var_18 = (min(var_18, ((((((arr_5 [0]) / (arr_5 [8]))) == (((~(arr_3 [i_0 + 2] [1])))))))));
                    var_19 = (max(var_19, (((((-var_5 ? (((arr_3 [i_0 - 1] [0]) / (arr_4 [8]))) : (((!(arr_6 [i_2] [i_0] [i_0] [i_0])))))) * (((arr_2 [12] [12]) ? -1 : (((arr_5 [1]) ? (arr_4 [12]) : (arr_7 [1] [i_2]))))))))));
                }
            }
        }
    }
    var_20 = ((((max(-6563, var_15))) < ((max((((8545 ? var_0 : var_14))), (max(var_5, var_12)))))));
    #pragma endscop
}
