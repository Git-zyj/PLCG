/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 70750
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=70750 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/70750
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            {
                var_14 = (arr_3 [i_0] [i_1] [i_1]);
                var_15 = (min(((max((((8388607 && (arr_2 [i_0] [7])))), 0))), (max(213, 10076608064317584286))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 23;i_2 += 1)
    {
        for (int i_3 = 0; i_3 < 23;i_3 += 1)
        {
            for (int i_4 = 0; i_4 < 23;i_4 += 1)
            {
                {
                    var_16 += (max((((2251799813685247 == (((514246639810137275 ? var_13 : (arr_5 [i_2]))))))), ((-((((arr_9 [i_2] [i_3] [i_3] [i_4]) || var_11)))))));
                    arr_10 [4] [i_3] [9] [9] = (arr_9 [14] [5] [i_4] [9]);
                }
            }
        }
    }
    var_17 = var_3;
    var_18 = var_12;
    #pragma endscop
}
