/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 103467
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=103467 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/103467
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (((((-((var_2 ? var_10 : var_4))))) ? (((36271 <= (var_5 ^ 64)))) : 49152));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 19;i_2 += 1)
            {
                {
                    arr_10 [i_0 + 4] [i_2] [i_0] = min((arr_8 [i_1] [i_2]), (arr_2 [i_1 + 1] [i_0 + 2]));
                    arr_11 [i_2] [i_1 - 1] [i_1] [i_2] &= ((~(var_9 <= var_8)));
                    var_13 = (max(var_13, 120));
                    var_14 = (!65);
                }
            }
        }
    }
    #pragma endscop
}
