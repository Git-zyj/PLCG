/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168469
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168469 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168469
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 2; i_0 < 13; i_0 += 4) 
    {
        var_16 = ((/* implicit */signed char) ((((/* implicit */_Bool) (~(((/* implicit */int) arr_1 [i_0 - 2] [i_0 - 2]))))) ? ((~(((/* implicit */int) arr_1 [i_0 + 2] [i_0 + 2])))) : (((((/* implicit */int) arr_1 [i_0 + 1] [i_0])) & (((/* implicit */int) arr_1 [i_0 - 1] [i_0 - 1]))))));
        var_17 *= ((/* implicit */signed char) (-(arr_0 [i_0])));
        var_18 &= (-(arr_0 [(short)12]));
    }
    var_19 = ((/* implicit */int) ((((/* implicit */unsigned long long int) ((/* implicit */int) var_1))) & (var_8)));
}
