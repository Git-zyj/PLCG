/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 54345
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=54345 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/54345
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        arr_2 [i_0] = ((((-(arr_1 [i_0]))) | ((var_3 ? (arr_0 [i_0]) : (arr_1 [i_0])))));
        var_14 -= -127;
        var_15 = (max(var_15, (((arr_1 [2]) | var_9))));
        arr_3 [i_0] [i_0] = -1;
    }
    for (int i_1 = 1; i_1 < 1;i_1 += 1)
    {
        var_16 = (min(var_4, (arr_5 [i_1 - 1] [i_1 - 1])));
        arr_6 [i_1] = arr_4 [i_1];
        arr_7 [i_1] [i_1] = ((((max(-18, (-32767 - 1)))) > (((arr_4 [i_1 - 1]) | -2))));
    }
    var_17 = 1;
    var_18 = (~var_1);
    var_19 = var_10;
    #pragma endscop
}
