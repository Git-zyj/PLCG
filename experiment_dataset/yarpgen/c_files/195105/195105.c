/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 195105
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=195105 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/195105
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_17 = (min(var_17, ((min((max((arr_1 [i_0] [i_0]), (arr_1 [i_0] [i_0]))), (var_3 && var_3))))));

        for (int i_1 = 1; i_1 < 14;i_1 += 1)
        {
            var_18 = 548372430;
            arr_5 [14] [14] = (max((((+(((arr_4 [i_0] [7]) ? var_1 : -8))))), 9170083424108147931));
        }
    }
    /* vectorizable */
    for (int i_2 = 1; i_2 < 1;i_2 += 1)
    {
        arr_8 [i_2] [i_2] = (arr_3 [i_2 - 1]);
        var_19 ^= (((((-(arr_3 [i_2])))) ? var_6 : 750153023));
    }
    for (int i_3 = 0; i_3 < 19;i_3 += 1)
    {
        var_20 -= (~1);
        arr_11 [i_3] = (max(1, var_15));
    }
    var_21 = -21846602;
    #pragma endscop
}
