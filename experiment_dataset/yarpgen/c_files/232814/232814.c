/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 232814
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=232814 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/232814
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 15;i_2 += 1)
            {
                {
                    var_11 = min((~1), (max((((arr_2 [i_0 + 2] [i_1 + 2]) ? 2147483647 : var_6)), 0)));
                    var_12 |= (max((max(-2147483644, (arr_4 [1] [i_1] [8]))), (arr_6 [i_0 + 1] [i_0] [i_0 + 1])));
                    var_13 = (min(var_13, 7904));
                    var_14 *= (((arr_0 [i_0 - 2] [i_1 - 2]) != 1));
                    var_15 = (((!(arr_2 [i_0] [i_1]))));
                }
            }
        }
    }
    var_16 |= 1032192;
    var_17 = var_4;
    #pragma endscop
}
