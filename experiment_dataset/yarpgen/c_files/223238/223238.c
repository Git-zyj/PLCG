/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 223238
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=223238 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/223238
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_12 = (((arr_4 [i_1] [i_1 - 1] [i_0]) && (arr_5 [i_1 + 2] [i_1 + 2])));
                var_13 = var_1;
            }
        }
    }
    var_14 = ((((min(var_9, -2417918788539562235))) <= (max(((var_0 << (var_10 + 509100803))), ((((var_1 + 9223372036854775807) >> (var_4 - 1003262087))))))));
    #pragma endscop
}
