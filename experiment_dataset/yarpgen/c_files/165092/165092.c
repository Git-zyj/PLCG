/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 165092
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=165092 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/165092
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_1] = (max((((min(4864692798503964932, 5125875920537654721)))), 1032751900));
                var_19 = (((((((arr_4 [i_0] [1]) ^ (arr_3 [i_0]))))) ? (((((var_6 ? 487454151 : 4303771233251820202))) ? (arr_2 [i_0] [i_1]) : (((arr_4 [i_1] [i_0]) ? (arr_2 [i_1] [i_0]) : (arr_0 [i_1]))))) : ((((!(arr_4 [i_1] [i_0])))))));
                var_20 = arr_1 [i_0];
            }
        }
    }
    var_21 = (min(var_21, 7));
    var_22 *= var_5;
    #pragma endscop
}
