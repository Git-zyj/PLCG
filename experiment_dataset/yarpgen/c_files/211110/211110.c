/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 211110
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=211110 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/211110
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 19;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 15;i_2 += 1)
            {
                {
                    var_18 = (min(var_18, var_9));
                    arr_7 [7] [7] = -28348;
                }
            }
        }
        var_19 *= (((((arr_0 [i_0]) + 2147483647)) << (((((arr_0 [i_0]) + 96)) - 15))));
    }
    var_20 = (max(var_20, (var_1 > var_12)));
    var_21 &= (((((var_17 ? (((-16101 ? var_6 : var_7))) : (max(-5345357991842171651, var_0))))) ? (((var_4 <= var_6) ? (!var_16) : var_14)) : (max((var_12 - var_17), var_9))));
    #pragma endscop
}
