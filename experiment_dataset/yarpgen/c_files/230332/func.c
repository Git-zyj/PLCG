/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 230332
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=230332 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/230332
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_20 = ((/* implicit */long long int) min((var_20), (var_6)));
    /* LoopSeq 2 */
    /* vectorizable */
    for (short i_0 = 0; i_0 < 13; i_0 += 2) /* same iter space */
    {
        var_21 = ((/* implicit */unsigned char) 1852262899U);
        var_22 = ((/* implicit */unsigned char) var_19);
    }
    for (short i_1 = 0; i_1 < 13; i_1 += 2) /* same iter space */
    {
        var_23 = ((/* implicit */unsigned char) ((((/* implicit */_Bool) var_5)) ? (((((/* implicit */_Bool) arr_2 [i_1] [i_1])) ? (((/* implicit */unsigned long long int) (~(arr_1 [(_Bool)1])))) : ((+(arr_2 [i_1] [i_1]))))) : (arr_2 [5ULL] [5ULL])));
        var_24 = ((/* implicit */int) (-(arr_2 [(short)2] [(short)2])));
    }
}
