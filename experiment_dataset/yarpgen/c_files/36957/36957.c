/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 36957
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=36957 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/36957
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 2; i_2 < 18;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_0] [i_2] = 5;
                    var_12 = ((((arr_4 [i_2 - 2] [i_2 - 2] [i_2 + 2]) < var_5)));
                    arr_7 [19] = var_5;
                    arr_8 [i_0] [i_0] [i_1] [i_2] |= (max(((((arr_4 [i_2 + 1] [i_2 + 3] [i_2 + 1]) > var_0))), ((((0 ? (-9223372036854775807 - 1) : (arr_5 [i_0] [i_1] [i_2]))) | 5))));
                }
            }
        }
    }
    var_13 = ((var_4 ? var_0 : (((((var_7 ? var_11 : 7))) & (min(var_6, 268435440))))));
    var_14 = var_11;
    #pragma endscop
}
