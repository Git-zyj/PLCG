/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97730
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97730 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97730
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_13 = ((/* implicit */unsigned long long int) var_9);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13; i_0 += 3) 
    {
        for (unsigned long long int i_1 = 3; i_1 < 10; i_1 += 3) 
        {
            {
                var_14 = ((/* implicit */signed char) min((var_14), (((/* implicit */signed char) ((unsigned long long int) var_10)))));
                var_15 = ((/* implicit */signed char) min((arr_2 [i_1] [i_1 + 2]), (((/* implicit */unsigned int) ((((/* implicit */_Bool) var_11)) || (((/* implicit */_Bool) arr_2 [i_1 - 2] [i_1 + 2])))))));
                arr_6 [i_1] = ((/* implicit */int) ((((/* implicit */unsigned long long int) (+(((unsigned int) var_8))))) - (var_10)));
            }
        } 
    } 
}
