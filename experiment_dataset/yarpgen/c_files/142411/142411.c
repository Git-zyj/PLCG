/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 142411
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=142411 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/142411
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(var_14, var_13));
    var_15 = var_6;
    var_16 ^= (min(var_4, (~var_1)));

    for (int i_0 = 1; i_0 < 19;i_0 += 1)
    {
        var_17 = (max(((((max((arr_2 [i_0] [i_0 + 1]), var_4))) ? (arr_2 [i_0] [i_0]) : (-2147483647 - 1))), (((((var_6 ? var_4 : var_4))) ? (86 != 32767) : var_13))));
        var_18 += var_7;
    }
    for (int i_1 = 2; i_1 < 6;i_1 += 1)
    {
        arr_5 [9] = arr_1 [0];
        arr_6 [i_1] = min(((var_2 ? 1435385465 : (~0))), (min((var_10 / var_1), var_12)));
        var_19 += 1;
        var_20 = max((max((var_12 | var_6), var_2)), (-1 * -1445659550));
        var_21 += (arr_1 [2]);
    }
    for (int i_2 = 0; i_2 < 16;i_2 += 1)
    {
        arr_9 [i_2] = ((-var_0 ? var_6 : ((var_10 / (arr_0 [i_2])))));
        arr_10 [i_2] = -1740;
    }
    #pragma endscop
}
