/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179622
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179622 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179622
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        var_19 |= (arr_0 [4]);
        var_20 = ((-(min((max((arr_2 [i_0] [10]), 11091223088152208811)), (((arr_0 [i_0]) ^ 12780889603537149116))))));
        var_21 = (-214483032 ? (((((var_10 ? -1627509473 : -14457))) ? 39757 : (arr_2 [i_0] [11]))) : 10956);
    }
    var_22 = ((~((var_0 ? 1627509460 : (-688828530 || -1627509473)))));
    var_23 = (min(var_23, var_14));
    var_24 -= ((-(((-901489372 == var_9) ? (max(47423, var_1)) : -25320))));
    #pragma endscop
}
