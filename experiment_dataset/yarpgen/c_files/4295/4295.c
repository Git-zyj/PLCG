/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4295
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4295 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4295
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, (((((var_12 || -10) ? -10 : (-9223372036854775807 - 1))) <= 54833))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            var_18 = ((((((((arr_2 [i_3] [20]) ? (arr_8 [23] [i_2] [21] [i_0]) : 9223372036854775789)) & (14 * 358667298)))) ? (max(31044, 9223372036854775784)) : (!var_1)));
                            var_19 = var_16;
                        }
                    }
                }
                arr_12 [i_1] [i_1] = (arr_0 [i_1]);
                arr_13 [i_1] [i_1] [i_1] = (~-10);
            }
        }
    }
    var_20 = var_11;
    #pragma endscop
}
