/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 123743
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=123743 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/123743
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (var_0 || var_5);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 21;i_1 += 1)
        {
            {
                var_17 += ((((min((((arr_1 [i_1]) ? -83756785865723483 : 3559611393)), (arr_2 [0] [i_1 - 1])))) ? (((~var_11) ? (arr_1 [i_0]) : -var_5)) : ((((149 + 106) > ((-(arr_2 [12] [i_1 - 1]))))))));
                var_18 -= ((((((1891588015420821303 ? 761412237 : -1891588015420821281)))) ? (arr_3 [i_0]) : (arr_2 [14] [14])));
            }
        }
    }
    var_19 = var_10;
    var_20 = (min(var_20, (((-((-(!var_6))))))));
    var_21 = var_1;
    #pragma endscop
}
