/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145721
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145721 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145721
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 = ((/* implicit */unsigned long long int) (signed char)(-127 - 1));
    var_12 = ((/* implicit */unsigned short) (signed char)-94);
    /* LoopNest 3 */
    for (long long int i_0 = 2; i_0 < 9; i_0 += 2) 
    {
        for (unsigned char i_1 = 1; i_1 < 7; i_1 += 4) 
        {
            for (signed char i_2 = 1; i_2 < 7; i_2 += 1) 
            {
                {
                    var_13 &= ((/* implicit */int) var_3);
                    /* LoopNest 2 */
                    for (signed char i_3 = 0; i_3 < 11; i_3 += 4) 
                    {
                        for (unsigned long long int i_4 = 2; i_4 < 9; i_4 += 1) 
                        {
                            {
                                var_14 = ((/* implicit */unsigned long long int) var_7);
                                arr_12 [i_0] [i_0] [i_0] [i_2] [3ULL] = ((/* implicit */signed char) ((arr_4 [i_3] [i_2]) ^ (((/* implicit */unsigned int) ((/* implicit */int) ((((/* implicit */unsigned int) ((((/* implicit */_Bool) arr_6 [i_2])) ? (((/* implicit */int) arr_0 [i_2])) : (((/* implicit */int) (signed char)110))))) == ((-(4294967295U)))))))));
                            }
                        } 
                    } 
                }
            } 
        } 
    } 
}
