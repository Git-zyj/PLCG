/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132876
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132876 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132876
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_6;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 23;i_2 += 1)
            {
                {
                    var_12 |= (((1 ? 2370031830 : -1)));
                    var_13 = (((arr_0 [i_0]) ? (min((~1924935466), 105553116266496)) : ((((arr_7 [i_0] [i_0]) % (max(2370031830, 2147483647)))))));
                    var_14 = ((0 ? ((((arr_7 [i_0] [i_2]) <= ((1924935465 ? var_4 : (arr_3 [i_0])))))) : -2147483643));
                }
            }
        }
    }
    var_15 = (min(var_15, var_10));
    var_16 &= (((var_0 ? (1 ^ 5302402296503714763) : var_5)));
    #pragma endscop
}
