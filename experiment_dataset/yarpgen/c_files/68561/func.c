/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68561
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68561 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68561
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 |= ((/* implicit */unsigned int) var_7);
    var_18 = ((/* implicit */short) -4402627223179996175LL);
    /* LoopNest 3 */
    for (unsigned long long int i_0 = 4; i_0 < 14; i_0 += 2) 
    {
        for (int i_1 = 1; i_1 < 14; i_1 += 4) 
        {
            for (unsigned long long int i_2 = 2; i_2 < 16; i_2 += 1) 
            {
                {
                    arr_8 [i_1] = ((/* implicit */signed char) (+(((int) (!(var_10))))));
                    arr_9 [i_0 + 3] [i_0] [i_0] [i_1] = ((/* implicit */unsigned int) ((((/* implicit */_Bool) -9223372036854775802LL)) ? ((((-(var_16))) & (((((/* implicit */long long int) ((/* implicit */int) (_Bool)1))) & (-2241307166581525800LL))))) : (((/* implicit */long long int) ((/* implicit */int) ((_Bool) ((((/* implicit */_Bool) var_9)) ? (arr_6 [i_0] [i_1] [i_2]) : (((/* implicit */int) var_11)))))))));
                }
            } 
        } 
    } 
    var_19 = ((/* implicit */unsigned int) (~(((/* implicit */int) (signed char)-29))));
}
