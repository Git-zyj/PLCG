/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 185094
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=185094 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/185094
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 ^= var_15;
    var_18 = ((/* implicit */int) var_13);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14; i_0 += 1) 
    {
        for (unsigned long long int i_1 = 0; i_1 < 14; i_1 += 2) 
        {
            {
                var_19 &= ((/* implicit */int) ((((/* implicit */unsigned long long int) (+((+(var_14)))))) / (arr_3 [i_1] [i_0])));
                arr_6 [i_1] = ((/* implicit */unsigned char) (-(((((/* implicit */_Bool) arr_0 [i_0])) ? (((/* implicit */int) arr_0 [i_1])) : (1362094828)))));
                arr_7 [i_0] = ((/* implicit */int) var_7);
            }
        } 
    } 
}
