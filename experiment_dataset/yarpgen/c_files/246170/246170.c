/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246170
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246170 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246170
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min((min(4611123068473966592, (~7745787964015576670))), ((min(1, var_10)))));
    var_13 = ((((max(var_11, ((max(var_5, var_9)))))) > (min((min(var_4, 7745787964015576675)), var_1))));

    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        arr_2 [i_0] = var_3;
        var_14 = var_3;
        arr_3 [i_0] [i_0] = (((((((arr_1 [i_0] [i_0]) || (arr_1 [i_0] [i_0]))))) >= (((arr_1 [i_0] [i_0]) / (arr_1 [i_0] [i_0])))));
        var_15 += ((((min(var_3, 61931))) ? var_7 : var_8));
        var_16 = ((-115 ? -96 : (114 | -101)));
    }
    #pragma endscop
}
