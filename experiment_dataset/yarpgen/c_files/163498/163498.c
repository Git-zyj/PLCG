/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 163498
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=163498 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/163498
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 |= (-((2077099531 ? (max(1, var_3)) : (((var_5 ? var_0 : var_6))))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_12 = (((((((max((arr_2 [i_0] [i_0] [i_1]), (arr_0 [8])))) ? ((-(arr_4 [i_1]))) : 0))) ? ((((var_7 ? var_9 : 1)) / (((arr_2 [i_1] [i_1] [6]) * 1)))) : ((~((-106 ? 1 : (arr_5 [5] [i_0])))))));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 11;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 11;i_3 += 1)
                    {
                        {
                            var_13 = (max(var_13, 15641582134316446506));
                            var_14 = (arr_3 [i_0] [4]);
                        }
                    }
                }
                var_15 = 1;
            }
        }
    }
    #pragma endscop
}
