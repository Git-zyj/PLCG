/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 188049
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=188049 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/188049
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                var_18 += ((!((min(var_8, (arr_0 [i_1] [i_1]))))));
                var_19 = (((((+(((arr_2 [6] [i_1]) - (arr_0 [i_0] [i_0])))))) + (((var_14 - -1911040889760148496) ? var_15 : 1911040889760148470))));
            }
        }
    }
    var_20 = (max(var_20, ((max((~var_11), (((((var_3 ? var_1 : -1911040889760148496))) ? var_7 : ((var_12 << (var_4 - 151))))))))));
    #pragma endscop
}
