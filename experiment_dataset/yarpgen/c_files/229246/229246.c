/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229246
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229246 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229246
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 *= ((!((min((max(var_7, (-127 - 1))), (min(var_13, 520093696)))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 22;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] = ((((((max(6039, 6039))))) ? (((27659 ? (((arr_3 [i_0] [i_0] [i_0 + 1]) * (arr_6 [i_1]))) : (((min(var_2, 30564))))))) : (min((arr_6 [i_0]), (arr_5 [i_0 - 1] [i_1 - 3] [i_2 + 1])))));
                    var_16 = (arr_6 [i_0]);
                }
            }
        }
    }
    var_17 *= (min(var_11, var_8));
    var_18 ^= var_0;
    var_19 = var_7;
    #pragma endscop
}
