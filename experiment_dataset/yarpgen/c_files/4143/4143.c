/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 4143
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=4143 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/4143
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = 193;
    var_20 = (min(8, ((min((-1211440603 >= var_6), var_18)))));
    var_21 = (max(var_5, var_4));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            {
                arr_5 [7] [i_1] = (((((max((((!(arr_2 [5] [i_1])))), (arr_3 [i_0 - 1] [i_0 + 1]))))) + 389790541));
                var_22 = (min(var_22, (((389790541 % (arr_1 [i_0]))))));
            }
        }
    }
    var_23 += ((((((var_11 ? var_8 : var_16))) ? (6755399441055744 & var_13) : var_18)));
    #pragma endscop
}
