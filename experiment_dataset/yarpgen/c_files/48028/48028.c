/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 48028
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=48028 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/48028
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((((min((min(168, 2965282269)), var_3))) % (((min(-7961, 18161910816911292369)) << (((min(412824201, 643381394)) - 412824147))))));
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_1] [i_0] = 14604922337144730965;
                arr_5 [i_0] [i_1] [i_1] = 1266027067;
                var_11 &= var_3;
            }
        }
    }
    #pragma endscop
}
