/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249230
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249230 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249230
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = ((/* implicit */unsigned int) 661444007);
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        var_16 += ((/* implicit */long long int) (unsigned short)0);
        var_17 = ((/* implicit */unsigned short) arr_1 [i_0] [i_0]);
        arr_3 [i_0] = ((/* implicit */unsigned short) min(((+(((((/* implicit */int) var_5)) << (((var_12) - (4505580661441732124ULL))))))), (((/* implicit */int) min((((/* implicit */short) arr_0 [i_0])), (var_1))))));
        var_18 |= ((/* implicit */_Bool) arr_1 [i_0] [i_0]);
    }
    var_19 = ((/* implicit */_Bool) var_13);
}
