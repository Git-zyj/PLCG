/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 167233
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=167233 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/167233
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = -6462;
    var_11 |= ((175 ? 0 : 0));
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 14;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 16;i_3 += 1)
                    {
                        arr_11 [1] [i_1] [i_0] [i_1] = 8;
                        var_12 = -1;
                    }
                    arr_12 [i_0] [i_1] [i_2 - 2] = ((+((max((arr_8 [i_0] [i_0 - 3]), (arr_6 [i_0] [i_1] [i_2]))))));
                    var_13 -= (max(2373368300366311365, ((((((8355840 ? -1 : 494851259))) ? 177 : (arr_5 [i_1] [i_1] [1]))))));
                }
            }
        }
    }
    var_14 = -var_7;
    #pragma endscop
}
