/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 88601
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=88601 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/88601
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = ((var_2 ? (((((57118 << (var_6 - 3530768030)))) ? ((4075241690820332304 ? 57099 : 2819422208)) : (var_7 < var_6))) : -91));
    var_11 = 57115;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] [i_1] [i_2] = ((57118 && (arr_6 [i_2])));
                    var_12 = (max(18766, (max((arr_4 [i_0]), (((arr_3 [i_0] [i_1]) / (arr_2 [i_0])))))));
                }
            }
        }
    }
    var_13 = (!57098);
    #pragma endscop
}
