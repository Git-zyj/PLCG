/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46778
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46778 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46778
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 12;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 11;i_3 += 1)
                {
                    {
                        var_11 = ((1 ? ((((arr_2 [i_0]) ? var_7 : (arr_3 [i_3])))) : ((min((arr_10 [i_0 + 2] [i_0 + 2] [i_2 - 1] [i_3] [i_3]), var_6)))));
                        var_12 += ((~(arr_1 [i_0 + 2])));
                        var_13 ^= (((((1 ? 1 : 1))) - (arr_8 [i_2 - 3] [i_1] [i_0 + 2])));
                    }
                }
            }
        }
        arr_11 [i_0 - 1] [i_0] = (arr_3 [i_0]);
    }
    var_14 = (((((242 ? 242 : -65))) ? 127 : ((var_3 ? var_1 : var_8))));
    var_15 = var_4;
    var_16 = -1;
    var_17 = (max(var_17, var_3));
    #pragma endscop
}
