/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50947
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50947 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50947
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (((48844 * var_15) & (~var_8)));

    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        arr_2 [i_0] = arr_0 [i_0 + 2];
        arr_3 [i_0] [i_0 + 3] = -31130;
        arr_4 [i_0] = (min((11 / 1), (min(9628855140781996064, 10396))));
    }
    for (int i_1 = 2; i_1 < 11;i_1 += 1)
    {
        arr_8 [i_1] = ((var_13 ? (((arr_5 [i_1 + 2]) ^ (arr_6 [i_1 + 2]))) : (arr_5 [i_1 - 2])));
        arr_9 [i_1] = (max(((~(arr_6 [i_1 + 3]))), (((arr_6 [i_1 + 1]) ? (arr_6 [i_1 + 3]) : (arr_6 [i_1 + 2])))));
    }
    #pragma endscop
}
