/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119722
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119722 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119722
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (~var_1);
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            {
                arr_4 [i_0 + 2] [i_1] = (~-77063657);
                arr_5 [i_1] [i_1] = ((!((!(arr_2 [i_1] [i_0])))));
                var_13 = (min(var_13, (var_0 && var_10)));

                /* vectorizable */
                for (int i_2 = 2; i_2 < 15;i_2 += 1)
                {
                    var_14 &= -var_1;
                    arr_9 [i_0] [6] [i_1] [i_0] = var_1;
                    var_15 -= ((!((((arr_3 [i_0]) ? (arr_2 [i_0] [i_0]) : (arr_2 [i_0] [10]))))));
                }
                var_16 = max((max((arr_1 [i_0] [i_0]), var_10)), ((((min(var_3, var_2))) | var_10)));
            }
        }
    }
    #pragma endscop
}
