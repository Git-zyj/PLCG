/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 136343
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=136343 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/136343
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = 3894538001;
    var_19 = min(var_2, ((-((max(var_1, var_2))))));

    for (int i_0 = 1; i_0 < 22;i_0 += 1)
    {
        arr_4 [i_0] = (((!(arr_0 [i_0 + 2] [21]))));

        /* vectorizable */
        for (int i_1 = 3; i_1 < 23;i_1 += 1)
        {
            /* LoopNest 2 */
            for (int i_2 = 0; i_2 < 24;i_2 += 1)
            {
                for (int i_3 = 1; i_3 < 22;i_3 += 1)
                {
                    {
                        var_20 = (max(var_20, ((((arr_8 [9] [i_1] [i_3 + 2]) * (arr_10 [20] [20]))))));
                        arr_13 [i_1] = -var_11;
                    }
                }
            }
            var_21 = (arr_11 [i_0] [i_0] [i_0] [i_0] [1] [i_1]);
            var_22 = (arr_1 [i_1 + 1]);
            arr_14 [i_1] = ((~(((!(arr_3 [i_1 + 1]))))));
        }
    }
    for (int i_4 = 2; i_4 < 22;i_4 += 1)
    {
        arr_19 [i_4] = var_6;
        arr_20 [i_4] [i_4] = (arr_1 [i_4]);
    }
    #pragma endscop
}
