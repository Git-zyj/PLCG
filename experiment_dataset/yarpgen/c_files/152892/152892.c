/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 152892
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=152892 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/152892
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_18 = (((((arr_1 [i_1 - 1]) * (arr_1 [i_1 - 1]))) % ((var_17 + (arr_1 [i_1 - 1])))));
                arr_5 [i_0] [i_1 - 1] = (((((arr_4 [i_0] [i_1] [i_1 - 1]) ? (arr_4 [i_0] [9] [i_1 - 1]) : (arr_4 [i_0] [i_1] [i_1 - 1]))) << ((((((~(arr_4 [i_0] [i_0] [i_1 - 1]))) + 948209513)) - 30))));
                var_19 = (arr_1 [i_0]);
            }
        }
    }
    var_20 *= var_9;
    var_21 = ((var_11 + ((var_4 ? var_2 : (((min(var_14, 39345))))))));
    #pragma endscop
}
