/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 105306
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=105306 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/105306
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = var_2;
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 8;i_2 += 1)
            {
                {
                    arr_9 [i_0] = (max((((!(arr_4 [i_0] [i_0 - 1] [i_0 + 1])))), 1370456256));
                    var_11 = ((+(((arr_4 [i_2 + 3] [i_1] [i_0 - 1]) ? (-32767 - 1) : 1))));
                }
            }
        }
    }
    var_12 = (min(((min(152, (min(var_5, 1))))), 21873));
    var_13 = (max(var_13, ((((~-1) ? (~31174) : (0 * var_0))))));
    #pragma endscop
}
