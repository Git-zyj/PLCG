/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 132369
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=132369 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/132369
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (min(var_17, -16968));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                arr_4 [i_1] [0] [i_1] = (arr_0 [i_0]);
                arr_5 [i_1] [i_1] [i_1] = ((((max((((var_9 < (arr_3 [i_1])))), (min(255, 255))))) < ((((~4294967284) < (((max((arr_3 [i_1]), var_4)))))))));
                arr_6 [i_1] [7] [i_1] = 238;
                var_18 = ((~(((arr_2 [i_0] [i_1] [i_1]) / ((max(9265, var_7)))))));
            }
        }
    }
    #pragma endscop
}
