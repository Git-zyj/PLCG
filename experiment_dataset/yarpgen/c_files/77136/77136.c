/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77136
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77136 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77136
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 4; i_0 < 9;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] [i_1] = arr_2 [i_0 - 3] [i_0];
                arr_5 [i_1] = (((((max(var_1, (arr_2 [i_0] [i_1])))) ? (max((arr_3 [i_0 - 3] [i_0] [i_1]), var_9)) : (arr_2 [i_0 + 1] [i_0 - 2]))));
            }
        }
    }
    /* LoopNest 3 */
    for (int i_2 = 0; i_2 < 10;i_2 += 1)
    {
        for (int i_3 = 4; i_3 < 9;i_3 += 1)
        {
            for (int i_4 = 3; i_4 < 7;i_4 += 1)
            {
                {
                    var_12 = var_9;
                    /* LoopNest 2 */
                    for (int i_5 = 3; i_5 < 9;i_5 += 1)
                    {
                        for (int i_6 = 0; i_6 < 10;i_6 += 1)
                        {
                            {
                                var_13 = (((arr_3 [i_5] [i_4] [i_5]) ? (arr_3 [i_5] [i_2] [i_5]) : (arr_3 [i_5] [i_2] [i_5])));
                                var_14 = (min((arr_6 [i_2]), ((-55 ? 119 : 103))));
                            }
                        }
                    }
                    var_15 = (arr_7 [i_2]);
                }
            }
        }
    }
    var_16 = (min(var_16, ((((((var_2 ? ((var_0 ? var_1 : var_1)) : ((var_5 ? var_5 : var_6))))) ? (((max(var_1, var_1)))) : 255)))));
    #pragma endscop
}
