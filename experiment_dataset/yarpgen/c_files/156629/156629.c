/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156629
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156629 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156629
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = ((var_14 ? (min(268433408, var_16)) : var_0));
    var_21 ^= var_3;
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        var_22 = (max((((((max((arr_7 [i_0] [i_0] [i_0]), var_8))) ? (((arr_1 [i_1]) | 268433385)) : (arr_0 [i_2] [i_3])))), (600712679 & 7516192768)));
                        arr_9 [i_0] [i_0] = 13;
                    }
                    arr_10 [i_0] [i_0] [i_2] [i_0] = var_6;
                }
            }
        }
    }
    var_23 = var_19;
    #pragma endscop
}
