/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127548
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127548 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127548
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 3; i_1 < 21;i_1 += 1)
        {
            {
                var_18 = (max(var_18, ((((((((65535 ? (arr_1 [i_0] [i_1 - 2]) : 0))) ? ((65535 ? (arr_1 [i_0] [i_0]) : var_10)) : (~0))) < (arr_0 [i_0]))))));
                var_19 = (arr_4 [i_0] [i_0] [i_0]);
                arr_5 [i_0] [19] = (((((-247 | (((-1 && (arr_3 [i_0] [2]))))))) ? var_4 : ((((var_7 / var_10) >= ((var_1 ? var_12 : (arr_0 [21]))))))));
            }
        }
    }
    var_20 = var_6;
    #pragma endscop
}
