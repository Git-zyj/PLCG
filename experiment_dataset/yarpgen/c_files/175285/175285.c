/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175285
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175285 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175285
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (-2147483647 - 1);
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_21 = (max(var_21, ((min((min(((1 ? var_16 : 59043)), var_12)), ((((arr_3 [i_0] [i_0]) != (arr_3 [i_0] [i_1])))))))));
                arr_4 [i_0] [i_0] |= (((((min(-4958782785285431100, -71))) ? -1 : ((-60 ? 1272035032 : 4958782785285431129)))) >> (((min(59043, (1787876687 >> 0))) - 58986)));
            }
        }
    }
    var_22 = var_13;
    #pragma endscop
}
