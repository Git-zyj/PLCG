/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 106118
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=106118 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/106118
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_11 = var_1;
    var_12 = var_3;

    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {

        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            arr_7 [i_1] [i_0] [i_0] = 8387878459542437672;

            for (int i_2 = 1; i_2 < 12;i_2 += 1)
            {
                var_13 ^= 3672133285136407570;
                arr_11 [i_0] [i_1] [i_2] = ((~var_2) ? 3277625875 : ((~((var_3 ? (arr_9 [i_0] [i_1] [i_2]) : 353793848780756351)))));
            }
        }
        var_14 -= (arr_10 [i_0] [i_0]);
        var_15 = (min(var_15, (((6095090469475995390 >> (13293695094215482257 - 13293695094215482222))))));
    }
    #pragma endscop
}
