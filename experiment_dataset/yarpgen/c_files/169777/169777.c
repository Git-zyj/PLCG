/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 169777
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=169777 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/169777
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, var_12));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] [i_0] = (arr_0 [i_0]);
        arr_3 [i_0] = (((((min(-2074135035265886294, (arr_1 [i_0]))))) && 1));
    }
    for (int i_1 = 3; i_1 < 15;i_1 += 1)
    {
        arr_8 [i_1] = (-(min(((max((arr_5 [i_1] [i_1]), var_11))), (min((arr_5 [i_1 + 1] [i_1 - 1]), (arr_7 [i_1 - 1] [i_1 - 3]))))));
        arr_9 [i_1 + 1] = 64827;
    }
    var_19 = (((var_0 | (min(var_0, 16711680)))) < ((((max(65535, var_0))) ? var_11 : var_14)));
    #pragma endscop
}
