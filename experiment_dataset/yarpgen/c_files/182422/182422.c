/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 182422
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=182422 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/182422
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (max((((((max((arr_4 [i_0]), 1))) < ((var_1 | (arr_3 [i_0])))))), ((((arr_1 [i_0]) ? (arr_0 [i_0]) : -127)))));
                arr_6 [4] [4] = ((-((1 ? (arr_2 [4] [i_1]) : (((!(arr_2 [0] [i_0 - 1]))))))));
            }
        }
    }
    var_15 = ((((max((((-73 ? -76 : var_8))), ((var_6 ? 2147483647 : var_13))))) ? var_13 : (((~(1 <= 1873776827))))));
    #pragma endscop
}
