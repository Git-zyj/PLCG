/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 19382
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=19382 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/19382
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= 1;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                arr_6 [1] [i_0 - 1] = (((arr_0 [i_0 + 1]) < ((var_9 - (arr_1 [i_0] [i_1])))));
                var_16 = (min(((~(arr_0 [i_0 - 1]))), (var_4 / 1)));
                arr_7 [i_0] [i_1] [i_0] = (6958712052500647220 % 6958712052500647220);
            }
        }
    }
    var_17 = ((var_10 ? (~17828404865963691080) : (min(((var_11 ? -55 : var_12)), 618339207745860550))));
    #pragma endscop
}
