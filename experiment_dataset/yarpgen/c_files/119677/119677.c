/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119677
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119677 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119677
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, ((((((max(var_9, var_7)) * (-127 - 1))))))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 10;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 10;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 3; i_3 < 8;i_3 += 1)
                    {
                        for (int i_4 = 2; i_4 < 9;i_4 += 1)
                        {
                            {
                                arr_15 [i_4 - 1] [i_3] [i_0] [i_3] [i_0] = (arr_7 [i_4 + 1] [i_1] [i_1] [i_0]);
                                arr_16 [i_0] [i_0] [i_3] [i_2] [i_2] [1] [i_4 - 1] = var_7;
                            }
                        }
                    }
                    var_13 = (min(var_13, (((-(arr_10 [i_0] [i_1] [i_1] [8] [i_2]))))));
                }
            }
        }
    }
    #pragma endscop
}
