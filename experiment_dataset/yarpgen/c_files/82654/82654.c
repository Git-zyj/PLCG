/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 82654
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=82654 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/82654
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((((min(var_13, var_9))) & var_9));
    var_17 = ((~(((1 ? 1 : var_4)))));

    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        var_18 = (min(var_18, (((((((arr_1 [11]) ? 70 : (arr_1 [4])))) ? ((max(1, (arr_0 [i_0])))) : (46006 <= var_10))))));
        var_19 += ((6 || ((max((arr_0 [i_0]), 6)))));
        var_20 = (max(var_20, (18446744073709551610 == 1)));
    }
    #pragma endscop
}
