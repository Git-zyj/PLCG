/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 121956
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=121956 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/121956
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 *= (!var_6);
    /* LoopNest 3 */
    for (int i_0 = 4; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {
                    var_13 = ((!(((((0 ^ (arr_1 [i_1 - 2] [i_2]))) ^ (((-7883516282467972082 ? 169 : 169))))))));
                    var_14 = 92;
                }
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 20;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 20;i_4 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_5 = 0; i_5 < 20;i_5 += 1)
                {
                    for (int i_6 = 3; i_6 < 19;i_6 += 1)
                    {
                        for (int i_7 = 0; i_7 < 20;i_7 += 1)
                        {
                            {
                                var_15 = var_0;
                                var_16 += ((-(max((arr_18 [i_3] [i_7] [i_5] [i_3] [i_3]), (((arr_16 [i_3] [i_4] [i_3] [i_6 - 3]) + 87))))));
                            }
                        }
                    }
                }
                /* LoopNest 3 */
                for (int i_8 = 2; i_8 < 19;i_8 += 1)
                {
                    for (int i_9 = 2; i_9 < 19;i_9 += 1)
                    {
                        for (int i_10 = 0; i_10 < 20;i_10 += 1)
                        {
                            {
                                arr_27 [i_3] [i_3] [i_4] [i_8] [i_9] [i_10] = ((!((max(var_5, var_3)))));
                                var_17 = ((!(!var_9)));
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
