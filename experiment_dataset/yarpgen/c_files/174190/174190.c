/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 174190
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=174190 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/174190
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = (min(var_20, ((min(((((var_0 ? var_15 : var_4)) << (var_11 / var_15))), ((((var_13 ? var_12 : var_14)))))))));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 7;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 7;i_2 += 1)
            {
                {
                    arr_9 [i_1] = (min((arr_3 [i_2 - 1] [i_2 + 1]), var_8));
                    var_21 -= (((min((min(var_8, var_1)), ((var_12 ? var_19 : (arr_2 [7]))))) <= (arr_1 [i_2])));
                }
            }
        }
    }
    var_22 = (max(var_16, var_9));
    #pragma endscop
}
