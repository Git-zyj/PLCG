/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 114970
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=114970 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/114970
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = var_14;
    var_16 = var_9;
    var_17 = var_6;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (min(72293988395660318, 1152921487426977792));
                var_18 = (min(var_18, (((+(((arr_0 [2]) ? (arr_0 [14]) : var_2)))))));
                var_19 = ((-(max(var_6, var_12))));
                var_20 += (((((-1 ? 1152921487426977792 : (arr_2 [i_1])))) ? (((arr_1 [0] [0]) ? ((((arr_1 [4] [4]) && var_2))) : (!var_10))) : ((((arr_0 [10]) && (arr_1 [4] [4]))))));
            }
        }
    }
    var_21 = var_6;
    #pragma endscop
}
