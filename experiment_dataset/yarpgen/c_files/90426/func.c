/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90426
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90426 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90426
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 3; i_0 < 10; i_0 += 2) 
    {
        for (unsigned int i_1 = 3; i_1 < 10; i_1 += 1) 
        {
            {
                var_16 ^= ((/* implicit */unsigned short) (~(arr_2 [11U] [9] [i_0])));
                var_17 = ((/* implicit */unsigned short) arr_0 [i_0]);
            }
        } 
    } 
    var_18 = ((/* implicit */short) var_9);
    var_19 = ((/* implicit */short) ((((/* implicit */_Bool) ((((/* implicit */_Bool) var_7)) ? (min((var_15), (((/* implicit */unsigned int) var_11)))) : (((/* implicit */unsigned int) ((/* implicit */int) (short)17958)))))) ? ((-(((((/* implicit */_Bool) var_12)) ? (((/* implicit */int) (unsigned short)16309)) : (var_9))))) : (var_9)));
}
