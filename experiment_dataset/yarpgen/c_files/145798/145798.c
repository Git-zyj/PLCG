/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145798
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145798 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145798
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        arr_4 [i_0] = ((~((-(arr_1 [i_0])))));
        arr_5 [i_0] [i_0] = ((var_12 ? (((var_15 <= ((max((arr_3 [i_0]), (arr_3 [i_0]))))))) : (arr_2 [i_0])));
        arr_6 [i_0] [i_0] = ((arr_1 [i_0]) | (arr_1 [i_0]));
        var_16 = ((471341435 == (-127 - 1)));
    }
    var_17 = var_15;
    var_18 = var_1;

    for (int i_1 = 2; i_1 < 15;i_1 += 1)
    {
        var_19 = ((!(((~(((var_15 && (arr_8 [i_1] [i_1])))))))));
        var_20 = (((((-(arr_7 [i_1 - 1])))) ? (arr_10 [i_1 - 1] [i_1 - 2]) : (max((arr_10 [i_1 + 3] [i_1 + 1]), (arr_7 [i_1 - 2])))));
        var_21 = -var_4;
    }
    #pragma endscop
}
