/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 78043
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=78043 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/78043
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */unsigned short) min((var_9), (min((min((var_9), (((/* implicit */int) var_6)))), (((/* implicit */int) ((((/* implicit */unsigned long long int) ((/* implicit */int) var_10))) != (1776110468532829537ULL))))))));
    var_13 = ((/* implicit */unsigned char) var_4);
    /* LoopNest 2 */
    for (unsigned short i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        for (unsigned int i_1 = 0; i_1 < 14; i_1 += 4) 
        {
            {
                var_14 = ((/* implicit */unsigned int) ((short) -2140817332));
                var_15 = ((/* implicit */unsigned char) arr_4 [i_0] [i_1]);
                arr_5 [i_0] [i_1] = -2140817332;
            }
        } 
    } 
}
