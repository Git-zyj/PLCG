/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85272
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85272 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85272
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_11 ? 128 : ((2147483647 ? ((4611686014132420608 ? 65535 : 18446744073709551594)) : 0))));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 1;i_4 += 1)
                        {
                            {
                                var_16 = (~247);
                                var_17 ^= (max(((-(min(var_6, 576460752034988032)))), ((((min(72057591890444288, 0)) >= (arr_5 [i_0] [8] [i_2] [i_4]))))));
                                arr_13 [i_3] [i_4] = ((var_11 ? 126 : var_0));
                            }
                        }
                    }
                    arr_14 [i_2] = ((~((~((var_13 ? -112 : -11))))));
                }
            }
        }
    }
    #pragma endscop
}
