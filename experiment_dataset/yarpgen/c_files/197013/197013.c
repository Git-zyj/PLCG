/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 197013
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=197013 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/197013
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 ^= var_2;
    var_17 = (min(var_17, (((1852285650 ? var_5 : 1852285650)))));

    for (int i_0 = 1; i_0 < 16;i_0 += 1)
    {
        arr_2 [i_0] = (((((2442681653 ? 15050449650204524902 : 0))) ? var_6 : (((arr_0 [i_0 + 2]) ? (arr_1 [i_0 + 2]) : (arr_0 [i_0 + 2])))));
        var_18 = (max(var_18, ((min((arr_1 [i_0 - 1]), var_3)))));
    }
    #pragma endscop
}
