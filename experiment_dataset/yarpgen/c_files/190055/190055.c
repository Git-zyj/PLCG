/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 190055
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=190055 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/190055
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = 1;
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 9;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 12;i_2 += 1)
            {
                {
                    var_17 ^= ((((((2432504093819166086 || 2432504093819166088) - (arr_6 [i_0] [i_1 + 1] [i_0])))) >= (~4503599627370495)));
                    var_18 = (arr_7 [i_0] [i_0] [i_0]);
                }
            }
        }
    }
    var_19 = ((!(((((2 ? 16014239979890385530 : var_1))) && ((min(0, 2432504093819166088)))))));
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 18;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 18;i_4 += 1)
        {
            {
                var_20 = (max(var_20, ((max((~4936729585380305336), 11)))));
                var_21 = (arr_10 [i_4]);
            }
        }
    }
    #pragma endscop
}
