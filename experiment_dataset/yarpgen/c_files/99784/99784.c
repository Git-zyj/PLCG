/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 99784
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=99784 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/99784
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((var_13 ? ((var_12 ? 65 : (220 | var_6))) : var_7));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_1] [i_1] = -1756036148;
                var_16 = (max(255, ((-(((arr_3 [i_1]) | (arr_1 [i_0])))))));
                var_17 = (max(var_17, (((((14 ? (!1756036148) : 22)) | ((((arr_1 [i_1]) | (arr_3 [i_0])))))))));
            }
        }
    }
    var_18 = (max(var_18, 16563323869349748663));
    var_19 = (((~var_14) ^ (~var_7)));
    #pragma endscop
}
