/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 162039
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=162039 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/162039
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 2; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_10 = (min(var_10, ((max(-var_6, ((var_3 ? var_6 : var_4)))))));
                arr_6 [i_1] = ((arr_5 [i_1] [i_0 + 1] [i_1]) ? ((196831682 ? 1 : 1)) : ((~(arr_5 [i_0] [i_0 - 2] [i_0]))));
                var_11 = (min(var_11, ((min((max(var_6, var_9)), ((max(1, 1))))))));
            }
        }
    }
    var_12 |= (!var_5);
    #pragma endscop
}
