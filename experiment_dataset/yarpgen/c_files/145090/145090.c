/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 145090
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=145090 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/145090
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 0; i_0 < 13;i_0 += 1) /* same iter space */
    {
        arr_2 [i_0] = (~var_13);
        arr_3 [i_0] = (max((max((arr_0 [i_0]), (arr_1 [i_0]))), ((5 ? (arr_1 [i_0]) : (arr_1 [i_0])))));
        arr_4 [i_0] = 1283470432;
        arr_5 [i_0] [i_0] = ((~((((min(var_3, (arr_1 [i_0])))) ? var_0 : 11834827891605129725))));
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1) /* same iter space */
    {
        arr_8 [i_1] = var_9;

        /* vectorizable */
        for (int i_2 = 0; i_2 < 13;i_2 += 1)
        {
            var_17 = (max(var_17, (arr_6 [i_1])));
            arr_12 [i_2] = (((~var_12) ? var_9 : (~1169735485)));
        }
        var_18 = (arr_9 [i_1] [i_1]);
        arr_13 [i_1] [i_1] = (~var_14);
    }
    var_19 -= var_8;
    #pragma endscop
}
