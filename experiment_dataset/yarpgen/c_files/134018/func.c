/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 134018
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=134018 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/134018
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_12 = ((/* implicit */_Bool) min((var_12), (((/* implicit */_Bool) min((((/* implicit */signed char) (!(((/* implicit */_Bool) ((((/* implicit */int) (short)20471)) << (((/* implicit */int) (_Bool)0)))))))), (var_2))))));
    /* LoopSeq 1 */
    for (short i_0 = 0; i_0 < 24; i_0 += 4) 
    {
        var_13 = ((/* implicit */_Bool) max((var_13), (((/* implicit */_Bool) var_10))));
        arr_2 [i_0] = ((/* implicit */int) arr_0 [i_0]);
        arr_3 [(unsigned char)20] = ((/* implicit */short) (-(((/* implicit */int) arr_0 [i_0]))));
        arr_4 [i_0] [i_0] = ((/* implicit */short) (~(((((/* implicit */int) var_11)) ^ (((/* implicit */int) arr_1 [i_0]))))));
    }
}
