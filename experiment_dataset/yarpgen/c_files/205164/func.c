/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 205164
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=205164 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/205164
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = var_9;
    /* LoopSeq 1 */
    for (long long int i_0 = 1; i_0 < 11; i_0 += 1) 
    {
        var_15 = ((/* implicit */unsigned short) var_2);
        arr_2 [(_Bool)1] [i_0] = ((/* implicit */_Bool) min((((((/* implicit */int) arr_0 [i_0 + 2] [i_0])) >> (((((/* implicit */int) arr_0 [i_0 + 1] [i_0 + 1])) - (44864))))), (((/* implicit */int) max((arr_0 [i_0 + 2] [i_0 + 2]), (arr_0 [i_0] [i_0 - 1]))))));
    }
}
