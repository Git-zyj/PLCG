/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 119399
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=119399 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/119399
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 17;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 15;i_2 += 1)
            {
                {
                    arr_7 [i_2] = (((min((((var_3 ? -195291953 : 232709467))), (max(var_8, (arr_3 [i_2 + 1] [i_1] [i_0]))))) < ((-((var_3 ? var_12 : var_4))))));
                    arr_8 [i_0] [i_2] [i_2] [6] = (arr_0 [4] [i_1]);
                }
            }
        }
    }
    var_13 &= var_10;
    var_14 = (((((((min(var_3, var_0))) ? var_8 : -17068893724696455870))) ? (((-(var_4 || -1683240055)))) : -var_5));
    var_15 = (min(var_15, var_3));
    var_16 = 120;
    #pragma endscop
}
