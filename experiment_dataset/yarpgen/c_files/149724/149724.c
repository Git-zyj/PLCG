/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 149724
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=149724 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/149724
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            {
                var_14 ^= ((max(0, var_7)));
                arr_6 [i_0] = (((var_0 ? (arr_2 [i_0] [i_0]) : (arr_2 [i_0] [i_0]))) > ((min((arr_2 [i_0] [i_0]), var_1))));
            }
        }
    }
    var_15 = ((18 ? ((var_11 ? ((1 ? var_6 : 1)) : var_12)) : (((max(var_11, var_3))))));
    var_16 = var_5;
    var_17 = (max(var_17, ((max(((var_0 ? ((min(255, 7))) : 1)), var_0)))));
    #pragma endscop
}
