/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 237451
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=237451 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/237451
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 25;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 25;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 25;i_3 += 1)
                    {
                        {
                            arr_10 [6] [9] [i_1] [i_1] = 4121848089874855735;
                            arr_11 [i_0] [i_1] [i_2] [i_3] = ((var_11 == ((((var_14 % var_8) || var_10)))));
                        }
                    }
                }
                arr_12 [1] = (((((arr_6 [i_0] [i_0 - 1]) || -13)) ? -3 : ((var_11 ? var_7 : var_7))));
                /* LoopNest 2 */
                for (int i_4 = 2; i_4 < 22;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 25;i_5 += 1)
                    {
                        {
                            var_15 |= ((((max((arr_15 [i_0]), var_1)) < var_13)));
                            var_16 = (((!((!(arr_17 [i_0] [i_0] [24]))))));
                            var_17 ^= (-((~(!-15740))));
                            var_18 += (var_13 / var_14);
                        }
                    }
                }
            }
        }
    }
    var_19 = var_14;
    var_20 = (min(var_20, (((-(var_9 < 13))))));
    var_21 = (min(var_21, var_7));
    #pragma endscop
}
