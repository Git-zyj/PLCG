/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 238522
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=238522 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/238522
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min((((((var_11 ? var_10 : -32749)) | (!var_7)))), (((min(8603190429859086915, -95)) << -var_4))));
    var_15 = 16383;
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                var_16 = (max(var_16, (((((max((arr_0 [i_0 + 1]), (arr_0 [i_0 + 1])))) - (((-32749 + 2147483647) >> (((arr_2 [i_0 + 1]) - 50149158)))))))));
                var_17 = (arr_5 [i_1] [i_1]);
            }
        }
    }
    #pragma endscop
}
