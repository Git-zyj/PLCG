/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 227763
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=227763 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/227763
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopSeq 1 */
    for (unsigned int i_0 = 0; i_0 < 18; i_0 += 1) 
    {
        arr_3 [i_0] [i_0] = ((/* implicit */unsigned short) min((((int) var_10)), (((/* implicit */int) min(((unsigned char)255), ((unsigned char)247))))));
        arr_4 [i_0] = ((/* implicit */_Bool) (~(arr_2 [i_0])));
    }
    var_20 = ((/* implicit */signed char) (-((~(((/* implicit */int) ((((/* implicit */int) (_Bool)0)) != (var_17))))))));
}
