/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 247214
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=247214 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/247214
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (min(var_16, var_5));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 1; i_2 < 22;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        {
                            var_17 = (~var_0);
                            var_18 = ((-(((arr_4 [i_1 + 1] [i_0]) ? var_14 : (arr_4 [i_1 - 1] [i_0])))));
                            var_19 = ((-(((arr_9 [i_1] [1] [i_1 + 1] [i_0]) ? (arr_9 [i_2] [i_1 + 1] [i_1 + 1] [i_1 + 1]) : (arr_9 [13] [i_1] [i_1 + 1] [i_0])))));
                        }
                    }
                }
                arr_10 [i_0] [i_0] [i_0] = (((arr_8 [10] [i_0] [22] [i_0] [i_0]) ^ ((1 ? var_5 : 1))));
                var_20 ^= ((((((arr_2 [i_1 + 1] [i_1 - 1]) ? var_0 : var_10))) ? (min(1, var_11)) : (arr_0 [i_0])));
            }
        }
    }
    var_21 = var_9;
    #pragma endscop
}
