/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 164467
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=164467 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/164467
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 15;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 13;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 15;i_3 += 1)
                    {
                        var_12 = (~((~(arr_1 [i_1]))));
                        var_13 ^= (max(((max(var_9, var_5))), (max((-9223372036854775807 - 1), -82))));
                    }
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        var_14 = ((max((arr_5 [i_1] [i_2 + 2] [i_2 - 3] [i_1]), var_0)));
                        var_15 = (((!(arr_0 [i_2 + 2]))) ? (var_6 != var_5) : (((arr_3 [i_1]) ? var_9 : (arr_4 [i_1]))));
                    }
                    for (int i_5 = 0; i_5 < 15;i_5 += 1)
                    {
                        arr_15 [i_1] [i_1] [i_2 - 2] [13] = ((max((arr_11 [i_0] [i_2 + 1]), var_4)));
                        arr_16 [i_1] [i_2] [i_2 - 1] [i_1] [i_1] = ((((arr_12 [i_1]) ? (arr_0 [i_1]) : var_11)));
                        var_16 = (min(var_16, (((max((arr_0 [i_0]), var_10))))));
                    }
                    var_17 = (min(var_17, var_3));
                    var_18 = (min(var_18, var_7));
                    var_19 = (max(var_19, (arr_4 [4])));
                }
            }
        }
    }
    var_20 = var_1;

    for (int i_6 = 2; i_6 < 22;i_6 += 1) /* same iter space */
    {
        var_21 ^= (~var_10);
        var_22 = var_1;
        var_23 = (arr_19 [21] [16]);
    }
    for (int i_7 = 2; i_7 < 22;i_7 += 1) /* same iter space */
    {
        var_24 ^= (arr_19 [i_7 + 1] [i_7 + 1]);
        var_25 = (max(var_25, 255));
    }
    #pragma endscop
}
