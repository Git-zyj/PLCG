/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 156827
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=156827 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/156827
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (var_12 ^ var_17);

    /* vectorizable */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_19 = (((arr_1 [i_0] [11]) | (arr_2 [i_0])));
        arr_3 [i_0] = (((arr_0 [i_0] [i_0]) ? (arr_2 [i_0]) : (var_10 >> 1)));
        var_20 ^= ((~(arr_2 [i_0])));
        var_21 ^= (((((65535 || (arr_1 [8] [i_0])))) >> (-91 + 119)));
    }
    var_22 = min((5975618776631796784 + var_8), ((var_10 ? (var_6 - var_3) : (1 - -1194655603))));
    var_23 = ((!((min(var_10, var_3)))));
    #pragma endscop
}
