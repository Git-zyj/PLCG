/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 39963
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=39963 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/39963
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (-4492094793604139105 & 1);
    var_19 = (min(var_19, var_6));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 1; i_3 < 1;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 12;i_4 += 1)
                        {
                            {
                                var_20 = (arr_1 [i_0]);
                                arr_15 [i_0] [i_1] [i_1] [i_2] [i_0] [i_3] [i_4] = ((((((((59417 ? -1920320719961054126 : 6132))) * ((var_7 ? (arr_2 [8] [i_1]) : (arr_6 [i_0] [i_0] [i_0])))))) ? (arr_0 [10]) : var_16));
                            }
                        }
                    }
                }
                arr_16 [i_0] [i_0] = (((arr_13 [i_0] [i_0] [i_0] [i_0]) | (((-(arr_9 [i_0]))))));
            }
        }
    }
    #pragma endscop
}
