/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 246877
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=246877 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/246877
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((var_14 ? (((((-17 ? 0 : -17))) ? var_4 : 20)) : ((var_3 ? (var_9 > var_6) : (~var_7)))));
    /* LoopNest 2 */
    for (int i_0 = 1; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_6 [i_0] = (max(((var_3 ? ((min(var_6, 6))) : var_12)), (((!(arr_3 [i_0] [i_0 - 1]))))));
                var_17 = ((var_1 ? (127 | -26035861) : (-12344 & 7301338911824742693)));
                arr_7 [6] [i_1] [12] &= ((((arr_3 [4] [4]) || (arr_3 [6] [6]))));
            }
        }
    }
    #pragma endscop
}
