/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 117751
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=117751 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/117751
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 19;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 17;i_1 += 1)
        {
            {
                var_15 ^= 15150;
                var_16 = (-(-127 - 1));
                var_17 = 16365765194287015080;
                var_18 = (max(var_18, (((~(min((arr_1 [i_0 - 1]), -22)))))));
            }
        }
    }
    var_19 ^= (~7635705292268047162);
    var_20 = var_9;

    for (int i_2 = 1; i_2 < 10;i_2 += 1) /* same iter space */
    {
        var_21 -= 127;
        var_22 = (arr_7 [i_2]);
        var_23 = 0;
        arr_8 [i_2] = (min(-2294, (~-1611943768)));
    }
    for (int i_3 = 1; i_3 < 10;i_3 += 1) /* same iter space */
    {
        var_24 = (~((~(arr_0 [i_3 + 1]))));
        var_25 ^= ((!(!var_13)));
    }
    for (int i_4 = 0; i_4 < 1;i_4 += 1)
    {
        var_26 |= (arr_5 [i_4]);
        var_27 = -191039589;

        for (int i_5 = 2; i_5 < 9;i_5 += 1) /* same iter space */
        {
            var_28 = (max(var_28, (arr_9 [i_4] [i_5 - 2])));
            var_29 = 127;
        }
        for (int i_6 = 2; i_6 < 9;i_6 += 1) /* same iter space */
        {
            /* LoopNest 2 */
            for (int i_7 = 1; i_7 < 9;i_7 += 1)
            {
                for (int i_8 = 0; i_8 < 10;i_8 += 1)
                {
                    {
                        arr_25 [i_4] [i_4] = (~((~(arr_23 [i_4]))));
                        var_30 = (max(-0, (arr_5 [i_4])));
                    }
                }
            }
            var_31 = var_9;
            var_32 = ((~(max(760410357, -316120052))));
            /* LoopNest 2 */
            for (int i_9 = 4; i_9 < 8;i_9 += 1)
            {
                for (int i_10 = 0; i_10 < 10;i_10 += 1)
                {
                    {

                        /* vectorizable */
                        for (int i_11 = 0; i_11 < 10;i_11 += 1)
                        {
                            var_33 ^= -4670;
                            var_34 = 16365765194287015080;
                        }
                        arr_33 [i_4] [i_4] [8] [i_10] = 3358153455;
                        var_35 = (arr_17 [8]);
                    }
                }
            }
        }
        for (int i_12 = 2; i_12 < 9;i_12 += 1) /* same iter space */
        {
            var_36 = ((-(((arr_20 [i_4] [i_12 - 2] [i_4] [i_12]) ? (min(127, 31319979846436111)) : (((-(arr_21 [i_4]))))))));
            var_37 = ((~(~17556167292628243825)));
        }
    }
    #pragma endscop
}
