/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 118334
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=118334 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/118334
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_20 = -var_11;
    var_21 = ((~(((~var_1) ? (((var_2 ? var_8 : 2147483647))) : var_12))));
    var_22 = (((((var_11 + (!var_10)))) || 4490990334024068202));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    arr_7 [i_2] [i_1] [i_1] [i_0] = -var_16;
                    arr_8 [i_0] = (((arr_5 [i_0] [9] [9]) & (((min((arr_0 [i_1]), var_14)) + (((min((arr_4 [8] [8]), (arr_4 [i_0] [i_2])))))))));
                }
            }
        }
    }
    var_23 = (min((((!(((var_4 ? var_15 : var_4)))))), var_1));
    #pragma endscop
}
