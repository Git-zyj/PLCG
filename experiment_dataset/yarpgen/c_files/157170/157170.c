/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 157170
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=157170 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/157170
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 |= var_15;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 17;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                var_18 = (((((~(((var_9 > (arr_1 [i_0] [i_0]))))))) ? (((arr_1 [i_0] [i_1]) ? (((arr_2 [i_0]) ? 126 : (arr_3 [i_0] [i_1]))) : (((((arr_3 [4] [4]) == var_1)))))) : (min((arr_1 [i_1 + 1] [i_1 + 2]), (arr_3 [i_1 + 1] [i_1 + 1])))));
                arr_4 [i_0] [i_0] [i_0] = 1;
                var_19 = ((+(((arr_2 [i_0]) ? (!var_1) : (((arr_0 [i_0]) ? var_5 : (arr_2 [i_0])))))));
                arr_5 [2] [i_1] [i_1] |= ((!((((arr_1 [i_0] [i_0]) ? var_12 : var_8)))));
            }
        }
    }
    #pragma endscop
}
