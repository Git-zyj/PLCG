/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 26501
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=26501 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/26501
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = ((max((var_9 - var_9), (~-104))));
    var_17 = var_15;

    for (int i_0 = 0; i_0 < 24;i_0 += 1)
    {
        /* LoopNest 2 */
        for (int i_1 = 0; i_1 < 24;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    arr_6 [i_0] [i_1] [i_2] [i_2] = ((-((2257872328 ? 2037094968 : 21199))));
                    arr_7 [i_0] [12] [i_2] = -98;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 24;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 24;i_4 += 1)
                        {
                            {
                                arr_14 [i_0] [20] [i_0] [i_2] [i_4] = ((((~(21199 ^ 16))) * var_11));
                                var_18 = (arr_0 [i_2]);
                                var_19 = ((((!2037094967) ? (arr_12 [i_0] [i_1] [i_0] [i_3]) : (arr_9 [i_0] [i_0] [i_2] [19] [i_2]))));
                            }
                        }
                    }
                }
            }
        }
        arr_15 [i_0] = var_11;
    }
    #pragma endscop
}
