/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 187207
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=187207 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/187207
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 25;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 23;i_1 += 1)
        {
            {
                arr_5 [i_0] [i_0] [i_0] = (((((-((var_10 ? (arr_0 [i_0]) : (arr_1 [i_1])))))) ? (min((((arr_2 [i_0] [i_1 + 1]) * 6056)), ((2756798793396133439 ? 1 : 64)))) : 3181));

                for (int i_2 = 4; i_2 < 24;i_2 += 1)
                {
                    arr_8 [i_0] [i_2] [i_0] [i_2] = (arr_0 [i_1 - 1]);
                    var_12 = (arr_3 [i_0]);
                    arr_9 [i_2] = (min(((-13139 ? (arr_6 [i_1 + 2] [i_1 + 2]) : (arr_6 [i_1 + 1] [i_1 + 2]))), (((arr_7 [i_2]) ? var_7 : 113))));
                }
                /* vectorizable */
                for (int i_3 = 0; i_3 < 25;i_3 += 1)
                {
                    arr_13 [i_3] [i_1 + 1] [i_0] [i_0] = var_5;
                    var_13 = (min(var_13, var_8));
                    var_14 *= var_2;
                }
                var_15 *= 93;
            }
        }
    }
    var_16 = var_5;
    #pragma endscop
}
