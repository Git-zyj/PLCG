/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 94729
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=94729 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/94729
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    for (int i_0 = 2; i_0 < 11;i_0 += 1)
    {
        var_12 = var_2;
        var_13 = ((!(~12292500486461421039)));
    }
    /* vectorizable */
    for (int i_1 = 1; i_1 < 19;i_1 += 1)
    {
        arr_4 [i_1 + 1] [i_1] = (arr_2 [i_1] [i_1]);
        arr_5 [i_1] = 2147483647;
        arr_6 [i_1] = (((arr_3 [i_1 - 1]) != (arr_2 [i_1 - 1] [i_1])));
        var_14 = ((var_4 ? (arr_3 [i_1 + 1]) : (arr_3 [i_1 + 2])));
    }
    var_15 ^= (((var_7 ? var_2 : var_0)));
    #pragma endscop
}
