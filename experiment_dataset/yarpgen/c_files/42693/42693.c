/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 42693
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=42693 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/42693
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 0;i_1 += 1)
        {
            {
                var_14 = (min((((!(arr_2 [i_0])))), (min((min((arr_4 [1] [1]), var_0)), var_0))));
                var_15 = ((-(min(var_5, (arr_2 [i_0])))));
                var_16 = (((((((((arr_3 [i_1] [i_0]) - (arr_4 [i_0 - 1] [1])))) + (((arr_1 [i_0] [i_1]) ? var_5 : (arr_3 [1] [1])))))) ? -var_13 : ((min((((arr_5 [i_0]) && var_11)), (!var_8))))));
                var_17 = (min(var_17, var_9));
            }
        }
    }
    var_18 = var_0;
    #pragma endscop
}
