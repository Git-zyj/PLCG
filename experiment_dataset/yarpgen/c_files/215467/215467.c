/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 215467
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=215467 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/215467
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 18;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 18;i_2 += 1)
            {
                {

                    for (int i_3 = 0; i_3 < 1;i_3 += 1)
                    {
                        var_13 = (max(var_13, (((var_8 ? (~var_2) : 1)))));
                        var_14 = (((((var_4 ? 4 : var_9))) / 6224217795644500313));
                        var_15 = (((var_4 > var_3) ? ((((-16 ? 61888 : 127)))) : 507835870));
                    }
                    /* LoopNest 2 */
                    for (int i_4 = 0; i_4 < 1;i_4 += 1)
                    {
                        for (int i_5 = 2; i_5 < 15;i_5 += 1)
                        {
                            {
                                arr_16 [i_5] [i_0] = (((var_1 < 1633721640678950909) || (((max(2286108279, -15))))));
                                var_16 = (min(var_16, (((-((max(var_1, -29))))))));
                            }
                        }
                    }
                }
            }
        }
    }
    var_17 *= var_1;
    #pragma endscop
}
