/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 189816
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=189816 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/189816
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 4; i_1 < 10;i_1 += 1)
        {
            {
                arr_6 [i_1] &= ((arr_4 [i_0]) + ((((!((11 != (arr_3 [i_1]))))))));
                var_18 = (max(var_18, ((min((((var_17 ? var_8 : 63864))), ((max(((((arr_4 [i_1]) > (arr_3 [i_1])))), (arr_1 [i_1 + 1])))))))));
                arr_7 [i_0] = ((159 ? (min((((arr_1 [i_0]) | -2070550621)), (arr_5 [i_0] [i_1 - 2] [i_0]))) : (100 != 1)));
            }
        }
    }
    var_19 = ((var_13 < ((-(var_6 || var_17)))));
    #pragma endscop
}
