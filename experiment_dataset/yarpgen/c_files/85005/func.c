/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 85005
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=85005 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/85005
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 = var_4;
    /* LoopSeq 1 */
    for (unsigned char i_0 = 0; i_0 < 14; i_0 += 3) 
    {
        var_15 -= ((/* implicit */_Bool) ((signed char) min(((~(((/* implicit */int) arr_0 [i_0] [i_0])))), ((-(((/* implicit */int) (unsigned char)0)))))));
        arr_3 [i_0] = ((/* implicit */long long int) arr_0 [i_0] [i_0]);
    }
    var_16 = ((/* implicit */short) var_7);
    var_17 = ((/* implicit */unsigned char) max((var_17), (((/* implicit */unsigned char) (((~((~(var_10))))) | (((/* implicit */unsigned long long int) ((/* implicit */int) var_5))))))));
}
