/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 61097
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=61097 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/61097
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 14;i_2 += 1)
            {
                {
                    var_13 = 3236446087639893055;
                    arr_9 [i_0] [12] [i_1] [i_2] = (((((((((arr_0 [i_0] [11]) ? (arr_1 [i_0]) : var_0))) ? (((arr_0 [12] [i_1]) % (arr_2 [i_0] [1]))) : (arr_3 [i_0] [7] [12])))) <= -var_4));
                    arr_10 [i_0] = (arr_5 [1] [i_0]);
                }
            }
        }
    }
    var_14 = ((var_0 | (max(var_7, var_3))));
    var_15 |= var_5;
    #pragma endscop
}
