/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 14631
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=14631 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/14631
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_17 = (max(var_17, (max((((var_7 ? var_9 : ((min(var_2, var_14)))))), ((var_2 ? 3026306715 : 12))))));
    var_18 = var_14;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                arr_7 [i_0] = arr_3 [i_0] [i_0];
                /* LoopNest 3 */
                for (int i_2 = 0; i_2 < 23;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 23;i_3 += 1)
                    {
                        for (int i_4 = 0; i_4 < 23;i_4 += 1)
                        {
                            {
                                var_19 = (arr_13 [i_0] [i_1] [0] [i_3] [13] [13]);
                                arr_16 [i_0] [i_0] [i_0] [i_0] [i_0] [i_0] &= ((var_9 ? (arr_5 [i_0] [i_0] [i_0]) : ((((max(var_8, (arr_0 [i_0])))) ? var_6 : (((-102 || (-2147483647 - 1))))))));
                                var_20 = var_7;
                            }
                        }
                    }
                }
            }
        }
    }
    #pragma endscop
}
