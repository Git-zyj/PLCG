/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 46137
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=46137 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/46137
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = ((var_8 ? var_6 : var_13));
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 14;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    var_15 = var_1;

                    for (int i_3 = 1; i_3 < 14;i_3 += 1)
                    {
                        var_16 *= ((~(((max(-59, 16))))));
                        var_17 |= ((+((max((arr_6 [i_3 + 1]), 18)))));
                        var_18 += (((arr_8 [i_0] [i_3 - 1] [i_2] [i_0 + 2]) ? (arr_9 [i_3] [12] [i_0] [i_0]) : (39 <= 93)));
                        arr_11 [i_0 - 2] [12] [i_0] [i_3 - 1] = ((+((min((arr_8 [i_0] [9] [i_3 + 2] [i_0 + 1]), (arr_5 [i_3 - 1]))))));
                    }
                    var_19 = (((((~(~127)))) || (arr_5 [i_0 - 2])));
                }
            }
        }
    }
    #pragma endscop
}
