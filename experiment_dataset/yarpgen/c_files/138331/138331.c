/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 138331
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=138331 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/138331
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 15;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_0] = ((((!(arr_2 [14] [i_1 + 3] [15]))) ? ((min((arr_2 [i_0] [i_1 + 3] [3]), (arr_2 [i_0] [i_1 + 3] [3])))) : ((((-127 - 1) == (arr_2 [i_0] [i_1 + 3] [1]))))));
                arr_5 [i_0] [i_0] [i_0] &= (((arr_2 [i_1] [0] [i_0]) * (max((arr_0 [i_1 + 1] [i_1 - 1]), (arr_0 [i_1 + 1] [i_1 + 1])))));
            }
        }
    }
    var_14 = var_7;
    #pragma endscop
}
