/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127184
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127184 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127184
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (short i_0 = 0; i_0 < 12; i_0 += 2) 
    {
        for (signed char i_1 = 0; i_1 < 12; i_1 += 1) 
        {
            {
                var_20 = ((unsigned long long int) ((signed char) arr_3 [i_0] [i_0] [(signed char)1]));
                var_21 -= ((/* implicit */long long int) (!(((/* implicit */_Bool) var_7))));
            }
        } 
    } 
    var_22 = ((/* implicit */unsigned int) min((var_22), (((/* implicit */unsigned int) (~(((/* implicit */int) var_6)))))));
    var_23 = ((/* implicit */int) min((var_23), (((/* implicit */int) ((long long int) (-(((((/* implicit */unsigned long long int) 144115187002114048LL)) - (13253387267304576919ULL)))))))));
    /* LoopSeq 1 */
    /* vectorizable */
    for (long long int i_2 = 1; i_2 < 9; i_2 += 1) 
    {
        var_24 = ((/* implicit */unsigned short) max((var_24), (((/* implicit */unsigned short) arr_3 [i_2 + 1] [i_2 + 1] [i_2]))));
        var_25 = ((/* implicit */unsigned int) min((var_25), (((/* implicit */unsigned int) (-(((/* implicit */int) ((unsigned short) var_5))))))));
    }
}
