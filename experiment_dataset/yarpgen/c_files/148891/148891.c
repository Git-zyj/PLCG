/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 148891
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=148891 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/148891
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 2; i_0 < 9;i_0 += 1)
    {
        arr_3 [i_0] = (((arr_0 [i_0]) | 126));
        arr_4 [i_0] [i_0 + 2] = (-var_4 / var_10);
        var_13 ^= (arr_2 [i_0 - 2] [i_0 - 1]);
        var_14 ^= (var_0 && -121);
        arr_5 [i_0] = ((-23653 ? (65535 && 16348491701688507617) : var_7));
    }
    for (int i_1 = 0; i_1 < 13;i_1 += 1)
    {
        arr_8 [i_1] = var_12;
        var_15 = (~var_4);
    }
    for (int i_2 = 0; i_2 < 20;i_2 += 1)
    {
        arr_12 [17] [i_2] = ((-36 ? ((var_8 ? ((min(1709, (arr_11 [i_2])))) : 151)) : -11667));
        var_16 = (arr_9 [i_2] [i_2]);
    }
    for (int i_3 = 0; i_3 < 14;i_3 += 1)
    {
        arr_16 [i_3] = (((((var_12 && ((((arr_14 [i_3]) ? var_6 : (arr_13 [i_3]))))))) >> (var_4 - 7313233011338552476)));
        var_17 ^= (-31945 ^ 18755);
    }
    var_18 = min(65535, var_4);
    var_19 = var_3;
    var_20 ^= ((-3 >= 97) / var_6);
    var_21 = var_5;
    #pragma endscop
}
