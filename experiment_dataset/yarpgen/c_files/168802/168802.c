/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 168802
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=168802 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/168802
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, var_7));
    var_13 ^= var_6;
    var_14 = (((((var_8 << (var_2 - 1306069674)))) ? var_0 : ((((var_2 ? var_0 : var_1)) + var_5))));

    for (int i_0 = 2; i_0 < 21;i_0 += 1)
    {
        arr_3 [i_0 - 2] = (min(299402299102062232, ((((arr_0 [i_0 - 2] [i_0 - 2]) && (arr_0 [i_0 + 1] [i_0 - 1]))))));
        var_15 = (min(var_15, (((~(arr_2 [i_0]))))));
        var_16 ^= (arr_2 [i_0 - 1]);
        var_17 = (max(var_17, ((((max(var_11, var_4) / (-299402299102062233 - -6395134333116217063)))))));
    }
    #pragma endscop
}
