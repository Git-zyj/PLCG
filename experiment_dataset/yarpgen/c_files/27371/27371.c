/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 27371
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=27371 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/27371
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 1;i_2 += 1)
            {
                {
                    var_18 = (min(((0 ? 15831875360404551709 : 0)), (arr_8 [i_0] [6] [1] [i_2])));
                    var_19 = (((((7223592267961553779 ? 2082903568326924439 : 1))) ? var_14 : ((1 ? -1202 : (arr_7 [i_0])))));
                }
            }
        }
    }
    var_20 *= var_2;
    var_21 = ((((((((255613842344270842 ? 0 : 7168))) | var_2))) ? 29494 : var_7));
    #pragma endscop
}
