/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68727
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68727 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68727
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 -= ((var_14 ? var_13 : (237 <= 1)));
    var_16 = (min(var_16, var_7));
    /* LoopNest 3 */
    for (int i_0 = 1; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 20;i_2 += 1)
            {
                {
                    arr_8 [i_0] [1] [i_1] [i_2] = (max(((max((arr_3 [2]), 1))), ((((((arr_1 [i_0] [i_1]) ? (arr_2 [i_2]) : 26))) ? (max((-2147483647 - 1), -27)) : ((94 ? (arr_4 [i_1]) : 54920))))));
                    arr_9 [i_2] [i_1] [i_0] = ((var_4 << (255 - 247)));
                }
            }
        }
    }
    var_17 -= (189 << var_8);
    var_18 = 70368744177663;
    #pragma endscop
}
