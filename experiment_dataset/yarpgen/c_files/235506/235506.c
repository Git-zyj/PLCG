/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 235506
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=235506 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/235506
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 11;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 11;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 11;i_2 += 1)
            {
                {
                    var_12 ^= var_3;
                    arr_8 [4] [i_1] [10] &= (arr_4 [i_1] [i_0]);
                    arr_9 [i_0] [i_1] [i_0] [3] = ((!((((arr_7 [i_2] [i_0] [i_0]) ? ((var_10 % (arr_1 [i_0]))) : var_2)))));
                }
            }
        }
    }
    var_13 = var_4;
    var_14 *= var_7;
    /* LoopNest 3 */
    for (int i_3 = 0; i_3 < 15;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 15;i_4 += 1)
        {
            for (int i_5 = 0; i_5 < 15;i_5 += 1)
            {
                {
                    arr_19 [i_4] [i_4] [0] [i_3] = var_2;
                    var_15 = (max(var_15, ((max(var_2, -32762)))));
                    arr_20 [i_3] = (arr_15 [12] [i_4] [i_3]);
                }
            }
        }
    }
    var_16 |= ((var_8 ? ((~(max(var_11, var_11)))) : ((-((var_6 ? var_2 : 8))))));
    #pragma endscop
}
