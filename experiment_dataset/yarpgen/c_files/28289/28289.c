/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 28289
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=28289 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/28289
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 &= var_14;
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 18;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 21;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 21;i_3 += 1)
                    {
                        {
                            var_16 = var_10;
                            var_17 = (~var_5);
                        }
                    }
                }
                /* LoopNest 2 */
                for (int i_4 = 1; i_4 < 18;i_4 += 1)
                {
                    for (int i_5 = 0; i_5 < 21;i_5 += 1)
                    {
                        {
                            arr_16 [i_0] [i_1] [i_4 - 1] [i_5] &= (var_8 * var_4);
                            var_18 = (min((!var_7), (((arr_11 [i_1 - 1] [i_4 + 1] [i_4] [i_4 + 1]) ? (arr_11 [i_1 - 1] [i_4 - 1] [i_4] [i_4 - 1]) : var_10))));
                        }
                    }
                }
                var_19 = (max(var_19, (((~((((var_4 ? -517778429500261744 : var_13)) ^ ((((arr_15 [i_1 + 1]) ? (arr_7 [i_0]) : 103))))))))));
                arr_17 [i_0] [i_0] [i_0] = var_7;
            }
        }
    }
    #pragma endscop
}
