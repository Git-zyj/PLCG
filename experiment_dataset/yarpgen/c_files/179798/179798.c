/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 179798
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=179798 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/179798
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_15 = ((((((min(34865, var_6))) ? (((var_3 ? var_13 : var_2))) : (min(1118892122, var_7)))) > var_13));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [i_1] [i_0] [i_0] = (min(((((!var_3) <= (arr_3 [11] [i_1] [i_0])))), (arr_3 [i_0] [i_0] [i_0])));
                arr_6 [6] [i_1] [i_1] = (((max((min(var_14, (arr_1 [i_1])), (!65))))));
            }
        }
    }
    #pragma endscop
}
