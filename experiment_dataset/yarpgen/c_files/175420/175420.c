/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 175420
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=175420 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/175420
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_14 = (min(24188, 18446744073709551615));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 1;i_1 += 1)
        {
            {
                var_15 = 9223372036854775805;
                var_16 = (((arr_3 [i_0] [i_1 - 1] [i_1]) ? (((1 ? ((9223372036854775802 ? 3625628024 : 1)) : (((((arr_1 [i_0] [i_1]) < (arr_0 [i_0])))))))) : (((-9223372036854775806 + 9223372036854775807) >> (((1 + 32749) - 32717))))));
                var_17 = (!247996824);
            }
        }
    }
    var_18 = (max(var_9, (((var_11 < 9223372036854775805) ^ -var_4))));
    #pragma endscop
}
