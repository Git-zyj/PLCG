/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 194540
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=194540 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/194540
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            {
                var_14 -= (~var_1);
                arr_7 [i_0] = ((arr_4 [i_0] [i_1] [i_1]) ? ((((max((arr_0 [i_1] [i_0]), (arr_0 [17] [i_0])))) | var_8)) : ((((((arr_1 [i_0]) ^ (arr_0 [i_1] [i_0])))) ? (arr_6 [i_0] [i_1]) : 60330)));
            }
        }
    }
    var_15 = var_2;
    var_16 = ((((!(((var_13 ? var_10 : var_8))))) ? (((((1 ? 5206 : 15075))) ? var_1 : var_1)) : ((((min(var_13, var_13))) - var_4))));
    #pragma endscop
}
