/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11944
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11944 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11944
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 -= var_5;
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    var_13 = 1;
                    var_14 = (((!(((-1016686019752089804 ? var_4 : (arr_3 [i_2] [i_1])))))));
                    arr_6 [i_0] [i_1] [0] [i_2] |= ((-112 && 7214) ? (((!(!58316)))) : (((7198 && (arr_5 [i_0])))));
                }
            }
        }
    }
    var_15 &= ((1440951479 ? (((!1440951492) ? -1016686019752089816 : 1)) : ((((!(!65528)))))));
    #pragma endscop
}
