/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 199317
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=199317 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/199317
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_17;
    var_20 &= (var_0 % var_4);
    /* LoopNest 3 */
    for (int i_0 = 3; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 10;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 13;i_2 += 1)
            {
                {
                    arr_8 [i_0] = var_18;
                    /* LoopNest 2 */
                    for (int i_3 = 0; i_3 < 13;i_3 += 1)
                    {
                        for (int i_4 = 1; i_4 < 9;i_4 += 1)
                        {
                            {
                                var_21 += ((((arr_5 [i_0 - 3]) ^ var_13)));
                                var_22 |= (((((arr_13 [i_2] [i_4 + 4] [i_3] [i_0 - 1] [i_3]) + var_14)) >= ((~(arr_2 [i_0 - 2] [i_0 - 1] [i_4 - 1])))));
                            }
                        }
                    }
                }
            }
        }
    }
    /* LoopNest 3 */
    for (int i_5 = 0; i_5 < 22;i_5 += 1)
    {
        for (int i_6 = 1; i_6 < 1;i_6 += 1)
        {
            for (int i_7 = 0; i_7 < 22;i_7 += 1)
            {
                {
                    var_23 &= (min(-9015444478685527602, -9015444478685527608));
                    arr_25 [i_6] = ((~(min(var_7, (arr_15 [i_6 - 1] [i_6 - 1])))));
                }
            }
        }
    }
    #pragma endscop
}
