/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 127235
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=127235 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/127235
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = var_9;
    var_20 = (min(var_20, (((!((min(var_7, var_13))))))));

    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        var_21 += (((min((arr_0 [i_0]), (arr_0 [i_0]))) / 14));
        arr_2 [1] &= ((((min((min((arr_0 [i_0]), 18446744073709551615)), ((min(var_9, var_12)))))) ? (((((arr_1 [i_0] [i_0]) ? -6141 : -14893)))) : (((arr_1 [i_0] [i_0]) ? ((-(arr_1 [i_0] [i_0]))) : ((-(arr_1 [i_0] [i_0])))))));
    }
    #pragma endscop
}
