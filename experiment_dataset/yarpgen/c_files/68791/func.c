/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 68791
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=68791 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/68791
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_15 = (-(((/* implicit */int) (!(((/* implicit */_Bool) ((281474976710655ULL) - (18446462598732840928ULL))))))));
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 15; i_0 += 2) 
    {
        for (unsigned char i_1 = 0; i_1 < 15; i_1 += 1) 
        {
            for (short i_2 = 3; i_2 < 13; i_2 += 1) 
            {
                {
                    var_16 = ((/* implicit */short) max((var_16), (((/* implicit */short) var_5))));
                    var_17 *= ((/* implicit */unsigned char) max(((-((-(281474976710656ULL))))), (((/* implicit */unsigned long long int) max((((((/* implicit */int) (short)7296)) / (((/* implicit */int) (short)-24033)))), (((/* implicit */int) (short)7296)))))));
                }
            } 
        } 
    } 
}
