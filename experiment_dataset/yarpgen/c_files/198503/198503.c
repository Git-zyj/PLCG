/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 198503
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=198503 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/198503
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_13 = ((((var_8 <= ((448 ? 65088 : 1099573695)))) ? (((((var_5 ? var_12 : var_11))) ? 2869109233749012271 : (var_2 <= var_6))) : 3977511490807008069));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 15;i_0 += 1)
    {
        var_14 = (min(var_14, ((((arr_0 [i_0] [i_0]) ? (arr_0 [i_0] [i_0]) : (arr_1 [i_0]))))));
        var_15 = (((arr_0 [i_0] [i_0]) * (arr_0 [i_0] [i_0])));
        arr_2 [i_0] = var_12;
    }
    #pragma endscop
}
