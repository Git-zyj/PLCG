/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 23525
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=23525 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/23525
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 3; i_0 < 18;i_0 += 1)
    {
        var_19 = ((!(arr_1 [7] [i_0])));
        var_20 = var_8;
        var_21 = ((~(arr_1 [i_0 + 1] [i_0 - 1])));
    }
    for (int i_1 = 2; i_1 < 10;i_1 += 1)
    {
        var_22 = ((!(((arr_0 [i_1] [i_1 + 2]) != (arr_0 [i_1] [i_1 + 2])))));
        var_23 = max(((((arr_0 [i_1] [i_1]) ? (arr_3 [i_1]) : (arr_2 [i_1] [i_1])))), (min((arr_0 [i_1 + 2] [i_1 - 2]), 1500737693)));
        arr_4 [i_1] = var_18;
    }
    for (int i_2 = 0; i_2 < 1;i_2 += 1)
    {
        arr_7 [i_2] = (arr_3 [4]);
        var_24 = (max(var_24, (((var_3 ? (((((arr_6 [7]) / var_1)) / -var_12)) : (arr_6 [i_2]))))));
    }

    for (int i_3 = 0; i_3 < 23;i_3 += 1)
    {
        var_25 = ((~(((arr_9 [i_3]) & 1))));
        var_26 = var_6;
    }
    #pragma endscop
}
