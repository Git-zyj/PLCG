/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 184521
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=184521 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/184521
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 1;i_1 += 1)
        {
            {
                var_14 = (arr_6 [i_0] [i_0]);
                var_15 = (max((arr_6 [i_0] [i_0]), (arr_1 [i_0])));
                var_16 = (((((-((-(arr_0 [i_0])))))) == (((((var_4 ? 4294967272 : (arr_5 [i_0])))) ? 4294967295 : (~var_10)))));
                arr_7 [i_0] [i_0] [i_0] |= (((arr_4 [i_0] [i_0]) != (arr_6 [i_0] [i_0])));
            }
        }
    }
    var_17 = ((!var_7) ? var_1 : (((~(!4294967295)))));
    #pragma endscop
}
