/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 97297
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=97297 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/97297
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 = (min(var_18, var_0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 2; i_1 < 20;i_1 += 1)
        {
            {
                var_19 = (min(var_19, var_4));
                var_20 = ((-(((arr_1 [i_0]) ^ (1152921504606322688 ^ var_1)))));
            }
        }
    }
    var_21 = (((var_0 / 1152921504606322697) ? ((((0 ? 1152921504606322725 : -616138347)) / 1152921504606322712)) : (((var_10 / 1915865840101806538) / 182))));
    #pragma endscop
}
