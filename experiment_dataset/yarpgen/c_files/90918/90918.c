/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 90918
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=90918 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/90918
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 13;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 13;i_1 += 1)
        {
            {
                var_18 ^= (((max(var_17, (arr_1 [i_0])))));
                var_19 = var_4;
                arr_4 [i_0] [12] = (arr_3 [i_0] [i_0]);
                var_20 = (min((max(var_5, (arr_1 [i_0]))), (arr_1 [i_1])));
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 13;i_2 += 1)
                {
                    for (int i_3 = 2; i_3 < 10;i_3 += 1)
                    {
                        {
                            arr_11 [i_0] [i_1] [11] [i_3] [i_0] = var_10;
                            arr_12 [i_0] [i_1] [i_0] [i_3 + 1] [5] [i_3 + 2] &= (((arr_5 [0] [0]) || ((max(((~(arr_2 [i_0]))), var_3)))));
                        }
                    }
                }
            }
        }
    }
    var_21 = (max((var_15 & var_15), var_14));
    #pragma endscop
}
