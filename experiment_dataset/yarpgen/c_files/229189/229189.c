/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229189
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229189 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229189
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_19 = ((25167 ? 1 : -1));
    var_20 = (845508270412230904 == (((-(var_18 | var_8)))));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 1;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 19;i_1 += 1)
        {
            {
                arr_6 [i_1] &= ((~(arr_0 [i_1] [i_0])));
                arr_7 [i_1] = ((((max((arr_2 [i_0] [i_1]), (arr_2 [i_0] [i_1])))) == ((((((((arr_4 [i_0] [i_0] [i_0]) + 2147483647)) >> (9338 - 9311)))) ? ((min((arr_2 [i_0] [i_1]), 1))) : (arr_4 [i_0] [i_0] [i_0])))));
            }
        }
    }
    var_21 = 121;
    var_22 = ((!(((min(1, -1623697252)) >= (var_16 <= var_14)))));
    #pragma endscop
}
