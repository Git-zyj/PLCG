/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 64979
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=64979 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/64979
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_11 ^= ((/* implicit */long long int) max((((/* implicit */unsigned long long int) (unsigned short)27753)), (13240638147884908648ULL)));
    /* LoopSeq 1 */
    for (_Bool i_0 = 0; i_0 < 1; i_0 += 1) 
    {
        var_12 *= ((/* implicit */long long int) (-(((/* implicit */int) var_2))));
        var_13 = ((((/* implicit */_Bool) -2036456037)) ? (-2080996507) : (((/* implicit */int) (unsigned short)63508)));
    }
    var_14 = ((/* implicit */long long int) var_1);
}
