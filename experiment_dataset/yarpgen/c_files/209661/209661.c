/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 209661
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=209661 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/209661
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_16 = (max(var_16, var_10));

    /* vectorizable */
    for (int i_0 = 0; i_0 < 21;i_0 += 1)
    {
        var_17 = (!-0);
        var_18 = ((252 ? ((1611142619 ? (arr_1 [i_0]) : var_3)) : var_5));
        /* LoopNest 3 */
        for (int i_1 = 0; i_1 < 21;i_1 += 1)
        {
            for (int i_2 = 1; i_2 < 19;i_2 += 1)
            {
                for (int i_3 = 2; i_3 < 17;i_3 += 1)
                {
                    {
                        var_19 = ((((!(arr_6 [i_0] [i_1]))) ? (!8275) : (~9814889420804150456)));
                        var_20 = (((arr_1 [i_1]) && (((~(arr_10 [i_0] [i_0] [i_0] [i_0] [i_0]))))));
                        var_21 = 7;
                    }
                }
            }
        }
        var_22 = (!-644241488);
    }
    for (int i_4 = 1; i_4 < 18;i_4 += 1)
    {
        var_23 -= (arr_12 [i_4] [i_4]);
        arr_14 [i_4] [i_4] = var_12;
    }
    for (int i_5 = 0; i_5 < 23;i_5 += 1)
    {
        var_24 = (arr_15 [22]);
        var_25 = var_10;
        var_26 = arr_15 [i_5];
    }
    var_27 = (min(var_27, 4294967295));
    #pragma endscop
}
