/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 11758
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=11758 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/11758
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 12;i_1 += 1)
        {
            {
                arr_5 [8] [i_1] = (~0);
                arr_6 [i_1] = (min(251, 251));

                for (int i_2 = 0; i_2 < 12;i_2 += 1)
                {
                    var_10 |= ((arr_7 [i_0]) ^ ((-(arr_0 [i_2]))));
                    var_11 = ((+((min((arr_1 [i_1] [i_2]), (arr_4 [i_0] [i_0]))))));
                    var_12 = ((((max((((arr_3 [9] [i_1] [i_2]) >> (var_2 + 3791861678694309363))), 0))) ^ ((~(((arr_0 [i_0]) % (arr_0 [i_0])))))));
                }
                var_13 = (max(21, ((-(arr_3 [i_0] [i_0] [i_0])))));
            }
        }
    }
    /* LoopNest 2 */
    for (int i_3 = 0; i_3 < 13;i_3 += 1)
    {
        for (int i_4 = 0; i_4 < 13;i_4 += 1)
        {
            {
                var_14 -= (var_9 << 0);
                var_15 += (((+-31152) + ((-((var_5 ? (arr_13 [i_4]) : (arr_12 [i_4])))))));
            }
        }
    }
    #pragma endscop
}
