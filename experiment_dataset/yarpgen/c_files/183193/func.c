/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 183193
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=183193 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/183193
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 3 */
    for (unsigned int i_0 = 3; i_0 < 6; i_0 += 2) 
    {
        for (unsigned short i_1 = 1; i_1 < 7; i_1 += 2) 
        {
            for (int i_2 = 0; i_2 < 10; i_2 += 1) 
            {
                {
                    var_13 = ((/* implicit */_Bool) (-(((unsigned long long int) min((((/* implicit */signed char) (_Bool)1)), ((signed char)126))))));
                    arr_9 [i_1] [i_1] [i_2] [i_1] = ((/* implicit */unsigned long long int) (-(((/* implicit */int) (!(((/* implicit */_Bool) ((((/* implicit */_Bool) arr_2 [i_0] [i_2])) ? (((/* implicit */int) var_0)) : (939990546)))))))));
                }
            } 
        } 
    } 
    var_14 = ((/* implicit */unsigned long long int) var_6);
}
