/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 77364
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=77364 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/77364
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_17 = ((/* implicit */unsigned int) var_6);
    /* LoopNest 3 */
    for (unsigned char i_0 = 0; i_0 < 13; i_0 += 4) 
    {
        for (unsigned short i_1 = 0; i_1 < 13; i_1 += 1) 
        {
            for (unsigned long long int i_2 = 2; i_2 < 10; i_2 += 3) 
            {
                {
                    arr_9 [2] [i_2 - 2] [i_1] [7] = ((/* implicit */_Bool) min((((((/* implicit */long long int) ((/* implicit */int) (short)-9568))) + (-280581378595863989LL))), (min((max((-280581378595863989LL), (((/* implicit */long long int) arr_8 [i_0] [i_0] [i_0])))), (arr_3 [i_0] [i_1] [i_2])))));
                    arr_10 [3LL] [i_1] [(_Bool)1] = ((/* implicit */long long int) arr_8 [i_0] [(unsigned char)7] [i_2]);
                }
            } 
        } 
    } 
}
