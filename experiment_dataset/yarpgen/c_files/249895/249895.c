/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249895
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249895 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249895
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 20;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 20;i_1 += 1)
        {
            {
                var_12 = (((var_0 + 2147483647) << ((((min((arr_3 [i_0]), 1678449328466856525)) != ((((!(arr_3 [i_1]))))))))));
                var_13 = (((~(arr_1 [i_0] [i_1]))) | (arr_1 [i_0] [i_0]));
                var_14 -= ((-var_2 ? ((((arr_2 [8]) || (arr_2 [10])))) : (151 != 558551906910208)));

                /* vectorizable */
                for (int i_2 = 0; i_2 < 20;i_2 += 1)
                {
                    var_15 = 7201045066812963411;
                    var_16 = (min(var_16, var_7));
                }
            }
        }
    }
    var_17 = (((var_2 ? ((var_11 ? var_3 : var_10)) : 767190167)));
    #pragma endscop
}
