/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 229858
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=229858 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/229858
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 16; i_0 += 2) 
    {
        for (unsigned char i_1 = 4; i_1 < 15; i_1 += 3) 
        {
            {
                var_19 = ((/* implicit */short) max((arr_0 [i_0]), ((+(max((((/* implicit */unsigned int) -64)), (var_6)))))));
                var_20 = ((/* implicit */unsigned long long int) (-(min((arr_3 [i_1 - 1] [i_1 + 1] [i_1 - 1]), (arr_3 [i_1 - 4] [i_1 - 1] [i_1 - 2])))));
            }
        } 
    } 
    var_21 = ((/* implicit */int) ((((unsigned long long int) ((_Bool) var_8))) - (((/* implicit */unsigned long long int) ((((/* implicit */unsigned int) ((/* implicit */int) (short)-27994))) % (var_3))))));
    var_22 = var_6;
    var_23 = ((/* implicit */unsigned long long int) var_9);
}
