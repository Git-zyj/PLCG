/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 186372
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=186372 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/186372
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (min(var_12, var_1));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 22;i_2 += 1)
            {
                {
                    arr_7 [i_0] [i_0] = -1828520947;

                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        var_13 = var_6;
                        arr_11 [i_0] [i_2] [i_1] [i_0] = (max((var_2 / 181288563), (((-562283285 ? -32744 : -32736)))));
                        var_14 = (((var_11 - var_5) / 2130706432));
                    }
                    /* vectorizable */
                    for (int i_4 = 0; i_4 < 22;i_4 += 1)
                    {
                        arr_15 [i_0] [i_0] [i_0] [i_0] = -var_9;
                        arr_16 [i_0] [i_1] [i_0] = (((((var_2 ? var_5 : var_10))) ? -1 : var_0));

                        for (int i_5 = 1; i_5 < 21;i_5 += 1)
                        {
                            var_15 += (!2);
                            var_16 += ((var_9 ? var_6 : var_4));
                            var_17 = (min(var_17, var_10));
                            var_18 += (~var_0);
                            var_19 += -var_4;
                        }
                        for (int i_6 = 0; i_6 < 22;i_6 += 1)
                        {
                            var_20 = -var_8;
                            arr_21 [i_0] [i_1] [i_0] [1] [7] = (!1322603974);
                            var_21 -= -2400051163;
                            var_22 *= var_5;
                            var_23 += var_6;
                        }
                    }
                }
            }
        }
    }
    var_24 = (((!(!var_10))));
    #pragma endscop
}
