/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 130685
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=130685 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/130685
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 3 */
    for (int i_0 = 2; i_0 < 12;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 14;i_1 += 1)
        {
            for (int i_2 = 4; i_2 < 12;i_2 += 1)
            {
                {
                    arr_10 [i_0] [i_1] [i_1] [i_2] = ((((arr_0 [i_0 + 1]) > (arr_2 [i_0 - 1]))));
                    arr_11 [i_2 - 2] [i_1] [i_0] [i_0] = ((((var_3 ^ (arr_4 [i_0 + 2] [i_2 + 1]))) << (((arr_4 [i_0 + 2] [i_2 + 1]) - 577120160))));
                    arr_12 [i_1] [i_2] [i_1] [i_0] = 32767;
                    var_12 = 19;
                    arr_13 [i_2] = ((~((1 - (arr_2 [i_0 + 1])))));
                }
            }
        }
    }
    var_13 = (min(var_13, var_1));
    var_14 = 237;
    var_15 = ((-(((var_8 ? var_1 : var_1)))));
    #pragma endscop
}
