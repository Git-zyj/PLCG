/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 249183
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=249183 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/249183
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = ((-4564364411931827963 ? 15910674140561595117 : 18446744073709551595));
    var_13 = ((-16 ? 33554431 : 0));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        for (int i_1 = 1; i_1 < 17;i_1 += 1)
        {
            {
                arr_4 [i_0] [i_1] = (max(((min((var_3 != 15), var_8))), (min(var_4, (max(var_4, var_8))))));
                var_14 = (min((((-4564364411931827963 < (min(var_4, (arr_3 [i_1])))))), ((var_0 ? -16 : (var_5 + 1984)))));
            }
        }
    }
    #pragma endscop
}
