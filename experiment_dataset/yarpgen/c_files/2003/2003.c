/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 2003
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=2003 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/2003
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    /* LoopNest 2 */
    for (int i_0 = 3; i_0 < 21;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            {
                /* LoopNest 2 */
                for (int i_2 = 0; i_2 < 1;i_2 += 1)
                {
                    for (int i_3 = 0; i_3 < 22;i_3 += 1)
                    {
                        {
                            arr_9 [i_1] [i_0] [i_2] = (min(((((76 ? var_10 : -25912)) | (arr_8 [i_0] [i_0 + 1] [i_0 + 1] [i_0] [2] [15]))), var_5));
                            arr_10 [i_0] [i_0] [i_0] [i_0] [i_0 + 1] [i_0] = (!var_18);
                            var_19 = (max(32766, (min(-25928, 1867118317))));
                        }
                    }
                }
                arr_11 [i_0] = (arr_2 [i_0]);
                var_20 = (max(var_20, (((!(!-8210))))));
            }
        }
    }
    var_21 -= ((var_11 ? (25928 / 65535) : var_15));
    #pragma endscop
}
