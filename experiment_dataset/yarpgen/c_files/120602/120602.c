/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 120602
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=120602 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/120602
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_18 |= ((-8274958295418183167 ? (max((!1), ((var_5 ? var_4 : var_16)))) : ((~(~var_17)))));
    var_19 = ((var_6 ? var_7 : var_14));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 22;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 22;i_1 += 1)
        {
            for (int i_2 = 3; i_2 < 21;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [1] = (arr_7 [i_0] [i_1] [i_2] [i_2]);
                    var_20 = var_13;
                    var_21 ^= (min((~54815), ((~(((arr_0 [i_1] [i_2]) ? (arr_4 [i_0] [i_1] [i_2]) : (arr_7 [11] [i_0] [i_0] [i_2])))))));
                }
            }
        }
    }
    var_22 = ((!((((min(var_12, var_12)) ? var_1 : (!-72))))));
    var_23 ^= -32763;
    #pragma endscop
}
