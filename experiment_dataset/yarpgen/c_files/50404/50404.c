/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 50404
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=50404 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/50404
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop

    /* vectorizable */
    for (int i_0 = 0; i_0 < 18;i_0 += 1)
    {
        arr_4 [i_0] [i_0] = (((!var_11) ? -113 : 10847));
        arr_5 [4] |= (~-4096);
        var_19 = (max(var_19, ((((arr_3 [1]) ? (arr_3 [16]) : (arr_3 [16]))))));
        arr_6 [i_0] = ((((((arr_2 [i_0] [i_0]) - 23512))) + ((~(arr_3 [i_0])))));
    }
    var_20 = var_5;
    var_21 = (((((-(max(var_16, 1232715266997476892))))) ? (((var_9 ? var_6 : var_15))) : ((var_17 ? var_6 : ((var_11 ? -4096 : 151))))));
    #pragma endscop
}
