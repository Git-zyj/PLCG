/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 170575
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=170575 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/170575
*/
#include "init.h"
#define max(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a > _b ? _a : _b; })
#define min(a,b) \
    ({ __typeof__ (a) _a = (a); \
       __typeof__ (b) _b = (b); \
       _a < _b ? _a : _b; })
void test() {
    var_14 -= ((/* implicit */signed char) var_7);
    var_15 = ((/* implicit */int) var_7);
    /* LoopSeq 1 */
    /* vectorizable */
    for (signed char i_0 = 0; i_0 < 22; i_0 += 4) 
    {
        /* LoopNest 2 */
        for (unsigned int i_1 = 0; i_1 < 22; i_1 += 3) 
        {
            for (unsigned short i_2 = 1; i_2 < 21; i_2 += 2) 
            {
                {
                    /* LoopNest 2 */
                    for (unsigned int i_3 = 0; i_3 < 22; i_3 += 4) 
                    {
                        for (short i_4 = 0; i_4 < 22; i_4 += 1) 
                        {
                            {
                                var_16 = ((((/* implicit */int) var_6)) != (arr_0 [i_2 + 1] [6U]));
                                var_17 = ((/* implicit */long long int) (+(((/* implicit */int) arr_9 [i_2 - 1] [i_3] [i_4] [i_4] [i_4]))));
                            }
                        } 
                    } 
                    var_18 += ((/* implicit */long long int) var_0);
                }
            } 
        } 
        arr_10 [(short)8] [i_0] = ((((/* implicit */_Bool) arr_6 [i_0] [i_0] [i_0])) ? (((/* implicit */int) arr_3 [i_0])) : (((((/* implicit */_Bool) arr_7 [i_0] [i_0])) ? (((/* implicit */int) var_12)) : (((/* implicit */int) arr_8 [i_0] [i_0] [i_0] [i_0] [i_0])))));
    }
}
