/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 95187
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=95187 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/95187
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_12 = (max(var_12, var_6));
    /* LoopNest 2 */
    for (int i_0 = 0; i_0 < 23;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 23;i_1 += 1)
        {
            {
                var_13 *= (~(max(((((arr_4 [i_0] [i_1]) > var_7))), -7551252475981752006)));
                arr_5 [i_0] [i_1] = ((var_5 ? (max(var_1, ((((arr_0 [i_1]) > 321147009))))) : (arr_1 [i_1])));
            }
        }
    }
    var_14 = (max(var_14, ((max(((((((var_8 ? var_9 : 321147009))) ? var_6 : var_0))), (min((max(var_8, var_9)), var_5)))))));
    #pragma endscop
}
