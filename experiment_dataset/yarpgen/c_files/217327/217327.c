/*
yarpgen version 2.0 (build e0f63b6 on 2025:11:04)
Seed: 217327
Invocation: /data/zyj/loop_generator/./baselines/mytest/../yarpgen/scripts/yarpgen --std=c --emit-align-attr=none --inp-as-args=none --emit-pragmas=none --expl-loop-param=true --seed=217327 -o /data2/zyj/Dataset_ISPASS/yarpgen/c_files/217327
*/
#include "init.h"
#define max(x,y)    ((x) > (y)? (x) : (y))
#define min(x,y)    ((x) < (y)? (x) : (y))
void test() {
    #pragma scop
    var_10 = (max(61, ((17 ? (min(139, 1556874098968319533)) : (max(-5556614578937761137, 228))))));
    var_11 = ((37006 ? (-2147483647 - 1) : 7635363969398465003));
    /* LoopNest 3 */
    for (int i_0 = 0; i_0 < 16;i_0 += 1)
    {
        for (int i_1 = 0; i_1 < 16;i_1 += 1)
        {
            for (int i_2 = 0; i_2 < 16;i_2 += 1)
            {
                {
                    arr_8 [i_0] [i_0] [i_1] [i_0] = ((-((-(arr_3 [i_0])))));
                    var_12 = ((-((-(min((arr_1 [i_0] [i_2]), -9223372036854775793))))));
                }
            }
        }
    }
    var_13 = var_2;
    #pragma endscop
}
