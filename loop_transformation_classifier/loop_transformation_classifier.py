...
class loop_transformation_classifiers:
    ...
    def check_fusion_distribution(self, i, check_list, tmp):
        '''
        i为维度,
        check_list为语句簇(共用循环)
        tmp为fusion, distribution判断
        '''
        ...
        if set(self.consts[0, check_list, i]) == set(self.consts[1, check_list, i]) == {0}:
            tmp = self.check_fusion_distribution(i + 1, check_list, tmp)
            if tmp == [1, 1]:
            ...
            if len(co_loop[0]) < len(co_loop[1]):
                ...
                tmp[0] = 1
                # print("fusion")
            elif co_loop[0] != co_loop[1] and set([str(x) for x in co_loop[0]]) != set([str(x) for x in co_loop[1]]):
                # print("fusion + distribution")
                return [1, 1]
                ...
                for group_dst in co_loop[1]:
                    if len(group_src) - 1 and len(group_dst) - 1:
                        ...
                        if len(new_check_list) - 1:
                            ...
                            tmp = self.check_fusion_distribution(i + 1, new_check_list, tmp) 
                            if tmp == [1, 1]:
                                # print("fusion + distribution")
                                return tmp
    ...
    def loop_transformation_analysis(self):
        ...
        for i in range(self.nstmts):
            for j in range(len(self.schedules[1][i])):
                if "zT" in self.schedules[1][i][j]:
                    self.schedules[1][i][j] = '0' # remove tiling dim
                    self.loop_types[1][i][j] = 'scalar'
        ...
        if (self.schedules[0] == self.schedules[1]).all() and self.answer[1] == 0 and self.answer[8] == 0:
            ...
            self.answer[4:6] = self.check_fusion_distribution(0, check_list, tmp) 
...
if __name__ == "__main__":
    ...
    parser_description = '''
    用于pluto-0.11.4所优化代码的循环变换种类判断，循环变换共分为:
    ...
    parser.add_argument("-o", dest="output", help="output of the classification answer", type=str, default='./classification_output.csv')
    ARGS = parser.parse_args()

    # libs = ['numpy', 'tree_sitter', 'argparse', 'pandas']
    ...
    #         # path to store .so file
    #         './tree-sitter-c/build/my-languages.so',
    #         [
    #             './tree-sitter-c', # path of tree-sitter-c
    #         ]
    #     )
