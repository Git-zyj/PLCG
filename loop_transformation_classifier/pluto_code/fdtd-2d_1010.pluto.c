...
for (t1=0;t1<=floord(_PB_TMAX-1,32);t1++) {
  for (t2=t1;t2<=floord(32*t1+_PB_NY+30,32);t2++) {
    for (t3=t1;t3<=min(floord(_PB_TMAX+_PB_NX-2,32),floord(32*t1+_PB_NX+30,32));t3++) {
      if ((t1 == t2) && (t1 == t3)) {
      ...
      if ((t1 == t3) && (t1 <= min(floord(_PB_TMAX-32,32),t2-1))) {
        for (t5=32*t2;t5<=min(32*t2+31,32*t1+_PB_NY+30);t5++) {
          ex[0][(-32*t1+t5-31)] = ex[0][(-32*t1+t5-31)] - SCALAR_VAL(0.5)*(hz[0][(-32*t1+t5-31)]-hz[0][(-32*t1+t5-31)-1]);;
          ey[0][(-32*t1+t5-31)] = _fict_[(32*t1+31)];;
        }
      }
      if ((t1 == t2) && (t1 == t3) && (t1 <= floord(_PB_TMAX-32,32))) {
      ...
      if ((t1 == t2) && (t1 <= min(floord(_PB_TMAX-32,32),t3-1))) {
        for (t6=32*t3;t6<=min(32*t3+31,32*t1+_PB_NX+30);t6++) {
          ey[(-32*t1+t6-31)][0] = ey[(-32*t1+t6-31)][0] - SCALAR_VAL(0.5)*(hz[(-32*t1+t6-31)][0]-hz[(-32*t1+t6-31)-1][0]);;
        }
      }
    }
  }
