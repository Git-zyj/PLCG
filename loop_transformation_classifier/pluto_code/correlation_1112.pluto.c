...
for (t1=0;t1<=_PB_M-1;t1++) {
  if (t1 <= _PB_M-2) {
    for (t3=ceild(t1-30,32);t3<=floord(_PB_M-1,32);t3++) {
      for (t4=max(32*t3,t1+1);t4<=min(_PB_M-1,32*t3+31);t4++) {
        corr[t1][t4] = SCALAR_VAL(0.0);;
      }
    }
  }
  if (t1 <= _PB_M-2) {
    corr[t1][t1] = SCALAR_VAL(1.0);;
  }
  mean[t1] = SCALAR_VAL(0.0);;
  stddev[t1] = SCALAR_VAL(0.0);;
  for (t3=0;t3<=_PB_N-1;t3++) {
    mean[t1] += data[t3][t1];;
  }
  mean[t1] /= float_n;;
  for (t3=0;t3<=_PB_N-1;t3++) {
    stddev[t1] += (data[t3][t1] - mean[t1]) * (data[t3][t1] - mean[t1]);;
    data[t3][t1] -= mean[t1];;
  }
  stddev[t1] /= float_n;;
  stddev[t1] = SQRT_FUN(stddev[t1]);;
  stddev[t1] = stddev[t1] <= eps ? SCALAR_VAL(1.0) : stddev[t1];;
  for (t3=0;t3<=floord(_PB_N-1,32);t3++) {
    for (t4=32*t3;t4<=min(_PB_N-1,32*t3+31);t4++) {
      data[t4][t1] /= SQRT_FUN(float_n) * stddev[t1];;
    }
  }
  for (t3=0;t3<=floord(t1-1,32);t3++) {
    for (t4=0;t4<=floord(_PB_N-1,32);t4++) {
      for (t5=32*t3;t5<=min(t1-1,32*t3+31);t5++) {
        for (t6=32*t4;t6<=min(_PB_N-1,32*t4+31);t6++) {
          corr[t5][t1] += (data[t6][t5] * data[t6][t1]);;
        }
      }
    }
  }
  for (t3=0;t3<=floord(t1-1,32);t3++) {
    for (t4=32*t3;t4<=min(t1-1,32*t3+31);t4++) {
      corr[t1][t4] = corr[t4][t1];;
    }
  }
}
/* End of CLooG code */
