...
for (t1=0;t1<=_PB_TMAX-1;t1++) {
  ey[0][0] = _fict_[t1];;
  for (t3=t1+1;t3<=t1+_PB_NX-1;t3++) {
    ey[(-t1+t3)][0] = ey[(-t1+t3)][0] - SCALAR_VAL(0.5)*(hz[(-t1+t3)][0]-hz[(-t1+t3)-1][0]);;
  }
  for (t2=t1+1;t2<=t1+_PB_NY-1;t2++) {
    ex[0][(-t1+t2)] = ex[0][(-t1+t2)] - SCALAR_VAL(0.5)*(hz[0][(-t1+t2)]-hz[0][(-t1+t2)-1]);;
    ey[0][(-t1+t2)] = _fict_[t1];;
    for (t3=t1+1;t3<=t1+_PB_NX-1;t3++) {
      ey[(-t1+t3)][(-t1+t2)] = ey[(-t1+t3)][(-t1+t2)] - SCALAR_VAL(0.5)*(hz[(-t1+t3)][(-t1+t2)]-hz[(-t1+t3)-1][(-t1+t2)]);;
      ex[(-t1+t3)][(-t1+t2)] = ex[(-t1+t3)][(-t1+t2)] - SCALAR_VAL(0.5)*(hz[(-t1+t3)][(-t1+t2)]-hz[(-t1+t3)][(-t1+t2)-1]);;
      hz[(-t1+t3-1)][(-t1+t2-1)] = hz[(-t1+t3-1)][(-t1+t2-1)] - SCALAR_VAL(0.7)* (ex[(-t1+t3-1)][(-t1+t2-1)+1] - ex[(-t1+t3-1)][(-t1+t2-1)] + ey[(-t1+t3-1)+1][(-t1+t2-1)] - ey[(-t1+t3-1)][(-t1+t2-1)]);;
    }
  }
}
/* End of CLooG code */
