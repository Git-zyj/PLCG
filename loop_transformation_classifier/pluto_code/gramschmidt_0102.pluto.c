...
/* Start of CLooG code */
for (t1=0;t1<=_PB_N-2;t1++) {
  for (t3=t1+1;t3<=_PB_N-1;t3++) {
    R[t1][t3] = SCALAR_VAL(0.0);;
  }
  nrm = SCALAR_VAL(0.0);;
  for (t3=0;t3<=_PB_M-1;t3++) {
    nrm += A[t3][t1] * A[t3][t1];;
  }
  R[t1][t1] = SQRT_FUN(nrm);;
  for (t3=0;t3<=_PB_M-1;t3++) {
    Q[t3][t1] = A[t3][t1] / R[t1][t1];;
  }
  for (t3=t1+1;t3<=_PB_N-1;t3++) {
    for (t5=0;t5<=_PB_M-1;t5++) {
      R[t1][t3] += Q[t5][t1] * A[t5][t3];;
    }
    for (t5=0;t5<=_PB_M-1;t5++) {
      A[t5][t3] = A[t5][t3] - Q[t5][t1] * R[t1][t3];;
    }
  }
}
nrm = SCALAR_VAL(0.0);;
for (t3=0;t3<=_PB_M-1;t3++) {
  nrm += A[t3][(_PB_N-1)] * A[t3][(_PB_N-1)];;
}
R[(_PB_N-1)][(_PB_N-1)] = SQRT_FUN(nrm);;
for (t3=0;t3<=_PB_M-1;t3++) {
  Q[t3][(_PB_N-1)] = A[t3][(_PB_N-1)] / R[(_PB_N-1)][(_PB_N-1)];;
}
/* End of CLooG code */
