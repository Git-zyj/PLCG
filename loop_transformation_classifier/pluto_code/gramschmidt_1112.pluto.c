...
for (t1=0;t1<=_PB_N-1;t1++) {
  if (t1 <= _PB_N-2) {
    for (t3=ceild(t1-30,32);t3<=floord(_PB_N-1,32);t3++) {
      for (t4=max(32*t3,t1+1);t4<=min(_PB_N-1,32*t3+31);t4++) {
        R[t1][t4] = SCALAR_VAL(0.0);;
      }
    }
  }
  nrm = SCALAR_VAL(0.0);;
  ...
  if (t1 <= _PB_N-2) {
    for (t3=ceild(t1-30,32);t3<=floord(_PB_N-1,32);t3++) {
      for (t5=0;t5<=floord(_PB_M-1,32);t5++) {
        for (t6=max(32*t3,t1+1);t6<=min(_PB_N-1,32*t3+31);t6++) {
          for (t8=32*t5;t8<=min(_PB_M-1,32*t5+31);t8++) {
            R[t1][t6] += Q[t8][t1] * A[t8][t6];;
          }
        }
      }
      for (t5=0;t5<=floord(_PB_M-1,32);t5++) {
        for (t6=max(32*t3,t1+1);t6<=min(_PB_N-1,32*t3+31);t6++) {
          for (t8=32*t5;t8<=min(_PB_M-1,32*t5+31);t8++) {
            A[t8][t6] = A[t8][t6] - Q[t8][t1] * R[t1][t6];;
          }
        }
      }
    }
