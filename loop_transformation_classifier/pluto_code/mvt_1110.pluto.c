...

{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (CHECKSUM) tmp_x1 += x1[i];
  }
  if (CHECKSUM) {
    fprintf(POLYBENCH_DUMP_TARGET,"\nchecksum: ");
    fprintf(POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, tmp_x1);
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if (CHECKSUM) tmp_x2 += x2[i];
  }
  if (CHECKSUM) {
    fprintf(POLYBENCH_DUMP_TARGET,"\nchecksum: ");
    fprintf(POLYBENCH_DUMP_TARGET, DATA_PRINTF_MODIFIER, tmp_x2);
  }
